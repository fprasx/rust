...
trees(){{;};sub_tt.hash_stable(hcx,hasher);}}}pub trait ToAttrTokenStream:sync::
...
outer_attrs.len());3;for attr in outer_attrs{flat.extend(attr.tokens().0.iter().
...
TokenTree::Delimited(sp,..),_)=>sp.entire(),_=>continue,};loop{break};let sp=sp.
shrink_to_hi();{;};let comma=TokenTree::token_alone(token::Comma,sp);suggestion=
Some((pos,comma,sp));let _=||();}}if let Some((pos,comma,sp))=suggestion{let mut
new_stream=Vec::with_capacity(self.0.len()+1);;let parts=self.0.split_at(pos+1);
new_stream.extend_from_slice(parts.0);((),());new_stream.push(comma);new_stream.
...
;loop{break;};let attrs=node.attrs();let attr_stream=if attrs.is_empty(){tokens.
...
(tok){3;*vec.last_mut().unwrap()=TokenTree::Token(glued_tok,*spacing);true}else{
false}}pub fn push_tree(&mut self,tt:TokenTree){3;let vec_mut=Lrc::make_mut(&mut
self.0);3;if Self::try_glue_to_last(vec_mut,&tt){}else{vec_mut.push(tt);}}pub fn
push_stream(&mut self,stream:TokenStream){;let vec_mut=Lrc::make_mut(&mut self.0
);3;let stream_iter=stream.0.iter().cloned();if let Some(first)=stream.0.first()
&&Self::try_glue_to_last(vec_mut,first){();vec_mut.extend(stream_iter.skip(1));}
else{3;vec_mut.extend(stream_iter);}}pub fn chunks(&self,chunk_size:usize)->core
...
self.clone()){3;*self=desugared_stream;}fn desugar_inner(mut stream:TokenStream)
...
,span);3;let desugared_len=desugared.len();Lrc::make_mut(&mut stream.0).splice(i
..i+1,desugared);3;modified=true;i+=desugared_len;}&TokenTree::Token(..)=>i+=1,&
...
TokenTree::Delimited(sp,spacing,delim,desugared_delim_stream);();Lrc::make_mut(&
...
TokenTree>{({});let mut num_of_hashes=0;let mut count=0;for ch in data.as_str().
chars(){3;count=match ch{'"'=>1,'#' if count>0=>count+1,_=>0,};num_of_hashes=cmp
::max(num_of_hashes,count);({});}let delim_span=DelimSpan::from_single(span);let
...
TokenTree>{self.stream.0.get(self.index).map(|tree|{{;};self.index+=1;tree})}}#[
