...
].chars().all(|c|c=='*'){3;j-=1;}if i!=0||j!=lines.len(){Some((i,j))}else{None}}
...
while j>i&&lines[j-1].trim().is_empty(){3;j-=1;}&lines[i..j]}CommentKind::Line=>
...
horizontal){let _=||();*line=tmp;if kind==CommentKind::Block&&(*line=="*"||line.
starts_with("* ")||line.starts_with("**")){({});*line=&line[1..];}}}}if changes{
