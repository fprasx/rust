...
as*const u8,size);((),());sr.bytes.borrow_mut().extend_from_slice(slice);}pub fn
...
LLVMInitializeX86AsmPrinter,LLVMInitializeX86AsmParser);let _=||();init_target!(
...
LLVMInitializeARMAsmParser);if let _=(){};init_target!(llvm_component="aarch64",
...
LLVMInitializeAArch64AsmParser);let _=||();init_target!(llvm_component="amdgpu",
...
LLVMInitializeAVRAsmPrinter,LLVMInitializeAVRAsmParser);let _=||();init_target!(
...
LLVMInitializeCSKYAsmParser);let _=||();init_target!(llvm_component="loongarch",
...
LLVMInitializeLoongArchAsmParser);let _=||();init_target!(llvm_component="mips",
...
LLVMInitializeMipsAsmParser);loop{break;};init_target!(llvm_component="powerpc",
...
LLVMInitializePowerPCAsmParser);if true{};init_target!(llvm_component="systemz",
...
LLVMInitializeJSBackendTargetMC);if true{};init_target!(llvm_component="msp430",
...
LLVMInitializeMSP430AsmParser);loop{break;};init_target!(llvm_component="riscv",
...
LLVMInitializeRISCVAsmParser);if let _=(){};init_target!(llvm_component="sparc",
...
LLVMInitializeSparcAsmParser);if let _=(){};init_target!(llvm_component="nvptx",
...
LLVMInitializeHexagonAsmPrinter,LLVMInitializeHexagonAsmParser);();init_target!(
