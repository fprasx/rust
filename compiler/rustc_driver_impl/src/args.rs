...
self.next_is_unstable_option{let _=||();self.inspect_unstable_option(&arg);self.
...
true;3;}else{self.inspect_unstable_option(unstable_option);}}self.expanded.push(
...
into_string(){Ok(arg)=>{if let Ok(args)=&mut res{();args.push(arg);}}Err(arg)=>{
