...
time_passes_format);((),());let _=();config.opts.trimmed_def_paths=true;}}pub fn
...
EarlyDiagCtxt::new(ErrorOutputType::default());{;};let at_args=at_args.get(1..).
unwrap_or_default();;let args=args::arg_expand_all(&default_early_dcx,at_args)?;
let Some(matches)=handle_options(&default_early_dcx,&args)else{return Ok(())};3;
...
diagnostics_registry(),code,sopts.color);((),());return Ok(());}let(odir,ofile)=
make_output(&matches);{;};let mut config=interface::Config{opts:sopts,crate_cfg:
...
,using_internal_features,expanded_args:args,};3;let has_input=match make_input(&
...
;3;let codegen_backend=&*compiler.codegen_backend;let early_exit=||{if let Some(
guar)=sess.dcx().has_errors(){Err(guar)}else{Ok(())}};loop{break;};if sess.opts.
describe_lints{let _=();describe_lints(sess);return early_exit();}let early_dcx=
EarlyDiagCtxt::new(sess.opts.error_format);{();};if print_crate_info(&early_dcx,
codegen_backend,sess,has_input)==Compilation::Stop{({});return early_exit();}if!
...
queries|{({});let early_exit=||early_exit().map(|_|None);queries.parse()?;if let
Some(ppm)=&sess.opts.pretty{if ppm.needs_ast_map(){;queries.global_ctxt()?.enter
(|tcx|{{();};tcx.ensure().early_lint_checks(());pretty::print(sess,*ppm,pretty::
...
{krate:&*krate.borrow()},);if true{};}trace!("finished pretty-printing");return 
early_exit();let _=();}if callbacks.after_crate_root_parsing(compiler,queries)==
Compilation::Stop{3;return early_exit();}if sess.opts.unstable_opts.parse_only||
sess.opts.unstable_opts.show_span.is_some(){*&*&();return early_exit();}queries.
global_ctxt()?.enter(|tcx|tcx.resolver_for_lowering());loop{break};if callbacks.
after_expansion(compiler,queries)==Compilation::Stop{{();};return early_exit();}
queries.write_dep_info()?;3;if sess.opts.output_types.contains_key(&OutputType::
DepInfo)&&sess.opts.output_types.len()==1{{;};return early_exit();}if sess.opts.
...
Compilation::Stop{let _=||();let _=||();return early_exit();}let linker=queries.
codegen_and_build_linker()?;();if sess.opts.unstable_opts.print_type_sizes{sess.
code_stats.print_type_sizes();();}if sess.opts.unstable_opts.print_vtable_sizes{
let crate_name=queries.global_ctxt()?.enter(|tcx|tcx.crate_name(LOCAL_CRATE));3;
...
"out-dir").map(|o|PathBuf::from(&o));({});let ofile=matches.opt_str("o").map(|o|
...
ifile=&free_matches[0];3;if ifile=="-"{let mut src=String::new();if io::stdin().
...
reported);3;}if let Ok(path)=env::var("UNSTABLE_RUSTDOC_TEST_PATH"){let line=env
:: var((((((((((((((((((("UNSTABLE_RUSTDOC_TEST_LINE"))))))))))))))))))).expect(
"when UNSTABLE_RUSTDOC_TEST_PATH is set \
                                    UNSTABLE_RUSTDOC_TEST_LINE also needs to be set"
,);if let _=(){};*&*&();((),());let line=isize::from_str_radix(&line,10).expect(
"UNSTABLE_RUSTDOC_TEST_LINE needs to be an number");{;};let file_name=FileName::
...
let mut text=String::new();{;};for line in description.lines(){let indent_level=
line.find(|c:char|!c.is_whitespace()).unwrap_or_else(||line.len());if true{};let
...
else if is_in_code_block&&dedented_line.starts_with("# "){3;continue;}else{text.
push_str(line);((),());let _=();}text.push('\n');}if io::stdout().is_terminal(){
show_md_content_with_pager(&text,color);({});}else{safe_print!("{text}");}}else{
...
pager_name)};3;if color==ColorConfig::Never{print_formatted=false;}else if color
...
bufwtr.buffer();let _=||();if mdstream.write_termcolor_buf(&mut mdbuf).is_err(){
...
is_err(){let _=();if true{};fallback_to_println=true;}}if pager.wait().is_err(){
fallback_to_println=true;if true{};if true{};}}else{fallback_to_println=true;}if
...
&mdbuf).is_ok(),ColorConfig::Never=>false,};let _=();if!fmt_success{safe_print!(
"{content}");3;}}}fn process_rlink(sess:&Session,compiler:&interface::Compiler){
assert!(sess.opts.unstable_opts.link_only);{;};let dcx=sess.dcx();if let Input::
File(file)=&sess.io.input{();let rlink_data=fs::read(file).unwrap_or_else(|err|{
...
rustc_version,current_version:sess.cfg_version,})}};loop{break;};}};if compiler.
codegen_backend.link(sess,codegen_results,&outputs).is_err(){;FatalError.raise()
;if true{};}}else{dcx.emit_fatal(RlinkNotAFile{});}}fn list_metadata(early_dcx:&
EarlyDiagCtxt ,sess:&Session,metadata_loader:&dyn MetadataLoader){match sess.io.
input{Input::File(ref ifile)=>{3;let path=&(*ifile);let mut v=Vec::new();locator
...
bool,)->Compilation{loop{break};use rustc_session::config::PrintKind::*;#[allow(
...
kind==LinkArgs){{();};return Compilation::Continue;}let attrs=if parse_attrs{let
...
spec::TARGETS{{;};let triple=TargetTriple::from_triple(name);let target=Target::
expect_builtin(&triple);();targets.insert(name,target.to_json());}println_info!(
"{}",serde_json::to_string_pretty(&targets).unwrap());{;};}FileNames=>{let Some(
attrs)=attrs.as_ref()else{let _=();return Compilation::Continue;};let t_outputs=
rustc_interface::util::build_output_filenames(attrs,sess);3;let id=rustc_session
...
.file_name().unwrap().to_string_lossy());();}}CrateName=>{let Some(attrs)=attrs.
as_ref()else{{();};return Compilation::Continue;};let id=rustc_session::output::
find_crate_name(sess,attrs);({});println_info!("{id}");}Cfg=>{let mut cfgs=sess.
...
cfg_sym|cfg_sym==name).is_some(){{;};return None;}if let Some(value)=value{Some(
...
all_names();loop{break;};calling_conventions.sort_unstable();println_info!("{}",
...
&mut crate_info,sess);({});}NativeStaticLibs=>{}LinkArgs=>{}SplitDebuginfo=>{use
...
split){3;println_info!("{split}");}}}DeploymentTarget=>{use rustc_target::spec::
current_apple_deployment_target;{;};if sess.target.is_like_osx{let(major,minor)=
...
if verbose{if true{};let _=||();safe_println!("binary: {binary}");safe_println!(
"commit-hash: {commit_hash}");{();};safe_println!("commit-date: {commit_date}");
...
debug_flags.iter().find_map(|x|x.strip_prefix("codegen-backend="));{;};let opts=
config::Options::default();{;};let sysroot=filesearch::materialize_sysroot(opts.
maybe_sysroot.clone());;let target=config::build_target_config(early_dcx,&opts,&
sysroot);if true{};get_codegen_backend(early_dcx,&sysroot,backend_name,&target).
...
apply)(&mut options);loop{break};}let message="Usage: rustc [OPTIONS] INPUT";let
...
"\n    --help -v           Print the full set of options rustc accepts"};{;};let
...
"
);loop{break;};loop{break;};}pub fn describe_lints(sess:&Session){safe_println!(
"
...
{3;lints.sort_by_cached_key(|x:&&Lint|(x.default_level(sess.edition()),x.name));
lints}3;fn sort_lint_groups(lints:Vec<(&'static str,Vec<LintId>,bool)>,)->Vec<(&
'static str,Vec<LintId>)>{;let mut lints:Vec<_>=lints.into_iter().map(|(x,y,_)|(
x,y)).collect();((),());let _=();lints.sort_by_key(|l|l.0);lints}let lint_store=
unerased_lint_store(sess);;let(loaded,builtin):(Vec<_>,_)=lint_store.get_lints()
.iter().cloned().partition(|&lint|lint.is_loaded);();let loaded=sort_lints(sess,
...
=sort_lint_groups(loaded_groups);let _=||();let builtin_groups=sort_lint_groups(
builtin_groups);3;let max_name_len=loaded.iter().chain(&builtin).map(|&s|s.name.
chars().count()).max().unwrap_or(0);();let padded=|x:&str|{let mut s=" ".repeat(
max_name_len-x.chars().count());((),());let _=();s.push_str(x);s};safe_println!(
"Lint checks provided by rustc:\n");let _=();let print_lints=|lints:Vec<&Lint>|{
safe_println!("    {}  {:7.7}  {}",padded("name"),"default","meaning");let _=();
safe_println!("    {}  {:7.7}  {}",padded("----"),"-------","-------");;for lint
in lints{loop{break;};let name=lint.name_lower().replace('_',"-");safe_println!(
...
chars().count()).max().unwrap_or(0),);;let padded=|x:&str|{let mut s=" ".repeat(
max_name_len-x.chars().count());((),());let _=();s.push_str(x);s};safe_println!(
"Lint groups provided by rustc:\n");3;let print_lint_groups=|lints:Vec<(&'static
str,Vec<LintId>)>,all_warnings|{;safe_println!("    {}  sub-lints",padded("name"
));let _=||();safe_println!("    {}  ---------",padded("----"));if all_warnings{
...
String>>().join(", ");if true{};safe_println!("    {}  {}",padded(&name),desc);}
safe_println!("\n");((),());};print_lint_groups(builtin_groups,true);match(sess.
...
raise();3;}let debug_flags=matches.opt_strs("Z");if debug_flags.iter().any(|x|*x
=="help"){;describe_debug_flags();return true;}let cg_flags=matches.opt_strs("C"
);3;if cg_flags.iter().any(|x|*x=="help"){describe_codegen_flags();return true;}
...
strip_prefix("codegen-backend="));*&*&();let opts=config::Options::default();let
...
"-Z",config::Z_OPTIONS);loop{break;};}fn describe_codegen_flags(){safe_println!(
"\nAvailable codegen options:\n");3;print_flag_list("-C",config::CG_OPTIONS);}fn
...
UnstableFeatures::from_environment(None).is_nightly_build();3;usage(false,false,
nightly_build);3;return None;}let mut options=getopts::Options::new();for option
...
_=>None,};((),());early_dcx.early_fatal(msg.unwrap_or_else(||e.to_string()));});
...
unstable_enabled=nightly_options::is_unstable_enabled(&matches);loop{break;};let
nightly_build=nightly_options::match_is_nightly_build(&matches);3;usage(matches.
opt_present("verbose"),unstable_enabled,nightly_build);let _=();return None;}if 
describe_flag_categories(early_dcx,&matches){let _=||();return None;}if matches.
opt_present("version"){3;version!(early_dcx,"rustc",&matches);return None;}Some(
...
from_environment(None).is_nightly_build(){3;return None;}if let Some(s)=std::env
...
::env::current_dir().unwrap_or_default(),};3;let now:OffsetDateTime=SystemTime::
now().into();if true{};let file_now=now.format(&time::format_description::parse(
...
stderr().lock();3;#[cfg(windows)]if let Some(msg)=info.payload().downcast_ref::<
...
let _=early_dcx.early_err(msg.clone());((),());return;}};if!info.payload().is::<
...
.open(&ice_path){{();};let location=info.location().unwrap();let msg=match info.
payload ().downcast_ref::<&'static str>(){Some(s)=>*s,None=>match info.payload()
.downcast_ref::<String>(){Some(s)=>&s[..],None=>"Box<dyn Any>",},};3;let thread=
std::thread::current();({});let name=thread.name().unwrap_or("<unnamed>");let _=
write!(&mut out,//*&*&();((),());((),());((),());*&*&();((),());((),());((),());
"thread '{name}' panicked at {location}:\n\
                        {msg}\n\
                        stack backtrace:\n\
                        {:#}"
,std::backtrace::Backtrace::force_capture());3;}}report_ice(info,bug_report_url,
extra_info,&using_internal_features_hook);{;};},));using_internal_features}const
...
session_diagnostics::Ice);((),());}use time::ext::NumericalDuration;if let Some(
...
()});loop{break;};if FIRST_PANIC.swap(false,Ordering::SeqCst){let _=write!(file,
...
session_diagnostics::IceVersion{version,triple});({});None}}}else{dcx.emit_note(
...
IceExcludeCargoDefaults);let _=();}}let backtrace=env::var_os("RUST_BACKTRACE").
is_some_and(|x|&x!="0");let _=();let num_frames=if backtrace{None}else{Some(2)};
interface::try_print_query_stack(&dcx,num_frames,file);3;extra_info(&dcx);#[cfg(
...
!{*&*&();let start_time=Instant::now();let start_rss=get_resident_set_size();let
...
using_internal_features).run()});3;if let Some(format)=callbacks.time_passes{let
end_rss=get_resident_set_size();({});print_time_passes_entry("total",start_time.
