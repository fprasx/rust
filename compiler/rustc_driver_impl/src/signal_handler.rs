...
256;({});static mut STACK_TRACE:[*mut libc::c_void;MAX_FRAMES]=[ptr::null_mut();
...
]};3;raw_errln!("error: rustc interrupted by SIGSEGV, printing backtrace\n");let
...
).position(cycled){3;let period=period.saturating_add(1);let Some(offset)=stack.
...
"\n### cycle encountered after {offset} frames with period {period}");if true{};
...
written+=rem.len()+1;loop{break};let random_depth=||8*16;if cyclic||stack.len()>
...
"note: we would appreciate a report at https://github.com/rust-lang/rust");3;let
new_size=STACK_SIZE.get().copied().unwrap_or(DEFAULT_STACK_SIZE)*2;3;raw_errln!(
"help: you can increase rustc's stack size by setting RUST_MIN_STACK={new_size}"
);if true{};let _=||();if true{};let _=||();written+=2;if written>24{raw_errln!(
...
alt_stack:libc::stack_t=mem::zeroed();loop{break};alt_stack.ss_sp=alloc(Layout::
from_size_align(alt_stack_size,1).unwrap()).cast();let _=||();alt_stack.ss_size=
alt_stack_size;3;libc::sigaltstack(&alt_stack,ptr::null_mut());let mut sa:libc::
sigaction=mem::zeroed();;sa.sa_sigaction=print_stack_trace as libc::sighandler_t
;((),());sa.sa_flags=libc::SA_NODEFER|libc::SA_RESETHAND|libc::SA_ONSTACK;libc::
