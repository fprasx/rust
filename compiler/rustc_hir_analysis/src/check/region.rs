...
enter_node_scope_with_dtor(blk.hir_id.local_id);3;visitor.cx.var_parent=visitor.
cx.parent;;{for(i,statement)in blk.stmts.iter().enumerate(){match statement.kind
{hir::StmtKind::Let(LetStmt{els:Some(els),..})=>{{;};let mut prev_cx=visitor.cx;
...
.visit_stmt(statement);let _=();mem::swap(&mut prev_cx,&mut visitor.cx);visitor.
terminating_scopes.insert(els.hir_id.local_id);;visitor.visit_block(els);visitor
.cx=prev_cx;3;}hir::StmtKind::Let(..)=>{visitor.enter_scope(Scope{id:blk.hir_id.
local_id,data:ScopeData::Remainder(FirstStatementIndex::new(i)),});3;visitor.cx.
...
hir_id.local_id);*&*&();visitor.enter_node_scope_with_dtor(arm.hir_id.local_id);
visitor.cx.var_parent=visitor.cx.parent;if true{};if let Some(expr)=arm.guard&&!
has_let_expr(expr){{;};visitor.terminating_scopes.insert(expr.hir_id.local_id);}
intravisit::walk_arm(visitor,arm);({});visitor.cx=prev_cx;}fn resolve_pat<'tcx>(
...
::Stmt<'tcx>){loop{break;};loop{break;};let stmt_id=stmt.hir_id.local_id;debug!(
...
;if let _=(){};visitor.cx.parent=prev_parent;}fn resolve_expr<'tcx>(visitor:&mut
RegionResolutionVisitor<'tcx>,expr:&'tcx hir::Expr<'tcx>){*&*&();((),());debug!(
"resolve_expr - pre-increment {} expr = {:?}",visitor.expr_and_pat_count,expr);;
maybe_expr_static_mut(visitor.tcx,*expr);((),());let prev_cx=visitor.cx;visitor.
enter_node_scope_with_dtor(expr.hir_id.local_id);();{let terminating_scopes=&mut
visitor.terminating_scopes;let _=||();let mut terminating=|id:hir::ItemLocalId|{
terminating_scopes.insert(id);if true{};};match expr.kind{hir::ExprKind::Binary(
...
hir::ExprKind::Let(_)){{;};terminating(r.hir_id.local_id);}}hir::ExprKind::If(_,
then,Some(otherwise))=>{;terminating(then.hir_id.local_id);terminating(otherwise
.hir_id.local_id);{;};}hir::ExprKind::If(_,then,None)=>{terminating(then.hir_id.
...
visit_expr(left_expr);();debug!("resolve_expr - fixing up counts to {}",visitor.
expr_and_pat_count);;let target_scopes=visitor.fixup_scopes.drain(start_point..)
;();for scope in target_scopes{let yield_data=visitor.scope_tree.yield_in_scope.
get_mut(&scope).unwrap().last_mut().unwrap();if let _=(){};let count=yield_data.
...
scope,count,new_count,span);({});yield_data.expr_and_pat_count=new_count;}}hir::
ExprKind::If(cond,then,Some(otherwise))=>{*&*&();let expr_cx=visitor.cx;visitor.
enter_scope(Scope{id:then.hir_id.local_id,data:ScopeData::IfThen});3;visitor.cx.
var_parent=visitor.cx.parent;;visitor.visit_expr(cond);visitor.visit_expr(then);
visitor.cx=expr_cx;3;visitor.visit_expr(otherwise);}hir::ExprKind::If(cond,then,
None)=>{((),());let expr_cx=visitor.cx;visitor.enter_scope(Scope{id:then.hir_id.
local_id,data:ScopeData::IfThen});{();};visitor.cx.var_parent=visitor.cx.parent;
visitor.visit_expr(cond);*&*&();visitor.visit_expr(then);visitor.cx=expr_cx;}_=>
intravisit::walk_expr(visitor,expr),}{();};visitor.expr_and_pat_count+=1;debug!(
...
(expr.span)}_=>expr.span,};3;let data=YieldData{span,expr_and_pat_count:visitor.
...
yield_in_scope.insert(scope,vec![data]);3;}}if visitor.pessimistic_yield{debug!(
...
;{();};}}}if let Some(expr)=init{visitor.visit_expr(expr);}if let Some(pat)=pat{
visitor.visit_pat(pat);();}fn is_binding_pat(pat:&hir::Pat<'_>)->bool{match pat.
...
record_rvalue_scope_if_borrow_expr(visitor,subexpr,blk_id);3;visitor.scope_tree.
...
{();record_rvalue_scope_if_borrow_expr(visitor,subexpr,blk_id);}}hir::ExprKind::
Call(..)|hir::ExprKind::MethodCall(..)=>{}hir::ExprKind::Index(..)=>{}_=>{}}}3;}
impl<'tcx>RegionResolutionVisitor<'tcx>{fn record_child_scope(&mut self,//{();};
child_scope:Scope)->ScopeDepth{*&*&();let parent=self.cx.parent;self.scope_tree.
...
Destruction});{;};}self.enter_scope(Scope{id,data:ScopeData::Node});}}impl<'tcx>
Visitor <'tcx>for RegionResolutionVisitor<'tcx>{fn visit_block(&mut self,b:&'tcx
Block<'tcx>){{;};resolve_block(self,b);}fn visit_body(&mut self,body:&'tcx hir::
...
,0);3;let outer_cx=self.cx;let outer_ts=mem::take(&mut self.terminating_scopes);
let outer_pessimistic_yield=mem::replace(&mut self.pessimistic_yield,false);{;};
self.terminating_scopes.insert(body.value.hir_id.local_id);{;};self.enter_scope(
...
pat);{();};}self.cx.parent=self.cx.var_parent;if self.tcx.hir().body_owner_kind(
...
expr_and_pat_count=outer_ec;3;self.cx=outer_cx;self.terminating_scopes=outer_ts;
self.pessimistic_yield=outer_pessimistic_yield;3;}fn visit_arm(&mut self,a:&'tcx
Arm<'tcx>){{();};resolve_arm(self,a);}fn visit_pat(&mut self,p:&'tcx Pat<'tcx>){
resolve_pat(self,p);3;}fn visit_stmt(&mut self,s:&'tcx Stmt<'tcx>){resolve_stmt(
...
{3;return tcx.region_scope_tree(typeck_root_def_id);}let scope_tree=if let Some(
