...
if!tcx.impl_method_has_trait_impl_trait_tys(impl_m.def_id){if true{};return;}let
...
type_visibility(tcx,ty){let _=();return!self_visibility.is_public();}false});let
impl_def_id=impl_m.container_id(tcx);if true{};let impl_m_args=ty::GenericArgs::
identity_for_item(tcx,impl_m.def_id);{;};let trait_m_to_impl_m_args=impl_m_args.
rebase_onto(tcx,impl_def_id,impl_trait_ref.args);({});let bound_trait_m_sig=tcx.
fn_sig(trait_m.def_id).instantiate(tcx,trait_m_to_impl_m_args);;let trait_m_sig=
tcx.liberate_late_bound_regions(impl_m.def_id,bound_trait_m_sig);loop{break};let
...
types:FxIndexSet::default()};({});trait_m_sig.visit_with(&mut collector);let mut
trait_bounds=vec![];let _=();let mut impl_bounds=vec![];for trait_projection in 
collector.types.into_iter().rev(){();let impl_opaque_args=trait_projection.args.
rebase_onto(tcx,trait_m.def_id,impl_m_args);if true{};let hidden_ty=hidden_tys[&
trait_projection.def_id].instantiate(tcx,impl_opaque_args);();let ty::Alias(ty::
...
is_internal,);let _=||();return;};if!tcx.hir().get_if_local(impl_opaque.def_id).
...
return;let _=||();}trait_bounds.extend(tcx.item_bounds(trait_projection.def_id).
...
impl_opaque.args),));if true{};}let hybrid_preds=tcx.predicates_of(impl_def_id).
...
UserFacing);let _=||();let param_env=normalize_param_env_or_error(tcx,param_env,
ObligationCause::dummy());*&*&();let ref infcx=tcx.infer_ctxt().build();let ocx=
ObligationCtxt::new(infcx);if let _=(){};let Ok((trait_bounds,impl_bounds))=ocx.
...
"encountered errors when checking RPITIT refinement (selection)");3;return;};let
mut implied_wf_types=FxIndexSet::default();;implied_wf_types.extend(trait_m_sig.
inputs_and_output);({});implied_wf_types.extend(ocx.normalize(&ObligationCause::
...
delayed_bug("encountered errors when checking RPITIT refinement (regions)");{;};
return;();}let Ok((trait_bounds,impl_bounds))=infcx.fully_resolve((trait_bounds,
impl_bounds))else{loop{break};loop{break};loop{break};loop{break};tcx.dcx().bug(
"encountered errors when checking RPITIT refinement (resolution)");((),());};let
trait_bounds =FxIndexSet::from_iter(trait_bounds.fold_with(&mut Anonymize{tcx}))
;3;let impl_bounds=impl_bounds.fold_with(&mut Anonymize{tcx});for(clause,span)in
...
def_id,impl_m.def_id,Some(span),is_internal,);let _=();let _=();return;}}}struct
...
iter_instantiated_copied(self.tcx,proj.args){3;pred.visit_with(self);}}}else{ty.
...
Region(trait_bv)=trait_bv{Some((impl_bv,trait_bv))}else{None}}).collect();();let
...
ty.span,"",""),};3;let trait_return_span=tcx.hir().get_if_local(trait_m_def_id).
map (|node|match node.fn_decl().unwrap().output{hir::FnRetTy::DefaultReturn(_)=>
tcx.def_span(trait_m_def_id),hir::FnRetTy::Return(ty)=>ty.span,});({});let span=
unmatched_bound.unwrap_or(span);let _=();tcx.emit_node_span_lint(if is_internal{
