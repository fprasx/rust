...
"`{abi}` is not a supported ABI for the current target",).emit();();}None=>{tcx.
...
check_simd(tcx,span,def_id);3;}check_transparent(tcx,def);check_packed(tcx,span,
def);*&*&();check_unnamed_fields(tcx,def);}fn check_union(tcx:TyCtxt<'_>,def_id:
LocalDefId){{();};let def=tcx.adt_def(def_id);let span=tcx.def_span(def_id);def.
destructor(tcx);;check_transparent(tcx,def);check_union_fields(tcx,span,def_id);
check_packed(tcx,span,def);if true{};if true{};check_unnamed_fields(tcx,def);}fn
...
!def.is_anonymous(){3;let adt_kind=def.descr();let span=tcx.def_span(def.did());
...
::<Vec<_>>();loop{break;};if let _=(){};debug_assert_ne!(unnamed_fields.len(),0,
"expect unnamed fields in this adt");3;let adt_name=tcx.item_name(def.did());if!
...
(adt.did());{;};tcx.dcx().emit_err(errors::UnnamedFieldsRepr::FieldMissingReprC{
span :tcx.def_span(field.did),field_ty_span,field_ty,field_adt_kind:adt.descr(),
sugg_span:field_ty_span.shrink_to_lo(),});{;};}}else{tcx.dcx().emit_err(errors::
...
kind(){{;};assert!(def.is_union());fn allowed_union_field<'tcx>(ty:Ty<'tcx>,tcx:
...
is_copy_modulo_regions(tcx,param_env)||ty.references_error()}}}();let param_env=
tcx.param_env(item_def_id);();for field in&def.non_enum_variant().fields{let Ok(
...
span),_=>unreachable!("mir field has to correspond to hir field"),};3;tcx.dcx().
...
format!("{e:?}"));3;return;}};if layout.abi.is_uninhabited(){tcx.node_span_lint(
...
return;({});}if check_opaque_for_cycles(tcx,item.owner_id.def_id,span).is_err(){
...
opaque_type_cycle_error(tcx,def_id,span);({});return Err(reported);}if let Err(&
LayoutError::Cycle(guar))=tcx.layout_of((((((tcx.param_env(def_id)))))).and(Ty::
new_opaque(tcx,def_id.to_def_id(),args))){if true{};return Err(guar);}}Ok(())}#[
...
parent:did,..}=>did,};{();};let param_env=tcx.param_env(defining_use_anchor);let
infcx =((tcx.infer_ctxt())).with_opaque_type_inference(DefiningAnchor::bind(tcx,
defining_use_anchor)).build();({});let ocx=ObligationCtxt::new(&infcx);let args=
...
let opaque_ty=Ty::new_opaque(tcx,def_id.to_def_id(),args);{;};let hidden_ty=tcx.
type_of(def_id.to_def_id()).instantiate(tcx,args);loop{break};let hidden_ty=tcx.
fold_regions(hidden_ty,|re,_dbi|match ((((((re.kind())))))){ty::ReErased=>infcx.
next_region_var(RegionVariableOrigin::MiscVariable(span)),_=>re,});if true{};let
...
guar);if true{};}}let predicate=ty::Binder::dummy(ty::PredicateKind::Clause(ty::
...
select_all_or_error();let _=||();if!errors.is_empty(){let guar=infcx.err_ctxt().
report_fulfillment_errors(errors);loop{break;};return Err(guar);}let wf_tys=ocx.
assumed_wf_types_and_report_errors(param_env,defining_use_anchor)?;if true{};let
implied_bounds=infcx.implied_bounds_tys(param_env,def_id,&wf_tys);let _=||();let
outlives_env=OutlivesEnvironment::with_bounds(param_env,implied_bounds);{;};ocx.
...
hidden_type.ty);loop{break};loop{break};key=infcx.resolve_vars_if_possible(key);
...
is_ty_var(){3;return Ok(());}if let ty::Alias(ty::Opaque,alias)=ty.ty.kind(){if 
alias.def_id==key.def_id.to_def_id()&&alias.args==key.args{3;return Ok(());}}let
...
,},})};let _=||();ty.ty=strip_vars(ty.ty);let hidden_ty=tcx.type_of(key.def_id).
...
def_id){DefKind::Static{..}=>{let _=||();let _=||();tcx.ensure().typeck(def_id);
maybe_check_static_with_link_section(tcx,def_id);{;};check_static_inhabited(tcx,
def_id);;check_static_linkage(tcx,def_id);}DefKind::Const=>{tcx.ensure().typeck(
def_id);3;}DefKind::Enum=>{check_enum(tcx,def_id);}DefKind::Fn=>{if let Some(i)=
...
check_impl_items_against_trait(tcx,def_id,impl_trait_header);let _=();if true{};
check_on_unimplemented(tcx,def_id);{();};}}DefKind::Trait=>{let assoc_items=tcx.
associated_items(def_id);3;check_on_unimplemented(tcx,def_id);for&assoc_item in 
assoc_items.in_definition_order(){match assoc_item.kind{ty::AssocKind::Fn=>{;let
abi=tcx.fn_sig(assoc_item.def_id).skip_binder().abi();3;forbid_intrinsic_abi(tcx
,assoc_item.ident(tcx).span,abi);;}ty::AssocKind::Type if assoc_item.defaultness
(tcx).has_value()=>{3;let trait_args=GenericArgs::identity_for_item(tcx,def_id);
...
tcx,def_id);({});}}DefKind::TyAlias=>{check_type_alias_type_params_are_used(tcx,
...
);3;}}_=>{for item in items{let def_id=item.id.owner_id.def_id;let generics=tcx.
...
);3;}_=>{}}}}}}DefKind::GlobalAsm=>{let it=tcx.hir().expect_item(def_id);let hir
::ItemKind::GlobalAsm(asm)=it.kind else{span_bug!(it.span,//if true{};if true{};
"DefKind::GlobalAsm but got {:#?}",it)};({});InlineAsmCtxt::new_global_asm(tcx).
...
,){*&*&();let Ok(ancestors)=trait_def.ancestors(tcx,impl_id)else{return};let mut
ancestor_impls =ancestors.skip(1).filter_map(|parent|{if parent.is_from_trait(){
None}else{Some((parent,parent.item(tcx,trait_item.def_id)))}});3;let opt_result=
...
impl_item,parent_impl);if true{};let _=||();}else{tcx.dcx().delayed_bug(format!(
...
first_item_span=tcx.def_span(first_item_ref);();struct_span_code_err!(tcx.dcx(),
first_item_span,E0749,"negative impls cannot have any items").emit();;}return;}}
let trait_def=tcx.trait_def(trait_ref.def_id);3;for&impl_item in impl_item_refs{
let ty_impl_item=tcx.associated_item(impl_item);3;let ty_trait_item=if let Some(
...
"missing associated item in trait");({});continue;};match ty_impl_item.kind{ty::
...
Type=>{loop{break;};compare_impl_ty(tcx,ty_impl_item,ty_trait_item,trait_ref);}}
...
trait_item_id);({});let is_implemented=leaf_def.as_ref().is_some_and(|node_item|
...
defining_node.is_from_trait());();if!is_implemented_here{let full_impl_span=tcx.
...
trait_item.ident(tcx)){{;};must_implement_one_of=None;}}}if let Some(leaf_def)=&
...
)).emit();let _=||();}}if!missing_items.is_empty(){let full_impl_span=tcx.hir().
...
"SIMD elements must have the same type").emit();({});return;}let len=if let ty::
Array(_ty,c)=(e.kind()){(c.try_eval_target_usize(tcx,tcx.param_env(def.did())))}
else{Some(fields.len()as u64)};let _=();let _=();if let Some(len)=len{if len==0{
struct_span_code_err!(tcx.dcx(),sp,E0075,"SIMD vector cannot be empty").emit();;
return;{;};}else if len>MAX_SIMD_LANES{struct_span_code_err!(tcx.dcx(),sp,E0075,
...
ReprPacked (pack)=r&&let Some(repr_pack)=repr.pack&&pack as u64!=repr_pack.bytes
(){let _=();let _=();let _=();let _=();struct_span_code_err!(tcx.dcx(),sp,E0634,
...
"packed type cannot transitively contain a `#[repr(align)]` type");let _=();err.
...
).align.is_some(){3;return Some(vec![(def.did(),DUMMY_SP)]);}stack.push(def_id);
...
;3;}if adt.is_union()&&!tcx.features().transparent_unions{feature_err(&tcx.sess,
sym::transparent_unions,((((((((tcx.def_span(((((((((adt.did()))))))))))))))))),
"transparent unions are unstable",).emit();let _=();}if adt.variants().len()!=1{
bad_variant_count(tcx,adt,tcx.def_span(adt.did()),adt.did());((),());return;}let
...
variants().iter().any(ty::VariantDef::is_field_list_non_exhaustive);let _=();let
...
check_non_exhaustive(tcx,ty).break_value())});let _=||();let non_trivial_fields=
field_infos .clone().filter_map(|(span,trivial,_non_exhaustive)|if!trivial{Some(
span)}else{None});3;let non_trivial_count=non_trivial_fields.clone().count();if 
non_trivial_count>=2{*&*&();bad_non_zero_sized_fields(tcx,adt,non_trivial_count,
non_trivial_fields,tcx.def_span(adt.did()),);if true{};if true{};return;}let mut
...
def_span(def_id),"zero-variant enum").emit();({});}}let repr_type_ty=def.repr().
...
),E0732,"`#[repr(inttype)]` must be specified").emit();let _=||();loop{break};}}
detect_discriminant_duplicate(tcx,def);let _=||();check_transparent(tcx,def);}fn
detect_discriminant_duplicate<'tcx>(tcx:TyCtxt<'tcx>,adt:ty::AdtDef<'tcx>){3;let
report=|dis:Discr<'tcx>,idx,err:&mut Diag<'_>|{{;};let var=adt.variant(idx);let(
...
VariantIdx::from_u32){((),());let explicit_variant=adt.variant(explicit_idx);let
ve_ident=var.name;((),());let _=();let ex_ident=explicit_variant.name;let sp=if 
distance_to_explicit>1{"variants"}else{"variant"};3;err.span_label(tcx.def_span(
...
!("{display_discr} assigned here"));{;};};let mut discrs=adt.discriminants(tcx).
...
error{{();};e.emit();}i+=1;}}fn check_type_alias_type_params_are_used<'tcx>(tcx:
TyCtxt<'tcx>,def_id:LocalDefId){if tcx.type_alias_is_lazy(def_id){();return;}let
generics=tcx.generics_of(def_id);3;if generics.own_counts().types==0{return;}let
ty=tcx.type_of(def_id).instantiate_identity();;if ty.references_error(){assert!(
tcx.dcx().has_errors().is_some());3;return;}let bounded_params=LazyCell::new(||{
...
let param_name=Ident::new(param.name,span);loop{break;};let has_explicit_bounds=
...
UnusedGenericParameterHelp::TyAlias{param_name},const_param_help,});3;diag.code(
E0091);3;diag.emit();}}}fn opaque_type_cycle_error(tcx:TyCtxt<'_>,opaque_def_id:
...
else{((),());let mut multispan:MultiSpan=spans.clone().into();for span in spans{
multispan.push_span_label(span,"this returned value is of `!` type");{();};}err.
span_note(multispan,"these returned values have a concrete \"never\" type");();}
err .help("this error will resolve once the item's body returns a concrete type"
);;}else{let mut seen=FxHashSet::default();seen.insert(span);err.span_label(span
,"recursive opaque type");*&*&();label=true;for(sp,ty)in visitor.returns.iter().
...
let mut visitor=OpaqueTypeCollector::default();3;ty.visit_with(&mut visitor);for
def_id in visitor.opaques{();let ty_span=tcx.def_span(def_id);if!seen.contains(&
ty_span){({});let descr=if ty.is_impl_trait(){"opaque "}else{""};err.span_label(
...
in visitor.closures{3;let Some(closure_local_did)=closure_def_id.as_local()else{
...
coroutine_interior_predicates=&typeck.coroutine_interior_predicates[&def_id];();
debug!(?coroutine_interior_predicates);if let _=(){};let infcx=tcx.infer_ctxt().
ignoring_regions().with_opaque_type_inference(DefiningAnchor::bind(tcx,typeck.//
hir_owner.def_id)).build();3;let mut fulfillment_cx=<dyn TraitEngine<'_>>::new(&
infcx);({});for(predicate,cause)in coroutine_interior_predicates{let obligation=
Obligation::new(tcx,cause.clone(),param_env,*predicate);let _=();fulfillment_cx.
...
hidden_type);let _=||();loop{break};let key=infcx.resolve_vars_if_possible(key);
