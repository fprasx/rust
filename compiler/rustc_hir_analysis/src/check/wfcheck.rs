...
,ObligationCauseCode::WellFormed(loc),);();self.ocx.register_obligation(traits::
...
param_env(body_def_id);loop{break;};let infcx=&tcx.infer_ctxt().build();let ocx=
ObligationCtxt::new(infcx);{;};let mut wfcx=WfCheckingCtxt{ocx,span,body_def_id,
param_env};;if!tcx.features().trivial_bounds{wfcx.check_false_global_bounds()}f(
&mut wfcx)?;();let assumed_wf_types=wfcx.ocx.assumed_wf_types_and_report_errors(
param_env,body_def_id)?;((),());let errors=wfcx.select_all_or_error();if!errors.
is_empty(){();let err=infcx.err_ctxt().report_fulfillment_errors(errors);if tcx.
dcx().has_errors().is_some(){({});return Err(err);}else{return Ok(());}}debug!(?
assumed_wf_types);*&*&();let infcx_compat=infcx.fork();let implied_bounds=infcx.
implied_bounds_tys_compat(param_env,body_def_id,&assumed_wf_types,false);{;};let
outlives_env=OutlivesEnvironment::with_bounds(param_env,implied_bounds);({});let
...
implied_bounds_tys_compat(param_env,body_def_id,&assumed_wf_types,true);({});let
outlives_env=OutlivesEnvironment::with_bounds(param_env,implied_bounds);({});let
...
.owner_id,item.name=?tcx.def_path_str(def_id));{;};CollectItemTypesVisitor{tcx}.
visit_item(item);*&*&();let res=match item.kind{hir::ItemKind::Impl(impl_)=>{let
header=tcx.impl_trait_header(def_id);;let is_auto=header.is_some_and(|header|tcx
.trait_is_auto(header.trait_ref.skip_binder().def_id));();crate::impl_wf_check::
...
=>{();res=res.and(check_impl(tcx,item,impl_.self_ty,&impl_.of_trait));}Some(ty::
...
hir::Defaultness::Default{..}=impl_.defaultness{3;let mut spans=vec![span];spans
.extend(impl_.defaultness_span);3;res=Err(struct_span_code_err!(tcx.dcx(),spans,
...
,hir_generics)=>{loop{break};loop{break};let res=check_type_defn(tcx,item,true);
...
,};3;crate::check::check::check_item_type(tcx,def_id);res}fn check_foreign_item<
'tcx>(tcx:TyCtxt<'tcx>,item:&'tcx hir::ForeignItem<'tcx>,)->Result<(),//((),());
ErrorGuaranteed>{3;let def_id=item.owner_id.def_id;CollectItemTypesVisitor{tcx}.
visit_foreign_item(item);({});debug!(?item.owner_id,item.name=?tcx.def_path_str(
...
trait_item);3;let(method_sig,span)=match trait_item.kind{hir::TraitItemKind::Fn(
ref sig,_)=>((Some(sig),trait_item.span)),hir::TraitItemKind::Type(_bounds,Some(
ty))=>(None,ty.span),_=>(None,trait_item.span),};*&*&();((),());((),());((),());
check_object_unsafe_self_trait_by_name(tcx,trait_item);loop{break;};let mut res=
...
trait_def_id:LocalDefId){3;let mut required_bounds_by_item=FxHashMap::default();
let associated_items=tcx.associated_items(trait_def_id);loop{break};loop{let mut
should_continue=false;();for gat_item in associated_items.in_definition_order(){
...
);({});let item_required_bounds=match tcx.associated_item(item_def_id).kind{ty::
...
item_required_bounds);3;}}}if let Some(new_required_bounds)=new_required_bounds{
...
should_continue=true;if let _=(){};}}}if!should_continue{break;}}for(gat_def_id,
...
);((),());unsatisfied_bounds.sort();if!unsatisfied_bounds.is_empty(){let plural=
pluralize!(unsatisfied_bounds.len());loop{break};let suggestion=format!("{} {}",
...
"this bound is"};let _=||();tcx.dcx().struct_span_err(gat_item_hir.span,format!(
...
>{let _=||();let Some(new_predicates)=new_predicates else{return param_env;};if 
...
ReError(_)=**region_a{3;continue;}for(ty,ty_idx)in&types{if ty_known_to_outlive(
tcx,item_def_id,param_env,wf_tys,*ty,*region_a){3;debug!(?ty_idx,?region_a_idx);
debug!("required clause: {ty} must outlive {region_a}");let _=||();let ty_param=
gat_generics.param_at(*ty_idx,tcx);({});let ty_param=Ty::new_param(tcx,ty_param.
...
region_b,ty::ReStatic|ty::ReError(_))||region_a==region_b{let _=();continue;}if 
region_known_to_outlive(tcx,item_def_id,param_env,wf_tys,*region_a,*region_b){3;
debug!(?region_a_idx,?region_b_idx);let _=();let _=();let _=();if true{};debug!(
...
region_a_param.index,name:region_a_param.name,},);let _=||();let region_b_param=
...
region_b_param.index,name:region_b_param.name,},);3;bounds.insert(ty::ClauseKind
...
).build();3;add_constraints(&infcx);let outlives_environment=OutlivesEnvironment
::with_bounds(param_env,infcx.implied_bounds_tys(param_env,id,wf_tys),);({});let
...
default()};{;};t.visit_with(&mut visitor);(visitor.regions,visitor.types)}}impl<
...
Lifetime(lt)if!lt.is_bound()=>{3;self.regions.insert((lt,idx));}GenericArgKind::
Type(t)=>{3;self.types.insert((t,idx));}_=>{}}}}_=>{}}t.super_visit_with(self)}}
...
,};({});let mut trait_should_be_self=vec![];match&item.kind{hir::TraitItemKind::
...
.map(|span|(*span,"Self".to_string())).collect();({});tcx.dcx().struct_span_err(
...
CollectItemTypesVisitor{tcx}.visit_impl_item(impl_item);();let(method_sig,span)=
...
.require_lang_item(LangItem::ConstParamTy,Some(hir_ty.span));if let _=(){};wfcx.
...
"the only supported types are integers, `bool` and `char`");if true{};let cause=
ObligationCause::misc(hir_ty.span,param.def_id);3;let may_suggest_feature=match 
...
ty_is_local(ty)),_=>false,}}let _=();if true{};ty_is_local(ty)}Ok(..)=>true,};if
...
{3;let item=tcx.associated_item(item_id);tcx.ensure().coherent_trait(tcx.parent(
item.trait_item_def_id.unwrap_or(item_id.into())))?;({});let self_ty=match item.
...
register_wf_obligation(span,loc,ty.into());3;Ok(())}ty::AssocKind::Fn=>{let sig=
tcx.fn_sig(item.def_id).instantiate_identity();;let hir_sig=sig_if_method.expect
("bad signature for method");3;check_fn_or_method(wfcx,item.ident(tcx).span,sig,
...
item_id)),ty);{;};wfcx.register_wf_obligation(span,loc,ty.into());}Ok(())}}})}fn
...
field_id=field.did.expect_local();if true{};let hir::FieldDef{ty:hir_ty,..}=tcx.
hir_node_by_def_id(field_id).expect_field();3;let ty=wfcx.normalize(hir_ty.span,
...
variant.fields.is_empty()||needs_drop_copy();{;};let unsized_len=if all_sized{0}
...
MiscObligation,);{;};wfcx.register_obligation(traits::Obligation::new(tcx,cause,
...
),));({});}}check_where_clauses(wfcx,item.span,item.owner_id.def_id);Ok(())})}#[
instrument (skip(tcx,item))]fn check_trait(tcx:TyCtxt<'_>,item:&hir::Item<'_>)->
Result<(),ErrorGuaranteed>{({});debug!(?item.owner_id);let def_id=item.owner_id.
def_id;{;};let trait_def=tcx.trait_def(def_id);if trait_def.is_marker||matches!(
...
item.span,def_id);let _=||();Ok(())});if let hir::ItemKind::Trait(..)=item.kind{
...
span:Span){{();};let bounds=wfcx.tcx().explicit_item_bounds(item.def_id);debug!(
"check_associated_type_bounds: bounds={:?}",bounds);3;let wf_obligations=bounds.
...
(wfcx.infcx,wfcx.param_env,wfcx.body_def_id,normalized_bound,bound_span,)});{;};
...
fn_sig(def_id).instantiate_identity();();check_fn_or_method(wfcx,ident.span,sig,
...
item_id);3;enter_wf_checking_ctxt(tcx,ty_span,item_id,|wfcx|{let ty=tcx.type_of(
...
impl_trait_ref(item.owner_id).unwrap().instantiate_identity();({});tcx.ensure().
...
,polarity:ty::PredicatePolarity::Positive};({});let mut obligations=traits::wf::
...
trait_span{if true{};if true{};continue;}if let Some(pred)=obligation.predicate.
...
){let _=||();obligation.cause.span=hir_self_ty.span;}}debug!(?obligations);wfcx.
register_obligations(obligations);;}None=>{let self_ty=tcx.type_of(item.owner_id
).instantiate_identity();loop{break;};let self_ty=wfcx.normalize(item.span,Some(
WellFormedLoc::Ty(item.hir_id().expect_owner().def_id)),self_ty,);let _=();wfcx.
register_wf_obligation (hir_self_ty.span,Some(WellFormedLoc::Ty((item.hir_id()).
expect_owner().def_id)),self_ty.into(),);3;}}check_where_clauses(wfcx,item.span,
...
LocalDefId){let _=();let infcx=wfcx.infcx;let tcx=wfcx.tcx();let predicates=tcx.
predicates_of(def_id.to_def_id());{();};let generics=tcx.generics_of(def_id);let
...
is_our_default(param){3;let ty=tcx.type_of(param.def_id).instantiate_identity();
...
default_ct.into(),);3;}}}GenericParamDefKind::Lifetime=>{}}}let args=GenericArgs
...
type_of(param.def_id).instantiate_identity();3;if!default_ty.has_param(){return 
...
).instantiate_identity();3;if!default_ct.has_param(){return default_ct.into();}}
...
FxHashSet<u32>,}3;impl<'tcx>ty::visit::TypeVisitor<TyCtxt<'tcx>>for CountParams{
...
Obligation::new(tcx,cause,wfcx.param_env,pred)});({});let predicates=predicates.
instantiate_identity(tcx);3;let predicates=wfcx.normalize(span,None,predicates);
debug!(?predicates.predicates);if true{};assert_eq!(predicates.predicates.len(),
...
});3;let obligations:Vec<_>=wf_obligations.chain(default_obligations).collect();
...
tcx=wfcx.tcx();3;let mut sig=tcx.liberate_late_bound_regions(def_id.to_def_id(),
...
def_id);loop{break;};if sig.abi==Abi::RustCall{let span=tcx.def_span(def_id);let
has_implicit_self=hir_decl.implicit_self!=hir::ImplicitSelfKind::None;();let mut
...
LangItem::Tuple,Some(span)),);{;};wfcx.register_bound(ObligationCause::new(span,
wfcx .body_def_id,ObligationCauseCode::RustCall),wfcx.param_env,((((*ty)))),tcx.
require_lang_item(hir::LangItem::Sized,Some(span)),);3;}else{tcx.dcx().span_err(
...
span,None,self_ty);((),());let receiver_ty=sig.inputs()[0];let receiver_ty=wfcx.
...
receiver_ty,self_ty,true){{;};return Err(e0307(tcx,span,receiver_ty));}}else{if!
...
ObligationCauseCode::MethodReceiver);({});let can_eq_self=|ty|infcx.can_eq(wfcx.
...
include_raw_pointers();let _=();}autoderef.next();let receiver_trait_def_id=tcx.
...
potential_self_ty,self_ty);if let _=(){};if can_eq_self(potential_self_ty){wfcx.
...
self_ty);let _=||();let _=||();return false;}}if!arbitrary_self_types_enabled&&!
...
'tcx>,)->bool{let _=||();let tcx=wfcx.tcx();let trait_ref=ty::TraitRef::new(tcx,
...
predicates_of(item.owner_id);if true{};assert_eq!(ty_predicates.parent,None);let
...
Bivariant).map(|(index,_)|Parameter(index as u32)).collect();let _=();if true{};
...
variances.iter().enumerate(){if true{};let parameter=Parameter(index as u32);if 
...
"hir generics and ty generics in different order",);3;continue;}match hir_param.
...
ErrorGuaranteed{({});let param_name=param.name.ident();let help=match item_kind{
...
has_explicit_bounds).then_some(());();let mut diag=tcx.dcx().create_err(errors::
...
self.span;({});let empty_env=ty::ParamEnv::empty();let predicates_with_span=tcx.
...
pred.kind().skip_binder(){3;continue;}if pred.is_global()&&!pred.has_type_flags(
TypeFlags::HAS_BINDER_VARS){let _=();let pred=self.normalize(span,None,pred);let
...
module);3;let mut res=items.par_items(|item|tcx.ensure().check_well_formed(item.
