...
[hir::WherePredicate<'tcx>])>,span:Span,){3;let tcx=self.tcx();let sized_def_id=
tcx.lang_items().sized_trait();3;let mut seen_negative_sized_bound=false;let mut
seen_positive_sized_bound=false;;let mut unbounds:SmallVec<[_;1]>=SmallVec::new(
);let _=();let mut search_bounds=|hir_bounds:&'tcx[hir::GenericBound<'tcx>]|{for
...
sized_def_id){if true{};seen_positive_sized_bound=true;}}_=>{}}}};search_bounds(
...
&&pred.is_param_bound(self_ty.to_def_id()){{;};search_bounds(pred.bounds);}}}if 
unbounds.len()>1{;tcx.dcx().emit_err(errors::MultipleRelaxedDefaultBounds{spans:
unbounds.iter().map(|ptr|ptr.span).collect(),});{;};}let mut seen_sized_unbound=
...
PredicatePolarity::Negative)}hir::TraitBoundModifier::Maybe=>continue,};3;let _=
...
Outlives(lifetime)=>{{();};let region=self.lower_lifetime(lifetime,None);bounds.
...
,filter:PredicateFilter,)->Bounds<'tcx>{{;};let mut bounds=Bounds::default();let
...
true}else{false}}}),&mut bounds,ty::List::empty(),only_self_bounds,);();debug!(?
...
AssocKind::Type};({});let candidate=if self.probe_trait_that_defines_assoc_item(
...
path_span,Some(binding),)?};if true{};let _=||();let(assoc_ident,def_scope)=tcx.
adjust_ident_and_get_scope(binding.ident,candidate.def_id(),hir_ref_id);({});let
...
kind)).emit();((),());self.set_tainted_by_errors(reported);}tcx.check_stability(
...
,});{();};}).or_insert(binding.span);let projection_ty=if let ty::AssocKind::Fn=
assoc_kind{;let mut emitted_bad_param_err=None;let mut num_bound_vars=candidate.
bound_vars().len();let _=();let args=candidate.skip_binder().args.extend_to(tcx,
...
param_span:tcx.def_span(param.def_id),},)});();let ty=tcx.type_of(param.def_id).
no_bound_vars().expect("ct params cannot have early bound vars");{;};ty::Const::
new_error(tcx,guar,ty).into()}};3;num_bound_vars+=1;arg});let output=tcx.fn_sig(
...
span_if_local(assoc_item.def_id),note:(),}));if true{};};let shifted_output=tcx.
shift_bound_var_indices(num_bound_vars,output);{;};let instantiation_output=ty::
EarlyBinder::bind(shifted_output).instantiate(tcx,args);({});let bound_vars=tcx.
late_bound_vars(binding.hir_id);;ty::Binder::bind_with_vars(instantiation_output
,bound_vars)}else{3;let alias_ty=candidate.map_bound(|trait_ref|{let ident=Ident
::new(assoc_item.name,binding.ident.span);{;};let item_segment=hir::PathSegment{
...
assoc_item.def_id,&item_segment,trait_ref.args,);*&*&();debug!(?alias_args);ty::
...
map_bound(|ty|tcx.type_of(ty.def_id).instantiate(tcx,ty.args));if true{};let ty=
check_assoc_const_binding_type(tcx,assoc_ident,ty,binding.hir_id);if true{};tcx.
feed_anon_const_type(anon_const.def_id,ty::EarlyBinder::bind(ty));();}alias_ty};
...
),hir::Term::Const(ct)=>ty::Const::from_anon_const(tcx,ct.def_id).into(),};3;let
...
(trait_ref.rebind(term));if true{};debug!(?late_bound_in_projection_ty);debug!(?
late_bound_in_term);loop{break;};if let _=(){};self.validate_late_bound_regions(
...
ty.has_param()&&!ty.has_escaping_bound_vars(){({});return ty;}let mut collector=
GenericParamAndBoundVarCollector {tcx,params:(Default::default()),vars:Default::
default(),depth:ty::INNERMOST,};({});let mut guar=ty.visit_with(&mut collector).
break_value();();let ty_note=ty.make_suggestable(tcx,false,None).map(|ty|crate::
...
().map(|_|owner_id)).unwrap();if true{};let _=||();let generics=tcx.generics_of(
...
param.index);3;}ty::ReBound(db,br)if db>=self.depth=>{self.vars.insert(match br.
...
));{();};return ControlFlow::Break(guar);}});}_=>{}}ControlFlow::Continue(())}fn
...
"unexpected escaping late-bound const var");3;return ControlFlow::Break(guar);}_
