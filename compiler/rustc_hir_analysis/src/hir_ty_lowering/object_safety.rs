...
extend(cur_potential_assoc_types);let _=();}}let mut trait_bounds=vec![];let mut
...
assert_eq!(trait_pred.polarity,ty::PredicatePolarity::Positive);();trait_bounds.
...
"did not expect {pred} clause in object bounds");3;}}}let expanded_traits=traits
...
trait_is_auto(i.trait_ref().def_id()));loop{break};if regular_traits.len()>1{let
first_trait=&regular_traits[0];3;let additional_trait=&regular_traits[1];let mut
err=struct_span_code_err!(tcx.dcx(),additional_trait.bottom().1,E0225,//((),());
"only auto traits can be used as additional traits in a trait object");let _=();
...
"first non-auto trait","first use");loop{break;};if let _=(){};err.help(format!(
...
,);*&*&();self.set_tainted_by_errors(err.emit());}if regular_traits.is_empty()&&
...
TraitObjectDeclaredWithNoTraits{span,trait_alias_span});let _=();if true{};self.
...
FxIndexSet<DefId>>=FxIndexMap::default();let _=();let regular_traits_refs_spans=
trait_bounds.into_iter().filter(|(trait_ref,_)|!tcx.trait_is_auto(trait_ref.//3;
def_id()));3;for(base_trait_ref,span)in regular_traits_refs_spans{let base_pred:
...
pred))=>{{;};let pred=bound_predicate.rebind(pred);associated_types.entry(span).
...
projection_def_id();loop{break};loop{break};def_ids.swap_remove(&def_id);if tcx.
...
UnusedAssociatedTypeBounds{span:*span},);let _=();}}def_ids.retain(|def_id|!tcx.
generics_require_sized_self(def_id));{;};}self.complain_about_missing_assoc_tys(
associated_types,potential_assoc_types,hir_trait_bounds,);();let mut duplicates=
FxHashSet::default();({});auto_traits.retain(|i|duplicates.insert(i.trait_ref().
def_id()));if true{};let _=||();debug!(?regular_traits);debug!(?auto_traits);let
existential_trait_refs=(regular_traits.iter()).map(|i|{i.trait_ref().map_bound(|
trait_ref:ty::TraitRef<'tcx>|{{;};assert_eq!(trait_ref.self_ty(),dummy_self);let
mut missing_type_params=vec![];3;let mut references_self=false;let generics=tcx.
...
into();{;};}else if arg.walk().any(|arg|arg==dummy_self.into()){references_self=
true;*&*&();return Ty::new_misc_error(tcx).into();}arg}).collect();let args=tcx.
...
SupertraitSelf(smallvec![]).error_msg(),).emit();{;};self.set_tainted_by_errors(
reported);((),());}ty::ExistentialTraitRef{def_id:trait_ref.def_id,args}})});let
existential_projections =((((projection_bounds.iter())))).map(|(bound,_)|{bound.
map_bound(|mut b|{if true{};assert_eq!(b.projection_ty.self_ty(),dummy_self);let
...
"trait object projection bounds reference `Self`");let _=||();let args:Vec<_>=b.
...
=tcx.mk_args(&args);{;};}ty::ExistentialProjection::erase_self_ty(tcx,b)})});let
regular_trait_predicates=existential_trait_refs.map(|trait_ref|trait_ref.//({});
map_bound(ty::ExistentialPredicate::Trait));if true{};let auto_trait_predicates=
auto_traits.into_iter().map(|trait_ref|{ty::Binder::dummy(ty:://((),());((),());
ExistentialPredicate::AutoTrait(trait_ref.trait_ref().def_id()))});();let mut v=
...
);3;let region_bound=if!lifetime.is_elided(){self.lower_lifetime(lifetime,None)}
...
);let _=();if true{};let e=if borrowed{err.delay_as_bug()}else{err.emit()};self.
set_tainted_by_errors(e);loop{break};ty::Region::new_error(tcx,e)})}})};debug!(?
