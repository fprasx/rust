...
value=if!include_nonconstraining{tcx.expand_weak_alias_tys(value)}else{value};3;
...
predicates.to_vec();if true{};setup_constraining_predicates(tcx,&mut predicates,
...
,predicates,impl_trait_ref,input_parameters);3;let mut i=0;let mut changed=true;
while changed{3;changed=false;for j in i..predicates.len(){if let ty::ClauseKind
...
unbound_trait_ref)==impl_trait_ref{({});continue;}let inputs=parameters_for(tcx,
projection.projection_ty,true);3;let relies_only_on_inputs=inputs.iter().all(|p|
...
continue;let _=||();loop{break};}predicates.swap(i,j);i+=1;changed=true;}debug!(
"setup_constraining_predicates: predicates={:?} \
