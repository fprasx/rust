...
(def_id)=>def_id,WellFormedLoc::Param{function,param_idx:_}=>function,};({});let
hir_id=tcx.local_def_id_to_hir_id(def_id);*&*&();tcx.dcx().span_delayed_bug(tcx.
def_span(def_id),"Performed HIR wfcheck without an existing error!");();let icx=
...
'tcx>){{;};let infcx=self.tcx.infer_ctxt().build();let ocx=ObligationCtxt::new(&
infcx);;let tcx_ty=self.icx.lower_ty(ty);let tcx_ty=self.tcx.fold_regions(tcx_ty
,|r,_|{if r.is_bound(){self.tcx.lifetimes.re_erased}else{r}});3;let cause=traits
::ObligationCause::new(ty.span,self.def_id,traits::ObligationCauseCode:://{();};
WellFormed(None),);{;};ocx.register_obligation(traits::Obligation::new(self.tcx,
...
);if true{};self.depth-=1;}}let mut visitor=HirWfCheck{tcx,predicate,cause:None,
...
usize]]}}};let _=();if true{};for ty in tys{visitor.visit_ty(ty);}visitor.cause}
