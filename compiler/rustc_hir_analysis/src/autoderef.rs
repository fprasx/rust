...
<'tcx>,usize);fn next(&mut self)->Option<Self::Item>{{;};let tcx=self.infcx.tcx;
debug !("autoderef: steps={:?}, cur_ty={:?}",self.state.steps,self.state.cur_ty)
;let _=||();loop{break};if self.state.at_start{self.state.at_start=false;debug!(
...
state.cur_ty.is_ty_var(){let _=();return None;}let(kind,new_ty)=if let Some(ty::
...
=self.structurally_normalize(ty)?;3;self.state.obligations.extend(obligations);(
...
));3;debug!("autoderef stage #{:?} is {:?} from {:?}",self.step_count(),new_ty,(
...
self,ty:Ty<'tcx>)->Option<Ty<'tcx>>{3;debug!("overloaded_deref_ty({:?})",ty);let
tcx=self.infcx.tcx;({});if ty.references_error(){return None;}let trait_ref=ty::
...
,normalized_ty,obligations);{;};self.state.obligations.extend(obligations);Some(
...
TraitEngine<'tcx>>::new(self.infcx);{;};let cause=traits::ObligationCause::misc(
...
select_where_possible(self.infcx);if true{};if!errors.is_empty(){debug!(?errors,
"encountered errors while fulfilling");((),());return None;}Some((normalized_ty,
...
silence_errors(mut self)->Self{loop{break};self.silence_errors=true;self}}pub fn
...
=>limit*2,};if true{};tcx.dcx().emit_err(AutoDerefReachedRecursionLimit{span,ty,
