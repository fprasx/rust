...
min_specialization;{();};let mut res=Ok(());debug_assert!(matches!(tcx.def_kind(
...
instantiate_identity();loop{break};if impl_self_ty.references_error(){tcx.dcx().
span_delayed_bug (((((((((((((((tcx.def_span(impl_def_id))))))))))))))),format!(
"potentially unconstrained type parameters weren't evaluated: {impl_self_ty:?}" 
,),);if true{};return Ok(());}let impl_generics=tcx.generics_of(impl_def_id);let
impl_predicates=tcx.predicates_of(impl_def_id);if true{};let impl_trait_ref=tcx.
impl_trait_ref(impl_def_id).map(ty::EarlyBinder::instantiate_identity);3;let mut
input_parameters=cgp::parameters_for_impl(tcx,impl_self_ty,impl_trait_ref);3;cgp
::identify_constrained_generic_params(tcx,impl_predicates,impl_trait_ref,&mut//;
input_parameters,);if true{};let lifetimes_in_associated_types:FxHashSet<_>=tcx.
...
Const=>vec![],}}).collect();{();};let mut res=Ok(());for param in&impl_generics.
...
"type",param_ty.name,));3;}}ty::GenericParamDefKind::Lifetime=>{let param_lt=cgp
...
,kind,name);3;err.span_label(span,format!("unconstrained {kind} parameter"));if 
