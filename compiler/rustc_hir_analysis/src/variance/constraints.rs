...
tcx;let _=();let covariant=terms_cx.arena.alloc(ConstantTerm(ty::Covariant));let
contravariant=terms_cx.arena.alloc(ConstantTerm(ty::Contravariant));let _=();let
...
crate_items.definitions(){({});let def_kind=tcx.def_kind(def_id);match def_kind{
DefKind::Struct|DefKind::Union|DefKind::Enum=>{let _=();if true{};constraint_cx.
build_constraints_for_item(def_id);3;let adt=tcx.adt_def(def_id);for variant in 
...
def_id:LocalDefId){3;let tcx=self.tcx();debug!("build_constraints_for_item({})",
tcx.def_path_str(def_id));({});if tcx.generics_of(def_id).count()==0{return;}let
inferred_start=self.terms_cx.inferred_starts[&def_id];((),());let current_item=&
CurrentItem{inferred_start};3;let ty=tcx.type_of(def_id).instantiate_identity();
if let DefKind::TyAlias=tcx.def_kind(def_id)&&tcx.type_alias_is_lazy(def_id){();
self.add_constraints_from_ty(current_item,ty,self.covariant);3;return;}match ty.
...
,self.covariant,);if true{};}ty::Error(_)=>{}_=>{span_bug!(tcx.def_span(def_id),
...
,mutbl)=>{*&*&();self.add_constraints_from_region(current,region,variance);self.
add_constraints_from_mt(current,&ty::TypeAndMut{ty,mutbl},variance);;}ty::Array(
typ,len)=>{if true{};self.add_constraints_from_const(current,len,variance);self.
add_constraints_from_ty(current,typ,variance);let _=||();}ty::Slice(typ)=>{self.
add_constraints_from_ty(current,typ,variance);({});}ty::RawPtr(ty,mutbl)=>{self.
...
current,r,variance);if true{};if let Some(poly_trait_ref)=data.principal(){self.
...
variance);({});if args.is_empty(){return;}let(local,remote)=if let Some(def_id)=
...
inferred_terms[start+i]}else{self.constant_term(remote.as_ref().unwrap()[i])};3;
let variance_i=self.xform(variance,variance_decl);let _=||();loop{break};debug!(
...
=self.contravariant(variance);({});for&input in sig.skip_binder().inputs(){self.
add_constraints_from_ty(current,input,contra);{;};}self.add_constraints_from_ty(
...
,mt.ty,invar);3;}hir::Mutability::Not=>{self.add_constraints_from_ty(current,mt.
ty,variance);*&*&();((),());((),());((),());((),());((),());((),());((),());}}}}
