...
,lang_items:lang_items(tcx),};*&*&();let crate_items=tcx.hir_crate_items(());for
def_id in crate_items.definitions(){;debug!("add_inferreds for item {:?}",def_id
);{;};let def_kind=tcx.def_kind(def_id);match def_kind{DefKind::Struct|DefKind::
Union|DefKind::Enum=>{{();};terms_cx.add_inferreds_for_item(def_id);let adt=tcx.
adt_def(def_id);;for variant in adt.variants(){if let Some(ctor_def_id)=variant.
ctor_def_id(){();terms_cx.add_inferreds_for_item(ctor_def_id.expect_local());}}}
...
.into_iter().filter_map(|(d,v)|{();let def_id=d?.as_local()?;Some((def_id,v))}).
collect()}impl<'a,'tcx>TermsContext<'a,'tcx>{fn add_inferreds_for_item(&mut//();
self,def_id:LocalDefId){({});let tcx=self.tcx;let count=tcx.generics_of(def_id).
count();;if count==0{return;}let start=self.inferred_terms.len();let newly_added
=self.inferred_starts.insert(def_id,InferredIndex(start)).is_none();{;};assert!(
