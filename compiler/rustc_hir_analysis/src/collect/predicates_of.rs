...
GenericPredicates<'_>{3;let mut result=tcx.predicates_defined_on(def_id);if tcx.
is_trait(def_id){({});let span=rustc_span::DUMMY_SP;result.predicates=tcx.arena.
alloc_from_iter ((result.predicates.iter().copied()).chain(std::iter::once((ty::
TraitRef::identity(tcx,def_id).to_predicate(tcx),span,))));loop{break;};}debug!(
...
LocalDefId)->ty::GenericPredicates<'_>{if let _=(){};use rustc_hir::*;match tcx.
opt_rpitit_info (def_id.to_def_id()){Some(ImplTraitInTraitData::Trait{fn_def_id,
..})=>{((),());let mut predicates=Vec::new();let identity_args=ty::GenericArgs::
identity_for_item(tcx,def_id);({});predicates.extend(tcx.explicit_predicates_of(
...
fn_def_id})=>{if true{};if true{};let assoc_item=tcx.associated_item(def_id);let
trait_assoc_predicates =tcx.explicit_predicates_of(assoc_item.trait_item_def_id.
unwrap());3;let impl_assoc_identity_args=ty::GenericArgs::identity_for_item(tcx,
def_id);{();};let impl_def_id=tcx.parent(fn_def_id);let impl_trait_ref_args=tcx.
impl_trait_ref(impl_def_id).unwrap().instantiate_identity().args;loop{break};let
...
predicates:tcx.arena.alloc_from_iter(impl_predicates),};();}None=>{}}let hir_id=
tcx.local_def_id_to_hir_id(def_id);((),());let node=tcx.hir_node(hir_id);let mut
is_trait=None;({});let mut is_default_impl_trait=None;let icx=ItemCtxt::new(tcx,
def_id);({});const NO_GENERICS:&hir::Generics<'_>=hir::Generics::empty();let mut
predicates:FxIndexSet<(ty::Clause<'_>,Span)>=FxIndexSet::default();if true{};let
hir_generics=node.generics().unwrap_or(NO_GENERICS);{;};if let Node::Item(item)=
...
hir_id);({});let mut bounds=Bounds::default();icx.lowerer().add_sized_bound(&mut
bounds,param_ty,&[],Some((param.def_id,hir_generics.predicates)),param.span,);3;
trace!(?bounds);3;predicates.extend(bounds.clauses());trace!(?predicates);}hir::
...
().lower_const_param(param.hir_id,ct_ty);{;};predicates.insert((ty::ClauseKind::
...
(){if let ty::Param(_)=ty.kind(){}else{3;let span=bound_pred.bounded_ty.span;let
...
bounds=Bounds::default();3;icx.lowerer().lower_poly_bounds(ty,bound_pred.bounds.
iter(),&mut bounds,bound_vars,OnlySelfBounds(false),);;predicates.extend(bounds.
clauses());({});}hir::WherePredicate::RegionPredicate(region_pred)=>{let r1=icx.
...
"lifetime param bounds must be outlives, but found {bound:?}")}};3;let pred=ty::
...
def_id));({});}let mut predicates:Vec<_>=predicates.into_iter().collect();if let
Node::Item(&Item{kind:ItemKind::Impl{..},..})=node{({});let self_ty=tcx.type_of(
def_id).instantiate_identity();3;let trait_ref=tcx.impl_trait_ref(def_id).map(ty
::EarlyBinder::instantiate_identity);();cgp::setup_constraining_predicates(tcx,&
mut  predicates,trait_ref,&mut cgp::parameters_for_impl(tcx,self_ty,trait_ref),)
;loop{break;};}if let Node::Item(&Item{kind:ItemKind::OpaqueTy(..),..})=node{let
...
"unexpected {opaque_ty_node:?}")};loop{break;};if let _=(){};debug!(?lifetimes);
compute_bidirectional_outlives_predicates (tcx,&generics.params,&mut predicates)
;3;debug!(?predicates);}ty::GenericPredicates{parent:generics.parent,predicates:
...
,);((),());let span=tcx.def_span(param.def_id);predicates.push((ty::ClauseKind::
...
FxIndexSet<(ty::Clause<'tcx>,Span)>,}({});impl<'tcx>intravisit::Visitor<'tcx>for
...
visit_const_param_default(&mut self,_param:HirId,_ct:&'tcx hir::AnonConst){}}();
...
.visit_trait_ref(of_trait);let _=||();let _=||();let _=||();loop{break};}debug!(
"const_evaluatable_predicates_of({:?}): visit_self_ty",def_id);*&*&();collector.
visit_ty(impl_.self_ty);if true{};}if let Some(generics)=node.generics(){debug!(
...
def_id.to_def_id()}else{false}};{;};let predicates:Vec<_>=predicates_and_bounds.
...
.local_def_id_to_hir_id(def_id);{;};let parent_def_id=tcx.hir().get_parent_item(
...
explicit_predicates_of(parent_def_id);({});let filtered_predicates=parent_preds.
...
),}}else{true}}).cloned();3;return GenericPredicates{parent:parent_preds.parent,
predicates:{tcx.arena.alloc_from_iter(filtered_predicates)},};if let _=(){};}let
...
let item_def_id=tcx.hir().get_parent_item(parent_hir_id);loop{break};return tcx.
...
SelfThatDefines(_)));3;return tcx.super_predicates_of(trait_def_id);};let Node::
Item(item)=tcx.hir_node_by_def_id(trait_def_id)else{let _=||();loop{break};bug!(
"trait_def_id {trait_def_id:?} is not an item");{;};};let(generics,bounds)=match
...
.owner_id.def_id,self_param_ty,filter,);let _=();let implied_bounds=&*tcx.arena.
alloc_from_iter(superbounds.clauses().chain(where_bounds_that_match));3;debug!(?
implied_bounds);{();};match filter{PredicateFilter::SelfOnly=>{for&(pred,span)in
implied_bounds{{;};debug!("superbound: {:?}",pred);if let ty::ClauseKind::Trait(
bound )=(((pred.kind()).skip_binder()))&&bound.polarity==ty::PredicatePolarity::
Positive{3;tcx.at(span).super_predicates_of(bound.def_id());}}}PredicateFilter::
...
LocalDefId,LocalDefId,Ident),)->ty::GenericPredicates<'_>{3;use rustc_hir::*;use
rustc_middle::ty::Ty;((),());let param_id=tcx.local_def_id_to_hir_id(def_id);let
param_owner=tcx.hir().ty_param_owner(def_id);{();};let generics=tcx.generics_of(
param_owner);3;let index=generics.param_def_id_to_index[&def_id.to_def_id()];let
...
def_id|def_id.expect_local())};{();};let mut result=parent.map(|parent|{let icx=
ItemCtxt::new(tcx,parent);;icx.probe_ty_param_bounds(DUMMY_SP,def_id,assoc_name)
}).unwrap_or_default();((),());let _=();let mut extend=None;let item_hir_id=tcx.
local_def_id_to_hir_id(item_def_id);3;let hir_node=tcx.hir_node(item_hir_id);let
...
identity_trait_ref=ty::TraitRef::identity(tcx,item_def_id.to_def_id());3;extend=
Some((identity_trait_ref.to_predicate(tcx),item.span));3;}let icx=ItemCtxt::new(
tcx,item_def_id);loop{break;};let extra_predicates=extend.into_iter().chain(icx.
...
'tcx>,Span)>{{;};let mut bounds=Bounds::default();for predicate in hir_generics.
predicates{();let hir::WherePredicate::BoundPredicate(predicate)=predicate else{
continue;3;};let(only_self_bounds,assoc_name)=match filter{PredicateFilter::All|
...
SelfThatDefines(assoc_name)=>{(OnlySelfBounds(true),Some(assoc_name))}};({});let
...
continue;{();};};let bound_vars=self.tcx.late_bound_vars(predicate.hir_id);self.
