...
LocalDefId,)->Ty<'_>{let _=||();let mut parent_def_id=def_id;while tcx.def_kind(
parent_def_id)==def::DefKind::OpaqueTy{if true{};parent_def_id=tcx.local_parent(
parent_def_id);{();};}let impl_def_id=tcx.local_parent(parent_def_id);match tcx.
...
hidden.build_mismatch_error(&concrete_type,def_id,tcx){3;d.emit();}}}}hidden.ty}
...
let hir_id=tcx.local_def_id_to_hir_id(def_id);if let _=(){};let scope=tcx.hir().
get_defining_scope(hir_id);{;};let mut locator=TaitConstraintLocator{def_id,tcx,
...
d.emit();{;};}}}}hidden.ty}else{let mut parent_def_id=def_id;while tcx.def_kind(
parent_def_id)==def::DefKind::OpaqueTy{if true{};parent_def_id=tcx.local_parent(
parent_def_id);();}let reported=tcx.dcx().emit_err(UnconstrainedOpaqueType{span:
...
self.tcx.hir_node_by_def_id(item_def_id);3;debug_assert!(!matches!(hir_node,Node
...
"inferring return types and opaque types do not mix well",);3;self.found=Some(ty
::OpaqueHiddenType{span:DUMMY_SP,ty:Ty::new_error(self.tcx,guar)});3;return;}let
...
tables.concrete_opaque_types{if opaque_type_key.def_id!=self.def_id{3;continue;}
constrained=true;3;let opaque_types_defined_by=self.tcx.opaque_types_defined_by(
item_def_id);3;if!opaque_types_defined_by.contains(&self.def_id){self.tcx.dcx().
...
def_id,self.tcx).map(|d|d.emit());3;prev.ty=Ty::new_error(self.tcx,guar);}}else{
...
'tcx Item<'tcx>){3;trace!(?it.owner_id);if it.owner_id.def_id!=self.def_id{self.
check(it.owner_id.def_id);;intravisit::walk_item(self,it);}}fn visit_impl_item(&
mut self,it:&'tcx ImplItem<'tcx>){3;trace!(?it.owner_id);if it.owner_id.def_id!=
...
self.check(it.owner_id.def_id);let _=();intravisit::walk_trait_item(self,it);}fn
...
.def_id!=def_id{{();};continue;}let concrete_type=tcx.erase_regions(hidden_type.
...
opaque_type_key.def_id),StashKey::OpaqueHiddenTypeMismatch,);let _=||();}}}else{
...
mir_opaque_ty.ty;3;}debug!(?owner_def_id);let mut locator=RpitConstraintChecker{
...
)in concrete_opaque_types{if def_id!=self.def_id{loop{break;};continue;}debug!(?
...
ExprKind::Closure(closure)=ex.kind{({});self.check(closure.def_id);}intravisit::
walk_expr(self,ex);{;};}fn visit_item(&mut self,it:&'tcx Item<'tcx>){trace!(?it.
...
<'tcx>){3;trace!(?it.owner_id);if it.owner_id.def_id!=self.def_id{self.check(it.
owner_id.def_id);;intravisit::walk_impl_item(self,it);}}fn visit_trait_item(&mut
self,it:&'tcx TraitItem<'tcx>){({});trace!(?it.owner_id);self.check(it.owner_id.
def_id);((),());((),());((),());let _=();intravisit::walk_trait_item(self,it);}}
