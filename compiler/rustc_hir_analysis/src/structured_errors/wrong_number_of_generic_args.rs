...
parent_hir_node(self.path_segment.hir_id);loop{break;};let parent_item=self.tcx.
hir_node_by_def_id(((self.tcx.hir()).get_parent_item(self.path_segment.hir_id)).
def_id,);{;};let hir::Node::TraitRef(hir::TraitRef{hir_ref_id:trait_ref_id,..})=
...
..})=parent_item else{3;return false;};trait_ref_id==id_in_of_trait}else{false}}
...
num_expected_lifetime_args()-self.num_provided_lifetime_args();let _=();assert!(
missing_args>0);();missing_args}fn num_missing_type_or_const_args(&self)->usize{
let  missing_args=self.num_expected_type_or_const_args_including_defaults()-self
.num_provided_type_or_const_args();{();};assert!(missing_args>0);missing_args}fn
...
num_params_to_take:usize,)->String{();debug!(?path_hir_id);if let Some(lt)=self.
...
num_params_to_take).collect::<Vec<_>>().join(", ");3;}let mut ret=Vec::new();let
mut ty_id=None;3;for(id,node)in self.tcx.hir().parent_iter(path_hir_id){debug!(?
id);({});if let hir::Node::Ty(_)=node{ty_id=Some(id);}if let Some(fn_decl)=node.
...
.hir_id==ty_id);();if in_arg||(in_ret&&fn_decl.lifetime_elision_allowed){return 
...
::BareFn(bare_fn)=ty.kind{bare_fn.generic_params}else{&[]};();ret.extend(params.
iter().filter_map(|p|{loop{break};let hir::GenericParamKind::Lifetime{kind:hir::
LifetimeParamKind::Explicit}=p.kind else{({});return None;};let hir::ParamName::
Plain(name)=p.name else{return None};();Some(name.to_string())}));if ret.len()>=
num_params_to_take{{;};return ret[..num_params_to_take].join(", ");}if let hir::
Node::Item(_)=node{loop{break;};break;}}self.gen_params.params.iter().skip(self.
...
MethodCall(hir::PathSegment{args:Some(_),..},..))});3;let fn_sig=self.tcx.hir().
get_if_local(self.def_id).and_then(hir::Node::fn_sig);{;};let is_used_in_input=|
...
def_path=self.tcx.def_path_str(self.def_id);{;};let def_kind=self.tcx.def_descr(
self.def_id);{;};let(quantifier,bound)=self.get_quantifier_and_bound();let kind=
self.kind();if true{};let provided_lt_args=self.num_provided_lifetime_args();let
provided_type_or_const_args=self.num_provided_type_or_const_args();let _=();let(
...
Diag<'tcx>{let _=();let _=();let span=self.path_segment.ident.span;let msg=self.
...
get_quantifier_and_bound();{();};let provided_args=self.num_provided_args();err.
...
too_many_args_provided(){3;return;}let args=self.gen_args.args.iter().skip(self.
get_lifetime_args_offset()).take(provided_args).enumerate();3;for(i,arg)in args{
err.span_label((((arg.span()))),if ((((((i+(((1))))))==provided_args))){format!(
"supplied {} {} argument{}" ,provided_args,self.kind(),pluralize!(provided_args)
)}else{String::new()},);let _=||();}}fn suggest(&self,err:&mut Diag<'_>){debug!(
...
suggest_adding_args(err);let _=||();}else if self.too_many_args_provided(){self.
suggest_moving_args_from_assoc_fn_to_trait(err);loop{break;};if let _=(){};self.
suggest_removing_args_or_generics(err);if let _=(){};}else{unreachable!();}}}}fn
...
angle_brackets{AngleBrackets::Missing=>{3;let span=self.path_segment.ident.span;
let sugg=format!("<{suggested_args}>");let _=||();debug!("sugg: {:?}",sugg);err.
span_suggestion_verbose(((((((span.shrink_to_hi())))))),msg,sugg,Applicability::
HasPlaceholders,);3;}AngleBrackets::Available=>{let(sugg_span,is_first)=if self.
...
num_provided_type_or_const_args()!=0;3;let has_bindings=!self.gen_args.bindings.
is_empty();((),());let sugg_prefix=if is_first{""}else{", "};let sugg_suffix=if 
...
AngleBrackets::Implied=>{((),());let _=();let _=();let _=();unreachable!();}}}fn
suggest_adding_type_and_const_args(&self,err:&mut Diag<'_>){((),());let _=();let
num_missing_args=self.num_missing_type_or_const_args();let _=();let msg=format!(
...
num_missing_args);({});debug!("suggested_args: {:?}",suggested_args);match self.
angle_brackets{AngleBrackets::Missing|AngleBrackets::Implied=>{();let span=self.
path_segment.ident.span;if true{};let sugg=format!("<{suggested_args}>");debug!(
"sugg: {:?}",sugg);{;};err.span_suggestion_verbose(span.shrink_to_hi(),msg,sugg,
Applicability::HasPlaceholders,);;}AngleBrackets::Available=>{let gen_args_span=
self.gen_args.span().unwrap();3;let sugg_offset=self.get_lifetime_args_offset()+
...
num_excess_lifetime_args();3;let trait_generics=self.tcx.generics_of(trait_);let
num_trait_generics_except_self=((((trait_generics.count()))))-if trait_generics.
has_self{1}else{0};let _=||();let _=||();let _=||();loop{break};let msg=format!(
...
self.tcx.sess.source_map();();let hir::ExprKind::MethodCall(_,rcvr,args,_)=expr.
kind else{;return;};if num_assoc_fn_excess_args!=num_trait_generics_except_self{
return;((),());}let Some(gen_args)=self.gen_args.span_ext()else{return;};let Ok(
generics)=sm.span_to_snippet(gen_args)else{loop{break};return;};let Ok(rcvr)=sm.
span_to_snippet (rcvr.span.find_ancestor_inside(expr.span).unwrap_or(rcvr.span))
else{((),());return;};let Ok(rest)=(match args{[]=>Ok(String::new()),[arg]=>{sm.
...
unwrap_or(first.span);3;let last_span=last.span.find_ancestor_inside(expr.span).
...
(trait_def_id);loop{break;};let method_name=self.tcx.item_name(self.def_id);err.
...
{((),());let _=();let num_provided_lt_args=self.num_provided_lifetime_args();let
num_provided_type_const_args=self.num_provided_type_or_const_args();let _=();let
...
let num_redundant_lt_args=self.num_excess_lifetime_args();let _=();if true{};let
num_redundant_type_or_const_args=self.num_excess_type_or_const_args();*&*&();let
num_redundant_args=num_redundant_lt_args+num_redundant_type_or_const_args;();let
redundant_lifetime_args=num_redundant_lt_args>0;if let _=(){};*&*&();((),());let
redundant_type_or_const_args=num_redundant_type_or_const_args>0;loop{break;};let
remove_entire_generics=num_redundant_args>=self.gen_args.args.len();let _=();let
provided_args_matches_unbound_traits=(((((((((((unbound_types.len())))))))))))==
num_redundant_type_or_const_args;3;let remove_lifetime_args=|err:&mut Diag<'_>|{
let mut lt_arg_spans=Vec::new();3;let mut found_redundant=false;for arg in self.
...
=true;if true{};}}else if found_redundant{break;}}let span_lo_redundant_lt_args=
lt_arg_spans[self.num_expected_lifetime_args()];3;let span_hi_redundant_lt_args=
...
num_redundant_lt_args=lt_arg_spans.len()-self.num_expected_lifetime_args();3;let
...
GenericArg::Infer(_)=>{();gen_arg_spans.push(arg.span());if gen_arg_spans.len()>
self.num_expected_type_or_const_args(){if let _=(){};found_redundant=true;}}_ if
found_redundant=>break,_=>{}}}let _=();let span_lo_redundant_type_or_const_args=
gen_arg_spans[self.num_expected_type_or_const_args()];let _=||();loop{break};let
span_hi_redundant_type_or_const_args=gen_arg_spans[gen_arg_spans.len()-1];();let
...
num_expected_type_or_const_args();if let _=(){};let msg_types_or_consts=format!(
"remove {these} generic argument{s}",these=pluralize!("this",//((),());let _=();
num_redundant_gen_args),s=pluralize!(num_redundant_gen_args),);loop{break;};err.
span_suggestion (span_redundant_type_or_const_args,msg_types_or_consts,(((""))),
Applicability::MaybeIncorrect,);{;};};if provided_args_matches_unbound_traits&&!
unbound_types.is_empty(){if!self.is_in_trait_impl(){3;let unused_generics=&self.
gen_args.args[self.num_expected_type_or_const_args()..];3;let suggestions=iter::
...
unwrap().with_lo(self.path_segment.ident.span.hi());loop{break};let msg=format!(
"remove these {}generics",if self.gen_args.parenthesized==hir:://*&*&();((),());
GenericArgsParentheses::ParenSugar{"parenthetical "}else{""},);loop{break;};err.
...
remove_lifetime_args(err);if true{};}else{assert!(redundant_type_or_const_args);
remove_type_or_const_args(err);();}}fn show_definition(&self,err:&mut Diag<'_>){
...
self.def_id);3;let(quantifier,bound)=self.get_quantifier_and_bound();let params=
...
name)).collect::<Vec<_>>().join(", ");let _=||();format!(": {params}")};format!(
"{} defined here, with {}{} {} parameter{}{}",def_kind,quantifier,bound,self.//;
kind(),pluralize!(bound),params,)};((),());let _=();err.span_note(spans,msg);}fn
