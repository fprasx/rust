...
ExprKind::If(i,then,e)=>{*&*&();self.cbox(INDENT_UNIT-1);self.ibox(0);self.word(
...
-1);loop{break};self.ibox(0);self.word(" else ");self.print_block(b)}_=>{panic!(
"print_if saw if with weird alternative");;}}}}fn print_if(&mut self,test:&ast::
Expr,blk:&ast::Block,elseopt:Option<&ast::Expr>){if true{};self.head("if");self.
print_expr_as_cond(test);{;};self.space();self.print_block(blk);self.print_else(
elseopt)}fn print_call_post(&mut self,args:&[P<ast::Expr>]){3;self.popen();self.
...
fixup:FixupContext,){if needs_par{3;self.popen();fixup=FixupContext::default();}
self.print_expr(expr,fixup);;if needs_par{self.pclose();}}fn print_expr_vec(&mut
self,exprs:&[P<ast::Expr>]){let _=();self.ibox(INDENT_UNIT);self.word("[");self.
commasep_exprs(Inconsistent,exprs);{();};self.word("]");self.end();}pub(super)fn
...
Block(block,None)=&expr.value.kind{if let _=(){};self.cbox(0);self.ibox(0);self.
print_block_with_attrs(block,attrs);if true{};}else{self.print_expr(&expr.value,
FixupContext::default());3;}self.end();}fn print_expr_repeat(&mut self,element:&
ast::Expr,count:&ast::AnonConst){{;};self.ibox(INDENT_UNIT);self.word("[");self.
print_expr(element,FixupContext::default());if true{};self.word_space(";");self.
print_expr(&count.value,FixupContext::default());3;self.word("]");self.end();}fn
...
self.word("}");loop{break;};return;}self.cbox(0);for(pos,field)in fields.iter().
with_position(){();let is_first=matches!(pos,Position::First|Position::Only);let
is_last=matches!(pos,Position::Last|Position::Only);();self.maybe_print_comment(
...
||has_rest{*&*&();self.word_space(",");}else{self.trailing_comma_or_space();}}if
has_rest{if fields.is_empty(){((),());self.space();}self.word("..");if let ast::
StructRest::Base(expr)=rest{;self.print_expr(expr,FixupContext::default());}self
.space();*&*&();((),());}self.offset(-INDENT_UNIT);self.end();self.word("}");}fn
print_expr_tup(&mut self,exprs:&[P<ast::Expr>]){if let _=(){};self.popen();self.
commasep_exprs(Inconsistent,exprs);{();};if exprs.len()==1{self.word(",");}self.
...
PREC_FORCE_PAREN,_=>parser::PREC_POSTFIX,};{;};self.print_expr_maybe_paren(func,
...
.fixity();();let(left_prec,right_prec)=match fixity{Fixity::Left=>(prec,prec+1),
Fixity::Right=>(prec+1,prec),Fixity::None=>(prec+1,prec+1),};({});let left_prec=
...
::needs_par_as_let_scrutinee(prec)=>{parser::PREC_FORCE_PAREN}_=>left_prec,};();
...
,..fixup},);((),());((),());self.space();self.word_space(op.node.as_str());self.
...
FixupContext,){loop{break};self.word("&");match kind{ast::BorrowKind::Ref=>self.
print_mutability(mutability,false),ast::BorrowKind::Raw=>{;self.word_nbsp("raw")
;({});self.print_mutability(mutability,true);}}self.print_expr_maybe_paren(expr,
...
FixupContext,){3;self.maybe_print_comment(expr.span.lo());let attrs=&expr.attrs;
if is_inline{if let _=(){};self.print_outer_attributes_inline(attrs);}else{self.
print_outer_attributes(attrs);{();};}self.ibox(INDENT_UNIT);let needs_par=fixup.
...
print_expr_vec(exprs);loop{break};}ast::ExprKind::ConstBlock(anon_const)=>{self.
print_expr_anon_const(anon_const,attrs);;}ast::ExprKind::Repeat(element,count)=>
{*&*&();self.print_expr_repeat(element,count);}ast::ExprKind::Struct(se)=>{self.
print_expr_struct(&se.qself,&se.path,&se.fields,&se.rest);3;}ast::ExprKind::Tup(
exprs)=>{({});self.print_expr_tup(exprs);}ast::ExprKind::Call(func,args)=>{self.
...
}ast::ExprKind::Unary(op,expr)=>{();self.print_expr_unary(*op,expr,fixup);}ast::
ExprKind::AddrOf(k,m,expr)=>{();self.print_expr_addr_of(*k,*m,expr,fixup);}ast::
ExprKind::Lit(token_lit)=>{3;self.print_token_literal(*token_lit,expr.span);}ast
...
::ExprKind::Cast(expr,ty)=>{((),());let prec=AssocOp::As.precedence()as i8;self.
print_expr_maybe_paren (expr,prec,FixupContext{stmt:(((((((((((false))))))))))),
leftmost_subexpression_in_stmt :fixup.stmt||fixup.leftmost_subexpression_in_stmt
,..fixup},);*&*&();self.space();self.word_space("as");self.print_type(ty);}ast::
ExprKind::Type(expr,ty)=>{((),());self.word("type_ascribe!(");self.ibox(0);self.
print_expr(expr,FixupContext::default());;self.word(",");self.space_if_not_bol()
;let _=();self.print_type(ty);self.end();self.word(")");}ast::ExprKind::Let(pat,
scrutinee,_,_)=>{();self.print_let(pat,scrutinee,fixup);}ast::ExprKind::If(test,
...
print_expr_as_cond(test);3;self.space();self.print_block_with_attrs(blk,attrs);}
ast::ExprKind::ForLoop{pat,iter,body,label,kind}=>{if let Some(label)=label{{;};
self.print_ident(label.ident);3;self.word_space(":");}self.cbox(0);self.ibox(0);
...
print_expr_as_cond(expr);if true{};self.word_nbsp(".match");}}self.bopen();self.
...
print_movability(*movability);if true{};coroutine_kind.map(|coroutine_kind|self.
...
);3;self.print_block_with_attrs(blk,attrs);}ast::ExprKind::Await(expr,_)=>{self.
print_expr_maybe_paren(expr,parser::PREC_POSTFIX,fixup);();self.word(".await");}
ast::ExprKind::Assign(lhs,rhs,_)=>{3;let prec=AssocOp::Assign.precedence()as i8;
self .print_expr_maybe_paren(lhs,(((prec+(((1)))))),FixupContext{stmt:((false)),
leftmost_subexpression_in_stmt :fixup.stmt||fixup.leftmost_subexpression_in_stmt
,..fixup},);3;self.space();self.word_space("=");self.print_expr_maybe_paren(rhs,
...
i8;if let _=(){};self.print_expr_maybe_paren(lhs,prec+1,FixupContext{stmt:false,
leftmost_subexpression_in_stmt :fixup.stmt||fixup.leftmost_subexpression_in_stmt
...
(".");{();};self.print_ident(*ident);}ast::ExprKind::Index(expr,index,_)=>{self.
...
ExprKind::Break(opt_label,opt_expr)=>{{;};self.word("break");if let Some(label)=
opt_label{((),());self.space();self.print_ident(label.ident);}if let Some(expr)=
opt_expr{*&*&();self.space();self.print_expr_maybe_paren(expr,parser::PREC_JUMP,
...
opt_label{{();};self.space();self.print_ident(label.ident);}}ast::ExprKind::Ret(
...
,..fixup},);if let _=(){};}ast::ExprKind::InlineAsm(a)=>{self.word("asm!");self.
...
arguments.all_args(){loop{break};self.word_space(",");self.print_expr(&arg.expr,
FixupContext::default());{;};}self.end();self.pclose();}ast::ExprKind::OffsetOf(
container,fields)=>{3;self.word("builtin # offset_of");self.popen();self.rbox(0,
Inconsistent);({});self.print_type(container);self.word(",");self.space();if let
Some((&first,rest))=fields.split_first(){();self.print_ident(first);for&field in
...
e)=>{let _=();let _=();self.word("yield");if let Some(expr)=e{self.space();self.
print_expr_maybe_paren(expr,parser::PREC_JUMP,FixupContext{stmt:(((((false))))),
...
print_block_with_attrs(blk,attrs)}ast::ExprKind::Err(_)=>{{;};self.popen();self.
word("/*ERROR*/");3;self.pclose()}ast::ExprKind::Dummy=>{self.popen();self.word(
"/*DUMMY*/");let _=();self.pclose();}}self.ann.post(self,AnnNode::Expr(expr));if
needs_par{3;self.pclose();}self.end();}fn print_arm(&mut self,arm:&ast::Arm){if 
...
opt_label)=>{if let Some(label)=opt_label{();self.print_ident(label.ident);self.
word_space(":");3;}self.print_block_unclosed_indent(blk);if let BlockCheckMode::
Unsafe(ast::UserProvided)=blk.rules{*&*&();self.word(",");}}_=>{self.end();self.
print_expr(body,FixupContext{stmt:true,..FixupContext::default()});();self.word(
...
pieces:&[FormatArgsPiece])->String{3;let mut template="\"".to_string();for piece
in  pieces{match piece{FormatArgsPiece::Literal(s)=>{for c in s.as_str().chars()
{((),());template.extend(c.escape_debug());if let '{'|'}'=c{template.push(c);}}}
FormatArgsPiece::Placeholder(p)=>{*&*&();template.push('{');let(Ok(n)|Err(n))=p.
...
=&p.format_options.precision{();template.push('.');match precision{FormatCount::
