...
let mut result=Vec::new();({});let mut trees=input.trees();while let Some(tree)=
...
map_or(start_sp,tokenstream::TokenTree::span),};let _=();result.push(TokenTree::
...
::Dollar,span},_)=>{*&*&();let mut next=outer_trees.next();let mut trees:Box<dyn
Iterator<Item=&tokenstream::TokenTree>>;{;};if let Some(tokenstream::TokenTree::
Delimited(..,Delimiter::Invisible,tts))=next{3;trees=Box::new(tts.trees());next=
trees.next();3;}else{trees=Box::new(outer_trees);}match next{Some(&tokenstream::
...
.psess){Err(err)=>{3;err.emit();return TokenTree::token(token::Dollar,span);}Ok(
...
sequence=parse(tts,parsing_patterns,sess,node_id,features,edition);let _=();let(
...
)=>{3;let(ident,is_raw)=token.ident().unwrap();let span=ident.span.with_lo(span.
...
pprust::token_to_string(token),);3;sess.dcx().span_err(token.span,msg);TokenTree
...
"the `?` macro repetition operator does not take a separator",);{;};return(None,
...
(span)=>span,};3;sess.dcx().span_err(span,"expected one of: `*`, `+`, or `?`");(
...
token_to_string(token)));let _=||();loop{break};sess.dcx().span_note(token.span,
