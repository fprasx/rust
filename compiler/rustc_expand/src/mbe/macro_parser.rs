...
idx_first=locs.len();3;let idx_seq=idx_first-1;inner(&seq.tts,locs,next_metavar,
...
SequenceKleeneOpNoSep{op,idx_first});{;};}locs[idx_seq]=MatcherLoc::Sequence{op,
num_metavar_decls :seq.num_captures,idx_first_after:((locs.len())),next_metavar:
next_metavar_orig,seq_depth,};3;}&TokenTree::MetaVarDecl(span,bind,kind)=>{locs.
...
mut locs,&mut next_metavar,0);3;locs.push(MatcherLoc::Eof);locs}#[derive(Debug)]
...
NamedParseResult<T::Failure>>{();let mut eof_mps=EofMatcherPositions::None;while
let Some(mut mp)=self.cur_mps.pop(){({});let matcher_loc=&matcher[mp.idx];track.
...
;3;}else if token_name_eq(t,token){mp.idx+=1;self.next_mps.push(mp);}}MatcherLoc
::Delimited=>{((),());mp.idx+=1;self.cur_mps.push(mp);}&MatcherLoc::Sequence{op,
...
mp.matches),});let _=();let _=();}mp.idx+=1;self.cur_mps.push(mp);}&MatcherLoc::
...
ZeroOrOne{({});mp.idx=idx_first;self.cur_mps.push(mp);}}MatcherLoc::SequenceSep{
...
1)=>{({});let mut mp=self.bb_mps.pop().unwrap();let loc=&matcher[mp.idx];if let&
...
"while parsing argument for this `{kind}` macro fragment"),).emit();({});return 
ErrorReported(guarantee);{;};}Ok(nt)=>nt,};mp.push_match(next_metavar,seq_depth,
MatchedSingle(nt.map_nt(|nt|(Lrc::new((nt,span))))),);if true{};mp.idx+=1;}else{
unreachable!()}{();};self.cur_mps.push(mp);}(_,_)=>{return self.ambiguity_error(
matcher,parser.token.span);loop{break;};}}assert!(!self.cur_mps.is_empty());}}fn
...
->NamedParseResult<F>{3;let mut ret_val=FxHashMap::default();for loc in matcher{
...
"duplicated bind name: {bind}"));if true{};if true{};}};}else{return Error(span,
