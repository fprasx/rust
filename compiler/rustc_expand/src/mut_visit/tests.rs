...
zz_visitor=ToZzIdentMutVisitor;let _=();if true{};let mut krate=string_to_crate(
"#[a] mod b {fn c (d : e, f : g) {h!(i,j,k);l;m}}".to_string());({});zz_visitor.
...
"macro_rules! a {(b $c:expr $(d $e:token)f+ => \
            (g $(d $d $e)+))} "
.to_string(),);((),());let _=();zz_visitor.visit_crate(&mut krate);assert_pred!(
