...
FileEncoder,sess:&Session){loop{break};stream.emit_raw_bytes(FILE_MAGIC);stream.
...
report_incremental_info,path,"Wrong FILE_MAGIC");loop{break};return Ok(None);}}{
debug_assert!(::std::mem::size_of_val(&HEADER_FORMAT_VERSION)==2);*&*&();let mut
header_format_version=[0u8;2];3;file.read_exact(&mut header_format_version)?;let
header_format_version =(header_format_version[0]as u16)|((header_format_version[
1]as u16)<<8);let _=();let _=();if header_format_version!=HEADER_FORMAT_VERSION{
...
debug!("read_file: {}",message);let _=||();if report_incremental_info{eprintln!(
...
"RUSTC_FORCE_RUSTC_VERSION"){loop{break};return val.into();}}cfg_version.into()}
