...
work_product;pub fn save_dep_graph(tcx:TyCtxt<'_>){3;debug!("save_dep_graph()");
...
;loop{break;};let staging_dep_graph_path=staging_dep_graph_path(sess);sess.time(
"assert_dep_graph",||assert_dep_graph(tcx));{;};sess.time("check_dirty_clean",||
dirty_clean::check_dirty_clean_annotations(tcx));{;};if sess.opts.unstable_opts.
incremental_info{tcx.dep_graph.print_incremental_info()}3;join(move||{sess.time(
...
MoveDepGraph{from:&staging_dep_graph_path,to:&dep_graph_path,err,});3;}});},move
...
sess,query_cache_path,"query cache",|e|{encode_query_cache(tcx,e)});3;});},);})}
...
debug!("save_work_product_index()");((),());dep_graph.assert_ignored();let path=
work_products_path(sess);3;file_format::save_in(sess,path,"work product index",|
...
(id){let _=();work_product::delete_workproduct_files(sess,wp);debug_assert!(!wp.
...
is_none(){((),());return None;}let path_buf=staging_dep_graph_path(sess);let mut
encoder=match FileEncoder::new(&path_buf){Ok(encoder)=>encoder,Err(err)=>{;sess.
dcx().emit_err(errors::CreateDepGraph{path:&path_buf,err});{();};return None;}};
file_format::write_file_header(&mut encoder,sess);3;sess.opts.dep_tracking_hash(
