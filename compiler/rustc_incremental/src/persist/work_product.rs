...
WorkProduct)>{3;debug!(?cgu_name,?files);sess.opts.incremental.as_ref()?;let mut
saved_files=UnordMap::default();{;};for(ext,path)in files{let file_name=format!(
"{cgu_name}.{ext}");;let path_in_incr_dir=in_incr_comp_dir_sess(sess,&file_name)
;();match link_or_copy(path,&path_in_incr_dir){Ok(_)=>{let _=saved_files.insert(
ext.to_string(),file_name);loop{break};}Err(err)=>{sess.dcx().emit_warn(errors::
CopyWorkProductToCache{from:path,to:&path_in_incr_dir,err,});loop{break;};}}}let
work_product=WorkProduct{cgu_name:cgu_name.to_string(),saved_files};{;};debug!(?
...
().into_sorted_stable_ord(){{;};let path=in_incr_comp_dir_sess(sess,path);if let
