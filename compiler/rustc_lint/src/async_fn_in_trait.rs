...
features().return_type_notation{();return;}if!cx.tcx.effective_visibilities(()).
is_reachable(item.owner_id.def_id){{;};return;}let hir::FnRetTy::Return(hir::Ty{
...
owner_id.def_id," + Send",);3;cx.tcx.emit_node_span_lint(ASYNC_FN_IN_TRAIT,item.
