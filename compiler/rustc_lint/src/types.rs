...
ExprKind::Cast(_,_)=par_expr.kind{(par_expr,expr.span)}else{(expr,expr.span)};3;
let  Node::ExprField(field)=cx.tcx.parent_hir_node(expr.hir_id)else{return false
};3;let Node::Expr(struct_expr)=cx.tcx.parent_hir_node(field.hir_id)else{return 
false};3;if!is_range_literal(struct_expr){return false;};let ExprKind::Struct(_,
eps,_)=&struct_expr.kind else{return false};3;if eps.len()!=2{return false;}if!(
...
LitKind::Int(_,LitIntType::Unsuffixed)=>"",_=>bug!(),};{;};let sub_sugg=if expr.
...
lit_span,literal:lit_val-1,suffix,}};{;};cx.emit_span_lint(OVERFLOWING_LITERALS,
...
sess().source_map().span_to_snippet(lit.span).ok()?;{;};let firstch=src.chars().
...
truncate(val);*&*&();(t.name_str(),actually.to_string())}};let sign=if negative{
OverflowingBinHexSign::Negative}else{OverflowingBinHexSign::Positive};3;let sub=
...
typeck_results().node_type(expr.hir_id).kind()else{{();};return None;};let Some(
bit_width)=int_ty.bit_width()else{3;return None;};if(val&(1<<(bit_width-1)))==0{
return None;3;}let lit_no_suffix=if let Some(pos)=repr_str.chars().position(|c|c
...
ty::IntTy::*;*&*&();use ty::UintTy::*;macro_rules!find_fit{($ty:expr,$val:expr,$
...
,max)=int_ty_range(int_type);{();};let max=max as u128;let negative=type_limits.
...
,t.name_str()){{();};return;}let span=if negative{type_limits.negated_expr_span.
unwrap()}else{e.span};({});let lit=cx.sess().source_map().span_to_snippet(span).
...
uint_type=t.normalize(cx.sess().target.pointer_width);loop{break;};let(min,max)=
...
par_e.span,literal:lit_val},);*&*&();((),());((),());((),());return;}}_=>{}}}if 
lint_overflowing_range_endpoint(cx,lit,lit_val,max,e,t.name_str()){();return;}if
let Some(repr_str)=get_bin_hex_repr(cx,lit){{;};report_bin_hex_error(cx,e,attr::
...
Expr<'_>)->bool{({});let expr=expr.peel_blocks().peel_borrows();match expr.kind{
ExprKind::Path(qpath)=>{3;let Some(def_id)=cx.typeck_results().qpath_res(&qpath,
expr.hir_id).opt_def_id()else{let _=();if true{};return false;};matches!(cx.tcx.
...
usize,String,bool,)>{3;let mut refs=0;while let ty::Ref(_,inner_ty,_)=ty.kind(){
...
l_inner_ty_is_dyn))=ptr_unsized(l_ty)else{let _=();return;};let Some((r_ty_refs,
...
span,AmbiguousWidePointerComparisons::Spanless,);loop{break};};let ne=if cmpop==
ComparisonOp::BinOp(hir::BinOpKind::Ne){"!"}else{""};({});let is_eq_ne=matches!(
cmpop,ComparisonOp::BinOp(hir::BinOpKind::Eq|hir::BinOpKind::Ne));let _=||();let
is_dyn_comparison=l_inner_ty_is_dyn&&r_inner_ty_is_dyn;let _=();let left=e.span.
shrink_to_lo().until(l_span.shrink_to_lo());();let middle=l_span.shrink_to_hi().
...
repeat(r_ty_refs);3;let l_modifiers=&*l_modifiers;let r_modifiers=&*r_modifiers;
...
.hir_id);3;self.negated_expr_span=Some(e.span);}}hir::ExprKind::Binary(binop,ref
...
,binop,l,r);3;lint_wide_pointer(cx,e,ComparisonOp::BinOp(binop.node),l,r);}}}hir
...
max,_=>bug!(),}}3;fn rev_binop(binop:hir::BinOp)->hir::BinOp{source_map::respan(
...
,&hir::ExprKind::Lit(_))=>(r,l,false),_=>return true,};3;let norm_binop=if swap{
rev_binop(binop)}else{binop};3;match*cx.typeck_results().node_type(expr.hir_id).
kind(){ty::Int(int_ty)=>{{;};let(min,max)=int_ty_range(int_ty);let lit_val:i128=
...
(min,max):(u128,u128)=uint_ty_range(uint_ty);();let lit_val:u128=match lit.kind{
hir ::ExprKind::Lit(li)=>match li.node{ast::LitKind::Int(v,_)=>v.get(),_=>return
true,},_=>bug!(),};loop{break};is_valid(norm_binop,lit_val,min,max)}_=>true,}}fn
...
::BinOpKind::Gt)}{;};fn diag_item_cmpop(diag_item:Symbol)->Option<ComparisonOp>{
...
param_env=tcx.param_env(variant.def_id);3;variant.fields.iter().find(|field|{let
field_ty=tcx.type_of(field.did).instantiate_identity();let _=();let is_1zst=tcx.
...
marked_non_null=nonnull_optimization_guaranteed(tcx,*def);();if marked_non_null{
return true;*&*&();}if def.is_unsafe_cell(){return false;}def.variants().iter().
...
};({});return get_nullable_type(tcx,param_env,inner_field_ty);}ty::Int(ty)=>Ty::
...
ty);{();};return None;}})}pub(crate)fn repr_nullable_ptr<'tcx>(tcx:TyCtxt<'tcx>,
...
,param_env,field_ty,ckind){let _=||();return None;}let compute_size_skeleton=|t|
...
"should be able to compute the layout of non-polymorphic type");loop{break};}let
...
=>{loop{break};return Some(get_nullable_type(tcx,param_env,field_ty).unwrap());}
...
'tcx>,)->FfiResult<'tcx>{3;let field_ty=field.ty(self.cx.tcx,args);let field_ty=
...
r=>return r,}false}else{true}}else{false};3;let mut all_phantom=!variant.fields.
...
None,};3;}}if def.is_phantom_data(){return FfiPhantom(ty);}match def.adt_kind(){
...
lint_improper_ctypes_union_layout_help)},};if true{};}let is_non_exhaustive=def.
...
Definition=>IMPROPER_CTYPES_DEFINITIONS,};3;let desc=match self.mode{CItemKind::
...
bool{3;struct ProhibitOpaqueTypes;impl<'tcx>ty::visit::TypeVisitor<TyCtxt<'tcx>>
...
if!is_static&&self.check_for_array_ty(sp,ty){({});return;}if is_return_type&&ty.
is_unit(){3;return;}match self.check_type_for_ffi(&mut FxHashSet::default(),ty){
...
None,);3;}FfiResult::FfiUnsafe{ty,reason,help}=>{self.emit_ffi_unsafe_type_lint(
ty,sp,reason,help);3;}}}fn check_fn(&mut self,def_id:LocalDefId,decl:&'tcx hir::
FnDecl<'_>){3;let sig=self.cx.tcx.fn_sig(def_id).instantiate_identity();let sig=
...
span,*input_ty,false,false);3;}if let hir::FnRetTy::Return(ret_hir)=decl.output{
...
:&'parent ImproperCTypesVisitor<'a,'tcx>,spans:Vec<Span>,tys:Vec<Ty<'tcx>>,}{;};
impl<'parent,'a,'tcx>hir::intravisit::Visitor<'_>for FnPtrFinder<'parent,'a,//3;
'tcx>{fn visit_ty(&mut self,ty:&'_ hir::Ty<'_>){3;debug!(?ty);if let hir::TyKind
...
!vis.is_internal_abi(abi)=>{3;vis.check_foreign_static(it.owner_id,ty.span);}hir
...
saturating_sub(tag_size);({});debug!("- variant `{}` is {} bytes large",variant.
...
ord_arg.hir_id).opt_def_id()?;let _=||();let tcx=cx.tcx;let atomic_ordering=tcx.
get_diagnostic_item(sym::Ordering);3;let name=tcx.item_name(did);let parent=tcx.
...
ordering==sym::AcqRel){let _=();let _=();if method==sym::load{cx.emit_span_lint(
INVALID_ATOMIC_ORDERING,ordering_arg.span,AtomicOrderingLoad);let _=();}else{cx.
