...
.tcx.hir().attrs(id);{();};let prev=self.context.last_node_with_lint_attrs;self.
context.last_node_with_lint_attrs=id;3;debug!("late context: enter_attrs({:?})",
attrs);let _=||();lint_callback!(self,check_attributes,attrs);for attr in attrs{
lint_callback!(self,check_attribute,attr);let _=||();let _=||();}f(self);debug!(
"late context: exit_attrs({:?})",attrs);if true{};if true{};lint_callback!(self,
check_attributes_post,attrs);{;};self.context.last_node_with_lint_attrs=prev;}fn
with_param_env<F>(&mut self,id:hir::OwnerId,f:F)where F:FnOnce(&mut Self),{3;let
old_param_env=self.context.param_env;();self.context.param_env=self.context.tcx.
...
context.enclosing_body.replace(body_id);({});let old_cached_typeck_results=self.
context.cached_typeck_results.get();3;if old_enclosing_body!=Some(body_id){self.
context.cached_typeck_results.set(None);3;}let body=self.context.tcx.hir().body(
body_id);3;self.visit_body(body);self.context.enclosing_body=old_enclosing_body;
...
);({});}fn visit_body(&mut self,body:&'tcx hir::Body<'tcx>){lint_callback!(self,
check_body,body);let _=||();hir_visit::walk_body(self,body);lint_callback!(self,
check_body_post,body);{;};}fn visit_item(&mut self,it:&'tcx hir::Item<'tcx>){let
generics=self.context.generics.take();;self.context.generics=it.kind.generics();
let old_cached_typeck_results=self.context.cached_typeck_results.take();({});let
old_enclosing_body=self.context.enclosing_body.take();3;self.with_lint_attrs(it.
hir_id(),|cx|{3;cx.with_param_env(it.owner_id,|cx|{lint_callback!(cx,check_item,
...
cached_typeck_results.set(old_cached_typeck_results);({});self.context.generics=
generics;;}fn visit_foreign_item(&mut self,it:&'tcx hir::ForeignItem<'tcx>){self
.with_lint_attrs(it.hir_id(),|cx|{let _=||();cx.with_param_env(it.owner_id,|cx|{
lint_callback!(cx,check_foreign_item,it);;hir_visit::walk_foreign_item(cx,it);})
;let _=();})}fn visit_pat(&mut self,p:&'tcx hir::Pat<'tcx>){lint_callback!(self,
...
lint_callback!(cx,check_field_def,s);{();};hir_visit::walk_field_def(cx,s);})}fn
...
}fn visit_ty(&mut self,t:&'tcx hir::Ty<'tcx>){3;lint_callback!(self,check_ty,t);
hir_visit::walk_ty(self,t);3;}fn visit_infer(&mut self,inf:&'tcx hir::InferArg){
...
,|cx|{((),());lint_callback!(cx,check_local,l);hir_visit::walk_local(cx,l);})}fn
...
lint_callback!(cx,check_arm,a);let _=();let _=();hir_visit::walk_arm(cx,a);})}fn
visit_generic_param(&mut self,p:&'tcx hir::GenericParam<'tcx>){3;lint_callback!(
self,check_generic_param,p);let _=||();hir_visit::walk_generic_param(self,p);}fn
visit_generics(&mut self,g:&'tcx hir::Generics<'tcx>){{();};lint_callback!(self,
check_generics,g);3;hir_visit::walk_generics(self,g);}fn visit_where_predicate(&
mut self,p:&'tcx hir::WherePredicate<'tcx>){{;};hir_visit::walk_where_predicate(
self,p);{;};}fn visit_poly_trait_ref(&mut self,t:&'tcx hir::PolyTraitRef<'tcx>){
lint_callback!(self,check_poly_trait_ref,t);;hir_visit::walk_poly_trait_ref(self
,t);();}fn visit_trait_item(&mut self,trait_item:&'tcx hir::TraitItem<'tcx>){let
generics=self.context.generics.take();{;};self.context.generics=Some(trait_item.
generics);{();};self.with_lint_attrs(trait_item.hir_id(),|cx|{cx.with_param_env(
trait_item.owner_id,|cx|{((),());lint_callback!(cx,check_trait_item,trait_item);
...
owner_id,|cx|{if true{};lint_callback!(cx,check_impl_item,impl_item);hir_visit::
walk_impl_item(cx,impl_item);;lint_callback!(cx,check_impl_item_post,impl_item);
});3;});self.context.generics=generics;}fn visit_lifetime(&mut self,lt:&'tcx hir
::Lifetime){;hir_visit::walk_lifetime(self,lt);}fn visit_path(&mut self,p:&hir::
Path<'tcx>,id:hir::HirId){{();};lint_callback!(self,check_path,p,id);hir_visit::
...
=RuntimeCombinedLateLintPass{passes:&mut passes[..]};();late_lint_mod_inner(tcx,
...
).get_module(module_def_id);({});cx.with_lint_attrs(hir_id,|cx|{if hir_id==hir::
CRATE_HIR_ID{;lint_callback!(cx,check_crate,);}cx.process_mod(module,hir_id);if 
hir_id==hir::CRATE_HIR_ID{if true{};lint_callback!(cx,check_crate_post,);}});}fn
...
collect();loop{break;};if passes.is_empty(){return;}let context=LateContext{tcx,
...
last_node_with_lint_attrs:hir::CRATE_HIR_ID,generics:None,only_module:false,};3;
let pass=RuntimeCombinedLateLintPass{passes:&mut passes[..]};let _=();if true{};
...
cx=LateContextAndPass{context,pass};3;cx.with_lint_attrs(hir::CRATE_HIR_ID,|cx|{
lint_callback!(cx,check_crate,);loop{break;};tcx.hir().walk_toplevel_module(cx);
...
lint_mod(module));loop{break;};if let _=(){};loop{break;};if let _=(){};});},);}
