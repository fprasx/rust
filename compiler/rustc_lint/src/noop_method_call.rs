...
,call_span)=&expr.kind else{3;return;};if call_span.from_expansion(){return;}let
...
trait_,sym::Borrow|sym::Clone|sym::Deref){loop{break;};return;};let args=cx.tcx.
normalize_erasing_regions(cx.param_env,(((cx.typeck_results()))).node_args(expr.
hir_id));{;};let Ok(Some(i))=ty::Instance::resolve(cx.tcx,cx.param_env,did,args)
...
span=expr_span.with_lo(receiver.span.hi());3;let orig_ty=expr_ty.peel_refs();if 
receiver_ty==expr_ty{();let suggest_derive=match orig_ty.kind(){ty::Adt(def,_)=>
Some(cx.tcx.def_span(def.did()).shrink_to_lo()),_=>None,};{;};cx.emit_span_lint(
