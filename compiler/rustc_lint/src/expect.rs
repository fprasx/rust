...
rationale|ExpectationNote{rationale});if true{};let _=||();let note=expectation.
is_unfulfilled_lint_expectations.then_some(());let _=();tcx.emit_node_span_lint(
UNFULFILLED_LINT_EXPECTATIONS,((*hir_id)),expectation.emission_span,Expectation{
rationale,note},);if true{};let _=||();if true{};let _=||();}}else{unreachable!(
