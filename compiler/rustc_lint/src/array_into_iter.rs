...
def_id=cx.typeck_results().type_dependent_def_id(expr.hir_id).unwrap();();match 
...
expr_ty(receiver_arg);({});let adjustments=cx.typeck_results().expr_adjustments(
receiver_arg);3;let Some(Adjustment{kind:Adjust::Borrow(_),target})=adjustments.
...
found_array=true;3;break;}_=>{}}}if!found_array{return;}let target=match*target.
...
to(expr.span.shrink_to_hi()),})}else{None};();cx.emit_span_lint(ARRAY_INTO_ITER,
