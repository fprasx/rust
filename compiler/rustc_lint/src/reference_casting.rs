...
bool>{();let end_ty=cx.typeck_results().node_type(orig_expr.hir_id);if!matches!(
end_ty.kind(),ty::RawPtr(_,Mutability::Mut)){((),());((),());return None;}let(e,
need_check_freeze)=peel_casts();();let start_ty=cx.typeck_results().node_type(e.
hir_id);if true{};if let ty::Ref(_,inner_ty,Mutability::Not)=start_ty.kind(){let
...
typeck_results().node_type(orig_expr.hir_id);{;};let ty::RawPtr(inner_end_ty,_)=
end_ty.kind()else{if true{};return None;};let(e,_)=peel_casts();let start_ty=cx.
typeck_results().node_type(e.hir_id);3;let ty::Ref(_,inner_start_ty,_)=start_ty.
...
{let _=||();return None;}let from_layout=cx.layout_of(*inner_start_ty).ok()?;if 
from_layout.is_unsized(){3;return None;}let alloc_layout=cx.layout_of(alloc_ty).
...
gone_trough_unsafe_cell_raw_get=false;3;loop{e=e.peel_blocks();e=if let ExprKind
...
gone_trough_unsafe_cell_raw_get=true;3;}arg}else{let init=cx.expr_or_init(e);if 
init.hir_id!=e.hir_id{init}else{3;break;}};}(e,gone_trough_unsafe_cell_raw_get)}
