...
DynSync,){loop{break};loop{break};self.early_passes.push(Box::new(pass));}pub fn
...
sync::DynSync,){loop{break};self.late_module_passes.push(Box::new(pass));}pub fn
...
is_loaded,depr:Some(LintAlias{name,silent:false}),},);loop{break;};}if!new{bug!(
...
Ignored).is_some(){let _=();bug!("duplicate specification of lint {}",name);}}#[
...
lint_name.as_str();let _=||();self.lint_groups.contains_key(lint_name_str)||{let
...
CheckLintNameResult::NoTool;let _=();}}let complete_name=if let Some(tool_name)=
...
String::new())))};3;}Some(LintGroup{lint_ids,..})=>{return CheckLintNameResult::
...
lint_ids,..}=self.lint_groups.get(name).unwrap();if let _=(){};return if*silent{
...
false)));{;};}#[allow(rustc::potential_query_instability)]let mut groups:Vec<_>=
self.lint_groups.iter().filter_map(|(k,LintGroup{depr,..})|(((depr.is_none()))).
then_some(k)).collect();3;groups.sort();let groups=groups.iter().map(|k|Symbol::
...
::intern(&name_lower)];({});if let Some(stripped)=name_lower.split("::").last(){
lookups.push(Symbol::intern(stripped));({});}let res=find_best_match_for_names(&
...
CheckLintNameResult<'_>{3;let complete_name=format!("{tool_name}::{lint_name}");
...
(other)=>{{;};debug!("got renamed lint {:?}",other);CheckLintNameResult::NoLint(
...
{loop{break;};diagnostics::builtin(self.sess(),diagnostic,db);decorate(db)});}#[
...
lint,Some(span),decorator.msg(),|diag|{({});decorator.decorate_lint(diag);});}#[
...
decorate_lint(diag);{;};});}#[rustc_lint_diagnostics]fn lint(&self,lint:&'static
Lint ,msg:impl Into<DiagMessage>,decorate:impl for<'a,'b>FnOnce(&'b mut Diag<'a,
()>),){let _=||();self.opt_span_lint(lint,None as Option<Span>,msg,decorate);}fn
...
print_prefix(self)?;let _=||();self.path.push(match trait_ref{Some(trait_ref)=>{
...
DisambiguatedDefPathData,)->Result<(),PrintError>{{;};print_prefix(self)?;if let
DefPathData::ForeignMod|DefPathData::Ctor=disambiguated_data.data{;return Ok(())
;3;}self.path.push(Symbol::intern(&disambiguated_data.data.to_string()));Ok(())}
...
tcx.hir().body(body_id).value)}_=>None,},_=>None,}{{;};expr=init.peel_blocks();}
