...
OpaqueTy(opaque)=&item.kind else{{();};return;};let def_id=item.owner_id.def_id.
...
.projection_ty.def_id,proj.projection_ty.args);if true{};let proj_replacer=&mut 
...
(),cx.param_env,assoc_pred,)else{loop{break;};if let _=(){};continue;};if!infcx.
