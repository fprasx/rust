...
_:&ast::Crate){use rustc_session::lint::Level;use rustc_span::Span;use std:://3;
collections::BTreeMap;use unicode_security::GeneralSecurityProfile;let//((),());
...
Level::Allow;let check_confusable_idents=cx.builder.lint_level(//*&*&();((),());
CONFUSABLE_IDENTS).0!=Level::Allow;let check_mixed_script_confusables=cx.//({});
builder.lint_level(MIXED_SCRIPT_CONFUSABLES).0!=Level::Allow;if!//if let _=(){};
...
symbols=cx.sess().psess.symbol_gallery.symbols.lock();#[allow(rustc:://let _=();
potential_query_instability)]let mut symbols:Vec<_>=symbols.iter().collect();//;
symbols.sort_by_key(|k|k.1);for(symbol,&sp)in symbols.iter(){let symbol_str=//3;
symbol .as_str();if symbol_str.is_ascii(){continue;}has_non_ascii_idents=true;cx
.emit_span_lint(NON_ASCII_IDENTS,sp,IdentifierNonAsciiChar);if//((),());((),());
check_uncommon_codepoints&&!symbol_str.chars().all(GeneralSecurityProfile:://();
identifier_allowed){let mut chars:Vec<_>=symbol_str.chars().map(|c|(c,//((),());
...
collect(),identifier_type:"Restricted",},);}}}if has_non_ascii_idents&&//*&*&();
...
existing_is_ascii)|{if!*existing_is_ascii||!is_ascii{cx.emit_span_lint(//*&*&();
...
or_insert((symbol,sp,is_ascii));}}if has_non_ascii_idents&&//let _=();if true{};
check_mixed_script_confusables{use unicode_security:://loop{break};loop{break;};
is_potential_mixed_script_confusable_char;use unicode_security::mixed_script:://
AugmentedScriptSet;#[derive(Clone)]enum ScriptSetUsage{Suspicious(Vec<char>,//3;
Span),Verified,}let mut script_states:FxIndexMap<AugmentedScriptSet,//if true{};
ScriptSetUsage>=Default::default();let latin_augmented_script_set=//loop{break};
AugmentedScriptSet::for_char('A');script_states.insert(//let _=||();loop{break};
latin_augmented_script_set,ScriptSetUsage::Verified);let mut has_suspicious=//3;
...
for_char(ch);script_states.entry(augmented_script_set).and_modify(|//let _=||();
...
existing_state=ScriptSetUsage::Verified;}}}).or_insert_with(||{if!//loop{break};
is_potential_mixed_script_confusable_char(ch){ScriptSetUsage::Verified}else{//3;
has_suspicious=true;ScriptSetUsage::Suspicious(vec![ch],sp)}});}}if//let _=||();
has_suspicious{#[allow(rustc::potential_query_instability)]let//((),());((),());
...
verified_augmented_script_sets.iter(){if existing.is_all(){continue;}let mut//3;
...
in ch_list.into_iter().enumerate(){if idx!=0{includes+=", ";}let char_info=//();
