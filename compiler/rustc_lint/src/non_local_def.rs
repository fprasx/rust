...
Body<'tcx>){3;self.body_depth+=1;}fn check_body_post(&mut self,_cx:&LateContext<
'tcx>,_body:&'tcx Body<'tcx>){3;self.body_depth-=1;}fn check_item(&mut self,cx:&
LateContext<'tcx>,item:&'tcx Item<'tcx>){if self.body_depth==0{{();};return;}let
parent=cx.tcx.parent(item.owner_id.def_id.into());();let parent_def_kind=cx.tcx.
...
Underscore){loop{break;};return;}let cargo_update=||{let oexpn=item.span.ctxt().
...
Typeof(_)|TyKind::Infer|TyKind::Err(_)=>false,};3;let of_trait_has_local_parent=
