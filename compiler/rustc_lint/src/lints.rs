...
Diag<'a,()>){({});diag.span_label(self.label,fluent::lint_label);rustc_session::
...
rustc_hir::intravisit::{self,Visitor};loop{break};struct WalkAssocTypes<'a,'b,G:
...
let mut visitor=WalkAssocTypes{err:diag};3;visitor.visit_ty(self.ty);}}#[derive(
...
self,diag:&'b mut Diag<'a,()>){({});diag.arg("ty",self.ty);diag.span_label(self.
...
err.message);((),());}if let Some(e)=err.nested{err=*e;}else{break;}}}}#[derive(
...
let mut expected_str=DiagStyledString::new();();expected_str.push(self.expected.
fn_sig(self.tcx).to_string(),false);3;let mut found_str=DiagStyledString::new();
found_str.push(self.found.fn_sig(self.tcx).to_string(),true);if let _=(){};diag.
...
,span)in self.spans{if true{};diag.span_label(span,format!("{c:?}"));}}}pub enum
...
SuggestionStyle::HideCodeAlways,);loop{break};diag.multipart_suggestion(fluent::
...
if can_suggest_binding{{;};let prefix=if self.is_assign_desugar{"let "}else{""};
...
lint_label);let _=();}NonSnakeCaseDiagSub::Help=>{diag.help(fluent::lint_help);}
...
,diag:&'b mut Diag<'a,()>){*&*&();diag.arg("predicate",self.predicate);diag.arg(
...
EmissionGuarantee ,F:SubdiagMessageOp<G>>(self,diag:&mut Diag<'_,G>,_f:F,){match
self{OverflowingBinHexSign::Positive=>{3;diag.note(fluent::lint_positive_note);}
...
arg("desc",self.desc);({});diag.span_label(self.label,fluent::lint_label);if let
Some(help)=self.help{();diag.help(help);}diag.note(self.note);if let Some(note)=
self.span_note{let _=();diag.span_note(note,fluent::lint_note);}}fn msg(&self)->
...
diag:&'b mut Diag<'a,()>){3;diag.arg("pre",self.pre);diag.arg("post",self.post);
diag.arg("def",self.cx.tcx.def_path_str(self.def_id));();if let Some(note)=self.
note{*&*&();diag.note(note.to_string());}if let Some(sugg)=self.suggestion{diag.
