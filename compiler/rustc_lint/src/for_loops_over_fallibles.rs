...
.span.lo()),end_span:pat.span.between(arg.span),};loop{break};cx.emit_span_lint(
...
lang_items().next_fn(){Some(recv)}else{3;return None;}}fn suggest_question_mark<
...
;3;}{let ty=cx.typeck_results().expr_ty(cx.tcx.hir().body(body_id).value);let ty
...
.hir().body_owner_def_id(body_id);if true{};let cause=ObligationCause::new(span,
body_def_id,rustc_infer::traits::ObligationCauseCode::MiscObligation,);({});ocx.
