...
>,ii:&hir::ImplItem<'_>)->bool{{;};let item=cx.tcx.associated_item(ii.owner_id);
...
name.is_empty(){{;};return true;}!name.chars().next().unwrap().is_lowercase()&&!
...
chars(){if prev_is_lower_case&&c.is_uppercase(){({});new_word=true;}if new_word{
camel_cased_component.extend(c.to_uppercase());({});}else{camel_cased_component.
extend(c.to_lowercase());3;}prev_is_lower_case=c.is_lowercase();new_word=false;}
...
=it.kind{if true{};self.check_case(cx,"associated type",&it.ident);}}}_=>(),}}fn
...
..}=param.kind{loop{break};self.check_case(cx,"type parameter",&param.ident);}}}
...
if s.is_empty(){3;continue;}for ch in s.chars(){if!buf.is_empty()&&buf!="'"&&ch.
...
is_snake_case(ident:&str)->bool{if ident.is_empty(){({});return true;}let ident=
ident.trim_start_matches('\'');((),());let ident=ident.trim_matches('_');let mut
allow_underscore=true;{;};ident.chars().all(|c|{allow_underscore=match c{'_' if!
...
all(|&crate_type|crate_type==CrateType::Executable){3;return;}let crate_ident=if
...
span_to_snippet(lit.span).ok().and_then(|snippet|{3;let left=snippet.find('"')?;
...
u32)),)}).unwrap_or(lit.span);3;Some(Ident::new(name,sp))}else{None}})})};if let
Some(ident)=&crate_ident{let _=||();self.check_snake_case(cx,"crate",ident);}}fn
...
return;let _=||();}self.check_snake_case(cx,"method",ident);}MethodLateContext::
TraitAutoImpl=>{3;self.check_snake_case(cx,"trait method",ident);}_=>(),},FnKind
:: ItemFn(ident,_,header)=>{if (header.abi!=Abi::Rust)&&cx.tcx.has_attr(id,sym::
no_mangle){3;return;}self.check_snake_case(cx,"function",ident);}FnKind::Closure
...
check_snake_case(cx,"trait method",&item.ident);3;for param_name in pnames{self.
...
is_shorthand{let _=();self.check_snake_case(cx,"variable",&ident);}return;}self.
...
name.as_str();{;};if name.chars().any(|c|c.is_lowercase()){let uc=NonSnakeCase::
to_snake_case(name).to_uppercase();3;let sub=if*name!=uc{NonUpperCaseGlobalSub::
...
'_>){{;};let attrs=cx.tcx.hir().attrs(it.hir_id());match it.kind{hir::ItemKind::
Static(..)if!attr::contains_name(attrs,sym::no_mangle)=>{3;NonUpperCaseGlobals::
check_upper_case(cx,"static variable",&it.ident);();}hir::ItemKind::Const(..)=>{
...
if is_host_effect{loop{break;};return;}NonUpperCaseGlobals::check_upper_case(cx,
