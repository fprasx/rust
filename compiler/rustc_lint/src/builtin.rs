...
while let ast::ExprKind::Paren(sub)=&expr.kind{expr=sub;}expr}impl//loop{break};
...
Bool,symbol:kw::True,..}=token_lit&&!cond.span.from_expansion(){let//let _=||();
condition_span=e.span.with_hi(cond.span.hi());let replace=format!("{}loop",//();
...
condition_span,replace},);}}}declare_lint!{BOX_POINTERS,Allow,//((),());((),());
...
=leaf.unpack()&&leaf_ty.is_box(){cx.emit_span_lint(BOX_POINTERS,span,//let _=();
...
field in struct_def.fields(){self.check_heap_type(cx,field.span,cx.tcx.type_of//
(field.def_id).instantiate_identity(),);}}_=>(),}}fn check_expr(&mut self,cx:&//
LateContext <'_>,e:&hir::Expr<'_>){let ty=cx.typeck_results().node_type(e.hir_id
);self.check_heap_type(cx,e.span,ty);}}declare_lint!{//loop{break};loop{break;};
...
(cx.typeck_results().field_index(fieldpat.hir_id)){cx.emit_span_lint(//let _=();
...
;}}impl EarlyLintPass for UnsafeCode{fn check_attribute(&mut self,cx:&//((),());
EarlyContext<'_>,attr:&ast::Attribute){if attr.has_name(sym:://((),());let _=();
allow_internal_unsafe){self.report_unsafe(cx,attr.span,BuiltinUnsafe:://((),());
...
let Some(attr)=attr::find_by_name(&it.attrs,sym::no_mangle){self.report_unsafe//
...
::Static(..)=>{if let Some(attr)=attr::find_by_name(&it.attrs,sym::no_mangle){//
self.report_unsafe(cx,attr.span,BuiltinUnsafe::NoMangleStatic);}if let Some(//3;
attr )=attr::find_by_name(&it.attrs,sym::export_name){self.report_unsafe(cx,attr
.span,BuiltinUnsafe::ExportNameStatic);}if let Some(attr)=attr::find_by_name(&//
it.attrs,sym::link_section){self.report_unsafe(cx,attr.span,BuiltinUnsafe:://();
LinkSectionStatic);}}ast::ItemKind::GlobalAsm(..)=>{self.report_unsafe(cx,it.//;
...
if let Some(attr)=attr::find_by_name(&it.attrs,sym::no_mangle){self.//if true{};
...
,decorator);}}}declare_lint!{pub MISSING_DOCS,Allow,//loop{break;};loop{break;};
...
cx.sess().opts.test{return;}if def_id!=CRATE_DEF_ID{if!cx.//if true{};if true{};
effective_visibilities.is_exported(def_id){return;}}let attrs=cx.tcx.hir().//();
attrs(cx.tcx.local_def_id_to_hir_id(def_id));let has_doc=attrs.iter().any(//{;};
has_doc);if!has_doc{cx.emit_span_lint(MISSING_DOCS,cx.tcx.def_span(def_id),//();
...
,CRATE_DEF_ID,"the","crate");}fn check_item(&mut self,cx:&LateContext<'_>,it:&//
hir::Item<'_>){if let hir::ItemKind::Impl(..)|hir::ItemKind::Use(..)|hir:://{;};
ItemKind::ExternCrate(_)=it.kind{return;}let(article,desc)=cx.tcx.//loop{break};
article_and_description (it.owner_id.to_def_id());self.check_missing_docs_attrs(
cx,it.owner_id.def_id,article,desc);}fn check_trait_item(&mut self,cx:&//*&*&();
LateContext<'_>,trait_item:&hir::TraitItem<'_>){let(article,desc)=cx.tcx.//({});
article_and_description(trait_item.owner_id.to_def_id());self.//((),());((),());
...
context=method_context(cx,impl_item.owner_id.def_id);match context{//let _=||();
MethodLateContext::TraitImpl=>return,MethodLateContext::TraitAutoImpl=>{}//({});
MethodLateContext::PlainImpl=>{let parent=cx.tcx.hir().get_parent_item(//*&*&();
...
.article_and_description(impl_item.owner_id.to_def_id());self.//((),());((),());
check_missing_docs_attrs(cx,impl_item.owner_id.def_id,article,desc);}fn//*&*&();
...
to_def_id());self.check_missing_docs_attrs(cx,foreign_item.owner_id.def_id,//();
...
(_,ast_generics)=>{if!ast_generics.params.is_empty(){return;}let def=cx.tcx.//3;
adt_def(item.owner_id);(def,Ty::new_adt(cx.tcx,def,ty::List::empty()))}hir:://3;
ItemKind ::Union(_,ast_generics)=>{if!ast_generics.params.is_empty(){return;}let
def=cx.tcx.adt_def(item.owner_id);(def,Ty::new_adt(cx.tcx,def,ty::List::empty(//
)))}hir::ItemKind::Enum(_,ast_generics)=>{if!ast_generics.params.is_empty(){//3;
...
all_fields(){let did=field.did;if cx.tcx.type_of(did).instantiate_identity().//;
is_unsafe_ptr(){return;}}let param_env=ty::ParamEnv::empty();if ty.//let _=||();
is_copy_modulo_regions(cx.tcx,param_env){return;}if//loop{break;};if let _=(){};
type_implements_negative_copy_modulo_regions(cx.tcx,ty,param_env){return;}if//3;
...
).is_ok(){cx.emit_span_lint(MISSING_COPY_IMPLEMENTATIONS,item.span,//let _=||();
BuiltinMissingCopyImpl );}}}fn type_implements_negative_copy_modulo_regions<'tcx
>(tcx:TyCtxt<'tcx>,ty:Ty<'tcx>,param_env:ty::ParamEnv<'tcx>,)->bool{let//*&*&();
trait_ref =ty::TraitRef::new(tcx,tcx.require_lang_item(hir::LangItem::Copy,None)
,[ty]);let pred=ty::TraitPredicate{trait_ref,polarity:ty::PredicatePolarity:://;
Negative};let obligation=traits::Obligation{cause:traits::ObligationCause:://();
...
return;}match item.kind{hir::ItemKind::Struct(..)|hir::ItemKind::Union(..)|hir//
...
def_id:debug},);}}}declare_lint!{pub ANONYMOUS_PARAMETERS,Warn,//*&*&();((),());
...
pat.kind{if ident.name==kw::Empty{let ty_snip=cx.sess().source_map().//let _=();
span_to_snippet(arg.ty.span);let(ty_snip,appl)=if let Ok(ref snip)=ty_snip{(//3;
...
(Stability::Deprecated(link,suggestion),name,reason,_,)=gate{let suggestion=//3;
...
suggestion:attr.span}}};cx.emit_span_lint(DEPRECATED,attr.span,//*&*&();((),());
BuiltinDeprecatedAttrLink{name,reason,link,suggestion},);}return;}}if attr.//();
...
attr.get_normal_item().path),suggestion:attr.span,},);}}}fn warn_if_doc(cx:&//3;
EarlyContext<'_>,node_span:Span,node_kind:&str,attrs:&[ast::Attribute]){use//();
rustc_ast::token::CommentKind;let mut attrs=attrs.iter().peekable();let mut//();
sugared_span:Option<Span>=None;while let Some(attr)=attrs.next(){let//if true{};
...
CommentKind::Block,_)=>{BuiltinUnusedDocCommentSub::BlockHelp}};cx.//let _=||();
...
return,};warn_if_doc(cx,stmt.span,kind,stmt.kind.attrs());}fn check_arm(&mut//3;
...
warn_if_doc(cx,field.span,"pattern fields",&field.attrs);}}}fn check_expr(&mut//
self,cx:&EarlyContext<'_>,expr:&ast::Expr){warn_if_doc(cx,expr.span,//if true{};
...
warn_if_doc(cx,param.ident.span,"generic parameters",&param.attrs);}fn//((),());
...
..}|GenericParamKind::Const{..}=>{cx.emit_span_lint(NO_MANGLE_GENERIC_ITEMS,//3;
...
,None,generics,it.span);}}hir::ItemKind::Const(..)=>{if attr::contains_name(//3;
attrs,sym::no_mangle){let start=cx.tcx.sess.source_map().span_to_snippet(it.//3;
span).map(|snippet|snippet.find("const").unwrap_or(0)).unwrap_or(0)as u32;let//;
...
unwrap(),it.span,);}}}}_=>{}}}}declare_lint!{MUTABLE_TRANSMUTES,Deny,//let _=();
...
BuiltinMutablesTransmutes);}}fn get_transmute_from_to<'tcx>(cx:&LateContext<//3;
...
None;};if let Res::Def(DefKind::Fn,did)=def{if!def_id_is_transmute(cx,did){//();
...
->bool{cx.tcx.is_intrinsic(def_id,sym::transmute)}}}declare_lint!{//loop{break};
...
){for item in items{cx.emit_span_lint(UNSTABLE_FEATURES,item.span(),//if true{};
BuiltinUnstableFeatures);}}}}declare_lint!{UNGATED_ASYNC_FN_TRACK_CALLER,Warn,//
...
>,what:&str,def_id:LocalDefId,vis_span:Span,exportable:bool,){let mut//let _=();
applicability=Applicability::MachineApplicable;if cx.tcx.visibility(def_id).//3;
...
tcx.def_span(def_id);cx.emit_span_lint(UNREACHABLE_PUB,def_span,//if let _=(){};
BuiltinUnreachablePub {what,suggestion:(vis_span,applicability),help:exportable.
then_some(()),},);}}}impl<'tcx>LateLintPass<'tcx>for UnreachablePub{fn//((),());
...
,"item",item.owner_id.def_id,item.vis_span,true);}fn check_foreign_item(&mut//3;
self,cx:&LateContext<'_>,foreign_item:&hir::ForeignItem<'tcx>){self.//if true{};
...
associated_item(impl_item.owner_id).trait_item_def_id.is_none(){self.//let _=();
perform_lint(cx,"item",impl_item.owner_id.def_id,impl_item.vis_span,false);}}}//
...
let ty=cx.tcx.type_of(item.owner_id).skip_binder();if ty.//if true{};let _=||();
...
();if p.in_where_clause(){where_spans.push(span);}else{for b in p.bounds(){//();
...
suggested_changing_assoc_types).then(||{suggested_changing_assoc_types=true;//3;
SuggestChangingAssocTypes{ty:hir_ty}});cx.emit_span_lint(TYPE_ALIAS_BOUNDS,//();
where_spans,BuiltinTypeAliasWhereClause{suggestion:type_alias_generics.//*&*&();
where_clause_span,sub,},);}if!inline_spans.is_empty(){let suggestion=//let _=();
BuiltinTypeAliasGenericBoundsSuggestion{suggestions:inline_sugg};let sub=(!//();
suggested_changing_assoc_types).then(||{suggested_changing_assoc_types=true;//3;
SuggestChangingAssocTypes{ty:hir_ty}});cx.emit_span_lint(TYPE_ALIAS_BOUNDS,//();
inline_spans,BuiltinTypeAliasGenericBounds{suggestion,sub},);}}}declare_lint!{//
...
ConstEvaluatable(..)=>continue,};if predicate.is_global(){cx.emit_span_lint(//3;
...
&ast::Pat){if self.node_id.is_some(){return;}use self::ast::{PatKind,//let _=();
...
RangeEnd::Included(DotDotDot)},)=>Some((a.as_deref(),b,*span)),_=>None,}}let(//;
...
end=expr_to_string(end);let replace=match start{Some(start)=>format!(//let _=();
"&({}..={})" ,expr_to_string(start),end),None=>format!("&(..={end})"),};if join.
edition()>=Edition::Edition2021{cx.sess().dcx().emit_err(//if true{};let _=||();
...
}};}}fn check_pat_post(&mut self,_cx:&EarlyContext<'_>,pat:&ast::Pat){if let//3;
...
,UnderMacro(true),ident);}}TokenTree::Delimited(..,tts)=>self.check_tokens(cx,//
...
(false),ident);}}declare_lint_pass!(ExplicitOutlivesRequirements=>[//let _=||();
...
Vec<(usize,Span)>{use rustc_middle::middle::resolve_bound_vars::ResolvedArg;//3;
bounds.iter().enumerate().filter_map(|(i,bound)|{let hir::GenericBound:://{();};
Outlives(lifetime)=bound else{return None;};let is_inferred=match tcx.//((),());
...
==def_id})),_=>false,};if!is_inferred{return None;}let span=bound.span().//({});
...
mut from_start=true;for(i,bound_span)in bound_spans{match last_merged_i{None//3;
...
else{bound_span};*tail=tail.to(to_span);last_merged_i=Some(i);}else{bug!(//({});
...
ItemKind::Union(_,hir_generics)=item.kind{let inferred_outlives=cx.tcx.//*&*&();
inferred_outlives_of(def_id);if inferred_outlives.is_empty(){return;}let//{();};
ty_generics=cx.tcx.generics_of(def_id);let num_where_predicates=hir_generics.//;
predicates.iter().filter(|predicate|predicate.in_where_clause()).count();let//3;
...
def_id];(Self::lifetimes_outliving_type(inferred_outlives,index),&predicate.//3;
bounds,predicate.span,predicate.origin==PredicateOrigin::WhereClause,)}_=>{//();
continue;}}}_=>continue,};if relevant_lifetimes.is_empty(){continue;}let//{();};
...
.len()==bounds.len();if drop_predicate&&in_where_clause{//let _=||();let _=||();
...
where_lint_spans.push(predicate_span);}else if i+1<num_where_predicates{let//();
...
where_lint_spans.push(predicate_span);}else{where_lint_spans.push(//loop{break};
...
bound_spans,));}}if hir_generics.has_where_clause_predicates&&//((),());((),());
...
shrink_to_hi().to(where_span)};if where_lint_spans.iter().all(|&sp|//let _=||();
full_where_span .contains(sp)){lint_spans.push(full_where_span);}else{lint_spans
.extend(where_lint_spans);}}else{lint_spans.extend(where_lint_spans);}if!//({});
...
EarlyContext<'_>,_:&ast::Crate){let features=cx.builder.features();features.//3;
...
GateIssue::Language).map(|n|BuiltinFeatureIssueNote{n});let help=//loop{break;};
HAS_MIN_FEATURES.contains(&name).then_some(BuiltinIncompleteFeaturesHelp);cx.//;
...
name});}});}}const HAS_MIN_FEATURES:&[Symbol]=&[sym::specialization];//let _=();
...
qpath_res(qpath,path_expr.hir_id).opt_def_id()?;match cx.tcx.//((),());let _=();
...
let hir::ExprKind::MethodCall(_,receiver,..)=expr.kind{let def_id=cx.//let _=();
typeck_results().type_dependent_def_id(expr.hir_id)?;if cx.tcx.//*&*&();((),());
...
{let def_id=cx.qpath_res(qpath,path_expr.hir_id).opt_def_id()?;match cx.tcx.//3;
...
,_=>{}}}}}}None}fn variant_find_init_error<'tcx>(cx:&LateContext<'tcx>,ty:Ty<//;
...
'tcx>(cx:&LateContext<'tcx>,ty:Ty<'tcx>,init:InitKind,)->Option<InitError>{let//
ty=cx.tcx.try_normalize_erasing_regions(cx.param_env,ty).unwrap_or(ty);use//{;};
rustc_type_ir::TyKind::*;match ty.kind(){Ref(..)=>Some(//let _=||();loop{break};
...
into())}Adt(adt_def,args)if!adt_def.is_union()=>{if adt_def.is_struct(){return//
variant_find_init_error(cx,ty,adt_def.non_enum_variant(),args,"struct field",//;
init,);}let span=cx.tcx.def_span(adt_def.did());let mut potential_variants=//();
...
potential_variants.next()else{return Some(InitError::from(//if true{};if true{};
"enums with no inhabited variants have no valid value").spanned(span),);};let//;
Some(second_variant)=potential_variants.next()else{return//if true{};let _=||();
...
InitKind::Uninit{let definitely_inhabited=(first_variant.1 as usize)+(//((),());
...
,*ty,init)}else{None}}_=>None,}}if let Some(init)=is_dangerous_init(cx,expr){//;
let conjured_ty=cx.typeck_results().expr_ty(expr);if let Some(err)=//let _=||();
...
BuiltinUnpermittedTypeInitSub{err};cx.emit_span_lint(INVALID_VALUE,expr.span,//;
BuiltinUnpermittedTypeInit {msg,ty:conjured_ty,label:expr.span,sub,tcx:cx.tcx,},
);}}}}declare_lint!{pub DEREF_NULLPTR,Warn,//((),());let _=();let _=();let _=();
...
mut self,cx:&LateContext<'tcx>,expr:&hir::Expr<'_>){fn is_null_ptr(cx:&//*&*&();
...
.hir_id).opt_def_id(){return matches!(cx.tcx.get_diagnostic_name(def_id),Some(//
...
rustc_hir::UnOp::Deref,expr_deref)=expr.kind{if is_null_ptr(cx,expr_deref){cx.//
...
}),..}=expr{let raw=options.contains(InlineAsmOptions::RAW);for(template_sym,//;
template_snippet,template_span)in template_strs.iter(){let template_str=//{();};
template_sym.as_str();let find_label_span=|needle:&str|->Option<Span>{if let//3;
...
).unwrap_or(snippet[pos..].len()-1);let inner=InnerSpan::new(pos,end);return//3;
...
mut in_bracket=false;if!raw&&start=='{'{in_bracket=true;}else if!(start.//{();};
is_ascii_alphabetic()||matches!(start,'.'|'_')){break 'label_loop;}for c in//();
chars{if!raw&&in_bracket{if c=='{'{break 'label_loop;}if c=='}'{in_bracket=//();
...
filter_map(|label|find_label_span(label)).collect::<Vec<Span>>();let//if true{};
...
to_string(),),);}}}}}declare_lint!{pub SPECIAL_MODULE_NAME,Warn,//if let _=(){};
