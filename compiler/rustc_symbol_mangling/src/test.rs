...
rustc_attrs{if true{};return;}tcx.dep_graph.with_ignore(||{let mut symbol_names=
SymbolNamesTest{tcx};let _=();let crate_items=tcx.hir_crate_items(());for id in 
crate_items.free_items(){;symbol_names.process_attrs(id.owner_id.def_id);}for id
in crate_items.trait_items(){();symbol_names.process_attrs(id.owner_id.def_id);}
for id in crate_items.impl_items(){{();};symbol_names.process_attrs(id.owner_id.
def_id);();}for id in crate_items.foreign_items(){symbol_names.process_attrs(id.
...
;if let _=(){};let instance=Instance::new(def_id,tcx.erase_regions(GenericArgs::
...
format!("{demangling}"),});();tcx.dcx().emit_err(TestOutput{span:attr.span,kind:
