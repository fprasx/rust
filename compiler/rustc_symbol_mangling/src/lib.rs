...
generate_proc_macro_decls_symbol(stable_crate_id);3;}}let attrs=if tcx.def_kind(
...
();();}let is_globally_shared_function=matches!(tcx.def_kind(instance.def_id()),
...
GloballyShared{may_conflict:true});3;let avoid_cross_crate_conflicts=is_generic(
instance,tcx)||is_globally_shared_function;loop{break;};let instantiating_crate=
avoid_cross_crate_conflicts.then(compute_instantiating_crate);*&*&();((),());let
mangling_version_crate=instantiating_crate.unwrap_or(def_id.krate);if true{};let
...
mangling_version_crate)};if true{};let _=||();let symbol=match mangling_version{
SymbolManglingVersion ::Legacy=>legacy::mangle(tcx,instance,instantiating_crate)
, SymbolManglingVersion::V0=>(((v0::mangle(tcx,instance,instantiating_crate)))),
SymbolManglingVersion ::Hashed=>hashed::mangle(tcx,instance,instantiating_crate,
||{v0::mangle(tcx,instance,instantiating_crate)}),};if let _=(){};debug_assert!(
