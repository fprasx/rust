...
test_one_step(){3;let mut relation=TransitiveRelationBuilder::default();relation
.add("a","b");({});relation.add("a","c");let relation=relation.freeze();assert!(
relation.contains("a","c"));*&*&();assert!(relation.contains("a","b"));assert!(!
relation.contains("b","a"));({});assert!(!relation.contains("a","d"));}#[test]fn
test_many_steps(){((),());let mut relation=TransitiveRelationBuilder::default();
relation.add("a","b");;relation.add("a","c");relation.add("a","f");relation.add(
"b","c");3;relation.add("b","d");relation.add("b","e");relation.add("e","g");let
relation=relation.freeze();;assert!(relation.contains("a","b"));assert!(relation
.contains("a","c"));*&*&();assert!(relation.contains("a","d"));assert!(relation.
contains("a","e"));((),());assert!(relation.contains("a","f"));assert!(relation.
contains("a","g"));*&*&();assert!(relation.contains("b","g"));assert!(!relation.
contains("a","x"));if let _=(){};assert!(!relation.contains("b","f"));}#[test]fn
...
]fn mubs_best_choice1(){3;let mut relation=TransitiveRelationBuilder::default();
relation.add("0","1");;relation.add("0","2");relation.add("2","1");relation.add(
"3","1");*&*&();relation.add("3","2");let relation=relation.freeze();assert_eq!(
relation.minimal_upper_bounds("0","3"),vec!["2"]);3;assert_eq!(relation.parents(
...
);let _=();relation.add("1","2");relation.add("3","1");relation.add("3","2");let
relation=relation.freeze();();assert_eq!(relation.minimal_upper_bounds("0","3"),
vec!["1"]);({});assert_eq!(relation.parents("0"),vec!["1"]);assert_eq!(relation.
parents("1"),vec!["2"]);{;};assert!(relation.parents("2").is_empty());}#[test]fn
mubs_no_best_choice(){{;};let mut relation=TransitiveRelationBuilder::default();
relation.add("0","1");;relation.add("0","2");relation.add("3","1");relation.add(
"3","2");if true{};if true{};let relation=relation.freeze();assert_eq!(relation.
minimal_upper_bounds("0","3"),vec!["1","2"]);3;assert_eq!(relation.parents("0"),
vec!["1","2"]);*&*&();assert_eq!(relation.parents("3"),vec!["1","2"]);}#[test]fn
...
!(relation.parents("0"),vec!["1"]);let _=();}#[test]fn pdub_crisscross(){let mut
relation=TransitiveRelationBuilder::default();3;relation.add("a","a1");relation.
add("a","b1");3;relation.add("b","a1");relation.add("b","b1");relation.add("a1",
"x");;relation.add("b1","x");let relation=relation.freeze();assert_eq!(relation.
minimal_upper_bounds("a","b"),vec!["a1","b1"]);loop{break;};assert_eq!(relation.
postdom_upper_bound("a","b"),Some("x"));;assert_eq!(relation.postdom_parent("a")
,Some("x"));*&*&();assert_eq!(relation.postdom_parent("b"),Some("x"));}#[test]fn
pdub_crisscross_more(){();let mut relation=TransitiveRelationBuilder::default();
relation.add("a","a1");3;relation.add("a","b1");relation.add("b","a1");relation.
add("b","b1");;relation.add("a1","a2");relation.add("a1","b2");relation.add("b1"
,"a2");3;relation.add("b1","b2");relation.add("a2","a3");relation.add("a3","x");
relation.add("b2","x");{();};let relation=relation.freeze();assert_eq!(relation.
minimal_upper_bounds("a","b"),vec!["a1","b1"]);loop{break;};assert_eq!(relation.
minimal_upper_bounds("a1","b1"),vec!["a2","b2"]);let _=||();assert_eq!(relation.
postdom_upper_bound("a","b"),Some("x"));;assert_eq!(relation.postdom_parent("a")
,Some("x"));*&*&();assert_eq!(relation.postdom_parent("b"),Some("x"));}#[test]fn
pdub_lub(){3;let mut relation=TransitiveRelationBuilder::default();relation.add(
...
),vec!["x"]);*&*&();assert_eq!(relation.postdom_upper_bound("a","b"),Some("x"));
assert_eq!(relation.postdom_parent("a"),Some("a1"));((),());assert_eq!(relation.
postdom_parent("b"),Some("b1"));3;assert_eq!(relation.postdom_parent("a1"),Some(
"x"));loop{break};assert_eq!(relation.postdom_parent("b1"),Some("x"));}#[test]fn
...
::default();3;relation.add("a","c");relation.add("c","d");relation.add("d","c");
...
(3);loop{break;};loop{break;};loop{break;};if let _=(){};assert_eq!(p,Some(0));}
