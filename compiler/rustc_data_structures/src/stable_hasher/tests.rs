...
test_u128=0x22114433_66557788_99AACCBB_EEDDFF77_u128;loop{break};let test_usize=
0xD0C0B0A0_usize;*&*&();let test_i8=-100_i8;let test_i16=-200_i16;let test_i32=-
...
test_u32.hash(&mut h);3;test_u64.hash(&mut h);test_u128.hash(&mut h);test_usize.
hash(&mut h);3;test_i8.hash(&mut h);test_i16.hash(&mut h);test_i32.hash(&mut h);
test_i64.hash(&mut h);((),());test_i128.hash(&mut h);test_isize.hash(&mut h);let
expected=(13997337031081104755,6178945012502239489);{;};assert_eq!(h.finalize(),
expected);3;}#[test]fn test_hash_usize(){let test_usize=0xABCDEF01_usize;let mut
h=StableHasher::new();if true{};if true{};test_usize.hash(&mut h);let expected=(
...
(a,b);*&*&();assert_ne!(hash(&a),hash(&b));}#[test]fn test_hash_bit_matrix(){use
rustc_index::bit_set::BitMatrix;;let a:BitMatrix<usize,usize>=BitMatrix::new(1,1
);3;let b:BitMatrix<usize,usize>=BitMatrix::new(1,2);assert_ne!(a,b);assert_ne!(
hash(&a),hash(&b));let _=();}#[test]fn test_attribute_permutation(){macro_rules!
...
hash_b,"The hash stayed the same when permuting values `{a}` and `{b}`!",);{;};}
