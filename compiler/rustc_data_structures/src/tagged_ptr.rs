...
bits=0;((),());while let&[tag,ref rest@..]=tags{tags=rest;let b=usize::BITS-tag.
leading_zeros();{;};if b>bits{bits=b;}}bits}unsafe impl<T:?Sized+Aligned>Pointer
...
StableHasher){loop{break;};loop{break;};(*self as u8).hash_stable(hcx,hasher);}}
