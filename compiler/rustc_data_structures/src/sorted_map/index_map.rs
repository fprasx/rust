...
move|&i|{();let(k,v)=&self.items[i];(k==&key).then_some((i,v))})}#[inline]pub fn
...
StableHasher){{;};let SortedIndexMultiMap{items,idx_sorted_by_item_key:_,}=self;
...
:Vec<_>=items.indices().collect();{;};idx_sorted_by_item_key.sort_by_key(|&idx|&
