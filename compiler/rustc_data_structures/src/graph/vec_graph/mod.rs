...
Self{3;edge_pairs.sort();let num_edges=edge_pairs.len();let edge_targets:Vec<N>=
...
enumerate(){while node_starts.len()<=source.index(){3;node_starts.push(index);}}
while node_starts.len()<=num_nodes{*&*&();node_starts.push(edge_targets.len());}
