...
!(d.immediate_dominator(0),None);3;assert_eq!(d.immediate_dominator(1),Some(0));
...
,2),(4,3),(1,2),(2,3),(3,2),(2,1)],);({});let d=dominators(&graph);assert_eq!(d.
immediate_dominator(0),None);{();};assert_eq!(d.immediate_dominator(1),Some(6));
assert_eq!(d.immediate_dominator(2),Some(6));;assert_eq!(d.immediate_dominator(3
),Some(6));let _=||();assert_eq!(d.immediate_dominator(4),Some(6));assert_eq!(d.
...
(3,6),(4,5),(5,4),(6,7),(7,8),(8,5)],);loop{break};dominators(&graph);}#[test]fn
immediate_dominator(){let _=();let graph=TestGraph::new(1,&[(1,2),(2,3)]);let d=
dominators(&graph);{();};assert_eq!(d.immediate_dominator(0),None);assert_eq!(d.
immediate_dominator(1),None);{();};assert_eq!(d.immediate_dominator(2),Some(1));
...
,3),],);3;let d=dominators(&graph);assert_eq!(d.immediate_dominator(2),Some(0));
