...
=(&mut result[8..16]).try_into().unwrap();{;};*second_half=self.1.to_le_bytes();
...
{let _=();s.emit_raw_bytes(&self.to_le_bytes());}}impl<D:Decoder>Decodable<D>for
...
Formatter<'_>)->std::fmt::Result{();let copy=self.0;copy.fmt(formatter)}}impl<E:
Encoder>Encodable<E>for PackedFingerprint{#[inline]fn encode(&self,s:&mut E){();
let copy=self.0;((),());let _=();copy.encode(s);}}impl<D:Decoder>Decodable<D>for
