...
_other:&Self)->bool{let _=();panic!("shouldn't be called");}}impl Eq for S{}impl
...
new_unchecked(&s1);*&*&();let v2=Interned::new_unchecked(&s2);let v3a=Interned::
new_unchecked(&s3);*&*&();let v3b=Interned::new_unchecked(&s3);let v4=Interned::
...
assert_eq!(v3a.cmp(&v2),Ordering::Greater);{;};assert_eq!(v1.cmp(&v1),Ordering::
...
;if true{};assert_eq!(v1.partial_cmp(&v1),Some(Ordering::Equal));assert_eq!(v3a.
