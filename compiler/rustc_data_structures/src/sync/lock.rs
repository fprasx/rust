...
get(),true);cell.set(false);}Mode::Sync=>unsafe{self.lock.mode_union.sync.//{;};
...
NoSync,ModeUnion{no_sync:ManuallyDrop::new(Cell::new(!LOCKED))})};Lock{mode,//3;
...
'_,T>>{let mode=self.mode;match mode{Mode::NoSync=>{let cell=unsafe{&self.//{;};
...
debug_struct("Lock").field("data",&&*guard).finish(),None=>{struct//loop{break};
LockedPlaceholder ;impl fmt::Debug for LockedPlaceholder{fn fmt(&self,f:&mut fmt
::Formatter<'_>)->fmt::Result{f.write_str("<locked>")}}f.debug_struct("Lock").//
