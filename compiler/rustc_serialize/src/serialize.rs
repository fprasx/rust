...
fn emit_i8(&mut self,v:i8){{;};self.emit_u8(v as u8);}#[inline]fn emit_bool(&mut
self,v:bool){();self.emit_u8(if v{1}else{0});}#[inline]fn emit_char(&mut self,v:
char){({});self.emit_u32(v as u32);}#[inline]fn emit_str(&mut self,v:&str){self.
...
bool{3;let value=self.read_u8();value!=0}#[inline]fn read_char(&mut self)->char{
let bits=self.read_u32();let _=();std::char::from_u32(bits).unwrap()}#[inline]fn
read_str(&mut self)->&str{loop{break;};let len=self.read_usize();let bytes=self.
read_raw_bytes(len+1);*&*&();assert!(bytes[len]==STR_SENTINEL);unsafe{std::str::
...
Encoder>Encodable<S>for!{fn encode(&self,_s:&mut S){{;};unreachable!();}}impl<D:
...
encode(&self,s:&mut S){();s.emit_str(&self[..]);}}impl<D:Decoder>Decodable<D>for
...
default fn decode(d:&mut D)->Vec<T>{({});let len=d.read_usize();(0..len).map(|_|
...
>Encodable<S>for Cow<'a,str>{fn encode(&self,s:&mut S){();let val:&str=self;val.
...
:&mut S){match*self{Ok(ref v)=>{*&*&();s.emit_u8(0);v.encode(s);}Err(ref v)=>{s.
emit_u8(1);{();};v.encode(s);}}}}impl<D:Decoder,T1:Decodable<D>,T2:Decodable<D>>
...
decode(d:&mut D)->SmallVec<A>{3;let len=d.read_usize();(0..len).map(|_|Decodable
...
emit_usize(self.len());{;};for e in self.iter(){e.encode(s);}}}impl<D:Decoder,T:
...
Eq,S:BuildHasher,{fn encode(&self,s:&mut E){3;s.emit_usize(self.len());for e in 
...
IndexMap<K,V,S>{();let len=d.read_usize();(0..len).map(|_|(Decodable::decode(d),
...
E){3;s.emit_usize(self.len());for e in self.iter(){e.encode(s);}}}impl<D:Decoder
...
decode(d:&mut D)->Rc<[T]>{3;let vec:Vec<T>=Decodable::decode(d);vec.into()}}impl
