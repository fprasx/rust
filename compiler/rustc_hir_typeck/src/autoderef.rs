...
Vec<Adjustment<'tcx>>>{3;let steps=autoderef.steps();if steps.is_empty(){return 
InferOk{obligations:vec![],value:vec![]};((),());}let mut obligations=vec![];let
targets =(((steps.iter()).skip(1)).map(|&(ty,_)|ty)).chain(iter::once(autoderef.
final_ty(false)));({});let steps:Vec<_>=steps.iter().map(|&(source,kind)|{if let
