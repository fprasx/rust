...
;3;let method=self.register_infer_ok_obligations(ok);if let ty::Ref(region,_,hir
...
>)>{loop{break};let ty=self.resolve_vars_if_possible(ty);let mut err=self.dcx().
...
,format!("{snippet}.len() "),Applicability::MachineApplicable,);3;}let reported=
...
,index_expr.span,base_expr);let _=();}ty::Slice(_)|ty::Array(_,_)=>{return self.
...
.span,self_ty,&[input_ty],PlaceOp::Index);{;};if let Some(result)=method{debug!(
"try_index_step: success, using overloaded indexing");if true{};let method=self.
register_infer_ok_obligations(result);{;};let mut adjustments=self.adjust_steps(
...
),target:method.sig.inputs()[0],});let _=||();}self.apply_adjustments(base_expr,
adjustments);3;self.write_method_call_and_enforce_effects(expr.hir_id,expr.span,
...
debug!("try_overloaded_place_op({:?},{:?},{:?})",span,base_ty,op);({});let(Some(
...
else{if true{};return None;};self.lookup_method_in_trait(self.misc(span),Ident::
...
"try_mutable_overloaded_place_op({:?},{:?},{:?})",span,base_ty,op);{;};let(Some(
mut_tr ),mut_op)=(match op{PlaceOp::Deref=>((((((((self.tcx.lang_items()))))))).
deref_mut_trait (),sym::deref_mut),PlaceOp::Index=>((((self.tcx.lang_items()))).
index_mut_trait(),sym::index_mut),})else{if true{};if true{};return None;};self.
...
last().unwrap().kind{((),());let _=();((),());let _=();exprs.push(expr);}debug!(
"convert_place_derefs_to_mutable: exprs={:?}",exprs);;let mut inside_union=false
;loop{break;};loop{break;};for(i,&expr)in exprs.iter().rev().enumerate(){debug!(
"convert_place_derefs_to_mutable: i={} expr={:?}",i,expr);3;let mut source=self.
node_ty(expr.hir_id);({});if matches!(expr.kind,hir::ExprKind::Unary(hir::UnOp::
Deref,_)){*&*&();inside_union=false;}if source.is_union(){inside_union=true;}let
...
::Deref,){({});let method=self.register_infer_ok_obligations(ok);if let ty::Ref(
...
).emit();if true{};}}source=adjustment.target;}self.typeck_results.borrow_mut().
...
base_expr);let _=();if!self.typeck_results.borrow().is_method_call(expr){debug!(
...
expect("place op takes something that is not a ref").ty;{;};let arg_ty=match op{
PlaceOp::Deref=>None,PlaceOp::Index=>{Some(((((self.typeck_results.borrow())))).
node_args(expr.hir_id).type_at(1))}};3;let arg_tys=arg_ty.as_slice();let method=
self.try_mutable_overloaded_place_op(expr.span,base_ty,arg_tys,op);3;let method=
match method{Some(ok)=>self.register_infer_ok_obligations(ok),None=>return,};();
...
expr.span,"input to mutable place op is not a mut ref?");{;};};let base_expr_ty=
...
"convert_place_op_to_mutable: converting autoref {:?}",adjustment);();let mutbl=
AutoBorrowMutability::Mut{allow_two_phase_borrow:AllowTwoPhase::No,};;adjustment
.kind=Adjust::Borrow(AutoBorrow::Ref(*region,mutbl));({});adjustment.target=Ty::
