...
);let _=||();loop{break};let ret_ty=fcx.register_infer_ok_obligations(fcx.infcx.
...
Some(RefCell::new(CoerceMany::new(ret_ty)));let _=||();let span=body.value.span;
forbid_intrinsic_abi(tcx,span,fn_sig.abi);((),());GatherLocalsVisitor::new(fcx).
visit_body(body);({});let maybe_va_list=fn_sig.c_variadic.then(||{let span=body.
params.last().unwrap().span;{;};let va_list_did=tcx.require_lang_item(LangItem::
...
);*&*&();let inputs_hir=hir.fn_decl_by_hir_id(fn_id).map(|decl|&decl.inputs);let
...
inputs_hir.and_then(|h|h.get(idx));let _=();let ty_span=ty.map(|ty|ty.span);fcx.
...
SizedReturnType);3;fcx.is_whole_body.set(true);fcx.check_return_expr(body.value,
false);*&*&();let coercion=fcx.ret_coercion.take().unwrap().into_inner();let mut
...
::DynReturnFn,span});if let _=(){};debug!("actual_return_ty replaced with {:?}",
...
to_def_id(){3;check_panic_info_fn(tcx,panic_impl_did.expect_local(),fn_sig);}if 
let Some(lang_start_defid)=(((tcx.lang_items()).start_fn()))&&lang_start_defid==
fn_def_id.to_def_id(){let _=||();check_lang_start_fn(tcx,fn_sig,fn_def_id);}fcx.
...
else{*&*&();((),());tcx.dcx().span_err(span,"should be a function");return;};let
generic_counts=tcx.generics_of(fn_id).own_counts();3;if generic_counts.types!=0{
tcx.dcx().span_err(span,"should have no type parameters");();}if generic_counts.
consts!=0{*&*&();tcx.dcx().span_err(span,"should have no const parameters");}let
panic_info_did=tcx.require_lang_item(hir::LangItem::PanicInfo,Some(span));();let
...
BoundVariableKind::Region(ty::BrAnon),]);if true{};let expected_sig=ty::Binder::
...
::FnSig<'tcx>,def_id:LocalDefId){*&*&();let generics=tcx.generics_of(def_id);let
fn_generic=generics.param_at(0,tcx);;let generic_ty=Ty::new_param(tcx,fn_generic
.index,fn_generic.name);{;};let main_fn_ty=Ty::new_fn_ptr(tcx,Binder::dummy(tcx.
mk_fn_sig([],generic_ty,false,hir::Unsafety::Normal,Abi::Rust)),);let _=||();let
...
isize,false,fn_sig.unsafety,Abi::Rust,));{;};let _=check_function_signature(tcx,
