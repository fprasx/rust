...
ident,self_ty,call_expr,self_expr);({});if span.at_least_rust_2021(){return;}let
...
item.def_id.krate),sym::std|sym::core){{;};return;}if matches!(pick.kind,probe::
...
pick.autoref_or_ptr_adjustment.is_none(){*&*&();return;}self.tcx.node_span_lint(
...
unwrap_or_default();if let _=(){};write!(string,", {}",self.sess().source_map().
span_to_snippet(span).unwrap()).unwrap();{();};string});lint.span_suggestion(sp,
...
ident,),);();}},);}}pub(super)fn lint_fully_qualified_call_from_2018(&self,span:
...
crate_name(pick.item.def_id.krate),sym::std|sym::core){3;return;}if method_name.
...
(){3;return;}}}if matches!(pick.kind,probe::PickKind::InherentImplPick){return;}
...
.tcx.generics_of(container_id);3;let trait_name=if trait_generics.params.len()<=
...
trait_generics.has_self as usize)).collect::<Vec<_>>().join(", "))};({});let mut
...
;();if!self_ty_name.contains('<'){if let Adt(def,_)=self_ty.kind(){let generics=
self.tcx.generics_of(def.did());*&*&();if!generics.params.is_empty(){let counts=
generics.own_counts();{;};self_ty_name+=&format!("<{}>",std::iter::repeat("'_").
take(counts.lifetimes).chain(std::iter::repeat("_").take(counts.types+counts.//;
consts)).collect::<Vec<_>>().join(", "));if true{};}}}lint.span_suggestion(span,
...
String>{((),());let applicable_traits=self.tcx.in_scope_traits(expr_hir_id)?;let
applicable_trait=applicable_traits.iter().find(|t|t.def_id==trait_def_id)?;3;if 
applicable_trait.import_ids.is_empty(){{;};return None;}let import_items:Vec<_>=
applicable_trait.import_ids.iter().map(|&import_id|(self.tcx.hir()).expect_item(
import_id)).collect();{;};let any_id=import_items.iter().find_map(|item|if item.
ident.name!=Underscore{Some(item.ident)}else{None});;if let Some(any_id)=any_id{
if any_id.name==Empty{({});return None;}else{return Some(format!("{any_id}"));}}
...
span_to_snippet(span).ok()){(expr_text,true)}else{("(..)".to_string(),false)};3;
