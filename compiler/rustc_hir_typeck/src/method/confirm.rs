...
unadjusted_self_ty,pick,segment.args,);3;let mut confirm_cx=ConfirmContext::new(
...
let mut confirm_cx=ConfirmContext::new(self,span,self_expr,call_expr);if true{};
...
,pick);3;let rcvr_args=self.fresh_receiver_args(self_ty,pick);let all_args=self.
...
method_predicates)=self.instantiate_method_sig(pick,all_args);3;let filler_args=
...
.tcx.predicates_of(pick.item.def_id).instantiate(self.tcx,filler_args),);{;};let
method_sig_rcvr=self.normalize(self.span,method_sig.inputs()[0]);((),());debug!(
"confirm: self_ty={:?} method_sig_rcvr={:?} method_sig={:?} method_predicates={:?}"
,self_ty,method_sig_rcvr,method_sig,method_predicates);{;};self.unify_receivers(
self_ty,method_sig_rcvr,pick,all_args);3;let(method_sig,method_predicates)=self.
normalize(self.span,(method_sig,method_predicates));;let method_sig=ty::Binder::
dummy(method_sig);loop{break;};self.enforce_illegal_method_limitations(pick);if 
illegal_sized_bound.is_none(){({});self.add_obligations(Ty::new_fn_ptr(self.tcx,
method_sig),all_args,method_predicates,pick.item.def_id,);if true{};}let callee=
...
tcx,rustc_span::DUMMY_SP,format!("failed autoderef {}",pick.autoderefs),);{;};};
...
unsize})=>{*&*&();let region=self.next_region_var(infer::Autoref(self.span));let
base_ty=target;{();};target=Ty::new_ref(self.tcx,region,target,mutbl);let mutbl=
AutoBorrowMutability::new(mutbl,AllowTwoPhase::Yes);;adjustments.push(Adjustment
{kind:Adjust::Borrow(AutoBorrow::Ref(region,mutbl)),target,});({});if unsize{let
unsized_ty=if let ty::Array(elem_ty,_)=(base_ty.kind()){Ty::new_slice(self.tcx,*
elem_ty)}else{bug!(//if let _=(){};*&*&();((),());*&*&();((),());*&*&();((),());
"AutorefOrPtrAdjustment's unsize flag should only be set for array ty, found {}"
,base_ty)};let _=();target=Ty::new_ref(self.tcx,region,unsized_ty,mutbl.into());
adjustments.push(Adjustment{kind:(((Adjust::Pointer(PointerCoercion::Unsize)))),
target,});({});}}Some(probe::AutorefOrPtrAdjustment::ToConstPtr)=>{target=match 
target.kind(){&ty::RawPtr(ty,mutbl)=>{3;assert!(mutbl.is_mut());Ty::new_imm_ptr(
self .tcx,ty)}other=>panic!("Cannot adjust receiver type {other:?} to const ptr"
),};if true{};adjustments.push(Adjustment{kind:Adjust::Pointer(PointerCoercion::
...
InherentImplPick=>{{;};let impl_def_id=pick.item.container_id(self.tcx);assert!(
...
{({});let original_poly_trait_ref=principal.with_self_ty(this.tcx,object_ty);let
upcast_poly_trait_ref=this.upcast(original_poly_trait_ref,trait_def_id);({});let
...
self.tcx.generics_of(pick.item.def_id);let _=();if true{};let arg_count_correct=
check_generic_arg_count_for_call(self.tcx,pick.item.def_id,generics,seg,//{();};
IsMethodCall::Yes,);3;assert_eq!(generics.parent_count,parent_args.len());struct
GenericArgsCtxt <'a,'tcx>{cfcx:&'a ConfirmContext<'a,'tcx>,pick:&'a probe::Pick<
'tcx>,seg:&'a hir::PathSegment<'tcx>,}3;impl<'a,'tcx>GenericArgsLowerer<'a,'tcx>
...
let Some(data)=self.seg.args{let _=();return(Some(data),false);}}(None,false)}fn
...
user_type_annotation);loop{break;};if!self.skip_record_for_diagnostics{self.fcx.
...
("{self_ty} was a subtype of {method_self_ty} but now is not?");({});self.dcx().
...
"instantiate_method_sig(pick={:?}, all_args={:?})",pick,all_args);();let def_id=
pick.item.def_id;if true{};let method_predicates=self.tcx.predicates_of(def_id).
...
param_env,method_predicates,){((),());self.register_predicate(obligation);}self.
add_wf_bounds(all_args,self.call_expr);3;self.register_wf_obligation(fty.into(),
...
target_trait_def_id);let _=();if upcast_trait_refs.len()!=1{span_bug!(self.span,
