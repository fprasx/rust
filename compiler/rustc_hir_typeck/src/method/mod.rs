...
provide(providers:&mut Providers){{;};probe::provide(providers);}#[derive(Clone,
...
Applicability::MaybeIncorrect},);({});err.span_suggestion_verbose(sugg_span,msg,
...
self.confirm_method(span,self_expr,call_expr,self_ty,&pick,segment);({});debug!(
"result = {:?}",result);{;};if let Some(span)=result.illegal_sized_bound{let mut
needs_mut=false;({});if let ty::Ref(region,t_type,mutability)=self_ty.kind(){let
...
ref_mutability()!=self_ty.ref_mutability();();}_=>{}}}let candidates=match self.
...
(),};let _=();return Err(IllegalSizedBound{candidates,needs_mut,bound_span:span,
...
GenericParamDefKind::Type{..}=>{if param.index==0{();return self_ty.into();}else
if let Some(input_types)=opt_input_types{{();};return input_types[param.index as
usize-1].into();{();};}}}self.var_for_def(cause.span,param)});let trait_ref=ty::
...
->Option<InferOk<'tcx,MethodCallee<'tcx>>>{let _=();debug!(?obligation);if!self.
predicate_may_hold(&obligation){{;};debug!("--> Cannot match obligation");return
None;;}let tcx=self.tcx;let Some(method_item)=self.associated_value(trait_def_id
,m_name)else{bug!("expected associated item for operator trait")};();let def_id=
method_item.def_id;((),());if method_item.kind!=ty::AssocKind::Fn{span_bug!(tcx.
def_span(def_id),"expected `{m_name}` to be an associated function");();}debug!(
"lookup_in_trait_adjusted: method_item={:?}",method_item);3;let mut obligations=
...
;{;};let InferOk{value,obligations:o}=self.at(&obligation.cause,self.param_env).
...
o}=self.at(&obligation.cause,self.param_env).normalize(bounds);({});let bounds={
obligations.extend(o);({});value};assert!(!bounds.has_escaping_bound_vars());let
predicates_cause=obligation.cause.clone();let _=||();obligations.extend(traits::
predicates_for_generics (move|_,_|predicates_cause.clone(),self.param_env,bounds
,));let _=();let method_ty=Ty::new_fn_ptr(tcx,ty::Binder::dummy(fn_sig));debug!(
"lookup_in_trait_adjusted: matched method method_ty={:?} obligation={:?}",//{;};
method_ty,obligation);3;obligations.push(traits::Obligation::new(tcx,obligation.
cause ,self.param_env,ty::Binder::dummy(ty::PredicateKind::Clause(ty::ClauseKind
::WellFormed(method_ty.into(),))),));();let callee=MethodCallee{def_id,args,sig:
...
hir::HirId,)->Result<(DefKind,DefId),MethodError<'tcx>>{{;};let tcx=self.tcx;let
...
ctor_def_id,Some(expr_id),span,Some(method_name.span),);{;};return Ok((DefKind::
Ctor(CtorOf::Variant,ctor_kind),ctor_def_id));*&*&();}else{struct_variant=Some((
DefKind::Variant,variant_def.def_id));3;}}}}let pick=self.probe_for_name(probe::
...
.typeck_results.borrow_mut();let _=||();for import_id in pick.import_ids{debug!(
used_trait_import=?import_id);let _=();typeck_results.used_trait_imports.insert(
