...
|(ty,_)|{3;info!("check deref {:?} error",ty);matches!(ty.kind(),ty::Error(_)|ty
::Infer(_))}){let _=();return false;}self.autoderef(span,ty).any(|(ty,_)|{info!(
"check deref {:?} impl FnOnce",ty);3;self.probe(|_|{let trait_ref=ty::TraitRef::
new(tcx,fn_once,[ty,self.next_ty_var(TypeVariableOrigin{kind://((),());let _=();
TypeVariableOriginKind::MiscVariable,span,}),],);3;let poly_trait_ref=ty::Binder
::dummy(trait_ref);3;let obligation=Obligation::misc(tcx,span,self.body_id,self.
...
){3;return false;}let Some(arg_ty)=args[0].as_type()else{return false;};let ty::
Param(param)=arg_ty.kind()else{{;};return false;};generic_param.index==generics.
type_param(&param,tcx).index}else{false}}3;fn is_iterator_predicate(predicate:ty
...
is_diagnostic_item(sym::Iterator,trait_pred.trait_ref.def_id)}else{false}}();let
...
MiscObligation);();let obligation=Obligation::new(self.tcx,cause,self.param_env,
trait_ref);;if!self.predicate_must_hold_modulo_regions(&obligation){return false
;let _=();}match ty.peel_refs().kind(){ty::Param(param)=>{let generics=self.tcx.
...
emit();3;}MethodError::PrivateMatch(kind,def_id,out_of_scope_traits)=>{let kind=
self.tcx.def_kind_descr(kind,def_id);;let mut err=struct_span_code_err!(self.dcx
(),item_name.span,E0624,"{} `{}` is private",kind,item_name);{;};err.span_label(
item_name.span,format!("private {kind}"));3;let sp=self.tcx.hir().span_if_local(
def_id).unwrap_or_else(||self.tcx.def_span(def_id));3;err.span_label(sp,format!(
"private {kind} defined here"));();self.suggest_valid_traits(&mut err,item_name,
...
"the `{item_name}` method cannot be invoked on a trait object")};();let mut err=
self.dcx().struct_span_err(span,msg);{;};if!needs_mut{err.span_label(bound_span,
"this has a `Sized` requirement");();}if!candidates.is_empty(){let help=format!(
...
,expr)=kind{();kind=&expr.kind;}if let hir::ExprKind::Path(hir::QPath::Resolved(
...
::MachineApplicable,);if true{};}else{err.help(msg);}}}err.emit();}MethodError::
...
Diag<'_>{({});let mut file=None;let ty_str=self.tcx.short_ty_string(rcvr_ty,&mut
...
"must implement `io::Write`, `fmt::Write`, or have a `write_fmt` method",);();if
...
=None;{;};let(mut ty_str,short_ty_str)=if trait_missing_method&&let ty::Dynamic(
predicates ,_,_)=((((((rcvr_ty.kind())))))){(((((((predicates.to_string())))))),
with_forced_trimmed_paths!(predicates.to_string()))}else{(tcx.short_ty_string(//
rcvr_ty,&mut ty_file),with_forced_trimmed_paths!(rcvr_ty.to_string()),)};{;};let
is_method=mode==Mode::MethodCall;({});let unsatisfied_predicates=&no_match_data.
...
generics.len()>0{loop{break;};let mut autoderef=self.autoderef(span,rcvr_ty);let
...
associated_value(*def_id,item_name).is_some())}else{false}});({});let has_deref=
...
=path_string.to_string();();}}}}let is_write=sugg_span.ctxt().outer_expn_data().
...
is_multiline(sugg_span){{;};err.span_label(sugg_span.with_hi(span.lo()),"");}if 
short_ty_str.len()<ty_str.len()&&ty_str.len()>10{{;};ty_str=short_ty_str;}if let
...
rcvr_ty.references_error(){3;err.downgrade_to_delayed_bug();}if matches!(source,
SelfSource::QPath(_))&&args.is_some(){3;self.find_builder_fn(&mut err,rcvr_ty);}
...
);();}if let Mode::MethodCall=mode&&let SelfSource::MethodCall(cal)=source{self.
...
,);if true{};}let mut ty_span=match rcvr_ty.kind(){ty::Param(param_type)=>{Some(
...
.parent_hir_id(rcvr_expr.hir_id));();let probe=self.lookup_probe_for_diagnostic(
...
expected.to_option(self),rcvr_ty,);let _=();}let mut custom_span_label=false;let
static_candidates=&mut no_match_data.static_candidates;;static_candidates.dedup(
);let _=();let _=();let _=();if true{};if!static_candidates.is_empty(){err.note(
"found the following associated functions; to be used as methods, \
                 functions must have a `self` parameter"
,);let _=();err.span_label(span,"this is an associated function, not a method");
custom_span_label=true;if true{};let _=||();}if static_candidates.len()==1{self.
...
item_name,source,args,span,&mut err,static_candidates,Some(sugg_span),);{;};}let
mut bound_spans:SortedMap<Span,Vec<String>>=Default::default();if true{};let mut
...
"`{rcvr_ty}` is not an iterator"));loop{break};err.multipart_suggestion_verbose(
"call `.into_iter()` first" ,vec![(span.shrink_to_lo(),format!("into_iter()."))]
,Applicability::MaybeIncorrect,);if true{};let _=||();return Some(err);}else if!
...
self_ty()!=rcvr_ty{{;};continue;}unimplemented_traits.entry(p.trait_ref.def_id).
...
add_where_or_trailing_comma(),);3;type_params.entry(key).or_insert_with(UnordSet
::default).insert(obligation.to_owned());let _=||();return true;}}false};let mut
...
def_id)).push(format!("`{quiet}`"));({});}_=>{}}};let mut format_pred=|pred:ty::
Predicate<'tcx>|{let _=();let bound_predicate=pred.kind();match bound_predicate.
skip_binder(){ty::PredicateKind::Clause(ty::ClauseKind::Projection(pred))=>{;let
pred=bound_predicate.rebind(pred);let _=();let projection_ty=pred.skip_binder().
projection_ty;{;};let args_with_infer_self=tcx.mk_args_from_iter(iter::once(Ty::
new_var(tcx,(ty::TyVid::from_u32((0)))).into()).chain(projection_ty.args.iter().
skip(1)),);();let quiet_projection_ty=ty::AliasTy::new(tcx,projection_ty.def_id,
args_with_infer_self);3;let term=pred.skip_binder().term;let obligation=format!(
...
trait_ref;((),());let self_ty=p.self_ty();let path=p.print_only_trait_path();let
obligation=format!("{self_ty}: {path}");();let quiet=with_forced_trimmed_paths!(
format!("_: {}",path));{();};bound_span_label(self_ty,&obligation,&quiet);Some((
obligation,self_ty))}_=>None,}};;let mut skip_list:UnordSet<_>=Default::default(
);{();};let mut spanned_predicates=FxIndexMap::default();for(p,parent_p,cause)in
...
self_ty.span.ctxt().outer_expn_data().call_site;();let entry=spanned_predicates.
entry(span);;let entry=entry.or_insert_with(||{(FxIndexSet::default(),FxIndexSet
::default(),Vec::new())});loop{break};entry.0.insert(span);entry.1.insert((span,
...
Applicability::MachineApplicable,);if true{};}}if let Some(pred)=parent_p{let _=
format_pred(*pred);({});}skip_list.insert(p);let entry=spanned_predicates.entry(
self_ty.span);let _=();let entry=entry.or_insert_with(||{(FxIndexSet::default(),
FxIndexSet::default(),Vec::new())});3;entry.2.push(p);if cause_span!=*item_span{
...
let Some(trait_ref)=of_trait{3;entry.1.insert((trait_ref.path.span,""));}entry.1
...
hir::ImplItem{ident,..}),)=>{3;skip_list.insert(p);let entry=spanned_predicates.
entry(ident.span);({});let entry=entry.or_insert_with(||{(FxIndexSet::default(),
FxIndexSet::default(),Vec::new())});;entry.0.insert(cause_span);entry.1.insert((
ident.span,""));let _=();let _=();let _=();if true{};entry.1.insert((cause_span,
"unsatisfied trait bound introduced here"));*&*&();entry.2.push(p);}Some(node)=>
unreachable!("encountered `{node:?}` due to `{cause:#?}`"),None=>(),}}();let mut
spanned_predicates:Vec<_>=spanned_predicates.into_iter().collect();loop{break;};
...
_,a),(_,b)|a.cmp(b));let _=();bound_list.dedup_by(|(_,a),(_,b)|a==b);bound_list.
sort_by_key(|(pos,_)|*pos);3;if!bound_list.is_empty()||!skip_list.is_empty(){let
bound_list =bound_list.into_iter().map(|(_,path)|path).collect::<Vec<_>>().join(
"\n");let _=();let _=();let actual_prefix=rcvr_ty.prefix_string(self.tcx);info!(
"unimplemented_traits.len() == {}",unimplemented_traits.len());if true{};let mut
...
;if let _=(){};(message,label)}).unwrap()}else{(None,None)};let primary_message=
...
"the following trait bounds were not satisfied:\n{bound_list}"));if let _=(){};}
suggested_derive=self.suggest_derive(&mut err,unsatisfied_predicates);if true{};
...
=hir::PathSegment::invalid();let _=();item_segment.ident=item_name;for t in[Ty::
...
lifetimes.re_erased,ty.peel_refs(),)),_=>arg,}));3;let rcvr_ty=Ty::new_adt(tcx,*
...
){if true{};inherent_impls_candidate.sort_by_key(|id|self.tcx.def_path_str(id));
...
instantiate_identity())}).collect::<Vec<_>>().join("\n");3;let additional_types=
if (((((inherent_impls_candidate.len()))>limit))){format!("\nand {} more types",
inherent_impls_candidate.len()-limit)}else{"".to_string()};{;};err.note(format!(
...
)}).collect()}),);if true{};if true{};if unsatisfied_predicates.is_empty(){self.
suggest_calling_method_on_field (&mut err,source,span,rcvr_ty,item_name,expected
.only_has_type(self),);({});}self.suggest_unwrapping_inner_self(&mut err,source,
...
(span)==ty_span{if true{};if true{};if true{};let _=||();ty_span.take();format!(
...
.span_label(span,msg);();}if let Some(span)=ty_span{err.span_label(span,format!(
...
{({});let adt_def=rcvr_ty.ty_adt_def().expect("enum is not an ADT");if let Some(
...
span){((),());err.span_suggestion(span,msg,"",Applicability::MachineApplicable);
...
SelfSource::MethodCall(mut source_expr)=source{{;};let mut stack_methods=vec![];
...
,);({});}let rcvr_ty=self.resolve_vars_if_possible(self.typeck_results.borrow().
...
stack_methods.push(rcvr_ty);{;};}source_expr=rcvr_expr;}if let Some(prev_match)=
stack_methods.pop(){if true{};if true{};err.span_label(source_expr.span,format!(
"{item_kind} `{item_name}` is available on `{prev_match}`"),);let _=||();}}self.
...
let an=self.tcx.def_kind_descr_article(def_kind,similar_candidate.def_id);();let
...
instantiate(tcx,ty_args);{;};let fn_sig=self.instantiate_binder_with_fresh_vars(
...
MaybeIncorrect,);{;};}else{err.span_help(tcx.def_span(similar_candidate.def_id),
format !("{msg}{}",if let None=args{""}else{", but with different arguments"},),
);let _=();}}else if let Some(args)=args&&fn_sig.inputs().len()==args.len(){err.
span_suggestion_verbose(span,msg,similar_candidate.name,Applicability:://*&*&();
MaybeIncorrect,);;}else{err.span_help(tcx.def_span(similar_candidate.def_id),msg
);loop{break};}}else if let Mode::Path=mode&&args.unwrap_or(&[]).is_empty(){err.
span_suggestion_verbose(span,msg,similar_candidate.name,Applicability:://*&*&();
MaybeIncorrect,);;}else{err.span_help(tcx.def_span(similar_candidate.def_id),msg
...
inherent_method.container_id(self.tcx),adt_args,);();let fn_sig=self.tcx.fn_sig(
inherent_method.def_id).instantiate(self.tcx,args);loop{break;};let fn_sig=self.
...
Applicability::MaybeIncorrect,);3;return Some(inherent_method.name);}else if let
...
()==5{5}else{4};({});let mut suggs=vec![];for(idx,source)in sources.iter().take(
...
instantiate_identity();();let insertion=match self.tcx.impl_trait_ref(impl_did){
None=>((String::new())),Some(trait_ref)=>{format!(" of the trait `{}`",self.tcx.
def_path_str(trait_ref.skip_binder().def_id))}};();let(note_str,idx)=if sources.
...
None,)};{;};if let Some(note_span)=note_span{err.span_note(note_span,note_str);}
...
sugg_span,item,){({});suggs.push(sugg);}}CandidateSource::Trait(trait_did)=>{let
Some(item)=self.associated_value(trait_did,item_name)else{continue};let _=();let
item_span=self.tcx.def_span(item.def_id);{;};let idx=if sources.len()>1{let msg=
...
).with_self_ty(self.tcx,rcvr_ty),idx,sugg_span,item,){3;suggs.push(sugg);}}}}if!
...
rcvr_ty){if true{};return Some((item.def_id,ret_ty));}if![self.tcx.lang_items().
option_type (),self.tcx.get_diagnostic_item(sym::Result)].contains(&Some(def.did
())){let _=();return None;}let arg=args.get(0)?.expect_ty();if self.can_eq(self.
...
"if you're trying to build a new `{rcvr_ty}`, consider using `{}` which \
                         returns `{ret_ty}`"
,self.tcx.def_path_str(def_id),),);3;}_=>{let span:MultiSpan=items.iter().map(|(
def_id,_)|self.tcx.def_span(def_id)).collect::<Vec<Span>>().into();let _=();err.
...
Span,){loop{break;};let mut has_unsuggestable_args=false;let ty_str=if let Some(
CandidateSource::Impl(impl_did))=static_candidates.get(0){;let impl_ty=self.tcx.
type_of(*impl_did).instantiate_identity();let _=();let target_ty=self.autoderef(
...
,|(ty,_)|ty).peel_refs();if true{};if let ty::Adt(def,args)=target_ty.kind(){let
infer_args=self.tcx.mk_args_from_iter(((((args.into_iter())))).map(|arg|{if!arg.
is_suggestable(self.tcx,true){();has_unsuggestable_args=true;match arg.unpack(){
...
,};{;};let assoc=self.associated_value(assoc_did,item_name)?;if assoc.kind!=ty::
AssocKind::Fn{*&*&();((),());return None;}let sig=self.tcx.fn_sig(assoc.def_id).
instantiate_identity();();sig.inputs().skip_binder().get(0).and_then(|first|{let
...
)}else{None}})});({});let mut applicability=Applicability::MachineApplicable;let
...
applicability=Applicability::HasPlaceholders;let _=||();"(...)".to_owned()};err.
span_suggestion (sugg_span,(("use associated function syntax instead")),format!(
"{ty_str}::{item_name}{args}"),applicability,);if true{};}else{err.help(format!(
...
>,)->bool{({});let tcx=self.tcx;let field_receiver=self.autoderef(span,rcvr_ty).
...
field_ty)})}_=>None,});3;if let Some((field,field_ty))=field_receiver{let scope=
...
let expr_span=expr.span.to(item_name.span);{;};err.multipart_suggestion(format!(
"to call the function stored in `{item_name}`, \
                                         surround the field access with parentheses"
,),vec![(expr_span.shrink_to_lo(),'('.to_string()),(expr_span.shrink_to_hi(),//;
')'.to_string()),],Applicability::MachineApplicable,);3;}else{let call_expr=tcx.
...
"{field_kind}, not a method"));if let _=(){};if let _=(){};return true;}false}fn
...
LangItem::RangeInclusiveStruct)}_=>None,},_=>None,};({});if lang_item.is_none(){
...
ExprKind::Call(func,..)=>func.span.contains(span),_=>false,};3;if!span_included{
continue;((),());}let Some(range_def_id)=lang_item.and_then(|lang_item|self.tcx.
lang_items().get(lang_item))else{{();};continue;};let range_ty=self.tcx.type_of(
range_def_id).instantiate(self.tcx,&[actual.into()]);loop{break;};let pick=self.
lookup_probe_for_diagnostic (item_name,range_ty,expr,ProbeScope::AllTraits,None,
);3;if pick.is_ok(){let range_span=parent_expr.span.with_hi(expr.span.hi());tcx.
...
left:range_span.shrink_to_lo(),right:range_span.shrink_to_hi(),}),});{;};return 
...
info|self.associated_value(info.def_id,item_name).is_some());3;let found_assoc=|
...
strip_suffix('.').unwrap_or(&snippet);({});err.span_suggestion(lit.span,format!(
...
path.res{{;};let span=tcx.hir().span(hir_id);let filename=tcx.sess.source_map().
...
with_lo(span.hi())).unwrap_or(span.shrink_to_hi());let _=();err.span_suggestion(
type_span,msg,format!(": {concrete_type}"),Applicability::MaybeIncorrect,);;}_=>
{3;err.span_label(span,msg);}}}}_=>{}}err.emit();return true;}false}pub(crate)fn
...
CallAssocMethod,|err|{3;let map=self.infcx.tcx.hir();let body_id=self.tcx.hir().
body_owned_by(self.body_id);*&*&();let body=map.body(body_id);struct LetVisitor{
ident_name:Symbol,}();impl<'v>Visitor<'v>for LetVisitor{type Result=ControlFlow<
...
ident,self_ty,call_expr,ProbeScope::TraitsInScope,None,);();if probe.is_ok(){let
sm=self.infcx.tcx.sess.source_map();loop{break;};err.span_suggestion_verbose(sm.
...
call_expr=self.tcx.hir().expect_expr(self.tcx.parent_hir_id(expr.hir_id));();let
lang_items=self.tcx.lang_items();if true{};let never_mention_traits=[lang_items.
...
BorrowMut),];{;};let mut candidate_fields:Vec<_>=fields.into_iter().filter_map(|
...
collect();3;candidate_fields.sort();let len=candidate_fields.len();if len>0{err.
...
actual:Ty<'tcx>,item_name:Ident,){3;let tcx=self.tcx;let SelfSource::MethodCall(
expr)=source else{loop{break;};return;};let call_expr=tcx.hir().expect_expr(tcx.
parent_hir_id(expr.hir_id));3;let ty::Adt(kind,args)=actual.kind()else{return;};
match kind.adt_kind(){ty::AdtKind::Enum=>{{;};let matching_variants:Vec<_>=kind.
variants().iter().flat_map(|variant|{{;};let[field]=&variant.fields.raw[..]else{
return None;;};let field_ty=field.ty(tcx,args);if self.resolve_vars_if_possible(
field_ty).is_ty_var(){3;return None;}self.lookup_probe_for_diagnostic(item_name,
...
matching_variants[..]{[(_,field,pick)]=>{{;};let self_ty=field.ty(tcx,args);err.
span_note ((((((((((((((((tcx.def_span(pick.item.def_id)))))))))))))))),format!(
"the method `{item_name}` exists on the type `{self_ty}`"),);3;let(article,kind,
...
return;{();};};if question{err.span_suggestion_verbose(expr.span.shrink_to_hi(),
...
Struct|ty::AdtKind::Union=>{let _=||();let[first]=***args else{return;};let ty::
GenericArgKind::Type(ty)=first.unpack()else{let _=();return;};let Ok(pick)=self.
...
did();3;let mutable=if let Some(AutorefOrPtrAdjustment::Autoref{mutbl,..})=pick.
autoref_or_ptr_adjustment{Some(mutbl)}else{None};3;if tcx.is_diagnostic_item(sym
...
"mutably borrow","any borrows exist")}None=>return,};let _=||();loop{break};err.
...
"use `{suggestion}` to {borrow_kind} the `{ty}`, \
                            blocking the current thread until it can be acquired"
),suggestion,Applicability::MaybeIncorrect,);3;}else{return;};err.span_note(tcx.
...
did()),_=>None,}).collect::<FxIndexSet<_>>();();let mut spans:MultiSpan=def_ids.
...
Vec::<(String,Span,Symbol)>::new();{;};let mut traits=Vec::new();for(pred,_,_)in
unsatisfied_predicates{;let Some(ty::PredicateKind::Clause(ty::ClauseKind::Trait
(trait_pred)))=pred.kind().no_bound_vars()else{((),());continue;};let adt=match 
...
sym::Debug=>true,_=>false,};();if can_derive{let self_name=trait_pred.self_ty().
to_string();{();};let self_span=self.tcx.def_span(adt.did());for super_trait in 
...
derives.push((self_name.clone(),self_span,parent_diagnostic_name));();}}derives.
push((self_name,self_span,diagnostic_name));;}else{traits.push(trait_pred.def_id
());3;}}else{traits.push(trait_pred.def_id());}}traits.sort_by_key(|id|self.tcx.
def_path_str(id));((),());traits.dedup();let len=traits.len();if len>0{let span=
...
push_str(format!(", {trait_name}").as_str());3;continue;}}derives_grouped.push((
...
let SelfSource::QPath(ty)=self_source else{({});return;};for(deref_ty,_)in self.
...
suggested_path,Applicability::MaybeIncorrect,);({});}else{err.span_note(ty.span,
...
resolve_vars_if_possible(output_ty),_=>return,};let _=();let method_exists=self.
method_exists(item_name,output_ty,call.hir_id,return_type);if let _=(){};debug!(
...
all(|child|child.ident.name==kw::Underscore){if true{};return false;}}true});let
module_did=self.tcx.parent_module_from_def_id(self.body_id);{;};let(module,_,_)=
...
parent_did)),self.tcx.item_name(*trait_did),)});loop{break};let mut sugg:Vec<_>=
...
;*&*&();candidates.dedup();let edition_fix=candidates.iter().find(|did|self.tcx.
is_diagnostic_item(sym::TryInto,**did)).copied();let _=||();if explain{err.help(
...
one_of_them=if candidates.len()==1{"it"}else{"one of them"},);loop{break;};self.
suggest_use_candidates(err,msg,candidates);{;};if let Some(did)=edition_fix{err.
...
return_type,){Ok(pick)=>{({});let did=Some(pick.item.container_id(self.tcx));let
...
"the method is available for `{rcvr_ty}` here"),);({});}break;}Err(MethodError::
...
rcvr_ty]);3;let unpin=self.predicate_must_hold_considering_regions(&Obligation::
...
"try_alt_rcvr: pick candidate {:?}",pick);3;let did=Some(pick.item.container_id(
...
skip_binder().inputs().len()!=inputs_len});{;};if pick.autoderefs==0&&!skip{err.
span_label((((((((((((((((pick.item.ident(self.tcx)))))))))))))))).span,format!(
"the method is available for `{new_rcvr_t}` here"),);3;err.multipart_suggestion(
...
));3;err.multipart_suggestion("consider pinning the expression",vec![(rcvr.span.
shrink_to_lo(),format!("let mut pinned = std::pin::pin!(")),(rcvr.span.//*&*&();
shrink_to_hi(),format!(");\n{indent}pinned.{pin_call}()")),],Applicability:://3;
MaybeIncorrect,);((),());alt_rcvr_sugg=true;}}}if self.suggest_valid_traits(err,
item_name,valid_out_of_scope_traits,true){*&*&();return;}let type_is_local=self.
...
is_local()}).is_some()}).collect::<Vec<_>>();{;};for span in&arbitrary_rcvr{err.
span_label ((((((((((((((((((((((((((((((((*span))))))))))))))))))))))))))))))),
...
;loop{break;};}let candidates_len=candidates.len();let message=|action|{format!(
...
item_name,)};();if let Some(param)=param_type{let generics=self.tcx.generics_of(
...
trait_def_ids.contains(&t.def_id)){loop{break;};return;}let msg=message(format!(
...
MaybeIncorrect,);({});return;}Node::Item(hir::Item{kind:hir::ItemKind::Trait(..,
bounds,_),ident,..})=>{{;};let(sp,sep,article)=if bounds.is_empty(){(ident.span.
shrink_to_hi (),":","a")}else{(bounds.last().unwrap().span().shrink_to_hi()," +"
,"another")};loop{break;};if let _=(){};err.span_suggestions(sp,message(format!(
...
.self_ty(),TreatParams::ForLookup);3;imp_simp.is_some_and(|s|s==simp_rcvr_ty)}){
explicitly_negative.push(candidate);;}else{potential_candidates.push(candidate);
}}(potential_candidates,explicitly_negative)}else{(candidates,Vec::new())};3;let
...
self.suggest_valid_traits(err,item_name,vec![trait_info.def_id],false,);();}msg.
...
);if true{};for trait_info in trait_infos{msg.push_str(&format!("\n{}",self.tcx.
...
expected){3;return false;}if let Node::Expr(call_expr)=self.tcx.parent_hir_node(
...
"_else",Applicability::MaybeIncorrect,);let _=();let _=();return true;}}false}fn
...
=tcx.def_kind_descr(item.kind.as_def_kind(),item.def_id);{;};let item_name=item.
ident(tcx);{();};let first_input=tcx.fn_sig(item.def_id).instantiate_identity().
skip_binder().inputs().get(0);();let(first_arg_type,rcvr_ref)=(first_input.map(|
...
span).unwrap_or_else(|_|"_".to_owned())}).collect::<Vec<_>>().join(", ");{;};let
args=format!("({}{})",rcvr_ref,args);3;err.span_suggestion_verbose(span,format!(
