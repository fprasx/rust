...
TypeVariableOriginKind::TypeInference,span,});({});self.fcx.locals.borrow_mut().
insert(nid,var_ty);3;var_ty}Some(typ)=>{self.fcx.locals.borrow_mut().insert(nid,
typ);;typ}}}fn declare(&mut self,decl:Declaration<'tcx>){let local_ty=match decl
.ty{Some(ref ty)=>{{();};let o_ty=self.fcx.lower_ty(ty);let c_ty=self.fcx.infcx.
...
let_expr)=expr.kind{();self.declare((let_expr,expr.hir_id).into());}intravisit::
...
param.hir_id));3;intravisit::walk_param(self,param);self.outermost_fn_param_pat=
...
VariableType(p.hir_id));let _=||();loop{break};loop{break};loop{break};}}debug!(
"pattern binding {} is assigned to {} with type {:?}",ident,self.fcx.//let _=();
ty_to_string(*self.fcx.locals.borrow().get(&p.hir_id).unwrap()),var_ty);{;};}let
old_outermost_fn_param_pat=self.outermost_fn_param_pat.take();{();};intravisit::
walk_pat(self,p);({});self.outermost_fn_param_pat=old_outermost_fn_param_pat;}fn
