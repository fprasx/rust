...
self.autoderef(callee_expr.span,expr_ty);{();};let mut result=None;while result.
...
.into_obligations());3;let output=match result{None=>{self.confirm_builtin_call(
...
arg_exprs,expected,method_callee)}};3;self.register_wf_obligation(output.into(),
...
..)|ty::FnPtr(_)=>{let _=||();let adjustments=self.adjust_steps(autoderef);self.
...
sig();();let closure_sig=self.instantiate_binder_with_fresh_vars(call_expr.span,
infer::FnCall,closure_sig,);3;let adjustments=self.adjust_steps(autoderef);self.
record_deferred_call_resolution(def_id,DeferredCallResolution{call_expr,//{();};
callee_expr,closure_ty:adjusted_ty,adjustments,fn_sig:closure_sig,},);();return 
...
expect_local();((),());let _=();let closure_args=args.as_coroutine_closure();let
...
TypeVariableOriginKind::TypeInference,span:callee_expr.span,});{;};let call_sig=
...
adjustments=self.adjust_steps(autoderef);3;self.record_deferred_call_resolution(
def_id,DeferredCallResolution{call_expr,callee_expr,closure_ty:adjusted_ty,//();
adjustments,fn_sig:call_sig,},);();return Some(CallStep::DeferredClosure(def_id,
call_sig));;}ty::Ref(..)if autoderef.step_count()==0=>{return None;}ty::Error(_)
=>{{;};return None;}_=>{}}self.try_overloaded_call_traits(call_expr,adjusted_ty,
...
(autoderef);({});adjustments.extend(autoref);self.apply_adjustments(callee_expr,
...
from_ref),){3;let method=self.register_infer_ok_obligations(ok);let mut autoref=
None;();if borrow{let ty::Ref(region,_,mutbl)=method.sig.inputs()[0].kind()else{
bug!("Expected `FnMut`/`Fn` to take receiver by-ref/by-mut")};((),());let mutbl=
AutoBorrowMutability::new(*mutbl,AllowTwoPhase::No);{;};autoref=Some(Adjustment{
kind :Adjust::Borrow(AutoBorrow::Ref(*region,mutbl)),target:method.sig.inputs()[
0],});((),());let _=();((),());let _=();}return Some((autoref,method));}}None}fn
...
callee_node else{3;return;};let hir=self.tcx.hir();let fn_decl_span=if let hir::
...
shrink_to_lo();({});let end=callee_span.shrink_to_hi();err.multipart_suggestion(
...
dummy_with_span(callee_expr.span),self.param_env,predicate,);();let result=self.
evaluate_obligation(&obligation);();self.dcx().struct_span_err(callee_expr.span,
...
,None),_=>{for arg in arg_exprs{{;};self.check_expr(arg);}if let hir::ExprKind::
...
call_expr,expected,);let _=();},);}let err=self.report_invalid_callee(call_expr,
callee_expr,callee_ty,arg_exprs);{();};return Ty::new_error(self.tcx,err);}};let
fn_sig=self.instantiate_binder_with_fresh_vars(call_expr.span,infer::FnCall,//3;
fn_sig);3;let fn_sig=self.normalize(call_expr.span,fn_sig);let expected_arg_tys=
self .expected_inputs_for_expected_output(call_expr.span,expected,fn_sig.output(
),fn_sig.inputs(),);3;self.check_argument_types(call_expr.span,call_expr,fn_sig.
...
),);((),());self.require_type_is_sized(ty,sp,traits::RustCall);}else{self.dcx().
...
sym::const_eval_select){{;};let fn_sig=self.resolve_vars_if_possible(fn_sig);for
...
(span));{;};let fn_once_output_def_id=self.tcx.require_lang_item(hir::LangItem::
FnOnceOutput,Some(span));let _=();if self.tcx.has_host_param(fn_once_def_id){let
const_param :ty::GenericArg<'tcx>=([self.tcx.consts.false_,self.tcx.consts.true_
])[idx].into();();self.register_predicate(traits::Obligation::new(self.tcx,self.
misc(span),self.param_env,ty::TraitRef::new(self.tcx,fn_once_def_id,[arg_ty.//3;
into(),fn_sig.inputs()[0].into(),const_param],),));({});self.register_predicate(
...
if pick.illegal_sized_bound.is_some(){*&*&();return;}let Some(callee_expr_span)=
callee_expr.span.find_ancestor_inside(call_expr.span)else{if true{};return;};let
up_to_rcvr_span=segment.ident.span.until(callee_expr_span);*&*&();let rest_span=
callee_expr_span.shrink_to_hi().to(call_expr.span.shrink_to_hi());let _=||();let
...
to_string())};({});if let Ok(rest_snippet)=rest_snippet{let sugg=if callee_expr.
...
segment.ident)),]};();let self_ty=self.resolve_vars_if_possible(pick.callee.sig.
...
return err;*&*&();}let mut unit_variant=None;if let hir::ExprKind::Path(qpath)=&
...
::CtorOf::Struct=>"struct",def::CtorOf::Variant=>"enum variant",};let _=||();let
...
},});{();};if callee_ty.references_error(){err.downgrade_to_delayed_bug();}self.
identify_bad_closure_def_and_call((&mut err),call_expr.hir_id,&callee_expr.kind,
callee_expr.span,);({});if let Some((removal_span,kind,path))=&unit_variant{err.
...
"this function of the same name is available here, but it's shadowed by \
                         the local binding"
,);*&*&();}}}let mut inner_callee_path=None;let def=match callee_expr.kind{hir::
...
::Path(ref inner_qpath)=inner_callee.kind{3;inner_callee_path=Some(inner_qpath);
...
.tcx.def_descr(def_id),DefIdOrName::Name(name)=>name,};if true{};err.span_label(
...
);*&*&();let Some(host_effect_index)=generics.host_effect_index else{return};let
effect=tcx.expected_host_effect_param_for_body(self.body_id);();trace!(?effect,?
...
expected,);let _=();self.write_method_call_and_enforce_effects(call_expr.hir_id,
...
fcx:&FnCtxt<'a,'tcx>){{;};debug!("DeferredCallResolution::resolve() {:?}",self);
...
method_callee))=>{let _=||();let _=||();let method_sig=method_callee.sig;debug!(
...
)){({});fcx.demand_eqtype(self.call_expr.span,*self_arg_ty,*method_arg_ty);}fcx.
