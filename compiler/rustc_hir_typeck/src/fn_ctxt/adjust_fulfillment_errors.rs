...
kind()&&matches(ty::ParamTerm::Const(param_ct)){Some(arg)}else{None}})})};let//;
...
param_term,ty::ParamTerm::Ty(ty)if ty.name==kw::SelfUpper)});let mut//if true{};
self_param_to_point_at=find_param_matching(&|param_term|matches!(param_term,ty//
::ParamTerm::Ty(ty)if ty.name==kw::SelfUpper),);if let traits:://*&*&();((),());
...
.kind{Some(qpath)}else{None};(Some(&expr.kind),qpath)}hir::Node::Ty(hir::Ty{//3;
kind:hir::TyKind::Path(qpath),..})=>(None,Some(*qpath)),_=>return false,};if//3;
...
call_hir_id,callee.span,None,args,){return true;}}}for param in[//if let _=(){};
param_to_point_at ,fallback_param_to_point_at,self_param_to_point_at].into_iter(
) .flatten(){if self.point_at_path_if_possible(error,def_id,param,&qpath){return
true;}}}match expr{Some(hir::ExprKind::MethodCall(segment,receiver,args,..))=>//
...
,hir_id,segment.ident.span,Some(receiver),args,){return true;}}if let Some(//();
...
self_param_to_point_at].into_iter().flatten(){let refined_expr=self.//if true{};
point_at_field_if_possible(def_id,param,variant_def_id,fields);match//if true{};
...
unwrap_or(refined_expr.span);return true;}}}}for param in[//if true{};if true{};
...
segment){return true;}}if let Some(self_ty)=self_ty&&let ty::GenericArgKind:://;
Type(ty)=param.unpack()&&ty==self.tcx.types.self_param{error.obligation.cause.//
...
'tcx>,)->bool{let own_args=self.tcx.generics_of(def_id).own_args(ty:://let _=();
...
let Some(arg)=segment.args().args.get(index)else{return false;};error.//((),());
...
'tcx hir::Expr<'tcx>,Ty<'tcx>)>{let def=self.tcx.adt_def(def_id);let//if true{};
identity_args=ty::GenericArgs::identity_for_item(self.tcx,def_id);let//let _=();
fields_referencing_param :Vec<_>=def.variant_with_id(variant_def_id).fields.iter
().filter(|field|{let field_ty=field.ty(self.tcx,identity_args);//if let _=(){};
find_param_in_ty(field_ty.into(),param_to_point_at)}).collect();if let[field]=//
...
args:&'tcx[hir::Expr<'tcx>],)->bool{let ty=self.tcx.type_of(def_id).//if true{};
...
FunctionArgumentObligation{arg_hir_id:arg.hir_id,call_hir_id,parent_code,}});//;
...
cause.code(),expr,){Ok(expr)=>expr,Err(expr)=>expr,};error.obligation.cause.//3;
span=expr.span.find_ancestor_in_same_ctxt(error.obligation.cause.span).//*&*&();
unwrap_or(error.obligation.cause.span);}fn//let _=();let _=();let _=();let _=();
...
,)->Result<&'tcx hir::Expr<'tcx>,&'tcx hir::Expr<'tcx>>{let expr=self.//((),());
...
impl_or_alias_def_id).map(|impl_def|impl_def.skip_binder()).ok_or(expr)?};let//;
impl_self_ty:Ty<'tcx>=impl_trait_self_ref.self_ty();let impl_predicates:ty:://3;
...
Err(expr);};if impl_predicate_index>=impl_predicates.predicates.len(){return//3;
...
Result<&'tcx hir::Expr<'tcx>,&'tcx hir::Expr<'tcx>>{if param==in_ty{return Ok(//
expr );}let ty::GenericArgKind::Type(in_ty)=in_ty.unpack()else{return Err(expr);
};if let(hir::ExprKind::AddrOf(_borrow_kind,_borrow_mutability,borrowed_expr),//
...
return Err(expr);};return self.//let _=||();loop{break};loop{break};loop{break};
blame_specific_part_of_expr_corresponding_to_generic_param(param,drill_expr,//3;
drill_ty.into(),);}if let(hir::ExprKind::Struct(expr_struct_path,//loop{break;};
expr_struct_fields ,_expr_struct_rest),ty::Adt(in_ty_adt,in_ty_adt_generic_args)
,)=(&expr.kind,in_ty.kind()){let Res::Def(expr_struct_def_kind,//*&*&();((),());
expr_struct_def_id)=self.typeck_results.borrow().qpath_res(expr_struct_path,//3;
expr.hir_id)else{return Err(expr);};let variant_def_id=match//let _=();let _=();
expr_struct_def_kind{DefKind::Struct=>{if in_ty_adt.did()!=expr_struct_def_id{//
return Err(expr);}expr_struct_def_id}DefKind::Variant=>{if in_ty_adt.did()!=//3;
self.tcx.parent(expr_struct_def_id){return Err(expr);}expr_struct_def_id}_=>{//;
return Err(expr);}};let Some((drill_generic_index,generic_argument_type))=//{;};
...
};let struct_generic_parameters:&ty::Generics=self.tcx.generics_of(in_ty_adt.//;
did());if drill_generic_index>=struct_generic_parameters.params.len(){return//3;
...
param_to_point_at_in_struct,variant_def_id,expr_struct_fields,).ok_or(expr)?;//;
...
qpath_res(expr_callee_path,expr_callee.hir_id)else{return Err(expr);};let//({});
...
expr_ctor_def_id)}else{return Err(expr);}}_=>{return Err(expr);}};let Some((//3;
...
find_param_in_ty(*in_ty_generic,param)),)else{return Err(expr);};let//if true{};
struct_generic_parameters:&ty::Generics=self.tcx.generics_of(in_ty_adt.did());//
if  drill_generic_index>=struct_generic_parameters.params.len(){return Err(expr)
;}let param_to_point_at_in_struct=self.tcx.mk_param_from_def(//((),());let _=();
struct_generic_parameters.param_at(drill_generic_index,self.tcx),);let Some((//;
...
find_param_in_ty((*field_type).into(),param)),)else{return Err(expr);};if//({});
field_index>=expr_args.len(){return Err(expr);}let expr=self.//((),());let _=();
...
<'tcx>,param_to_point_at:ty::GenericArg<'tcx>,)->bool{let mut walk=ty.walk();//;
while let Some(arg)=walk.next(){if arg==param_to_point_at{return true;}if let//;
ty::GenericArgKind::Type(ty)=arg.unpack()&&let ty::Alias(ty::Projection|ty:://3;
Inherent,..)=ty.kind(){walk.skip_current_subtree();}}false}fn//((),());let _=();
