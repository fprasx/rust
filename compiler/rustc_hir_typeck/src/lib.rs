...
def_id:DefId)->bool{();let typeck_root_def_id=tcx.typeck_root_def_id(def_id);if 
typeck_root_def_id!=def_id{3;return tcx.has_typeck_results(typeck_root_def_id);}
...
def_id:LocalDefId)->&ty::TypeckResults<'tcx>{3;let fallback=move||{let span=tcx.
...
span(id);let _=||();let body_id=node.body_id().unwrap_or_else(||{span_bug!(span,
"can't type-check body of {:?}",def_id);;});let body=tcx.hir().body(body_id);let
param_env=tcx.param_env(def_id);3;let root_ctxt=TypeckRootCtxt::new(tcx,def_id);
...
header.abi,decl,None,None)}else{tcx.fn_sig(def_id).instantiate_identity()};({});
...
expected_type.unwrap_or_else(fallback);{;};let expected_type=fcx.normalize(body.
value.span,expected_type);({});let wf_code=ObligationCauseCode::WellFormed(Some(
WellFormedLoc::Ty(def_id)));{;};fcx.register_wf_obligation(expected_type.into(),
body.value.span,wf_code);{;};fcx.require_type_is_sized(expected_type,body.value.
span,traits::ConstSized);();GatherLocalsVisitor::new(&fcx).visit_body(body);fcx.
check_expr_coercible_to_type(body.value,expected_type,None);{;};fcx.write_ty(id,
expected_type);let _=||();};fcx.type_inference_fallback();fcx.check_casts();fcx.
...
;((),());let typeck_results=fcx.resolve_type_vars_in_body(body);let _=fcx.infcx.
take_opaque_types();if let _=(){};assert_eq!(typeck_results.hir_owner,id.owner);
typeck_results}fn infer_type_if_missing<'tcx>(fcx:&FnCtxt<'_,'tcx>,node:Node<//;
'tcx>)->Option<Ty<'tcx>>{loop{break;};let tcx=fcx.tcx;let def_id=fcx.body_id;let
...
node{();let id=tcx.local_def_id_to_hir_id(def_id);match tcx.parent_hir_node(id){
...
qpath_to_string(qpath);if true{};let err=tcx.dcx().struct_span_err(span,format!(
...
span,"It looks like you're trying to break rust; would you like some ICE?",);();
diag.note("the compiler expectedly panicked. this is a feature.");{;};diag.note(
"we would appreciate a joke overview: \
         https://github.com/rust-lang/rust/issues/43162#issuecomment-320764675"
,);({});diag.note(format!("rustc {} running on {}",tcx.sess.cfg_version,config::
...
(" ")));((),());let _=();let _=();let _=();if excluded_cargo_defaults{diag.note(
"some of the compiler flags provided by cargo are hidden");;}}diag.emit()}pub fn
provide(providers:&mut Providers){((),());method::provide(providers);*providers=
