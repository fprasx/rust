...
is_none(){3;let data_idx;let one=VariantIdx::new(1);let zero=VariantIdx::new(0);
if def.variant(zero).fields.is_empty(){3;data_idx=one;}else if def.variant(one).
fields.is_empty(){{();};data_idx=zero;}else{return ty;}if def.variant(data_idx).
fields.len()==1{3;return def.variant(data_idx).single_field().ty(tcx,args);}}ty}
...
tcx.normalize_erasing_regions(self.param_env,ty)};3;let from=normalize(from);let
to=normalize(to);if true{};trace!(?from,?to);if from.has_non_region_infer()||to.
has_non_region_infer(){((),());((),());((),());let _=();tcx.dcx().span_bug(span,
"argument to transmute has inference variables");;}if from==to{return;}let skel=
|ty|SizeSkeleton::compute(ty,tcx,self.param_env);({});let sk_from=skel(from);let
sk_to=skel(to);3;trace!(?sk_from,?sk_to);if let(Ok(sk_from),Ok(sk_to))=(sk_from,
...
"size can vary because of {bad}")}}Err(err)=>err.to_string(),};({});let mut err=
...
();();}else{err.note(format!("source type: `{}` ({})",from,skeleton_string(from,
