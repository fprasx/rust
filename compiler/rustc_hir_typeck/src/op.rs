...
IsAssign::Yes,expected);({});let ty=if!lhs_ty.is_ty_var()&&!rhs_ty.is_ty_var()&&
is_builtin_binop(lhs_ty,rhs_ty,op){();self.enforce_builtin_binop_types(lhs.span,
lhs_ty,rhs.span,rhs_ty,op);let _=();Ty::new_unit(self.tcx)}else{return_ty};self.
...
IsAssign::Yes),expected,).is_err(){{;};err.downgrade_to_delayed_bug();}else{err.
...
expected:Expectation<'tcx>,)->Ty<'tcx>{((),());let _=();let tcx=self.tcx;debug!(
...
types.bool,None);if true{};let _=||();let lhs_diverges=self.diverges.get();self.
check_expr_coercible_to_type(rhs_expr,tcx.types.bool,None);();self.diverges.set(
lhs_diverges);loop{break;};tcx.types.bool}_=>{let(lhs_ty,rhs_ty,return_ty)=self.
...
(lhs_span,tcx.types.bool,lhs_ty);();self.demand_suptype(rhs_span,tcx.types.bool,
rhs_ty);*&*&();tcx.types.bool}BinOpCategory::Shift=>{lhs_ty}BinOpCategory::Math|
BinOpCategory::Bitwise=>{{;};self.demand_suptype(rhs_span,lhs_ty,rhs_ty);lhs_ty}
BinOpCategory::Comparison=>{{;};self.demand_suptype(rhs_span,lhs_ty,rhs_ty);tcx.
...
check_expr(lhs_expr);{;};let fresh_var=self.next_ty_var(TypeVariableOrigin{kind:
...
TypeVariableOriginKind::MiscVariable,span:rhs_expr.span,});({});let result=self.
lookup_op_method (((lhs_expr,lhs_ty)),Some((rhs_expr,rhs_ty_var)),Op::Binary(op,
is_assign),expected,);{;};let rhs_ty=self.check_expr_coercible_to_type(rhs_expr,
...
,target:method.sig.inputs()[1],};if let _=(){};self.typeck_results.borrow_mut().
adjustments_mut().entry(rhs_expr.hir_id).or_default().push(autoref);({});}}self.
...
note_unmet_impls_on_type(&mut err,errors,false);();(err,None)}IsAssign::No=>{let
...
(),lhs_ty),};let _=();let output_def_id=trait_def_id.and_then(|def_id|{self.tcx.
...
(&rhs_expr.span){if true{};err.span_label(lhs_expr.span,lhs_ty.to_string());err.
span_label(rhs_expr.span,rhs_ty.to_string());();}let suggest_derive=self.can_eq(
self.param_env,lhs_ty,rhs_ty);{;};self.note_unmet_impls_on_type(&mut err,errors,
suggest_derive);3;(err,output_def_id)}};if self.check_for_missing_semi(expr,&mut
err)&&let hir::Node::Expr(expr)=(self.tcx.parent_hir_node(expr.hir_id))&&let hir
::ExprKind::Assign(..)=expr.kind{loop{break};err.downgrade_to_delayed_bug();}let
...
as_str(),match is_assign{IsAssign::Yes=>"=",IsAssign::No=>"",},lhs_deref_ty,);3;
err .span_suggestion_verbose(lhs_expr.span.shrink_to_lo(),msg,"*",rustc_errors::
Applicability::MachineApplicable,);{;};}};let suggest_different_borrow=|err:&mut
...
is_assign),expected,).is_ok(){({});let op_str=op.node.as_str();err.note(format!(
...
to_string()),],rustc_errors::Applicability::MachineApplicable,);();}else{let mut
...
"&*",rustc_errors::Applicability::MachineApplicable,);();}else{err.span_help(sp,
"consider making this expression a mutable borrow",);loop{break};}};if let Some(
lhs_new_mutbl)=lhs_new_mutbl{3;suggest_new_borrow(lhs_new_mutbl,lhs_expr.span);}
if let Some(rhs_new_mutbl)=rhs_new_mutbl{{();};suggest_new_borrow(rhs_new_mutbl,
rhs_expr.span);if true{};}}}};let is_compatible_after_call=|lhs_ty,rhs_ty|{self.
...
lhs_inv_mutbl_ty=Ty::new_ref(self.tcx,*region,*lhs_deref_ty,lhs_inv_mutbl);({});
suggest_different_borrow((&mut err),lhs_inv_mutbl_ty,Some(lhs_inv_mutbl),rhs_ty,
None,);();if let Ref(region,rhs_deref_ty,mutbl)=rhs_ty.kind(){let rhs_inv_mutbl=
mutbl.invert();;let rhs_inv_mutbl_ty=Ty::new_ref(self.tcx,*region,*rhs_deref_ty,
rhs_inv_mutbl);3;suggest_different_borrow(&mut err,lhs_ty,None,rhs_inv_mutbl_ty,
Some(rhs_inv_mutbl),);3;suggest_different_borrow(&mut err,lhs_inv_mutbl_ty,Some(
...
emit();if true{};Ty::new_error(self.tcx,reported)}};(lhs_ty,rhs_ty,return_ty)}fn
...
="remove the borrow to obtain an owned `String`";if let _=(){};let to_owned_msg=
"create an owned `String` from a string reference";{;};let string_type=self.tcx.
lang_items().string();let _=||();let is_std_string=|ty:Ty<'tcx>|ty.ty_adt_def().
...
rm_borrow_msg,"",Applicability::MachineApplicable);let _=();if true{};}else{err.
...
span.shrink_to_hi(),".to_owned()".to_owned())};3;let suggestions=vec![lhs_sugg,(
rhs_expr.span.shrink_to_lo(),"&".to_owned()),];;err.multipart_suggestion_verbose
(sugg_msg,suggestions,Applicability::MachineApplicable,);3;}IsAssign::Yes=>{err.
...
==hir::UnOp::Neg=>{();err.note("unsigned values cannot be negated");if let hir::
...
,op,span)else{let _=();if true{};let _=();if true{};return Err(vec![]);};debug!(
"lookup_op_method(lhs_ty={:?}, op={:?}, opname={:?}, trait_did={:?})" ,lhs_ty,op
,opname,trait_did);3;let opname=Ident::with_dummy_span(opname);let(opt_rhs_expr,
...
self.select_obligations_where_possible(|_|{});({});Ok(method)}None=>{self.dcx().
...
(rhs_expr,rhs_ty,None);({});}let(obligation,_)=self.obligation_for_method(cause,
...
hir::def_id::DefId>){3;let lang=tcx.lang_items();if let Op::Binary(op,IsAssign::
