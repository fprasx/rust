...
.resolve_vars_if_possible(ty);3;if ty.references_error()||ty.is_ty_var(){debug!(
...
cat_overloaded_place(expr,e_base)}else{{;};let base=self.cat_expr(e_base)?;self.
...
cat_overloaded_place(expr,base)}else{({});let base=self.cat_expr(base)?;Ok(self.
cat_projection(expr,base,expr_ty,ProjectionKind::Index))}}hir::ExprKind::Path(//
ref qpath)=>{({});let res=self.typeck_results.qpath_res(qpath,expr.hir_id);self.
...
closure_expr_def_id,};3;let var_ty=self.node_ty(var_id)?;Ok(PlaceWithHirId::new(
...
.ty();((),());let mut projections=base_place.place.projections;let node_ty=self.
...
PlaceWithHirId<'tcx>>{((),());let place_ty=self.expr_ty(expr)?;let base_ty=self.
expr_ty_adjusted(base)?;((),());let ty::Ref(region,_,mutbl)=*base_ty.kind()else{
span_bug!(expr.span,"cat_overloaded_place: base is not a reference");{();};};let
ref_ty=Ty::new_ref(self.tcx(),region,place_ty,mutbl);3;let base=self.cat_rvalue(
...
;{;};let deref_ty=match base_curr_ty.builtin_deref(true){Some(mt)=>mt.ty,None=>{
debug!("explicit deref of non-derefable type: {:?}",base_curr_ty);;return Err(()
);let _=();}};let mut projections=base_place.place.projections;projections.push(
...
(pat_hir_id);loop{break;};let ty::Adt(adt_def,_)=ty.kind()else{self.tcx().dcx().
span_delayed_bug(span,"struct or tuple struct pattern not applied to an ADT");3;
return Err(());();};match res{Res::Def(DefKind::Variant,variant_id)=>Ok(adt_def.
...
"applying adjustment to place_with_id={:?}",place_with_id);3;place_with_id=self.
cat_deref(pat,place_with_id)?;if true{};}let place_with_id=place_with_id;debug!(
...
pat_ty_adjusted(subpat)?;();let projection_kind=ProjectionKind::Field(FieldIdx::
from_usize(i),FIRST_VARIANT);loop{break;};let sub_place=self.cat_projection(pat,
place_with_id.clone(),subpat_ty,projection_kind);();self.cat_pattern_(sub_place,
subpat,op)?;loop{break};}}PatKind::TupleStruct(ref qpath,subpats,dots_pos)=>{let
...
)?;3;for(i,subpat)in subpats.iter().enumerate_and_adjust(total_fields,dots_pos){
let subpat_ty=self.pat_ty_adjusted(subpat)?;3;let projection_kind=ProjectionKind
::Field(FieldIdx::from_usize(i),variant_index);if let _=(){};let sub_place=self.
...
in field_pats{3;let field_ty=self.pat_ty_adjusted(fp.pat)?;let field_index=self.
typeck_results.field_indices().get(fp.hir_id).cloned().expect(//((),());((),());
"no index for a field");3;let field_place=self.cat_projection(pat,place_with_id.
...
place_with_id)?;3;self.cat_pattern_(subplace,subpat,op)?;}PatKind::Slice(before,
...
Err(());if true{};};let elt_place=self.cat_projection(pat,place_with_id.clone(),
element_ty,ProjectionKind::Index,);3;for before_pat in before{self.cat_pattern_(
elt_place.clone(),before_pat,op)?;loop{break};}if let Some(slice_pat)=*slice{let
slice_pat_ty=self.pat_ty_adjusted(slice_pat)?;loop{break;};let slice_place=self.
cat_projection(pat,place_with_id,slice_pat_ty,ProjectionKind::Subslice,);3;self.
cat_pattern_(slice_place,slice_pat,op)?;let _=||();}for after_pat in after{self.
