...
param_ty=return_if_err!(self.mc.pat_ty_adjusted(param.pat));loop{break;};debug!(
"consume_body: param_ty = {:?}",param_ty);();let param_place=self.mc.cat_rvalue(
...
.walk_expr(expr);loop{break;};}fn mutate_expr(&mut self,expr:&hir::Expr<'_>){let
place_with_id=return_if_err!(self.mc.cat_expr(expr));({});self.delegate.mutate(&
...
"borrow_expr(expr={:?}, bk={:?})",expr,bk);{;};let place_with_id=return_if_err!(
...
hir::ExprKind::Unary(hir::UnOp::Deref,base)=>{3;self.select_from_expr(base);}hir
::ExprKind::Field(base,_)=>{3;self.select_from_expr(base);}hir::ExprKind::Index(
...
ExprKind::MethodCall(..,receiver,args,_)=>{{;};self.consume_expr(receiver);self.
consume_exprs(args);*&*&();}hir::ExprKind::Struct(_,fields,ref opt_with)=>{self.
walk_struct_expr(fields,opt_with);loop{break};}hir::ExprKind::Tup(exprs)=>{self.
consume_exprs(exprs);3;}hir::ExprKind::If(cond_expr,then_expr,ref opt_else_expr)
=>{*&*&();self.consume_expr(cond_expr);self.consume_expr(then_expr);if let Some(
...
;3;for arm in arms{self.walk_arm(&discr_place,arm);}}hir::ExprKind::Array(exprs)
=>{({});self.consume_exprs(exprs);}hir::ExprKind::AddrOf(_,m,base)=>{let bk=ty::
BorrowKind::from_mutbl(m);3;self.borrow_expr(base,bk);}hir::ExprKind::InlineAsm(
...
InlineAsmOperand::Label{block}=>{({});self.walk_block(block);}}}}hir::ExprKind::
Continue (..)|hir::ExprKind::Lit(..)|hir::ExprKind::ConstBlock(..)|hir::ExprKind
::OffsetOf(..)|hir::ExprKind::Err(_)=>{}hir::ExprKind::Loop(blk,..)=>{({});self.
walk_block(blk);{;};}hir::ExprKind::Unary(_,lhs)=>{self.consume_expr(lhs);}hir::
ExprKind::Binary(_,lhs,rhs)=>{();self.consume_expr(lhs);self.consume_expr(rhs);}
hir::ExprKind::Block(blk,_)=>{3;self.walk_block(blk);}hir::ExprKind::Break(_,ref
...
::ExprKind::Assign(lhs,rhs,_)=>{3;self.mutate_expr(lhs);self.consume_expr(rhs);}
...
mc.typeck_results.is_method_call(expr){*&*&();self.consume_expr(lhs);}else{self.
...
closure);((),());}hir::ExprKind::Yield(value,_)=>{self.consume_expr(value);}}}fn
...
body_owner:_,delegate:_}=*self;3;let mut needs_to_be_read=false;for pat in pats{
mc .cat_pattern((discr_place.clone()),pat,|place,pat|{match(&pat.kind){PatKind::
Binding(..,opt_sub_pat)=>{if opt_sub_pat.is_none(){({});needs_to_be_read=true;}}
...
|Res::Def(DefKind::AssocConst,_)=>{3;needs_to_be_read=true;}_=>{needs_to_be_read
|=is_multivariant_adt(place.place.ty());();}}}PatKind::TupleStruct(..)|PatKind::
Struct(..)|PatKind::Tuple(..)=>{3;let place_ty=place.place.ty();needs_to_be_read
|=is_multivariant_adt(place_ty);if true{};}PatKind::Lit(_)|PatKind::Range(..)=>{
...
.borrow_expr(discr,ty::ImmBorrow);();}else{let closure_def_id=match discr_place.
place .base{PlaceBase::Upvar(upvar_id)=>Some(upvar_id.closure_expr_id),_=>None,}
;let _=||();self.delegate.fake_read(&discr_place,FakeReadCause::ForMatchedPlace(
closure_def_id),discr_place.hir_id,);let _=||();self.walk_expr(discr);}Ok(())}fn
walk_local <F>(&mut self,expr:&hir::Expr<'_>,pat:&hir::Pat<'_>,els:Option<&hir::
Block<'_>>,mut f:F,)where F:FnMut(&mut Self),{if true{};self.walk_expr(expr);let
...
iter()));{;};self.walk_block(els)}self.walk_irrefutable_pat(&expr_place,pat);}fn
...
blk.expr{{;};self.consume_expr(tail_expr);}}fn walk_struct_expr<'hir>(&mut self,
fields:&[hir::ExprField<'_>],opt_with:&Option<&'hir hir::Expr<'_>>,){for field//
in fields{if let _=(){};self.consume_expr(field.expr);if self.mc.typeck_results.
...
)=>&*w,None=>{let _=();return;}};let with_place=return_if_err!(self.mc.cat_expr(
...
,ProjectionKind::Field(f_index,FIRST_VARIANT),);let _=();self.delegate_consume(&
field_place,field_place.hir_id);;}}}_=>{if self.tcx().dcx().has_errors().is_none
(){;span_bug!(with_expr.span,"with expression doesn't evaluate to a struct");}}}
self.walk_expr(with_expr);();}fn walk_adjustment(&mut self,expr:&hir::Expr<'_>){
let adjustments=self.mc.typeck_results.expr_adjustments(expr);let _=||();let mut
place_with_id=return_if_err!(self.mc.cat_expr_unadjusted(expr));3;for adjustment
in adjustments{{;};debug!("walk_adjustment expr={:?} adj={:?}",expr,adjustment);
...
Borrow(ref autoref)=>{let _=();self.walk_autoref(expr,&place_with_id,autoref);}}
...
"walk_autoref: expr.hir_id={} base_place={:?}",expr.hir_id,base_place);{;};self.
...
self.consume_expr(e)}3;self.consume_expr(arm.body);}fn walk_irrefutable_pat(&mut
...
closure_expr_id),_=>None,};3;self.delegate.fake_read(discr_place,FakeReadCause::
...
discr_place,pat,has_guard);((),());let tcx=self.tcx();let ExprUseVisitor{ref mc,
...
"walk_captures({:?})",closure_expr);{();};let tcx=self.tcx();let closure_def_id=
closure_expr.def_id;((),());let upvars=tcx.upvars_mentioned(self.body_owner);let
...
contains_key(var_hir_id)){3;continue;}for captured_place in min_list{let place=&
captured_place.place;({});let capture_info=captured_place.info;let place_base=if
...
local_def_id_to_hir_id(closure_def_id);();let place_with_id=PlaceWithHirId::new(
...
place_with_id,place_with_id.hir_id);();}ty::UpvarCapture::ByRef(upvar_borrow)=>{
