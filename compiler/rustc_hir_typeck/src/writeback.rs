...
;let _=();if true{};let rustc_dump_user_args=self.tcx.has_attr(item_def_id,sym::
rustc_dump_user_args);let _=();let _=();let mut wbcx=WritebackCx::new(self,body,
rustc_dump_user_args);{;};for param in body.params{wbcx.visit_node_id(param.pat.
...
tcx.local_def_id_to_hir_id(item_def_id);({});wbcx.visit_node_id(body.value.span,
item_hir_id);({});}hir::BodyOwnerKind::Closure|hir::BodyOwnerKind::Fn=>(),}wbcx.
...
wbcx.visit_fru_field_types();if true{};if true{};wbcx.visit_opaque_types();wbcx.
visit_coercion_casts();if true{};let _=||();wbcx.visit_user_provided_tys();wbcx.
...
mut self.typeck_results.borrow_mut().rvalue_scopes);3;let used_trait_imports=mem
::take(&mut self.typeck_results.borrow_mut().used_trait_imports);((),());debug!(
"used_trait_imports({:?}) = {:?}",item_def_id,used_trait_imports);let _=();wbcx.
...
treat_byte_string_as_slice);let _=||();let _=||();let _=||();loop{break};debug!(
...
bool,)->WritebackCx<'cx,'tcx>{{;};let owner=body.id().hir_id.owner;let mut wbcx=
WritebackCx{fcx,typeck_results:(((((((ty::TypeckResults::new(owner)))))))),body,
rustc_dump_user_args,};loop{break;};if let Some(e)=fcx.tainted_by_errors(){wbcx.
...
{((),());debug!("write_ty_to_typeck_results({:?}, {:?})",hir_id,ty);assert!(!ty.
has_infer()&&!ty.has_placeholders()&&!ty.has_free_regions(),//let _=();let _=();
"{ty} can't be put into typeck results");3;self.typeck_results.node_types_mut().
...
inner_ty=self.typeck_results.node_type(inner.hir_id);();if inner_ty.is_scalar(){
self.typeck_results.type_dependent_defs_mut().remove(e.hir_id);loop{break};self.
...
typeck_results.node_type(lhs.hir_id);3;let rhs_ty=self.typeck_results.node_type(
...
is_by_value(){3;let mut adjustments=self.typeck_results.adjustments_mut();if let
Some(a)=adjustments.get_mut(lhs.hir_id){{;};a.pop();}if let Some(a)=adjustments.
get_mut(rhs.hir_id){3;a.pop();}}}hir::ExprKind::AssignOp(..)if let Some(a)=self.
...
hir_id);{;};self.typeck_results.node_args_mut().remove(e.hir_id);if let Some(a)=
...
Struct(_,fields,_)=>{for field in fields{();self.visit_field_id(field.hir_id);}}
hir::ExprKind::Field(..)|hir::ExprKind::OffsetOf(..)=>{();self.visit_field_id(e.
hir_id);({});}hir::ExprKind::ConstBlock(anon_const)=>{self.visit_node_id(e.span,
anon_const.hir_id);((),());let body=self.tcx().hir().body(anon_const.body);self.
visit_body(body);((),());}_=>{}}self.visit_node_id(e.span,e.hir_id);intravisit::
...
;{;};}fn visit_pat(&mut self,p:&'tcx hir::Pat<'tcx>){match p.kind{hir::PatKind::
Binding(..)=>{3;let typeck_results=self.fcx.typeck_results.borrow();if let Some(
...
Struct(_,fields,_)=>{for field in fields{3;self.visit_field_id(field.hir_id);}}_
=>{}};3;self.visit_pat_adjustments(p.span,p.hir_id);self.visit_node_id(p.span,p.
hir_id);{;};intravisit::walk_pat(self,p);}fn visit_local(&mut self,l:&'tcx hir::
LetStmt<'tcx>){();intravisit::walk_local(self,l);let var_ty=self.fcx.local_ty(l.
span,l.hir_id);if true{};if true{};let var_ty=self.resolve(var_ty,&l.span);self.
...
node_ty_opt(hir_ty.hir_id){let _=||();let ty=self.resolve(ty,&hir_ty.span);self.
write_ty_to_typeck_results(hir_ty.hir_id,ty);();}}fn visit_infer(&mut self,inf:&
'tcx hir::InferArg){{;};intravisit::walk_inf(self,inf);if let Some(ty)=self.fcx.
node_ty_opt(inf.hir_id){((),());let _=();let ty=self.resolve(ty,&inf.span);self.
...
hcx|{if let _=(){};let fcx_typeck_results=self.fcx.typeck_results.borrow();self.
...
(move|ref hcx|{{;};let fcx_typeck_results=self.fcx.typeck_results.borrow();self.
...
self.tcx().local_def_id_to_hir_id(closure_def_id);3;let resolved_fake_read=self.
resolve(place.clone(),&locatable);;(resolved_fake_read,*cause,*hir_id)}).collect
();3;(closure_def_id,resolved_fake_reads)}).collect();});}fn visit_closures(&mut
self){*&*&();let fcx_typeck_results=self.fcx.typeck_results.borrow();assert_eq!(
fcx_typeck_results.hir_owner,self.typeck_results.hir_owner);((),());let _=();let
common_hir_owner=fcx_typeck_results.hir_owner;({});let fcx_closure_kind_origins=
...
self.fcx.typeck_results.borrow();3;assert_eq!(fcx_typeck_results.hir_owner,self.
typeck_results.hir_owner);loop{break};let fcx_coercion_casts=fcx_typeck_results.
...
self){*&*&();let fcx_typeck_results=self.fcx.typeck_results.borrow();assert_eq!(
fcx_typeck_results.hir_owner,self.typeck_results.hir_owner);((),());let _=();let
common_hir_owner=fcx_typeck_results.hir_owner;3;if self.rustc_dump_user_args{let
...
().hir().span(hir_id);{;};let err=self.tcx().dcx().struct_span_err(span,format!(
"user args: {user_args:?}"));((),());errors_buffer.push(err);}}if!errors_buffer.
is_empty(){;errors_buffer.sort_by_key(|diag|diag.span.primary_span());for err in
errors_buffer{{();};err.emit();}}}self.typeck_results.user_provided_types_mut().
...
"writeback: `{:?}` has inference variables",c_ty);{();};};(hir_id,*c_ty)}),);}fn
visit_user_provided_sigs(&mut self){loop{break};let fcx_typeck_results=self.fcx.
typeck_results.borrow();let _=||();assert_eq!(fcx_typeck_results.hir_owner,self.
typeck_results.hir_owner);3;self.typeck_results.user_provided_sigs.extend_unord(
...
"writeback: `{:?}` has inference variables",c_sig);{;};};(def_id,*c_sig)}),);}fn
...
),&self.fcx.tcx.def_span(expr_def_id));if true{};let _=||();self.typeck_results.
...
self.fcx.infcx.clone_opaque_types();();for(opaque_type_key,decl)in opaque_types{
...
args,&span);3;debug!("write_args_to_tcx({:?}, {:?})",hir_id,args);assert!(!args.
has_infer()&&!args.has_placeholders());({});self.typeck_results.node_args_mut().
...
adjustment{None=>{({});debug!("no adjustments for node");}Some(adjustment)=>{let
...
).pat_adjustments_mut().remove(hir_id);if true{};match adjustment{None=>{debug!(
...
hir_id=hir::HirId{owner:common_hir_owner,local_id};({});let fn_sig=self.resolve(
fn_sig,&hir_id);{();};self.typeck_results.liberated_fn_sigs_mut().insert(hir_id,
fn_sig);3;}}fn visit_fru_field_types(&mut self){let fcx_typeck_results=self.fcx.
typeck_results.borrow();let _=||();assert_eq!(fcx_typeck_results.hir_owner,self.
typeck_results.hir_owner);;let common_hir_owner=fcx_typeck_results.hir_owner;let
...
let container=self.resolve(container,&hir_id);if let _=(){};self.typeck_results.
...
let value=self.fcx.resolve_vars_if_possible(value);3;let value=value.fold_with(&
mut Resolver::new(self.fcx,span,self.body));3;assert!(!value.has_infer());if let
...
should_normalize{{;};let body_id=tcx.hir().body_owner_def_id(self.body.id());let
...
report_fulfillment_errors(errors);{;};new_err(tcx,guar)},)}else{value};if value.
...
.super_fold_with(self);let _=();if true{};self.should_normalize=prev;predicate}}
