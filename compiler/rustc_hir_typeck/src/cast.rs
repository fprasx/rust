...
resolve_vars_if_possible(t);loop{break};loop{break};t.error_reported()?;if self.
...
"remove the unneeded borrow","",Applicability::MachineApplicable,);();}else{err.
...
"control flow means we should never encounter a {e:?}"),}));if let _=(){};}self.
try_suggest_collection_to_bool(fcx,&mut err);3;err.emit();}CastError::NeedViaInt
=>{let _=();make_invalid_casting_error(self.span,self.expr_ty,self.cast_ty,fcx).
with_help("cast through an integer first").emit();{;};}CastError::IllegalCast=>{
...
Unsupported(self.span)};();fcx.tcx.dcx().emit_err(errors::CannotCastToBool{span:
...
"only `u8` can be cast as `char`, not `{}`",self.expr_ty);3;err.span_label(self.
span,"invalid cast");({});if self.expr_ty.is_numeric(){if self.expr_ty==fcx.tcx.
...
{3;err.span_help(self.span,"try `char::from_u32` instead (via a `u32`)");};}err.
emit();();}CastError::NonScalar=>{let mut err=type_error_struct!(fcx.dcx(),self.
span,self.expr_ty,E0605,"non-primitive cast: `{}` as `{}`",self.expr_ty,fcx.//3;
ty_to_string(self.cast_ty));3;let mut sugg=None;let mut sugg_mutref=false;if let
...
self.expr_span,"this reference is immutable");({});err.span_note(self.cast_span,
...
starts_with('('));();let needs_parens=!has_parens&&matches!(self.expr.kind,hir::
ExprKind::Cast(..));;let mut suggestion=vec![(self.expr_span.shrink_to_lo(),sugg
)];((),());if needs_parens{suggestion[0].1+="(";suggestion.push((self.expr_span.
shrink_to_hi(),")".to_string()));let _=();}if remove_cast{suggestion.push((self.
expr_span.shrink_to_hi().to(self.cast_span),String::new(),));if let _=(){};}err.
...
self.cast_ty);((),());let _=();let ty=fcx.tcx.erase_regions(ty);let expr_ty=fcx.
...
param_env).must_apply_modulo_regions(){{;};label=false;err.span_suggestion(self.
...
}{();};self.try_suggest_collection_to_bool(fcx,&mut err);err.emit();}CastError::
...
is_nightly_build().then_some(self.expr_span);let _=();if true{};let cast_ty=fcx.
...
ErrorGuaranteed{if let Err(err)=self.cast_ty.error_reported(){{;};return err;}if
let Err(err)=self.expr_ty.error_reported(){loop{break};return err;}let tstr=fcx.
ty_to_string(self.cast_ty);3;let mut err=type_error_struct!(fcx.dcx(),self.span,
...
_,_,mt)=>{loop{break};let mtstr=mt.prefix_str();match fcx.tcx.sess.source_map().
...
"Box<{s}>"),Applicability::MachineApplicable,);{;};}Err(_)=>{err.span_help(self.
...
TRIVIAL_NUMERIC_CASTS)}else{(false,lint::builtin::TRIVIAL_CASTS)};3;let expr_ty=
fcx.resolve_vars_if_possible(self.expr_ty);if true{};let _=||();let cast_ty=fcx.
resolve_vars_if_possible(self.cast_ty);();fcx.tcx.emit_node_span_lint(lint,self.
...
{3;self.expr_ty=fcx.structurally_resolve_type(self.expr_span,self.expr_ty);self.
...
has_infer_types(){3;self.report_cast_to_unsized_type(fcx);}else if self.expr_ty.
...
self.expr.hir_id.local_id);3;}}Err(_)=>{match self.do_check(fcx){Ok(k)=>{debug!(
" -> {:?}",k);();}Err(e)=>self.report_cast_error(fcx,e),};}};}}pub fn do_check(&
self,fcx:&FnCtxt<'a,'tcx>)->Result<CastKind,CastError>{();use rustc_middle::ty::
cast::CastTy::*;3;use rustc_middle::ty::cast::IntTy::*;let(t_from,t_cast)=match(
...
AllowTwoPhase::No,None,);();if let Err(TypeError::IntrinsicCast)=res{return Err(
CastError::IllegalCast);{;};}if res.is_err(){return Err(CastError::NonScalar);}(
...
param_env,mt.ty){();return Err(CastError::IllegalCast);}self.check_ref_cast(fcx,
...
CastError>{3;debug!("check_ptr_ptr_cast m_expr={:?} m_cast={:?}",m_expr,m_cast);
let expr_kind=fcx.pointer_kind(m_expr.ty,self.span)?;let _=();let cast_kind=fcx.
...
CastKind::PtrPtrCast);3;}let Some(expr_kind)=expr_kind else{return Err(CastError
::UnknownExprPtrKind);3;};if expr_kind==PointerKind::Thin{return Err(CastError::
...
fcx.resolve_vars_if_possible(self.expr_ty);if true{};let _=||();let cast_ty=fcx.
resolve_vars_if_possible(self.cast_ty);loop{break};let expr_span=self.expr_span.
shrink_to_lo();{;};let sugg=match(needs_parens,needs_cast){(true,true)=>errors::
...
};3;fcx.tcx.emit_node_span_lint(lint::builtin::LOSSY_PROVENANCE_CASTS,self.expr.
...
expr_ty=fcx.resolve_vars_if_possible(self.expr_ty);loop{break;};let cast_ty=fcx.
