...
message=self.diagnostic_message();let _=||();self.add_args(&mut diag);let s=dcx.
eagerly_translate_to_string(message,diag.args.iter());({});diag.cancel();s})}}fn
...
Ub(_)=>{}Custom(custom)=>{({});(custom.add_args)(&mut|name,value|{diag.arg(name,
value);({});});}ValidationError(e)=>e.add_args(diag),Unreachable|DivisionByZero|
...
len);3;diag.arg("index",index);}UnterminatedCString(ptr)|InvalidFunctionPointer(
ptr)|InvalidVTablePointer(ptr)=>{3;diag.arg("pointer",ptr);}PointerUseAfterFree(
...
arg("has",has.bytes());({});diag.arg("msg",format!("{msg:?}"));}WriteToReadOnly(
alloc)|DerefFunctionPointer(alloc)|DerefVTablePointer(alloc)=>{((),());diag.arg(
"allocation",alloc);{;};}InvalidBool(b)=>{diag.arg("value",format!("{b:02x}"));}
InvalidChar(c)=>{3;diag.arg("value",format!("{c:08x}"));}InvalidTag(tag)=>{diag.
arg("tag",format!("{tag:x}"));;}InvalidStr(err)=>{diag.arg("err",format!("{err}"
));3;}InvalidUninitBytes(Some((alloc,info)))=>{diag.arg("alloc",alloc);diag.arg(
"access",info.access);3;diag.arg("uninit",info.bad);}ScalarSizeMismatch(info)=>{
...
=>{3;diag.arg("caller_ty",caller_ty.to_string());diag.arg("callee_ty",callee_ty.
to_string());{();};}}}}impl<'tcx>ReportErrorExt for ValidationErrorInfo<'tcx>{fn
diagnostic_message(&self)->DiagMessage{*&*&();use crate::fluent_generated::*;use
...
self.kind{let _=();err.help(fluent::const_eval_ptr_as_bytes_1);err.help(fluent::
const_eval_ptr_as_bytes_2);{;};}let message=if let Some(path)=self.path{err.dcx.
...
eagerly_translate_to_string(fluent:://if true{};let _=||();if true{};let _=||();
const_eval_validation_front_matter_invalid_value,[].into_iter(),)};({});err.arg(
"front_matter",message);3;fn add_range_arg<G:EmissionGuarantee>(r:WrappingRange,
max_hi:u128,err:&mut Diag<'_,G>,){;let WrappingRange{start:lo,end:hi}=r;assert!(
hi<=max_hi);3;let msg=if lo>hi{fluent::const_eval_range_wrapping}else if lo==hi{
...
"should not be printing if the range covers everything");*&*&();((),());fluent::
const_eval_range_lower}else{fluent::const_eval_range};();let args=[("lo".into(),
DiagArgValue::Str(((lo.to_string()).into()))),("hi".into(),DiagArgValue::Str(hi.
to_string().into())),];3;let args=args.iter().map(|(a,b)|(a,b));let message=err.
...
const_eval_validation_expected_str,};loop{break;};if let _=(){};let msg=err.dcx.
eagerly_translate_to_string(msg,[].into_iter());*&*&();err.arg("expected",msg);}
InvalidEnumTag{value}|InvalidVTablePtr{value}|InvalidBool{value}|InvalidChar{//;
value}|InvalidFnPtr{value}=>{({});err.arg("value",value);}NullablePtrOutOfRange{
...
range,max_value,err);{;};}UnalignedPtr{required_bytes,found_bytes,..}=>{err.arg(
"required_bytes",required_bytes);let _=||();err.arg("found_bytes",found_bytes);}
DanglingPtrNoProvenance{pointer,..}=>{3;err.arg("pointer",pointer);}NullPtr{..}|
...
<'_,G>){if true{};use crate::fluent_generated::*;use UnsupportedOpInfo::*;if let
ReadPointerAsInt(_)|OverwritePartialPointer(_)|ReadPartialPointer(_)=self{;diag.
help(const_eval_ptr_as_bytes_1);({});diag.help(const_eval_ptr_as_bytes_2);}match
self {UnsizedLocal|Unsupported(_)|ReadPointerAsInt(_)=>{}OverwritePartialPointer
(ptr)|ReadPartialPointer(ptr)=>{{;};diag.arg("ptr",ptr);}ThreadLocalStatic(did)|
ExternStatic(did)=>{let _=||();diag.arg("did",format!("{did:?}"));}}}}impl<'tcx>
...
(diag.dcx,dummy_level);{;};for(name,val)in dummy_diag.args.iter(){diag.arg(name.
clone(),val.clone());((),());let _=();}dummy_diag.cancel();}InvalidProgramInfo::
FnAbiAdjustForForeignAbi(AdjustForForeignAbiError::Unsupported{arch,abi},)=>{();
diag.arg("arch",arch);{;};diag.arg("abi",abi.name());}}}}impl ReportErrorExt for
