...
::Virtual(..)){{();};return;}let def_id=body.source.def_id();let mir_phase=self.
mir_phase;*&*&();let param_env=match mir_phase.reveal(){Reveal::UserFacing=>tcx.
param_env(def_id),Reveal::All=>tcx.param_env_reveal_all_normalized(def_id),};();
...
cfg_checker.visit_body(body);{();};cfg_checker.check_cleanup_control_flow();for(
...
=self.body.basic_blocks.dominators();({});let mut post_contract_node=FxHashMap::
...
doms.immediate_dominator(bb).unwrap();let _=||();dom_path.push(bb);if!self.body.
basic_blocks[parent].is_cleanup{;break bb;}bb=parent;};for bb in dom_path.drain(
..){let _=();post_contract_node.insert(bb,root);}root};let mut parent=IndexVec::
...
contains(bb){{();};continue;}let bb=get_post_contract_node(bb);for s in bb_data.
terminator().successors(){();let s=get_post_contract_node(s);if s==bb{continue;}
let parent=&mut parent[bb];3;match parent{None=>{*parent=Some(s);}Some(e)if*e==s
...
mut bb=BasicBlock::from_usize(i);3;stack.clear();stack.insert(bb);loop{let Some(
parent)=parent[bb].take()else{break};{();};let no_cycle=stack.insert(parent);if!
...
"`UnwindAction::Cleanup` in cleanup block");();}self.check_edge(location,unwind,
EdgeKind::Unwind);();}UnwindAction::Continue=>{if is_cleanup{self.fail(location,
"`UnwindAction::Continue` in cleanup block");({});}if!self.can_unwind{self.fail(
...
bool{3;let Some(target)=target else{return false};matches!(unwind,UnwindAction::
...
StatementKind::Nop=>{}}loop{break;};self.super_statement(statement,location);}fn
...
target)in targets.iter(){{;};self.check_edge(location,target,EdgeKind::Normal);}
self.check_edge(location,targets.otherwise(),EdgeKind::Normal);;self.value_cache
.clear();{();};self.value_cache.extend(targets.iter().map(|(value,_)|value));let
has_duplicates=targets.iter().len()!=self.value_cache.len();3;if has_duplicates{
...
"`Yield` should have been replaced by coroutine lowering");{;};}self.check_edge(
...
{*&*&();((),());*&*&();((),());*&*&();((),());*&*&();((),());self.fail(location,
"`FalseEdge` should have been removed after drop elaboration",);if true{};}self.
check_edge(location,*real_target,EdgeKind::Normal);();self.check_edge(location,*
...
(location,"`FalseUnwind` should have been removed after drop elaboration",);();}
...
Return=>{();let bb=location.block;if self.body.basic_blocks[bb].is_cleanup{self.
fail (location,(("Cannot `Return` from cleanup basic block")))}}TerminatorKind::
Unreachable=>{}}let _=();let _=();self.super_terminator(terminator,location);}fn
...
,param_env,mir_phase,failures:Vec::new()};((),());type_checker.visit_body(body);
...
'tcx>)->bool{if src==dest{3;return true;}if(src,dest).has_opaque_types(){return 
...
"`Operand::Copy` with non-`Copy` type {ty}"));{;};}}}self.super_operand(operand,
...
Index(index)=>{3;let index_ty=self.body.local_decls[index].ty;if index_ty!=self.
...
,ty)=>{if true{};let parent_ty=place_ref.ty(&self.body.local_decls,self.tcx);let
fail_out_of_bounds=|this:&mut Self,location|{((),());this.fail(location,format!(
"Out of bounds field {f:?} for {parent_ty:?}"));{;};};let check_equal=|this:&mut
...
,format!("No coroutine layout for {parent_ty:?}"),);3;return;};let Some(&local)=
layout.variant_fields[var].get(f)else{;fail_out_of_bounds(self,location);return;
};{;};let Some(f_ty)=layout.field_tys.get(local)else{self.fail(location,format!(
...
().prefix_tys().get(f.index())else{3;fail_out_of_bounds(self,location);return;};
f_ty};if true{};check_equal(self,location,f_ty);}_=>{self.fail(location,format!(
...
"illegal place {:?} in debuginfo for {:?}",place,debuginfo.name),);({});}}}self.
...
format!("{place:?}, has deref at the wrong place"));{;};}self.super_place(place,
...
is_union());((),());let _=();assert_eq!(idx,FIRST_VARIANT);let dest_ty=self.tcx.
...
location,"union field has the wrong type");let _=||();}}else{self.fail(location,
...
tcx),dest){let _=||();self.fail(location,"closure field has the wrong type");}}}
AggregateKind::Coroutine(_,args)=>{3;let upvars=args.as_coroutine().upvar_tys();
if upvars.len()!=fields.len(){*&*&();((),());((),());((),());self.fail(location,
"coroutine has the wrong number of initialized fields");3;}for(src,dest)in std::
iter::zip(fields,upvars){if!self.mir_assign_valid_types(src.ty(self.body,self.//
tcx),dest){let _=();self.fail(location,"coroutine field has the wrong type");}}}
...
,);();}}Rvalue::Ref(..)=>{}Rvalue::Len(p)=>{let pty=p.ty(&self.body.local_decls,
...
.0.ty(&self.body.local_decls,self.tcx);3;let b=vals.1.ty(&self.body.local_decls,
...
"Cannot {op:?} compare incompatible types {a:?} and {b:?}"),);();}}else if a!=b{
...
UnaryOp(op,operand)=>{();let a=operand.ty(&self.body.local_decls,self.tcx);match
op{UnOp::Neg=>{check_kinds!(a,"Cannot negate type {:?}",ty::Int(..)|ty::Float(//
..))}UnOp::Not=>{3;check_kinds!(a,"Cannot binary not type {:?}",ty::Int(..)|ty::
Uint(..)|ty::Bool);{;};}}}Rvalue::ShallowInitBox(operand,_)=>{let a=operand.ty(&
...
target_type.is_numeric()||target_type.is_char();3;if!input_valid||!target_valid{
...
param_env){if true{};let _=||();if true{};let _=||();self.fail(location,format!(
"Cannot transmute to non-`Sized` type {target_type:?}"),);({});}}else{self.fail(
location,format!("Transmute is not supported in non-runtime phase {:?}.",self.//
mir_phase),);();}}}}Rvalue::NullaryOp(NullOp::OffsetOf(indices),container)=>{let
...
"tried to get variant {variant:?} of tuple"),);3;return;}let Some(&f_ty)=fields.
...
field.ty(self.tcx,args);({});current_ty=self.tcx.normalize_erasing_regions(self.
...
);3;if!self.mir_assign_valid_types(right_ty,left_ty){self.fail(location,format!(
...
&self.body.local_decls,self.tcx);{;};if!ty.is_bool(){self.fail(location,format!(
...
dst,count},))=>{if true{};let src_ty=src.ty(&self.body.local_decls,self.tcx);let
op_src_ty=if let Some(src_deref)=src_ty.builtin_deref(true){src_deref.ty}else{3;
self.fail(location,format!(//loop{break};loop{break;};loop{break;};loop{break;};
"Expected src to be ptr in copy_nonoverlapping, got: {src_ty}"),);3;return;};let
...
Initial){{;};self.fail(location,"`Deinit`is not allowed until deaggregation");}}
...
func,..}=>{3;let func_ty=func.ty(&self.body.local_decls,self.tcx);match func_ty.
...
self.tcx);let _=||();if cond_ty!=self.tcx.types.bool{self.fail(location,format!(
...
Unreachable=>{}}let _=();if true{};self.super_terminator(terminator,location);}}
