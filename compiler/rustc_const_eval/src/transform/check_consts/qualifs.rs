...
is_trivially_const_drop(ty){let _=||();return false;}let destruct_def_id=cx.tcx.
...
infer_ctxt().build();{();};let mut selcx=SelectionContext::new(&infcx);let Some(
...
ImplSource::Param(_)){({});return true;}if impl_src.borrow_nested_obligations().
is_empty(){*&*&();((),());return false;}let ocx=ObligationCtxt::new(&infcx);ocx.
register_obligations(impl_src.nested_obligations());loop{break;};let errors=ocx.
...
tcx).ty;();if let ty::Ref(..)=base_ty.kind(){return in_place::<Q,_>(cx,in_local,
...
is_union()&&Q::in_any_value_of_ty(cx,rvalue.ty(cx.body,cx.tcx)){3;return true;}}
...
Qualif,F:FnMut(Local)->bool,{{;};let mut place=place;while let Some((place_base,
...
ProjectionElem::Index(_)=>{}}{();};let base_ty=place_base.ty(cx.body,cx.tcx);let
proj_ty=base_ty.projection_ty(cx.tcx,elem).ty;{();};if!Q::in_any_value_of_ty(cx,
proj_ty){loop{break};return false;}if matches!(elem,ProjectionElem::Deref)&&!Q::
...
c)=>c,};3;let uneval=match constant.const_{Const::Ty(ct)if matches!(ct.kind(),ty
...
ALLOW_PROMOTED);3;if promoted.is_none()&&cx.tcx.trait_of_item(def).is_none(){let
