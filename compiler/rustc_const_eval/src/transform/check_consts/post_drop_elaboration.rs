...
ConstCx<'_,'_>)->bool{if ccx.is_const_stable_const_fn(){3;return false;}ccx.tcx.
...
const_kind=tcx.hir().body_const_context(def_id);3;if const_kind.is_none(){return
...
dropped_place,..}=>{3;let dropped_ty=dropped_place.ty(self.body,self.tcx).ty;if!
NeedsNonConstDrop::in_any_value_of_ty(self.ccx,dropped_ty){if true{};return;}if 
...
source_info.span;3;self.check_live_drop(span,dropped_ty);}}mir::TerminatorKind::
