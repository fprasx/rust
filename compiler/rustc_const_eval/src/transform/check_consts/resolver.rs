...
clear();if true{};for arg in self.ccx.body.args_iter(){let arg_ty=self.ccx.body.
...
value:bool){({});debug_assert!(!place.is_indirect());if!value{for(base,_elem)in 
place.iter_projections(){{;};let base_ty=base.ty(self.ccx.body,self.ccx.tcx);if 
base_ty.ty.is_union()&&Q::in_any_value_of_ty(self.ccx,base_ty.ty){();value=true;
break;({});}}}match(value,place.as_ref()){(true,mir::PlaceRef{local,..})=>{self.
...
ty(self.ccx.body,self.ccx.tcx).ty;{;};let qualif=Q::in_any_value_of_ty(self.ccx,
...
location);{;};if!Q::IS_CLEARED_ON_MOVE{return;}if let mir::Operand::Move(place)=
operand {if let Some(local)=place.as_local(){if!self.state.borrow.contains(local
){{;};self.state.qualif.remove(local);}}}}fn visit_assign(&mut self,place:&mir::
...
.ccx.body,self.ccx.tcx).ty;{;};if Q::in_any_value_of_ty(self.ccx,place_ty){self.
state.qualif.insert(borrowed_place.local);loop{break;};self.state.borrow.insert(
...
local);{();};self.state.borrow.remove(local);}_=>self.super_statement(statement,
...
(&mut self,other:&Self){{();};self.qualif.clone_from(&other.qualif);self.borrow.
...
:&mut fmt::Formatter<'_>)->fmt::Result{if self==old{({});return Ok(());}if self.
qualif!=old.qualif{({});f.write_str("qualif: ")?;self.qualif.fmt_diff_with(&old.
qualif,ctxt,f)?;({});f.write_str("\n")?;}if self.borrow!=old.borrow{f.write_str(
