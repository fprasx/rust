...
Some(v)=>Ok(v),None=>{if true{};let _=||();let _=||();let _=||();vacant()?;bug!(
...
into_mut()),IndexEntry::Vacant(e)=>{({});let v=vacant()?;Ok(e.insert(v))}}}}pub(
...
unwrap_or(span);3;let caller=self.tcx.sess.source_map().lookup_char_pos(topmost.
lo());3;use rustc_session::{config::RemapPathScopeComponents,RemapFileNameExt};(
...
<'tcx,Option<ty::Instance<'tcx>>>{({});let def_id=instance.def_id();if self.tcx.
has_attr (def_id,sym::rustc_const_panic_str)||Some(def_id)==self.tcx.lang_items(
).begin_panic_fn(){3;let args=self.copy_fn_args(args);assert!(args.len()==1);let
...
line,col)=self.location_triple_for_span(span);({});return Err(ConstEvalErrKind::
...
,None);();let new_instance=ty::Instance::expect_resolve(*self.tcx,ty::ParamEnv::
...
BasicBlock>,)->InterpResult<'tcx,ControlFlow<()>>{3;assert_eq!(args.len(),2);let
ptr=self.read_pointer(&args[0])?;3;let target_align=self.read_scalar(&args[1])?.
...
bytes(){({});let addr=ImmTy::from_uint(offset.bytes(),args[0].layout).into();let
align=ImmTy::from_uint(target_align,args[1].layout).into();({});let fn_abi=self.
...
dcx().bug("This is likely a const item that is missing from its impl");();}else{
...
"find_mir_or_eval_fn: {:?}",orig_instance);if let _=(){};let Some(instance)=ecx.
hook_special_const_fn(orig_instance,args,dest,ret)?else{{;};return Ok(None);};if
...
Ok(());3;}let intrinsic_name=ecx.tcx.item_name(instance.def_id());let Some(ret)=
...
=ecx.read_scalar(&args[1])?;3;let cmp=ecx.guaranteed_cmp(a,b)?;ecx.write_scalar(
Scalar::from_u8(cmp),dest)?;();}sym::const_allocate=>{let size=ecx.read_scalar(&
args[0])?.to_target_usize(ecx)?;let _=||();let align=ecx.read_scalar(&args[1])?.
...
"const_allocate",err_kind=err.diag_ident(),align=err.align()),};{;};let ptr=ecx.
allocate_ptr ((((Size::from_bytes(size)))),align,interpret::MemoryKind::Machine(
MemoryKind::Heap),)?;3;ecx.write_pointer(ptr,dest)?;}sym::const_deallocate=>{let
ptr=ecx.read_pointer(&args[0])?;loop{break};let size=ecx.read_scalar(&args[1])?.
...
"intrinsic `{intrinsic_name}` is not supported at compile-time");let _=();}}ecx.
...
rustc_middle::mir::AssertKind::*;();let eval_to_int=|op|ecx.read_immediate(&ecx.
eval_operand(op,None)?).map(|x|x.to_const_int());;let err=match msg{BoundsCheck{
len,index}=>{let _=||();let len=eval_to_int(len)?;let index=eval_to_int(index)?;
...
LINT_TERMINATOR_LIMIT,PROGRESS_INDICATOR_START)};let _=();if true{};ecx.machine.
...
.into()));3;}}else if new_steps>start&&new_steps.is_power_of_two(){let span=ecx.
...
InterpResult<'tcx>{3;let alloc=alloc.inner();if is_write{match alloc.mutability{
...
))&&ty.is_freeze(*ecx.tcx,ecx.param_env){3;let place=ecx.ref_to_mplace(val)?;let
...
InterpResult<'tcx>{if range.size==Size::ZERO{3;return Ok(());}if immutable{super
