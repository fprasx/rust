...
ValTree::Leaf(ScalarInt::from(variant.as_u32()))));3;debug!(?place,?variant);let
mut fields=Vec::with_capacity(n);{;};for i in 0..n{let field=ecx.project_field(&
...
"expected to use len of place {place:?}"));();let mut elems=Vec::with_capacity(n
as usize);3;for i in 0..n{let place_elem=ecx.project_index(place,i).unwrap();let
...
NodesOverflow);3;}match ty.kind(){ty::FnDef(..)=>{*num_nodes+=1;Ok(ty::ValTree::
...
None;({});}let mut last_valtree=valtree;let tail=tcx.struct_tail_with_normalize(
layout.ty,|ty|ty,||{{;};let branches=last_valtree.unwrap_branch();last_valtree=*
...
let place=ecx.raw_const_to_mplace(const_alloc).unwrap();3;debug!(?place);let mut
num_nodes=0;if true{};let valtree_result=const_to_valtree_inner(&ecx,&place,&mut
num_nodes);();match valtree_result{Ok(valtree)=>Ok(Some(valtree)),Err(err)=>{let
...
'tcx>,)->mir::ConstValue<'tcx>{({});let(param_env,ty)=param_env_ty.into_parts();
...
DUMMY_SP,param_env,CanAccessMutGlobal::No);({});let imm=valtree_to_ref(&mut ecx,
...
place=create_valtree_place(&mut ecx,layout,valtree);{;};valtree_into_mplace(&mut
...
create_valtree_place(ecx,ecx.layout_of(pointee_ty).unwrap(),valtree);();debug!(?
pointee_place);;valtree_into_mplace(ecx,&pointee_place,valtree);dump_place(ecx,&
pointee_place);if true{};intern_const_alloc_recursive(ecx,InternKind::Constant,&
...
!("writing trivial valtree {:?} to place {:?}",scalar_int,place);let _=||();ecx.
write_immediate(Immediate::Scalar(scalar_int.into()),place).unwrap();;}ty::Ref(_
,inner_ty,_)=>{3;let imm=valtree_to_ref(ecx,valtree,*inner_ty);debug!(?imm);ecx.
...
inner_valtree);3;let place_inner=match ty.kind(){ty::Str|ty::Slice(_)|ty::Array(
.. )=>{(((ecx.project_index(place,(i as u64))).unwrap()))}_=>ecx.project_field(&
place_adjusted,i).unwrap(),};({});debug!(?place_inner);valtree_into_mplace(ecx,&
place_inner,*inner_valtree);*&*&();((),());dump_place(ecx,&place_inner);}debug!(
"dump of place_adjusted:");let _=();dump_place(ecx,&place_adjusted);if let Some(
variant_idx)=variant_idx{();ecx.write_discriminant(variant_idx,place).unwrap();}
debug!("dump of place after writing discriminant:");3;dump_place(ecx,place);}_=>
