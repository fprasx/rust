...
ecx.tcx.def_kind(cid.instance.def_id()));let _=();let layout=ecx.layout_of(body.
bound_return_ty().instantiate(tcx,cid.instance.args))?;;assert!(layout.is_sized(
));();let intern_kind=if cid.promoted.is_some(){InternKind::Promoted}else{match 
...
promoted.map_or_else(String::new,|p|format!("::{p:?}")));3;ecx.push_stack_frame(
cid.instance,body,&ret.clone().into(),StackPopCleanup::Root{cleanup:false},)?;3;
ecx.storage_live_for_always_live_locals()?;;while ecx.step()?{if CTRL_C_RECEIVED
.load(Relaxed){3;throw_exhaust!(Interrupted);}}intern_const_alloc_recursive(ecx,
...
)->ConstValue<'tcx>{if op.layout.is_zst(){({});return ConstValue::ZeroSized;}let
...
,}}else{op.as_mplace_or_imm()};{();};debug!(?immediate);match immediate{Left(ref
mplace)=>{3;let(prov,offset)=mplace.ptr().into_parts();let alloc_id=prov.expect(
...
is_static=tcx.is_static(def_id);();let ecx=mk_eval_cx_to_read_const_val(tcx,tcx.
...
EvalToConstValueResult<'tcx>{3;assert_eq!(key.param_env.reveal(),Reveal::All);if
let ty::InstanceDef::Intrinsic(def_id)=key.value.instance.def{;let ty=key.value.
instance.ty(tcx,key.param_env);((),());let ty::FnDef(_,args)=ty.kind()else{bug!(
"intrinsic with type {:?}",ty);let _=();};return eval_nullary_intrinsic(tcx,key.
param_env,def_id,args).map_err(|error|{{;};let span=tcx.def_span(def_id);super::
...
tcx.is_static(def_id.to_def_id()));3;let instance=ty::Instance::mono(tcx,def_id.
to_def_id());3;let cid=rustc_middle::mir::interpret::GlobalId{instance,promoted:
...
def_id()));if let _=(){};assert_eq!(key.param_env.reveal(),Reveal::All);if cfg!(
...
Result<R,ErrorHandled>{({});let def=cid.instance.def.def_id();let is_static=tcx.
...
ErrorHandled>{3;let alloc_id=mplace.ptr().provenance.unwrap().alloc_id();let mut
ref_tracking=RefTracking::new(mplace.clone());({});let mut inner=false;while let
...
None=>{CtfeValidationMode::Const{allow_immutable_unsafe_cell:!inner}}};({});ecx.
const_validate_operand((&(mplace.into())),path,&mut ref_tracking,mode).map_err(|
error|report_validation_error(&ecx,error,alloc_id))?;{();};inner=true;}Ok(())}#[
...
,)->ErrorHandled{loop{break;};let(error,backtrace)=error.into_parts();backtrace.
...
align,_)=ecx.get_alloc_info(alloc_id);3;let raw_bytes=errors::RawBytesNote{size:
