...
Provenance>>>{*&*&();trace!("get_vtable(trait_ref={:?})",poly_trait_ref);let(ty,
poly_trait_ref)=self.tcx.erase_regions((ty,poly_trait_ref));if true{};if true{};
ensure_monomorphic_enough(*self.tcx,ty)?;();ensure_monomorphic_enough(*self.tcx,
poly_trait_ref)?;let _=||();loop{break};let vtable_symbolic_allocation=self.tcx.
reserve_and_set_vtable_alloc(ty,poly_trait_ref);loop{break};let vtable_ptr=self.
...
trait_ref=poly_trait_ref.with_self_ty(*self.tcx,ty);({});let trait_ref=self.tcx.
...
self.get_ptr_vtable(vtable)?;({});let layout=self.layout_of(ty)?;assert!(layout.
