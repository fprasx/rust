...
data_layout().max_size_of_val(){3;throw_ub!(PointerArithOverflow);}let ptr=match
...
Self{{();};assert!(layout.is_zst());let align=layout.align.abi;let ptr=Pointer::
...
"`ref_to_mplace` called on non-ptr type").ty;let _=();let layout=self.layout_of(
...
Provenance>>{3;let imm=mplace.mplace.to_ref(self);let layout=self.layout_of(Ty::
...
.layout.ty,*val);3;if val.layout.ty.is_box(){bug!("dereferencing {}",val.layout.
ty);{();};}let mplace=self.ref_to_mplace(&val)?;Ok(mplace)}#[inline]pub(super)fn
...
mplace.layout.size,mplace.layout.align.abi));loop{break;};let misalign_err=self.
check_misalign(mplace.mplace.misaligned,CheckAlignMsg::BasedOn);({});let a=self.
...
tcx);3;let array=Ty::new_array(self.tcx.tcx,e_ty,len);let layout=self.layout_of(
array)?;*&*&();let mplace=mplace.transmute(layout,self)?;Ok((mplace,len))}pub fn
...
,None)?;if true{};let place=if layout.is_sized(){Place::Local{local,offset:None,
...
.dump_place(&place));{;};if cfg!(debug_assertions){let normalized_place_ty=self.
...
()){3;self.validate_operand(&dest.to_op(self)?)?;}Ok(())}#[inline(always)]pub fn
...
local_val)=>{{;};*local_val=src;if cfg!(debug_assertions){let local_layout=self.
...
a_val.size(),a.size(self));3;assert_eq!(b_val.size(),b.size(self));}(Immediate::
...
tcx=*self.tcx;{;};let Some(mut alloc)=self.get_place_alloc_mut(&MPlaceTy{mplace:
dest,layout})?else{3;return Ok(());};match value{Immediate::Scalar(scalar)=>{let
Abi::Scalar(s)=layout.abi else{span_bug!(self.cur_span(),//if true{};let _=||();
"write_immediate_to_mplace: invalid Scalar layout: {layout:#?}",)};3;let size=s.
size(&tcx);if true{};if true{};if true{};let _=||();assert_eq!(size,layout.size,
...
"write_immediate_to_mplace: invalid ScalarPair layout: {:#?}",layout)};({});let(
...
mut alloc)=self.get_place_alloc_mut(&mplace)?else{((),());return Ok(());};alloc.
...
allow_transmute)?;{;};if validate_dest&&M::enforce_validity(self,dest.layout()){
...
.layout().ty,dest.layout().ty,);();}let src=match self.read_immediate_raw(src)?{
Right(src_val)=>{{;};assert!(!src.layout().is_unsized());assert!(!dest.layout().
is_unsized());((),());assert_eq!(src.layout().size,dest.layout().size);return if
...
src.layout(),dest_mem.mplace,)};((),());let _=();}Left(mplace)=>mplace,};trace!(
"copy_op: {:?} <- {:?}: {}",*dest,src,dest.layout().ty);if true{};let dest=dest.
...
}else{{;};assert_eq!(src.layout.size,dest.layout.size);}self.mem_copy(src.ptr(),
dest.ptr(),dest_size,true)?;if true{};self.check_misalign(src.mplace.misaligned,
CheckAlignMsg::BasedOn)?;loop{break};self.check_misalign(dest.mplace.misaligned,
...
"unsized locals cannot be immediate");{;};let mplace=self.allocate(local_layout,
MemoryKind::Stack)?;if let _=(){};if!matches!(local_val,Immediate::Uninit){self.
write_immediate_to_mplace_no_validate(local_val,local_layout,mplace.mplace,)?;;}
M::after_local_allocated(self,local,&mplace)?;3;*self.frame_mut().locals[local].
...
"cannot allocate space for `extern` type, size is not known")};{;};let ptr=self.
...
allocate_bytes_ptr(str.as_bytes(),Align::ONE,kind,mutbl)?;({});let meta=Scalar::
...
self.tcx.global_alloc(raw.alloc_id);3;let ptr=self.global_base_pointer(Pointer::
from(raw.alloc_id))?;3;let layout=self.layout_of(raw.ty)?;Ok(self.ptr_to_mplace(
...
mplace.meta().unwrap_meta().to_pointer(self)?;{;};let(ty,_)=self.get_ptr_vtable(
...
"`unpack_dyn_star` only makes sense on `dyn*` types");loop{break};let data=self.
project_field(val,0)?;{;};let vtable=self.project_field(val,1)?;let vtable=self.
read_pointer(&vtable.to_op(self)?)?;3;let(ty,_)=self.get_ptr_vtable(vtable)?;let
layout=self.layout_of(ty)?;{();};let data=data.transmute(layout,self)?;Ok((data,
