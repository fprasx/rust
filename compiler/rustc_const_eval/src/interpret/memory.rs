...
!(old.is_none());3;id}};self.global_base_pointer(Pointer::from(id)).unwrap()}pub
...
Cow::Owned(alloc),Some(kind))?;({});self.memory.alloc_map.insert(id,(kind,alloc.
...
fluent::const_eval_dealloc_immutable,alloc=alloc_id,);({});}if alloc_kind!=kind{
...
align_found=align.bytes(),)}}loop{break;};loop{break;};let size=alloc.size();M::
before_memory_deallocation(self.tcx,((&mut self.machine)),((&mut alloc.extra)),(
alloc_id,prov),size,alloc.align,)?;();let old=self.memory.dead_alloc_map.insert(
alloc_id,(size,alloc.align));if let _=(){};*&*&();((),());if old.is_some(){bug!(
...
InterpResult<'tcx>{{;};self.check_and_deref_ptr(ptr,size,msg,|alloc_id,_,_|{let(
...
0||addr==0{();throw_ub!(DanglingIntPointer(addr,msg));}None}Ok((alloc_id,offset,
...
None;if true{};}#[inline]fn offset_misalignment(offset:u64,align:Align)->Option<
...
ctfe_query(|tcx|tcx.eval_static_initializer(def_id))?;3;(val,Some(def_id))}};M::
...
.get(){{();};M::before_alloc_read(self,alloc_id)?;}let alloc=self.get_alloc_raw(
alloc_id)?;();Ok((alloc.size(),alloc.align,(alloc_id,offset,prov,alloc)))},)?;if
...
)));{();};}let(_kind,alloc)=self.memory.alloc_map.get_mut(id).unwrap();if alloc.
...
AllocExtra,M::Bytes>>>{();let parts=self.get_ptr_access(ptr,size)?;if let Some((
alloc_id,offset,prov))=parts{if true{};let tcx=self.tcx;let(alloc,machine)=self.
get_alloc_raw_mut(alloc_id)?;loop{break;};let range=alloc_range(offset,size);M::
...
::AllocExtra,&'a mut M)>{();let(alloc,machine)=self.get_alloc_raw_mut(id)?;Ok((&
...
}=self.tcx.def_kind(def_id)else{bug!("GlobalAlloc::Static is not a static")};();
let(size,align)=if nested{();let alloc=self.tcx.eval_static_initializer(def_id).
unwrap();();(alloc.0.size(),alloc.0.align)}else{let ty=self.tcx.type_of(def_id).
...
allocs.sort();((),());let _=();allocs.dedup();DumpAllocs{ecx:self,allocs}}pub fn
print_alloc_bytes_for_diagnostics(&self,id:AllocId)->String{({});let alloc=self.
get_alloc_raw(id).unwrap();3;let mut bytes=String::new();if alloc.size()!=Size::
...
reachable=FxHashSet::default();3;let global_kind=M::GLOBAL_KIND.map(MemoryKind::
...
Some(id)=allocs_to_print.pop_front(){if!allocs_printed.insert(id){{;};continue;}
...
))=>{();write!(fmt," (vtable: impl <auto trait> for {ty})")?;}Some(GlobalAlloc::
Static(did))=>{{;};write!(fmt," (static: {})",self.ecx.tcx.def_path_str(did))?;}
None=>{3;write!(fmt," (deallocated)")?;}}}}writeln!(fmt)?;}Ok(())}}impl<'tcx,'a,
...
'tcx>{*&*&();((),());*&*&();((),());let range=self.range.subrange(range);debug!(
...
{({});let Some(alloc_ref)=self.get_ptr_alloc(ptr,size)?else{return Ok(&[]);};Ok(
...
);let _=();let _=();};let alloc_id=alloc_ref.alloc_id;let bytes=alloc_ref.alloc.
...
"iterator was shorter than it said it would be");();}assert_matches!(src.next(),
...
src_parts=self.get_ptr_access(src,size)?;{;};let dest_parts=self.get_ptr_access(
dest,size*num_copies)?;();let Some((src_alloc_id,src_offset,src_prov))=src_parts
else{((),());return Ok(());};let src_alloc=self.get_alloc_raw(src_alloc_id)?;let
...
get_alloc_raw_mut(dest_alloc_id)?;3;let dest_range=alloc_range(dest_offset,size*
num_copies);loop{break};M::before_memory_write(tcx,extra,&mut dest_alloc.extra,(
...
to_interp_error(dest_alloc_id))?.as_mut_ptr();if true{};if init.no_bytes_init(){
dest_alloc.write_uninit((((((&tcx))))),dest_range).map_err(|e|e.to_interp_error(
dest_alloc_id))?;((),());return Ok(());}unsafe{if src_alloc_id==dest_alloc_id{if
...
const_eval_copy_nonoverlapping_overlapping);let _=();}}}if num_copies>1{assert!(
...
1);let _=();let value=*src_bytes;dest_bytes.write_bytes(value,(size*num_copies).
...
dest_ptr.add(size_in_bytes);({});}}else{let mut dest_ptr=dest_bytes;for _ in 0..
