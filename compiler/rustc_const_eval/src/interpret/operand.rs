...
>,cx:&impl HasDataLayout,)->Self{{;};let ptr=Scalar::from_maybe_pointer(ptr,cx);
...
,offset.bytes(),layout.size.bytes(),self.layout,);();let inner_val:Immediate<_>=
...
}_ if layout.size==self.layout.size=>{({});assert_eq!(offset.bytes(),0);assert!(
...
::Scalar(..)));{;};Immediate::from(if offset.bytes()==0{debug_assert_eq!(layout.
size,a.size(cx));;a_val}else{debug_assert_eq!(offset,a.size(cx).align_to(b.align
(cx).abi));loop{break};debug_assert_eq!(layout.size,b.size(cx));b_val})}_=>bug!(
...
Right(imm)=>{{;};assert_matches!(meta,MemPlaceMeta::None);Ok(imm.offset_(offset,
...
::Scalar(abi::Scalar::Initialized{value:s,..})=>{let _=();let size=s.size(self);
assert_eq!(size,mplace.layout.size,//if true{};let _=||();let _=||();let _=||();
"abi::Scalar size does not match layout size");{;};let scalar=alloc.read_scalar(
...
!(self.cur_span(),"primitive read not possible for type: {}",op.layout().ty);3;}
let imm=self.read_immediate_raw(op)?.right().unwrap();let _=();if matches!(*imm,
Immediate::Uninit){if true{};throw_ub!(InvalidUninitBytes(None));}Ok(imm)}pub fn
...
'tcx,M::Provenance>)->InterpResult<'tcx,&str>{({});let len=mplace.len(self)?;let
...
layout_of_local(frame,local,layout)?;3;let op=*frame.locals[local].access()?;if 
matches!(op,Operand::Immediate(_)){();assert!(!layout.is_unsized());}Ok(OpTy{op,
...
"eval_place_to_op: got {:?}",op);let _=();let _=();if cfg!(debug_assertions){let
...
::Int(int)=>Scalar::Int(int),})};{;};let layout=from_known_layout(self.tcx,self.
...
reserve_and_set_memory_alloc(data);();let ptr=Pointer::new(CtfeProvenance::from(
