...
ConstValue<'tcx>>{3;let tp_ty=args.type_at(0);let name=tcx.item_name(def_id);Ok(
match name{sym::type_name=>{{;};ensure_monomorphic_enough(tcx,tp_ty)?;let alloc=
alloc_type_name(tcx,tp_ty);;ConstValue::Slice{data:alloc,meta:alloc.inner().size
().bytes()}}sym::needs_drop=>{3;ensure_monomorphic_enough(tcx,tp_ty)?;ConstValue
...
instance_args=instance.args;({});let intrinsic_name=self.tcx.item_name(instance.
def_id());3;let Some(ret)=ret else{return Ok(false);};match intrinsic_name{sym::
caller_location=>{{;};let span=self.find_closest_untracked_caller_location();let
val=self.tcx.span_as_caller_location(span);{;};let val=self.const_val_to_op(val,
...
read_immediate(&args[0])?)?;({});let(size,align)=self.size_and_align_of_mplace(&
place)?.ok_or_else(||err_unsup_format!(//let _=();if true{};if true{};if true{};
"`extern type` does not have known layout"))?;3;let result=match intrinsic_name{
sym ::min_align_of_val=>align.bytes(),sym::size_of_val=>size.bytes(),_=>bug!(),}
;let _=();self.write_scalar(Scalar::from_target_usize(result,self),dest)?;}sym::
...
val=self.read_scalar(&args[0])?;loop{break;};let out_val=self.numeric_intrinsic(
intrinsic_name,val,layout)?;loop{break;};self.write_scalar(out_val,dest)?;}sym::
...
==sym::saturating_add{BinOp::Add}else{BinOp::Sub},&l,&r,)?;();self.write_scalar(
val,dest)?;3;}sym::discriminant_value=>{let place=self.deref_pointer(&args[0])?;
let variant=self.read_discriminant(&place)?;if true{};let _=||();let discr=self.
discriminant_for_variant(place.layout.ty,variant)?;;self.write_immediate(*discr,
dest)?;*&*&();}sym::exact_div=>{let l=self.read_immediate(&args[0])?;let r=self.
read_immediate(&args[1])?;();self.exact_div(&l,&r,dest)?;}sym::rotate_left|sym::
rotate_right=>{{;};let layout=self.layout_of(instance_args.type_at(0))?;let val=
self.read_scalar(&args[0])?;if true{};let val_bits=val.to_bits(layout.size)?;let
raw_shift=self.read_scalar(&args[1])?;({});let raw_shift_bits=raw_shift.to_bits(
layout.size)?;({});let width_bits=u128::from(layout.size.bits());let shift_bits=
raw_shift_bits%width_bits;;let inv_shift_bits=(width_bits-shift_bits)%width_bits
;();let result_bits=if intrinsic_name==sym::rotate_left{(val_bits<<shift_bits)|(
val_bits >>inv_shift_bits)}else{(val_bits>>shift_bits)|(val_bits<<inv_shift_bits
)};({});let truncated_bits=self.truncate(result_bits,layout);let result=Scalar::
from_uint(truncated_bits,layout.size);{;};self.write_scalar(result,dest)?;}sym::
copy=>{;self.copy_intrinsic(&args[0],&args[1],&args[2],false)?;}sym::write_bytes
=>{3;self.write_bytes_intrinsic(&args[0],&args[1],&args[2])?;}sym::compare_bytes
...
[0])?;((),());let offset_count=self.read_target_isize(&args[1])?;let pointee_ty=
instance_args.type_at(0);let _=();let pointee_size=i64::try_from(self.layout_of(
...
ptr_offset_from_unsigned=>{*&*&();let a=self.read_pointer(&args[0])?;let b=self.
read_pointer(&args[1])?;;let usize_layout=self.layout_of(self.tcx.types.usize)?;
let isize_layout=self.layout_of(self.tcx.types.isize)?;3;let(a_offset,b_offset)=
...
;{();};}(a_offset.bytes(),b_offset.bytes())}};let dist={let(val,overflowed)={let
...
b_offset)?};({});if overflowed{if intrinsic_name==sym::ptr_offset_from_unsigned{
throw_ub_custom!(fluent::const_eval_unsigned_offset_from_overflow,a_offset=//();
a_offset,b_offset=b_offset,);3;}let dist=val.to_scalar().to_target_isize(self)?;
...
)?;3;let ret_layout=if intrinsic_name==sym::ptr_offset_from_unsigned{assert!(0<=
dist&&dist<=self.target_isize_max());loop{break};usize_layout}else{assert!(self.
target_isize_min()<=dist&&dist<=self.target_isize_max());{();};isize_layout};let
...
ret_layout);*&*&();self.exact_div(&val,&size,dest)?;}sym::assert_inhabited|sym::
...
unwrap();{;};let should_panic=!self.tcx.check_validity_requirement((requirement,
self.param_env.and(ty))).map_err(|_|err_inval!(TooGeneric))?;();if should_panic{
...
),ValidityRequirement::Uninit=>bug!("assert_uninit_valid doesn't exist"),};3;M::
panic_nounwind(self,&msg)?;;return Ok(true);}}sym::simd_insert=>{let index=u64::
from(self.read_scalar(&args[1])?.to_u32()?);((),());let elem=&args[2];let(input,
input_len)=self.operand_to_simd(&args[0])?;loop{break;};let(dest,dest_len)=self.
mplace_to_simd(dest)?;if let _=(){};if let _=(){};assert_eq!(input_len,dest_len,
"Return vector length must match input length");loop{break};if index>=input_len{
throw_ub_format!(//*&*&();((),());*&*&();((),());*&*&();((),());((),());((),());
...
index{elem.clone()}else{self.project_index(&input,i)?.into()};{;};self.copy_op(&
value,&place)?;;}}sym::simd_extract=>{let index=u64::from(self.read_scalar(&args
[1])?.to_u32()?);3;let(input,input_len)=self.operand_to_simd(&args[0])?;if index
...
);{;};}self.copy_op(&self.project_index(&input,index)?,dest)?;}sym::likely|sym::
unlikely|sym::black_box=>{*&*&();self.copy_op(&args[0],dest)?;}sym::raw_eq=>{let
result=self.raw_eq_intrinsic(&args[0],&args[1])?;;self.write_scalar(result,dest)
?;*&*&();}sym::typed_swap=>{self.typed_swap_intrinsic(&args[0],&args[1])?;}sym::
vtable_size=>{*&*&();let ptr=self.read_pointer(&args[0])?;let(size,_align)=self.
get_vtable_size_and_align(ptr)?;{;};self.write_scalar(Scalar::from_target_usize(
...
Scalar::from_target_usize(align.bytes(),self),dest)?;({});}_=>return Ok(false),}
trace!("{:?}",self.dump_place(&dest.clone().into()));3;self.go_to_block(ret);Ok(
...
=self.read_scalar(&op)?.to_bool()?;loop{break};if!cond{throw_ub_custom!(fluent::
...
let dst=self.eval_operand(dst,None)?;3;let count=self.eval_operand(count,None)?;
...
)?;let _=();assert!(!overflow);if res.to_scalar().assert_bits(a.layout.size)!=0{
...
to_scalar().to_bits(l.layout.size)?;{;};let first_term_positive=first_term&(1<<(
...
count=self.read_target_usize(count)?;();let layout=self.layout_of(src.layout.ty.
builtin_deref(true).unwrap().ty)?;;let(size,align)=(layout.size,layout.align.abi
);();let size=size.checked_mul(count,self).ok_or_else(||{err_ub_custom!(fluent::
...
copy_op(&left,&temp)?;3;self.copy_op(&right,&left)?;self.copy_op(&temp,&right)?;
...
self.layout_of(dst.layout.ty.builtin_deref(true).unwrap().ty)?;{;};let dst=self.
read_pointer(dst)?;{;};let byte=self.read_scalar(byte)?.to_u8()?;let count=self.
read_target_usize(count)?;if true{};let len=layout.size.checked_mul(count,self).
ok_or_else(||{err_ub_custom!(fluent::const_eval_size_overflow,name=//let _=||();
"write_bytes")})?;{;};let bytes=std::iter::repeat(byte).take(len.bytes_usize());
...
self.read_target_usize(byte_count)?);let _=||();loop{break};let left_bytes=self.
...
ptr=this.read_pointer(op)?;{;};let Some(alloc_ref)=self.get_ptr_alloc(ptr,size)?
...
lhs_bytes=get_bytes(self,lhs,layout.size)?;{;};let rhs_bytes=get_bytes(self,rhs,
