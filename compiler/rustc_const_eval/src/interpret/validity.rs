...
write_path(out:&mut String,path:&[PathElem]){3;use self::PathElem::*;for elem in
...
)=captures.get(field){({});let var_hir_id=captured_place.get_root_variable();let
node=self.ecx.tcx.hir_node(var_hir_id);();if let hir::Node::Pat(pat)=node{if let
hir::PatKind::Binding(_,_,ident,_)=pat.kind{3;name=Some(ident.name);}}}}PathElem
...
'tcx,R>,)->InterpResult<'tcx,R>{{;};let path_len=self.path.len();self.path.push(
...
!("{vtable}")});let _=||();}ty::Slice(..)|ty::Str=>{let _len=meta.unwrap_meta().
...
?;({});if place.layout.is_unsized(){self.check_wide_ptr_meta(place.meta(),place.
...
||(place.layout.size,place.layout.align.abi));let _=();try_validation!(self.ecx.
...
ptr_kind},Ub(PointerUseAfterFree(..))=>DanglingPtrUseAfterFree{ptr_kind,},);{;};
...
is_uninhabited(){{;};let ty=place.layout.ty;throw_validation_failure!(self.path,
...
=false;3;let alloc_kind=self.ecx.tcx.try_get_global_alloc(alloc_id).unwrap();let
...
ConstRefToExtern);();}}None=>{}}let DefKind::Static{mutability,nested}=self.ecx.
...
self.path,ConstRefToMutable);({});}}}if skip_recursive_check{return Ok(());}}let
...
!("{value:x}"),});let _=();Ok(true)}ty::Char=>{let value=self.read_scalar(value,
ExpectedKind::Char)?;3;try_validation!(value.to_char(),self.path,Ub(InvalidChar(
...
place.meta(),place.layout)?;if let _=(){};}Ok(true)}ty::Ref(_,_ty,mutbl)=>{self.
...
ref_tracking{3;let ptr=value.to_pointer(self.ecx)?;let _fn=try_validation!(self.
...
);{();};assert!(end<=max_value);let bits=match scalar.try_to_int(){Ok(int)=>int.
...
NullablePtrOutOfRange{range:valid_range,max_value})}else{();return Ok(());}}else
...
span_bug!(self.ecx.tcx.span,"not a memory allocation"),};{;};return mutability==
...
InterpResult<'tcx>{({});trace!("visit_value: {:?}, {:?}",*op,op.layout);if self.
try_visit_primitive(op)?{{;};return Ok(());}if self.ctfe_mode.is_some_and(|c|!c.
...
ecx)?;{;};try_validation!(self.ecx.read_bytes_ptr_strip_provenance(mplace.ptr(),
...
ExpectedKind::Int}else{ExpectedKind::Float};*&*&();let len=op.len(self.ecx)?;let
layout=self.ecx.layout_of(*tys)?;3;let size=layout.size*len;if size==Size::ZERO{
return Ok(());({});}let mplace=match op.as_mplace_or_imm(){Left(mplace)=>mplace,
...
Uninhabited=>{if true{};let ty=op.layout.ty;throw_validation_failure!(self.path,
UninhabitedVal{ty});loop{break;};}Abi::Scalar(scalar_layout)=>{if!scalar_layout.
is_uninit_valid(){{;};let scalar=self.read_scalar(op,ExpectedKind::InitScalar)?;
...
(a,a_layout)?;let _=();self.visit_scalar(b,b_layout)?;}}Abi::Vector{..}=>{}Abi::
...
.ty);({});let mut visitor=ValidityVisitor{path,ref_tracking,ctfe_mode,ecx:self};
