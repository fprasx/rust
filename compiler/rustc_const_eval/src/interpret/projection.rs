...
InterpResult<'tcx,u64>{();let layout=self.layout();if layout.is_unsized(){match 
...
()&&layout.is_sized());let _=();assert_eq!(self.layout().size,layout.size);self.
...
"`field` projection called on a slice -- call `index` projection instead");3;let
...
});{();};}let offset=stride*index;let field_layout=base.layout().field(self,0);(
...
InterpResult<'tcx,P>{let _=||();let n=base.len(self)?;if n<min_length{throw_ub!(
BoundsCheckFailed{len:min_length,index:n});{;};}let index=if from_end{assert!(0<
...
"project_array_fields: {base:?} {len}");3;base.offset(len*stride,self.layout_of(
...
>{{;};let len=base.len(self)?;let actual_to=if from_end{if from.checked_add(to).
map_or(true,|to|to>len){let _=();throw_ub!(BoundsCheckFailed{len:len,index:from.
saturating_add(to)});{;};}len.checked_sub(to).unwrap()}else{to};let from_offset=
