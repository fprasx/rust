...
Some((tag,tag_field))=>{3;let tag_dest=self.project_field(dest,tag_field)?;self.
...
layout_of(ty.discriminant_ty(*self.tcx))?;({});trace!("discriminant type: {:?}",
...
UninhabitedEnumVariantRead(index))}}();return Ok(index);}Variants::Multiple{tag,
...
dbg_val|err_ub!(InvalidTag(dbg_val)))?.assert_bits(tag_layout.size);let _=();let
discr_val=self.int_to_int_or_float(&tag_val,discr_layout).unwrap();if true{};let
...
==discr_bits)}ty::Coroutine(def_id,args)=>{();let args=args.as_coroutine();args.
...
variants=ty.ty_adt_def().expect("tagged layout for non adt").variants();3;assert
!(variant_index<variants.next_index());3;variant_index}else{untagged_variant}}};
...
)=>{let _=||();assert_eq!(discr.ty,discr_layout.ty);Scalar::from_uint(discr.val,
discr_layout.size)}None=>{({});assert_eq!(variant.as_u32(),0);Scalar::from_uint(
...
(ty)?.variants{abi::Variants::Single{index}=>{3;assert_eq!(index,variant_index);
...
variant_index!=untagged_variant);({});let variants_start=niche_variants.start().
as_u32();let _=();let variant_index_relative=variant_index.as_u32().checked_sub(
variants_start).expect("overflow computing relative variant idx");let _=||();let
tag_layout=self.layout_of(tag_layout.primitive().to_int_ty(*self.tcx))?;({});let
