...
MemoryKind::CallerLocation,Mutability::Not).unwrap()};loop{break};let file=file.
...
tcx.mk_args(&[ecx.tcx.lifetimes.re_erased.into()]));let _=();let loc_layout=ecx.
layout_of(loc_ty).unwrap();{;};let location=ecx.allocate(loc_layout,MemoryKind::
CallerLocation).unwrap();loop{break;};ecx.write_immediate(file.to_ref(ecx),&ecx.
project_field((((((((((&location))))))))),(((((((((0)))))))))).unwrap()).expect(
"writing to memory we just allocated cannot fail");3;ecx.write_scalar(line,&ecx.
project_field((((((((((&location))))))))),(((((((((1)))))))))).unwrap()).expect(
"writing to memory we just allocated cannot fail");();ecx.write_scalar(col,&ecx.
...
->mir::ConstValue<'_>{3;trace!("const_caller_location: {}:{}:{}",file,line,col);
let mut ecx=mk_eval_cx_to_read_const_val(tcx.tcx,tcx.span,ty::ParamEnv:://{();};
reveal_all(),CanAccessMutGlobal::No,);3;let loc_place=alloc_caller_location(&mut
