...
CompileTimeInterpreter::new(CanAccessMutGlobal::No,CheckAlignment::Error);();let
mut cx=InterpCx::new(tcx,rustc_span::DUMMY_SP,ParamEnv::reveal_all(),machine);3;
...
=>{bug!("ValidityRequirement::Uninit should have been handled above")}}};{;};let
...
pointee.ty)?;3;if pointee.align.abi.bytes()>1{return Ok(false);}if pointee.size.
bytes()>0{if true{};return Ok(false);}}match&this.fields{FieldsShape::Primitive|
...
field(cx,idx),cx,init_kind)?{3;return Ok(false);}}}}match&this.variants{Variants
