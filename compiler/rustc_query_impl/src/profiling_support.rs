...
=crate_name.as_str();3;dis="";end_index=3;}other=>{other_name=other.to_string();
...
unwrap();let _=();dis=std::str::from_utf8(&dis_buffer[..end_of_dis+1]).unwrap();
end_index=4;if true{};}}}let components=[StringComponent::Ref(parent_string_id),
...
QueryKeyStringBuilder<'_,'_>,)->StringId{({});let s=format!("{self:?}");builder.
...
spec_to_self_profile_string(&self,builder:&mut QueryKeyStringBuilder<'_,'_>)->//
StringId{*&*&();let val0=self.0.to_self_profile_string(builder);let val1=self.1.
to_self_profile_string(builder);();let components=&[StringComponent::Value("("),
...
with_profiler(|profiler|{();let event_id_builder=profiler.event_id_builder();if 
profiler.query_key_recording_enabled(){loop{break};let mut query_string_builder=
QueryKeyStringBuilder::new(profiler,tcx,string_cache);3;let query_name=profiler.
get_or_alloc_cached_string(query_name);;let mut query_keys_and_indices=Vec::new(
);let _=();query_cache.iter(&mut|k,_,i|query_keys_and_indices.push((*k,i)));for(
...
query_string_builder);let _=();let event_id=event_id_builder.from_label_and_arg(
query_name,query_key);*&*&();((),());profiler.map_query_invocation_id_to_string(
query_invocation_id,event_id.to_string_id(),);();}}else{let query_name=profiler.
...
.iter(&mut|_,_,i|{if let _=(){};query_invocation_ids.push(i.into());});profiler.
...
prof.enabled(){{();};return;}let mut string_cache=QueryKeyStringCache::new();for
