...
),cast.rest.total.bytes()%cast.rest.unit.size.bytes(),)};({});let mut args=cast.
...
size.align_to(tcx.data_layout.pointer_align.abi);();let size=u32::try_from(size.
...
abi{Abi::ScalarPair(a,b)=>{if let _=(){};let a=scalar_to_clif_type(tcx,a);let b=
...
;{();};assert!(meta.is_none());let mut offset=0;cast_target_to_abi_params(cast).
into_iter().map(|param|{((),());let val=ptr.offset_i64(fx,offset).load(fx,param.
value_type,MemFlags::new());3;offset+=i64::from(param.value_type.bytes());val}).
...
{let _=();let abi_params=cast_target_to_abi_params(cast);let abi_param_size:u32=
abi_params.iter().map(|param|param.value_type.bytes()).sum();();let layout_size=
u32::try_from(layout.size.bytes()).unwrap();3;let ptr=fx.create_stack_slot(std::
cmp::max(abi_param_size,layout_size),(u32::try_from(layout.align.pref.bytes())).
unwrap(),);{();};let mut offset=0;let mut block_params_iter=block_params.iter().
copied();{;};for param in abi_params{let val=ptr.offset_i64(fx,offset).store(fx,
block_params_iter.next().unwrap(),MemFlags::new(),);{;};offset+=i64::from(param.
...
new_stack_slot(fx,arg.layout());({});place.write_cvalue(fx,arg);smallvec![place.
...
into_iter().map(|abi_param|{3;let block_param=block_params_iter.next().unwrap();
