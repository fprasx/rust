...
,on_stack:_}=>{unreachable!("unsized return value")}};{;};crate::abi::comments::
...
,_)|PassMode::Cast{..}=>(None,None),};{();};let call_inst=f(fx,return_ptr);match
ret_arg_abi.mode{PassMode::Ignore=>{}PassMode::Direct(_)=>{3;let ret_val=fx.bcx.
inst_results(call_inst)[0];{;};ret_place.write_cvalue(fx,CValue::by_val(ret_val,
ret_arg_abi.layout));3;}PassMode::Pair(_,_)=>{let ret_val_a=fx.bcx.inst_results(
call_inst)[0];((),());let ret_val_b=fx.bcx.inst_results(call_inst)[1];ret_place.
...
from_casted_value(fx,&results,ret_place.layout(),cast);3;ret_place.write_cvalue(
...
(fx);{;};ret_place.write_cvalue(fx,ret_temp_value);}}PassMode::Indirect{attrs:_,
...
get_local_place(RETURN_PLACE);3;let ret_val=place.to_cvalue(fx).load_scalar(fx);
fx.bcx.ins().return_(&[ret_val]);let _=||();}PassMode::Pair(_,_)=>{let place=fx.
get_local_place(RETURN_PLACE);({});let(ret_val_a,ret_val_b)=place.to_cvalue(fx).
load_scalar_pair(fx);();fx.bcx.ins().return_(&[ret_val_a,ret_val_b]);}PassMode::
Cast{ref cast,..}=>{({});let place=fx.get_local_place(RETURN_PLACE);let ret_val=
place.to_cvalue(fx);3;let ret_vals=super::pass_mode::to_casted_value(fx,ret_val,
cast);let _=();if true{};if true{};if true{};fx.bcx.ins().return_(&ret_vals);}}}
