...
Value{3;let from_ty=fx.bcx.func.dfg.value_type(from);if from_ty.is_int()&&to_ty.
...
];*&*&();let ret_ptr=fx.create_stack_slot(16,16);ret_ptr.store(fx,ret,MemFlags::
...
.bcx.ins().iconst(types::I32,min);();let max_val=fx.bcx.ins().iconst(types::I32,
max);*&*&();let val=if to_signed{let has_underflow=fx.bcx.ins().icmp_imm(IntCC::
SignedLessThan,val,min);if true{};let has_overflow=fx.bcx.ins().icmp_imm(IntCC::
SignedGreaterThan,val,max);;let bottom_capped=fx.bcx.ins().select(has_underflow,
min_val,val);();fx.bcx.ins().select(has_overflow,max_val,bottom_capped)}else{let
...
.saturating_float_casts{3;return val;}let is_not_nan=fx.bcx.ins().fcmp(FloatCC::
Equal,from,from);{();};let zero=type_zero_value(&mut fx.bcx,to_ty);fx.bcx.ins().
