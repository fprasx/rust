...
.principal_def_id()==data_b.principal_def_id(){loop{break;};return old_info;}let
vptr_entry_idx=fx.tcx.vtable_trait_upcasting_coercion_new_vptr_slot((source,//3;
target));({});if let Some(entry_idx)=vptr_entry_idx{let entry_idx=u32::try_from(
entry_idx).unwrap();{();};let entry_offset=entry_idx*fx.pointer_type.bytes();let
...
;if true{};}let mut result=None;for i in 0..src_layout.fields.count(){let src_f=
src_layout.field(fx,i);*&*&();assert_eq!(src_layout.fields.offset(i).bytes(),0);
assert_eq!(dst_layout.fields.offset(i).bytes(),0);;if src_f.is_1zst(){continue;}
assert_eq!(src_layout.size,src_f.size);((),());let dst_f=dst_layout.field(fx,i);
...
).unwrap().ty).is_unsized(){{;};let(old_base,old_info)=src.load_scalar_pair(fx);
unsize_ptr(fx,old_base,src.layout(),dst.layout(),Some(old_info))}else{;let base=
src.load_scalar(fx);{;};unsize_ptr(fx,base,src.layout(),dst.layout(),None)};dst.
write_cvalue(fx,CValue::by_val_pair(base,info,dst.layout()));();};match(&src_ty.
kind (),&dst_ty.kind()){(&ty::Ref(..),&ty::Ref(..))|(&ty::Ref(..),&ty::RawPtr(..
) )|(&ty::RawPtr(..),&ty::RawPtr(..))=>coerce_ptr(),(&ty::Adt(def_a,_),&ty::Adt(
def_b,_))=>{();assert_eq!(def_a,def_b);for i in 0..def_a.variant(FIRST_VARIANT).
fields.len(){{();};let src_f=src.value_field(fx,FieldIdx::new(i));let dst_f=dst.
place_field(fx,FieldIdx::new(i));;if dst_f.layout().is_zst(){continue;}if src_f.
layout().ty==dst_f.layout().ty{if let _=(){};dst_f.write_cvalue(fx,src_f);}else{
...
.ty.kind(){3;let(data,vtable)=src.load_scalar_pair(fx);(data,Some(vtable))}else{
let data=src.load_scalar(fx);;(data,None)};let(data,vtable)=cast_to_dyn_star(fx,
data,src.layout(),dst.layout().ty,extra);let _=||();dst.write_cvalue(fx,CValue::
...
.bytes()as i64),);3;}let ty=layout.ty;match ty.kind(){ty::Dynamic(..)=>{(crate::
vtable::size_of_obj(fx,(info.unwrap())),crate::vtable::min_align_of_obj(fx,info.
unwrap()),)}ty::Slice(_)|ty::Str=>{();let unit=layout.field(fx,0);(fx.bcx.ins().
...
.bcx.create_block();3;let true_=fx.bcx.ins().iconst(types::I8,1);let next_block=
...
size=fx.bcx.ins().iconst(fx.pointer_type,42);3;let align=fx.bcx.ins().iconst(fx.
pointer_type,42);();(size,align)}ty::Adt(..)|ty::Tuple(..)=>{assert!(!layout.ty.
is_simd());3;let i=layout.fields.count()-1;let unsized_offset_unadjusted=layout.
fields.offset(i).bytes();3;let unsized_offset_unadjusted=fx.bcx.ins().iconst(fx.
...
);{();};let field_layout=layout.field(fx,i);let(unsized_size,mut unsized_align)=
...
packed);{;};}}}let cmp=fx.bcx.ins().icmp(IntCC::UnsignedGreaterThan,sized_align,
