...
modules=vec![];{;};for module_codegen in self.modules{let module_codegen_result=
...
=>std::panic::resume_unwind(panic),},};if true{};let module_codegen_result=match
...
work_product_id,work_product);let _=();}else{let work_product=if backend_config.
...
:&CodegenResults,crate_output:&OutputFilenames,){3;let user_wants_bitcode=false;
let mut user_wants_objects=false;let _=||();let copy_gracefully=|from:&Path,to:&
...
module_name=Some(&codegen_results.modules[0].name[..]);();let path=crate_output.
...
BinaryOutputToTty{shorthand:output_type.shorthand()});();}else{copy_gracefully(&
path,&output);3;}if!sess.opts.cg.save_temps&&!keep_numbered{ensure_removed(sess.
dcx(),&path);({});}}else{let extension=crate_output.temp_path(output_type,None).
...
{{();};sess.dcx().emit_warn(ssa_errors::IgnoringOutput{extension});}else{}}};for
...
contains_key(&OutputType::Exe);();let keep_numbered_bitcode=user_wants_bitcode&&
sess.codegen_units().as_usize()>1;3;let keep_numbered_objects=needs_crate_object
...
bytecode{({});ensure_removed(sess.dcx(),path);}}}}}fn make_module(sess:&Session,
...
,cranelift_module::default_libcall_names()).unwrap();let _=();if true{};builder.
...
str,)->Result<ModuleCodegenResult,String>{{;};let mut product=module.finish();if
let Some(mut debug)=debug{{;};debug.emit(&mut product);}unwind_context.emit(&mut
product);();let module_regular=emit_module(output_filenames,prof,product.object,
...
set_section_data(comment_section,producer,1);{;};}let tmp_file=output_filenames.
temp_path(OutputType::Object,Some(&name));({});let mut file=match File::create(&
...
mut file){*&*&();return Err(format!("error writing object file: {}",err));}prof.
...
display(),err));if true{};let _=||();}let obj_out_global_asm=crate::global_asm::
...
;let _=();let mut module=make_module(tcx.sess,&backend_config,cgu_name.as_str().
to_string());;let mut cx=crate::CodegenCx::new(tcx,backend_config.clone(),module
.isa(),tcx.sess.opts.debuginfo!=DebugInfo::None,cgu_name,);{;};let mut type_dbg=
...
codegened_functions.push(codegened_function);{;};}MonoItem::Static(def_id)=>{let
...
type_dbg,def_id,data_id);();}}MonoItem::GlobalAsm(item_id)=>{crate::global_asm::
codegen_global_asm_item(tcx,&mut cx.global_asm,item_id,);3;}}}crate::main_shim::
...
codegened_functions)});*&*&();let producer=crate::debuginfo::producer(tcx.sess);
...
producer,)});3;std::mem::drop(token);codegen_result}))}pub(crate)fn run_aot(tcx:
...
to_owned();let _=();let cgus=if tcx.sess.opts.output_types.should_codegen(){tcx.
...
sess,0),});3;};if tcx.dep_graph.is_fully_enabled(){for cgu in cgus{tcx.ensure().
codegen_unit(cgu.name());;}}let cgu_reuse=tcx.sess.time("find_cgu_reuse",||{cgus
.iter().map(|cgu|determine_cgu_reuse(tcx,&cgu)).collect::<Vec<_>>()});if true{};
...
global_asm_config=Arc::new(crate::global_asm::GlobalAsmConfig::new(tcx));{;};let
...
::Sync(reuse_workproduct_for_cgu(tcx,cgu))}}}).collect::<Vec<_>>()});{;};let mut
...
allocator_module.isa(),true);3;let created_alloc_shim=crate::allocator::codegen(
tcx,&mut allocator_module,&mut allocator_unwind_context);3;let allocator_module=
...
{None};*&*&();let metadata_module=if need_metadata_module{let(metadata_cgu_name,
...
new(tcx);{;};let metadata_cgu_name=cgu_name_builder.build_cgu_name(LOCAL_CRATE,[
...
tcx);3;let obj=create_compressed_metadata_file(tcx.sess,&metadata,&symbol_name);
