...
Debug},);();self.object.section_mut(section_id).set_data(data,if id==SectionId::
EhFrame{8}else{1});*&*&();let symbol_id=self.object.section_symbol(section_id);(
...
&id).unwrap().1,0),DebugRelocName::Symbol(id)=>{3;let id=id.try_into().unwrap();
...
symbol_section_and_offset(symbol_id).unwrap_or((symbol_id,0))}};{;};self.object.
