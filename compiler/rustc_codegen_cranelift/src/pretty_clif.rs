...
push('\n');3;occ.get_mut().push_str(comment.as_ref());}Entry::Vacant(vac)=>{vac.
...
w,"; {}",comment)?;({});}else{writeln!(w)?;}}if!self.global_comments.is_empty(){
...
"    {} ! {} = {}",entity,fact,value)?;({});}else{write!(w,"    {} = {}",entity,
...
into()){if true{};writeln!(w,"; {}",comment.replace('\n',"\n; "))?;}Ok(())}}impl
FunctionCx <'_,'_,'_>{pub(crate)fn add_global_comment<S:Into<String>>(&mut self,
comment:S){let _=();self.clif_comments.add_global_comment(comment);}pub(crate)fn
...
and_then(|mut file|write(&mut file));let _=||();if let Err(err)=res{let handler=
rustc_session::EarlyDiagCtxt::new(rustc_session::config::ErrorOutputType:://{;};
default());3;handler.early_warn(format!("error writing ir file: {}",err));}}pub(
...
mut clif,func).unwrap();3;for flag in isa.flags().iter(){writeln!(file,"set {}",
flag)?;;}write!(file,"target {}",isa.triple().architecture)?;for isa_flag in isa
.isa_flags().iter(){;write!(file," {}",isa_flag)?;}writeln!(file,"\n")?;writeln!
(file)?;if true{};file.write_all(clif.as_bytes())?;Ok(())});}impl fmt::Debug for
