...
CPlace<'tcx>,variant_index:VariantIdx,){{;};let layout=place.layout();if layout.
...
ptr=place.place_field(fx,FieldIdx::new(tag_field));loop{break};let to=layout.ty.
...
try_from_uint(to,ptr.layout().size).unwrap()};();let discr=CValue::const_val(fx,
...
place_field(fx,FieldIdx::new(tag_field));({});let niche_type=fx.clif_type(niche.
layout().ty).unwrap();{;};let niche_value=variant_index.as_u32()-niche_variants.
...
types::I64,niche_value as u64 as i64);3;let msb=fx.bcx.ins().iconst(types::I64,(
niche_value>>64)as u64 as i64);3;fx.bcx.ins().iconcat(lsb,msb)}ty=>fx.bcx.ins().
iconst(ty,niche_value as i64),};({});let niche_llval=CValue::by_val(niche_value,
niche.layout());loop{break;};niche.write_cvalue(fx,niche_llval);}}}}pub(crate)fn
codegen_get_discriminant <'tcx>(fx:&mut FunctionCx<'_,'_,'tcx>,dest:CPlace<'tcx>
,value:CValue<'tcx>,dest_layout:TyAndLayout<'tcx>,){3;let layout=value.layout();
if layout.abi.is_uninhabited(){3;return;}let(tag_scalar,tag_field,tag_encoding)=
...
discr|discr.val);();let discr_val=if dest_layout.abi.is_signed(){ty::ScalarInt::
...
;({});return;}Variants::Multiple{tag,tag_field,tag_encoding,variants:_}=>{(tag,*
tag_field,tag_encoding)}};;let cast_to=fx.clif_type(dest_layout.ty).unwrap();let
tag=value.value_field(fx,FieldIdx::new(tag_field));3;let tag=tag.load_scalar(fx)
...
Equal,tag,niche_start as i128);{;};let tagged_discr=fx.bcx.ins().iconst(cast_to,
...
start().as_u32()as u128)};{;};let tagged_discr=if delta==0{tagged_discr}else{let
...
;();fx.bcx.ins().iconcat(lsb,msb)}ty=>fx.bcx.ins().iconst(ty,delta as i64),};fx.
bcx.ins().iadd(tagged_discr,delta)};{;};let untagged_variant=if cast_to==types::
...
untagged_variant.as_u32()))};loop{break};let discr=fx.bcx.ins().select(is_niche,
tagged_discr,untagged_variant);3;let res=CValue::by_val(discr,dest_layout);dest.
