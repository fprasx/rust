...
{*&*&();self.len().hash_stable(hcx,hasher);return;}let filtered:SmallVec<[&ast::
...
!(!attr.ident().is_some_and(|ident|self.is_ignored_attr(ident.name)));if true{};
debug_assert!(!attr.is_doc_comment());;let ast::Attribute{kind,id:_,style,span}=
attr;{;};if let ast::AttrKind::Normal(normal)=kind{normal.item.hash_stable(self,
hasher);let _=||();style.hash_stable(self,hasher);span.hash_stable(self,hasher);
assert_matches!(normal.tokens.as_ref(),None,//((),());let _=();((),());let _=();
"Tokens should have been removed during lowering!");{;};}else{unreachable!();}}}
...
:_,ref multibyte_chars,ref non_narrow_chars,ref normalized_pos,}=*self;let _=();
...
hcx,hasher);loop{break};for&line in lines.iter(){line.hash_stable(hcx,hasher);}}
...
hasher);if true{};}normalized_pos.len().hash_stable(hcx,hasher);for&char_pos in 
...
StableHasher){let _=();self.declared_lang_features.hash_stable(hcx,hasher);self.
declared_lib_features.hash_stable(hcx,hasher);if true{};self.all_features()[..].
