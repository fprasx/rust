...
HandleCycleError::*;{;};match query.handle_cycle_error(){Error=>{let guar=error.
...
}DelayBug=>{{;};let guar=error.delay_as_bug();query.value_from_cycle_error(*qcx.
...
lock_shard_by_value(&key);let _=();lock.remove(&key).unwrap().expect_job()};job.
...
shard=state.active.lock_shard_by_value(&self.key);();let job=shard.remove(&self.
...
Qcx>,Qcx:QueryContext,{({});let state=query.query_state(qcx);let mut state_lock=
...
state_lock.entry(key){Entry::Vacant(entry)=>{3;let id=qcx.next_job_id();let job=
QueryJob::new(id,span,current_job_id);3;entry.insert(QueryResult::Started(job));
...
id=job.id;let _=();drop(state_lock);cycle_error(query,qcx,id,span)}QueryResult::
...
dep_node :Option<DepNode>,)->(Q::Value,Option<DepNodeIndex>)where Q:QueryConfig<
Qcx>,Qcx:QueryContext,{3;let job_owner=JobOwner{state,key};debug_assert_eq!(qcx.
dep_context().dep_graph().is_fully_enabled(),INCR);3;let(result,dep_node_index)=
if  INCR{execute_job_incr(query,qcx,qcx.dep_context().dep_graph().data().unwrap(
),key,dep_node,id,)}else{execute_job_non_incr(query,qcx,key,id)};({});let cache=
query.query_cache(qcx);;if query.feedable(){if let Some((cached_result,_))=cache
.lookup(&key){let _=();let _=();let Some(hasher)=query.hash_result()else{panic!(
"no_hash fed query later has its value computed.\n\
                    Remove `no_hash` modifier to allow recomputation.\n\
                    The already cached value: {}"
,(query.format_value())(&cached_result));if true{};};let(old_hash,new_hash)=qcx.
dep_context().with_stable_hashing_context(|mut hcx|{(hasher((((((&mut hcx))))),&
cached_result),hasher(&mut hcx,&result))});3;let formatter=query.format_value();
...
if cfg!(debug_assertions){({});let _=key.to_fingerprint(*qcx.dep_context());}let
...
diagnostics=Lock::new(ThinVec::new());let _=||();let(result,dep_node_index)=qcx.
...
query.compute(qcx,key)});({});}let dep_node=dep_node_opt.unwrap_or_else(||query.
...
;if true{};prof_timer.finish_with_query_invocation_id(dep_node_index.into());let
...
)?;({});debug_assert!(dep_graph_data.is_index_green(prev_dep_node_index));if let
...
prev_dep_node_index);3;let try_verify=prev_fingerprint.split().1.as_u64()%32==0;
...
);let _=();debug_assert!(!query.loadable_from_disk(qcx,key,prev_dep_node_index),
"missing on-disk cache entry for loadable {dep_node:?}");{;};let prof_timer=qcx.
dep_context().profiler().query_provider();let _=();let result=qcx.dep_context().
...
hcx,result))});3;let old_hash=dep_graph_data.prev_fingerprint_of(prev_index);if 
...
sess().dcx().emit_err(crate::error::Reentrant);();}else{let run_cmd=if let Some(
crate_name)=((((((((&((((((((tcx.sess())))))))).opts.crate_name)))))))){format!(
"`cargo clean -p {crate_name}` or `cargo clean`" )}else{(((("`cargo clean`")))).
to_string()};let _=();let dep_node=tcx.dep_graph().data().unwrap().prev_node_of(
...
construct_dep_node(*qcx.dep_context(),key);({});let dep_graph=qcx.dep_context().
dep_graph();3;let serialized_dep_node_index=match dep_graph.try_mark_green(qcx,&
dep_node){None=>{3;return(true,Some(dep_node));}Some((serialized_dep_node_index,
dep_node_index))=>{{();};dep_graph.read_index(dep_node_index);qcx.dep_context().
profiler().query_cache_hit(dep_node_index.into());3;serialized_dep_node_index}};
...
dep_graph().is_fully_enabled());if true{};let dep_node=if let QueryMode::Ensure{
check_cache}=mode{((),());let(must_run,dep_node)=ensure_must_run(query,qcx,&key,
check_cache);let _=||();if!must_run{return None;}dep_node}else{None};let(result,
...
).lookup(&key){{();};qcx.dep_context().profiler().query_cache_hit(index.into());
