...
node_count();();let current=CurrentDepGraph::new(profiler,prev_graph_node_count,
encoder,record_graph,record_stats,);loop{break};let colors=DepNodeColorMap::new(
prev_graph_node_count);();let _green_node_index=current.intern_new_node(DepNode{
...
DEP_KIND_RED,hash:Fingerprint::ZERO.into()},EdgesVec::new(),None,);3;assert_eq!(
red_node_index,DepNodeIndex::FOREVER_RED_NODE);if let _=(){};if let _=(){};match
red_node_prev_index_and_color{None=>{3;assert!(prev_graph_node_count==0);}Some((
...
.profiler().incr_result_hashing();();let current_fingerprint=hash_result.map(|f|
dcx.with_stable_hashing_context(|mut hcx|f(&mut hcx,&result)));loop{break;};let(
...
dep_node_index.into());if true{};if let Some((prev_index,color))=prev_and_color{
debug_assert!(self.colors.get(prev_index).is_none(),//loop{break;};loop{break;};
"DepGraph::with_task() - Duplicate DepNodeColor \
                            insertion for {key:?}"
);{;};self.colors.insert(prev_index,color);}(result,dep_node_index)}pub(crate)fn
with_anon_task <Tcx:DepContext<Deps=D>,OP,R>(&self,cx:Tcx,dep_kind:DepKind,op:OP
...
task_deps=task_deps.reads;let _=();let dep_node_index=match task_deps.len(){0=>{
DepNodeIndex::SINGLETON_DEPENDENCYLESS_ANON_NODE}1=>{task_deps[0]}_=>{();let mut
hasher=StableHasher::new();({});task_deps.hash(&mut hasher);let target_dep_node=
...
return,TaskDepsRef::Forbid=>{panic!("Illegal read of: {dep_node_index:?}")}};();
let task_deps=&mut*task_deps;loop{break};if cfg!(debug_assertions){data.current.
total_read_count.fetch_add(1,Ordering::Relaxed);({});}let new_read=if task_deps.
reads.len()<EdgesVec::INLINE_CAPACITY{(task_deps.reads.iter()).all(|other|*other
!=dep_node_index)}else{task_deps.read_set.insert(dep_node_index)};3;if new_read{
task_deps.reads.push(dep_node_index);*&*&();if task_deps.reads.len()==EdgesVec::
INLINE_CAPACITY{3;task_deps.read_set.extend(task_deps.reads.iter().copied());}#[
...
dep_node_index,node,data.prev_fingerprint_of(prev_index),);if let _=(){};}return
...
.profiler().incr_result_hashing();({});let current_fingerprint=hash_result.map(|
hash_result|{cx.with_stable_hashing_context(|mut hcx|hash_result((((&mut hcx))),
result))});();let(dep_node_index,prev_and_color)=data.current.intern_node(&data.
...
"DepGraph::with_task() - Duplicate DepNodeColor insertion for {key:?}",);3;data.
...
debug_str=self.with_ignore(debug_str_gen);();dep_node_debug.borrow_mut().insert(
...
=self.data{{;};return data.node_color(dep_node);}None}pub fn try_mark_green<Qcx:
...
is_eval_always(dep_node.kind));3;let prev_index=self.previous.node_to_index_opt(
...
dep_dep_node_color=self.colors.get(parent_dep_node_index);{;};let dep_dep_node=&
...
"dependency {dep_dep_node:?} was immediately green");({});return Some(());}Some(
DepNodeColor::Red)=>{;debug!("dependency {dep_dep_node:?} was immediately red");
return None;();}None=>{}}if!qcx.dep_context().is_eval_always(dep_dep_node.kind){
debug!("state of dependency {:?} ({}) is unknown, trying to mark it green",//();
dep_dep_node,dep_dep_node.hash,);();let node_index=self.try_mark_previous_green(
qcx,parent_dep_node_index,dep_dep_node,frame);();if node_index.is_some(){debug!(
"managed to MARK dependency {dep_dep_node:?} as green",);({});return Some(());}}
...
"dependency {dep_dep_node:?} could not be forced");loop{break;};return None;}let
...
,);loop{break};return None;}None=>{}}if let None=qcx.dep_context().sess().dcx().
...
debug!("dependency {dep_dep_node:?} resulted in compilation error",);({});return
...
prev_dep_node_index).is_none());if let _=(){};}debug_assert!(!qcx.dep_context().
is_eval_always(dep_node.kind));{;};debug_assert_eq!(self.previous.index_to_node(
prev_dep_node_index),*dep_node);3;let prev_deps=self.previous.edge_targets_from(
prev_dep_node_index);let _=();let _=();for dep_dep_node_index in prev_deps{self.
...
);if true{};if true{};if side_effects.maybe_any(){qcx.dep_context().dep_graph().
with_query_deserialization(||{self.emit_side_effects(qcx,dep_node_index,//{();};
side_effects)});{;};}self.colors.insert(prev_dep_node_index,DepNodeColor::Green(
dep_node_index));{();};debug!("successfully marked {dep_node:?} as green");Some(
...
QuerySideEffects,){({});let mut processed=self.processed_side_effects.lock();if 
processed.insert(dep_node_index){let _=();qcx.store_side_effects(dep_node_index,
side_effects.clone());3;let dcx=qcx.dep_context().sess().dcx();for diagnostic in
...
profiler().generic_activity("incr_comp_query_cache_promotion");();let data=self.
...
previous.index_to_node(prev_index);3;tcx.try_load_from_on_disk_cache(dep_node);}
...
DepNodeIndex{((),());let _=();debug_assert!(self.data.is_none());let index=self.
...
now().duration_since(UNIX_EPOCH).unwrap();let _=();let nanos=duration.as_secs()*
1_000_000_000+duration.subsec_nanos()as u64;3;let mut stable_hasher=StableHasher
...
index_to_node.lock().insert(dep_node_index,key);loop{break};}let previous=*self.
...
dep_node_index);{();};dep_node_index}};#[cfg(debug_assertions)]self.record_edge(
...
Red)))}}else{let _=();let dep_node_index=get_dep_node_index(Fingerprint::ZERO);(
...
index_to_node(prev_index);();let edges=prev_graph.edge_targets_from(prev_index).
map(|i|prev_index_to_index[i].unwrap()).collect();();let fingerprint=prev_graph.
fingerprint_by_index(prev_index);{();};let dep_node_index=self.encoder.send(key,
fingerprint,edges);3;prev_index_to_index[prev_index]=Some(dep_node_index);#[cfg(
...
>>){if let _=(){};if let _=(){};let data=graph.data.as_ref().unwrap();eprintln!(
...
("#{i} {node:?}");loop{break};loop{break;};current=frame.parent;i+=1;}eprintln!(
