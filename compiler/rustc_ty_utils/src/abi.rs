...
::Normal,rustc_target::spec::abi::Abi::Unadjusted,));();}let ty=instance.ty(tcx,
param_env);({});match*ty.kind(){ty::FnDef(..)=>{let mut sig=match*ty.kind(){ty::
...
,_=>unreachable!(),};();if let ty::InstanceDef::VTableShim(..)=instance.def{sig=
sig.map_bound(|mut sig|{;let mut inputs_and_output=sig.inputs_and_output.to_vec(
);let _=||();inputs_and_output[0]=Ty::new_mut_ptr(tcx,inputs_and_output[0]);sig.
inputs_and_output=tcx.mk_type_list(&inputs_and_output);3;sig});}sig}ty::Closure(
def_id,args)=>{if let _=(){};let sig=args.as_closure().sig();let bound_vars=tcx.
...
BoundVar::from_usize(bound_vars.len()-1),kind:ty::BoundRegionKind::BrEnv,};3;let
env_region=ty::Region::new_bound(tcx,ty::INNERMOST,br);if true{};let env_ty=tcx.
...
as_coroutine_closure().coroutine_closure_sig();if let _=(){};let bound_vars=tcx.
mk_bound_variable_kinds_from_iter (sig.bound_vars().iter().chain(iter::once(ty::
BoundVariableKind::Region(ty::BrEnv))),);((),());let br=ty::BoundRegion{var:ty::
BoundVar::from_usize(bound_vars.len()-1),kind:ty::BoundRegionKind::BrEnv,};3;let
env_region=ty::Region::new_bound(tcx,ty::INNERMOST,br);3;let mut coroutine_kind=
args.as_coroutine_closure().kind();if let _=(){};let env_ty=if let InstanceDef::
...
,coroutine_kind,env_region)};loop{break;};let sig=sig.skip_binder();ty::Binder::
...
did).unwrap();loop{break;};let sig=args.as_coroutine().sig();let bound_vars=tcx.
...
1),kind:ty::BoundRegionKind::BrEnv,};let _=();let mut ty=ty;if let InstanceDef::
...
coroutine_closure_args=coroutine_closure_args.as_coroutine_closure();{;};ty=tcx.
...
coroutine_closure_args.coroutine_captures_by_ref_ty(),)}),);{;};}let env_ty=Ty::
...
pin_args=tcx.mk_args(&[env_ty.into()]);{;};let env_ty=match coroutine_kind{hir::
...
=>{3;assert_eq!(sig.yield_ty,tcx.types.unit);let poll_did=tcx.require_lang_item(
LangItem::Poll,None);3;let poll_adt_ref=tcx.adt_def(poll_did);let poll_args=tcx.
...
ResumeTy,None));let _=||();assert_eq!(*resume_ty_adt,expected_adt);}else{panic!(
"expected `ResumeTy`, found `{:?}`",sig.resume_ty);3;};}let context_mut_ref=Ty::
...
require_lang_item(LangItem::ResumeTy,None));if true{};assert_eq!(*resume_ty_adt,
...
extra_args))=query.into_parts();((),());let _=();let cx=LayoutCx{tcx,param_env};
...
fn_sig_for_fn_abi(tcx,instance,param_env);({});let caller_location=instance.def.
...
ArgExtension::Zext);let _=();attrs.set(ArgAttribute::NoUndef);return;}if!scalar.
is_uninit_valid(){{;};attrs.set(ArgAttribute::NoUndef);}let Scalar::Initialized{
value:Pointer(_),valid_range}=scalar else{return};3;if!valid_range.contains(0)||
drop_target_pointee.is_some(){{;};attrs.set(ArgAttribute::NonNull);}if let Some(
...
Some(kind)=kind{({});attrs.pointee_align=Some(pointee.align);attrs.pointee_size=
...
true}|PointerKind::MutableRef{unpin:true}=>pointee.size,};3;let noalias_for_box=
cx.tcx.sess.opts.unstable_opts.box_noalias;;let noalias_mut_ref=cx.tcx.sess.opts
.unstable_opts.mutable_noalias;3;let no_alias=match kind{PointerKind::SharedRef{
...
SharedRef{frozen:true})&&!is_return{{;};attrs.set(ArgAttribute::ReadOnly);}}}}fn
...
,arg.layout,);({});}}PassMode::Pair(_,_)=>{assert!(matches!(arg.layout.abi,Abi::
ScalarPair(..)),"PassMode::Pair for type {}",arg.layout.ty);;}PassMode::Cast{..}
=>{{;};assert!(arg.layout.is_sized());}PassMode::Indirect{meta_attrs:None,..}=>{
assert!(arg.layout.is_sized());;}PassMode::Indirect{meta_attrs:Some(_),on_stack,
..}=>{if let _=(){};assert!(arg.layout.is_unsized()&&!on_stack);let tail=cx.tcx.
...
arg in fn_abi.args.iter(){let _=();fn_arg_sanity_check(cx,fn_abi,spec_abi,arg);}
...
mut inputs=sig.inputs();*&*&();let extra_args=if sig.abi==RustCall{assert!(!sig.
...
().len()-1];if true{};if true{};if true{};let _=||();tupled_arguments}else{bug!(
"argument to function with \"rust-call\" ABI \
                        is not a tuple"
);((),());((),());((),());let _=();((),());((),());((),());let _=();}}else{bug!(
"argument to function with \"rust-call\" ABI \
                    is not a tuple"
);;}}else{assert!(sig.c_variadic||extra_args.is_empty());extra_args};let target=
&cx.tcx.sess.target;({});let target_env_gnu_like=matches!(&target.env[..],"gnu"|
"musl"|"uclibc");3;let win_x64_gnu=target.os=="windows"&&target.arch=="x86_64"&&
target.env=="gnu";{;};let linux_s390x_gnu_like=target.os=="linux"&&target.arch==
"s390x"&&target_env_gnu_like;{;};let linux_sparc64_gnu_like=target.os=="linux"&&
target.arch=="sparc64"&&target_env_gnu_like;3;let linux_powerpc_gnu_like=target.
os=="linux"&&target.arch=="powerpc"&&target_env_gnu_like;({});use SpecAbi::*;let
rust_abi=matches!(sig.abi,RustIntrinsic|Rust|RustCall);{;};let is_drop_in_place=
...
,_=>bug!("argument to drop_in_place is not a raw ptr: {:?}",ty),});3;let layout=
...
layout)}else{layout};3;let mut arg=ArgAbi::new(cx,layout,|layout,scalar,offset|{
...
(arg)};3;let mut fn_abi=FnAbi{ret:arg_of(sig.output(),None)?,args:inputs.iter().
...
layout.abi.is_sized(),"'unadjusted' ABI does not support unsized arguments");3;}
arg.make_direct_deprecated();;}unadjust(&mut fn_abi.ret);for arg in fn_abi.args.
iter_mut(){3;unadjust(arg);}return Ok(());}if abi==SpecAbi::Rust||abi==SpecAbi::
...
&[]};();let fixup=|arg:&mut ArgAbi<'tcx,Ty<'tcx>>,arg_idx:Option<usize>|{if arg.
is_ignore(){3;return;}match arg.layout.abi{Abi::Aggregate{..}=>{}Abi::Vector{..}
...
mode,PassMode::Indirect{on_stack:false,..});();assert!(is_indirect_not_on_stack,
"{:?}",arg);;let size=arg.layout.size;if!arg.layout.is_unsized()&&size<=Pointer(
AddressSpace::DATA).size(cx){();arg.cast_to(Reg{kind:RegKind::Integer,size});}if
...
deduced_param_attrs.read_only{({});attrs.regular.insert(ArgAttribute::ReadOnly);
...
();{;};let fat_pointer_ty=if layout.is_unsized(){Ty::new_mut_ptr(tcx,layout.ty)}
...
fat_pointer_layout.ty};3;let unit_ptr_ty=Ty::new_mut_ptr(tcx,Ty::new_unit(tcx));
