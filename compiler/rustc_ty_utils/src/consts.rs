...
"cannot destructure constant {:?}",const_)};({});let branches=match valtree{ty::
...
fields;let _=();let mut field_consts=Vec::with_capacity(fields.len());for(field,
field_valtree)in iter::zip(fields,branches){;let field_ty=field.ty(tcx,args);let
field_const=ty::Const::new_value(tcx,*field_valtree,field_ty);({});field_consts.
push(field_const);3;}debug!(?field_consts);(field_consts,Some(variant_idx))}ty::
...
>>();({});(fields,None)}_=>bug!("cannot destructure constant {:?}",const_),};let
...
[node];3;let maybe_supported_error=|a|maybe_supported_error(tcx,a,root_span);let
...
root_span)?}&ExprKind::Literal{lit,neg}=>{{;};let sp=node.span;match tcx.at(sp).
...
lit);3;ty::Const::new_value(tcx,val,node.ty)}&ExprKind::ZstLiteral{user_ty:_}=>{
...
=>{3;let fun=recurse_build(tcx,body,*fun,root_span)?;let mut new_args=Vec::<ty::
Const<'tcx>>::with_capacity(args.len());{;};for&id in args.iter(){new_args.push(
...
,lhs,root_span)?;({});let rhs=recurse_build(tcx,body,rhs,root_span)?;ty::Const::
new_expr (tcx,(((Expr::Binop(op,lhs,rhs)))),node.ty)}&ExprKind::Unary{op,arg}if 
check_unop(op)=>{({});let arg=recurse_build(tcx,body,arg,root_span)?;ty::Const::
...
!tcx.features().generic_const_exprs{();return Ok(None);}match tcx.def_kind(def){
DefKind::AnonConst|DefKind::InlineConst=>(),_=>return Ok(None),}();let body=tcx.
thir_body(def)?;loop{break};let(body,body_id)=(&*body.0.borrow(),body.1);let mut
is_poly_vis=IsThirPolymorphic{is_poly:false,thir:body};{;};visit::walk_expr(&mut
is_poly_vis,&body[body_id]);let _=();if!is_poly_vis.is_poly{return Ok(None);}let
