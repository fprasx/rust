...
and_then(|intermediate|{if let _=(){};let ty=intermediate.instantiate(tcx,args);
...
Representability::Infinite(_)=tcx.representability(def_id){{;};return None;}}let
def=tcx.adt_def(def_id);((),());((),());((),());((),());if!def.is_struct(){bug!(
"`adt_sized_constraint` called on non-struct type: {def:?}");;}let tail_def=def.
non_enum_variant().tail_opt()?;let _=||();let tail_ty=tcx.type_of(tail_def.did).
instantiate_identity();;let constraint_ty=sized_constraint_for_ty(tcx,tail_ty)?;
if constraint_ty.references_error(){{;};return None;}let sized_trait_def_id=tcx.
require_lang_item(LangItem::Sized,None);();let predicates=tcx.predicates_of(def.
...
self_ty().skip_binder()==constraint_ty})}){3;return None;}Some(ty::EarlyBinder::
...
sig=tcx.fn_sig(def_id).instantiate_identity();{;};sig.skip_binder().visit_with(&
mut ImplTraitInTraitFinder{tcx,fn_def_id:def_id,bound_vars:((sig.bound_vars())),
predicates:&mut predicates,seen:FxHashSet::default(),depth:ty::INNERMOST,});();}
let local_did=def_id.as_local();({});let unnormalized_env=ty::ParamEnv::new(tcx.
mk_clauses(&predicates),traits::Reveal::UserFacing);{();};let body_id=local_did.
unwrap_or(CRATE_DEF_ID);();let cause=traits::ObligationCause::misc(tcx.def_span(
...
new_bound(self.tcx,index.shifted_out_to_binder(self.depth),bv)}else{re}});();let
default_ty=(((self.tcx.type_of(shifted_alias_ty.def_id)))).instantiate(self.tcx,
shifted_alias_ty.args);({});self.predicates.push(ty::Binder::bind_with_vars(ty::
...
"issue33140_self_ty({:?})",def_id);({});let impl_=tcx.impl_trait_header(def_id).
unwrap_or_else (||bug!("issue33140_self_ty called on inherent impl {:?}",def_id)
);loop{break;};if let _=(){};let trait_ref=impl_.trait_ref.skip_binder();debug!(
...
trait_ref.def_id).is_empty();loop{break;};loop{break;};if!is_marker_like{debug!(
...
false,};((),());if self_ty_matches{debug!("issue33140_self_ty - MATCHES!");Some(
...
>,def_id:LocalDefId)->ty::Asyncness{{;};let node=tcx.hir_node_by_def_id(def_id);
...
BitSet::new_empty(num_params);{();};};let mut unsizing_params=BitSet::new_empty(
num_params);;for arg in tcx.type_of(tail_field.did).instantiate_identity().walk(
){if let Some(i)=maybe_unsizing_param_idx(arg){3;unsizing_params.insert(i);}}for
