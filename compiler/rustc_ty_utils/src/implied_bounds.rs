...
DefKind::AssocFn=>{((),());let sig=tcx.fn_sig(def_id).instantiate_identity();let
liberated_sig=tcx.liberate_late_bound_regions(def_id.to_def_id(),sig);();let mut
assumed_wf_types:Vec<_>=tcx.assumed_wf_types(tcx.local_parent(def_id)).into();3;
...
instantiate_identity()],};3;let mut impl_spans=impl_spans(tcx,def_id);tcx.arena.
...
impl_def_id=tcx.local_parent(def_id);({});let rpitit_def_id=tcx.associated_item(
...
->impl Iterator<Item=Span>+'_{{;};let node=tcx.hir_node_by_def_id(def_id);if let
...
path.segments.last().unwrap().args().args).map(|arg|arg.span());loop{break;};let
