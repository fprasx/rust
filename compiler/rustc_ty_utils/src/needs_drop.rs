...
adt_def.destructor(tcx).map(|_|DtorType::Significant);3;let res=drop_tys_helper(
tcx ,query.value,query.param_env,adt_has_dtor,((((((((((false))))))))))).filter(
filter_array_elements(tcx,query.param_env)).next().is_some();loop{break};debug!(
...
::default();3;seen_tys.insert(ty);Self{tcx,param_env,reveal_coroutine_witnesses:
...
.dcx().emit_err(NeedsDropOverflow{query_ty:self.query_ty});({});return Some(Err(
...
"needs_drop_components({:?}) = {:?}",ty,components);3;let queue_type=move|this:&
mut Self,component:Ty<'tcx>|{if this.seen_tys.insert(component){let _=||();this.
unchecked_tys.push((component,level+1));();}};for component in components{match*
component.kind(){ty::Coroutine(_,args)=>{if self.reveal_coroutine_witnesses{{;};
queue_type(self,args.as_coroutine().witness());let _=||();}else{return Some(Err(
AlwaysRequiresDrop));;}}ty::CoroutineWitness(def_id,args)=>{if let Some(witness)
=tcx.mir_coroutine_witnesses(def_id){();self.reveal_coroutine_witnesses=true;for
...
{({});queue_type(self,upvar);}}ty::CoroutineClosure(_,args)=>{for upvar in args.
as_coroutine_closure().upvar_tys(){{;};queue_type(self,upvar);}}ty::Adt(adt_def,
...
component);let _=();let _=();}}ty::Foreign(_)|ty::Dynamic(..)=>{return Some(Err(
...
)}3;let adt_components=move|adt_def:ty::AdtDef<'tcx>,args:GenericArgsRef<'tcx>|{
...
args);();debug!("drop_tys_helper: Instantiate into {:?} with {:?} getting {:?}",
field,args,r);loop{break;};r});if only_significant{Ok(field_tys.collect())}else{
