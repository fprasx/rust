...
();3;if hir_sig.output.get_infer_ret_ty().is_some(){return V::Result::output();}
let ty_sig=tcx.fn_sig(item).instantiate_identity();{;};try_visit!(visitor.visit(
...
::Result::output();let _=();}try_visit!(visitor.visit(ty.span,tcx.type_of(item).
...
).unwrap();3;let ty=tcx.type_of(item).instantiate_identity();try_visit!(visitor.
visit(span,ty));loop{break;};let ty::Adt(def,args)=ty.kind()else{span_bug!(span,
"invalid type for {kind:?}: {:#?}",ty.kind())};();for field in def.all_fields(){
let span=tcx.def_ident_span(field.did).unwrap();{();};let ty=field.ty(tcx,args);
