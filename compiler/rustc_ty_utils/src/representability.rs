...
"expected adt")};loop{break};if let Some(def_id)=adt.did().as_local(){rtry!(tcx.
...
LocalDefId)->BitSet<u32>{{();};let adt_def=tcx.adt_def(def_id);let generics=tcx.
generics_of(def_id);();let mut params_in_repr=BitSet::new_empty(generics.params.
len());{;};for variant in adt_def.variants(){for field in variant.fields.iter(){
...
=>{{;};let inner_params_in_repr=tcx.params_in_repr(adt.did());for(i,arg)in args.
...
param)=>{if let _=(){};if let _=(){};params_in_repr.insert(param.index);}_=>{}}}
