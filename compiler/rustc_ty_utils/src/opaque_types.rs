...
span:Span,value:impl TypeVisitable<TyCtxt<'tcx>>){3;let old=self.span;self.span=
Some(span);;value.visit_with(self);self.span=old;}fn parent_impl_trait_ref(&self
)->Option<ty::TraitRef<'tcx>>{();let parent=self.parent()?;if matches!(self.tcx.
...
opaque_hir_id=self.tcx.local_def_id_to_hir_id(opaque_def_id);{;};let scope=self.
tcx.hir().get_defining_scope(opaque_hir_id);*&*&();while hir_id!=scope&&hir_id!=
CRATE_HIR_ID{({});hir_id=self.tcx.hir().get_parent_item(hir_id).into();}hir_id==
...
id){{;};let items=self.collector.tcx.opaque_types_defined_by(id);self.collector.
...
hir().body(id);((),());self.visit_body(body);}}TaitInBodyFinder{collector:self}.
...
check_tait_defining_scope(alias_ty.def_id.expect_local()){{();};return;}}}}self.
opaques.push(alias_ty.def_id.expect_local());let _=();let parent_count=self.tcx.
...
def_id).instantiate_identity_iter_copied(){{;};trace!(?pred);self.visit_spanned(
...
;let _=||();}Err(NotUniqueParam::DuplicateParam(arg))=>{self.tcx.dcx().emit_err(
...
def_id){3;continue;}if!assoc.defaultness(self.tcx).is_final(){continue;}if!self.
...
;{;};self.visit_opaque_ty(alias_ty);}}ty::Adt(def,_)if def.did().is_local()=>{if
let CollectionMode::ImplTraitInAssocTypes=self.mode{;return;}if!self.seen.insert
(def.did().expect_local()){({});return;}for variant in def.variants().iter(){for
...
!(?kind);3;let mut collector=OpaqueTypeCollector::new(tcx,item);super::sig_types
