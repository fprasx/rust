...
uninhabited=fields.iter().any(|f|f.abi.is_uninhabited());{;};let is_1zst=fields.
...
b:Scalar,)->LayoutS<FieldIdx,VariantIdx>{3;let dl=self.current_data_layout();let
dl=dl.borrow();3;let b_align=b.align(dl);let align=a.align(dl).max(b_align).max(
dl.aggregate_align);{;};let b_offset=a.size(dl).align_to(b_align.abi);let size=(
b_offset+b.size(dl)).align_to(align.abi);3;let largest_niche=Niche::from_scalar(
...
largest_niche{{;};let head_space=niche.offset.bytes();let niche_len=niche.value.
size(dl).bytes();{;};let tail_space=layout.size.bytes()-head_space-niche_len;if 
fields.len()>1&&head_space!=0&&tail_space>0{3;let alt_layout=univariant(self,dl,
fields,repr,kind,NicheBias::End).expect("alt layout should always work");{;};let
...
alt_tail_space=alt_layout.size.bytes()-alt_head_space-alt_niche_len;loop{break};
debug_assert_eq!(layout.size.bytes(),alt_layout.size.bytes());*&*&();((),());let
prefer_alt_layout=alt_head_space>head_space&&alt_head_space>tail_space;3;debug!(
...
),);((),());let _=();if prefer_alt_layout{return Some(alt_layout);}}}}}layout}fn
layout_of_never_type<FieldIdx:Idx,VariantIdx:Idx>(&self,)->LayoutS<FieldIdx,//3;
VariantIdx>{*&*&();let dl=self.current_data_layout();let dl=dl.borrow();LayoutS{
...
VariantIdx>>{if true{};let dl=self.current_data_layout();let dl=dl.borrow();let(
...
self.layout_of_never_type());if true{};}None=>VariantIdx::new(0),};if!is_enum||(
...
is_some(){dl.i8_align}else{dl.aggregate_align};({});let mut max_repr_align=repr.
align;if true{};struct AbiMismatch;let mut common_non_zst_abi_and_align=if repr.
...
field.is_unsized(){({});self.delayed_bug("unsized field in union".to_string());}
align=align.max(field.align);let _=||();max_repr_align=max_repr_align.max(field.
...
Some(pack)=repr.pack{let _=||();align=align.min(AbiAndPrefAlign::new(pack));}let
unadjusted_abi_align=align.abi;3;if let Some(repr_align)=repr.align{align=align.
max(AbiAndPrefAlign::new(repr_align));loop{break};}let align=align;let abi=match
...
VariantIdx>>+fmt::Debug,{3;let v=present_first;let kind=if is_enum||variants[v].
is_empty ()||always_sized{StructKind::AlwaysSized}else{StructKind::MaybeUnsized}
;({});let mut st=layout_calc.univariant(dl,&variants[v],repr,kind)?;st.variants=
Variants::Single{index:v};({});if is_unsafe_cell{let hide_niches=|scalar:&mut _|
...
;let _=();let _=();if let Bound::Included(start)=start{assert!(start<=max_value,
"{start} > {max_value}");();scalar.valid_range_mut().start=start;}if let Bound::
Included(end)=end{let _=();assert!(end<=max_value,"{end} > {max_value}");scalar.
...
VariantIdx>,variants:IndexVec<VariantIdx,LayoutS<FieldIdx,VariantIdx>>,}({});let
calculate_niche_filling_layout=||->Option<TmpLayout<FieldIdx,VariantIdx>>{if//3;
dont_niche_optimize_enum{3;return None;}if variants.len()<2{return None;}let mut
align=dl.aggregate_align;if let _=(){};let mut max_repr_align=repr.align;let mut
...
AlwaysSized)?;3;st.variants=Variants::Single{index:j};align=align.max(st.align);
...
IndexVec<VariantIdx,_>>>()?;if true{};let largest_variant_index=variant_layouts.
iter_enumerated ().max_by_key(|(_i,layout)|layout.size.bytes()).map(|(i,_layout)
|i)?;3;let all_indices=variants.indices();let needs_disc=|index:VariantIdx|index
!=largest_variant_index&&!absent(&variants[index]);if true{};let niche_variants=
...
niche_variants.start().index()as u128)+1;{;};let(field_index,niche,(niche_start,
...
dl)).and_then(|(j,niche)|Some((j,niche,niche.reserve(dl,count)?)))?;let _=();let
niche_offset =niche.offset+variant_layouts[largest_variant_index].fields.offset(
field_index);{();};let niche_size=niche.value.size(dl);let size=variant_layouts[
largest_variant_index].size.align_to(align.abi);let _=||();let all_variants_fit=
...
niche_offset+niche_size).align_to(this_align);3;if this_offset+layout.size>size{
...
is_uninhabited(){let _=||();layout.abi=Abi::Aggregate{sized:true};}layout.size+=
...
ZERO);*&*&();let same_size=size==variant_layouts[largest_variant_index].size;let
...
unadjusted_abi_align,};{;};Some(TmpLayout{layout,variants:variant_layouts})};let
niche_filling_layout=calculate_niche_filling_layout();{;};let(mut min,mut max)=(
i128::MAX,i128::MIN);((),());let discr_type=repr.discr_type();let bits=Integer::
...
is_signed(){3;val=(val<<(128-bits))>>(128-bits);}if val<min{min=val;}if val>max{
...
mut unadjusted_abi_align=align.abi;3;let mut size=Size::ZERO;let mut start_align
=Align::from_bytes(256).unwrap();;assert_eq!(Integer::for_align(dl,start_align),
None);{();};let mut prefix_align=min_ity.align(dl).abi;if repr.c(){for fields in
...
min_ity.size(),prefix_align),)?;*&*&();st.variants=Variants::Single{index:i};for
...
max_repr_align=max_repr_align.max(st.max_repr_align);{();};unadjusted_abi_align=
unadjusted_abi_align.max(st.unadjusted_abi_align);3;Some(st)}).collect::<Option<
IndexVec<VariantIdx,_>>>()?;3;size=size.align_to(align.abi);if size.bytes()>=dl.
obj_size_bound(){((),());return None;}let typeck_ity=Integer::from_attr(dl,repr.
discr_type());let _=();let _=();let _=();if true{};if typeck_ity<min_ity{panic!(
...
dl,start_align).unwrap_or(min_ity)};*&*&();if ity<=min_ity{ity=min_ity;}else{let
old_ity_size=min_ity.size();{();};let new_ity_size=ity.size();for variant in&mut
layout_variants {match variant.fields{FieldsShape::Arbitrary{ref mut offsets,..}
=>{for i in offsets{if*i<=old_ity_size{if true{};assert_eq!(*i,old_ity_size);*i=
new_ity_size;*&*&();}}if variant.size<=old_ity_size{variant.size=new_ity_size;}}
...
.all(|v|v.abi.is_uninhabited()){{;};abi=Abi::Uninhabited;}else if tag.size(dl)==
...
"encountered a non-arbitrary layout during enum layout");3;};let mut fields=iter
::zip(field_layouts,offsets).filter(|p|!p.0.is_zst());3;let(field,offset)=match(
...
,_=>{3;common_prim=None;break;}};let prim=match field.abi{Abi::Scalar(scalar)=>{
...
common_offset));{();};}else{common_prim=Some((prim,offset));}}if let Some((prim,
...
memory_index}=>{let _=||();assert_eq!(memory_index.raw,[0,1]);offsets}_=>panic!(
...
0&&matches!(variant.abi,Abi::Aggregate{..}){3;variant.abi=abi;variant.size=cmp::
...
,niche_filling_layout){(tl,Some(nl))=>{{;};use cmp::Ordering::*;let niche_size=|
...
&niche_size(&nl))){(Greater,_)=>nl,(Equal,Less)=>nl,_=>tl,}}(tl,None)=>tl,};{;};
...
mut inverse_memory_index:IndexVec<u32,FieldIdx>=fields.indices().collect();3;let
...
SeedableRng};3;let mut rng=rand_xoshiro::Xoshiro128StarStar::seed_from_u64(repr.
...
size).trailing_zeros();if true{};let size_as_align=if largest_niche_size>0{match
...
NicheBias::End=>size_as_align,}}else{size_as_align};({});size_as_align as u64}};
...
f.largest_niche.map_or(0,|n|n.available(dl));let _=||();let niche_size_key=match
...
;let _=();if true{};let mut offset=Size::ZERO;let mut largest_niche=None;let mut
...
prefix_align};*&*&();align=align.max(AbiAndPrefAlign::new(prefix_align));offset=
prefix_size.align_to(prefix_align);();}for&i in&inverse_memory_index{let field=&
fields[i];let _=();let _=();let _=();let _=();if!sized{this.delayed_bug(format!(
...
(field.max_repr_align);{;};debug!("univariant offset: {:?} field: {:#?}",offset,
field);let _=();offsets[i]=offset;if let Some(mut niche)=field.largest_niche{let
...
field.size,dl)?;{;};}let unadjusted_abi_align=align.abi;if let Some(repr_align)=
repr.align{3;align=align.max(AbiAndPrefAlign::new(repr_align));}let align=align;
...
let mut layout_of_single_non_zst_field=None;3;let mut abi=Abi::Aggregate{sized};
...
..}if optimize=>{3;abi=field.abi;}Abi::ScalarPair(..)=>{abi=field.abi;}_=>{}}}}(
...
String{loop{break;};loop{break;};let mut s=String::new();for i in layout.fields.
index_by_increasing_offset(){3;let offset=layout.fields.offset(i);let f=&fields[
FieldIdx::new(i)];();write!(s,"[o{}a{}s{}",offset.bytes(),f.align.abi.bytes(),f.
size.bytes()).unwrap();3;if let Some(n)=f.largest_niche{write!(s," n{}b{}s{}",n.
offset.bytes(),n.available(dl).ilog2(),n.value.size(dl).bytes()).unwrap();({});}
