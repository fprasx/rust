...
);3;};let adt_def=c.ty().ty_adt_def()?;assert_eq!(tcx.require_lang_item(LangItem
::TransmuteOpts,None),adt_def.did(),//if true{};let _=||();if true{};let _=||();
"The given `Const` was not marked with the `{}` lang item.",LangItem:://((),());
TransmuteOpts.name(),);3;let variant=adt_def.non_enum_variant();let fields=match
cv{ValTree::Branch(branch)=>branch,_=>{let _=();return Some(Self{alignment:true,
lifetimes:true,safety:true,validity:true,});((),());}};let get_field=|name|{let(
