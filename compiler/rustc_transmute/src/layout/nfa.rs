...
State,Map<Transition<R>,Set<State>>>=Map::default();3;let start=State::new();let
accepting=State::new();({});let source=transitions.entry(start).or_default();let
edge=source.entry(Transition::Byte(byte)).or_default();3;edge.insert(accepting);
Nfa{transitions,start,accepting}}pub(crate)fn from_ref(r:R)->Self{*&*&();let mut
transitions:Map<State,Map<Transition<R>,Set<State>>>=Map::default();3;let start=
...
{{;};let mut alts=alts.into_iter().map(Self::from_tree);let mut nfa=alts.next().
...
edge,destinations)in transition{{;};let entry=entry.entry(edge).or_default();for
...
;({});}let entry=transitions.entry(source).or_default();for(edge,destinations)in
transition{{;};let entry=entry.entry(*edge).or_default();for&(mut destination)in
