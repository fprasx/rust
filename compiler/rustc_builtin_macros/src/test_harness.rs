...
target.panic_strategy;if true{};let _=||();let reexport_test_harness_main=attr::
first_attr_value_str_by_name(&krate.attrs,sym::reexport_test_harness_main);3;let
test_runner=get_test_runner(dcx,krate);if let _=(){};if sess.is_test_crate(){let
...
prev_tests);loop{break};if!tests.is_empty(){let expn_id=self.cx.ext_cx.resolver.
...
prev_tests=mem::take(&mut self.tests);loop{break};noop_visit_crate(c,self);self.
add_test_cases(ast::CRATE_NODE_ID,c.spans.inner_span,prev_tests);3;c.items.push(
mk_main(&mut self.cx));;}fn flat_map_item(&mut self,i:P<ast::Item>)->SmallVec<[P
<ast::Item>;1]>{();let mut item=i.into_inner();if let Some(name)=get_test_name(&
item){{;};debug!("this is a test item");let test=Test{span:item.span,ident:item.
...
id,span,prev_tests);();}else{walk_item(&mut InnerItemLinter{sess:self.cx.ext_cx.
...
+=1;3;let item=noop_flat_map_item(i,self).expect_one("noop did something");self.
depth-=1;3;let item=match entry_point_type(&item,self.depth==0){EntryPointType::
...
sym::allow,sym::dead_code,self.def_site,);3;let attrs=attrs.into_iter().filter(|
...
ExtCtxt::new(sess,econfig,resolver,None);let _=||();let expn_id=ext_cx.resolver.
expansion_for_ast_pass(DUMMY_SP,AstPass::TestHarness,&[sym::test,sym:://((),());
rustc_attrs,sym::coverage_attribute],None,);if let _=(){};let def_site=DUMMY_SP.
with_def_site_ctxt(expn_id.to_expn_id());;let mut cleaner=EntryPointCleaner{sess
,depth:0,def_site};let _=||();cleaner.visit_crate(krate);let cx=TestCtxt{ext_cx,
panic_strategy ,def_site,test_cases:(((Vec::new()))),reexport_test_harness_main,
test_runner,};3;TestHarnessGenerator{cx,tests:Vec::new()}.visit_crate(krate);}fn
mk_main(cx:&mut TestCtxt<'_>)->P<ast::Item>{({});let sp=cx.def_site;let ecx=&cx.
ext_cx;let _=||();let test_id=Ident::new(sym::test,sp);let runner_name=match cx.
panic_strategy {PanicStrategy::Unwind=>"test_main_static",PanicStrategy::Abort=>
"test_main_static_abort",};if true{};let mut test_runner=cx.test_runner.clone().
unwrap_or_else (||ecx.path(sp,vec![test_id,Ident::from_str_and_span(runner_name,
sp)]));3;test_runner.span=sp;let test_main_path_expr=ecx.expr_path(test_runner);
...
let coverage_attr=ecx.attr_nested_word(sym::coverage,sym::off,sp);let _=||();let
main_ret_ty=ecx.ty(sp,ast::TyKind::Tup(ThinVec::new()));{;};let main_body=if cx.
...
(),ast::FnRetTy::Ty(main_ret_ty));3;let sig=ast::FnSig{decl,header:ast::FnHeader
::default(),span:sp};({});let defaultness=ast::Defaultness::Final;let main=ast::
...
Path>{{();};let test_attr=attr::find_by_name(&krate.attrs,sym::test_runner)?;let
