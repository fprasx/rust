...
is_const:bool,){let _=();let krate=sym::rustc_serialize;let typaram=sym::__D;let
...
),Ident::new(sym::decode,trait_span),];if true{};let exprdecode=cx.expr_path(cx.
path_global(trait_span,recurse));3;let blkarg=Ident::new(sym::_d,trait_span);let
...
len(),Named(fields)=>fields.len(),};{;};let fn_read_struct_field_path:Vec<_>=cx.
def_site_path(&[sym::rustc_serialize,sym::Decoder,sym::read_struct_field]);3;let
...
exprdecode.clone(),],),)});let _=();let result=cx.expr_ok(trait_span,result);let
...
fields)=>{{();};let variant=Ident::new(sym::i,trait_span);let mut arms=ThinVec::
with_capacity(fields.len()+1);();let mut variants=ThinVec::with_capacity(fields.
...
name));3;let path=cx.path(trait_span,vec![substr.type_ident,ident]);let decoded=
...
exprdecode.clone()],),)});let _=();arms.push(cx.arm(v_span,cx.pat_lit(v_span,cx.
expr_usize(v_span,i)),decoded));3;}arms.push(cx.arm_unreachable(trait_span));let
result =cx.expr_ok(trait_span,cx.expr_match(trait_span,cx.expr_ident(trait_span,
variant),arms),);3;let lambda=cx.lambda(trait_span,vec![blkarg,variant],result);
...
Decoder,sym::read_enum_variant]);({});let result=cx.expr_call_global(trait_span,
fn_read_enum_variant_path,thin_vec![blkdecoder,variant_array_ref,lambda],);3;let
...
is_tuple)=>{{;};let path_expr=cx.expr_path(outer_pat_path);if matches!(is_tuple,
...
).enumerate().map(|(i,&(ident,span))|{3;let arg=getarg(cx,span,ident.name,i);cx.
