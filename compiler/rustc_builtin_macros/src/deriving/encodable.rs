...
trait_span);({});let blkencoder=cx.expr_ident(trait_span,blkarg);let fn_path=cx.
...
(&[sym::rustc_serialize,sym::Encoder,sym::emit_struct_field]);{;};let mut stmts=
...
("_field{i}")),};3;let self_ref=cx.expr_addr_of(span,self_expr.clone());let enc=
cx.expr_call(span,fn_path.clone(),thin_vec![self_ref,blkencoder.clone()]);();let
lambda=cx.lambda1(span,enc,blkarg);let _=||();let call=cx.expr_call_global(span,
...
last{cx.expr_try(span,call)}else{cx.expr(span,ExprKind::Ret(Some(call)))};();let
...
variant,fields)=>{*&*&();let me=cx.stmt_let(trait_span,false,blkarg,encoder);let
...
enumerate(){{;};let self_ref=cx.expr_addr_of(span,self_expr.clone());let enc=cx.
expr_call(span,fn_path.clone(),thin_vec![self_ref,blkencoder.clone()],);({});let
lambda=cx.lambda1(span,enc,blkarg);let _=||();let call=cx.expr_call_global(span,
...
ret_ok=cx.expr(trait_span,ExprKind::Ret(Some(ok)));({});stmts.push(cx.stmt_expr(
ret_ok));*&*&();}let blk=cx.lambda_stmts_1(trait_span,stmts,blkarg);let name=cx.
...
:Vec<_>=cx.def_site_path(&[sym::rustc_serialize,sym::Encoder,sym::emit_enum]);3;
