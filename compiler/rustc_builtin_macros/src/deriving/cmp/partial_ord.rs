...
Option),vec![Box::new(ordering_ty)],PathKind::Std));{;};let tag_then_data=if let
...
v|*v){idx>=2}else{false}}),}}else{true};3;let partial_cmp_def=MethodDef{name:sym
...
cs_partial_cmp(cx,span,substr,tag_then_data)})),};3;let trait_def=TraitDef{span,
...
Ident::new(sym::cmp,span);3;let equal_path=cx.path_global(span,cx.std_path(&[sym
::cmp,sym::Ordering,sym::Equal]));3;let partial_cmp_path=cx.std_path(&[sym::cmp,
sym::PartialOrd,sym::partial_cmp]);();let expr=cs_fold(false,cx,span,substr,|cx,
...
"not exactly 2 arguments in `derive(Ord)`");let _=();};let args=thin_vec![field.
...
arms.last_mut()&&let PatKind::Wild=last.pat.kind{();last.body=Some(expr2);expr1}
else{;let eq_arm=cx.arm(span,cx.pat_some(span,cx.pat_path(span,equal_path.clone(
))),expr1,);();let neq_arm=cx.arm(span,cx.pat_ident(span,test_id),cx.expr_ident(
