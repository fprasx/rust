...
),is_const:bool,){let _=();let bounds;let substructure;let is_simple;match item{
...
current_expansion.id.expn_data().parent.expect_local();3;let has_derive_copy=cx.
resolver.has_derive_copy(container_id);;if has_derive_copy&&!params.iter().any(|
param|matches!(param.kind,ast::GenericParamKind::Type{..})){{();};bounds=vec![];
is_simple=true;loop{break};substructure=combine_substructure(Box::new(|c,s,sub|{
cs_clone_simple("Clone",c,s,sub,false)}));3;}else{bounds=vec![];is_simple=false;
substructure =combine_substructure(Box::new(|c,s,sub|cs_clone("Clone",c,s,sub)))
;3;}}ItemKind::Union(..)=>{bounds=vec![Path(path_std!(marker::Copy))];is_simple=
...
mut seen_type_names=FxHashSet::default();({});let mut process_variant=|variant:&
...
BlockOrExpr{3;let ctor_path;let all_fields;let fn_path=cx.std_path(&[sym::clone,
...
clone(),args)};3;let vdata;match substr.fields{Struct(vdata_,af)=>{ctor_path=cx.
path(trait_span,vec![substr.type_ident]);if true{};all_fields=af;vdata=*vdata_;}
EnumMatching(..,variant,af)=>{let _=();ctor_path=cx.path(trait_span,vec![substr.
type_ident,variant.ident]);{();};all_fields=af;vdata=&variant.data;}EnumTag(..)|
...
field|{{;};let Some(ident)=field.name else{cx.dcx().span_bug(trait_span,format!(
