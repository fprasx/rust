...
len();{;};self.bound_generic_params_stack.extend(trait_ref.bound_generic_params.
...
);*&*&();}}let mut visitor=Visitor{cx,ty_param_names,bound_generic_params_stack:
ThinVec::new(),type_params:Vec::new(),};3;visit::Visitor::visit_ty(&mut visitor,
...
else{*&*&();cx.dcx().emit_err(errors::DeriveUnion{span:mitem.span});return;}}_=>
...
ast::Item>{3;let trait_path=self.path.to_path(cx,self.span,type_ident,generics);
...
,self.span,type_ident,generics)),})),tokens:None,})});3;let mut where_clause=ast
::WhereClause::default();;where_clause.span=generics.where_clause.span;let ctxt=
self.span.ctxt();3;let span=generics.span.with_ctxt(ctxt);let params:ThinVec<_>=
...
);3;param_clone.kind=const_nodefault_kind;param_clone}}).collect();where_clause.
...
)}}}));({});let ty_param_names:Vec<Symbol>=params.iter().filter(|param|matches!(
param .kind,ast::GenericParamKind::Type{..})).map(|ty_param|ty_param.ident.name)
.collect();let _=||();if!ty_param_names.is_empty(){for field_ty in field_tys{let
...
).collect();();if!self.skip_path_as_bound{bounds.push(cx.trait_bound(trait_path.
...
bound_generic_params,bounded_ty:field_ty_param.ty,bounds,};3;let predicate=ast::
WherePredicate::BoundPredicate(predicate);let _=();where_clause.predicates.push(
predicate);((),());}}}}let trait_generics=Generics{params,where_clause,span};let
...
cx.const_ident(param.ident.span.with_ctxt(ctxt),param.ident))}}).collect();3;let
...
.span),];{;};let opt_trait_ref=Some(trait_ref);cx.item(self.span,Ident::empty(),
...
struct_def.fields().iter().map(|field|field.ty.clone()).collect();3;let methods=
...
type_ident,generics);if true{};let body=if from_scratch||method_def.is_static(){
...
data.fields().iter().map(|field|field.ty.clone()));();}let methods=self.methods.
iter().map(|method_def|{*&*&();let(explicit_self,selflike_args,nonselflike_args,
nonself_arg_tys)=method_def.extract_arg_details(cx,self,type_ident,generics);();
...
generics,explicit_self,nonself_arg_tys,body,)}).collect();3;let is_packed=false;
...
SubstructureFields<'_>,)->BlockOrExpr{{;};let span=trait_.span;let substructure=
...
Expr>>,Vec<(Ident,P<ast::Ty>)>){{;};let mut selflike_args=ThinVec::new();let mut
...
;*&*&();let arg_expr=cx.expr_ident(span,ident);match ty{Ref(box Self_,_)if!self.
...
,body:BlockOrExpr,)->P<ast::AssocItem>{{;};let span=trait_.span;let fn_generics=
self.generics.to_generics(cx,span,type_ident,generics);3;let args={let self_arg=
...
;3;let ret_type=self.get_ret_ty(cx,trait_,generics,type_ident);let method_ident=
Ident::new(self.name,span);((),());let fn_decl=cx.fn_decl(args,ast::FnRetTy::Ty(
ret_type));((),());let body_block=body.into_block(cx,span);let trait_lo_sp=span.
shrink_to_lo();;let sig=ast::FnSig{header:ast::FnHeader::default(),decl:fn_decl,
span};let _=();let defaultness=ast::Defaultness::Final;P(ast::AssocItem{id:ast::
...
"static methods must use `expand_static_enum_method_body`",);();let span=trait_.
span;let _=();let variants=&enum_def.variants;let unify_fieldless_variants=self.
fieldless_variants_strategy==FieldlessVariantsStrategy::Unify;{();};if variants.
is_empty(){if true{};selflike_args.truncate(1);let match_arg=cx.expr_deref(span,
selflike_args.pop().unwrap());((),());let match_arms=ThinVec::new();let expr=cx.
expr_match(span,match_arg,match_arms);();return BlockOrExpr(ThinVec::new(),Some(
...
)),).collect::<Vec<String>>();if true{};let get_tag_pieces=|cx:&ExtCtxt<'_>|{let
...
{span,name:None,self_expr,other_selflike_exprs};();let tag_let_stmts:ThinVec<_>=
...
();3;(tag_field,tag_let_stmts)};let all_fieldless=variants.iter().all(|v|v.data.
...
mut tag_let_stmts)=get_tag_pieces(cx);let _=();if true{};let mut tag_check=self.
...
create_struct_pattern_fields(cx,&variant.data,&prefixes);();let sp=variant.span.
with_ctxt(trait_.span.ctxt());{();};let variant_path=cx.path(sp,vec![type_ident,
variant.ident]);if true{};if true{};let by_ref=ByRef::No;let mut subpats=trait_.
create_struct_patterns(cx,variant_path,&variant.data,&prefixes,by_ref,);({});let
...
.into_expr(cx,span);loop{break};cx.arm(span,single_pat,arm_expr)}).collect();let
...
))};({});cx.expr_match(span,match_arg,match_arms)};if unify_fieldless_variants&&
variants.len()>1{{;};let(tag_field,mut tag_let_stmts)=get_tag_pieces(cx);let mut
tag_check_plus_match=self.call_substructure_method(cx,trait_,type_ident,//{();};
nonselflike_args,&EnumTag(tag_field,Some(get_match_expr(selflike_args))),);({});
...
Expr>],)->BlockOrExpr{{;};let summary=enum_def.variants.iter().map(|v|{let sp=v.
span.with_ctxt(trait_.span.ctxt());();let summary=trait_.summarise_struct(cx,&v.
...
StaticFields{3;let mut named_idents=Vec::new();let mut just_spans=Vec::new();for
field in struct_def.fields(){({});let sp=field.span.with_ctxt(self.span.ctxt());
...
by_ref,Mutability::Not),path,None,),),)});3;let struct_path=struct_path.clone();
...
);3;cx.pat_struct(self.span,struct_path,field_pats)}VariantData::Tuple(..)=>{let
...
struct_field.span.with_ctxt(self.span.ctxt());3;let mut exprs:Vec<_>=mk_exprs(i,
struct_field,sp);3;let self_expr=exprs.remove(0);let other_selflike_exprs=exprs;
...
"string")}else{None};let _=();if true{};if let Some(ty)=exception{cx.sess.psess.
...
::BuiltinLintDiag::ByteSliceInPackedStructWithDerive,);({});}else{field_expr=cx.
...
all_fields.split_first().unwrap()}else{all_fields.split_last().unwrap()};{;};let
base_expr=f(cx,CsFold::Single(base_field));{;};let op=|old,field:&FieldInfo|{let
new=f(cx,CsFold::Single(field));();f(cx,CsFold::Combine(field.span,old,new))};if
