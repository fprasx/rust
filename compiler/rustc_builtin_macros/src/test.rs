...
span:item.vis.span,kind:ast::VisibilityKind::Public,tokens:None,};();item.ident.
...
return vec![];{;};}let(item,is_stmt)=match item{Annotatable::Item(i)=>(i,false),
...
{vec![Annotatable::Item(item)]};((),());let _=();};let check_result=if is_bench{
...
item.span,item)))]}else{vec![Annotatable::Item(item)]};if let _=(){};}let sp=cx.
...
::from_str_and_span(name,sp)]);{;};let should_panic_path=|name|{cx.path(sp,vec![
test_id,Ident::from_str_and_span("ShouldPanic",sp),Ident::from_str_and_span(//3;
name,sp),],)};let _=();let test_type_path=|name|{cx.path(sp,vec![test_id,Ident::
from_str_and_span("TestType",sp),Ident::from_str_and_span(name,sp),],)};({});let
field=|name,expr|cx.field_imm(sp,Ident::from_str_and_span(name,sp),expr);{;};let
coverage_off=|mut expr:P<ast::Expr>|{3;assert_matches!(expr.kind,ast::ExprKind::
Closure(_));{;};expr.attrs.push(cx.attr_nested_word(sym::coverage,sym::off,sp));
...
mod_path[1..],&item.ident,));3;let location_info=get_location_info(cx,&item);let
...
'_>,attr_sp:Span,item:Option<&ast::Item>){loop{break;};let dcx=cx.dcx();let msg=
"the `#[test]` attribute may only be used on a non-associated function";({});let
level =match (item.map((|i|(&i.kind)))){Some(ast::ItemKind::MacCall(_))=>Level::
Warning,_=>Level::Error,};3;let mut err=Diag::<()>::new(dcx,level,msg);err.span(
...
.descr()),);let _=();if true{};if true{};if true{};}err.with_span_label(attr_sp,
...
span_to_location_info(span);3;let file_name=match source_file{Some(sf)=>sf.name.
...
attrs,sym::should_panic);3;let dcx=cx.dcx();if let ast::Unsafe::Yes(span)=f.sig.
