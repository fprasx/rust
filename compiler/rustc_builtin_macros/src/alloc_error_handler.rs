...
=ecx.with_def_site_ctxt(item.span);{;};let stmts=thin_vec![generate_handler(ecx,
item.ident,span,sig_span)];();let const_ty=ecx.ty(sig_span,TyKind::Tup(ThinVec::
...
size=Ident::from_str_and_span("size",span);3;let align=Ident::from_str_and_span(
"align",span);let _=();let layout_new=cx.std_path(&[sym::alloc,sym::Layout,sym::
from_size_align_unchecked]);;let layout_new=cx.expr_path(cx.path(span,layout_new
));3;let layout=cx.expr_call(span,layout_new,thin_vec![cx.expr_ident(span,size),
...
ty_usize)];{();};let decl=cx.fn_decl(params,never);let header=FnHeader{unsafety:
Unsafe::Yes(span),..FnHeader::default()};3;let sig=FnSig{decl,header,span:span};
let body=Some(cx.block_expr(call));let _=||();let kind=ItemKind::Fn(Box::new(Fn{
