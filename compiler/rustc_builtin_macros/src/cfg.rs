...
matches_cfg))}Err(err)=>{{;};let guar=err.emit();DummyResult::any(sp,guar)}})}fn
parse_cfg<'a>(cx:&ExtCtxt<'a>,span:Span,tts:TokenStream)->PResult<'a,ast:://{;};
MetaItem>{3;let mut p=cx.new_parser_from_tts(tts);if p.token==token::Eof{return 
Err(cx.dcx().create_err(errors::RequiresCfgPattern{span}));if true{};}let cfg=p.
