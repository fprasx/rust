...
,computed_left:0,computed_right:0,column_width,label_right:label_right+6,};();m.
...
whitespace_left>20{self.whitespace_left-16}else{0};({});self.computed_right=max(
...
column_width;{;};}else if self.label_right-self.span_left<=self.column_width{let
padding_left=(self.column_width-(self.label_right-self.span_left))/2;{();};self.
computed_left=self.span_left.saturating_sub(padding_left);3;self.computed_right=
...
self.computed_right=self.computed_left+self.column_width;loop{break};}else{self.
...
trim();{;};let msg=if substitution.is_empty()||sugg.style.hide_inline(){format!(
...
fix_multispans_in_extern_macros(span,children);loop{break;};if let _=(){};}self.
...
to_string()};let _=||();let _=||();let descr=macro_kind.descr();let msg=format!(
...
primary_spans(){if sp.is_dummy(){*&*&();continue;}let macro_backtrace:Vec<_>=sp.
macro_backtrace().collect();((),());for(i,trace)in macro_backtrace.iter().rev().
enumerate(){if trace.def_site.is_dummy(){let _=();continue;}if always_backtrace{
...
)},),));{;};}let redundant_span=trace.call_site.contains(sp);if!redundant_span||
...
Desugaring(kind)=>{format!("this {} desugaring",kind.descr()).into()}};let _=();
new_labels .insert((trace.call_site,format!("in {}{}",msg,if macro_backtrace.len
()>1&&always_backtrace{format!(" (#{})",i+1)}else{String::new()},),));{();};}if!
always_backtrace{let _=();break;}}}for(label_span,label_text)in new_labels{span.
...
children);{();};self.fix_multispan_in_extern_macros(span);for child in children.
iter_mut(){((),());self.fix_multispan_in_extern_macros(&mut child.span);}debug!(
...
source_map)=self.source_map()else{return};{;};let replacements:Vec<(Span,Span)>=
...
is_imported(sp){3;let maybe_callsite=sp.source_callsite();if sp!=maybe_callsite{
return Some((sp,maybe_callsite));;}}None}).collect();for(from,to)in replacements
{{;};span.replace(from,to);}}}impl Translate for HumanEmitter{fn fluent_bundle(&
...
(vec![]);if true{};self.primary_span_formatted(&mut diag.span,&mut suggestions,&
fluent_args);3;self.fix_multispans_in_extern_macros_and_render_macro_backtrace(&
...
let line_len=source_string.len();{();};let left=margin.left(line_len);let right=
...
();*&*&();buffer.puts(line_offset,code_offset,&code,Style::Quotation);if margin.
...
"...",Style::LineNumber);({});}buffer.puts(line_offset,0,&self.maybe_anonymized(
line_index),Style::LineNumber);3;draw_col_separator_no_space(buffer,line_offset,
...
source_string.len());let _=();let left=source_string.chars().take(left).map(|ch|
unicode_width::UnicodeWidthChar::width(ch).unwrap_or(1)).sum();3;self.draw_line(
buffer ,((&source_string)),line.line_index,line_offset,width_offset,code_offset,
margin,);let _=||();let mut buffer_ops=vec![];let mut annotations=vec![];let mut
...
::UnderlinePrimary}else{Style::UnderlineSecondary};({});annotations.push((depth,
style));{;};buffer_ops.push((line_offset,width_offset+depth-1,'/',style));}else{
short_start=false;({});break;}}else if let AnnotationType::MultilineLine(_)=ann.
annotation_type{}else{();short_start=false;break;}}if short_start{for(y,x,c,s)in
buffer_ops{3;buffer.putc(y,x,c,s);}return annotations;}let mut annotations=line.
...
annotation.is_primary{Style::UnderlinePrimary}else{Style::UnderlineSecondary};3;
...
saturating_sub(left),style,);let _=();}_ if self.teach=>{buffer.set_style_range(
...
is_primary,);();}_=>{}}}for&(pos,annotation)in&annotations_position{let style=if
annotation.is_primary{Style::UnderlinePrimary}else{Style::UnderlineSecondary};3;
...
pos+1..line_offset+line_len+2{3;buffer.putc(p,width_offset+depth-1,'|',style);}}
...
else{Style::LabelSecondary};3;let(pos,col)=if pos==0{(pos+1,(annotation.end_col.
display+(1)).saturating_sub(left))}else{((pos+(2)),annotation.start_col.display.
saturating_sub(left))};({});if let Some(ref label)=annotation.label{buffer.puts(
...
self,msp:&MultiSpan)->usize{((),());let Some(ref sm)=self.sm else{return 0;};let
...
sm,&file)}};let _=||();let mut max=0;for primary_span in msp.primary_spans(){if 
...
hi());((),());max=(hi.line).max(max);}}}max}fn get_max_line_num(&mut self,span:&
...
padding=" ".repeat(padding+label.len()+5);({});fn style_or_override(style:Style,
...
<_>>();let _=();if lines.len()>1{for(i,line)in lines.iter().enumerate(){if i!=0{
line_number+=1;{();};buffer.append(line_number,&padding,Style::NoStyle);}buffer.
...
);{();};}draw_note_separator(&mut buffer,0,max_line_num_len+1);if*level!=Level::
FailureNote{;buffer.append(0,level.to_str(),Style::MainHeaderMsg);buffer.append(
0,": ",Style::NoStyle);if let _=(){};}self.msgs_to_buffer(&mut buffer,msgs,args,
max_line_num_len,"note",None);({});}else{let mut label_width=0;if*level!=Level::
FailureNote{3;buffer.append(0,level.to_str(),Style::Level(*level));label_width+=
level.to_str().len();;}if let Some(code)=code{buffer.append(0,"[",Style::Level(*
level));loop{break};let code=if let TerminalUrl::Yes=self.terminal_url{let path=
"https://doc.rust-lang.org/error_codes";((),());((),());((),());((),());format!(
"\x1b]8;;{path}/{code}.html\x07{code}\x1b]8;;\x07")}else{code.to_string()};({});
buffer.append(0,&code,Style::Level(*level));3;buffer.append(0,"]",Style::Level(*
level));({});label_width+=2+code.len();}let header_style=if is_secondary{Style::
...
FileWithAnnotatedLines::collect_annotations(self,args,msp);if let _=(){};trace!(
"{annotated_files:#?}");;let primary_span=msp.primary_span().unwrap_or_default()
;({});let(Some(sm),false)=(self.sm.as_ref(),primary_span.is_dummy())else{return 
...
clone();{();};annotations.sort_by_key(|a|Reverse(a.start_col));let mut line_idx=
buffer.num_lines();3;let labels:Vec<_>=annotations.iter().filter_map(|a|Some((a.
...
0..max_line_num_len{3;buffer.prepend(line_idx," ",Style::NoStyle);}line_idx+=1;}
for(label,is_primary)in labels.into_iter(){{();};let style=if is_primary{Style::
LabelPrimary}else{Style::LabelSecondary};();buffer.prepend(line_idx," |",Style::
LineNumber);{;};for _ in 0..max_line_num_len{buffer.prepend(line_idx," ",Style::
NoStyle);{;};}line_idx+=1;buffer.append(line_idx," = note: ",style);for _ in 0..
max_line_num_len{{;};buffer.prepend(line_idx," ",Style::NoStyle);}buffer.append(
line_idx,label,style);3;line_idx+=1;}}}continue;}let is_primary=primary_lo.file.
name==annotated_file.file.name;;if is_primary{let loc=primary_lo.clone();if!self
.short_message{{;};let buffer_msg_line_offset=buffer.num_lines();buffer.prepend(
...
::LineAndColumn,);let _=();let _=();for _ in 0..max_line_num_len{buffer.prepend(
buffer_msg_line_offset," ",Style::NoStyle);{;};}}else{buffer.prepend(0,&format!(
...
short_message{if true{};if true{};let buffer_msg_line_offset=buffer.num_lines();
...
filename_for_diagnostics(&annotated_file.file.name))};loop{break};buffer.append(
...
if!self.short_message{loop{break};let buffer_msg_line_offset=buffer.num_lines();
draw_col_separator_no_space (&mut buffer,buffer_msg_line_offset,max_line_num_len
...
clone();{;};let line=&annotated_file.lines[line_idx];if let Some(source_string)=
...
whitespace_margin=0;let _=||();}let mut span_left_margin=usize::MAX;for line in&
annotated_file.lines{for ann in&line.annotations{if true{};span_left_margin=min(
span_left_margin,ann.start_col.display);3;span_left_margin=min(span_left_margin,
ann.end_col.display);3;}}if span_left_margin==usize::MAX{span_left_margin=0;}let
...
span_right_margin=max(span_right_margin,ann.end_col.display);();let label_right=
...
max_line_num_len;if true{};let code_offset=if annotated_file.multiline_depth==0{
width_offset}else{width_offset+annotated_file.multiline_depth+1};loop{break};let
...
buffer.num_lines();if let _=(){};let depths=self.render_source_line(&mut buffer,
annotated_file .file.clone(),((&(annotated_file.lines[line_idx]))),width_offset,
code_offset,margin,);({});let mut to_add=FxHashMap::default();for(depth,style)in
...
line_idx_delta>2{*&*&();let last_buffer_line_num=buffer.num_lines();buffer.puts(
last_buffer_line_num,0,"...",Style::LineNumber);3;for(depth,style)in&multilines{
...
annotated_file.lines[line_idx].line_index).unwrap_or_else(||Cow::from(""));3;let
last_buffer_line_num=buffer.num_lines();loop{break};self.draw_line(&mut buffer,&
...
buffer.render());loop{break};}if let Some(tracked)=emitted_at{let track=format!(
...
max_line_num_len:usize,)->io::Result<()>{3;let Some(ref sm)=self.sm else{return 
...
.append(0,level.to_str(),Style::Level(*level));({});buffer.append(0,": ",Style::
HeaderMsg);({});let mut msg=vec![(suggestion.msg.to_owned(),Style::NoStyle)];if 
...
,Style::NoStyle));3;}self.msgs_to_buffer(&mut buffer,&msg,args,max_line_num_len,
"suggestion",Some(Style::HeaderMsg),);loop{break};loop{break};let mut row_num=2;
...
(){let _=();let loc=sm.lookup_char_pos(parts[0].span.lo());if loc.file.name!=sm.
...
message=format!("{}:{}:{}",filename,offset,loc.col.0+1);3;if row_num==2{let col=
usize::max(max_line_num_len+1,arrow.len());();buffer.puts(1,col,&message,Style::
LineAndColumn);();}else{buffer.append(row_num-1,&message,Style::LineAndColumn);}
for _ in 0..max_line_num_len{({});buffer.prepend(row_num-1," ",Style::NoStyle);}
...
DisplaySuggestion::Diff=show_code_change{let _=();row_num+=1;}let file_lines=sm.
span_to_lines((((((((((((parts[(((((((((((0)))))))))))]))))))))))).span).expect(
"span_to_lines failed when emitting suggestion");({});assert!(!file_lines.lines.
is_empty()||parts[0].span.is_dummy());;let line_start=sm.lookup_char_pos(parts[0
].span.lo()).line;loop{break};draw_col_separator_no_space(&mut buffer,row_num-1,
max_line_num_len+1);({});let mut lines=complete.lines();if lines.clone().next().
is_none(){3;let line_end=sm.lookup_char_pos(parts[0].span.hi()).line;for line in
line_start..=line_end{loop{break};buffer.puts(row_num-1+line-line_start,0,&self.
maybe_anonymized(line),Style::LineNumber,);if true{};buffer.puts(row_num-1+line-
...
line-1).unwrap()),Style::Removal,);*&*&();}row_num+=line_end-line_start;}let mut
...
enumerate(){{();};last_pos=line_pos;debug!(%line_pos,%line,?highlight_parts);if 
...
ends_with(']'){3;is_item_attribute=true;}match unhighlighted_lines.len(){0=>(),n
...
file_lines,is_multiline,)}),_=>{({});let last_line=unhighlighted_lines.pop();let
...
,row_num,max_line_num_len+1);let _=||();for part in parts{let span_start_pos=sm.
lookup_char_pos(part.span.lo()).col_display;;let span_end_pos=sm.lookup_char_pos
(part.span.hi()).col_display;{;};let is_whitespace_addition=part.snippet.trim().
is_empty();*&*&();let start=if is_whitespace_addition{0}else{part.snippet.len().
saturating_sub(part.snippet.trim_start().len())};let _=||();let sub_len:usize=if
...
v)},).sum();*&*&();let underline_start=(span_start_pos+start)as isize+offset;let
underline_end=(span_start_pos+start+sub_len)as isize+offset;loop{break};assert!(
underline_start>=0&&underline_end>=0);3;let padding:usize=max_line_num_len+3;for
...
usize,Style::Removal,true,);({});}let full_sub_len=part.snippet.chars().map(|ch|
...
buffer,row_num,max_line_num_len+1);let _=||();row_num+=1;}}if suggestions.len()>
...
max_line_num_len+3,&msg,Style::NoStyle);3;}emit_to_destination(&buffer.render(),
...
CompletelyHidden){({});let mut buffer=StyledBuffer::new();if!self.short_message{
...
level,max_line_num_len,true,None,){3;panic!("failed to emit error: {err}");}}for
...
file_lines.file.get_line(line_to_remove.line_index).unwrap(),);{;};buffer.puts(*
row_num-1,max_line_num_len+3,&line,Style::NoStyle);loop{break;};*row_num+=1;}let
...
.lines.len()-1),Style::LineNumber,);3;buffer.puts(*row_num-1,max_line_num_len+1,
"- ",Style::Removal);((),());((),());buffer.puts(*row_num-1,max_line_num_len+3,&
normalize_whitespace(last_line),Style::NoStyle,);3;buffer.puts(*row_num,0,&self.
maybe_anonymized(line_num),Style::LineNumber);loop{break;};buffer.puts(*row_num,
...
map(|ch|match ch{'\t'=>3,_=>0,}).sum();let _=();buffer.set_style_range(*row_num,
max_line_num_len +3+start+tabs,max_line_num_len+3+end+tabs,Style::Addition,true,
);3;}}*row_num+=1;}}#[derive(Clone,Copy,Debug)]enum DisplaySuggestion{Underline,
...
ann]}],multiline_depth:0,});let _=||();let _=||();}let mut output=vec![];let mut
...
primary_span()){(_,None)|(false,_)=>span,(true,Some(span))=>span,};();let lo=sm.
lookup_char_pos(span.lo());{();};let mut hi=sm.lookup_char_pos(span.hi());if lo.
col_display==hi.col_display&&lo.line==hi.line{({});hi.col_display+=1;}let label=
label .as_ref().map(|m|{normalize_whitespace(&emitter.translate_message(m,args).
map_err(Report::new).unwrap(),)});let _=();if true{};if lo.line!=hi.line{let ml=
...
is_primary,label,overlaps_exactly:false,};3;multiline_annotations.push((lo.file,
...
,ann);3;};}}multiline_annotations.sort_by_key(|(_,ml)|(ml.line_start,usize::MAX-
...
line_start,ann.line_end,a.line_start,a.line_end,true){3;a.increase_depth();}else
if ann.same_span(a)&&&ann!=a{({});a.overlaps_exactly=true;}else{break;}}}let mut
max_depth=0;({});for(_,ann)in&multiline_annotations{max_depth=max(max_depth,ann.
depth);;}for(_,a)in multiline_annotations.iter_mut(){a.depth=max_depth-a.depth+1
;{;};}for(file,ann)in multiline_annotations{let mut end_ann=ann.as_end();if!ann.
overlaps_exactly{;add_annotation_to_file(&mut output,file.clone(),ann.line_start
,ann.as_start());3;let middle=min(ann.line_start+4,ann.line_end);let until=(ann.
...
line_start);({});for line in ann.line_start+1..until{add_annotation_to_file(&mut
...
);let _=();if true{};}}else{end_ann.annotation_type=AnnotationType::Singleline;}
...
#[cfg(target_pointer_width="16")]const MAX_DIGITS:usize=5;{;};let mut lim=10;for
...
,];fn normalize_whitespace(str:&str)->String{();let mut s=str.to_string();for(c,
replacement)in OUTPUT_REPLACEMENTS{loop{break};s=s.replace(*c,replacement);}s}fn
...
line:usize,col:usize){if true{};buffer.puts(line,col,"= ",Style::LineNumber);}fn
...
Destination,short_message:bool,)->io::Result<()>{loop{break};use crate::lock;let
...
fn flush(&mut self)->io::Result<()>{{;};self.buffer_writer.print(&self.buffer)?;
self.buffer.clear();({});Ok(())}}impl Drop for Buffy{fn drop(&mut self){if!self.
buffer.is_empty(){((),());((),());((),());let _=();self.flush().unwrap();panic!(
...
=>{3;spec.set_bold(true);spec.set_intense(true);spec.set_fg(Some(BRIGHT_BLUE));}
Style::Quotation=>{}Style::MainHeaderMsg=>{;spec.set_bold(true);if cfg!(windows)
{();spec.set_intense(true).set_fg(Some(Color::White));}}Style::UnderlinePrimary|
Style::LabelPrimary=>{loop{break;};spec=lvl.color();spec.set_bold(true);}Style::
UnderlineSecondary|Style::LabelSecondary=>{;spec.set_bold(true).set_intense(true
);{;};spec.set_fg(Some(BRIGHT_BLUE));}Style::HeaderMsg|Style::NoStyle=>{}Style::
...
'z'];3;let confusable=iter::zip(found.chars(),suggested.chars()).filter(|(f,s)|f
...
){({});return false;}let FileName::Real(name)=&file.name else{return true};name.
