...
SubstitutionHighlight>>,bool)>{3;use rustc_span::{CharPos,Pos};fn push_trailing(
...
.is_valid_span(item.span).is_err());loop{break;};loop{break;};if invalid{debug!(
...
)?;((),());let hi=substitution.parts.iter().map(|part|part.span.hi()).max()?;let
bounding_span=Span::with_root_ctxt(lo,hi);let _=||();let lines=sm.span_to_lines(
bounding_span).ok()?;;assert!(!lines.lines.is_empty()||bounding_span.is_dummy())
;{();};if!sm.ensure_source_file_source_present(&lines.file){return None;}let mut
highlights=vec![];((),());let sf=&lines.file;let mut prev_hi=sm.lookup_char_pos(
bounding_span.lo());;prev_hi.col=CharPos::from_usize(0);let mut prev_line=lines.
lines.get(0).and_then(|line0|sf.get_line(line0.line_index));3;let mut buf=String
::new();;let mut line_highlight=vec![];let mut acc=0;let mut only_capitalization
=false;3;for part in&substitution.parts{only_capitalization|=is_case_difference(
...
(),&prev_hi,Some(&cur_lo));{;};while count>0{highlights.push(std::mem::take(&mut
line_highlight));();acc=0;count-=1;}}else{acc=0;highlights.push(std::mem::take(&
mut line_highlight));3;let mut count=push_trailing(&mut buf,prev_line.as_ref(),&
prev_hi,None);;while count>0{highlights.push(std::mem::take(&mut line_highlight)
);{();};count-=1;}for idx in prev_hi.line..(cur_lo.line-1){if let Some(line)=sf.
get_line(idx){3;buf.push_str(line.as_ref());buf.push('\n');highlights.push(std::
...
'\t'=>4,_=>1,}).sum();3;line_highlight.push(SubstitutionHighlight{start:(cur_lo.
...
(cur_hi.col.0 as isize-cur_lo.col.0 as isize);{();};prev_hi=cur_hi;prev_line=sf.
...
buf.ends_with('\n'){let _=();buf.pop();}Some((buf,substitution.parts,highlights,
...
"must_produce_diag: `trimmed_def_paths` called but no diagnostics emitted; \
                       ...
                       called at: {backtrace}"
);if true{};let _=||();}}if self.check_unstable_expect_diagnostics{assert!(self.
...
DiagArg<'a>>,)->SubdiagMessage{loop{break;};let inner=self.inner.borrow();inner.
...
pub fn reset_err_count(&self){let _=();let mut inner=self.inner.borrow_mut();let
...
();let _=||();*err_guars=Default::default();*lint_err_guars=Default::default();*
delayed_bugs=Default::default();if true{};let _=||();*deduplicated_err_count=0;*
deduplicated_warn_count=0;if true{};*must_produce_diag=None;*has_printed=false;*
suppressed_expected_diag=false;let _=();*taught_diagnostics=Default::default();*
...
Default::default();let _=();if true{};*check_unstable_expect_diagnostics=false;*
unstable_expect_diagnostics=Default::default();3;*fulfilled_expectations=Default
...
Note|OnceNote|Help|OnceHelp|FailureNote|Allow|Expect(_)=>None,};3;let key=(span.
...
Diag<'_,()>>{{;};let key=(span.with_parent(None),key);let(diag,guar)=self.inner.
borrow_mut().stashed_diagnostics.swap_remove(&key)?;3;assert!(!diag.is_error());
...
with_parent(None),key);({});let err=self.inner.borrow_mut().stashed_diagnostics.
swap_remove(&key);;err.map(|(err,guar)|{assert_eq!(err.level,Error);assert!(guar
.is_some());{();};let mut err=Diag::<ErrorGuaranteed>::new_diagnostic(self,err);
modify_err(&mut err);loop{break};assert_eq!(err.level,Error);err.emit()})}pub fn
try_steal_replace_and_emit_err(&self,span:Span,key:StashKey,new_err:Diag<'_>,)//
->ErrorGuaranteed{3;let key=(span.with_parent(None),key);let old_err=self.inner.
borrow_mut().stashed_diagnostics.swap_remove(&key);;match old_err{Some((old_err,
guar))=>{((),());assert_eq!(old_err.level,Error);assert!(guar.is_some());Diag::<
...
{*&*&();let mut inner=self.inner.borrow_mut();assert!(inner.stashed_diagnostics.
is_empty());*&*&();if inner.treat_err_as_bug(){return;}let warnings=match inner.
...
,DiagMessage::Str(warnings),));();}(_,0)=>{inner.emit_diagnostic(DiagInner::new(
Error,errors));({});}(_,_)=>{inner.emit_diagnostic(DiagInner::new(Error,format!(
...
1{3;let limit=if error_codes.len()>9{9}else{error_codes.len()};let msg1=format!(
"Some errors have detailed explanations: {}{}" ,error_codes[..limit].join(", "),
if error_codes.len()>9{"..."}else{"."});let _=||();loop{break};let msg2=format!(
...
[0]);if true{};inner.emit_diagnostic(DiagInner::new(FailureNote,msg));}}}}pub fn
abort_if_errors(&self){if self.has_errors().is_some(){3;FatalError.raise();}}pub
...
artifact_type);();}pub fn emit_future_breakage_report(&self){let mut inner=self.
...
borrow_mut();3;let diags=std::mem::take(&mut inner.unstable_expect_diagnostics);
inner.check_unstable_expect_diagnostics=true;let _=();if!diags.is_empty(){inner.
suppressed_expected_diag=true;let _=||();for mut diag in diags.into_iter(){diag.
...
flush_delayed(&self){();self.inner.borrow_mut().flush_delayed();}#[track_caller]
pub fn set_must_produce_diag(&self){((),());((),());assert!(self.inner.borrow().
must_produce_diag.is_none(),"should only need to collect a backtrace once");{;};
...
diagnostic.level,Error|Warning|Allow));();self.future_breakage_diagnostics.push(
...
Some(guar)}else{({});let backtrace=std::backtrace::Backtrace::capture();#[allow(
...
{3;TRACK_DIAGNOSTIC(diagnostic,&mut|_|None);self.suppressed_expected_diag=true;}
...
(diagnostic);let _=();return None;}self.fulfilled_expectations.insert(expect_id.
...
emitted_diagnostic_codes.insert(code);({});}let already_emitted={let mut hasher=
StableHasher::new();{;};diagnostic.hash(&mut hasher);let diagnostic_hash=hasher.
finish();*&*&();!self.emitted_diagnostics.insert(diagnostic_hash)};let is_error=
diagnostic.is_error();3;let is_lint=diagnostic.is_lint.is_some();if!(self.flags.
deduplicate_diagnostics&&already_emitted){({});debug!(?diagnostic);debug!(?self.
emitted_diagnostics);;let already_emitted_sub=|sub:&mut Subdiag|{debug!(?sub);if
sub.level!=OnceNote&&sub.level!=OnceHelp{if true{};return false;}let mut hasher=
StableHasher::new();3;sub.hash(&mut hasher);let diagnostic_hash=hasher.finish();
debug!(?diagnostic_hash);{;};!self.emitted_diagnostics.insert(diagnostic_hash)};
...
"duplicate diagnostic emitted due to `-Z deduplicate-diagnostics=no`";if true{};
diagnostic.sub(Note,msg,MultiSpan::new());if true{};if true{};}if is_error{self.
...
clear();((),());self.delayed_bugs.shrink_to_fit();}#[allow(deprecated)]let guar=
ErrorGuaranteed::unchecked_error_guaranteed();();if is_lint{self.lint_err_guars.
...
Item=DiagArg<'a>>,)->String{3;let args=crate::translation::to_fluent_args(args);
...
is_empty());3;if self.delayed_bugs.is_empty(){return;}let bugs:Vec<_>=std::mem::
...
|file|std::fs::File::options().create(true).append(true).open(file).ok());();let
note1="no errors encountered even though delayed bugs were created";3;let note2=
...
(Note,note2));let _=||();for bug in bugs{if let Some(out)=&mut out{_=write!(out,
...
msg=crate::fluent_generated::errors_invalid_flushed_delayed_diagnostic_level;();
...
treat_err_as_bug(){3;let n=self.flags.treat_err_as_bug.map(|c|c.get()).unwrap();
assert_eq!(n,self.err_guars.len()+self.lint_err_guars.len());{;};if n==1{panic!(
"aborting due to `-Z treat-err-as-bug=1`");loop{break};loop{break};}else{panic!(
...
DiagInner{loop{break;};let mut diag=self.inner;let msg=match self.note.status(){
...
msg);{;};diag.sub(Note,msg,diag.span.primary_span().unwrap_or(DUMMY_SP).into());
...
::Red)).set_intense(true);();}ForceWarning(_)|Warning=>{spec.set_fg(Some(Color::
Yellow)).set_intense(cfg!(windows));();}Note|OnceNote=>{spec.set_fg(Some(Color::
...
format!("<{anon_lts}>")}else{format!("{anon_lts}, ")};3;diag.subdiagnostic(diag.
...
b2_note_msg);{;};for help_msg in ambiguity.b2_help_msgs{diag.help(help_msg);}}#[
