...
TranslateErrorKind::*;if true{};match self{Self::One{id,args,kind}=>{writeln!(f,
...
(f,"help: try using `{{${id}}}` instead")?;if true{};if true{};}else{writeln!(f,
...
fmt(fallback,f)?;if true{};let _=||();}Self::Two{primary,fallback}=>{writeln!(f,
