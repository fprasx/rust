...
try_new(ftl.into()).expect("Failed to parse an FTL string.");({});let langid_en=
langid!("en-US");if true{};#[cfg(parallel_compiler)]let mut bundle:FluentBundle=
...
);if true{};let mut args=FluentArgs::new();args.set("name","Foo");args.set("ty",
"std::string::String");if let _=(){};{let message=DiagMessage::FluentIdentifier(
...
"borrow this binding in the pattern to avoid moving the value");3;}{let message=
DiagMessage::FluentIdentifier((("mir_build_borrow_of_moved_value").into()),Some(
"value_borrowed_label".into()),);3;assert_eq!(dummy.translate_message(&message,&
args).unwrap(),"value borrowed here after move");{;};}{let message=DiagMessage::
FluentIdentifier ((((((((("mir_build_borrow_of_moved_value")))).into())))),Some(
"occurs_because_label".into()),);3;assert_eq!(dummy.translate_message(&message,&
args).unwrap(),//*&*&();((),());((),());((),());((),());((),());((),());((),());
...
unwrap_err();();if let TranslateError::Two{primary:box TranslateError::One{kind:
...
message,&args).unwrap_err();if let _=(){};if let TranslateError::Two{primary:box
...
"{err:#?}")};let _=();if true{};if true{};if true{};assert_eq!(format!("{err}"),
