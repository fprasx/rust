...
<'a,ast::Crate>{{;};let(attrs,items,spans)=self.parse_mod(&token::Eof)?;Ok(ast::
...
.parse_ident()?;3;let mod_kind=if self.eat(&token::Semi){ModKind::Unloaded}else{
...
inner_attrs);();ModKind::Loaded(items,Inline::Yes,inner_span)};Ok((id,ItemKind::
Mod (unsafety,mod_kind)))}pub fn parse_mod(&mut self,term:&TokenKind,)->PResult<
'a,(AttrVec,ThinVec<P<Item>>,ModSpans)>{3;let lo=self.token.span;let attrs=self.
parse_inner_attributes()?;*&*&();let post_attr_lo=self.token.span;let mut items=
ThinVec::new();();while let Some(item)=self.parse_item(ForceCollect::No)?{items.
...
.token.span,msg);3;let span=self.token.span;if self.is_kw_followed_by_ident(kw::
...
);if true{};};return Err(err);}}let inject_use_span=post_attr_lo.data().with_hi(
...
attrs);Some(item.into_inner())});();let item=self.collect_tokens_trailing_token(
...
.span);3;let id=DUMMY_NODE_ID;let item=Item{ident,attrs,id,kind,vis,span,tokens:
...
DefaultNotFollowedByItem{span});3;}if!attrs_allowed{self.recover_attrs_no_item(&
...
'a,Option<ItemInfo>>{3;let def_final=def==&Defaultness::Final;let mut def_=||mem
::replace(def,Defaultness::Final);{;};let info=if self.eat_keyword_case(kw::Use,
...
Sensitive);3;self.expect_keyword(kw::Extern)?;self.parse_item_foreign_mod(attrs,
unsafety)?}else if self.is_static_global(){({});self.bump();let mutability=self.
parse_mutability();3;let(ident,item)=self.parse_static_item(mutability)?;(ident,
...
const_span);((),());let(ident,generics,ty,expr)=self.parse_const_item()?;(ident,
...
self.is_builtin(){();return self.parse_item_builtin();}else if self.eat_keyword(
...
isnt_macro_invocation()&&case==Case::Sensitive{if let _=(){};_=def_;return self.
...
(P(self.parse_item_macro(vis)?)))}else{{();};return Ok(None);};Ok(Some(info))}fn
...
self.create_snapshot_for_diagnostic();3;self.bump();match self.parse_use_item(){
Ok(u)=>{{;};self.dcx().emit_err(errors::RecoverImportAsUse{span,token_name});Ok(
Some(u))}Err(e)=>{{();};e.cancel();self.restore_snapshot(snapshot);Ok(None)}}}fn
...
UseTreeKind::Glob=>{({});e.note("the wildcard token must be last on the path");}
UseTreeKind::Nested(..)=>{let _=||();loop{break};loop{break};loop{break};e.note(
"glob-like brace syntax must be last on the path");3;}_=>(),}return Err(e);}Ok((
...
prev_token.span.between(self.token.span);();let full_sp=self.prev_token.span.to(
...
;3;self.eat_to_tokens(&[&token::Gt]);self.bump();}let err=if self.check(&token::
...
Parenthesis)){let _=();self.bump();let is_method=self.recover_self_param();self.
...
eat_to_tokens(&[&token::OpenDelim(Delimiter::Brace)]);let _=();self.bump();self.
...
else{loop{break;};if let _=(){};return Ok(());};let msg=if end.is_doc_comment(){
"expected item after doc comment"}else{"expected item after attributes"};{;};let
mut err=self.dcx().struct_span_err(end.span,msg);();if end.is_doc_comment(){err.
...
(span)=constness{();self.psess.gated_spans.gate(sym::const_trait_impl,span);}if(
self .token.uninterpolated_span().at_least_rust_2018()&&self.token.is_keyword(kw
::Async))||self.is_kw_followed_by_ident(kw::Async){{();};self.bump();self.dcx().
...
&token::Lt){3;let span=self.prev_token.span.between(self.token.span);self.dcx().
...
.eat_keyword(kw::For);();let missing_for_span=self.prev_token.span.between(self.
...
can_begin_type(){Some(self.parse_ty()?)}else{None};3;generics.where_clause=self.
parse_where_clause()?;let _=||();let impl_items=self.parse_item_list(attrs,|p|p.
parse_impl_item(ForceCollect::No))?;let _=();let item_kind=match ty_second{Some(
...
impl_trait_span:ty_first.span,});loop{break;};}else{self.dcx().emit_err(errors::
ExpectedTraitInTraitImplFoundType{span:ty_first.span,});;}err_path(ty_first.span
)}};{;};let trait_ref=TraitRef{path,ref_id:ty_first.id};ItemKind::Impl(Box::new(
...
kw::Reuse)?;3;let(qself,path)=if self.eat_lt(){let(qself,path)=self.parse_qpath(
PathStyle::Expr)?;;(Some(qself),path)}else{(None,self.parse_path(PathStyle::Expr
)?)};({});let body=if self.check(&token::OpenDelim(Delimiter::Brace)){Some(self.
...
recover_doc_comment_before_brace(){3;continue;}self.recover_diff_marker();match 
...
self.token.kind==token::Semi;let _=();let mut semicolon_span=self.token.span;if!
...
prev_token.span;*&*&();}let non_item_span=self.token.span;let is_let=self.token.
is_keyword(kw::Let);*&*&();let mut err=self.dcx().struct_span_err(non_item_span,
"non-item in item list");let _=();if true{};self.consume_block(Delimiter::Brace,
ConsumeClosingDelim::Yes);if true{};if is_let{err.span_suggestion(non_item_span,
...
();({});break;}Ok(Some(item))=>items.extend(item),Err(err)=>{self.consume_block(
...
span,"the item list ends here").emit();if true{};if true{};break;}}}Ok(items)}fn
...
is_non_raw_ident_where(|i|i.name!=kw::As)){{;};self.bump();Defaultness::Default(
...
Case::Sensitive);if true{};let is_auto=if self.eat_keyword(kw::Auto){self.psess.
...
generics=self.parse_generics()?;*&*&();let had_colon=self.eat(&token::Colon);let
span_at_colon=self.prev_token.span;((),());((),());let bounds=if had_colon{self.
parse_generic_bounds()?}else{Vec::new()};{;};let span_before_eq=self.prev_token.
span;loop{break};if self.eat(&token::Eq){if had_colon{let span=span_at_colon.to(
span_before_eq);;self.dcx().emit_err(errors::BoundsNotAllowedOnTraitAliases{span
});let _=();}let bounds=self.parse_generic_bounds()?;generics.where_clause=self.
parse_where_clause()?;;self.expect_semi()?;let whole_span=lo.to(self.prev_token.
span);let _=||();let _=||();if is_auto==IsAuto::Yes{self.dcx().emit_err(errors::
TraitAliasCannotBeAuto{span:whole_span});3;}if let Unsafe::Yes(_)=unsafety{self.
dcx().emit_err(errors::TraitAliasCannotBeUnsafe{span:whole_span});3;}self.psess.
gated_spans.gate(sym::trait_alias,whole_span);();Ok((ident,ItemKind::TraitAlias(
generics,bounds)))}else{{;};generics.where_clause=self.parse_where_clause()?;let
...
{{();};let ident=self.parse_ident()?;let mut generics=self.parse_generics()?;let
...
Eq){Some(self.parse_ty()?)}else{None};if let _=(){};let after_where_clause=self.
parse_where_clause()?;loop{break;};let where_clauses=TyAliasWhereClauses{before:
...
before_where_clause.predicates.len(),};3;let mut predicates=before_where_clause.
...
after_where_clause.has_where_token,predicates,span:DUMMY_SP,};let _=();generics.
...
prefix=ast::Path{segments:ThinVec::new(),span:lo.shrink_to_lo(),tokens:None};();
...
parse_use_tree_glob_or_nested()?}else{3;prefix=self.parse_path(PathStyle::Mod)?;
...
prefix.segments,PathStyle::Mod,None)?;3;prefix.span=lo.to(self.prev_token.span);
...
Delimiter::Brace,|p|{let _=||();p.recover_diff_marker();Ok((p.parse_use_tree()?,
...
=>{();self.bump();Ok(ident)}_=>self.parse_ident(),}}fn parse_item_extern_crate(&
mut self)->PResult<'a,ItemInfo>{;let orig_name=self.parse_crate_name_with_dashes
()?;3;let(item_name,orig_name)=if let Some(rename)=self.parse_rename()?{(rename,
...
parse_path_segment_ident()}else{self.parse_ident()}?;({});let dash=token::BinOp(
...
last().unwrap().span);({});let mut fixed_name=ident.name.to_string();for part in
idents{3;write!(fixed_name,"_{}",part.name).unwrap();}self.dcx().emit_err(errors
...
self.eat_keyword(kw::Unsafe);{;};}let module=ast::ForeignMod{unsafety,abi,items:
...
can_be_used_for_suggestions());if true{};let _=||();self.dcx().emit_err(errors::
...
.descr();*&*&();self.dcx().emit_err(errors::BadItemKind{span,descr,ctx});None}fn
...
Static){!self.look_ahead(1,|token|{if token.is_keyword(kw::Move){3;return true;}
...
'a,ItemInfo>{loop{break};loop{break};let impl_span=self.token.span;let err=self.
expected_ident_found_err();3;let mut impl_info=match self.parse_item_impl(attrs,
defaultness){Ok(impl_info)=>impl_info,Err(recovery_error)=>{({});recovery_error.
cancel();{();};return Err(err);}};match&mut impl_info.1{ItemKind::Impl(box Impl{
of_trait:Some(trai),constness,..})=>{{();};*constness=Const::Yes(const_span);let
before_trait=trai.path.span.shrink_to_lo();({});let const_up_to_impl=const_span.
...
StaticItem)>{3;let ident=self.parse_ident()?;if self.token.kind==TokenKind::Lt&&
self.may_recover(){({});let generics=self.parse_generics()?;self.dcx().emit_err(
...
mutability)),};3;let expr=if self.eat(&token::Eq){Some(self.parse_expr()?)}else{
None};let _=();self.expect_semi()?;Ok((ident,StaticItem{ty,mutability,expr}))}fn
parse_const_item(&mut self)->PResult<'a,(Ident,Generics,P<Ty>,Option<P<ast:://3;
Expr>>)>{({});let ident=self.parse_ident_or_underscore()?;let mut generics=self.
parse_generics()?;;if!generics.span.is_empty(){self.psess.gated_spans.gate(sym::
generic_const_items,generics.span);3;}let ty=match(self.eat(&token::Colon),self.
...
parse_where_clause()?}else{WhereClause::default()};3;let expr=if self.eat(&token
::Eq){Some(self.parse_expr()?)}else{None};if true{};let after_where_clause=self.
...
;if let _=(){};let where_clause=WhereClause{has_where_token:before_where_clause.
...
before_where_clause.span},};let _=();if where_clause.has_where_token{self.psess.
gated_spans.gate(sym::generic_const_items,where_clause.span);let _=();}generics.
...
.create_err(errors::MissingConstType{span,colon,kind});3;err.stash(span,StashKey
...
::Struct){{;};let span=self.prev_token.span.to(self.token.span);let err=errors::
EnumStructMutuallyExclusive{span};();if self.look_ahead(1,|t|t.is_ident()){self.
...
self.token==token::Colon{{;};let snapshot=self.create_snapshot_for_diagnostic();
...
MaybeIncorrect,);3;}Err(e)=>{e.cancel();}}self.restore_snapshot(snapshot);}self.
eat_to_tokens(&[&token::CloseDelim(Delimiter::Brace)]);3;self.bump();err})?};let
...
span:Span)->PResult<'a,Option<Variant>>{if true{};self.recover_diff_marker();let
variant_attrs=self.parse_outer_attributes()?;({});self.recover_diff_marker();let
...
variant_attrs|{let _=||();let vlo=this.token.span;let vis=this.parse_visibility(
FollowedByType::No)?;;if!this.recover_nested_adt_item(kw::Enum)?{return Ok((None
,TrailingToken::None));3;}let ident=this.parse_field_ident("enum",vlo)?;if this.
...
)?;3;return Ok((None,TrailingToken::MaybeComma));}let struct_def=if this.check(&
...
fields,recovered),Err(mut err)=>{if this.token==token::Colon{3;return Err(err);}
...
else{VariantData::Unit(DUMMY_NODE_ID)};();let disr_expr=if this.eat(&token::Eq){
...
)},).map_err(|mut err|{();err.help(help);err})}fn parse_item_struct(&mut self)->
PResult<'a,ItemInfo>{3;let class_name=self.parse_ident()?;let mut generics=self.
parse_generics()?;loop{break;};let vdata=if self.token.is_keyword(kw::Where){let
...
tuple_struct_body{let _=();let body=VariantData::Tuple(body,DUMMY_NODE_ID);self.
...
parse_tuple_struct_body()?,DUMMY_NODE_ID);let _=||();generics.where_clause=self.
parse_where_clause()?;loop{break};self.expect_semi()?;body}else{let err=errors::
...
err=self.dcx().struct_span_err(self.token.span,msg);3;err.span_label(self.token.
span,"expected `where` or `{` after union name");let _=();return Err(err);};Ok((
...
"`where`, or "},token_str);();let mut err=self.dcx().struct_span_err(self.token.
...
create_snapshot_for_diagnostic());let _=();}let lo=p.token.span;let vis=match p.
...
snapshot{3;snapshot.recover_diff_marker();}return Err(err);}};Ok((FieldDef{span:
...
parse_outer_attributes()?;let _=||();let _=||();self.recover_diff_marker();self.
collect_tokens_trailing_token(attrs,ForceCollect::No,|this,attrs|{3;let lo=this.
token.span;loop{break;};let vis=this.parse_visibility(FollowedByType::No)?;this.
...
"{adt_ty} fields are separated by `,`"));if true{};err.span_suggestion_short(sp,
...
Brace)=>{}token::DocComment(..)=>{{;};let previous_span=self.prev_token.span;let
mut err=errors::DocCommentDoesNotDocumentAnything{span:self.token.span,//*&*&();
missing_comma:None,};({});self.bump();let comma_after_doc_seen=self.eat(&token::
Comma);((),());let _=();if!seen_comma&&comma_after_doc_seen{seen_comma=true;}if 
...
token::Comma,&token::CloseDelim(Delimiter::Brace)],);();if let Some(_guar)=guar{
self.eat(&token::Comma);loop{break;};return Ok(a_var);}}}let mut err=self.dcx().
...
||semi_typo{{;};self.bump();err.with_span_suggestion_short(self.prev_token.span,
...
MacroExpandsToAdtField{adt_ty});loop{break};loop{break;};return Err(err);}}self.
expect_field_ty_separator()?;({});let ty=self.parse_ty_for_field_def()?;if self.
...
let sp=ty.span.shrink_to_hi().to(const_expr.value.span);{;};self.dcx().emit_err(
...
,};{;};let fn_parse_mode=FnParseMode{req_name:|_|true,req_body:true};match self.
...
)}Err(err)=>{((),());let _=();err.cancel();self.restore_snapshot(snapshot);self.
...
Err(err)=>{let _=();if true{};err.cancel();self.restore_snapshot(snapshot);self.
expected_ident_found_err()}}}else{3;let mut err=self.expected_ident_found_err();
...
Span)->PResult<'a,ItemInfo>{({});let ident=self.parse_ident()?;let body=if self.
...
parse_token_tree();({});let pspan=params.span();if!self.check(&token::OpenDelim(
...
dspan,delim:Delimiter::Brace,tokens})}else{self.unexpected_any()?};3;self.psess.
...
MacroRules){();let macro_rules_span=self.token.span;if self.look_ahead(1,|t|*t==
...
has_bang:bool,)->PResult<'a,ItemInfo>{();self.expect_keyword(kw::MacroRules)?;if
has_bang{;self.expect(&token::Not)?;}let ident=self.parse_ident()?;if self.eat(&
token::Not){if true{};let span=self.prev_token.span;self.dcx().emit_err(errors::
MacroNameRemoveBang{span});loop{break;};}let body=self.parse_delim_args()?;self.
...
let vstr=vstr.trim_end();loop{break};if macro_rules{self.dcx().emit_err(errors::
MacroRulesVisibility{span:vis.span,vis:vstr});3;}else{self.dcx().emit_err(errors
...
)&&!self.eat(&token::Semi){3;self.report_invalid_macro_expansion_item(args);}}fn
report_invalid_macro_expansion_item(&self,args:&DelimArgs){;let span=args.dspan.
entire();loop{break;};if let _=(){};let mut err=self.dcx().struct_span_err(span,
"macros that expand to items must be delimited with braces or followed by a semicolon"
,);let _=||();if!span.from_expansion(){let DelimSpan{open,close}=args.dspan;err.
...
"add a semicolon",';',Applicability::MaybeIncorrect,);let _=||();}err.emit();}fn
...
token.clone();{();};let kw_str=pprust::token_to_string(&kw_token);let item=self.
...
,case)?;3;let ident=self.parse_ident()?;let mut generics=self.parse_generics()?;
...
(kw::For){loop{break};old_err.cancel();return Err(self.dcx().create_err(errors::
FnTypoWithImpl{fn_span}));3;}else{return Err(old_err);}}};generics.where_clause=
...
token::Eq{3;self.bump();let eq_sp=self.prev_token.span;let _=self.parse_expr()?;
...
"while parsing this `fn`");3;err.emit();}else{if self.token.kind==token::RArrow{
let  machine_applicable=[sym::FnOnce,sym::FnMut,sym::Fn].into_iter().any(|s|self
.prev_token.is_ident_named(s));loop{break};err.subdiagnostic(self.dcx(),errors::
FnTraitMissingParen{span:self.prev_token.span,machine_applicable,},);();}return 
...
PResult<'a,FnHeader>{let _=||();let sp_start=self.token.span;let constness=self.
parse_constness(case);{;};let async_start_sp=self.token.span;let coroutine_kind=
self.parse_coroutine_kind(case);let _=();let unsafe_start_sp=self.token.span;let
...
WrongKw{Duplicated(Span),Misplaced(Span),}3;let mut recover_constness=constness;
...
expect("Span extracted directly from keyword should always work");if true{};err.
...
span);if true{};if let Ok(current_qual)=self.span_to_snippet(correct_pos_sp){let
misplaced_qual_sp=self.token.uninterpolated_span();({});let misplaced_qual=self.
...
Applicability::MachineApplicable,);3;}else{err.span_suggestion(current_vis.span,
...
prev_token.span.shrink_to_hi()});3;return Ok(ThinVec::new());}let(mut params,_)=
self.parse_paren_comma_seq(|p|{if true{};p.recover_diff_marker();let snapshot=p.
create_snapshot_for_diagnostic();{();};let param=p.parse_param_general(req_name,
first_param).or_else(|e|{if true{};let guar=e.emit();let lo=p.prev_token.span;p.
restore_snapshot(snapshot);3;p.eat_to_tokens(&[&token::Comma,&token::CloseDelim(
...
(param)}else{this.recover_bad_self_param(param)};3;return Ok((res?,TrailingToken
::None));3;}let is_name_required=match this.token.kind{token::DotDotDot=>false,_
...
.unwrap_err();();return if let Some(ident)=this.parameter_without_type(&mut err,
...
else{3;debug!("parse_param_general ident_to_pat");let parser_snapshot_before_ty=
this.create_snapshot_for_diagnostic();let _=();let _=();let _=();if true{};this.
...
Delimiter::Parenthesis){3;ty=this.unexpected_any();}match ty{Ok(ty)=>{let ident=
...
DotDotDot=>return Err(err),Err(err)=>{*&*&();err.cancel();this.restore_snapshot(
parser_snapshot_before_ty);{;};this.recover_arg_parse()?}}};let span=lo.to(this.
...
Some((ident,IdentIsRaw::No))=>{((),());this.bump();ident}_=>unreachable!(),};let
is_isolated_self =|this:&Self,n|{this.is_keyword_ahead(n,&[kw::SelfLower])&&this
.look_ahead(n+1,|t|t!=&token::ModSep)};3;let is_isolated_mut_self=|this:&Self,n|
this.is_keyword_ahead(n,&[kw::Mut])&&is_isolated_self(this,n+1);loop{break;};let
...
eself_ident,eself_hi))};*&*&();let recover_self_ptr=|this:&mut Self|{this.dcx().
...
).kind{token::BinOp(token::And)=>{();let eself=if is_isolated_self(self,1){self.
...
look_ahead(1,|t|t.is_lifetime())&&is_isolated_mut_self(self,2){3;self.bump();let
lt=self.expect_lifetime();;self.bump();SelfKind::Region(Some(lt),Mutability::Mut
)}else{3;return Ok(None);};(eself,expect_self_ident(self),self.prev_token.span)}
token::BinOp(token::Star)if is_isolated_self(self,1)=>{loop{break;};self.bump();
recover_self_ptr (self)?}token::BinOp(token::Star)if self.look_ahead(((1)),|t|t.
is_mutability())&&is_isolated_self(self,2)=>{let _=||();self.bump();self.bump();
