...
num_bump_calls;let _=();if true{};loop{let attr=if self.check(&token::Pound){let
...
AfterOuterAttribute{prev_outer_attr_sp}})};if let _=(){};let inner_parse_policy=
...
Outer{({});let span=self.token.span;let mut err=self.dcx().struct_span_err(span,
fluent::parse_inner_doc_comment_not_permitted);({});err.code(E0753);if let Some(
...
{let _=();outer_attrs.push(attr);}}else{break;}}Ok(AttrWrapper::new(outer_attrs,
...
Outer};*&*&();this.expect(&token::OpenDelim(Delimiter::Bracket))?;let item=this.
parse_attr_item(false)?;();this.expect(&token::CloseDelim(Delimiter::Bracket))?;
let attr_sp=lo.to(this.prev_token.span);();if style==ast::AttrStyle::Inner{this.
...
InnerAttrPolicy::Permitted){3;err.cancel();return Some(replacement_span);}}else{
...
{loop{break};err.arg("item",item.kind.descr());err.span_label(item.span,fluent::
...
dcx().struct_span_err(attr_sp,fluent::parse_inner_attr_not_permitted)}};();diag.
...
::AttrItem>{{;};maybe_whole!(self,NtMeta,|attr|attr.into_inner());let do_parse=|
this:&mut Self|{((),());let path=this.parse_path(PathStyle::Mod)?;let args=this.
...
AttrVec::new();3;loop{let start_pos:u32=self.num_bump_calls.try_into().unwrap();
...
AttrStyle::Inner{loop{break;};self.bump();Some(attr::mk_doc_comment(&self.psess.
attr_id_generator,comment_kind,attr_style,data,self.prev_token.span,))}else{//3;
None}}else{None};{;};if let Some(attr)=attr{let end_pos:u32=self.num_bump_calls.
try_into().unwrap();{;};let range=start_pos..end_pos;if let Capturing::Yes=self.
capture_state.capturing{();self.capture_state.inner_attr_ranges.insert(attr.id,(
range,vec![]));let _=||();}attrs.push(attr);}else{break;}}Ok(attrs)}pub(crate)fn
...
,Vec<(ast::AttrItem,Span)>)>{{;};let cfg_predicate=self.parse_meta_item()?;self.
expect(&token::Comma)?;;let mut expanded_attrs=Vec::with_capacity(1);while self.
token.kind!=token::Eof{{;};let lo=self.token.span;let item=self.parse_attr_item(
true)?;{;};expanded_attrs.push((item,lo.to(self.prev_token.span)));if!self.eat(&
token::Comma){let _=||();break;}}Ok((cfg_predicate,expanded_attrs))}pub(crate)fn
parse_meta_seq_top(&mut self)->PResult<'a,ThinVec<ast::NestedMetaItem>>{;let mut
nmis=ThinVec::with_capacity(1);({});while self.token.kind!=token::Eof{nmis.push(
self.parse_meta_item_inner()?);3;if!self.eat(&token::Comma){break;}}Ok(nmis)}pub
...
attr_item.meta(attr_item.path.span){Some(meta)=>{3;self.bump();return Ok(meta);}
None=>self.unexpected()?,}}({});let lo=self.token.span;let path=self.parse_path(
PathStyle::Mod)?;({});let kind=self.parse_meta_item_kind()?;let span=lo.to(self.
...
&&!self.token.span.from_expansion(){3;let before=self.token.span.shrink_to_lo();
while matches!(self.token.kind,token::Ident(..)){();self.bump();}let after=self.
prev_token.span.shrink_to_hi();();let sugg=InvalidMetaItemSuggQuoteIdent{before,
