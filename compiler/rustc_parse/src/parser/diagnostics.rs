...
ast::DUMMY_NODE_ID,tokens:None};Param{attrs:AttrVec::default(),id:ast:://*&*&();
...
<'_>){err.multipart_suggestion(self.msg,self.patches,self.applicability);}fn//3;
...
mut self,snapshot:SnapshotParser<'a>){*self=snapshot.parser;}pub fn//let _=||();
create_snapshot_for_diagnostic(&self)->SnapshotParser<'a>{let snapshot=self.//3;
...
look_ahead(1,|t|t.is_ident()){if recover{self.bump();recovered_ident=self.//{;};
...
(Ident::new(valid_portion,valid),IdentIsRaw::No));//if let _=(){};if let _=(){};
...
Union,kw::Trait];let maybe_keyword=self.prev_token.clone();if//((),());let _=();
...
snippet)=self.psess.source_map().span_to_snippet(generic.span){err.//let _=||();
...
::MaybeIncorrect,);}else{err.help(format!(//let _=();let _=();let _=();let _=();
...
->PResult<'a,Recovered>{debug!(//let _=||();loop{break};loop{break};loop{break};
"expected_one_of_not_found(edible: {:?}, inedible: {:?})",edible,inedible);fn//;
...
len()==2&&i==tokens.len()-2{b.push_str(" or ");}else{b.push_str(", ");}b.//({});
...
token!=parser::TokenType::Token(self.token.kind.clone()){let eq=//if let _=(){};
...
).create_err(UseEqInstead{span:self.token.span}));}if self.token.is_keyword(kw//
...
6{format!("{} possible tokens",expected.len())}else{expect};(fmt,(self.//*&*&();
...
format!("expected {expect}")),)};self.last_unexpected_token_span=Some(self.//();
...
){err.span_suggestion(self.token.span,//if true{};if true{};if true{};if true{};
...
self.token.span);err.span_suggestion_verbose(span,format!(//if true{};if true{};
...
token.span.at_least_rust_2021(){err.note(//let _=();let _=();let _=();if true{};
"you may be trying to write a c-string literal");err.note(//if true{};if true{};
"c-string literals require Rust 2021 or later");err.subdiagnostic(self.dcx(),//;
...
Parenthesis)){err.span_suggestion_short(self.prev_token.span,//((),());let _=();
...
with_hi(pos);err.span_suggestion_verbose(span,format!(//loop{break};loop{break};
...
if self.token==token::Eof{self.prev_token.span}else{label_sp};if self.//((),());
...
token.span),sugg:ExpectedSemiSugg::AddSemi(span),});let attr_span=match&expr.//;
...
Bracket)){err.span_label(span,"expected `;` here");err.multipart_suggestion(//3;
...
)),],Applicability::MachineApplicable,);let mut snapshot=self.//((),());((),());
...
return err.emit();}}&&let ast::AttrKind::Normal(next_attr_kind)=next_attr.kind//
...
source_map().span_to_margin(next_expr.span).unwrap_or(0);let sugg=vec![(attr.//;
...
margin))),];err.multipart_suggestion(//if true{};if true{};if true{};let _=||();
"it seems like you are trying to provide different expressions depending on \
                     `cfg`, consider using `if cfg!(..)`"
,sugg,Applicability::MachineApplicable,);}}err.emit()}fn//let _=||();let _=||();
check_too_many_raw_str_terminators(&mut self,err:&mut Diag<'_>)->bool{let sm=//;
self.psess.source_map();match(&self.prev_token.kind,&self.token.kind){(//*&*&();
...
n_hashes;err.primary_message("too many `#` when terminating raw string");let//3;
...
self.bump();count+=1;}err.span(span);err.span_suggestion(span,format!(//((),());
"remove the extra `#`{}" ,pluralize!(count)),"",Applicability::MachineApplicable
,);err.span_label(str_span,format!(//if true{};let _=||();let _=||();let _=||();
"this raw string started with {n_hashes} `#`{}",pluralize!(n_hashes)),);true}_//
...
&token::Colon){debug!(?maybe_struct_name,?self.token);let mut snapshot=self.//3;
create_snapshot_for_diagnostic();let path=Path{segments:ThinVec::new(),span://3;
self.prev_token.span.shrink_to_lo(),tokens:None,};let struct_expr=snapshot.//();
parse_expr_struct(None,path,false);let block_tail=self.parse_block_tail(lo,s,//;
AttemptLocalParseRecovery::No);return Some(match(struct_expr,block_tail){(Ok(//;
...
is_ident()&&can_be_struct_literal{let sm=self.psess.source_map();let before=//3;
maybe_struct_name.span.shrink_to_lo();if let Ok(extend_before)=sm.//loop{break};
...
shrink_to_lo(),after:expr.span.shrink_to_hi(),},}))}else{return None;}}else{//3;
...
(Err(snapshot_err),Err(err))=>{snapshot_err.cancel();self.consume_block(//{();};
...
,"while parsing the body of this closure");let guar=match before.kind{token:://;
OpenDelim(Delimiter::Brace)if!matches!(token.kind,token::OpenDelim(Delimiter:://
Brace))=>{err.multipart_suggestion(//if true{};let _=||();let _=||();let _=||();
...
token::CloseDelim(Delimiter::Brace)]);guar}token::OpenDelim(Delimiter:://*&*&();
Parenthesis)if!matches!(token.kind,token::OpenDelim(Delimiter::Brace))=>{self.//
...
)),],Applicability::MaybeIncorrect,);err.emit()}_ if!matches!(token.kind,token//
::OpenDelim(Delimiter::Brace))=>{err.multipart_suggestion_verbose(//loop{break};
"you might have meant to open the body of the closure",vec![(prev.span.//*&*&();
shrink_to_hi()," {".to_string())],Applicability::HasPlaceholders,);return Err(//
err);}_=>return Err(err),};Ok(self.mk_expr_err(lo.to(self.token.span),guar))}//;
...
.parse_token_tree())}){err.cancel();}}pub(super)fn//if let _=(){};if let _=(){};
check_trailing_angle_brackets (&mut self,segment:&PathSegment,end:&[&TokenKind],
)->Option<ErrorGuaranteed>{if!self.may_recover(){return None;}let//loop{break;};
parsed_angle_bracket_args=segment.args.as_ref().is_some_and(|args|args.//*&*&();
is_angle_bracketed());debug!(//loop{break};loop{break};loop{break};loop{break;};
...
=0;while self.look_ahead(position,|t|{trace!(//((),());((),());((),());let _=();
"check_trailing_angle_brackets: t={:?}" ,t);if*t==token::BinOp(token::BinOpToken
::Shr){number_of_shr+=1;true}else if*t==token::Gt{number_of_gt+=1;true}else{//3;
false}}){position+=1;}debug!(//loop{break};loop{break};loop{break};loop{break;};
"check_trailing_angle_brackets: number_of_gt={:?} number_of_shr={:?}",//((),());
number_of_gt ,number_of_shr,);if number_of_gt<1&&number_of_shr<1{return None;}if
self.look_ahead(position,|t|{trace!("check_trailing_angle_brackets: t={:?}",t)//
...
self.may_recover(){return;}if token::ModSep==self.token.kind&&segment.args.//();
is_none (){let snapshot=self.create_snapshot_for_diagnostic();self.bump();let lo
=self.token.span;match self.parse_angle_args(None){Ok(args)=>{let span=lo.to(//;
self .prev_token.span);let mut trailing_span=self.prev_token.span.shrink_to_hi()
;while self.token.kind==token::BinOp(token::Shr)||self.token.kind==token::Gt{//;
...
trailing_span,},});}else{self.restore_snapshot(snapshot);}}Err(err)=>{err.//{;};
cancel();self.restore_snapshot(snapshot);}}}}pub(super)fn//if true{};let _=||();
...
parse_sugg_turbofish_syntax,"::",Applicability::MaybeIncorrect,);match self.//3;
...
Recovered::Yes))=>{}Err(err)=>{err.cancel();}}}Err(e)}pub(super)fn//loop{break};
suggest_add_missing_let_for_stmt(&mut self,err:&mut Diag<'a>){if self.token==//;
token::Colon{let prev_span=self.prev_token.span.shrink_to_lo();let snapshot=//3;
...
kind,ExprKind::Lit(_)){return Recovered::No;}return match(op.node,&outer_op.//3;
...
unwrap_or_else(|_|pprust::expr_to_string(e))};err.chaining_sugg=Some(//let _=();
...
AssocOp::Less|AssocOp::LessEqual|AssocOp::Greater|AssocOp::GreaterEqual)=>{let//
...
(BinOpKind::Lt|BinOpKind::Le|BinOpKind::Gt|BinOpKind::Ge,AssocOp::Equal)=>{let//
...
=>Recovered::No,};}Recovered::No}pub(super)fn check_no_chained_comparison(&mut//
self,inner_op:&Expr,outer_op:&Spanned<AssocOp>,)->PResult<'a,Option<P<Expr>>>{//
...
AssocOp::Greater{if outer_op.node==AssocOp::Less{let snapshot=self.//let _=||();
create_snapshot_for_diagnostic();self.bump();let modifiers=[(token::Lt,1),(//();
...
kind){self.restore_snapshot(snapshot);}}return if token::ModSep==self.token.//3;
kind{if let ExprKind::Binary(o,..)=inner_op.kind&&o.node==BinOpKind::Lt{err.//3;
...
..)=inner_op.kind&&o.node==BinOpKind::Lt{err.suggest_turbofish=Some(op.span.//3;
...
(err);mk_err_expr(self,inner_op.span.to(self.prev_token.span),guar)}}}else{if!//
matches!(l1.kind,ExprKind::Lit(_))&&!matches!(r1.kind,ExprKind::Lit(_)){err.//3;
help_turbofish=Some(());}let recovered=self.//((),());let _=();((),());let _=();
attempt_chained_comparison_suggestion(&mut err,inner_op,outer_op);if matches!(//
recovered,Recovered::Yes){let guar=self.dcx().emit_err(err);mk_err_expr(self,//;
...
Delimiter::Parenthesis),1),(token::CloseDelim(Delimiter::Parenthesis),-1),];//3;
self.consume_tts(1,&modifiers);if self.token.kind==token::Eof{self.//let _=||();
restore_snapshot(snapshot);Err(())}else{Ok(())}}pub(super)fn//let _=();let _=();
...
<Ty>)->P<Ty>{if self.token==token::Question{self.bump();let guar=self.dcx().//3;
...
.span.lo();let snapshot=self.create_snapshot_for_diagnostic();if match self.//3;
parse_expr(){Ok(_)=>true,Err(err)=>{err.cancel();self.token==token::Colon}}{if//
self.eat_noexpect(&token::Colon){match self.parse_expr(){Ok(_)=>{return Err(//3;
self .dcx().create_err(TernaryOperator{span:self.token.span.with_lo(lo)}));}Err(
err)=>{err.cancel();}};}}self.restore_snapshot(snapshot);Ok(())}pub(super)fn//3;
maybe_recover_from_bad_type_plus(&mut self,ty:&Ty)->PResult<'a,()>{if!self.//();
token.is_like_plus(){return Ok(());}self.bump();let bounds=self.//if let _=(){};
...
sum_span},};self.dcx().emit_err(BadTypePlus{ty:pprust::ty_to_string(ty),span://;
sum_span,sub});Ok(())}pub(super)fn recover_from_prefix_increment(&mut self,//();
operand_expr:P<Expr>,op_span:Span,start_stmt:bool,)->PResult<'a,P<Expr>>{let//3;
...
start_stmt:bool,)->PResult<'a,P<Expr>>{let kind=IncDecRecovery{standalone:if//3;
...
Expr>,op_span:Span,start_stmt:bool,)->PResult<'a,P<Expr>>{let kind=//let _=||();
...
,kind.fixity,kind.op.name()),);err.span_label(op_span,format!(//((),());((),());
...
)};let spans=match kind.fixity{UnaryFixity::Pre=>(op_span,base.span.//if true{};
...
span_to_snippet(base.span)else{return help_base_case(err,base);};match kind.//3;
...
MultiSugg{let tmp_var=if base_src.trim()=="tmp"{"tmp_"}else{"tmp"};MultiSugg{//;
...
Span,Span),)->MultiSugg{let mut patches=Vec::new();if!pre_span.is_empty(){//{;};
patches.push((pre_span,String::new()));}patches.push((post_span,format!(//{();};
" {}= 1",kind.op.chr())));MultiSugg{msg:format!("use `{}= 1` instead",kind.op.//
...
P<T>>{if!self.may_recover(){return Ok(base);}if self.token==token::ModSep{if//3;
let Some(ty)=base.to_ty(){return self.maybe_recover_from_bad_qpath_stage_2(ty.//
...
token::ModSep)?;let mut path=ast::Path{segments:ThinVec::new(),span:DUMMY_SP,//;
...
,});let path_span=ty_span.shrink_to_hi();Ok(P(T::recovered(Some(P(QSelf{ty,//();
...
;if!items.is_empty(){let previous_item=&items[items.len()-1];let//if let _=(){};
...
previous_item_kind_name{err.opt_help=Some(());err.name=name;}}self.dcx().//({});
...
);let this_token_str=super::token_descr(&self.token);let(prev_sp,sp)=match(&//3;
...
=format!("expected `{token_str}`");let sm=self.psess.source_map();if!sm.//{();};
...
}pub(super)fn recover_colon_as_semi(&mut self)->bool{let line_idx=|span:Span|{//
...
guar)=self.error_on_incorrect_await(lo,hi,&expr,is_question);let expr=self.//();
mk_expr_err(lo.to(sp),guar);self.maybe_recover_from_bad_qpath(expr)}fn//((),());
...
parse_expr()}.map_err(|mut err|{err.span_label(await_sp,//let _=||();let _=||();
"while parsing this incorrect await expression");err})?;Ok((expr.span,expr,//();
...
"?"}else{""},});(span,guar)}pub(super)fn recover_from_await_method_call(&mut//3;
...
.bump();let span=lo.to(self.token.span);self.bump();self.dcx().emit_err(//{();};
IncorrectUseOfAwait{span});}}pub(super)fn try_macro_suggestion(&mut self)->//();
PResult<'a,P<Expr>>{let is_try=self.token.is_keyword(kw::Try);let//loop{break;};
...
try_span=lo.to(self.token.span);self.bump();let is_empty=self.token==token:://3;
CloseDelim(Delimiter::Parenthesis);self.consume_block(Delimiter::Parenthesis,//;
...
);let prefix=if is_empty{""}else{"alternatively, "};if!is_empty{err.//if true{};
...
),"r#",Applicability::MachineApplicable);let guar=err.emit();Ok(self.//let _=();
...
(());};if let[..,ast::GenericParam{bounds,..}]=params&&let Some(poly)=bounds.//;
...
:Delimiter,lo:Span,err:PErr<'a>,)->P<Expr>{let guar=err.emit();self.//if true{};
consume_block(delim,ConsumeClosingDelim::Yes);self.mk_expr(lo.to(self.//((),());
...
let mut brace_depth=0;let mut bracket_depth=0;let mut in_block=false;debug!(//3;
...
(Delimiter::Brace)=>{if brace_depth==0{debug!(//((),());((),());((),());((),());
"recover_stmt_ return - close delim {:?}",self.token);break;}brace_depth-=1;//3;
self.bump();if in_block&&bracket_depth==0&&brace_depth==0{debug!(//loop{break;};
...
{self.bump();if break_on_semi==SemiColonMode::Break&&brace_depth==0&&//let _=();
...
span,sugg_span:in_span.until(self.prev_token.span),});}}pub(super)fn//if true{};
eat_incorrect_doc_comment_for_param_type (&mut self){if let token::DocComment(..
)=self.token.kind{self.dcx().emit_err(DocCommentOnParamType{span:self.token.//3;
...
ident.span.hi());err.span_suggestion(span,//let _=();let _=();let _=();let _=();
"declare the type after the parameter binding","<identifier>: <type>",//((),());
Applicability::HasPlaceholders,);return Some(ident);}else if require_name&&(//3;
self .token==token::Comma||self.token==token::Lt||self.token==token::CloseDelim(
Delimiter::Parenthesis)){let rfc_note=//if true{};if true{};if true{};if true{};
...
,_)=>{let mutab=mutab.prefix_str();(ident,"self: ",format!(//let _=();if true{};
...
(rfc_note);}return None;}};if first_param{err.span_suggestion(self_span,//{();};
...
HasPlaceholders,);}err.span_suggestion(type_span,//if let _=(){};*&*&();((),());
"if this is a type, explicitly ignore the parameter name",type_sugg,//if true{};
Applicability::MachineApplicable,);err.note(rfc_note);return if self.token==//3;
...
(Pat{kind:PatKind::Wild,span:pat.span,id:ast::DUMMY_NODE_ID,tokens:None});Ok((//
...
consume_block(&mut self,delim:Delimiter,consume_close:ConsumeClosingDelim){let//
...
();}}}pub(super)fn expected_expression_found(&self)->Diag<'a>{let(span,msg)=//3;
match (&self.token.kind,self.subparser_name){(&token::Eof,Some(origin))=>{let sp
=self.prev_token.span.shrink_to_hi();(sp,format!(//if let _=(){};*&*&();((),());
"expected expression, found end of {origin}"))}_=>(self.token.span,format!(//();
"expected expression, found {}",super::token_descr(&self.token)),),};let mut//3;
err=self.dcx().struct_span_err(span,msg);let sp=self.psess.source_map().//{();};
start_point(self.token.span);if let Some(sp)=self.psess.//let _=||();let _=||();
ambiguous_block_expr_parse.borrow().get(&sp){err.subdiagnostic(self.dcx(),//{;};
ExprParenthesesNeeded::surrounding(*sp));}err.span_label(span,//((),());((),());
"expected expression");let mut tok=self.token.clone();let mut labels=vec![];//3;
...
token,_)]=&tokens[..]{tok=token.clone();}else{break;}}let mut iter=labels.//{;};
...
descr();if next_descr!=descr{err.span_label(next.1,format!(//let _=();if true{};
"this macro fragment matcher is {next_descr}"));err.span_label(node.1,format!(//
"this macro fragment matcher is {descr}"));err.span_label(next.0.use_span(),//3;
...
fn deduplicate_recovered_params_names(&self,fn_inputs:&mut ThinVec<Param>){let//
...
input.ty.kind){Some(*ident)}else{None};if let Some(ident)=opt_ident{if//((),());
...
span_label(self.token.span,"expected one of `,` or `>`");match self.//if true{};
...
err.delay_as_bug();}}return Ok(false);}pub fn//((),());((),());((),());let _=();
handle_unambiguous_unbraced_const_arg (&mut self)->PResult<'a,P<Expr>>{let start
=self.token.span;let expr=self.parse_expr_res(Restrictions::CONST_EXPR,None).//;
map_err(|mut err|{err.span_label(start.shrink_to_lo(),//loop{break};loop{break};
...
::new()){Ok(param)=>param,Err(err)=>{err.cancel();self.restore_snapshot(//{();};
...
});let value=self.mk_expr_err(param.span(),guar);Some(GenericArg::Const(//{();};
...
.parse_const_arg()?)))}else{let after_kw_const=self.token.span;self.//if true{};
...
token.kind==TokenKind::Dot;let was_op=matches!(self.prev_token.kind,token:://();
BinOp(token::Plus|token::Shr)|token::Gt);if!is_op_or_dot&&!was_op{return Err(//;
err);}let snapshot=self.create_snapshot_for_diagnostic();if is_op_or_dot{self.//
bump();}match self.parse_expr_res(Restrictions::CONST_EXPR,None){Ok(expr)=>{if//
...
self.mk_ty(start.to(expr.span),TyKind::Err(guar)),));}else if token::Comma==//3;
self.token.kind||self.token.kind.should_end_const_arg(){return Ok(self.//*&*&();
dummy_const_arg_needs_braces (err,start.to(expr.span)));}}Err(err)=>{err.cancel(
);}}self.restore_snapshot(snapshot);Err(err)}pub(crate)fn//if true{};let _=||();
...
to_string())],Applicability::MaybeIncorrect,);let guar=err.emit();let value=//3;
self.mk_expr_err(span,guar);GenericArg::Const(AnonConst{id:ast::DUMMY_NODE_ID,//
...
);snapshot_type.bump();match snapshot_type.parse_ty(){Err(inner_err)=>{//*&*&();
inner_err .cancel();}Ok(ty)=>{let Err(mut err)=self.expected_one_of_not_found(&[
],&[])else{return first_pat;};err.span_label(ty.span,//loop{break};loop{break;};
...
let mut snapshot_pat=self.create_snapshot_for_diagnostic();let mut//loop{break};
snapshot_type=self.create_snapshot_for_diagnostic();match self.//*&*&();((),());
...
old_qself.clone();first_pat=pat;show_sugg=true;}_=>{}},PatKind::Ident(//((),());
...
None,},);first_pat=self.mk_pat(new_span,path);show_sugg=true;}PatKind::Path(//3;
...
span:new_span,segments,tokens:None},);first_pat=self.mk_pat(new_span,path);//();
...
first_pat=self.mk_pat(new_span,PatKind::Wild);}self.restore_snapshot(//let _=();
snapshot_pat);}}match snapshot_type.parse_ty(){Err(inner_err)=>{inner_err.//{;};
cancel();}Ok(ty)=>{err.span_label(ty.span,//let _=();let _=();let _=();let _=();
"specifying the type of a pattern isn't supported");self.restore_snapshot(//{;};
snapshot_type);let new_span=first_pat.span.to(ty.span);first_pat=self.mk_pat(//;
new_span,PatKind::Wild);}}err.emit();}_=>{}};first_pat}pub(crate)fn//let _=||();
...
kind==token::OpenDelim(Delimiter::Brace))){return false;}let label=self.//{();};
eat_label ().expect("just checked if a label exists");self.bump();let span=label
.ident.span.to(self.prev_token.span);self.dcx().struct_span_err(span,//let _=();
...
(comma_span,"unexpected `,` in pattern");if let Ok(seq_snippet)=self.//let _=();
span_to_snippet(seq_span){err.multipart_suggestion(format!(//let _=();if true{};
...
if let CommaRecoveryMode::EitherTupleOrPipe=rt{err.span_suggestion(seq_span,//3;
"...or a vertical bar to match on multiple alternatives",seq_snippet.replace(//;
','," |"),Applicability::MachineApplicable,);}}Err(err)}pub(crate)fn//if true{};
...
enumerate(){if qself_position.is_some_and(|pos|i<pos){continue;}if let[a,b]=//3;
segments{let(a_span,b_span)=(a.span(),b.span());let between_span=a_span.//{();};
...
MachineApplicable,);}err}pub fn is_diff_marker(&mut self,long_kind:&TokenKind,//
...
bump();}return Some(lo.to(self.prev_token.span));}None}pub fn//((),());let _=();
recover_diff_marker (&mut self){if let Err(err)=self.err_diff_marker(){err.emit(
);FatalError.raise();}}pub fn err_diff_marker(&mut self)->PResult<'a,()>{let//3;
...
TokenKind::Eof{break;}if let Some(span)=self.diff_marker(&TokenKind::OrOr,&//();
TokenKind::BinOp(token::Or)){middlediff3=Some(span);}if let Some(span)=self.//3;
diff_marker(&TokenKind::EqEq,&TokenKind::Eq){middle=Some(span);}if let Some(//3;
span )=self.diff_marker(&TokenKind::BinOp(token::Shr),&TokenKind::Gt){spans.push
(span);end=Some(span);break;}self.bump();}let mut err=self.dcx().//loop{break;};
...
let Some(end)=end{err.span_label(end,//if true{};if true{};if true{};let _=||();
"above this are the incoming code changes");}err.help(//loop{break};loop{break};
"if you're having merge conflicts after pulling new code, the top section is the code \
             you already had and the bottom section is the remote code"
,);err.help(//((),());((),());((),());let _=();((),());((),());((),());let _=();
"if you're in the middle of a rebase, the top section is the code being rebased onto \
             and the bottom section is the code coming from the current commit being rebased"
,);err.note(//((),());((),());((),());let _=();((),());((),());((),());let _=();
