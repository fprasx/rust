...
.1)).chain(std::iter::repeat_with(||{let _=();let token=cursor_snapshot.next();(
FlatToken::Token(token.0),token.1)})).take(self.num_calls);loop{break;};if!self.
replace_ranges.is_empty(){((),());let mut tokens:Vec<_>=tokens.collect();let mut
...
(),"Range {range:?} has greater len than {new_tokens:?}");3;let filler=std::iter
...
capture_cfg{({});return Ok(f(self,attrs.attrs)?.0);}let start_token=(self.token.
...
(self,attrs.attrs);({});self.capture_state.capturing=prev_capturing;let(mut ret,
...
attrs())){{();};return Ok(ret);}let mut inner_attr_replace_ranges=Vec::new();for
...
inner_attr.span,"Missing token range for attribute");3;}}let replace_ranges_end=
...
TrailingToken::Gt=>{3;assert_eq!(self.token.kind,token::Gt);}TrailingToken::Semi
=>{3;assert_eq!(self.token.kind,token::Semi);end_pos+=1;captured_trailing=true;}
TrailingToken::MaybeComma=>{if self.token.kind==token::Comma{((),());end_pos+=1;
captured_trailing=true;3;}}}if self.break_last_token{assert!(!captured_trailing,
"Cannot set break_last_token and have trailing token");if true{};end_pos+=1;}let
...
start_calls)..(range.end-start_calls),tokens)}).collect()};if true{};let tokens=
...
=Some(tokens.clone());*&*&();}}let final_attrs=ret.attrs();if self.capture_cfg&&
...
start_pos.try_into().unwrap())..(end_pos.try_into().unwrap());loop{break;};self.
...
open_delim_sp:Option<(Delimiter,Span,Spacing)>,inner:Vec<AttrTokenTree>,}{;};let
mut stack=vec![FrameData{open_delim_sp:None,inner:vec![]}];if let _=(){};let mut
...
"Token stack was empty for token: {token:?}"));if true{};let(open_delim,open_sp,
open_spacing)=frame_data.open_delim_sp.unwrap();{;};assert_eq!(open_delim,delim,
"Mismatched open/close delims: open={open_delim:?} close={span:?}");3;let dspan=
DelimSpan::from_pair(open_sp,span);;let dspacing=DelimSpacing::new(open_spacing,
spacing);*&*&();let stream=AttrTokenStream::new(frame_data.inner);let delimited=
AttrTokenTree::Delimited(dspan,dspacing,delim,stream);let _=();stack.last_mut().
...
().expect("Missing final buf!");();if break_last_token{let last_token=final_buf.
inner.pop().unwrap();;if let AttrTokenTree::Token(last_token,spacing)=last_token
{{();};let unglued_first=last_token.kind.break_two_token_op().unwrap().0;let mut
first_span=last_token.span.shrink_to_lo();((),());first_span=first_span.with_hi(
first_span.lo()+rustc_span::BytePos(1));{;};final_buf.inner.push(AttrTokenTree::
