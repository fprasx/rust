...
mut err=psess.dcx.struct_fatal(msg);3;if let Some(sp)=sp{err.span(sp);}err.emit(
...
Result<Parser<'_>,Vec<Diag<'_>>>{({});let end_pos=source_file.end_position();let
stream=maybe_file_to_stream(psess,source_file,None)?;loop{break};let mut parser=
stream_to_parser(psess,stream,None);();if parser.token==token::Eof{parser.token.
...
Item)->TokenStream{((),());let source=pprust::item_to_string(item);let filename=
...
crate_to_string_for_macros(krate);let _=||();loop{break};let filename=FileName::
