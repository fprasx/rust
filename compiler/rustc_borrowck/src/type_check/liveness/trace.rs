...
LocalUseMap::build(&relevant_live_locals,elements,body);();if typeck.tcx().sess.
...
=LiveLoans::new(borrow_set.len());();let outlives_constraints=&borrowck_context.
constraints.outlives_constraints;();let graph=outlives_constraints.graph(typeck.
infcx.num_region_vars());let _=();if true{};let region_graph=graph.region_graph(
...
borrowck_context.constraints.liveness_constraints.loans=Some(live_loans);3;};let
...
relevant_live_locals);*&*&();results.dropck_boring_locals(boring_locals);}struct
...
reset_local_state();3;self.add_defs_for(local);self.compute_use_live_points_for(
local);{();};self.compute_drop_live_points_for(local);let local_ty=self.cx.body.
local_decls[local].ty;let _=();if true{};if!self.use_live_at.is_empty(){self.cx.
...
&self.cx.typeck;{;};move||LivenessContext::compute_drop_data(typeck,local_ty)});
...
"compute_use_live_points_for(local={:?})",local);({});self.stack.extend(self.cx.
local_use_map.uses(local));3;while let Some(p)=self.stack.pop(){let block_start=
self.cx.elements.to_block_start(p);({});let previous_defs=self.defs.last_set_in(
block_start..=p);3;let previous_live_at=self.use_live_at.last_set_in(block_start
...
=>None,};3;if let Some(exclusive)=exclusive_start{self.use_live_at.insert_range(
exclusive+1..=p);;continue;}else{self.use_live_at.insert_range(block_start..=p);
let block=self.cx.elements.to_location(block_start).block;{;};self.stack.extend(
...
mpi)=self.cx.move_data.rev_lookup.find_local(local)else{return};let _=();debug!(
...
insert(drop_point){if true{};self.drop_locations.push(location);self.stack.push(
...
move_data.move_paths[mpi].place,self.cx.elements.to_location(term_point),);({});
...
term_location.block),term_location,);if true{};let block=term_location.block;let
...
"compute_drop_live_points_for_block: use-live at {:?}",p);{();};return;}if!self.
...
"compute_drop_live_points_for_block: not initialized");loop{break};continue;}let
...
pred_term_loc);3;continue;}if self.use_live_at.contains(pred_term_point){debug!(
...
"compute_drop_live_points_for_block: pushed to stack");let _=();self.stack.push(
...
flow_inits.get();{();};if state.contains(mpi){return true;}let move_paths=&self.
...
or_insert_with({();let typeck=&self.typeck;move||Self::compute_drop_data(typeck,
...
drop_location.to_locations(),ConstraintCategory::Boring,data,);({});}}drop_data.
...
live_at);{;};polonius::add_drop_of_var_derefs_origin(self.typeck,dropped_local,&
...
IntervalSet<PointIndex>,){{;};debug!("make_all_regions_live(value={:?})",value);
debug !("make_all_regions_live: live_at={}",values::pretty_print_points(elements
,live_at.iter()),);3;value.visit_with(&mut for_liveness::FreeRegionsVisitor{tcx:
typeck.tcx(),param_env:typeck.param_env,op:|r|{{();};let live_region_vid=typeck.
borrowck_context.universal_regions.to_region_vid(r);{;};typeck.borrowck_context.
...
'tcx>{{;};debug!("compute_drop_data(dropped_ty={:?})",dropped_ty,);match typeck.
