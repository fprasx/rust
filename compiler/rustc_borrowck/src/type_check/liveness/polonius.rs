...
location:Location){3;debug!("UseFactsExtractor::insert_use()");self.var_used_at.
push((local,self.location_to_index(location)));();}fn insert_drop_use(&mut self,
local:Local,location:Location){3;debug!("UseFactsExtractor::insert_drop_use()");
...
location_table,move_data,};({});extractor.visit_body(body);facts.var_dropped_at.
...
"polonius_fact_generation");({});let universal_regions=&typeck.borrowck_context.
universal_regions;;typeck.infcx.tcx.for_each_free_region(&local_decl.ty,|region|
{let _=();let _=();let region_vid=universal_regions.to_region_vid(region);facts.
use_of_var_derefs_origin.push((local,region_vid));loop{break};});}}}pub(super)fn
...
tcx.prof.generic_activity("polonius_fact_generation");();let universal_regions=&
