...
=use_polonius||AllFacts::enabled(typeck.tcx());if true{};let polonius_drop_used=
facts_enabled.then(||{let _=();if true{};let mut drop_used=Vec::new();polonius::
populate_access_facts(typeck,body,location_table,move_data,&mut drop_used);({});
drop_used});loop{break;};trace::trace(typeck,body,elements,flow_inits,move_data,
...
Either::Left(local)}else{Either::Right(local)}});();debug!("{} total variables",
body.local_decls.len());if true{};if true{};debug!("{} variables need liveness",
relevant_live_locals.len());let _=||();debug!("{} regions outlive free regions",
...
location:Location){let _=||();self.record_regions_live_at(region,location);self.
...
Location(location)=>{3;self.record_regions_live_at(ty,location);}}self.super_ty(
