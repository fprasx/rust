...
QueryRegionConstraints{outlives,member_constraints}=query_constraints;();let mut
...
r|self.to_region_vid(r));let _=();}self.constraints.member_constraints=tmp;for&(
...
expect_local(),);3;debug!(?closure_mapping);let backup=(self.category,self.span,
self.from_closure);if true{};self.from_closure=true;for outlives_requirement in&
...
instantiate(self.tcx,|vid|closure_mapping[vid]).into()}};let _=();self.category=
outlives_requirement.category;();self.span=outlives_requirement.blame_span;self.
convert(ty::OutlivesPredicate(subject,outlived_region),self.category);();}(self.
...
implicit_region_bound,known_type_outlives_obligations,..}=*self;let _=();let mut
outlives_predicates=vec![(predicate,constraint_category)];;for iteration in 0..{
if outlives_predicates.is_empty(){let _=();break;}if!self.tcx.recursion_limit().
...
);{;};}let mut next_outlives_predicates=vec![];for(ty::OutlivesPredicate(k1,r2),
...
}3;let origin=infer::RelateParamBound(self.span,t1,None);TypeOutlives::new(&mut*
...
constraint_category,);if let _=(){};}GenericArgKind::Const(_)=>unreachable!(),}}
...
type_test:TypeTest<'tcx>){{;};debug!("add_type_test(type_test={:?})",type_test);
...
constraint_category:ConstraintCategory<'tcx>,){3;let b=self.to_region_vid(b);let
a=self.to_region_vid(a);if true{};self.add_outlives(b,a,constraint_category);}fn
...
replace_placeholders_with_nll(bound);{;};let type_test=self.verify_to_type_test(
kind,a,bound);((),());let _=();((),());let _=();self.add_type_test(type_test);}}
