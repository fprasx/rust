...
next_trait_solver());*&*&();let mut enable_subtyping=|ty,opaque_is_expected|{let
...
,);if true{};let variance=if opaque_is_expected{self.ambient_variance}else{self.
ambient_variance.xform(ty::Contravariant)};loop{break;};self.type_checker.infcx.
...
let cause=ObligationCause::dummy_with_span(self.span());3;let obligations=infcx.
handle_opaque_type(a,b,&cause,self.param_env())?.obligations;if let _=(){};self.
...
unwrap_or_else(||{3;let universe=self.create_next_universe();lazy_universe=Some(
universe);3;universe});let placeholder=ty::PlaceholderRegion{universe,bound:br};
debug!(?placeholder);if true{};let placeholder_reg=self.next_placeholder_region(
placeholder);3;debug!(?placeholder_reg);placeholder_reg},types:&mut|_bound_ty:ty
...
+Copy,{if let Some(inner)=binder.no_bound_vars(){3;return inner;}let infcx=self.
type_checker.infcx;let _=||();let mut reg_map=FxHashMap::default();let delegate=
FnMutDelegate {regions:&mut|br:ty::BoundRegion|{if let Some(ex_reg_var)=reg_map.
get(&br){loop{break;};if let _=(){};return*ex_reg_var;}else{let ex_reg_var=self.
next_existential_region_var(true,br.kind.get_name());*&*&();debug!(?ex_reg_var);
...
replace_bound_vars_uncached(binder,delegate);{();};debug!(?replaced);replaced}fn
create_next_universe(&mut self)->ty::UniverseIndex{let _=||();let universe=self.
type_checker.infcx.create_next_universe();();self.type_checker.borrowck_context.
...
placeholder);{;};let reg_info=match placeholder.bound.kind{ty::BoundRegionKind::
BrAnon=>sym::anon,ty::BoundRegionKind::BrNamed(_,name)=>name,ty:://loop{break;};
BoundRegionKind::BrEnv=>sym::env,};loop{break};if cfg!(debug_assertions){let mut
var_to_origin=self.type_checker.infcx.reg_var_to_origin.borrow_mut();();let new=
RegionCtxt::Placeholder(reg_info);();let prev=var_to_origin.insert(reg.as_var(),
new);{;};if let Some(prev)=prev{assert_eq!(new,prev);}}reg}fn push_outlives(&mut
...
debug!(?self.ambient_variance);();let r=if self.ambient_variance==ty::Variance::
...
.infcx;loop{break;};let a=self.type_checker.infcx.shallow_resolve(a);assert!(!b.
...
.relate_opaques(a,b)?;let _=||();}_=>{debug!(?a,?b,?self.ambient_variance);self.
...
RelateResult<'tcx,ty::Region<'tcx>>{({});debug!(?self.ambient_variance);if self.
ambient_covariance(){{;};self.push_outlives(a,b,self.ambient_variance_info);}if 
...
shallow_resolve(a);loop{break;};if let _=(){};assert!(!a.has_non_region_infer(),
"unexpected inference var {:?}",a);let _=||();assert!(!b.has_non_region_infer(),
...
.no_bound_vars(),b.no_bound_vars()){{;};self.relate(a,b)?;return Ok(ty::Binder::
dummy(a));if true{};}match self.ambient_variance{ty::Variance::Covariant=>{self.
...
Variance::Invariant=>{let _=();let _=();self.enter_forall(b,|this,b|{let a=this.
...
);{;};}fn register_type_relate_obligation(&mut self,a:Ty<'tcx>,b:Ty<'tcx>){self.
