...
.as_coroutine_closure();{;};let tupled_upvars_ty=ty::CoroutineClosureSignature::
...
::MiscVariable(body.span),||{RegionCtxt::Unknown}),);();let next_ty_var=||{self.
...
c_variadic,user_provided_sig.unsafety,user_provided_sig.abi,);if let _=(){};}let
...
split_last().unwrap();if true{};if true{};debug!(?normalized_output_ty);debug!(?
...
(argument_index+1);if let _=(){};let mir_input_ty=body.local_decls[local].ty;let
mir_input_span=body.local_decls[local].source_info.span;let _=();if true{};self.
...
;if true{};let output_span=body.local_decls[RETURN_PLACE].source_info.span;self.
