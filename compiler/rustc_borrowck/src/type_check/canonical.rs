...
old_universe=self.infcx.universe();let _=();let TypeOpOutput{output,constraints,
error_info}=op.fully_perform(self.infcx,locations.span(self.body))?;{;};debug!(?
...
for(predicate,span)in instantiated_predicates{{();};debug!(?span,?predicate);let
category=ConstraintCategory::Predicate(span);((),());((),());let predicate=self.
normalize_with_category(predicate,locations,category);({});self.prove_predicate(
...
param_env=self.param_env;loop{break;};let result:Result<_,ErrorGuaranteed>=self.
...
UserType::Ty(user_ty)=user_ty else{bug!()};3;if let ty::Infer(_)=user_ty.kind(){
...
)?;((),());let _=();let _=();let _=();Ok(())},"ascribe_user_type_skip_wf",),);}}
