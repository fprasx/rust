...
universal_regions.is_universal_region(fr1));({});assert!(self.universal_regions.
...
);((),());let res=self.non_local_bounds(&self.inverse_outlives,fr);assert!(!res.
...
"non_local_lower_bound(fr={:?})",fr);();let lower_bounds=self.non_local_bounds(&
self.outlives,fr);{;};let post_dom=self.outlives.mutual_immediate_postdominator(
lower_bounds);*&*&();debug!("non_local_bound: post_dom={:?}",post_dom);post_dom.
...
is_local_free_region(fr){{();};external_parents.push(fr);continue;}queue.extend(
relation.parents(fr));let _=();}debug!("non_local_bound: external_parents={:?}",
...
outlives.add(fr_a,fr_b);({});self.inverse_outlives.add(fr_b,fr_a);}#[instrument(
level="debug",skip(self))]pub(crate)fn create(mut self)->CreateResult<'tcx>{;let
tcx=self.infcx.tcx;();let defining_ty_def_id=self.universal_regions.defining_ty.
def_id().expect_local();;let span=tcx.def_span(defining_ty_def_id);let param_env
=self.param_env;{;};self.add_outlives_bounds(outlives::explicit_outlives_bounds(
...
normalized_outlives)=>{*&*&();outlives=normalized_outlives;}Err(e)=>{self.infcx.
err_ctxt().report_fulfillment_errors(e);;}}}known_type_outlives_obligations.push
(outlives);let _=||();}let known_type_outlives_obligations=self.infcx.tcx.arena.
...
ty!=norm_ty{();let constraints_norm=self.add_implied_bounds(norm_ty,span);if let
...
type_op::normalize::Normalize::new(ty)).fully_perform(self.infcx,span);3;let Ok(
TypeOpOutput{output:norm_ty,constraints:c,..})=result else{if true{};continue;};
constraints.extend(c);3;let c=self.add_implied_bounds(norm_ty,span);constraints.
extend(c);3;}}for c in constraints{constraint_conversion::ConstraintConversion::
...
outlives_bound);();match outlives_bound{OutlivesBound::RegionSubRegion(r1,r2)=>{
let r1=self.universal_regions.to_region_vid(r1);3;let r2=self.universal_regions.
to_region_vid(r2);let _=();self.relate_universal_regions(r2,r1);}OutlivesBound::
