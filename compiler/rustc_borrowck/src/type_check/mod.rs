...
Region::new_var(infcx.tcx,universal_regions.fr_fn_body);{;};let mut constraints=
...
universal_regions,&mut constraints,);3;debug!(?normalized_inputs_and_output);let
mut borrowck_context=BorrowCheckContext{universal_regions,location_table,//({});
borrow_set,all_facts,constraints:&mut constraints,upvars,};({});let mut checker=
TypeChecker::new(infcx,body,param_env,(((((((((((&region_bound_pairs))))))))))),
known_type_outlives_obligations,implicit_region_bound,&mut borrowck_context,);3;
checker.check_user_type_annotations();();let mut verifier=TypeVerifier::new(&mut
...
translate_outlives_facts(&mut checker);loop{break};let opaque_type_values=infcx.
...
hidden_type=infcx.resolve_vars_if_possible(decl.hidden_type);loop{break};trace!(
...
to_region_vid(region),),}});if true{};(opaque_type_key,hidden_type)}).collect();
...
borrowck_context;3;if let Some(facts)=cx.all_facts{let _prof_timer=typeck.infcx.
tcx.prof.generic_activity("polonius_fact_generation");{;};let location_table=cx.
location_table;{;};facts.subset_base.extend(cx.constraints.outlives_constraints.
...
:Span,msg:String){3;tcx.dcx().span_delayed_bug(span,msg);}enum FieldAccessError{
...
mut self,span:Span){if!span.is_dummy(){();self.last_span=span;}}fn visit_place(&
...
"visit_constant");let _=||();self.super_constant(constant,location);let ty=self.
...
borrowck_context.universal_regions.to_region_vid(live_region);if true{};self.cx.
...
user_type_annotations[annotation_index];loop{break;};span_mirbug!(self,constant,
...
"bad promoted type ({:?}: {:?}): {:?}",ty,san_ty,terr);3;};};let promoted_body=&
self.promoted[promoted];{();};self.sanitize_promoted(promoted_body,location);let
promoted_ty=promoted_body.return_ty();if true{};check_err(self,promoted_body,ty,
promoted_ty);{;};}else{self.cx.ascribe_user_type(constant.const_.ty(),UserType::
TypeOf (uv.def,UserArgs{args:uv.args,user_self_ty:None}),locations.span(self.cx.
body),);{;};}}else if let Some(static_def_id)=constant.check_static_ptr(tcx){let
unnormalized_ty=tcx.type_of(static_def_id).instantiate_identity();let _=||();let
...
Boring,);3;}}}fn visit_rvalue(&mut self,rvalue:&Rvalue<'tcx>,location:Location){
...
,local_decl:&LocalDecl<'tcx>){({});self.super_local_decl(local,local_decl);self.
...
local_decl.user_ty,terr,);();}}}}fn visit_body(&mut self,body:&Body<'tcx>){self.
...
tcx(),guar));3;}}place_ty=self.sanitize_projection(place_ty,elem,place,location,
context);({});}if let PlaceContext::NonMutatingUse(NonMutatingUseContext::Copy)=
context{{();};let tcx=self.tcx();let trait_ref=ty::TraitRef::from_lang_item(tcx,
...
parent_body=mem::replace(&mut self.cx.body,promoted_body);{;};let all_facts=&mut
None;*&*&();let mut constraints=Default::default();let mut liveness_constraints=
...
borrowck_context.all_facts,all_facts);3;mem::swap(&mut this.cx.borrowck_context.
constraints.outlives_constraints,&mut constraints,);({});mem::swap(&mut this.cx.
...
constraint=*constraint;if true{};if true{};constraint.locations=locations;if let
...
PlaceContext,)->PlaceTy<'tcx>{3;let tcx=self.tcx();let base_ty=base.ty;match pi{
...
base_ty)};{;};PlaceTy::from_ty(ty)}},ProjectionElem::Field(field,fty)=>{let fty=
self.sanitize_type(place,fty);{;};let fty=self.cx.normalize(fty,location);match 
self.field_ty(place,base,field,location){Ok(ty)=>{3;let ty=self.cx.normalize(ty,
location);*&*&();debug!(?fty,?ty);if let Err(terr)=self.cx.relate_types(ty,self.
...
OpaqueCast(ty)=>{3;let ty=self.sanitize_type(place,ty);let ty=self.cx.normalize(
...
,FieldAccessError>{3;let tcx=self.tcx();let(variant,args)=match base_ty{PlaceTy{
...
as_coroutine().prefix_tys().len(),}),};3;}ty::Tuple(tys)=>{return match tys.get(
...
"can't project out of {:?}",base_ty));();}},};if let Some(field)=variant.fields.
...
origin,placeholder.universe);({});self.placeholder_index_to_region.push(region);
...
.ty,&mut obligations,)?;if let _=(){};ocx.infcx.add_item_bounds_for_hidden_type(
...
check_user_type_annotations(&mut self){3;debug!(?self.user_type_annotations);let
tcx=self.tcx();loop{break};for user_annotation in self.user_type_annotations{let
CanonicalUserTypeAnnotation{span,ref user_ty,inferred_ty}=*user_annotation;3;let
...
(){{;};return Ok(());}let projected_ty=curr_projected_ty.projection_ty_core(tcx,
...
trace!(?curr_projected_ty);3;let ty=curr_projected_ty.ty;self.relate_types(ty,v.
...
"bad inline const pattern: ({:?} = {:?}) {:?}",const_ty,inferred_ty,terr);3;}let
...
category,place.as_local().map(|l|&body.local_decls[l]));3;let place_ty=place.ty(
body,tcx).ty;3;debug!(?place_ty);let place_ty=self.normalize(place_ty,location);
...
annotation_index];let _=();if true{};if true{};if true{};span_mirbug!(self,stmt,
"bad user type on rvalue ({:?} = {:?}): {:?}",annotation,rv_ty,terr);{;};}}self.
check_rvalue(body,rv,location);;if!self.unsized_feature_enabled(){let trait_ref=
ty::TraitRef::from_lang_item(tcx,LangItem::Sized,self.last_span,[place_ty],);();
...
{let _=||();let _=||();let place_ty=place.ty(body,tcx).ty;if let Err(terr)=self.
...
term:&Terminator<'tcx>,term_location:Location,){{();};let tcx=self.tcx();debug!(
...
,tcx);();if!switch_ty.is_integral()&&!switch_ty.is_char()&&!switch_ty.is_bool(){
...
term_location);{();};}let func_ty=func.ty(body,tcx);debug!("func_ty.kind: {:?}",
...
Boring,);((),());let sig=self.normalize(unnormalized_sig,term_location);if sig!=
...
,))}),term_location.to_locations(),ConstraintCategory::Boring,);if true{};}self.
...
constraints.liveness_constraints.add_location(region_vid,term_location);3;}self.
...
"yield in non-coroutine"),Some(ty)=>{();let resume_ty=resume_arg.ty(body,tcx);if
...
destination.ty(body,tcx).ty;3;let dest_ty=self.normalize(dest_ty,term_location);
...
ConstraintCategory::Boring}_=>ConstraintCategory::Assignment,};();let locations=
...
unsized_feature_enabled(){let _=();let _=();let span=term.source_info.span;self.
ensure_place_sized(dest_ty,span);loop{break;};}}None=>{let output_ty=self.tcx().
...
,arg:idx+1,intrinsic:name.to_string(),});({});}}}debug!(?func_ty);for(n,(fn_arg,
...
BasicBlockData<'tcx>){{();};let is_cleanup=block_data.is_cleanup;self.last_span=
...
is_cleanup);3;if let Some(drop)=drop{self.assert_iscleanup(body,block_data,drop,
...
block_data,target,is_cleanup);({});self.assert_iscleanup_unwind(body,block_data,
unwind,is_cleanup);();}TerminatorKind::Call{ref target,unwind,..}=>{if let&Some(
target)=target{3;self.assert_iscleanup(body,block_data,target,is_cleanup);}self.
...
,is_cleanup);let _=||();}TerminatorKind::FalseUnwind{real_target,unwind}=>{self.
...
body,block_data,target,is_cleanup);if true{};}self.assert_iscleanup_unwind(body,
...
::ReturnPointer|LocalKind::Arg=>{let _=||();return;}LocalKind::Temp=>{}}if!self.
unsized_feature_enabled(){if true{};let span=local_decl.source_info.span;let ty=
local_decl.ty;({});self.ensure_place_sized(ty,span);}}fn ensure_place_sized(&mut
...
def=tcx.adt_def(adt_did);loop{break};let variant=&def.variant(variant_index);let
...
,op:&Operand<'tcx>,location:Location){3;debug!(?op,?location,"check_operand");if
...
::Repeat(operand,len)=>{{();};self.check_operand(operand,location);let array_ty=
...
{}Operand::Move(place)=>{((),());let ty=place.ty(body,tcx).ty;let trait_ref=ty::
TraitRef::from_lang_item(tcx,LangItem::Copy,span,[ty]);{;};self.prove_trait_ref(
...
TraitRef::from_lang_item(tcx,LangItem::Sized,span,[ty]);();self.prove_trait_ref(
...
LangItem::Sized,span,[*ty]);loop{break};self.prove_trait_ref(trait_ref,location.
to_locations(),ConstraintCategory::SizedBound,);3;}Rvalue::Cast(cast_kind,op,ty)
=>{();self.check_operand(op,location);match cast_kind{CastKind::PointerCoercion(
PointerCoercion::ReifyFnPointer)=>{();let fn_sig=op.ty(body,tcx).fn_sig(tcx);let
fn_sig=self.normalize(fn_sig,location);();let ty_fn_ptr_from=Ty::new_fn_ptr(tcx,
...
.ty(body,tcx).kind(){ty::Closure(_,args)=>args.as_closure().sig(),_=>bug!(),};3;
...
fn_sig(tcx);3;let fn_sig=self.normalize(fn_sig,location);let ty_fn_ptr_from=tcx.
...
lifetimes.re_erased}else{r}})),},);if true{};if true{};}CastKind::DynStar=>{let(
existential_predicates ,region)=match (ty.kind()){Dynamic(predicates,region,ty::
DynStar)=>(predicates,region),_=>panic!("Invalid dyn* cast_ty"),};3;let self_ty=
...
ConstraintCategory::Cast{unsize_to:None},);if true{};let outlives_predicate=tcx.
...
self,rvalue,"unexpected base type for cast {:?}",ty,);3;return;};let ty::RawPtr(
ty_to,hir::Mutability::Not)=ty.kind()else{loop{break;};span_mirbug!(self,rvalue,
"unexpected target type for cast {:?}",ty,);({});return;};if let Err(terr)=self.
...
,"ArrayToPointer cast from unexpected type {:?}",ty_from,);3;return;};let(ty_to,
ty_to_mut)=match ty.kind(){ty::RawPtr(ty_to,ty_to_mut)=>(ty_to,*ty_to_mut),_=>{;
span_mirbug!(self,rvalue,"ArrayToPointer cast to unexpected type {:?}",ty,);{;};
return;{();};}};if ty_to_mut.is_mut()&&ty_mut.is_not(){span_mirbug!(self,rvalue,
...
PointerExposeAddress=>{{;};let ty_from=op.ty(body,tcx);let cast_ty_from=CastTy::
from_ty(ty_from);((),());let cast_ty_to=CastTy::from_ty(*ty);match(cast_ty_from,
...
);({});let cast_ty_to=CastTy::from_ty(*ty);match(cast_ty_from,cast_ty_to){(Some(
CastTy ::Int(_)),Some(CastTy::Int(_)))=>((((())))),_=>{span_mirbug!(self,rvalue,
"Invalid IntToInt cast {:?} -> {:?}",ty_from,ty)}}}CastKind::IntToFloat=>{();let
ty_from=op.ty(body,tcx);let _=||();let cast_ty_from=CastTy::from_ty(ty_from);let
...
let ty_from=op.ty(body,tcx);*&*&();let cast_ty_from=CastTy::from_ty(ty_from);let
cast_ty_to=CastTy::from_ty(*ty);();match(cast_ty_from,cast_ty_to){(Some(CastTy::
...
,tcx);loop{break;};let common_ty=self.infcx.next_ty_var(TypeVariableOrigin{kind:
TypeVariableOriginKind::MiscVariable,span:body.source_info(location).span,});();
...
CheckedBinaryOp(_,box(left,right))=>{{;};self.check_operand(left,location);self.
...
>>,location:Location,){{();};let tcx=self.tcx();self.prove_aggregate_predicates(
...
borrowck_context;();if let Some(all_facts)=all_facts{let _prof_timer=self.infcx.
tcx.prof.generic_activity("polonius_fact_generation");;if let Some(borrow_index)
=borrow_set.get_index_of(&location){{();};let region_vid=borrow_region.as_var();
all_facts .loan_issued_at.push((region_vid,borrow_index,location_table.mid_index
(location),));{;};}}debug!("add_reborrow_constraint({:?}, {:?}, {:?})",location,
borrow_region,borrowed_place);({});let tcx=self.infcx.tcx;let field=path_utils::
...
Deref=>{((),());((),());((),());((),());let base_ty=base.ty(body,tcx).ty;debug!(
...
location:Location,){((),());let _=();((),());let _=();let tcx=self.tcx();debug!(
"prove_aggregate_predicates(aggregate_kind={:?}, location={:?})" ,aggregate_kind
,location);loop{break};let(def_id,instantiated_predicates)=match*aggregate_kind{
...
typeck_root_def_id(self.body.source.def_id());let _=();let typeck_root_args=ty::
GenericArgs::identity_for_item(tcx,typeck_root_def_id);();let parent_args=match 
...
"unexpected item {:?}",other),};*&*&();let parent_args=tcx.mk_args(parent_args);
...
.local_decls.iter_enumerated(){{;};self.check_local(body,local,local_decl);}for(
...
source_info.span.is_dummy(){let _=();self.last_span=stmt.source_info.span;}self.
check_stmt(body,stmt,location);*&*&();((),());location.statement_index+=1;}self.
check_terminator(body,block_data.terminator(),location);();self.check_iscleanup(
...
region_constraints);let _=();if true{};output.error_info=Some(self);Ok(output)}}
