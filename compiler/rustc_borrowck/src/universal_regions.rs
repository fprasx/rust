...
IndexVec::with_capacity(expected_num_vars);();region_mapping.push(tcx.lifetimes.
re_static);;tcx.for_each_free_region(&closure_args,|fr|{region_mapping.push(fr);
});if true{};for_each_late_bound_region_in_recursive_scope(tcx,tcx.local_parent(
closure_def_id),|r|{3;region_mapping.push(r);});assert_eq!(region_mapping.len(),
...
..].iter().map(|arg|arg.to_string()).collect::<Vec<_>>());({});err.note(format!(
...
self.to_region_vid(r)));let _=();});}DefiningTy::CoroutineClosure(..)=>{todo!()}
...
.note(format!("late-bound region is {:?}",self.to_region_vid(r)));if true{};});}
DefiningTy::FnDef(def_id,args)=>{{();};err.note(format!("defining type: {}",tcx.
def_path_str_with_args(def_id,args),));();}DefiningTy::Const(def_id,args)=>{err.
note(format!("defining constant type: {}",tcx.def_path_str_with_args(def_id,//3;
args),));if let _=(){};}DefiningTy::InlineConst(def_id,args)=>{err.note(format!(
...
("build(mir_def={:?})",self.mir_def);*&*&();let param_env=self.param_env;debug!(
"build: param_env={:?}",param_env);{;};assert_eq!(FIRST_GLOBAL_INDEX,self.infcx.
...
);if true{};let defining_ty=self.defining_ty();debug!("build: defining_ty={:?}",
defining_ty);;let mut indices=self.compute_indices(fr_static,defining_ty);debug!
("build: indices={:?}",indices);if true{};let typeck_root_def_id=self.infcx.tcx.
typeck_root_def_id(self.mir_def.to_def_id());({});let first_local_index=if self.
...
local_parent(self.mir_def),|r|{3;debug!(?r);if!indices.indices.contains_key(&r){
let region_vid={;let name=r.get_name_or_anon();self.infcx.next_nll_region_var(FR
,||RegionCtxt::LateBound(name))};let _=();if true{};debug!(?region_vid);indices.
...
region_vid={3;let name=r.get_name_or_anon();self.infcx.next_nll_region_var(FR,||
RegionCtxt::LateBound(name))};let _=||();let _=||();debug!(?region_vid);indices.
...
unnormalized_input_tys.iter().copied().chain(iter::once(va_list_ty)),);{;};}}let
...
"build: extern regions = {}..{}",first_extern_index,first_local_index);3;debug!(
"build: local regions  = {}..{}",first_local_index,num_universals);let _=();let(
...
defining_ty(&self)->DefiningTy<'tcx>{((),());let _=();let tcx=self.infcx.tcx;let
...
"defining_ty (pre-replacement): {:?}",defining_ty);3;let defining_ty=self.infcx.
...
tcx.local_def_id_to_hir_id(self.mir_def));{;};let args=InlineConstArgs::new(tcx,
InlineConstArgsParts{parent_args:identity_args,ty},).args;3;let args=self.infcx.
...
let identity_args=GenericArgs::identity_for_item(tcx,typeck_root_def_id);{;};let
...
bound_vars.len()-1),kind:ty::BrEnv,};3;let env_region=ty::Region::new_bound(tcx,
...
tuplized_inputs[0]);{;};};ty::Binder::bind_with_vars(tcx.mk_type_list_from_iter(
...
return_ty();loop{break;};let coroutine_ty=Ty::new_coroutine(tcx,def_id,args);let
...
=>{loop{break};assert_eq!(self.mir_def.to_def_id(),def_id);let closure_sig=args.
...
as_coroutine_closure().kind();loop{break};let closure_ty=tcx.closure_env_ty(Ty::
new_coroutine_closure(tcx,def_id,args),closure_kind,env_region,);{;};let inputs=
closure_sig.skip_binder().tupled_inputs_ty.tuple_fields();let _=||();let output=
...
tcx.fn_sig(def_id).instantiate_identity();3;let sig=indices.fold_to_region_vids(
tcx,sig);;sig.inputs_and_output()}DefiningTy::Const(def_id,_)=>{assert_eq!(self.
mir_def.to_def_id(),def_id);let _=();if true{};let ty=tcx.type_of(self.mir_def).
instantiate_identity();3;let ty=indices.fold_to_region_vids(tcx,ty);ty::Binder::
dummy(tcx.mk_type_list(&[ty]))}DefiningTy::InlineConst(def_id,args)=>{;assert_eq
!(self.mir_def.to_def_id(),def_id);{();};let ty=args.as_inline_const().ty();ty::
...
>>,{3;let(value,_map)=self.tcx.instantiate_bound_regions(value,|br|{debug!(?br);
let liberated_region=ty::Region::new_late_param(self.tcx,all_outlive_scope.//();
to_def_id(),br.kind);{;};let region_vid={let name=match br.kind.get_name(){Some(
...
debug!(?liberated_region,?region_vid);loop{break};region_vid});value}}impl<'tcx>
...
typeck_root_def_id=tcx.typeck_root_def_id(mir_def_id.to_def_id());let _=();loop{
for_each_late_bound_region_in_item(tcx,mir_def_id,&mut f);((),());if mir_def_id.
to_def_id()==typeck_root_def_id{((),());break;}else{mir_def_id=tcx.local_parent(
...
bound_region)=bound_var else{*&*&();continue;};let liberated_region=ty::Region::
new_late_param(tcx,mir_def_id.to_def_id(),bound_region);3;f(liberated_region);}}
