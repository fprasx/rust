...
borrows.clone_from(&self.borrows.entry_set_for_block(block));({});state.uninits.
clone_from(&self.uninits.entry_set_for_block(block));if true{};state.ever_inits.
...
apply_statement_effect(&mut state.uninits,stmt,loc);();self.ever_inits.analysis.
...
apply_terminator_effect(&mut state.borrows,term,loc);({});self.uninits.analysis.
apply_terminator_effect(&mut state.uninits,term,loc);3;self.ever_inits.analysis.
...
first_lo=first_location.statement_index;3;let first_hi=first_bb_data.statements.
...
kill_location).or_default().push(borrow_index);if true{};return;}for succ_bb in 
first_bb_data.terminator().successors(){if self.visited.insert(succ_bb){();self.
visit_stack.push(succ_bb);{;};}}while let Some(block)=self.visit_stack.pop(){let
...
num_stmts){3;let kill_location=Location{block,statement_index:kill_stmt};debug!(
"borrow {:?} gets killed at {:?}",borrow_index,kill_location);loop{break;};self.
borrows_out_of_scope_at_location.entry(kill_location).or_default().push(//{();};
borrow_index);{;};continue;}for succ_bb in bb_data.terminator().successors(){if 
self.visited.insert(succ_bb){{;};self.visit_stack.push(succ_bb);}}}self.visited.
...
borrow_region=borrow_data.region;({});let location=borrow_data.reserve_location;
...
let sccs=self.regioncx.constraint_sccs();();let universal_regions=self.regioncx.
universal_regions();if let _=(){};for successor in self.regioncx.region_graph().
depth_first_search(issuing_region){();let scc=sccs.scc(successor);for constraint
in (((((self.regioncx.applied_member_constraints(scc)))))){if universal_regions.
is_universal_region(constraint.min_choice){let _=||();return;}}if self.regioncx.
is_region_live_at_all_points(successor){;return;}}let first_block=loan_issued_at
.block;({});let first_bb_data=&self.body.basic_blocks[first_block];let first_lo=
...
or_default().push(loan_idx);3;return;}for succ_bb in first_bb_data.terminator().
successors(){if self.visited.insert(succ_bb){3;self.visit_stack.push(succ_bb);}}
while let Some(block)=self.visit_stack.pop(){3;let bb_data=&self.body[block];let
...
"loan {:?} gets killed at {:?}",loan_idx,kill_location);let _=();if true{};self.
loans_out_of_scope_at_location .entry(kill_location).or_default().push(loan_idx)
;{;};continue;}for succ_bb in bb_data.terminator().successors(){if self.visited.
...
start..=end{if true{};let location=Location{block,statement_index};if location==
...
,regioncx);let _=||();for(loan_idx,loan_data)in borrow_set.iter_enumerated(){let
issuing_region=loan_data.region;3;let loan_issued_at=loan_data.reserve_location;
polonius_prec.precompute_loans_out_of_scope(loan_idx,issuing_region,//if true{};
loan_issued_at,);{;};}assert_eq!(borrows_out_of_scope_at_location,polonius_prec.
...
get(&location){if true{};let _=||();trans.kill_all(indices.iter().copied());}}fn
kill_borrows_on_place(&self,trans:&mut impl GenKill<BorrowIndex>,place:Place<//;
'tcx>){if true{};if true{};debug!("kill_borrows_on_place: place={:?}",place);let
...
other_borrows_of_local);loop{break};}return;}let definitely_conflicting_borrows=
...
.borrow_set.locals_state_at_exit,){let _=||();return;}let index=self.borrow_set.
...
=>{3;self.kill_borrows_on_place(trans,Place::from(*local));}mir::StatementKind::
