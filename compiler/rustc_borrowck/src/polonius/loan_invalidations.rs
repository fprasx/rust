...
,body,dominators};3;visitor.visit_body(body);}struct LoanInvalidationsGenerator<
...
Assume(op))=>{();self.consume_operand(location,op);}StatementKind::Intrinsic(box
...
Terminator<'tcx>,location:Location){({});self.check_activations(location);match&
...
WriteKind::StorageDeadOrDrop};if true{};self.access_place(location,*drop_place,(
...
fn_span:_,}=>{let _=();self.consume_operand(location,func);for arg in args{self.
consume_operand(location,&arg.node);();}self.mutate_place(location,*destination,
Deep);{;};}TerminatorKind::Assert{cond,expected:_,msg,target:_,unwind:_}=>{self.
consume_operand(location,cond);let _=();use rustc_middle::mir::AssertKind;if let
...
resume_arg,drop:_}=>{3;self.consume_operand(location,value);let borrow_set=self.
...
borrowed_place){({});self.all_facts.loan_invalidated_at.push((resume,i));}}self.
...
ReadKind::Copy)),LocalMutationIsAllowed::No,);({});}Operand::Move(place)=>{self.
...
LocalMutationIsAllowed::No);let _=();}&Rvalue::AddressOf(mutability,place)=>{let
...
::Not=>(Deep,Read(ReadKind::Borrow(BorrowKind::Shared))),};();self.access_place(
...
};if true{};self.access_place(location,place,(Shallow(af),Read(ReadKind::Copy)),
...
location,operand1);3;self.consume_operand(location,operand2);}Rvalue::NullaryOp(
...
_is_local_mutation_allowed:LocalMutationIsAllowed,){*&*&();let(sd,rw)=kind;self.
...
borrow,location){3;assert!(allow_two_phase_borrow(borrow.kind));return Control::
...
BorrowIndex,l:Location){*&*&();let lidx=self.location_table.start_index(l);self.
...
location){3;let borrow=&self.borrow_set[borrow_index];assert!(match borrow.kind{
BorrowKind::Shared|BorrowKind::Fake=>false,BorrowKind::Mut{..}=>true,});();self.
