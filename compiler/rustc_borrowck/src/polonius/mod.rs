...
{if true{};let Some(all_facts)=all_facts else{return;};let _prof_timer=tcx.prof.
generic_activity("polonius_fact_generation");let _=();emit_move_facts(all_facts,
...
(all_facts,tcx,location_table,body,borrow_set);();emit_loan_invalidations_facts(
...
push((child,parent));3;}}let fn_entry_start=location_table.start_index(Location{
...
body.local_kind(local)==LocalKind::Arg);3;all_facts.path_assigned_at_base.push((
...
path_moved_at_base.push((path,fn_entry_start));3;}}all_facts.path_moved_at_base.
...
=borrow_set.len();loop{break;};if let _=(){};if let _=(){};if let _=(){};debug!(
...
.index();3;let placeholder_loan_idx=borrow_count+universal_region_idx;all_facts.
...
"emit_universal_region_facts: emitting polonius `known_placeholder_subset` \
                     fr1={:?}, fr2={:?}"
,fr1,fr2);if let _=(){};all_facts.known_placeholder_subset.push((fr1,fr2));}}}fn
