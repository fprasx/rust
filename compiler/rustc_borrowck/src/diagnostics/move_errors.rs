...
group_move_errors();let _=();for error in grouped_errors{self.report(error);}}fn
group_move_errors(&mut self)->Vec<GroupedMoveError<'tcx>>{*&*&();((),());let mut
grouped_errors=Vec::new();3;let errors=std::mem::take(&mut self.move_errors);for
...
local)=place.as_local(){({});let local_decl=&self.body.local_decls[local];if let
...
self.move_spans(original_path.as_ref(),location);let _=||();grouped_errors.push(
...
Span,){if let _=(){};debug!(?match_place,?match_span,"append_binding_error");let
from_simple_let=match_place.is_none();{;};let match_place=match_place.unwrap_or(
move_from);if true{};match self.move_data.rev_lookup.find(match_place.as_ref()){
...
"appending local({bind_to:?}) to list");();if!binds_to.is_empty(){binds_to.push(
bind_to);;}return;}}debug!("found a new move error location");let(binds_to,span)
=if from_simple_let{(vec![],statement_span)}else{(vec![bind_to],match_span)};();
grouped_errors.push(GroupedMoveError::MovesFromPlace{span,move_from,//if true{};
original_path,kind,binds_to,});({});}LookupResult::Exact(_)=>{let LookupResult::
Parent(Some(mpi))=self.move_data.rev_lookup.find(move_from.as_ref())else{*&*&();
unreachable!("Probably not unreachable...");();};for ge in&mut*grouped_errors{if
...
bind_to],});3;}};}fn report(&mut self,error:GroupedMoveError<'tcx>){let(mut err,
...
use_spans.args_or_use(),Some(use_spans),original_path,kind)}};let _=||();debug!(
...
cannot_move_out_of_interior_noncopy(span,ty,Some(is_index))}},span,)};({});self.
add_move_hints(error,&mut err,err_span);if let _=(){};self.buffer_error(err);}fn
...
Diag<'tcx>{((),());let ty=deref_target_place.ty(self.body,self.infcx.tcx).ty;let
upvar_field =self.prefixes(move_place.as_ref(),PrefixSet::All).find_map(|p|self.
is_upvar_field_projection(p));if true{};let deref_base=match deref_target_place.
...
move_place,span);3;}}debug!("report: ty={:?}",ty);let mut err=match ty.kind(){ty
...
"closure kind not inferred by borrowck"),};({});let capture_description=format!(
"captured variable in an `{closure_kind}` closure");({});let upvar=&self.upvars[
...
"{place_name}, as `{upvar_name}` is a {capture_description}")};if true{};debug!(
...
borrowed_content_source(deref_base);({});let move_place_ref=move_place.as_ref();
...
};();let msg_opt=CapturedMessageOpt{is_partial_move:false,is_loop_message:false,
is_move_msg:(false),is_loop_move:(false),maybe_reinitialized_locations_is_empty:
true,};{;};if let Some(use_spans)=use_spans{self.explain_captures(&mut err,span,
...
;{;};err.subdiagnostic(self.dcx(),crate::session_diagnostics::TypeNoCopy::Label{
is_partial_move:false,ty:place_ty,place:&place_desc,span,},);{;};}else{binds_to.
sort();let _=||();binds_to.dedup();self.add_move_error_details(err,&binds_to);}}
GroupedMoveError::MovesFromValue{mut binds_to,..}=>{();binds_to.sort();binds_to.
dedup();if true{};if true{};self.add_move_error_suggestions(err,&binds_to);self.
add_move_error_details(err,&binds_to);();}GroupedMoveError::OtherIllegalMove{ref
original_path,use_spans,..}=>{({});let span=use_spans.var_or_use();let place_ty=
...
CaptureArgLabel::MoveOutPlace{place:place_desc,args_span,}});if let _=(){};self.
...
MaybeIncorrect,);if true{};}_=>{err.span_suggestion_verbose(span.shrink_to_lo(),
...
infcx.tcx.sess.source_map().span_to_snippet(pat_span)else{3;continue;};let Some(
...
trim_start();let _=||();let(pat_span,suggestion,to_remove)=if inner_pat_snippet.
...
trim_start();3;let pat_span=pat_span.with_hi(pat_span.lo()+BytePos((pat_snippet.
...
"consider removing the {to_remove}"),suggestion.to_string(),));();}}suggestions.
...
bind_to.ty,place:place_desc,span:binding_span,},);{;};}}if binds_to.len()>1{err.
...
let local_place:PlaceRef<'tcx>=local.into();();let local_ty=local_place.ty(self.
