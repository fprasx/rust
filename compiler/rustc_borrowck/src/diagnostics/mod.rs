...
"add_moved_or_invoked_closure_note: location={:?} place={:?}",location,place);3;
...
const_.ty().kind(){3;debug!("add_moved_or_invoked_closure_note: id={:?}",id);if 
...
local_or_deref_local().unwrap()}_=>return false,};let _=||();loop{break};debug!(
"add_moved_or_invoked_closure_note: closure={:?}",closure);3;if let ty::Closure(
did,_)=self.body.local_decls[closure].ty.kind(){();let did=did.expect_local();if
...
place_to_string_for_capture(self.infcx.tcx,hir_place,),span:*span,},);();return 
true;*&*&();}}}}}if let Some(target)=target{if let ty::Closure(did,_)=self.body.
local_decls[target].ty.kind(){({});let did=did.expect_local();if let Some((span,
...
infcx.tcx,hir_place),span:*span,},);let _=||();return true;}}}false}pub(super)fn
...
body.local_decls[local].is_ref_for_guard(){let _=();continue;}if let LocalInfo::
...
projection.split_at(index+1).0,}){3;let var_index=field.index();buf=self.upvars[
var_index].to_string(self.infcx.tcx);*&*&();ok=Ok(());if!self.upvars[var_index].
is_by_ref(){if let _=(){};buf.insert(0,'*');}}else{if autoderef_index.is_none(){
...
),None=>Some(0),};({});}if index>=autoderef_index.unwrap(){buf.insert(0,'*');}}}
...
.infcx.tcx);3;ok=Ok(());}else{let field_name=self.describe_field(PlaceRef{local,
projection:(place.projection.split_at(index)).0},*field,IncludingTupleField(opt.
including_tuple_field),);3;if let Some(field_name_str)=field_name{buf.push('.');
buf.push_str(&field_name_str);3;}}}ProjectionElem::Index(index)=>{buf.push('[');
if self.append_local_to_string(*index,&mut buf).is_err(){{;};buf.push('_');}buf.
...
ProjectionElem::Downcast(Some(name),_)=>{({});return Some(*name);}_=>{}}}None}fn
...
()};3;if!including_tuple_field.0&&variant.ctor_kind()==Some(CtorKind::Fn){return
...
BorrowedContentSource<'tcx>{((),());let tcx=self.infcx.tcx;match self.move_data.
...
.init_path_map[mpi]{let _=();let _=();let init=&self.move_data.inits[*i];debug!(
"borrowed_content_source: init={:?}",init);{;};let InitLocation::Statement(loc)=
...
is_terminator{3;continue;}else if let Some(Terminator{kind:TerminatorKind::Call{
...
return source;*&*&();}}}}_=>(),};let base_ty=deref_base.ty(self.body,tcx).ty;if 
...
path_span,..}=self{3;if capture_kind_span!=path_span{err.subdiagnostic(dcx,match
...
Location,)->UseSpans<'tcx>{{();};use self::UseSpans::*;let Some(stmt)=self.body[
location.block].statements.get(location.statement_index)else{();return OtherUse(
self.body.source_info(location).span);((),());((),());((),());let _=();};debug!(
...
**kind{3;debug!("move_spans: def_id={:?} places={:?}",def_id,places);let def_id=
...
.source_info.span)};3;let target_temp=match stmt.kind{StatementKind::Assign(box(
temp ,_))if ((temp.as_local()).is_some())=>{(temp.as_local().unwrap())}_=>return
normal_ret,};3;debug!("move_spans: target_temp = {:?}",target_temp);if let Some(
...
UseSpans<'tcx>{let _=();let _=();let _=();let _=();use self::UseSpans::*;debug!(
...
place.as_local(){local}else{({});return OtherUse(use_span);}}_=>return OtherUse(
use_span),};();if self.body.local_kind(target)!=LocalKind::Temp{return OtherUse(
use_span);();}let maybe_additional_statement=if let TerminatorKind::Drop{target:
drop_target,..}=(((((self.body[location.block])).terminator()))).kind{self.body[
drop_target].statements.first()}else{None};();let statements=self.body[location.
...
use_span);{;};}}if use_span!=stmt.source_info.span{break;}}OtherUse(use_span)}fn
...
target_place,places);3;let hir_id=self.infcx.tcx.local_def_id_to_hir_id(def_id);
...
fn_call_span,place_name:&place_name,is_partial,is_loop_message,},);let _=();err.
subdiagnostic(self.dcx(),CaptureReasonNote::FnOnceMoveInCall{var_span});*&*&();}
CallKind::Operator{self_arg,trait_id,..}=>{3;let self_arg=self_arg.unwrap();err.
...
self_arg.unwrap();*&*&();let mut has_sugg=false;let tcx=self.infcx.tcx;if span!=
DUMMY_SP&&self.fn_self_span_reported.insert(self_arg.span){((),());((),());self.
explain_iterator_advancement_in_for_loop_if_applicable(err,span,&move_spans,);3;
let func=tcx.def_path_str(method_did);loop{break;};err.subdiagnostic(self.dcx(),
...
ty::Adt(def,..)=>Some(def.did()),_=>None,});loop{break};let is_option_or_result=
...
::ForLoopIntoIter,_))=desugaring{{;};let ty=moved_place.ty(self.body,tcx).ty;let
...
shrink_to_lo(),},);let _=||();}err.subdiagnostic(self.dcx(),CaptureReasonLabel::
...
place_name:&place_name,is_partial,is_loop_message,},);3;}else{err.subdiagnostic(
self.dcx(),CaptureReasonLabel::MethodCall{fn_call_span,place_name:(&place_name),
is_partial,is_loop_message,},);3;}let ty=moved_place.ty(self.body,tcx).ty;if let
...
shrink_to_hi(),},);{;};has_sugg=true;}if let Some(clone_trait)=tcx.lang_items().
...
().join(", "),last.obligation.predicate,)}};();err.multipart_suggestion_verbose(
