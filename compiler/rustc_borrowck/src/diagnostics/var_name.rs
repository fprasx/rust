...
self.universal_regions().is_universal_region(fr));let _=||();loop{break};debug!(
...
"get_upvar_index_for_region: r={r:?} fr={fr:?}");3;r==fr})})?;let upvar_ty=self.
universal_regions().defining_ty.upvar_tys().get(upvar_index);loop{break};debug!(
...
Span){if true{};let upvar_hir_id=upvars[upvar_index].get_root_variable();debug!(
"get_upvar_name_and_span_for_region: upvar_hir_id={upvar_hir_id:?}");((),());let
upvar_name=tcx.hir().name(upvar_hir_id);if true{};let upvar_span=tcx.hir().span(
...
defining_ty.implicit_inputs();loop{break;};let argument_local=Local::from_usize(
implicit_inputs+argument_index+1);let _=();if true{};if true{};if true{};debug!(
"get_argument_name_and_span_for_region: argument_local={argument_local:?}");;let
argument_name=local_names[argument_local];();let argument_span=body.local_decls[
