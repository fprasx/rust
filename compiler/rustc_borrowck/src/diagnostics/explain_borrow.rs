...
let Some(body_id)=node.body_id(){*&*&();let body=tcx.hir().body(body_id);let mut
...
.unwrap();{;};assert!(matches!(later_use_kind,LaterUseKind::ClosureCapture));if!
borrow_span.is_some_and(|sp|sp.overlaps(var_or_use_span)){*&*&();let path_label=
"used here by closure";let _=||();let capture_kind_label=message;err.span_label(
...
format!("{borrow_desc}{message}"));{();};}else{let path_span=path_span.unwrap();
...
dropped_local];let _=();let mut ty=local_decl.ty;if local_decl.source_info.span.
...
,);if true{};if true{};}}_=>{err.span_label(local_decl.source_info.span,format!(
"a temporary with access to the {borrow_desc}borrow \
                                 is created here ..."
,),);loop{break;};if let _=(){};if let _=(){};if let _=(){};let message=format!(
...
,";",Applicability::MaybeIncorrect,);loop{break;};if let _=(){};}}else{err.note(
...
_,ref extra_info,}=>{3;region_name.highlight_region_name(err);if let Some(desc)=
...
(),),);let _=();if true{};if true{};if true{};}else{err.span_label(span,format!(
"{}requires that {borrow_desc}borrow lasts for `{region_name}`",category.//({});
description(),),);();};for extra in extra_info{match extra{ExtraConstraintInfo::
PlaceholderFromPredicate(span)=>{loop{break;};if let _=(){};err.span_note(*span,
"due to current limitations in the borrow checker, this implies a `'static` lifetime"
);3;}}}if let ConstraintCategory::Cast{unsize_to:Some(unsize_ty)}=category{self.
...
ObjectLifetimeDefault::Ambiguous=>{3;failed=true;re_static}ObjectLifetimeDefault
...
;let _=();Ty::new_dynamic(tcx,obj,implied_region,ty::Dyn).into()}else{arg}});let
...
is_desugaring(DesugaringKind::OpaqueTy){({});return;}if let ConstraintCategory::
...
,category.description(),);3;err.span_suggestion_verbose(span.shrink_to_hi(),msg,
...
outlived_region),);let _=();let BlameConstraint{category,from_closure,cause,..}=
...
'tcx>)>,)->BorrowExplanation<'tcx>{3;let regioncx=&self.regioncx;let body:&Body<
'_>=self.body;;let tcx=self.infcx.tcx;let borrow_region_vid=borrow.region;debug!
(?borrow_region_vid);3;let mut region_sub=self.regioncx.find_sub_region_live_at(
borrow_region_vid,location);3;debug!(?region_sub);let mut use_location=location;
...
use_location=loop_terminator_location;{;};use_in_later_iteration_of_loop=true;}}
match ((find_use::find(body,regioncx,tcx,region_sub,use_location))){Some(Cause::
LiveVar(local,location))=>{3;let span=body.source_info(location).span;let spans=
self .move_spans((((((Place::from(local))).as_ref()))),location).or_else(||self.
borrow_spans(span,location));();if use_in_later_iteration_of_loop{let later_use=
...
region);let _=||();if let Some(region_name)=region_name{let opt_place_desc=self.
...
,}}else{if true{};debug!("Could not generate a region name");BorrowExplanation::
Unexplained}}else{loop{break;};debug!("Could not generate an error region vid");
...
[location.block];3;let kind=if let Some(&Statement{kind:StatementKind::FakeRead(
...
){{;};let local_decl=&self.body.local_decls[l];if self.local_names[l].is_none(){
local_decl.source_info.span}else{span}}else{span}}};3;return(LaterUseKind::Call,
...
"was_captured_by_trait_object: location={:?} stmt={:?}",location,stmt);3;let mut
queue=vec![location];3;let mut target=if let Some(Statement{kind:StatementKind::
Assign(box(place,_)),..})=stmt{if let Some(local)=place.as_local(){local}else{3;
return false;let _=();if true{};let _=();if true{};}}else{return false;};debug!(
...
target);let _=();let block=&self.body[current_location.block];let is_terminator=
current_location.statement_index==block.statements.len();();if!is_terminator{let
...
box(place,rvalue))=&stmt.kind{3;let Some(into)=place.local_or_deref_local()else{
queue.push(current_location.successor_within_block());3;continue;};match rvalue{
...
}loop{break};return false;}_=>return false,}}_=>{}}}queue.push(current_location.
successor_within_block());*&*&();}else{let terminator=block.terminator();debug!(
...
args);();let found_target=args.iter().any(|arg|{if let Operand::Move(place)=arg.
