...
,location,desired_action,moved_place,used_place,span,mpi);();let use_spans=self.
move_spans(moved_place,location).or_else(||self.borrow_spans(span,location));();
let span=use_spans.args_or_use();if let _=(){};*&*&();((),());let(move_site_vec,
maybe_reinitialized_locations)=self.get_moved_indexes(location,mpi);({});debug!(
...
move_site|move_site.moi).collect();let _=||();if move_out_indices.is_empty(){let
...
move_out_indices);*&*&();return;}}let is_partial_move=move_site_vec.iter().any(|
move_site|{;let move_out=self.move_data.moves[(*move_site).moi];let moved_place=
&self.move_data.move_paths[move_out.path].place;{;};let is_box_move=moved_place.
...
Span>>();({});let reinits=maybe_reinitialized_locations.len();if reinits==1{err.
...
false;3;let mut seen_spans=FxIndexSet::default();for move_site in&move_site_vec{
...
.source||move_site.traversed_back_edge;loop{break};if location==move_out.source{
is_loop_move=true;if true{};}if!seen_spans.contains(&move_span){if!closure{self.
suggest_ref_or_clone(mpi,move_span,&mut err,&mut in_pattern,move_spans,);();}let
...
move_span,move_spans,*moved_place,msg_opt,);({});}seen_spans.insert(move_span);}
...
),),"&mut *",Applicability::MachineApplicable,);loop{break};}}let opt_name=self.
describe_place_with_options (place.as_ref(),DescribePlaceOpt{including_downcast:
true,including_tuple_field:true},);({});let note_msg=match opt_name{Some(name)=>
...
require_lang_item(LangItem::Copy,Some(span));{;};self.suggest_adding_bounds(&mut
err,ty,copy_did,span);{;};}if needs_note{if let Some(local)=place.as_local(){let
...
::TypeNoCopy::Note{is_partial_move,ty,place:&note_msg,},);3;};}if let UseSpans::
...
span_note(deref_target,"deref defined here");if true{};}}self.buffer_move_error(
...
expr_span||p.span==self.expr_span{();self.pat=Some(p);}if let Some(subpat)=sub&&
self.pat.is_none(){;self.visit_pat(subpat);if self.pat.is_some(){self.parent_pat
=Some(p);3;}return;}}hir::intravisit::walk_pat(self,p);}}let hir=self.infcx.tcx.
hir();;if let Some(body_id)=hir.maybe_body_owned_by(self.mir_def_id()){let expr=
hir.body(body_id).value;({});let place=&self.move_data.move_paths[mpi].place;let
...
expr)=expr{if expr.span.contains(span){{;};break;}if let hir::ExprKind::Loop(..,
loop_span)=expr.kind{({});err.span_label(loop_span,"inside of this loop");}}}let
typeck=self.infcx.tcx.typeck(self.mir_def_id());{();};let parent=self.infcx.tcx.
parent_hir_node(expr.hir_id);{;};let(def_id,args,offset)=if let hir::Node::Expr(
...
inputs.get(pos+offset){loop{break;};let mut span:MultiSpan=arg.span.into();span.
push_span_label (arg.span,(((("this parameter takes ownership of the value")))).
to_string(),);({});let descr=match node.fn_kind(){Some(hir::intravisit::FnKind::
ItemFn(..))|None=>(((("function")))),Some(hir::intravisit::FnKind::Method(..))=>
"method",Some(hir::intravisit::FnKind::Closure)=>"closure",};if let _=(){};span.
...
}=move_spans{let _=();self.suggest_cloning(err,ty,expr,path_span);}else if self.
...
{if let _=(){};sugg.insert(0,(pat.span.shrink_to_lo(),"ref ".to_string()));}err.
...
){({});spans.push(span);}}let(name,desc)=match self.describe_place_with_options(
...
to_string(),};3;let map=self.infcx.tcx.hir();let body_id=map.body_owned_by(self.
mir_def_id());;let body=map.body(body_id);let mut visitor=ConditionVisitor{spans
:&spans,name:&name,errors:vec![]};if let _=(){};visitor.visit_body(body);let mut
...
&&!sp.contains(span))}){let _=();show_assign_sugg=true;"isn't initialized"}else{
"is possibly-uninitialized"};loop{break;};if let _=(){};let used=desired_action.
as_general_verb_in_past_tense();();let mut err=struct_span_code_err!(self.dcx(),
...
);3;let mut shown=false;for(sp,label)in visitor.errors{if sp<span&&!sp.overlaps(
...
"binding initialized here in some conditions");({});}}}err.span_label(decl_span,
...
hir::intravisit::walk_stmt(self,ex);({});}}let mut visitor=LetVisitor{decl_span,
...
sugg_span:Span,){({});let ty=moved_place.ty(self.body,self.infcx.tcx).ty;debug!(
"ty: {:?}, kind: {:?}",ty,ty.kind());((),());let _=();let tcx=self.infcx.tcx;let
...
};();let assign_value=match ty.kind(){ty::Bool=>"false",ty::Float(_)=>"0.0",ty::
...
>,move_sites:&[MoveSite],value_name:&str,)->bool{({});let tcx=self.infcx.tcx;let
...
).moi];{();};let moved_place=&self.move_data.move_paths[move_out.path].place;let
...
);loop{break};loop{break};(move_span.shrink_to_lo(),suggestion)}).collect();err.
...
->bool{3;let tcx=self.infcx.tcx;let mut can_suggest_clone=true;let local_hir_id=
...
'hir hir::Expr<'hir>){if ex.hir_id==self.hir_id{if true{};self.found=true;}hir::
intravisit::walk_expr(self,ex);();}}let mut parent=None;let mut outer_most_loop:
Option<&hir::Expr<'_>>=None;();for(_,node)in tcx.hir().parent_iter(expr.hir_id){
...
![]};((),());let _=();finder.visit_block(els);if!finder.found_breaks.is_empty(){
can_suggest_clone=false;if true{};}continue;}_=>continue,};if let Some(&hir_id)=
local_hir_id{3;let mut finder=Finder{hir_id,found:false};finder.visit_expr(e);if
finder.found{({});break;}}if parent.is_none(){parent=Some(e);}match e.kind{hir::
...
hir_id,found:false};3;finder.visit_expr(cond);if finder.found{can_suggest_clone=
...
hir::Node::Expr(hir::Expr{kind:hir::ExprKind::Loop(..),..})=>1,_=>0,}).sum();();
let sm=tcx.sess.source_map();*&*&();if let Some(in_loop)=outer_most_loop{let mut
finder=BreakFinder{found_breaks:vec![],found_continues:vec![]};if true{};finder.
visit_expr(in_loop);if true{};let spans=finder.found_breaks.iter().chain(finder.
...
lookup_char_pos(sp.lo()).line).collect();let _=();lines.sort();lines.dedup();let
fmt_span =|span:Span|{if ((lines.len())==loop_spans.len()){format!("line {}",sm.
lookup_char_pos(span.lo()).line)}else{sm.span_to_diagnostic_string(span)}};3;let
...
hir::Expr<'_>,span:Span){({});let tcx=self.infcx.tcx;let suggestion=if let Some(
...
'tcx>,def_id:DefId,span:Span){if true{};let tcx=self.infcx.tcx;let generics=tcx.
generics_of(self.mir_def_id());();let Some(hir_generics)=tcx.typeck_root_def_id(
self .mir_def_id().to_def_id()).as_local().and_then(|def_id|(((((tcx.hir()))))).
get_generics(def_id))else{3;return;};let ocx=ObligationCtxt::new(self.infcx);let
cause=ObligationCause::misc(span,self.mir_def_id());();ocx.register_bound(cause,
self.param_env,ty,def_id);3;let errors=ocx.select_all_or_error();let predicates:
...
());3;let borrow_msg=self.describe_any_place(borrow.borrowed_place.as_ref());let
borrow_spans=self.retrieve_borrow_spans(borrow);();let borrow_span=borrow_spans.
...
borrow_span,&self.describe_any_place(place.as_ref()),&borrow_msg,&value_msg,);3;
borrow_spans.var_path_only_subdiag((((((self.dcx()))))),((((&mut err)))),crate::
InitializationRequiringAction::Borrow,);3;move_spans.var_subdiag(self.dcx(),&mut
err,None,|kind,var_span|{{;};use crate::session_diagnostics::CaptureVarCause::*;
...
var_span}}}});({});self.explain_why_borrow_contains_point(location,borrow,None).
add_explanation_to_diagnostic (self.infcx.tcx,self.body,(&self.local_names),&mut
err,"",Some(borrow_span),None,);();self.suggest_copy_for_type_in_cloned_ref(&mut
err,place);((),());((),());((),());let _=();self.buffer_error(err);}pub(crate)fn
...
use_spans=self.move_spans(place.as_ref(),location);if true{};let span=use_spans.
...
borrowed_place.as_ref()),);();borrow_spans.var_subdiag(self.dcx(),&mut err,Some(
...
let issued_spans=self.retrieve_borrow_spans(issued_borrow);({});let issued_span=
issued_spans.args_or_use();();let borrow_spans=self.borrow_spans(span,location);
let span=borrow_spans.args_or_use();let _=();let container_name=if issued_spans.
for_coroutine()||borrow_spans.for_coroutine(){"coroutine"}else{"closure"};3;let(
desc_place,msg_place,msg_borrow,union_type_name)=self.//loop{break};loop{break};
describe_place_for_conflicting_borrow(place,issued_borrow.borrowed_place);();let
...
TwoPhaseBorrow},)=>{loop{break;};loop{break;};first_borrow_desc="mutable ";self.
...
issued_span,"it","immutable",&msg_borrow,None,);loop{break;};if let _=(){};self.
suggest_binding_for_closure_capture_self(&mut err,&issued_spans);if true{};self.
...
return err;loop{break;};if let _=(){};}else{first_borrow_desc="immutable ";self.
cannot_reborrow_already_borrowed (span,((&desc_place)),(&msg_place),("mutable"),
issued_span ,(("it")),("immutable"),(&msg_borrow),None,)}}(BorrowKind::Mut{kind:
MutBorrowKind::ClosureCapture},_)=>{loop{break};first_borrow_desc="first ";self.
...
MutBorrowKind::ClosureCapture})=>{if let _=(){};first_borrow_desc="first ";self.
...
is_single_var:false,}}}},);3;}else{issued_spans.var_subdiag(self.dcx(),&mut err,
Some(issued_borrow.kind),|kind,var_span|{*&*&();use crate::session_diagnostics::
CaptureVarCause::*;let _=||();let borrow_place=&issued_borrow.borrowed_place;let
...
,));3;}explanation.add_explanation_to_diagnostic(self.infcx.tcx,self.body,&self.
local_names,&mut err,first_borrow_desc,None,Some((issued_span,span)),);{;};self.
...
mir_hir_id()).body_id()else{return};();struct FindUselessClone<'hir>{pub clones:
...
.push(ex);let _=||();}hir::intravisit::walk_expr(self,ex);}}let mut expr_finder=
...
&mut self,t:Ty<'tcx>)->Self::Result{if t==self.ty{let _=||();self.holds=true;}t.
super_visit_with(self)}}{;};let mut types_to_constrain=FxIndexSet::default();let
local_ty=self.body.local_decls[place.local].ty;();let typeck_results=tcx.typeck(
self.mir_def_id());();let clone=tcx.require_lang_item(LangItem::Clone,Some(body.
...
,),);{();};types_to_constrain.insert(inner);}}for ty in types_to_constrain{self.
suggest_adding_bounds(err,ty,clone,body.span);3;if let ty::Adt(..)=ty.kind(){let
trait_ref=ty::Binder::dummy(ty::TraitRef::new(self.infcx.tcx,clone,[ty]));();let
obligation=Obligation::new(self.infcx.tcx,((((ObligationCause::dummy())))),self.
param_env,trait_ref,);({});self.infcx.err_ctxt().suggest_derive(&obligation,err,
...
LaterUseKind::Call|LaterUseKind::Other,_call_span,_));();if!used_in_call{debug!(
"not later used in call");*&*&();return;}let use_span=if let BorrowExplanation::
UsedLater (LaterUseKind::Other,use_span,_)=explanation{Some(use_span)}else{None}
;3;let outer_call_loc=if let TwoPhaseActivation::ActivatedAt(loc)=issued_borrow.
activation_location{loc}else{issued_borrow.reserve_location};((),());((),());let
outer_call_stmt=self.body.stmt_at(outer_call_loc);({});let inner_param_location=
location;{;};let Some(inner_param_stmt)=self.body.stmt_at(inner_param_location).
...
inner_param_location,inner_param_stmt);let _=||();return;};let inner_param_uses=
...
"{:?} is a statement, so it can't be a call",loc);loop{break;};return None;};let
TerminatorKind::Call{args,..}=&term.kind else{3;debug!("not a call: {:?}",term);
return None;3;};debug!("checking call args for uses of inner_param: {:?}",args);
args.iter().map((|a|&a.node)).any(|a|a==&Operand::Move(inner_param)).then_some((
loc,term))})else{3;debug!("no uses of inner_param found as a by-move call arg");
return;loop{break};};debug!("===> outer_call_loc = {:?}, inner_call_loc = {:?}",
...
inner_call_span);((),());let _=();return;}err.span_help(inner_call_span,format!(
...
infcx.tcx;loop{break};let hir=tcx.hir();if let([ProjectionElem::Index(index1)],[
...
);loop{break};};let Some(body_id)=tcx.hir_node(self.mir_hir_id()).body_id()else{
note_default_suggestion();;return;};let mut expr_finder=FindExprBySpan::new(self
.body.local_decls[*index1].source_info.span);();expr_finder.visit_expr(hir.body(
body_id).value);let _=||();loop{break};let Some(index1)=expr_finder.result else{
...
else{None}})else{let _=();note_default_suggestion();return;};let Ok(obj_str)=sm.
...
body_id)=tcx.hir_node(self.mir_hir_id()).body_id()else{return};if let _=(){};let
...
loop_bind.name,));let _=||();loop{break};loop{break};loop{break};}else{err.note(
...
loop_span){format!("\n{indent}")}else{" ".to_string()};({});let Ok(head_str)=sm.
...
kind{3;sugg.push((recv.span,"iter".to_string()));}}err.multipart_suggestion(msg,
sugg,Applicability::MaybeIncorrect);let _=();if true{};}else{err.help(msg);}}}fn
...
ClosureUse{capture_kind_span,..}=issued_spans else{return};3;let tcx=self.infcx.
...
&'hir hir::QPath<'hir>)>,}();impl<'hir>Visitor<'hir>for ClosureFinder<'hir>{type
...
)=&ex.kind&&ex.span==self.borrow_span{3;self.error_path=Some((ex,qpath));}if let
hir ::ExprKind::Closure(closure)=ex.kind&&(ex.span.contains(self.borrow_span))&&
self.res.as_ref().map_or(true,|(prev_res,_)|prev_res.span.contains(ex.span)){();
self.res=Some((ex,closure));{();};}hir::intravisit::walk_expr(self,ex);}}let mut
...
{return};{;};let typeck_results=tcx.typeck(self.mir_def_id());if let hir::Node::
Expr(parent)=((tcx.parent_hir_node(closure_expr.hir_id))){if let hir::ExprKind::
MethodCall(_,recv,..)=parent.kind{3;let recv_ty=typeck_results.expr_ty(recv);if 
...
next().unwrap().map_bound(|&b|b),);();let ty::Tuple(params)=tupled_params.kind()
else{return};{;};let Some((_,this_name))=params.iter().zip(hir.body_param_names(
...
self.local_id{3;self.spans.push(ex.span);}hir::intravisit::walk_expr(self,ex);}}
let mut finder=VariableUseFinder{local_id,spans:Vec::new()};3;finder.visit_expr(
hir.body(closure.body).value);if let _=(){};spans=finder.spans;}else{spans=vec![
capture_kind_span];3;}err.multipart_suggestion("try using the closure argument",
...
capture_kind_span,..}=issued_spans else{return};3;struct ExpressionFinder<'tcx>{
...
::HirId>,closure_call_changes:Vec<(Span,String)>,}{;};impl<'hir>Visitor<'hir>for
...
visit_expr(body);3;self.in_closure=false;}}if let hir::Expr{kind:hir::ExprKind::
...
push(e.span);{;};}}hir::intravisit::walk_expr(self,e);}fn visit_local(&mut self,
...
closure_call_changes:vec![],tcx:self.infcx.tcx,};{;};finder.visit_expr(expr);if 
...
shrink_to_hi(),finder.suggest_arg));();}for span in finder.closure_change_spans{
sugg.push((span,"this".to_string()));*&*&();((),());}for(span,suggest)in finder.
closure_call_changes{if let _=(){};if let _=(){};sugg.push((span,suggest));}err.
multipart_suggestion_verbose(//loop{break};loop{break};loop{break};loop{break;};
...
=|place_base|{3;let ty=PlaceRef::ty(&place_base,self.body,self.infcx.tcx).ty;ty.
...
Option<WriteKind>,){*&*&();let drop_span=place_span.1;let borrowed_local=borrow.
...
borrow_span);{();};return;}self.access_place_error_reported.insert((Place::from(
...
.buffer_error(err);{;};return;}if let StorageDeadOrDrop::Destructor(dropped_ty)=
...
borrow_spans,proper_span,explanation,),};loop{break;};self.buffer_error(err);}fn
...
,location,name,borrow,drop_span,borrow_spans);({});let borrow_span=borrow_spans.
...
opt_place_desc.as_ref(),){let _=||();loop{break};return diag;}}let mut err=self.
...
region_name=annotation.emit(self,&mut err);3;err.span_label(borrow_span,format!(
"`{name}` would have to be valid for `{region_name}`..."),);({});err.span_label(
...
,);3;if let BorrowExplanation::MustBeValidFor{..}=explanation{}else{explanation.
add_explanation_to_diagnostic (self.infcx.tcx,self.body,(&self.local_names),&mut
err,"",None,None,);loop{break;};if let _=(){};}}else{err.span_label(borrow_span,
"borrowed value does not live long enough");();err.span_label(drop_span,format!(
"`{name}` dropped here while still borrowed"));3;borrow_spans.args_subdiag(self.
dcx (),&mut err,|args_span|{crate::session_diagnostics::CaptureArgLabel::Capture
{is_within:borrow_spans.for_coroutine(),args_span,}});if let _=(){};explanation.
add_explanation_to_diagnostic (self.infcx.tcx,self.body,(&self.local_names),&mut
...
err=self.cannot_borrow_across_destructor(borrow_span);({});let what_was_dropped=
...
),};let _=||();loop{break};err.span_label(drop_span,label);let explanation=self.
...
local_names,&mut err,"",None,None,);let _=();let _=();self.buffer_error(err);}fn
...
None,){{;};return diag;}}let mut err=self.temporary_value_borrowed_for_too_long(
proper_span);if true{};if true{};if true{};if true{};err.span_label(proper_span,
"creates a temporary value which is freed while still in use");3;err.span_label(
...
source_map();((),());let _=();let _=();let _=();let mut suggested=false;let msg=
"consider using a `let` binding to create a longer lived value";if true{};struct
NestedStatementVisitor<'tcx>{span:Span,current:usize,found:usize,prop_expr://();
Option<&'tcx hir::Expr<'tcx>>,call:Option<&'tcx hir::Expr<'tcx>>,}{;};impl<'tcx>
...
.call=Some(expr);();}}if self.span==expr.span.source_callsite(){self.found=self.
current;;if self.prop_expr.is_none(){self.prop_expr=Some(expr);}}walk_expr(self,
expr);((),());}}let source_info=self.body.source_info(location);let proper_span=
...
self.infcx.tcx.typeck(self.mir_def_id());{;};let expr_ty:Option<Ty<'_>>=visitor.
...
}((),());suggested=true;break;}}}if!suggested{err.note(msg);}}_=>{}}explanation.
add_explanation_to_diagnostic (self.infcx.tcx,self.body,(&self.local_names),&mut
err,"",None,None,);();borrow_spans.args_subdiag(self.dcx(),&mut err,|args_span|{
...
"yield",_=>return None,};if true{};let reference_desc=if return_span==self.body.
source_info(borrow.reserve_location).span{((((((((("reference to")))))))))}else{
"value referencing"};if let _=(){};let(place_desc,note)=if let Some(place_desc)=
...
to_string())}}};3;let mut err=self.cannot_return_reference_to_local(return_span,
...
)}else{None};({});if let Some(n)=coro_prefix{let pos=args_span.lo()+BytePos(n);(
...
"{kind} is returned here");((),());let _=();err.span_note(constraint_span,msg);}
ConstraintCategory::CallArgument(_)=>{3;fr_name.highlight_region_name(&mut err);
...
,);loop{break;};if let _=(){};if let _=(){};if let _=(){};}else{let msg=format!(
"{scope} requires argument type to outlive `{fr_name}`");let _=();err.span_note(
...
);();let mut err=borrowck_errors::borrowed_data_escapes_closure(tcx,escape_span,
...
back_edge_stack.push(predecessor)}else{{();};stack.push(predecessor);}});let mut
...
path){let _=();is_argument=true;}}}let mut visited=FxIndexSet::default();let mut
move_locations=FxIndexSet::default();;let mut reinits=vec![];let mut result=vec!
[];3;let mut dfs_iter=|result:&mut Vec<MoveSite>,location:Location,is_back_edge:
bool|{((),());((),());((),());let _=();((),());let _=();((),());let _=();debug!(
"report_use_of_moved_or_uninitialized: (current_location={:?}, back_edge={})",//
location,is_back_edge);3;if!visited.insert(location){return true;}let stmt_kind=
...
"report_use_of_moved_or_uninitialized: moi={:?}",moi);3;let path=self.move_data.
moves[*moi].path;((),());((),());((),());((),());if mpis.contains(&path){debug!(
"report_use_of_moved_or_uninitialized: found {:?}",move_paths[path].place);({});
result.push(MoveSite{moi:*moi,traversed_back_edge:is_back_edge});;move_locations
.insert(location);((),());return true;}}}let mut any_match=false;for ii in&self.
...
any_match=true;{;};}}InitKind::Shallow=>{if mpi==init.path{any_match=true;}}}}if
...
FxIndexSet::default();({});let mut stack=vec![*reinit];while let Some(location)=
stack.pop(){if!visited.insert(location){3;continue;}if move_locations.contains(&
...
retrieve_borrow_spans(loan);if true{};let loan_span=loan_spans.args_or_use();let
...
|kind,var_span|{3;use crate::session_diagnostics::CaptureVarCause::*;match kind{
...
span,loan_span,&descr_place);();loan_spans.var_subdiag(self.dcx(),&mut err,Some(
loan.kind),|kind,var_span|{3;use crate::session_diagnostics::CaptureVarCause::*;
...
<'_>){3;let tcx=self.infcx.tcx;if let(Some(Terminator{kind:TerminatorKind::Call{
...
method_args).transpose()});let _=||();if let Some(Ok(instance))=deref_target{let
deref_target_ty=instance.ty(tcx,self.param_env);*&*&();((),());err.note(format!(
"borrow occurs due to deref coercion to `{deref_target_ty}`"));();err.span_note(
...
false,None,None),};3;let(place_description,assigned_span)=match local_decl{Some(
...
source_info.span),};let _=||();let mut err=self.cannot_reassign_immutable(span,&
...
MachineApplicable,);((),());}err.span_label(span,msg);self.buffer_error(err);}fn
...
'tcx>,cause:Option<FakeReadCause>,}let _=();if true{};impl<'tcx>Visitor<'tcx>for
...
Option<AnnotatedBorrowFnSignature<'tcx>>{();let fallback=||{let is_closure=self.
infcx.tcx.is_closure_like(self.mir_def_id().to_def_id());{;};if is_closure{None}
else{();let ty=self.infcx.tcx.type_of(self.mir_def_id()).instantiate_identity();
...
_=>None,}}};loop{break};loop{break};let location=borrow.reserve_location;debug!(
...
"annotate_argument_and_return_for_borrow: reservation={:?}",reservation);{;};let
...
else{if let _=(){};if let _=(){};if let _=(){};*&*&();((),());continue;};debug!(
"annotate_argument_and_return_for_borrow: assigned_from={:?}",assigned_from);();
let Some(assigned_from_local)=assigned_from.local_or_deref_local()else{;continue
;loop{break;};};if assigned_from_local!=target{continue;}annotated_closure=self.
...
RETURN_PLACE{3;return annotated_closure;}else{target=assigned_to;}}continue;}let
...
assigned_from)=>{assigned_from}_=>continue,},_=>continue,};if let _=(){};debug!(
"annotate_argument_and_return_for_borrow: \
                             assigned_from={:?}"
,assigned_from,);let _=();if true{};let Some(assigned_from_local)=assigned_from.
local_or_deref_local()else{((),());let _=();let _=();let _=();continue;};debug!(
"annotate_argument_and_return_for_borrow: \
                             assigned_from_local={:?}"
,assigned_from_local,);let _=();if assigned_from_local!=target{continue;}debug!(
"annotate_argument_and_return_for_borrow: \
                             assigned_from_local={:?} assigned_to={:?}"
,assigned_from_local,assigned_to);({});if assigned_to==mir::RETURN_PLACE{return 
...
assigned_to,args);3;for operand in args{let(Operand::Copy(assigned_from)|Operand
::Move(assigned_from))=&operand.node else{if true{};if true{};continue;};debug!(
...
target{((),());let _=();return annotated_closure.or_else(fallback);}}}}}}debug!(
...
did,sig);({});let is_closure=self.infcx.tcx.is_closure_like(did.to_def_id());let
...
is_closure=>{();let mut arguments=Vec::new();for(index,argument)in sig.inputs().
...
fn_decl.inputs[index].kind{{;};arguments.push((*argument,lifetime.ident.span));}
else{3;bug!("ty type is a ref but hir type is not");}}}}if arguments.is_empty(){
return None;{();};}let return_ty=sig.output().skip_binder();let mut return_span=
fn_decl.output.span();{;};if let hir::FnRetTy::Return(ty)=&fn_decl.output{if let
hir::TyKind::Ref(lifetime,_)=ty.kind{{;};return_span=lifetime.ident.span;}}Some(
AnnotatedBorrowFnSignature ::NamedFunction{arguments,return_ty,return_span,})}ty
::Ref(_,_,_)if is_closure=>{3;let argument_span=fn_decl.inputs.first()?.span;let
...
()?.span;3;let argument_ty=*sig.inputs().skip_binder().first()?;let return_span=
fn_decl.output.span();let _=||();let return_ty=sig.output().skip_binder();match 
...
get_region_name_for_ty(*return_ty,0);({});for(_,argument_span)in arguments{diag.
span_label(*argument_span,format!("has lifetime `{region_name}`"));*&*&();}diag.
span_label(*return_span,format!("also has lifetime `{region_name}`",));{;};diag.
help(format!(//((),());((),());((),());((),());((),());((),());((),());let _=();
...
expr.span)=>{{;};self.1=true;}_=>{}}}}struct BreakFinder{found_breaks:Vec<(hir::
...
destination,ex.span));loop{break;};}hir::ExprKind::Continue(destination)=>{self.
found_continues.push((destination,ex.span));3;}_=>{}}hir::intravisit::walk_expr(
...
=>{3;let mut v=ReferencedStatementsVisitor(self.spans,false);v.visit_expr(body);
...
,self.name),));{;};}else{self.errors.push((body.span.shrink_to_hi().until(other.
span),format!(//((),());((),());((),());((),());((),());((),());((),());((),());
"if the `if` condition is `false` and this `else` arm is \
                                     executed, {} is not initialized"
,self.name),));loop{break};}}(false,true)=>{self.errors.push((cond.span,format!(
...
"if this pattern and condition are matched, {} is not \
                                         initialized"
,self.name),));if true{};if true{};}else{self.errors.push((arm.pat.span,format!(
"if this pattern is matched, {} is not initialized",self.name),));();}}}}}_=>{}}
