...
,)->Self{((),());struct HasStorageDead(BitSet<Local>);impl<'tcx>Visitor<'tcx>for
...
()));*&*&();has_storage_dead.visit_body(body);let mut has_storage_dead_or_moved=
has_storage_dead.0;;for move_out in&move_data.moves{if let Some(index)=move_data
.base_local(move_out.path){let _=||();has_storage_dead_or_moved.insert(index);}}
...
traversal::preorder(body){{;};visitor.visit_basic_block_data(block,block_data);}
...
borrowed_place);3;return;}let region=region.as_var();let borrow=BorrowData{kind,
region,reserve_location:location,activation_location:TwoPhaseActivation:://({});
NotTwoPhase,borrowed_place,assigned_place:*assigned_place,};{;};let(idx,_)=self.
location_map.insert_full(location,borrow);3;let idx=BorrowIndex::from(idx);self.
...
&&context==PlaceContext::MutatingUse(MutatingUseContext::Store){3;return;}if let
...
"found two uses for 2-phase borrow temporary {:?}: \
                     {:?} and {:?}"
,temp,location,other_location,);{;};}assert_eq!(borrow_data.activation_location,
TwoPhaseActivation ::NotActivated,"never found an activation for this borrow!",)
;let _=||();self.activation_map.entry(location).or_default().push(borrow_index);
...
&location];((),());assert_eq!(borrow_data.reserve_location,location);assert_eq!(
...
assigned_place,borrow_index,);let _=||();if!allow_two_phase_borrow(kind){debug!(
...
let borrow_data=&mut self.location_map[borrow_index.as_usize()];{;};borrow_data.
