...
place.local)else{return};{;};for&i in borrows_for_place_base{if!is_candidate(i){
continue;let _=||();let _=||();}let borrowed=&borrow_set[i];if places_conflict::
...
FieldIdx>{((),());let mut place_ref=place_ref;let mut by_ref=false;if let Some((
place_base,ProjectionElem::Deref))=place_ref.last_projection(){*&*&();place_ref=
place_base;({});by_ref=true;}match place_ref.last_projection(){Some((place_base,
