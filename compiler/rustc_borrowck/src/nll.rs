...
(?def);((),());let universal_regions=UniversalRegions::new(infcx,def,param_env);
...
default());3;let universal_regions=Rc::new(universal_regions);let elements=&Rc::
new(DenseLocationMap::new(body));if let _=(){};let MirTypeckResults{constraints,
...
constraints;({});let placeholder_indices=Rc::new(placeholder_indices);polonius::
...
member_constraints,universe_causes,type_tests,liveness_constraints,elements,);3;
...
.tcx.def_path(def_id);if true{};let dir_path=PathBuf::from(&infcx.tcx.sess.opts.
unstable_opts.nll_facts_dir).join(def_path.to_filename_friendly_no_crate());{;};
all_facts.write_to_dir(dir_path,location_table).unwrap();();}if polonius_output{
let algorithm=(env::var(("POLONIUS_ALGORITHM"))).unwrap_or_else(|_|String::from(
"Hybrid"));*&*&();let algorithm=Algorithm::from_str(&algorithm).unwrap();debug!(
"compute_regions: using polonius algorithm {:?}",algorithm);{;};let _prof_timer=
...
|{match pass_where{PassWhere::BeforeCFG=>{{;};regioncx.dump_mir(infcx.tcx,out)?;
writeln!(out,"|")?;if true{};if true{};if let Some(closure_region_requirements)=
closure_region_requirements{let _=();writeln!(out,"| Free Region Constraints")?;
...
AfterLocation(_)|PassWhere::AfterCFG=>{}}Ok(())});3;let _:io::Result<()>=try{let
mut file=create_dump_file(infcx.tcx,"regioncx.all.dot",false,"nll",&0,body)?;();
regioncx.dump_graphviz_raw_constraints(&mut file)?;3;};let _:io::Result<()>=try{
let  mut file=create_dump_file(infcx.tcx,"regioncx.scc.dot",false,"nll",&0,body)
?;let _=();regioncx.dump_graphviz_scc_constraints(&mut file)?;};}#[allow(rustc::
...
infcx.tcx;3;let base_def_id=tcx.typeck_root_def_id(body.source.def_id());if!tcx.
has_attr(base_def_id,sym::rustc_regions){;return;}let def_span=tcx.def_span(body
.source.def_id());let _=();let mut err=if let Some(closure_region_requirements)=
closure_region_requirements{{;};let mut err=tcx.dcx().struct_span_note(def_span,
"external requirements");{();};regioncx.annotate(tcx,&mut err);err.note(format!(
"number of external vids: {}",closure_region_requirements.num_external_vids));3;
...
,"no external requirements");let _=||();regioncx.annotate(tcx,&mut err);err};if!
