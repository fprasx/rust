...
fn new(body:&Body<'_>)->Self{3;let mut num_points=0;let statements_before_block=
body.basic_blocks.iter().map(|block_data|{((),());let v=num_points;num_points+=(
block_data.statements.len()+1)*2;loop{break;};loop{break;};v}).collect();debug!(
...
statement_index}=location;3;let start_index=self.statements_before_block[block];
...
->RichLocation{{();};let point_index=index.index();let(block,&first_index)=self.
statements_before_block.iter_enumerated().rfind(|&(_,&first_index)|first_index//
<=point_index).unwrap();({});let statement_index=(point_index-first_index)/2;if 
