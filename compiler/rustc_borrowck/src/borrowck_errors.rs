...
" (via {msg})")};{;};let mut err=struct_span_code_err!(self.dcx(),new_loan_span,
...
via(opt_via),opt_via,),);3;if let Some(old_load_end_span)=old_load_end_span{err.
...
opt_via)),);{;};if let Some(old_load_end_span)=old_load_end_span{err.span_label(
...
"closures are constructed here in different iterations of loop",);{;};}else{err.
span_label(old_loan_span,"first closure is constructed here");();err.span_label(
...
"{container_name} construction occurs here{opt_via}"),);let _=();err.span_label(
...
"{second_borrow_desc}borrow occurs here{opt_via}"));loop{break;};err.span_label(
old_loan_span ,format!("{container_name} construction occurs here{old_opt_via}")
,);loop{break;};if let Some(previous_end_span)=previous_end_span{err.span_label(
...
msg_new),kind_new,noun_old,kind_old,via(msg_old),);if true{};if msg_new==""{err.
span_label(span,format!("{kind_new} borrow occurs here"));*&*&();err.span_label(
old_span,format!("{kind_old} borrow occurs here"));();}else{err.span_label(span,
format!(//((),());let _=();let _=();let _=();((),());let _=();let _=();let _=();
"{kind_new} borrow of {msg_new} -- which overlaps with {msg_old} -- occurs here"
,),);{;};err.span_label(old_span,format!("{} borrow occurs here{}",kind_old,via(
msg_old)));{;};}if let Some(old_load_end_span)=old_load_end_span{err.span_label(
...
'tcx>{3;let coroutine_kind=self.body.coroutine.as_ref().unwrap().coroutine_kind;
