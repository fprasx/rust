...
self.rev_scc_graph.is_some(){3;return;}let graph=self.constraint_sccs.reverse();
let  mut paired_scc_regions=((self.universal_regions.universal_regions())).map(|
region|(self.constraint_sccs.scc(region),region)).collect::<Vec<_>>();if true{};
...
start+=chunk.len();3;}self.rev_scc_graph=Some(ReverseSccGraph{graph,scc_regions,
