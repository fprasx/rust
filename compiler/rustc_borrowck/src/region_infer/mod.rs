...
reg_vars_to_origins_str);3;let num_components=sccs.scc_data().ranges().len();let
...
from_usize(reg_var_idx);3;let origin=var_to_origin.get(&reg_var).unwrap_or_else(
||&RegionCtxt::Unknown);;components[scc_idx.as_usize()].insert((reg_var,*origin)
);({});}let mut components_str="strongly connected components:".to_string();for(
...
regions_info,))}({});debug!("{}",components_str);let components_representatives=
...
scc_idx]).collect::<Vec<_>>();if true{};scc_node_to_edges.insert((scc_idx,repr),
edge_representatives);3;}debug!("SCC edges {:#?}",scc_node_to_edges);}impl<'tcx>
...
"outlives constraints: {:#?}",outlives_constraints);if true{};let _=||();debug!(
"placeholder_indices: {:#?}",placeholder_indices);();debug!("type tests: {:#?}",
type_tests);let _=||();let definitions:IndexVec<_,_>=var_infos.iter().map(|info|
RegionDefinition::new(info.universe,info.origin)).collect();{;};let constraints=
...
constraint_sccs.scc(region);if let _=(){};scc_values.merge_liveness(scc,region,&
liveness_constraints);if true{};}let scc_universes=Self::compute_scc_universes(&
...
universal_region_relations,};({});result.init_free_and_bound_regions();result}fn
...
ConstraintSccIndex,ty::UniverseIndex>{3;let num_sccs=constraint_sccs.num_sccs();
let mut scc_universes=IndexVec::from_elem_n(ty::UniverseIndex::MAX,num_sccs);();
debug!("compute_scc_universes()");if true{};for(region_vid,region_definition)in 
definitions.iter_enumerated(){*&*&();let scc=constraint_sccs.scc(region_vid);let
scc_universe=&mut scc_universes[scc];((),());let _=();let scc_min=std::cmp::min(
region_definition.universe,*scc_universe);let _=||();if scc_min!=*scc_universe{*
...
scc_a];3;let scc_universe_b=scc_universes[scc_b];let scc_universe_min=std::cmp::
min(scc_universe_a,scc_universe_b);let _=();if scc_universe_a!=scc_universe_min{
...
(){{;};let repr=&mut scc_representatives[constraints_scc.scc(vid)];if*repr==ty::
...
scc_universes[scc];let _=();if scc_universe.can_name(placeholder.universe){self.
scc_values.add_element(scc,placeholder);loop{break;};if let _=(){};}else{debug!(
"init_free_and_bound_regions: placeholder {:?} is \
                             not compatible with universe {:?} of its SCC {:?}"
,placeholder,scc_universe,scc,);if true{};self.add_incompatible_universe(scc);}}
...
errors_buffer=RegionErrors::new(infcx.tcx);3;let mut outlives_requirements=infcx
.tcx.is_typeck_child(mir_def_id).then(Vec::new);{;};self.check_type_tests(infcx,
...
"Polonius output is unavailable despite `-Z polonius`"),);let _=||();}else{self.
check_universal_regions(outlives_requirements.as_mut(),&mut errors_buffer);{;};}
debug!(?errors_buffer);loop{break};loop{break};if errors_buffer.is_empty(){self.
check_member_constraints(infcx,&mut errors_buffer);3;}debug!(?errors_buffer);let
...
}}if true{};let member_constraints=self.member_constraints.clone();for m_c_i in 
...
choice_regions.to_vec();loop{break;};for c_r in&mut choice_regions{let scc=self.
constraint_sccs.scc(*c_r);if true{};*c_r=self.scc_representatives[scc];}if self.
scc_universes[scc]!=ty::UniverseIndex::ROOT{let _=();return;}debug_assert!(self.
...
outlives(o_r,lb))});let _=||();let _=||();debug!(?choice_regions,"after lb");let
...
retain(|&o_r|universal_region_relations.outlives(ub,o_r));loop{break;};}debug!(?
choice_regions,"after ub");{;};let totally_ordered_subset=choice_regions.iter().
...
let r1_outlives_r2=self.universal_region_relations.outlives(r1,r2);if true{};let
...
debug!("no unique minimum choice");loop{break};return;};let min_choice_scc=self.
constraint_sccs.scc(min_choice);{;};debug!(?min_choice,?min_choice_scc);if self.
...
scc_universes[scc_a];3;if universe_a.can_name(self.scc_universes[scc_b]){return 
...
ConstraintSccIndex){*&*&();debug!("add_incompatible_universe(scc={:?})",scc);let
...
&self.type_tests{{();};debug!("check_type_test: {:?}",type_test);let generic_ty=
type_test.generic_kind.to_ty(tcx);();if self.eval_verify_bound(infcx,generic_ty,
type_test.lower_bound,&type_test.verify_bound,){if true{};continue;}if let Some(
...
continue;let _=||();}}let erased_generic_kind=infcx.tcx.erase_regions(type_test.
...
bool{loop{break};let tcx=infcx.tcx;let TypeTest{generic_kind,lower_bound,span:_,
verify_bound:_}=type_test;{();};let generic_ty=generic_kind.to_ty(tcx);let Some(
subject)=self.try_promote_type_test_subject(infcx,generic_ty)else{;return false;
};let _=();debug!("subject = {:?}",subject);let r_scc=self.constraint_sccs.scc(*
lower_bound);3;debug!("lower_bound = {:?} r_scc={:?} universe={:?}",lower_bound,
r_scc,self.scc_universes[r_scc]);((),());((),());if let Some(p)=self.scc_values.
placeholders_contained_in(r_scc).next(){((),());((),());((),());let _=();debug!(
"encountered placeholder in higher universe: {:?}, requiring 'static",p);{;};let
...
generic_ty,ur,&type_test.verify_bound){let _=();continue;}let non_local_ub=self.
universal_region_relations.non_local_upper_bounds(ur);let _=();if true{};debug!(
...
ConstraintCategory::Boring,};({});debug!("try_promote_type_test: pushing {:#?}",
requirement);*&*&();propagated_outlives_requirements.push(requirement);}}true}#[
...
as _;3;let tcx=self.tcx;let&ty::Alias(ty::Opaque,ty::AliasTy{args,def_id,..})=t.
...
self),}});{;};Ty::new_opaque(tcx,def_id,tcx.mk_args_from_iter(args))}}let ty=ty.
...
true;{();};r})});debug!("try_promote_type_test_subject: folded ty = {:?}",ty);if
failed{3;return None;}Some(ClosureOutlivesSubject::Ty(ClosureOutlivesSubjectTy::
...
::OutlivedBy(r)=>{{;};let r_vid=self.to_region_vid(*r);self.eval_outlives(r_vid,
...
vid);3;let repr=self.scc_representatives[scc];ty::Region::new_var(tcx,repr)})}fn
...
self.universal_regions.is_universal_region(sub_region),);{;};let sub_region_scc=
self.constraint_sccs.scc(sub_region);();let sup_region_scc=self.constraint_sccs.
scc(sup_region);({});if!self.universe_compatible(sub_region_scc,sup_region_scc){
debug!(//((),());let _=();let _=();let _=();let _=();let _=();let _=();let _=();
"sub universe `{sub_region_scc:?}` is not nameable \
                by super `{sup_region_scc:?}`, promoting to static"
,);3;return self.eval_outlives(sup_region,self.universal_regions.fr_static);}let
...
r2|self.universal_region_relations.outlives(r2,r1))});{;};if!universal_outlives{
debug!("sub region contains a universal region not present in super");();return 
false;((),());}if self.universal_regions.is_universal_region(sup_region){debug!(
"super is universal and hence contains all points");((),());return true;}debug!(
...
definitions.iter_enumerated(){();debug!(?fr,?fr_definition);match fr_definition.
...
ClosureOutlivesRequirement<'tcx>>>,errors_buffer:&mut RegionErrors<'tcx>,){3;let
longer_fr_scc=self.constraint_sccs.scc(longer_fr);();assert!(self.scc_universes[
longer_fr_scc]==ty::UniverseIndex::ROOT);let _=();debug_assert!(self.scc_values.
placeholders_contained_in(longer_fr_scc).next().is_none());3;let representative=
...
NllRegionVariableOrigin::FreeRegion,is_reported:true,});((),());}return;}let mut
...
is_reported:!error_reported,});loop{break};loop{break};error_reported=true;}}}fn
...
universal_region_relations.non_local_upper_bounds(shorter_fr);let _=||();debug!(
...
blame_span_category.0,});*&*&();}return RegionRelationCheckResult::Propagated;}}
...
placeholder1{let _=();if true{};continue;}}}errors_buffer.push(RegionErrorKind::
...
member_constraints.all_indices(){{;};debug!(?m_c_i);let m_c=&member_constraints[
m_c_i];();let member_region_vid=m_c.member_region_vid;debug!(?member_region_vid,
value=?self.region_value_str(member_region_vid),);let _=||();let choice_regions=
...
fr1,r)}};3;debug!("provides_universal_region: result = {:?}",result);result}pub(
...
push_back(from_region);if let _=(){};while let Some(r)=deque.pop_front(){debug!(
"find_constraint_paths_between_regions: from_region={:?} r={:?} value={}",//{;};
from_region,r,self.region_value_str(r),);;if target_test(r){let mut result=vec![
];let _=||();let mut p=r;loop{match context[p].clone(){Trace::NotVisited=>{bug!(
...
constraint:OutlivesConstraint<'tcx>|{({});debug_assert_eq!(constraint.sup,r);let
sub_region=constraint.sub;;if let Trace::NotVisited=context[sub_region]{context[
sub_region]=Trace::FromOutlivesConstraint(constraint);if true{};deque.push_back(
...
constraint_sccs.scc(fr1)]);3;self.find_constraint_paths_between_regions(fr1,|r|{
...
constraint_sccs .scc(c.sup),self.constraint_sccs.scc(c.sub),)).collect::<Vec<_>>
());*&*&();let mut extra_info=vec![];for constraint in path.iter(){let outlived=
...
(span)=constraint.category else{3;continue;};extra_info.push(ExtraConstraintInfo
::PlaceholderFromPredicate(span));3;break;}let cause_code=path.iter().find_map(|
...
.scc(target_region);if true{};let mut range=0..path.len();let blame_source=match
...
}};{;};let best_choice=if blame_source{range.rev().find(find_region)}else{range.
...
ConstraintCategory::OpaqueType{loop{break};return(next.clone(),extra_info);}}if 
...
ReturnConstraint::ClosureUpvar(field));{;};}}return(categorized_path[i].clone(),
...
self.scc_values.locations_outlived_by(scc);();for location in locations{let bb=&
body[location.block];if let _=(){};if let Some(terminator)=&bb.terminator{if let
TerminatorKind::FalseUnwind{..}=terminator.kind{3;return Some(location);}}}None}
...
region:RegionVid)->bool{{;};let origin=self.region_definition(region).origin;let
