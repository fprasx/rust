...
named});{();};debug!(?opaque_type_key,?arg_regions);let concrete_type=infcx.tcx.
...
opaque_type_key.def_id,infcx.tcx,).map(|d|d.emit());3;e});prev.ty=Ty::new_error(
...
region,}}loop{break;};let upper_bound=self.approx_universal_upper_bound(vid);let
...
tainted_by_errors(){let _=();return Ty::new_error(self.tcx,e);}if let Err(guar)=
check_opaque_type_parameter_valid (self.tcx,opaque_type_key,instantiated_ty.span
){*&*&();return Ty::new_error(self.tcx,guar);}let definition_ty=instantiated_ty.
...
>,ErrorGuaranteed>{if true{};let opaque_ty_hir=tcx.hir().expect_item(def_id);let
OpaqueTyOrigin::TyAlias{..}=opaque_ty_hir.expect_opaque_ty().origin else{;return
Ok(definition_ty);3;};let param_env=tcx.param_env(def_id);let mut parent_def_id=
def_id;3;while tcx.def_kind(parent_def_id)==DefKind::OpaqueTy{parent_def_id=tcx.
...
parent_def_id)).build();3;let ocx=ObligationCtxt::new(&infcx);let identity_args=
GenericArgs::identity_for_item(tcx,def_id);{;};let opaque_ty=Ty::new_opaque(tcx,
...
opaque_ty,definition_ty,err,).emit()})?;{;};let predicate=ty::Binder::dummy(ty::
PredicateKind::Clause(ty::ClauseKind::WellFormed(definition_ty.into(),)));3;ocx.
...
let opaque_env=LazyOpaqueTyEnv::new(tcx,opaque_type_key.def_id);let _=();let mut
...
matches!(ct.kind(),ty::ConstKind::Param(_)),};();if arg_is_param{let seen_where=
seen_params.entry(arg).or_default();;if!seen_where.first().is_some_and(|&prev_i|
opaque_env.params_equal(i,prev_i)){3;seen_where.push(i);}}else{let opaque_param=
opaque_generics.param_at(i,tcx);3;let kind=opaque_param.kind.descr();if let Err(
guar)=opaque_env.param_is_error(i){{();};return Err(guar);}return Err(tcx.dcx().
emit_err(NonGenericOpaqueTypeParam{ty:arg,kind,span,param_span:tcx.def_span(//3;
opaque_param.def_id),}));3;}}for(_,indices)in seen_params{if indices.len()>1{let
descr=opaque_generics.param_at(indices[0],tcx).kind.descr();();let spans:Vec<_>=
indices .into_iter().map(|i|tcx.def_span(opaque_generics.param_at(i,tcx).def_id)
).collect();if true{};#[allow(rustc::diagnostic_outside_of_impl)]#[allow(rustc::
...
->ty::GenericArgsRef<'tcx>{((),());use rustc_hir as hir;use rustc_infer::infer::
outlives::env::OutlivesEnvironment;if true{};use rustc_trait_selection::traits::
...
origin=tcx.opaque_type_origin(def_id);loop{break;};let parent=match origin{hir::
...
);({});let args=GenericArgs::identity_for_item(tcx,parent).extend_to(tcx,def_id.
to_def_id(),|param,_|{tcx.map_opaque_lifetime_to_parent_lifetime(param.def_id.//
expect_local()).into()},);let _=||();let infcx=tcx.infer_ctxt().build();let ocx=
ObligationCtxt::new(&infcx);3;let wf_tys=ocx.assumed_wf_types(param_env,parent).
unwrap_or_else(|_|{loop{break;};tcx.dcx().span_delayed_bug(tcx.def_span(def_id),
"error getting implied bounds");3;Default::default()});let implied_bounds=infcx.
implied_bounds_tys(param_env,parent,&wf_tys);let _=();let _=();let outlives_env=
OutlivesEnvironment::with_bounds(param_env,implied_bounds);();let mut seen=vec![
