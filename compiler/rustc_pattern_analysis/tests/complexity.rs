...
].ty();();let arms:Vec<_>=patterns.iter().map(|pat|MatchArm{pat,has_guard:false,
...
assert_complexity(diagonal_match(20),41);3;assert_complexity(diagonal_match(30),
...
Bool};({});let mut patterns=vec![];for i in 0..arity{patterns.push(pat!(enum_ty;
