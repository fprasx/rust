...
'p,'tcx>>,ErrorGuaranteed>{();let Some(&ty)=column.head_ty()else{return Ok(Vec::
new());;};let set=column.analyze_ctors(cx,&ty)?;if set.present.is_empty(){return
Ok(Vec::new());if let _=(){};*&*&();((),());}let mut witnesses=Vec::new();if cx.
...
wit;loop{break;};if let _=(){};witnesses.push(pat);}}}Ok(witnesses)}pub(crate)fn
...
lint_level:lint_level.as_str(),lint_name:"non_exhaustive_omitted_patterns",};();
use rustc_errors::LintDiagnostic;;let mut err=rcx.tcx.dcx().struct_span_warn(arm
.pat.data().span,"");loop{break};err.primary_message(decorator.msg());decorator.
decorate_lint(&mut err);((),());let _=();let _=();let _=();err.emit();}}}Ok(())}
