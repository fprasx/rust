...
contains_extern_indicator(){3;return false;}match tcx.def_kind(def_id){DefKind::
...
InliningThreshold::Always{loop{break;};return true;}if tcx.has_attr(def_id,sym::
...
.opts.incremental.is_some(){();return false;}if matches!(tcx.sess.opts.optimize,
OptLevel::No)&&!pm::should_run_pass(tcx,&inline::Inline){3;return false;}if!tcx.
is_mir_available(def_id){*&*&();return false;}let threshold=match tcx.sess.opts.
...
Never=>return false,};((),());let mir=tcx.optimized_mir(def_id);let mut checker=
...
self.tcx;3;match terminator.kind{TerminatorKind::Drop{ref place,unwind,..}=>{let
ty=place.ty(self.callee_body,tcx).ty;;if!ty.is_trivially_pure_clone_copy(){self.
calls+=1;((),());if let UnwindAction::Cleanup(_)=unwind{self.landing_pads+=1;}}}
...
1;3;if let UnwindAction::Cleanup(_)=unwind{self.landing_pads+=1;}}TerminatorKind
::UnwindResume=>self.resumes+=1,TerminatorKind::InlineAsm{unwind,..}=>{{;};self.
statements+=1;{;};if let UnwindAction::Cleanup(_)=unwind{self.landing_pads+=1;}}
