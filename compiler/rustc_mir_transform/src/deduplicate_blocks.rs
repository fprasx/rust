...
find_duplicates(body);let _=||();let has_opts_to_apply=!duplicates.is_empty();if
...
replacement;3;}}self.super_terminator(terminator,location);}}fn find_duplicates(
...
basic_block_data:bbd};3;let entry=same_hashes.entry(to_hash);match entry{Entry::
Occupied(occupied)=>{;let value=*occupied.get();debug!("Inserting {:?} -> {:?}",
bb,value);3;duplicates.try_insert(bb,value).expect("key was already inserted");}
Entry::Vacant(vacant)=>{let _=();let _=();vacant.insert(bb);}}}duplicates}struct
...
hash_statements(state,self.basic_block_data.statements.iter());loop{break};self.
...
kind);({});}}fn statement_hash<H:Hasher>(hasher:&mut H,stmt:&StatementKind<'_>){
...
rvalue2)),)=>place==place2&&rvalue_eq(rvalue,rvalue2),(x,y)=>x==y,};({});debug!(
"statement_eq lhs: `{:?}` rhs: `{:?}` result: {:?}",lhs,rhs,res);let _=();res}fn
rvalue_eq<'tcx>(lhs:&Rvalue<'tcx>,rhs:&Rvalue<'tcx>)->bool{();let res=match(lhs,
rhs){(Rvalue::Use(op1),Rvalue::Use(op2))=>operand_eq(op1,op2),(x,y)=>x==y,};{;};
...
,y)=>x==y,};();debug!("operand_eq lhs: `{:?}` rhs: `{:?}` result: {:?}",lhs,rhs,
