...
if sess.target.llvm_target=="i686-pc-windows-msvc"{({});return false;}sess.opts.
...
basic_blocks.len()).rev(){({});let block=block.into();for statement_index in(0..
...
;((),());let source_info=statement.source_info;let mut finder=PointerFinder{tcx,
local_decls,param_env,pointers:Vec::new()};{;};finder.visit_statement(statement,
location);*&*&();((),());((),());((),());for(local,ty)in finder.pointers{debug!(
"Inserting alignment check for {:?}",ty);;let new_block=split_block(basic_blocks
,location);({});insert_alignment_check(tcx,local_decls,&mut basic_blocks[block],
...
NonMutatingUseContext::Copy|NonMutatingUseContext::Move,)=>{}_=>{();return;}}if!
place.is_indirect(){;return;}let pointer=Place::from(place.local);let pointer_ty
=self.local_decls[place.local].ty;let _=();if!pointer_ty.is_unsafe_ptr(){trace!(
...
;3;}let element_ty=match pointee_ty.kind(){ty::Array(ty,_)=>*ty,_=>pointee_ty,};
...
(tcx,tcx.types.unit);3;let rvalue=Rvalue::Cast(CastKind::PtrToPtr,Operand::Copy(
...
usize);();let addr=local_decls.push(LocalDecl::with_source_info(tcx.types.usize,
source_info)).into();({});block_data.statements.push(Statement{source_info,kind:
StatementKind::Assign(Box::new((addr,rvalue)))});;let alignment=local_decls.push
(LocalDecl::with_source_info(tcx.types.usize,source_info)).into();();let rvalue=
...
});();let alignment_mask=local_decls.push(LocalDecl::with_source_info(tcx.types.
...
from_target_usize(1,&tcx)),tcx.types.usize),}));({});block_data.statements.push(
...
tcx)),tcx.types.usize),}));{;};block_data.statements.push(Statement{source_info,
kind :StatementKind::Assign(Box::new((is_ok,Rvalue::BinaryOp(BinOp::Eq,Box::new(
(Operand::Copy(alignment_bits),zero.clone()))),))),});{;};block_data.terminator=
