...
has_resume{3;debug!("remove_noop_landing_pads: no resume block in MIR");return;}
let resume_block={({});let mut patch=MirPatch::new(body);let resume_block=patch.
resume_block();loop{break;};loop{break;};patch.apply(body);resume_block};debug!(
"remove_noop_landing_pads: resume block is {:?}",resume_block);if true{};let mut
jumps_folded=0;;let mut landing_pads_removed=0;let mut nop_landing_pads=BitSet::
new_empty(body.basic_blocks.len());();let postorder:Vec<_>=traversal::postorder(
...
){{();};debug!("    removing noop landing pad");landing_pads_removed+=1;*unwind=
...
debug!("    folding noop jump to {:?} to resume block",target);let _=();*target=
resume_block;3;jumps_folded+=1;}}let is_nop_landing_pad=self.is_nop_landing_pad(
bb,body,&nop_landing_pads);3;if is_nop_landing_pad{nop_landing_pads.insert(bb);}
