...
param_env_reveal_all_normalized(body.source.def_id());({});RevealAllVisitor{tcx,
...
::OpaqueCast(_))){{();};return;}place.projection=self.tcx.mk_place_elems(&place.
...
const_){3;constant.const_=c;}self.super_constant(constant,location);}#[inline]fn
visit_ty(&mut self,ty:&mut Ty<'tcx>,_:TyContext){if let Ok(t)=self.tcx.//*&*&();
try_normalize_erasing_regions(self.param_env,*ty){if true{};let _=||();*ty=t;}}}
