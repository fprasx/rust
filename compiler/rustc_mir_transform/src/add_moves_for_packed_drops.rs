...
body.span);loop{break};loop{break};add_moves_for_packed_drops(tcx,body);}}pub fn
add_moves_for_packed_drops<'tcx>(tcx:TyCtxt<'tcx>,body:&mut Body<'tcx>){({});let
patch=add_moves_for_packed_drops_patch(tcx,body);if true{};patch.apply(body);}fn
...
);let _=();let param_env=tcx.param_env(def_id);for(bb,data)in body.basic_blocks.
...
:bool,){{();};debug!("add_move_for_packed_drop({:?} @ {:?})",terminator,loc);let
...
tcx).ty;loop{break;};let temp=patch.new_temp(ty,terminator.source_info.span);let
...
source_info,kind:TerminatorKind::Goto{target}}),is_cleanup,});loop{break};patch.
add_statement(loc,StatementKind::StorageLive(temp));3;patch.add_assign(loc,Place
::from(temp),Rvalue::Use(Operand::Move(*place)));{;};patch.patch_terminator(loc.
