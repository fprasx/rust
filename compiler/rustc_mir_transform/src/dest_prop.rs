...
body:&mut Body<'tcx>){{();};let def_id=body.source.def_id();let mut allocations=
Allocations::default();({});trace!(func=?tcx.def_path_str(def_id));let borrowed=
...
iterate_to_fixpoint();{();};let points=DenseLocationMap::new(body);let mut live=
save_as_intervals(&points,body,live);let _=();let mut round_count=0;loop{let mut
...
body,&points,&live,round_count);let _=();FilterInformation::filter_liveness(&mut
candidates,&points,&live,&mut allocations.write_info,body,);loop{break;};let mut
merged_locals:BitSet<Local>=BitSet::new_empty(body.local_decls.len());();let mut
merges=FxIndexMap::default();{();};for(src,candidates)in candidates.c.iter(){if 
merged_locals.contains(*src){3;continue;}let Some(dest)=candidates.iter().find(|
...
round_count+=1;let _=||();apply_merges(body,tcx,&merges,&merged_locals);}trace!(
...
_location:Location){if let Some(dest)=self.merges.get(local){3;*local=*dest;}}fn
...
Location,){({});v.retain(|dest|{let remove=f(*dest);if remove==CandidateFilter::
...
CandidateFilter,at:Location,){loop{break;};let candidates=entry.get_mut();Self::
vec_filter_candidates(p,candidates,f,at);if true{};if candidates.len()==0{entry.
...
::Keep{({});return true;}let IndexEntry::Occupied(entry)=self.c.entry(*src)else{
return false;{();};};Self::entry_filter_candidates(entry,*src,|dest|{if dest==p{
CandidateFilter::Remove}else{CandidateFilter::Keep}},at,);();false});}}#[derive(
...
write_info_alloc,at:Location::START,};*&*&();this.internal_filter_liveness();}fn
internal_filter_liveness (&mut self){for(block,data)in traversal::preorder(self.
body){*&*&();self.at=Location{block,statement_index:data.statements.len()};self.
...
write_info.writes;({});for p in writes{let other_skip=self.write_info.skip_pair.
and_then(|(a,b)|{if a==*p{Some(b)}else if b==*p{Some(a)}else{None}});{;};let at=
...
add_place(*lhs);if true{};match rhs{Rvalue::Use(op)=>{self.add_operand(op);self.
consider_skipping_for_assign_use(*lhs,op,body);{;};}Rvalue::Repeat(op,_)=>{self.
add_operand(op);loop{break};}Rvalue::Cast(_,op,_)|Rvalue::UnaryOp(_,op)|Rvalue::
ShallowInitBox(op,_)=>{();self.add_operand(op);}Rvalue::BinaryOp(_,ops)|Rvalue::
CheckedBinaryOp(_,ops)=>{for op in[&ops.0,&ops.1]{;self.add_operand(op);}}Rvalue
::Aggregate(_,ops)=>{for op in ops{if let _=(){};self.add_operand(op);}}Rvalue::
...
)=places_to_candidate_pair(lhs,rhs,body){let _=();self.skip_pair=Some(pair);}}fn
for_terminator<'tcx>(&mut self,terminator:&TerminatorKind<'tcx>){3;self.reset();
...
{();self.add_place(*place);}}InlineAsmOperand::InOut{in_value,out_place,..}=>{if
let Some(place)=out_place{3;self.add_place(*place);}self.add_operand(in_value);}
...
Place<'_>){();self.writes.push(place.local);}fn add_operand<'tcx>(&mut self,op:&
Operand<'tcx>){match op{Operand::Move(p)=>(self.add_place(*p)),Operand::Copy(_)|
Operand::Constant(_)=>(),}}fn reset(&mut self){((),());self.writes.clear();self.
...
mem::swap(&mut a,&mut b);3;}if is_local_required(a,body){std::mem::swap(&mut a,&
...
'alloc>{if true{};candidates.clear();candidates_reverse.clear();let mut visitor=
FindAssignments{body,candidates,borrowed};;visitor.visit_body(body);for(_,cands)
in candidates.iter_mut(){if true{};cands.sort();cands.dedup();}for(src,cands)in 
...
}if is_local_required(src,self.body){((),());return;}self.candidates.entry(src).
...
Local,PointIndex>,round:usize,){({});let locals_live_at=|location|{let location=
...
locals_live_at(loc))?;let _=||();loop{break};loop{break};loop{break};}Ok(())});}
