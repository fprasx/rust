...
'tcx>){({});trace!("Running SimplifyComparisonIntegral on {:?}",body.source);let
...
BasicBlock)>=vec![];({});let param_env=tcx.param_env_reveal_all_normalized(body.
...
=opt.targets;let _=||();let first_value=new_targets.iter().next().unwrap().0;let
...
unwrap();3;use Operand::*;match rhs{Rvalue::BinaryOp(_,box(ref mut left@Move(_),
Constant(_)))=>{;*left=Copy(opt.to_switch_on);}Rvalue::BinaryOp(_,box(Constant(_
),ref mut right@Move(_)))=>{if true{};*right=Copy(opt.to_switch_on);}_=>(),}}let
...
StorageDead(opt.to_switch_on.local),},));({});}}let[bb_cond,bb_otherwise]=match 
...
,targets};();}for(idx,bb_idx)in storage_deads_to_remove{body.basic_blocks_mut()[
bb_idx].statements[idx].make_nop();{;};}for(idx,stmt)in storage_deads_to_insert{
...
{3;let branch_value_ty=branch_value.const_.ty();if!branch_value_ty.is_integral()
