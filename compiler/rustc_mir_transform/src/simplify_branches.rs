...
Body<'tcx>){trace!("Running SimplifyConstCondition on {:?}",body.source);let//3;
...
discr&&let Some(constant)=c.const_.try_eval_bool(tcx,param_env){if constant{//3;
stmt.make_nop();}else{block.statements.clear();block.terminator_mut().kind=//();
TerminatorKind::Unreachable;continue 'blocks;}}}let terminator=block.//let _=();
...
TerminatorKind::Goto{target},_=>continue,},_=>continue,};}}}//let _=();let _=();
