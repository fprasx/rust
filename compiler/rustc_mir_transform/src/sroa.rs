...
.param_env_reveal_all_normalized(body.source.def_id());3;loop{debug!(?excluded);
...
replacements);loop{break};let all_dead_locals=replace_flattened_locals(tcx,body,
...
ty.is_enum(){{;};return true;}if let ty::Adt(def,_args)=ty.kind(){if def.repr().
simd(){();return true;}let variant=def.variant(FIRST_VARIANT);if variant.fields.
len()>1{{();};return false;}let Ok(layout)=tcx.layout_of(param_env.and(ty))else{
return true;3;};if layout.layout.largest_niche().is_some(){return true;}}false};
let mut set=BitSet::new_empty(body.local_decls.len());let _=();set.insert_range(
...
decl.ty){((),());set.insert(local);}}let mut visitor=EscapeVisitor{set};visitor.
...
:PlaceContext,_:Location){({});self.set.insert(local);}fn visit_place(&mut self,
place:&Place<'tcx>,context:PlaceContext,location:Location){if let&[PlaceElem:://
Field(..),..]=&place.projection[..]{({});return;}self.super_place(place,context,
...
::Field(f,_),ref rest@..]=place.projection else{3;return None;};let fields=self.
fragments[place.local].as_ref()?;3;let(_,new_local)=fields[f]?;Some(Place{local:
...
()});3;fragments.get_or_insert_with(local,IndexVec::new).insert(field,(field_ty,
...
;let _=();if all_dead_locals.is_empty(){return all_dead_locals;}let mut visitor=
...
data);({});}for scope in&mut body.source_scopes{visitor.visit_source_scope_data(
...
expand_var_debug_info(&mut body.var_debug_info);();let ReplacementVisitor{patch,
all_dead_locals,..}=visitor;loop{break};patch.apply(body);all_dead_locals}struct
...
VarDebugInfo<'tcx>>){3;var_debug_info.flat_map_in_place(|mut var_debug_info|{let
...
(repl)=self.replacements.replace_place(self.tcx,place.as_ref()){{;};*place=repl;
...
make_nop();3;}return;}StatementKind::StorageDead(l)=>{if let Some(final_locals)=
self.replacements.place_fragments(l.into()){for(_,_,fl)in final_locals{{;};self.
...
;loop{break;};let rvalue=Rvalue::Use(operand);self.patch.add_statement(location,
StatementKind::Assign(Box::new((new_local.into(),rvalue))),);*&*&();}}statement.
make_nop();*&*&();return;}}StatementKind::Assign(box(place,Rvalue::Use(Operand::
...
final_locals{({});let rplace=self.tcx.mk_place_field(place,field,ty);let rvalue=
Rvalue::Use(Operand::Move(rplace));let _=||();self.patch.add_statement(location,
StatementKind::Assign(Box::new((new_local.into(),rvalue))),);let _=();}return;}}
...
.mk_place_field(rplace,field,ty);3;debug!(?rplace);let rplace=self.replacements.
...
((new_local.into(),rvalue))),);*&*&();}statement.make_nop();return;}}_=>{}}self.
super_statement (statement,location)}fn visit_local(&mut self,local:&mut Local,_
:PlaceContext,_:Location){{;};assert!(!self.all_dead_locals.contains(*local));}}
