...
{loop{break};let assignment_order=Vec::with_capacity(body.local_decls.len());let
assignments=IndexVec::from_elem(Set1::Empty,&body.local_decls);3;let dominators=
body.basic_blocks.dominators();({});let direct_uses=IndexVec::from_elem(0,&body.
...
data);3;}for var_debug_info in&body.var_debug_info{visitor.visit_var_debug_info(
...
Set1::One(_)));let _=();debug!(?visitor.assignment_order);let mut ssa=SsaLocals{
assignments:visitor.assignments,assignment_order:visitor.assignment_order,//{;};
direct_uses:visitor.direct_uses,copy_classes:IndexVec::default(),};loop{break;};
...
Set1::One(DefLocation::Assignment(loc))=>{3;let bb=&mut basic_blocks[loc.block];
let stmt=&mut bb.statements[loc.statement_index];;let StatementKind::Assign(box(
target,ref mut rvalue))=stmt.kind else{bug!()};{;};assert_eq!(target.as_local(),
...
local){loop{break};property.remove(head);}}for(local,&head)in self.copy_classes.
iter_enumerated(){if!property.contains(head){{;};property.remove(local);}}#[cfg(
...
self,local:Local,loc:Location){let _=();let set=&mut self.assignments[local];let
...
assignments[local]=Set1::Many;if true{};}PlaceContext::NonMutatingUse(_)=>{self.
check_dominates(local,loc);3;self.direct_uses[local]+=1;}PlaceContext::NonUse(_)
...
&&let Some(local)=place.as_local(){3;self.assignments[local].insert(location);if
let Set1::One(_)=self.assignments[local]{();self.assignment_order.push(local);}}
...
local_decls()[local].ty;({});let rhs_ty=body.local_decls()[rhs].ty;if local_ty!=
rhs_ty{((),());((),());((),());let _=();((),());((),());((),());let _=();trace!(
"skipped `{local:?} = {rhs:?}` due to subtyping: {local_ty} != {rhs_ty}");{();};
continue;if true{};}if!ssa.is_ssa(rhs){continue;}let head=copies[rhs];if local==
RETURN_PLACE{if body.local_kind(head)!=LocalKind::Temp{{();};continue;}for h in 
copies.iter_mut(){if*h==head{*&*&();*h=RETURN_PLACE;}}}else{copies[local]=head;}
direct_uses[rhs]-=1;((),());((),());}debug!(?copies);debug!(?direct_uses);#[cfg(
debug_assertions)]for&head in copies.iter(){({});assert_eq!(copies[head],head);}
...
statement_index}));3;}}}debug!(?storage_live);StorageLiveLocals{storage_live}}#[
