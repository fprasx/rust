...
.iter().map(|ps|ps.len()).collect();if true{};pred_count[START_BLOCK]+=1;let mut
new_blocks=Vec::new();{;};let cur_len=body.basic_blocks.len();for block in body.
...
;3;new_blocks.push(call_guard);*destination=BasicBlock::new(idx);}_=>{}}}debug!(
