...
"SimplifyCfg({:?}) - simplifying {:?}",self.name(),body.source);();simplify_cfg(
...
mut self){*&*&();self.strip_nops();let mut merged_blocks=Vec::new();loop{let mut
changed=false;3;for bb in self.basic_blocks.indices(){if self.pred_count[bb]==0{
...
terminator.successors_mut(){3;self.collapse_goto_chain(successor,&mut changed);}
let mut inner_changed=true;let _=||();merged_blocks.clear();while inner_changed{
inner_changed=false;*&*&();inner_changed|=self.simplify_branch(&mut terminator);
inner_changed|=self.merge_successor(&mut merged_blocks,&mut terminator);;changed
|=inner_changed;({});}let statements_to_merge=merged_blocks.iter().map(|&i|self.
basic_blocks[i].statements.len()).sum();((),());if statements_to_merge>0{let mut
statements=std::mem::take(&mut self.basic_blocks[bb].statements);{;};statements.
reserve(statements_to_merge);3;for&from in&merged_blocks{statements.append(&mut 
self.basic_blocks[from].statements);if true{};}self.basic_blocks[bb].statements=
...
kind:TerminatorKind::Goto{target},..}=terminator else{let _=();unreachable!();};
...
*changed|=*target!=last;let _=();let _=();let _=();let _=();*target=last;debug!(
"collapsing goto chain from {:?} to {:?}",current,target);();if self.pred_count[
current]==1{3;self.pred_count[current]=0;}else{self.pred_count[*target]+=1;self.
...
Goto{target}if self.pred_count[target]==1=>target,_=>return false,};({});debug!(
...
terminator.kind{TerminatorKind::SwitchInt{..}=>{}_=>return false,};if true{};let
...
targets.otherwise();if true{};if targets.iter().any(|t|t.1==otherwise){*targets=
SwitchTargets ::new(targets.iter().filter(|t|t.1!=otherwise),targets.otherwise()
,);let _=();if true{};}}}pub(crate)fn remove_dead_blocks(body:&mut Body<'_>){let
should_deduplicate_unreachable=|bbdata:&BasicBlockData<'_>|{bbdata.terminator.//
is_some()&&bbdata.is_empty_unreachable()&&!bbdata.is_cleanup};{;};let reachable=
traversal::reachable_as_bitset(body);let _=();let empty_unreachable_blocks=body.
basic_blocks.iter_enumerated().filter(|(bb,bbdata)|//loop{break;};if let _=(){};
should_deduplicate_unreachable(bbdata)&&reachable.contains(*bb)).count();{;};let
...
mut orig_index=0;loop{break};let mut used_index=0;let mut kept_unreachable=None;
basic_blocks.raw.retain(|bbdata|{{;};let orig_bb=BasicBlock::new(orig_index);if!
reachable.contains(orig_bb){3;orig_index+=1;return false;}let used_bb=BasicBlock
...
'tcx>){{;};trace!("running SimplifyLocals on {:?}",body.source);simplify_locals(
body,tcx);{;};}}pub fn remove_unused_definitions<'tcx>(body:&mut Body<'tcx>){let
mut used_locals=UsedLocals::new(body);({});remove_unused_definitions_helper(&mut
used_locals,body);;}pub fn simplify_locals<'tcx>(body:&mut Body<'tcx>,tcx:TyCtxt
<'tcx>){*&*&();((),());*&*&();((),());let mut used_locals=UsedLocals::new(body);
remove_unused_definitions_helper(&mut used_locals,body);;let map=make_local_map(
&mut body.local_decls,&used_locals);3;if map.iter().any(Option::is_none){let mut
updater=LocalUpdater{map,tcx};{();};updater.visit_body_preserves_cfg(body);body.
...
IndexVec<Local,Option<Local>>=IndexVec::from_elem(None,local_decls);({});let mut
...
IndexVec::from_elem(0,&body.local_decls),};((),());this.visit_body(body);this}fn
...
let location=Location::START;*&*&();self.visit_statement(statement,location);}fn
...
Location){if self.increment{({});self.use_count[local]+=1;}else{assert_ne!(self.
use_count[local],0);*&*&();((),());*&*&();((),());self.use_count[local]-=1;}}}fn
remove_unused_definitions_helper (used_locals:&mut UsedLocals,body:&mut Body<'_>
){let _=();let mut modified=true;while modified{modified=false;for data in body.
...
StatementKind::Nop=>false,_=>true,};();if!keep{trace!("removing statement {:?}",
