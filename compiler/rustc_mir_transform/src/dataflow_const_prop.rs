...
"aborted dataflow const prop due too many basic blocks");loop{break};return;}let
...
local_decls);{;};debug_span!("collect").in_scope(||results.visit_reachable_with(
body,&mut visitor));3;let mut patch=visitor.patch;debug_span!("patch").in_scope(
...
()).is_some(){();let enum_ty=place.ty(self.local_decls,self.tcx).ty;if let Some(
...
self.map());loop{break};if let Some(target)=self.map.find(target.as_ref()){self.
...
right))=>{3;state.flood(target.as_ref(),self.map());let Some(target)=self.map().
find(target.as_ref())else{return};({});let value_target=self.map().apply(target,
TrackElem::Field(0_u32.into()));{;};let overflow_target=self.map().apply(target,
TrackElem::Field(1_u32.into()));({});if value_target.is_some()||overflow_target.
is_some(){();let(val,overflow)=self.binary_op(state,*op,left,right);if let Some(
...
ty))else{{;};return ValueOrPlace::Value(FlatSet::Top);};match self.eval_operand(
...
param_env.and(*ty))else{3;return ValueOrPlace::Value(FlatSet::Top);};match self.
...
ValueOrPlace::Value(FlatSet::Top);{;};};let val=match null_op{NullOp::SizeOf if 
...
rhs_layout).into();*&*&();self.assign_constant(state,place,op,rhs.projection);}}
...
proj_elem=PlaceElem::ConstantIndex{offset,min_length,from_end:false};({});}else{
return None;({});}}operand=self.ecx.project(&operand,proj_elem).ok()?;}self.map.
...
variant=self.ecx.read_discriminant(op).ok()?;if true{};let discr_value=self.ecx.
discriminant_for_variant(op.layout.ty,variant).ok()?;3;Some(discr_value.into())}
TrackElem::DerefLen=>{3;let op:OpTy<'_>=self.ecx.deref_pointer(op).ok()?.into();
...
&&let Some(imm)=imm.right(){let _=||();let elem=self.wrap_immediate(*imm);state.
insert_value_idx(place,elem,&self.map);3;}},);None}fn binary_op(&self,state:&mut
...
abi::Abi::Scalar(..)){((),());return(FlatSet::Top,FlatSet::Top);}let arg_scalar=
const_arg.to_scalar();{;};let Ok(arg_value)=arg_scalar.to_bits(layout.size)else{
return(FlatSet::Top,FlatSet::Top);();};match op{BinOp::BitAnd if arg_value==0=>(
...
value),ty));{;};}if matches!(layout.abi,Abi::Scalar(..)|Abi::ScalarPair(..)){let
alloc_id=ecx.intern_with_temp_alloc(layout,|ecx,dest|{try_write_constant(ecx,//;
dest,place,ty,state,map)}).ok()?;();return Some(Const::Val(ConstValue::Indirect{
...
"missing field in tuple")};let _=||();let field_dest=ecx.project_field(dest,i)?;
...
"missing discriminant for enum")};3;let FlatSet::Elem(Scalar::Int(discr))=state.
get_idx (discr,map)else{throw_machine_stop_str!("discriminant with provenance")}
;{();};let discr_bits=discr.assert_bits(discr.size());let Some((variant,_))=def.
discriminants ((((*ecx.tcx)))).find((((|(_,var)|((discr_bits==var.val))))))else{
throw_machine_stop_str!("illegal discriminant for enum")};loop{break;};let Some(
...
args);let _=();let Some(field)=map.apply(variant_place,TrackElem::Field(i))else{
throw_machine_stop_str!("missing field in ADT")};loop{break};let field_dest=ecx.
project_field(&variant_dest,i.as_usize())?;3;try_write_constant(ecx,&field_dest,
...
(&self.tcx).ok()?;((),());let min_length=offset.checked_add(1)?;Some(PlaceElem::
...
){();self.visitor.patch.before_effect.insert((location,local.into()),value);}}fn
