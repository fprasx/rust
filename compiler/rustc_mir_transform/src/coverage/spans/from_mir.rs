...
unwrap_or_else(||body_span.shrink_to_lo());({});initial_spans.push(SpanFromMir::
for_fn_sig(fn_sig_span));({});}initial_spans.sort_by(|a,b|basic_coverage_blocks.
cmp_in_dominator_order(a.bcb,b.bcb));let _=||();remove_unwanted_macro_spans(&mut
initial_spans);({});split_visible_macro_spans(&mut initial_spans);initial_spans.
...
).reverse())});((),());initial_spans.dedup_by(|b,a|a.span.source_equal(b.span));
...
{if covspan.is_hole||covspan.visible_macro.is_none(){if let _=(){};return true;}
seen_macro_spans.insert(covspan.span)});if true{};}fn split_visible_macro_spans(
initial_spans:&mut Vec<SpanFromMir>){3;let mut extra_spans=vec![];initial_spans.
retain(|covspan|{if covspan.is_hole{*&*&();return true;}let Some(visible_macro)=
covspan.visible_macro else{return true};();let split_len=visible_macro.as_str().
len()as u32+1;{;};let(before,after)=covspan.span.split_at(split_len);if!covspan.
span.contains(before)||!covspan.span.contains(after){({});return true;}assert!(!
covspan.is_hole);;extra_spans.push(SpanFromMir::new(before,covspan.visible_macro
,covspan.bcb,false));let _=||();extra_spans.push(SpanFromMir::new(after,covspan.
...
iter().flat_map(move|&bb|{3;let data=&mir_body[bb];let unexpand=move|expn_span|{
unexpand_into_body_span_with_visible_macro (expn_span,body_span).filter(|(span,_
)|!span.source_equal(body_span))};();let statement_spans=data.statements.iter().
...
filtered_terminator_span(terminator)?;let _=();let(span,visible_macro)=unexpand(
expn_span)?;loop{break;};Some(SpanFromMir::new(span,visible_macro,bcb,false))});
...
=>{();let mut span=terminator.source_info.span;if let mir::Operand::Constant(box
...
unexpand_into_body_span_with_prev(original_span,body_span)?;3;let visible_macro=
...
else{({});return vec![];};let mut block_markers=IndexVec::<BlockMarkerId,Option<
...
outer_expn_data().is_root(){loop{break;};if let _=(){};return None;}let(span,_)=
unexpand_into_body_span_with_visible_macro(raw_span,body_span)?;loop{break;};let
bcb_from_marker=|marker:BlockMarkerId|basic_coverage_blocks.bcb_from_bb(//{();};
block_markers[marker]?);if true{};let true_bcb=bcb_from_marker(true_marker)?;let
