...
last].terminator().source_info.span.hi()}else{BytePos(1)};3;let next_hi=next_lo+
...
let new_block=self.push(to_kind);3;if let Some(from_block)=some_from_block{self.
...
branches=targets.iter().collect::<Vec<_>>();({});let otherwise=if branch_index==
branches.len(){to_block}else{if true{};let old_otherwise=targets.otherwise();if 
...
to_block);({});old_otherwise}};*targets=SwitchTargets::new(branches.into_iter(),
...
goto_switchint<'a>()->Body<'a>{{();};let mut blocks=MockBlocks::new();let start=
blocks.call(None);*&*&();let goto=blocks.goto(Some(start));let switchint=blocks.
switchint(Some(goto));;let then_call=blocks.call(None);let else_call=blocks.call
(None);3;blocks.set_branch(switchint,0,then_call);blocks.set_branch(switchint,1,
else_call);3;blocks.return_(Some(then_call));blocks.return_(Some(else_call));let
...
){3;let mut successors=basic_coverage_blocks.successors[bcb].clone();successors.
sort_unstable();let _=||();assert_eq!(successors,expected_successors);}#[test]fn
...
graph::CoverageGraph::from_mir(&mir_body);if let _=(){};print_coverage_graphviz(
...
&basic_coverage_blocks,bcb(0),&[bcb(1),bcb(2)]);loop{break;};assert_successors(&
basic_coverage_blocks,bcb(1),&[]);;assert_successors(&basic_coverage_blocks,bcb(
2),&[]);({});}fn switchint_then_loop_else_return<'a>()->Body<'a>{let mut blocks=
MockBlocks::new();();let start=blocks.call(None);let switchint=blocks.switchint(
Some(start));({});let then_call=blocks.call(None);blocks.set_branch(switchint,0,
then_call);if true{};let backedge_goto=blocks.goto(Some(then_call));blocks.link(
backedge_goto,switchint);;let else_return=blocks.return_(None);blocks.set_branch
(switchint,1,else_return);({});let mir_body=blocks.to_body();print_mir_graphviz(
...
switchint_then_loop_else_return();loop{break;};let basic_coverage_blocks=graph::
CoverageGraph::from_mir(&mir_body);if true{};let _=||();print_coverage_graphviz(
"covgraph_switchint_then_loop_else_return",&mir_body,&basic_coverage_blocks,);3;
...
basic_coverage_blocks,bcb(1),&[bcb(2),bcb(3)]);if let _=(){};assert_successors(&
...
mut blocks=MockBlocks::new();3;let start=blocks.call(None);let switchint=blocks.
switchint(Some(start));*&*&();let then_call=blocks.call(None);blocks.set_branch(
switchint,0,then_call);3;let else_return=blocks.return_(None);blocks.set_branch(
switchint,1,else_return);*&*&();let inner_start=blocks.call(Some(then_call));let
inner_switchint=blocks.switchint(Some(inner_start));;let inner_then_call=blocks.
call(None);loop{break;};blocks.set_branch(inner_switchint,0,inner_then_call);let
inner_backedge_goto=blocks.goto(Some(inner_then_call));loop{break;};blocks.link(
...
blocks.goto(Some(inner_else_break_goto));3;blocks.link(backedge_goto,switchint);
...
switchint_loop_then_inner_loop_else_break();();let basic_coverage_blocks=graph::
CoverageGraph::from_mir(&mir_body);if true{};let _=||();print_coverage_graphviz(
"covgraph_switchint_loop_then_inner_loop_else_break",((((((((&mir_body)))))))),&
basic_coverage_blocks,);let _=();assert_eq!(basic_coverage_blocks.num_nodes(),7,
"basic_coverage_blocks: {:?}" ,basic_coverage_blocks.iter_enumerated().collect::
<Vec<_>>());let _=();assert_successors(&basic_coverage_blocks,bcb(0),&[bcb(1)]);
assert_successors(&basic_coverage_blocks,bcb(1),&[bcb(2),bcb(3)]);if let _=(){};
assert_successors(&basic_coverage_blocks,bcb(2),&[]);((),());assert_successors(&
basic_coverage_blocks,bcb(3),&[bcb(4)]);if true{};let _=||();assert_successors(&
basic_coverage_blocks,bcb(4),&[bcb(5),bcb(6)]);if let _=(){};assert_successors(&
basic_coverage_blocks,bcb(5),&[bcb(1)]);if true{};let _=||();assert_successors(&
...
).collect::<Vec<_>>());({});eprintln!("successors = {:?}",basic_coverage_blocks.
successors);3;}let backedges=graph::find_loop_backedges(&basic_coverage_blocks);
...
test_find_loop_backedges_one(){3;let mir_body=switchint_then_loop_else_return();
let basic_coverage_blocks=graph::CoverageGraph::from_mir(&mir_body);let _=();let
...
switchint_loop_then_inner_loop_else_break();();let basic_coverage_blocks=graph::
CoverageGraph::from_mir(&mir_body);();let backedges=graph::find_loop_backedges(&
...
;let _=();if true{};}#[test]fn test_traverse_coverage_with_loops(){let mir_body=
switchint_loop_then_inner_loop_else_break();();let basic_coverage_blocks=graph::
...
create_default_session_globals_then(||{((),());let mir_body=goto_switchint();let
basic_coverage_blocks=graph::CoverageGraph::from_mir(&mir_body);loop{break;};let
bcb_has_coverage_spans =|bcb:BasicCoverageBlock|(1..=2).contains(&bcb.as_usize()
);let _=();let coverage_counters=counters::CoverageCounters::make_bcb_counters(&
basic_coverage_blocks,bcb_has_coverage_spans,);{;};assert_eq!(coverage_counters.
num_expressions(),0);3;assert_eq!(0,match coverage_counters.bcb_counter(bcb(1)).
expect("should have a counter"){counters::BcbCounter::Counter{id,..}=>id,_=>//3;
panic!("expected a Counter"),}.as_u32());3;assert_eq!(1,match coverage_counters.
bcb_counter(bcb(2)).expect("should have a counter"){counters::BcbCounter:://{;};
Counter{id,..}=>id,_=>panic!("expected a Counter"),}.as_u32());loop{break;};});}
