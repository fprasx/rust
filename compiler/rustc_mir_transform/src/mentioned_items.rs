...
new();({});MentionedItemsVisitor{tcx,body,mentioned_items:&mut mentioned_items}.
...
super_rvalue(rvalue,location);3;let span=||self.body.source_info(location).span;
...
operand,target_ty)=>{let _=||();let source_ty=operand.ty(self.body,self.tcx);let
...
ty::Str|ty::Slice(..)))=>false,_=>true,};loop{break};if may_involve_vtable{self.
...
ty(self.body,self.tcx);();self.mentioned_items.push(Spanned{node:MentionedItem::
...
operand.ty(self.body,self.tcx);if true{};self.mentioned_items.push(Spanned{node:
