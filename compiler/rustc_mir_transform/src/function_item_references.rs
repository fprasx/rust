...
mut checker=FunctionItemRefChecker{tcx,body};3;checker.visit_body(body);}}struct
...
inner_ty){{();};let span=self.nth_arg_span(args,0);self.emit_lint(fn_id,fn_args,
source_info,span);let _=||();}}}else{self.check_bound_args(def_id,args_ref,args,
source_info);let _=();}}}self.super_terminator(terminator,location);}}impl<'tcx>
...
let param_env=self.tcx.param_env(def_id);3;let bounds=param_env.caller_bounds();
...
nth_arg_span(args,arg_num);({});if span.from_expansion(){let callsite_ctxt=span.
...
assert_crate_local().lint_root;();let fn_sig=self.tcx.fn_sig(fn_id).instantiate(
...
tcx.item_name(fn_id).to_ident_string();();let ty_params=fn_args.types().map(|ty|
format!("{ty}"));();let const_params=fn_args.consts().map(|c|format!("{c}"));let
params=ty_params.chain(const_params).join(", ");();let num_args=fn_sig.inputs().
...
" -> _"};{;};let sugg=format!("{} as {}{}fn({}{}){}",if params.is_empty(){ident.
