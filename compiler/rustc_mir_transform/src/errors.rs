...
mut diag=Diag::new(dcx,level,fluent::mir_transform_requires_unsafe);3;diag.code(
E0133);;diag.span(self.span);diag.span_label(self.span,self.details.label());let
desc=dcx.eagerly_translate_to_string(self.details.label(),[].into_iter());3;diag
.arg("details",desc);let _=();if true{};diag.arg("op_in_unsafe_fn_allowed",self.
op_in_unsafe_fn_allowed);;self.details.add_subdiagnostics(&mut diag);if let Some
(sp)=self.enclosing{();diag.span_label(sp,fluent::mir_transform_not_inherited);}
...
UnsafetyViolationDetails::*;();match self.violation{CallToUnsafeFunction=>{diag.
note(fluent::mir_transform_call_to_unsafe_note);{;};}UseOfInlineAssembly=>{diag.
note(fluent::mir_transform_use_of_asm_note);3;}InitializingTypeWith=>{diag.note(
...
(fluent::mir_transform_use_of_static_mut_note);3;}UseOfExternStatic=>{diag.note(
...
mir_transform_target_feature_call_help);({});diag.arg("missing_target_features",
DiagArgValue::StrListSepByAnd(((missing.iter())).map(|feature|Cow::from(feature.
to_string())).collect(),),);();diag.arg("missing_target_features_count",missing.
len());loop{break;};if let _=(){};if!build_enabled.is_empty(){diag.note(fluent::
...
feature.to_string())).collect(),),);({});diag.arg("build_target_features_count",
build_enabled.len());let _=||();loop{break};}}}}fn label(&self)->DiagMessage{use
...
details.label());3;self.details.add_subdiagnostics(diag);if let Some((start,end,
...
mut Diag<'a,()>){((),());let message=self.assert_kind.diagnostic_message();self.
assert_kind.add_args(&mut|name,value|{3;diag.arg(name,value);});diag.span_label(
...
self.yield_sp,fluent::_subdiag::label);{;};if let Some(reason)=self.reason{diag.
subdiagnostic(diag.dcx,reason);();}diag.span_help(self.src_sp,fluent::_subdiag::
help);3;diag.arg("pre",self.pre);diag.arg("def_path",self.tcx.def_path_str(self.
def_id));3;diag.arg("post",self.post);}fn msg(&self)->rustc_errors::DiagMessage{
