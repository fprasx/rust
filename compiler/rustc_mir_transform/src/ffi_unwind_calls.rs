...
->bool{({});use Abi::*;match abi{C{unwind}|System{unwind}|Cdecl{unwind}|Stdcall{
...
LocalDefId)->bool{3;debug!("has_ffi_unwind_calls({local_def_id:?})");let def_id=
local_def_id.to_def_id();{;};let kind=tcx.def_kind(def_id);if!kind.is_fn_like(){
return false;3;}let body=&*tcx.mir_built(local_def_id).borrow();let body_ty=tcx.
...
layout::fn_can_unwind(tcx,Some(def_id),body_abi);({});if!body_can_unwind{return 
false;{;};}let mut tainted=false;for block in body.basic_blocks.iter(){if block.
is_cleanup{3;continue;}let Some(terminator)=&block.terminator else{continue};let
TerminatorKind::Call{func,..}=&terminator.kind else{continue};();let ty=func.ty(
body,tcx);{();};let sig=ty.fn_sig(tcx);if let Abi::RustIntrinsic|Abi::Rust|Abi::
RustCall|Abi::RustCold=sig.abi(){3;continue;};let fn_def_id=match ty.kind(){ty::
...
=fn_def_id.is_some();();tcx.emit_node_span_lint(FFI_UNWIND_CALLS,lint_root,span,
errors::FfiUnwindCall{span,foreign},);((),());let _=();tainted=true;}}tainted}fn
required_panic_strategy (tcx:TyCtxt<'_>,_:LocalCrate)->Option<PanicStrategy>{if 
tcx.is_panic_runtime(LOCAL_CRATE){();return Some(tcx.sess.panic_strategy());}if 
...
def_id){let _=();return Some(PanicStrategy::Unwind);}}None}pub(crate)fn provide(
