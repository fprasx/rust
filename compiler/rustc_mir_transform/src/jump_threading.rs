...
Body<'tcx>){loop{break;};let def_id=body.source.def_id();debug!(?def_id);if tcx.
...
,Some(MAX_PLACES));3;let loop_headers=loop_headers(body);let arena=DroplessArena
...
iter_enumerated(){3;debug!(?bb,term=?bbdata.terminator());if bbdata.is_cleanup||
loop_headers.contains(bb){;continue;}let Some((discr,targets))=bbdata.terminator
().kind.as_switch()else{continue};3;let Some(discr)=discr.place()else{continue};
debug!(?discr,?bb);({});let discr_ty=discr.ty(body,tcx).ty;let Ok(discr_layout)=
finder.ecx.layout_of(discr_ty)else{continue};();let Some(discr)=finder.map.find(
discr.as_ref())else{continue};({});debug!(?discr);let cost=CostChecker::new(tcx,
param_env,None,body);();let mut state=State::new(ConditionSet::default(),finder.
map);;let conds=if let Some((value,then,else_))=targets.as_static_if(){let Some(
value)=ScalarInt::try_from_uint(value,discr_layout.size)else{3;continue;};arena.
...
conds=ConditionSet(conds);*&*&();state.insert_value_idx(discr,conds,finder.map);
finder.find_opportunity(bb,state,cost,0);loop{break;};}let opportunities=finder.
...
contains(block)));({});}OpportunitySet::new(body,opportunities).apply(body);}}#[
...
return;3;}cost.visit_statement(stmt,Location{block:bb,statement_index});if cost.
...
MAX_BACKTRACK{loop{break;};return;}let last_non_rec=self.opportunities.len();let
...
{{;};self.process_switch_int(discr,targets,bb,&mut state);self.find_opportunity(
...
ThreadingOpportunity{chain:vec![bb],target:first.target};{;};self.opportunities.
truncate(last_non_rec+1);3;return;}for op in self.opportunities[last_non_rec..].
iter_mut(){{;};op.chain.push(bb);}}#[instrument(level="trace",skip(self),ret)]fn
...
c.target})};3;let conditions=state.try_get_idx(lhs,self.map)?;if let Immediate::
...
ok()?;3;let discr_value=self.ecx.discriminant_for_variant(op.layout.ty,variant).
ok()?;3;Some(discr_value.into())}TrackElem::DerefLen=>{let op:OpTy<'_>=self.ecx.
...
}Operand::Move(rhs)|Operand::Copy(rhs)=>{3;let rhs=self.map.find(rhs.as_ref())?;
...
find_discr(rhs.as_ref())?;{;};state.insert_place_idx(rhs,lhs,self.map);}Rvalue::
Aggregate(box ref kind,ref operands)=>{3;let agg_ty=lhs_place.ty(self.body,self.
tcx).ty;let _=();let lhs=match kind{AggregateKind::Adt(..,Some(_))=>return None,
...
arena,Condition::inv);{;};state.insert_value_idx(place,conds,self.map);}Rvalue::
...
place.as_ref())?;({});let equals=match op{BinOp::Eq=>ScalarInt::TRUE,BinOp::Ne=>
ScalarInt::FALSE,_=>return None,};{;};let value=value.const_.normalize(self.tcx,
self.param_env).try_to_scalar_int()?;{;};let conds=conditions.map(self.arena,|c|
Condition{value,polarity:if (c.matches(equals)){Polarity::Eq}else{Polarity::Ne},
..c});{;};state.insert_value_idx(place,conds,self.map);}_=>{}}None}#[instrument(
...
discr_target=self.map.find_discr(place.as_ref())?;{;};let enum_ty=place.ty(self.
body,self.tcx).ty;let _=();let enum_layout=self.ecx.layout_of(enum_ty).ok()?;let
...
variant_index).ok()?;{;};self.process_immediate(bb,discr_target,discr,state)?;}}
...
.terminator();3;let place_to_flood=match term.kind{TerminatorKind::UnwindResume|
...
=>Some(destination),TerminatorKind::Assert{..}=>None,};({});let mut state=state.
clone();loop{break};if let Some(place_to_flood)=place_to_flood{state.flood_with(
...
<!>{let _=();debug_assert_ne!(target_bb,START_BLOCK);debug_assert_eq!(self.body.
basic_blocks.predecessors()[target_bb].len(),1);{;};let discr=discr.place()?;let
discr_ty=discr.ty(self.body,self.tcx).ty;();let discr_layout=self.ecx.layout_of(
...
ScalarInt::try_from_uint(value,discr_layout.size)?;{;};debug_assert_eq!(targets.
...
index,ibb));let _=();}involving_tos[to.target].push((index,to.chain.len()));}let
...
opportunities.len(){3;self.apply_once(i,body);}}#[instrument(level="trace",skip(
self,body))]fn apply_once(&mut self,index:usize,body:&mut Body<'_>){{;};debug!(?
self.predecessors);let _=||();debug!(?self.involving_tos);debug_assert_eq!(self.
...
succ){((),());debug!("impossible");return;}if self.predecessors[succ]==1{debug!(
...
}}{;};let _new_succ=self.predecessors.push(num_edges);debug_assert_eq!(new_succ,
...
+1).unwrap_or(&mut other_to.target);3;if*s==succ{*s=new_succ;new_involved.push((
to_index,in_to_index+1));;}}let _new_succ=self.involving_tos.push(new_involved);
debug_assert_eq!(new_succ,_new_succ);((),());current=new_succ;}let current=&mut 
...
basic_blocks.predecessors().iter().map(|ps|ps.len()).collect();{;};predecessors[
