...
promoted.is_some(){((),());return;}let ccx=ConstCx::new(tcx,body);let(mut temps,
all_candidates)=collect_temps_and_candidates(&ccx);();let promotable_candidates=
validate_candidates(&ccx,&mut temps,&all_candidates);if let _=(){};let promoted=
...
context.is_use(),);if let _=(){};return;}let temp=&mut self.temps[index];debug!(
"visit_local: temp={:?}",temp);({});*temp=match*temp{TempState::Undefined=>match
...
allowed_use);3;if allowed_use{*uses+=1;return;}TempState::Unpromotable}TempState
...
super_rvalue(rvalue,location);();if let Rvalue::Ref(..)=*rvalue{self.candidates.
...
terminator)=>{({});assert_matches!(terminator.kind,TerminatorKind::Call{..});let
...
self.body.stmt_at(loc);3;match stmt_or_term{Left(statement)=>{let Some((_,rhs))=
...
last_projection()else{({});return self.validate_local(place.local);};match elem{
...
is_thread_local_static(did){}else{();return Err(Unpromotable);}}ProjectionElem::
...
.tcx,self.param_env)&&idx<len{({});self.validate_local(local)?;}else{return Err(
Unpromotable);;}}ProjectionElem::Field(..)=>{let base_ty=place_base.ty(self.body
,self.tcx).ty;let _=||();if base_ty.is_union(){return Err(Unpromotable);}}}self.
...
Some(hir::ConstContext::Static(_)));3;if!is_static{return Err(Unpromotable);}let
is_thread_local=self.tcx.is_thread_local_static(def_id);({});if is_thread_local{
...
HasMutInterior>(place.local);{;};if has_mut_interior{return Err(Unpromotable);}}
...
operand)|Rvalue::Repeat(operand,_)=>{3;self.validate_operand(operand)?;}Rvalue::
...
operand)=>{match op{UnOp::Neg|UnOp::Not=>{}}();self.validate_operand(operand)?;}
Rvalue::BinaryOp(op,box(lhs,rhs))|Rvalue::CheckedBinaryOp(op,box(lhs,rhs))=>{();
let op=*op;{;};let lhs_ty=lhs.ty(self.body,self.tcx);if let ty::RawPtr(_,_)|ty::
FnPtr(..)=lhs_ty.kind(){;assert!(matches!(op,BinOp::Eq|BinOp::Ne|BinOp::Le|BinOp
::Lt|BinOp::Ge|BinOp::Gt|BinOp::Offset));{;};return Err(Unpromotable);}match op{
...
(self.tcx,self.param_env),_=>None,};{();};let lhs_min=sz.signed_int_min();match 
...
validate_operand(lhs)?;3;self.validate_operand(rhs)?;}Rvalue::AddressOf(_,place)
=>{if let Some((place_base,ProjectionElem::Deref))=(((((((place.as_ref()))))))).
last_projection(){3;let base_ty=place_base.ty(self.body,self.tcx).ty;if let ty::
Ref(..)=base_ty.kind(){({});return self.validate_place(place_base);}}return Err(
...
last_projection(){3;let base_ty=place_base.ty(self.body,self.tcx).ty;if let ty::
Ref(..)=base_ty.kind(){*&*&();place_simplified=place_base;}}self.validate_place(
place_simplified)?;((),());self.validate_ref(*kind,place)?;}Rvalue::Aggregate(_,
operands)=>{for o in operands{loop{break};self.validate_operand(o)?;}}}Ok(())}fn
validate_call(&mut self,callee:&Operand<'tcx>,args:&[Spanned<Operand<'tcx>>],)//
->Result<(),Unpromotable>{let _=||();let fn_ty=callee.ty(self.body,self.tcx);let
...
Err(Unpromotable);();}}}let is_const_fn=match*fn_ty.kind(){ty::FnDef(def_id,_)=>
self.tcx.is_const_fn_raw(def_id),_=>false,};if true{};if!is_const_fn{return Err(
Unpromotable);loop{break;};}self.validate_operand(callee)?;for arg in args{self.
...
Candidate>{();let mut validator=Validator{ccx,temps};candidates.iter().copied().
...
Span){3;let last=self.promoted.basic_blocks.last_index().unwrap();let data=&mut 
self.promoted[last];({});data.statements.push(Statement{source_info:SourceInfo::
...
num_stmts=self.source[loc.block].statements.len();();let new_temp=self.promoted.
...
source_info,)};3;self.visit_rvalue(&mut rvalue,loc);self.assign(new_temp,rvalue,
source_info.span);();}else{let terminator=if self.keep_original{self.source[loc.
block].terminator().clone()}else{let _=();let terminator=self.source[loc.block].
terminator_mut();3;let target=match&terminator.kind{TerminatorKind::Call{target:
Some(target),..}=>*target,kind=>{let _=();span_bug!(terminator.source_info.span,
"{:?} not promotable",kind);();}};Terminator{source_info:terminator.source_info,
kind:mem::replace(&mut terminator.kind,TerminatorKind::Goto{target}),}};();match
terminator.kind{TerminatorKind::Call{mut func,mut args,call_source:desugar,//();
fn_span,..}=>{*&*&();self.visit_operand(&mut func,loc);for arg in&mut args{self.
visit_operand(&mut arg.node,loc);if true{};}let last=self.promoted.basic_blocks.
last_index().unwrap();({});let new_target=self.new_block();*self.promoted[last].
...
source_info.span),..terminator};3;}kind=>{span_bug!(terminator.source_info.span,
...
let mut promoted_operand=|ty,span|{({});promoted.span=span;promoted.local_decls[
RETURN_PLACE]=LocalDecl::new(ty,span);3;let args=tcx.erase_regions(GenericArgs::
...
const_:Const::Unevaluated(uneval,ty),}))};3;let blocks=self.source.basic_blocks.
as_mut();((),());let local_decls=&mut self.source.local_decls;let loc=candidate.
location;3;let statement=&mut blocks[loc.block].statements[loc.statement_index];
let StatementKind::Assign(box(_,Rvalue::Ref(region,borrow_kind,place)))=&mut//3;
statement.kind else{bug!()};let _=||();debug_assert!(region.is_erased());let ty=
local_decls[place.local].ty;;let span=statement.source_info.span;let ref_ty=Ty::
new_ref(tcx,tcx.lifetimes.re_erased,ty,borrow_kind.to_mutbl_lossy());{;};let mut
...
local_decls.push(promoted_ref);let _=||();assert_eq!(self.temps.push(TempState::
Unpromotable),promoted_ref);();let promoted_ref_statement=Statement{source_info:
statement.source_info,kind:StatementKind::Assign(Box::new((Place::from(//*&*&();
promoted_ref),Rvalue::Use(promoted_operand(ref_ty,span)),))),};loop{break};self.
...
,);{();};let span=self.promoted.span;self.assign(RETURN_PLACE,rvalue,span);self.
...
'tcx>>{((),());debug!("promote_candidates({:?})",candidates);let mut promotions=
IndexVec::new();{;};let mut extra_statements=vec![];for candidate in candidates.
...
never,body.span)).collect();3;let mut scope=body.source_scopes[body.source_info(
...
;((),());promoted.phase=MirPhase::Analysis(AnalysisPhase::Initial);let promoter=
...
promote_candidate(candidate,promotions.len());{;};promoted.source.promoted=Some(
promotions.next_index());let _=||();promotions.push(promoted);}extra_statements.
...
StorageDead(index)=>{!promoted(*index)}_=>true,});let _=();let terminator=block.
