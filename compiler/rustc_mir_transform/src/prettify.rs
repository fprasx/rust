...
len())};();for local in(0..=body.arg_count).map(Local::from_usize){finder.track(
...
track(local);let _=();}if finder.map.iter().is_sorted(){return;}let mut updater=
...
local);3;}updater.visit_body_preserves_cfg(body);permute(&mut body.local_decls,&
...
into_iter_enumerated().collect();({});enumerated.sort_by_key(|p|map[p.0]);*data=
...
track(&mut self,l:Local){if self.seen.insert(l){3;self.map.push(l);}}}impl<'tcx>
Visitor<'tcx>for LocalFinder{fn visit_local(&mut self,l:Local,context://((),());
PlaceContext,_location:Location){if context.is_use(){{;};self.track(l);}}}struct
