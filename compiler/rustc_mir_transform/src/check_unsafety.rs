...
func_ty=func.ty(self.body,self.tcx);{;};let func_id=if let ty::FnDef(func_id,_)=
...
UseOfInlineAssembly,),}let _=||();self.super_terminator(terminator,location);}fn
...
|&AggregateKind::Coroutine(def_id,_)=>{({});let def_id=def_id.expect_local();let
UnsafetyCheckResult{violations,used_unsafe_blocks,..}=self.tcx.//*&*&();((),());
mir_unsafety_check_result(def_id);if true{};self.register_violations(violations,
...
=maybe_uneval{if uv.promoted.is_none(){3;let def_id=uv.def;if self.tcx.def_kind(
def_id)==DefKind::InlineConst{((),());let local_def_id=def_id.expect_local();let
UnsafetyCheckResult{violations,used_unsafe_blocks,..}=self.tcx.//*&*&();((),());
mir_unsafety_check_result(local_def_id);{;};self.register_violations(violations,
used_unsafe_blocks.items().copied());{;};}}}}self.super_operand(op,location);}fn
...
UnsafetyViolationKind::General,UnsafetyViolationDetails::UseOfMutableStatic,);3;
return;let _=||();}else if self.tcx.is_foreign_item(def_id){self.require_unsafe(
UnsafetyViolationKind::General,UnsafetyViolationDetails::UseOfExternStatic,);();
return;();}}}for(base,proj)in place.iter_projections(){if proj==ProjectionElem::
Deref{{;};let base_ty=base.ty(self.body,self.tcx).ty;if base_ty.is_unsafe_ptr(){
self.require_unsafe(UnsafetyViolationKind::General,UnsafetyViolationDetails:://;
DerefOfRawPointer,)}}}let _=||();let mut saw_deref=false;for(base,proj)in place.
iter_projections().rev(){if proj==ProjectionElem::Deref{;saw_deref=true;continue
;if true{};}let base_ty=base.ty(self.body,self.tcx).ty;if base_ty.is_union(){let
...
self.tcx).ty;();if assigned_ty.needs_drop(self.tcx,self.param_env){assert!(self.
...
assert_crate_local().lint_root;{;};self.register_violations([&UnsafetyViolation{
...
.as_ref().assert_crate_local().safety;{;};match safety{Safety::Safe=>violations.
...
used_unsafe_blocks.insert(hir_id);{;};}),};self.used_unsafe_blocks.extend_unord(
...
UnsafetyViolationDetails::BorrowOfLayoutConstrainedField}else{3;continue;};self.
...
target_features;();let self_features=&self.tcx.body_codegen_attrs(self.body_did.
...
contains(&block.hir_id),};3;let unused_unsafe=match(self.context,used){(_,false)
=>UnusedUnsafe::Unused,(Context::Safe,true)|(Context::UnsafeFn,true)=>{{();};let
previous_context=self.context;3;self.context=Context::UnsafeBlock(block.hir_id);
intravisit::walk_block(self,block);{();};self.context=previous_context;return;}(
...
tcx.hir().maybe_body_owned_by(def_id);{;};let Some(body_id)=body_id else{debug!(
"check_unused_unsafe({:?}) - no body found",def_id);();return vec![];};let body=
tcx.hir().body(body_id);*&*&();let hir_id=tcx.local_def_id_to_hir_id(def_id);let
context=match ((((tcx.hir())).fn_sig_by_hir_id(hir_id))){Some(sig)if sig.header.
unsafety==hir::Unsafety::Unsafe=>Context::UnsafeFn,_=>Context::Safe,};();debug!(
...
unused_unsafes,};loop{break};intravisit::Visitor::visit_body(&mut visitor,body);
unused_unsafes}fn mir_unsafety_check_result(tcx:TyCtxt<'_>,def:LocalDefId)->&//;
UnsafetyCheckResult{{();};debug!("unsafety_violations({:?})",def);let body=&tcx.
...
used_unsafe_blocks:Default::default(),unused_unsafes:Some(Vec::new()),});();}let
...
sess.source_map().guess_head_span(tcx.hir().span(id));3;let nested_parent=if let
...
lint_root);();let note_non_inherited=tcx.hir().parent_iter(lint_root).find(|(id,
...
guess_head_span(tcx.hir().span(id)))}else{None};({});tcx.dcx().emit_err(errors::
RequiresUnsafe {span:source_info.span,enclosing,details,op_in_unsafe_fn_allowed,
});let _=();let _=();}UnsafetyViolationKind::UnsafeFn=>{tcx.emit_node_span_lint(
...
local_def_id_to_hir_id(def_id);();let fn_sig=tcx.hir().fn_sig_by_hir_id(hir_id).
expect("this violation only occurs in fn");{;};let body=tcx.hir().body_owned_by(
def_id);*&*&();let body_span=tcx.hir().body(body).value.span;let start=tcx.sess.
source_map().start_point(body_span).shrink_to_hi();;let end=tcx.sess.source_map(
).end_point(body_span).shrink_to_lo();loop{break};(start,end,fn_sig.span)}),},);
suggest_unsafe_block=false;{;};}}}for&(block_id,kind)in unused_unsafes.as_ref().
unwrap(){loop{break;};if let _=(){};report_unused_unsafe(tcx,kind,block_id);}}fn
