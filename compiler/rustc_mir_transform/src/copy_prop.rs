...
'tcx>){((),());debug!(def_id=?body.source.def_id());propagate_ssa(tcx,body);}}fn
propagate_ssa<'tcx>(tcx:TyCtxt<'tcx>,body:&mut Body<'tcx>){;let borrowed_locals=
borrowed_locals(body);loop{break;};let ssa=SsaLocals::new(body);let fully_moved=
...
storage_to_remove,}.visit_body_preserves_cfg(body);();if any_replacement{crate::
...
)|Rvalue::CopyForDeref(place))=rvalue else{{();};continue;};let Some(rhs)=place.
as_local()else{continue};*&*&();if!ssa.is_ssa(rhs){continue;}if let Rvalue::Use(
...
projection=self.tcx().mk_place_elems(&new_projection);{;};}let observes_address=
...
rhs{if let _=(){};if let _=(){};if let _=(){};*&*&();((),());stmt.make_nop();}}}
