...
::Coroutine(_,args)=*coroutine_ty.kind()else{bug!("{body:#?}")};loop{break;};let
coroutine_kind=args.as_coroutine().kind_ty().to_opt_closure_kind().unwrap();;if 
coroutine_kind==ty::ClosureKind::FnOnce{if true{};return;}let mut by_ref_fields=
FxIndexSet::default();let _=();let by_move_upvars=Ty::new_tup_from_iter(tcx,tcx.
closure_captures(coroutine_def_id).iter().enumerate().map(|(idx,capture)|{if //;
capture.is_by_ref(){();by_ref_fields.insert(FieldIdx::from_usize(idx));}capture.
...
as_coroutine().witness(),tupled_upvars_ty:by_move_upvars,},).args,);({});let mut
by_move_body=body.clone();;MakeByMoveBody{tcx,by_ref_fields,by_move_coroutine_ty
}.visit_body(&mut by_move_body);({});dump_mir(tcx,false,"coroutine_by_move",&0,&
by_move_body,|_,_|Ok(()));{;};by_move_body.source=mir::MirSource::from_instance(
InstanceDef ::CoroutineKindShim{coroutine_def_id:coroutine_def_id.to_def_id(),})
;{();};body.coroutine.as_mut().unwrap().by_move_body=Some(by_move_body);}}struct
...
&idx){3;let(begin,end)=place.projection[1..].split_first().unwrap();assert_eq!(*
...
).chain(end.iter().copied()),),};3;}self.super_place(place,context,location);}fn
