...
always_live_locals=&always_storage_live_locals(body);{;};let maybe_storage_live=
MaybeStorageLive ::new(Cow::Borrowed(always_live_locals)).into_engine(tcx,body).
iterate_to_fixpoint().into_results_cursor(body);let _=();let maybe_storage_dead=
...
in traversal::reachable(body){({});lint.visit_basic_block_data(bb,data);}}struct
...
.body.source_info(location).span;({});self.tcx.sess.dcx().span_delayed_bug(span,
...
TerminatorKind::Call{args,destination,..}=>{{;};self.places.clear();self.places.
insert(destination.as_ref());{;};let mut has_duplicates=false;for arg in args{if
let Operand::Move(place)=&arg.node{();has_duplicates|=!self.places.insert(place.
as_ref());let _=||();loop{break};}}if has_duplicates{self.fail(location,format!(
"encountered overlapping memory in `Move` arguments to `Call` terminator: {:?}" 
,terminator.kind,),);{();};}}_=>{}}self.super_terminator(terminator,location);}}
