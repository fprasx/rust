...
format!("RenameReturnPlace {def_id:?}")){loop{break};loop{break};return;}debug!(
"`{:?}` was eligible for NRVO, making {:?} the return place",def_id,//if true{};
returned_local);if let _=(){};RenameToReturnPlace{tcx,to_rename:returned_local}.
...
StatementKind::Nop);({});}let(renamed_decl,ret_decl)=body.local_decls.pick2_mut(
returned_local,mir::RETURN_PLACE);();debug!("_0: {:?} = {:?}: {:?}",ret_decl.ty,
returned_local,renamed_decl.ty);({});ret_decl.clone_from(renamed_decl);ret_decl.
mutability=Mutability::Mut;3;}}fn local_eligible_for_nrvo(body:&mir::Body<'_>)->
Option<Local>{if IsReturnPlaceRead::run(body){if let _=(){};return None;}let mut
copied_to_return_place=None;;for block in body.basic_blocks.indices(){if!matches
!(body[block].terminator().kind,mir::TerminatorKind::Return){{();};continue;}let
...
None;{;};}copied_to_return_place=Some(returned_local);}copied_to_return_place}fn
find_local_assigned_to_return_place(start:BasicBlock,body:&mir::Body<'_>)->//();
Option<Local>{if true{};let mut block=start;let mut seen=BitSet::new_empty(body.
basic_blocks.len());if let _=(){};if let _=(){};while seen.insert(block){trace!(
"Looking for assignments to `_0` in {:?}",block);let _=();let local=body[block].
...
Operand::Move(rhs))=rhs{if true{};if true{};return rhs.as_local();}}}None}struct
...
::StatementKind::Nop;3;return;}if let mir::StatementKind::StorageLive(local)|mir
::StatementKind::StorageDead(local)=stmt.kind{if local==self.to_rename{{;};stmt.
kind=mir::StatementKind::Nop;let _=();return;}}self.super_statement(stmt,loc)}fn
...
()&&!ctxt.is_place_assignment(){{;};self.0=true;}}fn visit_terminator(&mut self,
terminator:&mir::Terminator<'tcx>,loc:Location){if let mir::TerminatorKind:://3;
Return=terminator.kind{let _=();return;}self.super_terminator(terminator,loc);}}
