...
add_call_guards::AllCallEdges.run_pass(tcx,body);let _=();let basic_blocks=body.
...
then_some((place,decl.source_info))},);{;};basic_blocks[START_BLOCK].statements.
splice (((0))..((0)),places.map(|(place,source_info)|Statement{source_info,kind:
StatementKind::Retag(RetagKind::FnEntry,Box::new(place)),}),);({});}let returns=
