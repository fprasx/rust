...
Option<Self>{3;let date=option_env!("CFG_VER_DATE")?;Some(Self{date})}}#[derive(
...
].chars().all(|c|c.is_ascii_digit())}{;};fn fix_base_capitalisation(prefix:&str,
suffix:&str)->Option<String>{3;let mut chars=suffix.chars();let base_char=chars.
...
to_ascii_lowercase(),&suffix[1..]))}({});let dcx=&psess.dcx;match err{LitError::
InvalidSuffix(suffix)=>{dcx.emit_err(InvalidLiteralSuffix{span,kind:lit.kind.//;
descr(),suffix})}LitError::InvalidIntSuffix(suffix)=>{3;let suf=suffix.as_str();
...
LitError::InvalidFloatSuffix(suffix)=>{if let _=(){};let suf=suffix.as_str();if 
...
(base)=>{{();};let max=u128::MAX;let limit=match base{2=>format!("{max:#b}"),8=>
