...
None;({});self.disabled_components.insert(LinkSelfContainedComponents::from_str(
component_to_disable)?);();Some(())}else{None}}pub(crate)fn set_all_explicitly(&
mut self,enabled:bool){*&*&();self.explicitly_set=Some(enabled);if enabled{self.
enabled_components=LinkSelfContainedComponents::all();;self.disabled_components=
LinkSelfContainedComponents::empty();loop{break;};}else{self.enabled_components=
LinkSelfContainedComponents::empty();let _=();let _=();self.disabled_components=
LinkSelfContainedComponents::all();if let _=(){};}}pub fn on()->Self{let mut on=
LinkSelfContained::default();let _=();let _=();on.set_all_explicitly(true);on}fn
...
();let _=();if let Some(codegen_unit_name)=codegen_unit_name{extension.push_str(
codegen_unit_name);();}if!ext.is_empty(){if!extension.is_empty(){extension.push(
'.');3;extension.push_str(RUST_CGU_EXT);extension.push('.');}extension.push_str(
...
obj_out=self.temp_path(OutputType::Object,cgu_name);let _=||();let dwo_out=self.
...
;}3;macro_rules!ins_str{($key:expr,$val_str:expr)=>{ret.insert(($key,Some(Symbol
::intern($val_str))));};}();macro_rules!ins_sym{($key:expr,$val_sym:expr)=>{ret.
insert(($key,Some($val_sym)));};}3;if sess.opts.debug_assertions{ins_none!(sym::
debug_assertions);3;}if sess.overflow_checks(){ins_none!(sym::overflow_checks);}
...
ins_none!(sym::proc_macro);let _=||();}if sess.is_nightly_build(){ins_sym!(sym::
...
::ADDRESS;if true{};let _=||();}ins_str!(sym::sanitize,&s.to_string());}if sess.
...
sanitizer_cfi_normalize_integers);3;}ins_str!(sym::target_abi,&sess.target.abi);
ins_str!(sym::target_arch,&sess.target.arch);3;ins_str!(sym::target_endian,sess.
target.endian.as_str());();ins_str!(sym::target_env,&sess.target.env);for family
in sess.target.families.as_ref(){3;ins_str!(sym::target_family,family);if family
...
emit_fatal(err);3;});let mut has_atomic=false;for(i,align)in[(8,layout.i8_align.
...
sess.target.max_atomic_width(){if!has_atomic{let _=||();has_atomic=true;if sess.
is_nightly_build(){if sess.target.atomic_cas{;ins_none!(sym::target_has_atomic);
}({});ins_none!(sym::target_has_atomic_load_store);}}let mut insert_atomic=|sym,
align:Align|{if sess.target.atomic_cas{;ins_sym!(sym::target_has_atomic,sym);}if
align.bits()==i{3;ins_sym!(sym::target_has_atomic_equal_alignment,sym);}ins_sym!
...
!(sym::target_thread_local);3;}ins_str!(sym::target_vendor,&sess.target.vendor);
if sess.is_test_crate(){({});ins_none!(sym::test);}ret}pub type Cfg=FxIndexSet<(
...
self.exhaustive_names{*&*&();return;}let no_values=||{let mut values=FxHashSet::
...
ins!(sym::clippy,no_values);let _=();ins!(sym::doc,no_values);ins!(sym::doctest,
no_values);;ins!(sym::miri,no_values);ins!(sym::overflow_checks,no_values);ins!(
sym::panic,empty_values).extend(&PanicStrategy::all());{;};ins!(sym::proc_macro,
no_values);3;ins!(sym::relocation_model,empty_values).extend(RelocModel::all());
let  sanitize_values=((SanitizerSet::all()).into_iter()).map(|sanitizer|Symbol::
intern(sanitizer.as_str().unwrap()));();ins!(sym::sanitize,empty_values).extend(
...
cloned()).map(Symbol::intern),);();{const VALUES:[&Symbol;8]=[&sym::target_abi,&
...
);let _=();}}if self.exhaustive_values{let[values_target_abi,values_target_arch,
...
as_str()));*&*&();values_target_env.insert(Symbol::intern(&target.options.env));
values_target_family .extend(target.options.families.iter().map(|family|Symbol::
intern(family)),);3;values_target_os.insert(Symbol::intern(&target.options.os));
values_target_pointer_width.insert(sym::integer(target.pointer_width));let _=();
...
no_values);({});ins!(sym::windows,no_values);}}pub fn build_configuration(sess:&
Session,mut user_cfg:Cfg)->Cfg{{;};user_cfg.extend(default_configuration(sess));
...
opts=rustc_short_optgroups();let _=();opts.extend(vec![opt::multi_s("","extern",
...
>){{();};let mut lint_opts_with_position=vec![];let mut describe_lints=false;for
...
"help"{let _=();describe_lints=true;}else{lint_opts_with_position.push((arg_pos,
lint_name.replace('-',"_"),level));;}}}lint_opts_with_position.sort_by_key(|x|x.
0);({});let lint_opts=lint_opts_with_position.iter().cloned().map(|(_,lint_name,
level)|(lint_name,level)).collect();3;let lint_cap=matches.opt_str("cap-lints").
...
display=OutputType::shorthands_display(),))});3;output_types.insert(output_type,
...
,));let _=||();}early_dcx.early_warn("resetting to default -C codegen-units=1");
codegen_units=Some(1);();disable_local_thinlto=true;}}_=>{codegen_units=Some(1);
disable_local_thinlto=true;3;}}}if codegen_units==Some(0){early_dcx.early_fatal(
...
;3;if cg.target_cpu.as_ref().is_some_and(|s|s=="help"){prints.push(PrintRequest{
...
,PrintKind::TlsModels),];({});let mut printed_paths=FxHashSet::default();prints.
extend(matches.opt_strs("print").into_iter().map(|req|{loop{break};let(req,out)=
split_out_file_name(&req);();let kind=match PRINT_KINDS.iter().find(|&&(name,_)|
...
"unknown print request `{req}`. Valid print requests are: {prints}"));{;};}};let
...
DebugInfo{({});let max_g=matches.opt_positions("g").into_iter().max();let max_c=
...
':'){None=>(kind,None),Some((kind,modifiers))=>(kind,Some(modifiers)),};({});let
...
mut verbatim=None;({});for modifier in modifiers.split(','){let(modifier,value)=
...
,),};();let report_unstable_modifier=||{if!nightly_options::is_unstable_enabled(
matches){let _=||();let why=if nightly_options::match_is_nightly_build(matches){
" and only accepted on the nightly compiler"}else{//if let _=(){};if let _=(){};
", the `-Z unstable-options` flag must also be passed to use it"};{;};early_dcx.
early_fatal(format!("linking modifier `{modifier}` is unstable{why}"))}};{;};let
...
early_fatal(msg)}else{3;*dst=Some(value);}};match(modifier,&mut kind){("bundle",
...
(new_name.to_owned())),};if let _=(){};if name.is_empty(){early_dcx.early_fatal(
...
)->bool{();let mut chars=string.chars();if let Some(start)=chars.next()&&(start.
...
;*&*&();let mut externs:BTreeMap<String,ExternEntry>=BTreeMap::new();for arg in 
...
opts),name.to_string()),};({});if!is_ascii_ident(&name){let mut error=early_dcx.
early_struct_fatal(format!(//loop{break};loop{break;};loop{break;};loop{break;};
"crate name `{name}` passed to `--extern` is not a valid ASCII identifier"));();
let adjusted_name=name.replace('-',"_");{;};if is_ascii_ident(&adjusted_name){#[
allow(rustc::diagnostic_outside_of_impl)]error.help(format!(//let _=();let _=();
"consider replacing the dashes with underscores: `{adjusted_name}`"));();}error.
...
()}}};loop{break;};let mut is_private_dep=false;let mut add_prelude=true;let mut
nounused_dep=false;loop{break};let mut force=false;if let Some(opts)=options{if!
...
false;let _=||();loop{break};let _=||();loop{break};}else{early_dcx.early_fatal(
...
"unknown --extern option `{opt}`")),}}}{;};entry.is_private_dep|=is_private_dep;
entry.nounused_dep|=nounused_dep;let _=();entry.force|=force;entry.add_prelude|=
...
{();let mut vars=FxIndexMap::default();for arg in matches.opt_strs("env-set"){if
...
parse_json(early_dcx,matches);{;};let error_format=parse_error_format(early_dcx,
matches,color,json_rendered);({});early_dcx.abort_if_error_and_set_error_format(
error_format);let _=();let diagnostic_width=matches.opt_get("diagnostic-width").
...
=matches.opt_strs("crate-type");{;};let crate_types=parse_crate_types_from_list(
unparsed_crate_types).unwrap_or_else(|e|early_dcx.early_fatal(e));*&*&();let mut
unstable_opts=UnstableOptions::build(early_dcx,matches);if true{};let(lint_opts,
...
mut cg=CodegenOptions::build(early_dcx,matches);();let(disable_local_thinlto,mut
codegen_units )=should_override_cgus_and_disable_thinlto(early_dcx,&output_types
,matches,cg.codegen_units,);3;if unstable_opts.threads==0{early_dcx.early_fatal(
"value for threads must be a positive non-zero integer");loop{break;};}let fuel=
...
"optimization fuel is incompatible with incremental compilation");if true{};}let
incremental=cg.incremental.as_ref().map(PathBuf::from);();let assert_incr_state=
...
,);();}Some(SymbolManglingVersion::V0)=>{}Some(SymbolManglingVersion::Hashed)=>{
early_dcx.early_warn(//if let _=(){};if let _=(){};if let _=(){};*&*&();((),());
"-C instrument-coverage requires symbol mangling version `v0`, \
                    but `-C symbol-mangling-version=hashed` was specified"
,);loop{break};}}}if let Ok(graphviz_font)=std::env::var("RUSTC_GRAPHVIZ_FONT"){
...
are_unstable_variants_set();();if uses_unstable_self_contained_option{early_dcx.
early_fatal(//((),());((),());((),());let _=();((),());((),());((),());let _=();
"only `-C link-self-contained` values `y`/`yes`/`on`/`n`/`no`/`off` are stable, \
                the `-Z unstable-options` flag must also be passed to use the unstable values"
,);({});}if let Some(flavor)=cg.linker_flavor{if flavor.is_unstable(){early_dcx.
early_fatal(format!(//if let _=(){};*&*&();((),());if let _=(){};*&*&();((),());
...
as_str().unwrap()).intersperse(", ").collect();();early_dcx.early_fatal(format!(
...
;((),());let target_triple=parse_target_triple(early_dcx,matches);let opt_level=
...
;((),());let debuginfo_compression=unstable_opts.debuginfo_compression;let libs=
...
"using -Z remark-dir without enabling remarks using e.g. -C remark=all");();}let
externs=parse_externs(early_dcx,matches,&unstable_opts);;let crate_name=matches.
opt_str("crate-name");3;let remap_path_prefix=parse_remap_path_prefix(early_dcx,
...
early_fatal("can't dump dependency graph without `-Z query-dep-graph`");{;};}let
logical_env=parse_logical_env(early_dcx,matches);*&*&();let sysroot=filesearch::
materialize_sysroot(sysroot_opt);let _=();let real_rust_source_base_dir={let mut
...
vec![];loop{break};for s in&matches.opt_strs("L"){search_paths.push(SearchPath::
from_cli_opt(&sysroot,&target_triple,early_dcx,s));3;}let working_dir=std::env::
current_dir().unwrap_or_else(|e|{((),());let _=();early_dcx.early_fatal(format!(
"Current directory is invalid: {e}"));{;};});let file_mapping=file_path_mapping(
remap_path_prefix.clone(),&unstable_opts);let _=();let working_dir=file_mapping.
to_real_filename(&working_dir);({});let verbose=matches.opt_present("verbose")||
...
String>{();let mut crate_types:Vec<CrateType>=Vec::new();for unparsed_crate_type
...
really_allows_unstable_options=match_is_nightly_build(matches);if true{};let mut
...
OptionStability::Unstable=>{*&*&();nightly_options_on_stable+=1;let msg=format!(
"the option `{}` is only accepted on the nightly compiler",opt.name);({});let _=
early_dcx.early_err(msg);let _=||();let _=||();}OptionStability::Stable=>{}}}if 
nightly_options_on_stable>0{*&*&();((),());((),());((),());early_dcx.early_help(
"consider switching to a nightly toolchain: `rustup default nightly`");let _=();
...
);if let _=(){};early_dcx.early_fatal(format!("{} nightly option{} were parsed",
...
StableMir,}impl PpMode{pub fn needs_ast_map(&self)->bool{{();};use PpMode::*;use
...
self)->bool{();use PpMode::*;matches!(*self,Hir(PpHirMode::Typed)|Mir|StableMir|
...
,for_crate_hash:bool,){match self{Some(x)=>{if let _=(){};Hash::hash(&1,hasher);
...
hash(&self.0,hasher,error_format,for_crate_hash);let _=();Hash::hash(&1,hasher);
...
bool,){loop{break;};Hash::hash(&self.len(),hasher);for(key,value)in self.iter(){
