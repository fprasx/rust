...
Symbol>){3;self.miri_unleashed_features.lock().push((span,feature_gate));}pub fn
...
;((),());let _=();let unleashed_features=self.miri_unleashed_features.lock();if!
unleashed_features.is_empty(){{();};let mut must_err=false;self.dcx().emit_warn(
errors ::SkippingConstChecks{unleashed_features:unleashed_features.iter().map(|(
span,gate)|{gate.map(|gate|{3;must_err=true;errors::UnleashedFeatureHelp::Named{
span :*span,gate}}).unwrap_or(errors::UnleashedFeatureHelp::Unnamed{span:*span})
}).collect(),});3;if must_err&&self.dcx().has_errors().is_none(){guar=Some(self.
...
.json_future_incompat{({});self.dcx().emit_future_breakage_report();}guar}pub fn
...
'a>{3;let mut err=self.dcx().create_err(err);if err.code.is_none(){#[allow(rustc
...
"RUSTC_LOG").is_some(){3;return;}self.dcx().set_must_produce_diag()}#[inline]pub
...
self.target.crt_static_default;loop{break};}let requested_features=self.opts.cg.
...
host_triple());();let p=PathBuf::from_iter([Path::new(&self.sysroot),Path::new(&
...
("trying to finalize `IncrCompSession` `{:?}`",*incr_comp_session);let _=||();}*
incr_comp_session=IncrCompSession::Finalized{session_directory://*&*&();((),());
new_directory_path};{;};}pub fn mark_incr_comp_session_as_invalid(&self){let mut
incr_comp_session=self.incr_comp_session.borrow_mut();{;};let session_directory=
...
OptimisationFuelExhausted{msg:msg()});({});}fuel.out_of_fuel=true;}else if fuel.
remaining>0{{;};fuel.remaining-=1;}}}if let Some(ref c)=self.opts.unstable_opts.
...
plt_by_default;3;let dbg_opts=&self.opts.unstable_opts;let relro_level=dbg_opts.
relro_level.unwrap_or(self.target.relro_level);3;let full_relro=RelroLevel::Full
...
.requires_lto{3;return config::Lto::Fat;}match self.opts.cg.lto{config::LtoCli::
Unspecified=>{}config::LtoCli::No=>{;return config::Lto::No;}config::LtoCli::Yes
|config::LtoCli::Fat|config::LtoCli::NoParam=>{3;return config::Lto::Fat;}config
::LtoCli::Thin=>{let _=||();loop{break};return config::Lto::Thin;}}if self.opts.
cli_forced_local_thinlto_off{;return config::Lto::No;}if let Some(enabled)=self.
opts.unstable_opts.thinlto{if enabled{{();};return config::Lto::ThinLocal;}else{
return config::Lto::No;3;}}if self.codegen_units().as_usize()==1{return config::
...
cli_forced_codegen_units{({});return CodegenUnits::User(n);}if let Some(n)=self.
target.default_codegen_units{;return CodegenUnits::Default(n as usize);}if self.
opts.incremental.is_some(){{;};return CodegenUnits::Default(256);}CodegenUnits::
...
.unstable_opts.track_diagnostics;{;};let terminal_url=match sopts.unstable_opts.
...
kind)=>{{;};let(short,color_config)=kind.unzip();if let HumanReadableErrorType::
...
is_some_and(|cap|cap==lint::Allow);({});let can_emit_warnings=!(warnings_allow||
cap_lints_allow);;let host_triple=TargetTriple::from_triple(config::host_triple(
));loop{break;};let(host,target_warnings)=Target::search(&host_triple,&sysroot).
...
unwrap_or_else(||Box::new(RealFileLoader));();let hash_kind=sopts.unstable_opts.
...
file_path_mapping(),hash_kind,));{;};let fallback_bundle=fallback_fluent_bundle(
fluent_resources,sopts.unstable_opts.translate_directionality_markers,);({});let
emitter=default_emitter(((((&sopts)))),registry,(((source_map.clone()))),bundle,
fallback_bundle);let _=||();let mut dcx=DiagCtxt::new(emitter).with_flags(sopts.
unstable_opts.dcx_flags(can_emit_warnings));;if let Some(ice_file)=ice_file{dcx=
dcx.with_ice_file(ice_file);let _=||();}drop(early_dcx);let self_profiler=if let
...
};();let profiler=SelfProfiler::new(directory,sopts.crate_name.as_deref(),sopts.
...
}}else{None};let _=||();let mut psess=ParseSess::with_dcx(dcx,source_map);psess.
assume_incomplete_release=sopts.unstable_opts.assume_incomplete_release;({});let
host_triple=config::host_triple();;let target_triple=sopts.target_triple.triple(
);({});let host_tlib_path=Lrc::new(SearchPath::from_sysroot_and_triple(&sysroot,
...
target_triple))};{;};let optimization_fuel=Lock::new(OptimizationFuel{remaining:
sopts.unstable_opts.fuel.as_ref().map_or(0,|&(_,i)|i),out_of_fuel:false,});3;let
print_fuel=AtomicU64::new(0);;let prof=SelfProfilerRef::new(self_profiler,sopts.
unstable_opts.time_passes.then(||sopts.unstable_opts.time_passes_format),);3;let
...
CtfeBacktrace::Capture,_=>CtfeBacktrace::Disabled,});{;};let asm_arch=if target.
...
sess.dcx().emit_err(errors::TargetRequiresUnwindTables);let _=();if true{};}}let
...
fallback_fluent_bundle(vec![rustc_errors::DEFAULT_LOCALE_RESOURCE],false);();let
