...
);3;debug_assert_eq!(span,removed_span);}pub fn merge(&self,mut spans:FxHashMap<
...
entry(gate).or_default().append(&mut gate_spans);{();};}*inner=spans;}}#[derive(
...
add_feature_diagnostics_for_issue(&mut err,sess,feature,issue,false,None);();let
...
future_incompatible.reference));3;err.stash(span,StashKey::EarlySyntaxWarning);}
...
.dcx(),CliFeatureDiagnosticHelp{feature});3;}else if let Some(span)=inject_span{
err.subdiagnostic(sess.dcx(),FeatureDiagnosticSuggestion{feature,span});3;}else{
...
ui_testing());3;}else if let Some(suggestion)=SuggestUpgradeCompiler::new(){err.
...
,fallback_bundle).sm(Some(sm.clone())),);((),());let dcx=DiagCtxt::new(emitter);
...
sm=Lrc::new(SourceMap::new(FilePathMapping::empty()));({});let emitter=Box::new(
HumanEmitter ::new(stderr_destination(ColorConfig::Auto),fallback_bundle.clone()
,));((),());let fatal_dcx=DiagCtxt::new(emitter);let dcx=DiagCtxt::new(Box::new(
...
msg:impl Into<DiagMessage>,){{;};self.buffered_lints.with_lock(|buffered_lints|{
