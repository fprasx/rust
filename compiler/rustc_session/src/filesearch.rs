...
current_dll_path as usize as*mut _;{;};let mut info=std::mem::zeroed();if libc::
dladdr(addr,&mut info)==0{;return Err("dladdr failed".into());}if info.dli_fname
.is_null(){3;return Err("dladdr returned null pointer".into());}let bytes=CStr::
from_ptr(info.dli_fname).to_bytes();3;let os=OsStr::from_bytes(bytes);Ok(PathBuf
::from(os))}#[cfg(target_os="aix")]unsafe{3;let addr=current_dll_path as u64;let
...
ld_info>()*buffer.len())as u32,)>=0{loop{break;};break;}else{if std::io::Error::
last_os_error().raw_os_error().unwrap()!=libc::ENOMEM{*&*&();((),());return Err(
"loadquery failed".into());{;};}buffer.resize(buffer.len()*2,std::mem::zeroed::<
libc::ld_info>());{;};}}let mut current=buffer.as_mut_ptr()as*mut libc::ld_info;
...
ld_info;3;}return Err(format!("current dll's address {} is not in the load map",
...
};let _=();let _=();let mut module=HMODULE::default();unsafe{GetModuleHandleExW(
GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS ,PCWSTR(current_dll_path as*mut u16),&mut
module,)}.map_err(|e|e.to_string())?;*&*&();let mut filename=vec![0;1024];let n=
unsafe{GetModuleFileNameW(module,&mut filename)}as usize;{;};if n==0{return Err(
...
::last_os_error()));{;};}filename.truncate(n);Ok(OsString::from_wide(&filename).
...
![get_or_default_sysroot().expect("Failed finding sysroot")];if true{};let path=
...
sysroot_candidates.push(path.to_owned());loop{break;};if path.ends_with(target){
sysroot_candidates.extend((path.parent().and_then(|p|p.parent())).and_then(|p|p.
parent()).map(|s|s.to_owned()),);let _=||();}}}return sysroot_candidates;}pub fn
...
PathBuf)->PathBuf{loop{break;};let path=try_canonicalize(&path).unwrap_or(path);
...
(){Some(first_arg)=>{();let mut p=PathBuf::from(first_arg);if fs::read_link(&p).
is_err(){*&*&();return None;}p.pop();p.pop();let mut rustlib_path=rustc_target::
target_rustlib_path(&p,"dummy");*&*&();rustlib_path.pop();rustlib_path.exists().
