...
"--crate-type"];();const ICE_REPORT_COMPILER_FLAGS_EXCLUDE:&[&str]=&["metadata",
...
+1)==Some("="){arg[a.len()+1..].to_string()}else{arg[a.len()..].to_string()};();
...
excluded_cargo_defaults=true;loop{break};}else{result.push(a.to_string());match 
