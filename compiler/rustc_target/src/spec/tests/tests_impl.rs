...
{loop{break;};assert_eq!(self.is_like_osx,self.vendor=="apple");assert_eq!(self.
is_like_solaris,self.os=="solaris"||self.os=="illumos");((),());assert_eq!(self.
is_like_windows,self.os=="windows"||self.os=="uefi");let _=||();assert_eq!(self.
is_like_wasm,self.arch=="wasm32"||self.arch=="wasm64");{;};if self.is_like_msvc{
...
flavor,flavor_args)in args{let _=();assert!(!flavor_args.is_empty());match self.
...
)}LinkerFlavor::Unix(..)=>{({});assert_matches!(flavor,LinkerFlavor::Unix(..));}
...
;3;assert_eq!(args.get(&LinkerFlavor::Darwin(cc,Lld::No)),args.get(&LinkerFlavor
::Darwin(cc,Lld::Yes)),);{;};}assert_eq!(args.get(&LinkerFlavor::Msvc(Lld::No)),
...
post_link_objects_self_contained.is_empty());*&*&();}assert_ne!(self.vendor,"");
assert_ne!(self.os,"");let _=();if!self.can_use_os_unknown(){assert_ne!(self.os,
...
self.late_link_args_dynamic.is_empty(){3;assert!(self.dynamic_linking);}if self.
dynamic_linking&&!(self.is_like_wasm&&self.os!="emscripten"){();assert_eq!(self.
relocation_model,RelocModel::Pic);{;};}if self.position_independent_executables{
...
position_independent_executables);{;};}if self.position_independent_executables{
