...
("macos",arch);3;base.max_atomic_width=Some(128);base.frame_pointer=FramePointer
::Always;;base.add_pre_link_args(LinkerFlavor::Darwin(Cc::Yes,Lld::No),&["-m64"]
);loop{break};base.supported_sanitizers=SanitizerSet::ADDRESS|SanitizerSet::CFI|
SanitizerSet::LEAK|SanitizerSet::THREAD;loop{break;};loop{break;};base.features=
"-rdrnd,-aes,-pclmul,-rtm,-fsgsbase".into();{;};assert_eq!(base.cpu,"core-avx2",
