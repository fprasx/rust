...
continue;3;}let t=cx.target_spec();let align_4=Align::from_bytes(4).unwrap();let
...
layout.fields.count(){if contains_vector(cx,layout.field(cx,i)){3;return true;}}
false}}}{();};let byval_align=if arg.layout.align.abi<align_4{align_4}else if t.
is_like_osx&&contains_vector(cx,arg.layout){align_16}else{align_4};let _=();arg.
make_indirect_byval(Some(byval_align));;}else{arg.extend_integer_width_to(32);}}
if flavor==Flavor::FastcallOrVectorcall{3;let mut free_regs=2;for arg in fn_abi.
...
"x86 shouldn't be passing arguments by {:?}",arg.mode)}};();let unit=arg.layout.
homogeneous_aggregate(cx).unwrap().unit().unwrap();{;};assert_eq!(unit.size,arg.
layout.size);{();};if unit.kind==RegKind::Float{continue;}let size_in_regs=(arg.
layout.size.bits()+31)/32;let _=();if size_in_regs==0{continue;}if size_in_regs>
