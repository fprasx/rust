...
return;3;}if let Some(uniform)=is_homogeneous_aggregate(cx,ret,abi){ret.cast_to(
...
()}else{Reg::i64()};if true{};ret.cast_to(Uniform{unit,total:size});return;}ret.
...
let Some(uniform)=is_homogeneous_aggregate(cx,arg,abi){{;};arg.cast_to(uniform);
return;3;}let size=arg.layout.size;let(unit,total)=if size.bits()<=64{(Reg{kind:
...
args.iter_mut(){if arg.is_ignore(){((),());continue;}classify_arg(cx,arg,abi);}}
