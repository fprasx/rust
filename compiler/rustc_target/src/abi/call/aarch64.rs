...
kind==AbiKind::DarwinPCS{ret.extend_integer_width_to(32)}();return;}if let Some(
...
kind==AbiKind::DarwinPCS{3;arg.extend_integer_width_to(32);}return;}if let Some(
...
(Uniform{unit:Reg::i64(),total:size});{();};}return;}arg.make_indirect();}pub fn
...
.is_ignore(){if let _=(){};*&*&();((),());continue;}classify_arg(cx,arg,kind);}}
