...
,ptx:PlaceContext,location:Location,){if true{};let _=||();let _=place_ref;self.
super_projection_elem(elem,ptx,location);;}fn visit_local(&mut self,local:&Local
,ptx:PlaceContext,location:Location){loop{break;};let _=(local,ptx,location);}fn
...
visit_ty(&mut self,ty:&Ty,location:Location){3;let _=location;self.super_ty(ty)}
...
}fn visit_args(&mut self,args:&GenericArgs,location:Location){();let _=location;
...
self,var_debug_info:&VarDebugInfo){3;self.super_var_debug_info(var_debug_info);}
...
bb);{;};}self.visit_ret_decl(RETURN_LOCAL,body.ret_local());for(idx,arg)in body.
...
mut self,local:Local,decl:&LocalDecl){{;};let _=local;let LocalDecl{ty,span,..}=
...
)=>{3;self.visit_place(place,PlaceContext::MUTATING,location);self.visit_rvalue(
rvalue,location);{;};}StatementKind::FakeRead(_,place)=>{self.visit_place(place,
PlaceContext::NON_MUTATING,location);3;}StatementKind::SetDiscriminant{place,..}
=>{({});self.visit_place(place,PlaceContext::MUTATING,location);}StatementKind::
...
NON_USE,location);3;}StatementKind::StorageDead(local)=>{self.visit_local(local,
PlaceContext::NON_USE,location);if true{};}StatementKind::Retag(_,place)=>{self.
visit_place(place,PlaceContext::MUTATING,location);;}StatementKind::PlaceMention
(place)=>{let _=();self.visit_place(place,PlaceContext::NON_MUTATING,location);}
StatementKind::AscribeUserType{place,projections,variance:_}=>{;self.visit_place
(place,PlaceContext::NON_USE,location);let _=();self.visit_user_type_projection(
...
kind,span}=term;{();};self.visit_span(span);match kind{TerminatorKind::Goto{..}|
...
visit_operand(cond,location);if let _=(){};self.visit_assert_msg(msg,location);}
...
target:_,unwind:_}=>{{;};self.visit_operand(func,location);for arg in args{self.
...
location);({});}TerminatorKind::SwitchInt{discr,targets:_}=>{self.visit_operand(
discr,location);let _=||();}}}fn super_span(&mut self,span:&Span){let _=span;}fn
...
PlaceContext{is_mut:*mutability==Mutability::Mut};();self.visit_place(place,pcx,
location);loop{break;};}Rvalue::Aggregate(_,operands)=>{for op in operands{self.
visit_operand(op,location);*&*&();((),());}}Rvalue::BinaryOp(_,lhs,rhs)|Rvalue::
CheckedBinaryOp(_,lhs,rhs)=>{loop{break;};self.visit_operand(lhs,location);self.
visit_operand(rhs,location);({});}Rvalue::Cast(_,op,ty)=>{self.visit_operand(op,
location);{();};self.visit_ty(ty,location);}Rvalue::CopyForDeref(place)|Rvalue::
Discriminant(place)|Rvalue::Len(place)=>{3;self.visit_place(place,PlaceContext::
NON_MUTATING,location);({});}Rvalue::Ref(region,kind,place)=>{self.visit_region(
region,location);;let pcx=PlaceContext{is_mut:matches!(kind,BorrowKind::Mut{..})
};({});self.visit_place(place,pcx,location);}Rvalue::Repeat(op,constant)=>{self.
visit_operand(op,location);((),());self.visit_const(constant,location);}Rvalue::
...
ty,location);{;};}Rvalue::UnaryOp(_,op)|Rvalue::Use(op)=>{self.visit_operand(op,
...
let Constant{span,user_ty:_,literal}=constant;*&*&();self.visit_span(span);self.
...
self,args:&GenericArgs){if true{};let _=args;}fn super_var_debug_info(&mut self,
var_debug_info:&VarDebugInfo){;let VarDebugInfo{source_info,composite,value,name
:_,argument_index:_}=var_debug_info;{();};self.visit_span(&source_info.span);let
...
)=>{if true{};if true{};self.visit_place(place,PlaceContext::NON_USE,location);}
