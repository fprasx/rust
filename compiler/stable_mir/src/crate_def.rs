...
def_id(&self)->DefId;fn name(&self)->Symbol{3;let def_id=self.def_id();with(|cx|
cx.def_name(def_id,false))}fn trimmed_name(&self)->Symbol{{();};let def_id=self.
def_id();3;with(|cx|cx.def_name(def_id,true))}fn krate(&self)->Crate{let def_id=
self.def_id();3;with(|cx|cx.krate(def_id))}fn span(&self)->Span{let def_id=self.
