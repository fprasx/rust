...
ItemLocalId);#[inline]fn to_stable_hash_key(&self,hcx:&HirCtx)->(DefPathHash,//;
ItemLocalId){({});let BodyId{hir_id}=*self;hir_id.to_stable_hash_key(hcx)}}impl<
...
opt_hash_including_bodies,nodes:_,bodies:_}=*self;{;};opt_hash_including_bodies.
...
hasher:&mut StableHasher){{();};let AttributeMap{opt_hash,map:_}=*self;opt_hash.
...
mut StableHasher){3;let Crate{owners:_,opt_hir_hash}=self;opt_hir_hash.unwrap().
