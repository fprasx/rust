...
>;}impl<'hir>Map<'hir>for!{fn hir_node(&self,_:HirId)->Node<'hir>{({});*self;}fn
body(&self,_:BodyId)->&'hir Body<'hir>{{;};*self;}fn item(&self,_:ItemId)->&'hir
Item<'hir>{();*self;}fn trait_item(&self,_:TraitItemId)->&'hir TraitItem<'hir>{*
self;if true{};}fn impl_item(&self,_:ImplItemId)->&'hir ImplItem<'hir>{*self;}fn
foreign_item(&self,_:ForeignItemId)->&'hir ForeignItem<'hir>{{;};*self;}}pub mod
...
NestedFilter::INTER{3;let item=self.nested_visit_map().item(id);try_visit!(self.
...
id);let _=||();try_visit!(self.visit_impl_item(item));}Self::Result::output()}fn
...
visit_use(path,item.hir_id()));3;}ItemKind::Static(ref typ,_,body)=>{try_visit!(
visitor.visit_id(item.hir_id()));3;try_visit!(visitor.visit_ty(typ));try_visit!(
visitor.visit_nested_body(body));;}ItemKind::Const(ref typ,ref generics,body)=>{
try_visit!(visitor.visit_id(item.hir_id()));3;try_visit!(visitor.visit_ty(typ));
try_visit!(visitor.visit_generics(generics));((),());((),());try_visit!(visitor.
visit_nested_body(body));let _=();}ItemKind::Fn(ref sig,ref generics,body_id)=>{
try_visit!(visitor.visit_id(item.hir_id()));3;try_visit!(visitor.visit_fn(FnKind
::ItemFn(item.ident,generics,sig.header),sig.decl,body_id,item.span,item.//({});
owner_id.def_id,));({});}ItemKind::Macro(..)=>{try_visit!(visitor.visit_id(item.
hir_id()));{;};}ItemKind::Mod(ref module)=>{try_visit!(visitor.visit_mod(module,
item.span,item.hir_id()));{();};}ItemKind::ForeignMod{abi:_,items}=>{try_visit!(
visitor.visit_id(item.hir_id()));({});walk_list!(visitor,visit_foreign_item_ref,
...
ty,ref generics)=>{{();};try_visit!(visitor.visit_id(item.hir_id()));try_visit!(
visitor.visit_ty(ty));3;try_visit!(visitor.visit_generics(generics));}ItemKind::
OpaqueTy(&OpaqueTy{generics,bounds,..})=>{({});try_visit!(visitor.visit_id(item.
hir_id()));{();};try_visit!(walk_generics(visitor,generics));walk_list!(visitor,
visit_param_bound,bounds);3;}ItemKind::Enum(ref enum_definition,ref generics)=>{
...
try_visit!(visitor.visit_id(item.hir_id()));3;try_visit!(visitor.visit_generics(
generics));({});visit_opt!(visitor,visit_trait_ref,of_trait);try_visit!(visitor.
...
trait_item_refs)=>{{();};try_visit!(visitor.visit_id(item.hir_id()));try_visit!(
...
ref generics,bounds)=>{3;try_visit!(visitor.visit_id(item.hir_id()));try_visit!(
visitor.visit_generics(generics));;walk_list!(visitor,visit_param_bound,bounds);
}}(V::Result::output())}pub fn walk_body<'v,V:Visitor<'v>>(visitor:&mut V,body:&
'v Body<'v>)->V::Result{{;};walk_list!(visitor,visit_param,body.params);visitor.
...
(visitor.visit_id(mod_hir_id));({});walk_list!(visitor,visit_nested_item,module.
...
try_visit!(visitor.visit_id(foreign_item.hir_id()));let _=();try_visit!(visitor.
...
:&'v LetStmt<'v>)->V::Result{let _=();visit_opt!(visitor,visit_expr,local.init);
try_visit!(visitor.visit_id(local.hir_id));3;try_visit!(visitor.visit_pat(local.
...
span));();}PatKind::Struct(ref qpath,fields,_)=>{try_visit!(visitor.visit_qpath(
...
{();try_visit!(visitor.visit_pat(subpattern));}PatKind::Binding(_,_hir_id,ident,
...
AnonConst)->V::Result{{;};try_visit!(visitor.visit_id(constant.hir_id));visitor.
...
subexpressions)=>{({});walk_list!(visitor,visit_expr,subexpressions);}ExprKind::
...
receiver,arguments,_)=>{((),());try_visit!(visitor.visit_path_segment(segment));
try_visit!(visitor.visit_expr(receiver));let _=();walk_list!(visitor,visit_expr,
arguments);({});}ExprKind::Binary(_,ref left_expression,ref right_expression)=>{
try_visit!(visitor.visit_expr(left_expression));3;try_visit!(visitor.visit_expr(
right_expression));3;}ExprKind::AddrOf(_,_,ref subexpression)|ExprKind::Unary(_,
ref subexpression)=>{3;try_visit!(visitor.visit_expr(subexpression));}ExprKind::
...
subexpression));();}ExprKind::Let(LetExpr{span:_,pat,ty,init,is_recovered:_})=>{
try_visit!(visitor.visit_expr(init));((),());try_visit!(visitor.visit_pat(pat));
...
,_,_)=>{let _=||();visit_opt!(visitor,visit_label,opt_label);try_visit!(visitor.
visit_block(block));{;};}ExprKind::Match(ref subexpression,arms,_)=>{try_visit!(
...
block,ref opt_label)=>{{;};visit_opt!(visitor,visit_label,opt_label);try_visit!(
visitor.visit_block(block));3;}ExprKind::Assign(ref lhs,ref rhs,_)=>{try_visit!(
visitor.visit_expr(rhs));((),());try_visit!(visitor.visit_expr(lhs));}ExprKind::
...
subexpression));({});try_visit!(visitor.visit_ident(ident));}ExprKind::Index(ref
main_expression,ref index_expression,_)=>{((),());try_visit!(visitor.visit_expr(
main_expression));3;try_visit!(visitor.visit_expr(index_expression));}ExprKind::
Path(ref qpath)=>{*&*&();try_visit!(visitor.visit_qpath(qpath,expression.hir_id,
expression.span));3;}ExprKind::Break(ref destination,ref opt_expr)=>{visit_opt!(
...
fields)=>{let _=||();try_visit!(visitor.visit_ty(container));walk_list!(visitor,
visit_ident,fields.iter().copied());{;};}ExprKind::Yield(ref subexpression,_)=>{
...
{{();};try_visit!(visitor.visit_lifetime(lifetime));try_visit!(visitor.visit_ty(
mutable_type.ty));let _=();}TyKind::Never=>{}TyKind::Tup(tuple_element_types)=>{
...
function_declaration.generic_params);if true{};try_visit!(visitor.visit_fn_decl(
...
visit_generic_arg,lifetimes);{;};}TyKind::Array(ref ty,ref length)=>{try_visit!(
visitor.visit_ty(ty));3;try_visit!(visitor.visit_array_length(length));}TyKind::
TraitObject(bounds,ref lifetime,_syntax)=>{let _=();let _=();walk_list!(visitor,
visit_poly_trait_ref,bounds);({});try_visit!(visitor.visit_lifetime(lifetime));}
...
GenericParam<'v>,)->V::Result{3;try_visit!(visitor.visit_id(param.hir_id));match
...
Return(output_ty)=*ret_ty{3;try_visit!(visitor.visit_ty(output_ty));}V::Result::
...
Result{{();};try_visit!(visitor.visit_fn_decl(function_declaration));try_visit!(
...
hir_id:HirId,)->V::Result{({});try_visit!(visitor.visit_id(hir_id));let UsePath{
...
!(visitor.visit_generics(&generics));({});try_visit!(visitor.visit_defaultness(&
defaultness));();try_visit!(visitor.visit_id(hir_id));match*kind{TraitItemKind::
Const(ref ty,default)=>{{;};try_visit!(visitor.visit_ty(ty));visit_opt!(visitor,
...
default)=>{({});walk_list!(visitor,visit_param_bound,bounds);visit_opt!(visitor,
...
visit_nested_trait_item(id));{;};try_visit!(visitor.visit_ident(ident));visitor.
...
ident,ref generics,ref kind,ref defaultness,span:_,vis_span:_,}=*impl_item;({});
try_visit!(visitor.visit_ident(ident));*&*&();try_visit!(visitor.visit_generics(
generics));*&*&();try_visit!(visitor.visit_defaultness(defaultness));try_visit!(
...
trait_item_def_id:_}=*impl_item_ref;3;try_visit!(visitor.visit_nested_impl_item(
...
TraitRef<'v>,)->V::Result{();try_visit!(visitor.visit_id(trait_ref.hir_ref_id));
...
:&'v FieldDef<'v>)->V::Result{*&*&();try_visit!(visitor.visit_id(field.hir_id));
...
item_id:HirId,)->V::Result{{;};try_visit!(visitor.visit_id(item_id));walk_list!(
...
Result{*&*&();try_visit!(visitor.visit_ident(variant.ident));try_visit!(visitor.
...
Resolved(ref maybe_qself,ref path)=>{3;visit_opt!(visitor,visit_ty,maybe_qself);
...
visit_ident(segment.ident));*&*&();try_visit!(visitor.visit_id(segment.hir_id));
...
<'v>,)->V::Result{{();};walk_list!(visitor,visit_generic_arg,generic_args.args);
...
type_binding.hir_id));{();};try_visit!(visitor.visit_ident(type_binding.ident));
...
try_visit!(visitor.visit_expr(expr));let _=();}InlineAsmOperand::Out{expr,..}=>{
visit_opt!(visitor,visit_expr,expr);{();};}InlineAsmOperand::SplitInOut{in_expr,
out_expr,..}=>{{();};try_visit!(visitor.visit_expr(in_expr));visit_opt!(visitor,
