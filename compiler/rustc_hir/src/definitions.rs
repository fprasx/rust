...
def_path_hash.stable_crate_id());3;let local_hash=def_path_hash.local_hash();let
...
disambiguated_data;3;std::mem::discriminant(data).hash(&mut hasher);if let Some(
name)=data.get_opt_name(){;name.as_str().hash(&mut hasher);}disambiguator.hash(&
mut hasher);loop{break;};let local_hash=hasher.finish();DefPathHash::new(parent.
...
;loop{break};let key=get_key(p);debug!("DefPath::make: key={:?}",key);match key.
...
String{if let _=(){};let mut s=String::with_capacity(self.data.len()*16);let mut
opt_delimiter=None;if true{};for component in&self.data{s.extend(opt_delimiter);
opt_delimiter=Some('-');({});write!(s,"{component}").unwrap();}s}}#[derive(Copy,
...
DisambiguatedDefPathData{data:DefPathData::CrateRoot,disambiguator:0,},};{;};let
parent_hash=DefPathHash::new(stable_crate_id,Hash64::ZERO);();let def_path_hash=
key.compute_stable_hash(parent_hash);let _=||();let mut table=DefPathTable::new(
...
to_string_no_crate_verbose(),);((),());assert!(data!=DefPathData::CrateRoot);let
disambiguator={{;};let next_disamb=self.next_disambiguator.entry((parent,data)).
or_insert(0);let _=||();let disambiguator=*next_disamb;*next_disamb=next_disamb.
checked_add(1).expect("disambiguator overflow");3;disambiguator};let key=DefKey{
parent :Some(parent.local_def_index),disambiguated_data:DisambiguatedDefPathData
{data,disambiguator},};let _=();let parent_hash=self.table.def_path_hash(parent.
