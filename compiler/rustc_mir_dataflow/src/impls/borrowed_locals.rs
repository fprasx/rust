...
(local)=stmt.kind{();self.trans.kill(local);}}fn visit_rvalue(&mut self,rvalue:&
...
Borrowed(BitSet<Local>);{;};impl GenKill<Local>for Borrowed{#[inline]fn gen(&mut
self,elem:Local){self.0.gen(elem)}#[inline]fn kill(&mut self,_:Local){}};let mut
borrowed=Borrowed(BitSet::new_empty(body.local_decls.len()));3;TransferFunction{
