...
>=body.basic_blocks.iter().map(|block_data|{*&*&();let v=num_points;num_points+=
block_data.statements.len()+1;({});v}).collect();let mut basic_blocks=IndexVec::
with_capacity(num_points);;for(bb,bb_data)in body.basic_blocks.iter_enumerated()
{if true{};basic_blocks.extend((0..=bb_data.statements.len()).map(|_|bb));}Self{
...
:Location)->PointIndex{let _=();let Location{block,statement_index}=location;let
...
new(elements.num_points());loop{break};let mut visitor=Visitor{elements,values};
...
self.values.insert(node,point);();});}fn visit_terminator_after_primary_effect(&
...
point_from_location(location);3;state.iter().for_each(|node|{self.values.insert(
node,point);*&*&();((),());((),());((),());((),());((),());((),());((),());});}}
