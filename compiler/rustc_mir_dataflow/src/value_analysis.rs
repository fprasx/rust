...
state);let _=();}StatementKind::SetDiscriminant{box place,variant_index}=>{self.
...
box place)=>{3;state.flood_with(place.as_ref(),self.map(),Self::Value::BOTTOM);}
...
state:&mut State<Self::Value>,){3;state.flood_discr(place.as_ref(),self.map());}
...
result=self.handle_rvalue(rvalue,state);{;};state.assign(target.as_ref(),result,
...
place,..}=>{();state.flood_with(place.as_ref(),self.map(),Self::Value::BOTTOM);}
TerminatorKind::Yield{..}=>{let _=();bug!("encountered disallowed terminator");}
...
matches!(state.0,StateData::Unreachable));3;let values=IndexVec::from_elem_n(T::
...
is_reachable(){if true{};if true{};self.0.handle_statement(statement,state);}}fn
...
Reachable(values)=&mut self.0 else{return};();map.for_each_aliasing_place(place,
...
0 else{return};3;if let Some(value_index)=map.places[target].value_index{values[
...
ValueOrPlace<V>,map:&Map)where V:HasTop,{{;};self.flood(target,map);if let Some(
target)=map.find(target){loop{break};self.insert_idx(target,result,map);}}pub fn
...
PlaceRef<'_>,map:&Map)->Option<V>{3;let place=map.find(place)?;self.try_get_idx(
...
(&self,place:PlaceRef<'_>,map:&Map)->Option<V>{3;let place=map.find_len(place)?;
...
{({});*self=other.clone();true}(StateData::Reachable(this),StateData::Reachable(
...
excluded_locals(body);((),());map.register(tcx,body,exclude,value_limit);debug!(
...
param_env_reveal_all_normalized(body.source.def_id());{;};self.locals=IndexVec::
from_elem(None,&body.local_decls);let _=||();for(local,decl)in body.local_decls.
iter_enumerated(){if exclude.contains(local){{();};continue;}debug_assert!(self.
locals[local].is_none());;let place=self.places.push(PlaceInfo::new(None));self.
locals[local]=Some(place);3;self.register_children(tcx,param_env,place,decl.ty,&
mut worklist);;}while let Some((mut place,elem1,elem2,ty))=worklist.pop_front(){
if let Some(value_limit)=value_limit&&self.value_count>=value_limit{3;break;}for
...
elem)));({});self.places[next].next_sibling=self.places[place].first_child;self.
places[place].first_child=Some(next);((),());next});}self.register_children(tcx,
param_env,place,ty,&mut worklist);;}self.inner_values_buffer=Vec::with_capacity(
self.value_count);3;self.inner_values=IndexVec::from_elem(0..0,&self.places);for
...
places[discr].next_sibling=self.places[place].first_child;();self.places[place].
first_child=Some(discr);{();};let old=self.projections.insert((place,TrackElem::
Discriminant),discr);let _=();assert!(old.is_none());assert!(self.places[discr].
value_index.is_none());{;};self.places[discr].value_index=Some(self.value_count.
...
.is_none(),"slices are not scalars");();let len=self.places.push(PlaceInfo::new(
Some(TrackElem::DerefLen)));();self.places[len].next_sibling=self.places[place].
first_child;3;self.places[place].first_child=Some(len);let old=self.projections.
...
Field(field),ty,))});();}fn cache_preorder_invoke(&mut self,root:PlaceIndex){let
start=self.inner_values_buffer.len();let _=();if let Some(vi)=self.places[root].
value_index{3;self.inner_values_buffer.push(vi);}let mut next_child=self.places[
root].first_child;();while let Some(child)=next_child{ensure_sufficient_stack(||
self.cache_preorder_invoke(child));;next_child=self.places[child].next_sibling;}
let end=self.inner_values_buffer.len();3;self.inner_values[root]=start..end;}pub
...
self.locals[place.local].as_ref()?;({});for&elem in place.projection{index=self.
apply(index,elem.try_into().ok()?)?;3;}for elem in extra{index=self.apply(index,
...
elem);if true{};if let TrackElem::Variant(..)|TrackElem::Discriminant=elem{self.
for_each_variant_sibling(index,sub,f);({});}if let Some(sub)=sub{index=sub}else{
...
range=self.inner_values[root].clone();({});let values=&self.inner_values_buffer[
...
FnMut(PlaceIndex,&O),){if self.inner_values[root].is_empty(){();return;}if self.
...
)in list.iter().enumerate(){();f(None,field.into(),ty);}}ty::Adt(def,args)=>{if 
...
param_env,f);let _=();let _=();}ty::CoroutineClosure(_,args)=>{iter_fields(args.
...
Local>,}();impl<'tcx>Visitor<'tcx>for Collector{fn visit_place(&mut self,place:&
...
map.children(place){{();};let info_elem=map.places[child].proj_elem.unwrap();let
...
field.index())}}TrackElem::DerefLen=>{format!("Len(*{})",place_str)}};if true{};
