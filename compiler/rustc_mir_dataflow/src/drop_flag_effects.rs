...
first_child;({});while let Some(child_index)=next_child{let move_path_children=&
move_data.move_paths[child_index];3;if let Some(&elem)=move_path_children.place.
projection.last(){if cond(elem){if true{};return Some(child_index);}}next_child=
...
place=mir::Place::from(arg);3;let lookup_result=move_data.rev_lookup.find(place.
as_ref());({});on_lookup_result_bits(move_data,lookup_result,|mpi|{callback(mpi,
...
FnMut(MovePathIndex,DropFlagState),{*&*&();let move_data=&ctxt.move_data;debug!(
"drop_flag_effects_for_location({:?})",loc);();for mi in&move_data.loc_map[loc]{
...
"drop_flag_effects: assignment for location({:?})",loc);({});for_location_inits(
...
*ii];3;match init.kind{InitKind::Deep=>{let path=init.path;on_all_children_bits(
...
_,variant_idx)=*downcast else{3;unreachable!();};if variant_idx!=active_variant{
