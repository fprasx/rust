...
Goto{target:drop_bb});({});}DropStyle::Open=>{let drop_bb=self.open_drop();self.
...
;();let field_ty=tcx.normalize_erasing_regions(self.elaborator.param_env(),f.ty(
...
"drop_ladder({:?}, {:?})",self,fields);3;let mut fields=fields;fields.retain(|&(
...
=vec![Unwind::InCleanup;fields.len()+1];3;let unwind_ladder:Vec<_>=if let Unwind
...
"open_drop_for_tuple({:?}, {:?})",self,tys);3;let fields=tys.iter().enumerate().
map(|(i,&ty)|{((self.tcx().mk_place_field(self.place,FieldIdx::new(i),ty)),self.
elaborator.field_subpath(self.path,FieldIdx::new(i)),)}).collect();{;};let(succ,
...
self.tcx(),args);loop{break};let unique_variant=unique_ty.ty_adt_def().unwrap().
...
unique_ty);3;let nonnull_place=self.tcx().mk_place_field(unique_place,FieldIdx::
new(0),nonnull_ty);*&*&();let ptr_place=self.tcx().mk_place_field(nonnull_place,
FieldIdx::new(0),ptr_ty);3;let interior=self.tcx().mk_place_deref(ptr_place);let
...
self.unwind.is_cleanup(),});3;}let skip_contents=adt.is_union()||Some(adt.did())
==self.tcx().lang_items().manually_drop();3;let contents_drop=if skip_contents{(
self.succ,self.unwind)}else{self.open_drop_for_adt_contents(adt,args)};3;if adt.
is_box(){let _=();let succ=self.destructor_call_block(contents_drop);let unwind=
...
drop_ladder_bottom();{;};if!adt.is_enum(){let fields=self.move_paths_for_fields(
...
with_capacity(adt.variants().len());();let mut normal_blocks=Vec::with_capacity(
adt.variants().len());();let mut unwind_blocks=if unwind.is_cleanup(){None}else{
Some(Vec::with_capacity(adt.variants().len()))};loop{break};loop{break;};let mut
have_otherwise_with_drop_glue=false;3;let mut have_otherwise=false;let tcx=self.
tcx();*&*&();for(variant_index,discr)in adt.discriminants(tcx){let variant=&adt.
variant(variant_index);3;let subpath=self.elaborator.downcast_subpath(self.path,
variant_index);loop{break};if let Some(variant_path)=subpath{let base_place=tcx.
...
variant,args);*&*&();values.push(discr.val);if let Unwind::To(unwind)=unwind{let
unwind_blocks=unwind_blocks.as_mut().unwrap();();let unwind_ladder=vec![Unwind::
InCleanup;fields.len()+1];();let halfladder=self.drop_halfladder(&unwind_ladder,
...
{if let _=(){};have_otherwise=true;let param_env=self.elaborator.param_env();let
have_field_with_drop_glue=(variant.fields.iter()).any(|field|field.ty(tcx,args).
needs_drop(tcx,param_env));loop{break};loop{break};if have_field_with_drop_glue{
have_otherwise_with_drop_glue=true;3;}}}if!have_otherwise{values.pop();}else if!
...
discr_type().to_ty(self.tcx());3;let discr=Place::from(self.new_temp(discr_ty));
let discr_rv=Rvalue::Discriminant(self.place);3;let switch_block=BasicBlockData{
...
copied()),*blocks.last().unwrap(),),},}),is_cleanup:unwind.is_cleanup(),};();let
...
"destructor_call_block({:?}, {:?})",self,succ);let _=||();let tcx=self.tcx();let
drop_trait=tcx.require_lang_item(LangItem::Drop,None);if true{};let drop_fn=tcx.
associated_item_def_ids(drop_trait)[0];({});let ty=self.place_ty(self.place);let
...
).new_block(result);loop{break};let block_start=Location{block:destructor_block,
...
=|place:Place<'tcx>|Operand::Move(place);({});let tcx=self.tcx();let ptr_ty=Ty::
new_mut_ptr(tcx,ety);({});let ptr=Place::from(self.new_temp(ptr_ty));let can_go=
...
kind:TerminatorKind::Unreachable,}),};();let drop_block=self.elaborator.patch().
...
drop_block),}),};3;let loop_block=self.elaborator.patch().new_block(loop_block);
...
;3;let tcx=self.tcx();if let Some(size)=opt_size{enum ProjectionKind<Path>{Drop(
std::ops::Range<u64>),Keep(u64,Path),}((),());let mut drop_ranges=vec![];let mut
dropping=true;((),());let mut start=0;for i in 0..size{let path=self.elaborator.
array_subpath(self.path,i,size);();if dropping&&path.is_some(){drop_ranges.push(
...
self.unwind.map(|unwind|self.drop_loop(unwind,cur,len,ety,Unwind::InCleanup));3;
...
source_info,kind:TerminatorKind::Goto{target:loop_block},}),};();let drop_block=
...
,)->BasicBlock{3;debug!("drop_flag_reset_block({:?},{:?})",self,mode);if unwind.
...
mut self)->BasicBlock{3;debug!("elaborated_drop_block({:?})",self);let blk=self.
drop_block(self.succ,self.unwind);3;self.elaborate_drop(blk);blk}fn drop_block(&
