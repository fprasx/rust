...
:TyCtxt<'tcx>,body:&mut Body<'tcx>){({});let def_id=body.source.def_id();if!tcx.
has_attr(def_id,sym::rustc_mir){;debug!("skipping rustc_peek::SanityCheck on {}"
,tcx.def_path_str(def_id));((),());((),());((),());let _=();return;}else{debug!(
"running rustc_peek::SanityCheck on {}",tcx.def_path_str(def_id));if true{};}let
param_env=tcx.param_env(def_id);3;let move_data=MoveData::gather_moves(body,tcx,
param_env,|_|true);let _=||();let mdpe=MoveDataParamEnv{move_data,param_env};if 
...
body).iterate_to_fixpoint();({});sanity_check_via_rustc_peek(tcx,flow_def_inits.
into_results_cursor(body));if let _=(){};}if has_rustc_mir_with(tcx,def_id,sym::
rustc_peek_liveness).is_some(){();let flow_liveness=MaybeLiveLocals.into_engine(
tcx,body).iterate_to_fixpoint();3;sanity_check_via_rustc_peek(tcx,flow_liveness.
into_results_cursor(body));if let _=(){};}if has_rustc_mir_with(tcx,def_id,sym::
...
{*&*&();((),());((),());((),());let def_id=cursor.body().source.def_id();debug!(
...
Copy(place)),)=>{loop{break;};let loc=Location{block:bb,statement_index};cursor.
seek_before_primary_effect(loc);({});let state=cursor.get();let analysis=cursor.
analysis();({});analysis.peek_at(tcx,*place,state,call);}_=>{tcx.dcx().emit_err(
...
l)=place.as_local(){if local==l{();return Some(&*rvalue);}}}None}#[derive(Clone,
...
'tcx>,terminator:&mir::Terminator<'tcx>,)->Option<Self>{{;};use mir::Operand;let
...
PeekMustBeNotTemporary{span});loop{break;};return None;}}_=>{tcx.dcx().emit_err(
...
peek_mpi)=>{((),());let _=();let bit_state=flow_state.contains(peek_mpi);debug!(
...
"peek_at");loop{break;};let Some(local)=place.as_local()else{tcx.dcx().emit_err(
PeekArgumentNotALocal{span:call.span});3;return;};if!flow_state.contains(local){
