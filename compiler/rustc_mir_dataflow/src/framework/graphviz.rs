...
:&Self::Node)->dot::LabelText<'_>{((),());let mut label=Vec::new();self.results.
replace_with(|results|{;let mut fmt=BlockFormatter{results:results.take().unwrap
().into_results_cursor(self.body),style:self.style,bg:Background::Light,};3;fmt.
...
" sides=\"rb\"",);if true{};write!(w,r#"<table{table_fmt}>"#)?;match self.style{
...
"AFTER"])?}}3;self.bg=Background::Light;self.results.seek_to_block_start(block);
let block_start_state=self.results.get().clone();;self.write_row_with_full_state
(w,"","(on start)")?;let _=();self.write_statements_and_terminator(w,block)?;let
...
TerminatorKind::Call{target:Some(_),..}=>"(on unwind)",_=>"(on end)",};{;};self.
write_row_with_full_state(w,"",after_terminator_name)?;3;}match terminator.kind{
mir::TerminatorKind::Call{destination,..}=>{((),());((),());self.write_row(w,"",
"(on successful return)",|this,w,fmt|{();let state_on_unwind=this.results.get().
clone();loop{break;};this.results.apply_custom_effect(|analysis,state|{analysis.
apply_call_return_effect(state,block,CallReturnPlaces::Call(destination),);3;});
...
fmt|{*&*&();let state_on_coroutine_drop=this.results.get().clone();this.results.
apply_custom_effect(|analysis,state|{();analysis.apply_call_return_effect(state,
resume,CallReturnPlaces::Yield(resume_arg),);let _=||();loop{break};});write!(w,
r#"<td balign="left" colspan="{colspan}" {fmt} align="left">{diff}</td>"#,//{;};
colspan=this.style.num_state_columns(),fmt=fmt,diff=diff_pretty(this.results.//;
...
r#"<td balign="left" colspan="{colspan}" {fmt} align="left">{diff}</td>"#,//{;};
colspan=this.style.num_state_columns(),fmt=fmt,diff=diff_pretty(this.results.//;
get(),&state_on_unwind,this.results.analysis()),)})?;if true{};}_=>{}};write!(w,
...
state_column_names{({});write!(w,"<td {fmt}>{name}</td>")?;}write!(w,"</tr>")}fn
...
next_in_dataflow_order(&mut diffs_after);3;let before=diffs_before.as_mut().map(
...
)?;let _=||();}write!(w,r#"<td {fmt} align="left">{after}</td>"#)})?;}let after=
next_in_dataflow_order(&mut diffs_after);3;let before=diffs_before.as_mut().map(
...
::Result<()>{self.write_row(w,i,mir,|this,w,fmt|{3;let state=this.results.get();
...
OutputStyle::BeforeAndAfter).then_some(vec![]),};3;results.visit_with(body,std::
...
Direction::IS_BACKWARD{let _=();if true{};self.prev_state.clone_from(state);}}fn
...
DebugDiffWithAdapter{new,old,ctxt});let _=();let raw_diff=raw_diff.replace('\n',
r#"<br align="left"/>"#);((),());let mut inside_font_tag=false;let html_diff=re.
...
html_diff else{let _=();return raw_diff;};if inside_font_tag{html_diff.push_str(
