...
;let _=||();let mut block=|n,kind|{let nop=mir::Statement{source_info,kind:mir::
...
source_info,kind}),is_cleanup:false,})};3;let dummy_place=mir::Place{local:mir::
...
call_source:mir::CallSource::Misc,fn_span:DUMMY_SP,},);loop{break};block(3,mir::
TerminatorKind::Return);{();};block(0,mir::TerminatorKind::Return);block(4,mir::
...
target.block();*&*&();let mut ret=self.bottom_value(self.body);ret.insert(Self::
BASIC_BLOCK_OFFSET+block.index());if true{};let target=match target{SeekTarget::
...
{Effect::Before.at_index(self.body[block].statements.len())};();loop{ret.insert(
self.effect(pos));if true{};if pos==target{return ret;}if D::IS_FORWARD{pos=pos.
...
at_index(location.statement_index));if let _=(){};assert!(state.insert(idx));}fn
...
idx=self.effect(Effect::Primary.at_index(location.statement_index));{;};assert!(
...
seek_to_target=|targ|{();use SeekTarget::*;match targ{BlockEntry(block)=>cursor.
...
seek_to_target(from);if let _=(){};for to in every_target(){dbg!(from);dbg!(to);
