...
mut move_paths=IndexVec::new();let _=();let mut path_map=IndexVec::new();let mut
...
place});if true{};if let Some(parent)=parent{let next_sibling=mem::replace(&mut 
move_paths[parent].first_child,Some(move_path));if true{};move_paths[move_path].
next_sibling=next_sibling;let _=();}let path_map_ent=path_map.push(smallvec![]);
assert_eq!(path_map_ent,move_path);{;};let init_path_map_ent=init_path_map.push(
smallvec![]);loop{break;};assert_eq!(init_path_map_ent,move_path);move_path}enum
...
Place<'tcx>)->MovePathResult{loop{break};let data=&mut self.builder.data;debug!(
...
place_ty.references_error(){let _=||();return MovePathResult::Error;}match elem{
ProjectionElem::Deref=>match place_ty.kind(){ty::Ref(..)|ty::RawPtr(..)=>{{();};
return MovePathResult::Error;loop{break};}ty::Adt(adt,_)=>{if!adt.is_box(){bug!(
...
MovePathResult::Error;3;}if adt.is_union(){union_path.get_or_insert(base);}}ty::
...
create_move_path(&mut self,place:Place<'tcx>){3;let _=self.move_path_for(place);
...
Location{block:bb,statement_index:block.statements.len()};if let _=(){};builder.
...
"gather_args: adding init {:?} of {:?} for argument {:?}",init,path,arg);3;self.
...
"gather_terminator({:?}, {:?})",loc,term);let _=();(Gatherer{builder:self,loc}).
...
local].is_deref_temp());*&*&();let rev_lookup=&mut self.builder.data.rev_lookup;
rev_lookup.un_derefer.insert(local,reffed.as_ref());3;let base_local=rev_lookup.
un_derefer.deref_chain(local).first().unwrap().local;3;rev_lookup.locals[local]=
...
place));if true{};self.gather_init(place.as_ref(),InitKind::Shallow);}else{self.
gather_init(place.as_ref(),InitKind::Deep);if true{};}self.gather_rvalue(rval);}
StatementKind::FakeRead(box(_,place))=>{let _=();self.create_move_path(*place);}
...
lhs);3;self.gather_operand(rhs);}Rvalue::Aggregate(ref _kind,ref operands)=>{for
operand in operands{();self.gather_operand(operand);}}Rvalue::CopyForDeref(..)=>
...
::SwitchInt{ref discr,..}=>{3;self.gather_operand(discr);}TerminatorKind::Yield{
ref value,resume_arg:place,..}=>{*&*&();((),());self.gather_operand(value);self.
create_move_path(place);*&*&();self.gather_init(place.as_ref(),InitKind::Deep);}
...
arg.node);({});}if let Some(_bb)=target{self.create_move_path(destination);self.
...
=>{let _=();self.gather_operand(in_value);if let Some(out_place)=out_place{self.
...
:place.local,projection:self.builder.tcx.mk_place_elems(base)};();let base_path=
match (((((self.move_path_for(base_place)))))){MovePathResult::Path(path)=>path,
MovePathResult::Union(path)=>{loop{break;};self.record_move(place,path);return;}
MovePathResult::Error=>{3;return;}};let base_ty=base_place.ty(self.builder.body,
...
elem=ProjectionElem::ConstantIndex{offset,min_length:len,from_end:false};{;};let
path =self.add_move_path(base_path,elem,|tcx|tcx.mk_place_elem(base_place,elem))
;let _=||();self.record_move(place,path);}}else{match self.move_path_for(place){
...
source:self.loc});();debug!("gather_move({:?}, {:?}): adding move {:?} of {:?}",
self.loc,place,move_out,path);3;self.builder.data.path_map[path].push(move_out);
...
"gather_init({:?}, {:?}): adding init {:?} of {:?}",self.loc,place,init,path);3;
