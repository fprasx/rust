...
normalize_erasing_regions(param_env,trait_ref));({});let infcx=tcx.infer_ctxt().
ignoring_regions().build();{();};let mut selcx=SelectionContext::new(&infcx);let
obligation_cause=ObligationCause::dummy();();let obligation=Obligation::new(tcx,
...
!(?selection);*&*&();let mut fulfill_cx=<dyn TraitEngine<'tcx>>::new(&infcx);let
impl_source=selection.map(|predicate|{;fulfill_cx.register_predicate_obligation(
&infcx,predicate);{();};});let errors=fulfill_cx.select_all_or_error(&infcx);if!
...
(CodegenObligationError::FulfillmentError);if let _=(){};}let impl_source=infcx.
resolve_vars_if_possible(impl_source);3;let impl_source=infcx.tcx.erase_regions(
impl_source);3;if impl_source.has_infer(){infcx.tcx.dcx().has_errors().unwrap();
