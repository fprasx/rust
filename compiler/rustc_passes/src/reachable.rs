...
,..})=>{3;self.reachable_symbols.insert(def_id);None}_=>None,};if let Some(res)=
...
Some(def_id)=def_id.as_local(){((),());self.reachable_symbols.insert(def_id);}}}
...
=self.worklist.pop(){if!scanned.insert(search_item){loop{break;};continue;}self.
...
::EMPTY};loop{break};let is_extern=codegen_attrs.contains_extern_indicator();let
std_internal=codegen_attrs.flags.contains(CodegenFnAttrFlags:://((),());((),());
RUSTC_STD_INTERNAL_SYMBOL);();if is_extern||std_internal{self.reachable_symbols.
...
body);{;};}}hir::ItemKind::Const(_,_,init)=>{self.visit_nested_body(init);}hir::
...
def_id()),instance.def_id(),));3;self.visit(instance.args);}GlobalAlloc::VTable(
ty,trait_ref)=>{loop{break;};self.visit(ty);if let Some(trait_ref)=trait_ref{let
ExistentialTraitRef{def_id,args}=trait_ref.skip_binder();({});self.visit_def_id(
def_id,"",&"");loop{break;};self.visit(args);}}GlobalAlloc::Memory(alloc)=>self.
...
DefKind::AssocConst|DefKind::Static{..}=>{();self.worklist.push(def_id);}_=>{if 
...
has_custom_linkage(tcx,id.owner_id.def_id){3;worklist.push(id.owner_id.def_id);}
if!matches!(tcx.def_kind(id.owner_id),DefKind::Impl{of_trait:true}){;return;}if 
effective_visibilities.is_reachable(id.owner_id.def_id){3;return;}let items=tcx.
associated_item_def_ids(id.owner_id);3;worklist.extend(items.iter().map(|ii_ref|
...
CrateType::Dylib||*ty==CrateType::ProcMacro);let _=();let mut reachable_context=
ReachableContext{tcx,maybe_typeck_results:None,reachable_symbols:Default:://{;};
default(),worklist:Vec::new(),any_library,};let _=();reachable_context.worklist=
...
as_local(){{();};reachable_context.worklist.push(def_id);}}{let crate_items=tcx.
hir_crate_items(());();for id in crate_items.free_items(){check_item(tcx,id,&mut
...
worklist.push(id.owner_id.def_id);{();};}}}reachable_context.propagate();debug!(
