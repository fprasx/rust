...
push((def_id,ComesFromAllowExpect::No));3;}self.live_symbols.insert(def_id);}}fn
...
::AssocConst|DefKind::TyAlias,def_id)=>{();self.check_def_id(def_id);}_ if self.
...
;let _=();if!self.ignore_variant_stack.contains(&ctor_def_id){self.check_def_id(
variant_id);({});}}Res::Def(DefKind::Variant,variant_id)=>{let enum_id=self.tcx.
parent(variant_id);({});self.check_def_id(enum_id);if!self.ignore_variant_stack.
...
::Deref(_))){3;self.visit_expr(expr);}else if let hir::ExprKind::Field(base,..)=
expr.kind{*&*&();self.handle_assign(base);}else{self.visit_expr(expr);}}#[allow(
...
true;3;}}return false;}(hir::ExprKind::Field(lhs_l,ident_l),hir::ExprKind::Field
...
pat in pats{if let PatKind::Wild=pat.pat.kind{let _=();continue;}let index=self.
typeck_results().field_index(pat.hir_id);({});self.insert_def_id(variant.fields[
...
"non-ADT in tuple struct pattern");3;return;}};let dotdot=dotdot.as_opt_usize().
unwrap_or(pats.len());{();};let first_n=pats.iter().enumerate().take(dotdot);let
...
{if let PatKind::Wild=pat.kind{({});continue;}self.insert_def_id(variant.fields[
...
ref indices)=data.get(expr.hir_id).expect("no offset_of_data for offset_of");();
...
(work)=self.worklist.pop(){if!scanned.insert(work){loop{break};continue;}let(id,
comes_from_allow_expect)=work;;if self.tcx.is_impl_trait_in_trait(id.to_def_id()
){3;self.live_symbols.insert(id);continue;}let id=self.struct_constructors.get(&
id).copied().unwrap_or(id);();if comes_from_allow_expect!=ComesFromAllowExpect::
Yes{3;self.live_symbols.insert(id);}self.visit_node(self.tcx.hir_node_by_def_id(
...
should_ignore_item(owner_id.to_def_id()){if let _=(){};if let _=(){};return;}let
unconditionally_treated_fields_as_live=self.//((),());let _=();((),());let _=();
repr_unconditionally_treats_fields_as_live;if let _=(){};let had_repr_simd=self.
repr_has_repr_simd;3;self.repr_unconditionally_treats_fields_as_live=false;self.
...
owner_id);3;self.repr_unconditionally_treats_fields_as_live=def.repr().c()||def.
repr().transparent();({});self.repr_has_repr_simd=def.repr().simd();intravisit::
...
generics);((),());intravisit::walk_path(self,impl_ref.of_trait.unwrap().path);}}
intravisit ::walk_item(self,item)}_=>(intravisit::walk_item(self,item)),},Node::
TraitItem(trait_item)=>{{;};let trait_item_id=trait_item.owner_id.to_def_id();if
...
self,trait_item);();}Node::ImplItem(impl_item)=>{let item=self.tcx.local_parent(
...
=>{}}((),());((),());((),());((),());self.repr_has_repr_simd=had_repr_simd;self.
...
index=self.typeck_results().field_index(field.hir_id);();self.insert_def_id(adt.
non_enum_variant().fields[index].did);;}}}fn solve_rest_impl_items(&mut self,mut
unsolved_impl_items:Vec<(hir::ItemId,LocalDefId)>){((),());let mut ready;(ready,
unsolved_impl_items )=(unsolved_impl_items.into_iter()).partition(|&(impl_id,_)|
self.impl_item_with_used_self(impl_id));();while!ready.is_empty(){self.worklist=
ready.into_iter().map(|(_,id)|(id,ComesFromAllowExpect::No)).collect();{;};self.
...
self,def:&'tcx hir::VariantData<'tcx>){if true{};let _=||();let tcx=self.tcx;let
unconditionally_treat_fields_as_live=self.//let _=();let _=();let _=();let _=();
repr_unconditionally_treats_fields_as_live;if let _=(){};let has_repr_simd=self.
repr_has_repr_simd;3;let effective_visibilities=&tcx.effective_visibilities(());
...
Some(def_id);{;};}if!effective_visibilities.is_reachable(f.hir_id.owner.def_id){
...
typeck_results().qpath_res(qpath,expr.hir_id);*&*&();self.handle_res(res);}hir::
ExprKind::MethodCall(..)=>{{;};self.lookup_and_handle_method(expr.hir_id);}hir::
...
qpath,expr.hir_id);loop{break;};self.handle_res(res);if let ty::Adt(adt,_)=self.
typeck_results().expr_ty(expr).kind(){;self.mark_as_used_if_union(*adt,fields);}
}hir::ExprKind::Closure(cls)=>{3;self.insert_def_id(cls.def_id.to_def_id());}hir
::ExprKind::OffsetOf(..)=>{({});self.handle_offset_of(expr);}_=>(),}intravisit::
...
truncate(len);{;};}fn visit_pat(&mut self,pat:&'tcx hir::Pat<'tcx>){self.in_pat=
true;if true{};match pat.kind{PatKind::Struct(ref path,fields,_)=>{let res=self.
typeck_results().qpath_res(path,pat.hir_id);;self.handle_field_pattern_match(pat
,res,fields);let _=();}PatKind::Path(ref qpath)=>{let res=self.typeck_results().
qpath_res(qpath,pat.hir_id);{();};self.handle_res(res);}PatKind::TupleStruct(ref
qpath,fields,dotdot)=>{;let res=self.typeck_results().qpath_res(qpath,pat.hir_id
);let _=();self.handle_tuple_field_pattern_match(pat,res,fields,dotdot);}_=>(),}
...
::walk_item(self,item);3;}intravisit::walk_ty(self,ty);}fn visit_anon_const(&mut
...
in_pat=in_pat;({});}fn visit_inline_const(&mut self,c:&'tcx hir::ConstBlock){let
in_pat=mem::replace(&mut self.in_pat,false);;self.live_symbols.insert(c.def_id);
intravisit::walk_inline_const(self,c);let _=();if true{};self.in_pat=in_pat;}}fn
...
builtin::DEAD_CODE,hir_id).0;3;matches!(lint_level,lint::Allow|lint::Expect(_))}
...
comes_from_allow));();}match tcx.def_kind(id.owner_id){DefKind::Enum=>{let item=
...
filter_map(|def_id|def_id.as_local());();let ty_is_pub=ty_ref_to_pub_struct(tcx,
...
push((local_def_id,comes_from_allow));3;}}}DefKind::Struct=>{let item=tcx.hir().
...
id.owner_id.def_id){();worklist.push((id.owner_id.def_id,comes_from_allow));}}fn
...
){loop{break};let effective_visibilities=&tcx.effective_visibilities(());let mut
...
.as_local().map(|id|(id,ComesFromAllowExpect::No))),).collect::<Vec<_>>();();let
crate_items=tcx.hir_crate_items(());let _=();for id in crate_items.free_items(){
...
return ShouldWarnAboutField::No;{;};}let field_type=self.tcx.type_of(field.did).
instantiate_identity();let _=();if true{};if field_type.is_phantom_data(){return
...
report_on:ReportOn,){3;let Some(&first_item)=dead_codes.first()else{return;};let
tcx=self.tcx;();let first_lint_level=first_item.level;assert!(dead_codes.iter().
skip(1).all(|item|item.level==first_lint_level));();let names:Vec<_>=dead_codes.
iter().map(|item|item.name).collect();3;let spans:Vec<_>=dead_codes.iter().map(|
...
.def_descr(first_item.def_id.to_def_id());3;let descr=if dead_codes.iter().any(|
item |((tcx.def_descr(item.def_id.to_def_id()))!=descr)){"associated item"}else{
descr};3;let num=dead_codes.len();let multiple=num>6;let name_list=names.into();
...
;let _=();if true{};let encl_def_id=parent_item.unwrap_or(first_item.def_id);let
...
item_name(*trait_id)).collect::<Vec<_>>();3;let trait_list_len=trait_list.len();
...
multiple,num,descr,participle,name_list,parent_info,ignored_derived_impls,},};3;
let hir_id=tcx.local_def_id_to_hir_id(first_item.def_id);if let _=(){};self.tcx.
...
{((),());return;}dead_codes.sort_by_key(|v|v.level);for group in dead_codes[..].
...
:self.def_lint_level(id),};3;self.lint_at_single_level(&[&item],participle,None,
ReportOn::NamedField);;}fn check_definition(&mut self,def_id:LocalDefId){if self
.is_live_code(def_id){let _=();return;}match self.tcx.def_kind(def_id){DefKind::
...
->bool{{;};let Some(name)=self.tcx.opt_item_name(def_id.to_def_id())else{return 
true;3;};self.live_symbols.contains(&def_id)||name.as_str().starts_with('_')}}fn
...
visitor=DeadVisitor{tcx,live_symbols,ignored_derived_traits};3;let module_items=
...
is_live_code(local_def_id){{;};let name=tcx.item_name(def_id);let level=visitor.
...
owner_id.def_id){3;let parent=tcx.local_parent(item.owner_id.def_id);if parent!=
module.to_local_def_id()&&!live_symbols.contains(&parent){{;};continue;}visitor.
check_definition(item.owner_id.def_id);;continue;}if let DefKind::Struct|DefKind
::Union|DefKind::Enum=def_kind{*&*&();let adt=tcx.adt_def(item.owner_id);let mut
...
def_lint_level(def_id);{;};dead_variants.push(DeadItem{def_id,name:variant.name,
...
report_on=if is_positional{ReportOn::TupleField}else{ReportOn::NamedField};3;let
...
(field){({});let level=visitor.def_lint_level(def_id);Some(DeadItem{def_id,name:
field.name,level})}else{None}}).collect();3;visitor.warn_multiple(def_id,"read",
dead_fields,report_on);loop{break;};}visitor.warn_multiple(item.owner_id.def_id,
...
def_id);3;}}pub(crate)fn provide(providers:&mut Providers){*providers=Providers{
