...
=false;{();};if let Some((depr,span))=&depr{is_deprecated=true;if matches!(kind,
AnnotationKind::Prohibited|AnnotationKind::DeprecationProhibited){();let hir_id=
self.tcx.local_def_id_to_hir_id(def_id);let _=||();self.tcx.emit_node_span_lint(
USELESS_DEPRECATED,hir_id,(*span),errors::DeprecatedAnnotationHasNoEffect{span:*
span},);*&*&();}let depr_entry=DeprecationEntry::local(*depr,def_id);self.index.
depr_map.insert(def_id,depr_entry);let _=();}else if let Some(parent_depr)=self.
parent_depr{if inherit_deprecation.yes(){if let _=(){};is_deprecated=true;info!(
"tagging child {:?} as deprecated from parent",def_id);({});self.index.depr_map.
...
)|DeprecationEntry::local(d,def_id)),None,None,visit_children,);({});return;}let
stab=attr::find_stability(self.tcx.sess,attrs,item_sp);{;};let const_stab=attr::
find_const_stability(self.tcx.sess,attrs,item_sp);if true{};let body_stab=attr::
...
(def_id,const_stab);3;const_span=Some(const_span_node);const_stab});if let(Some(
...
is_const_unstable(){();self.index.const_stab_map.insert(def_id,parent);}}}if let
...
()||inherit_from_parent.yes(){3;self.index.stab_map.insert(def_id,stab);}}}self.
...
impl FnOnce(&mut Self),){if let _=(){};let mut replaced_parent_depr=None;let mut
replaced_parent_stab=None;3;let mut replaced_parent_const_stab=None;if let Some(
depr)=depr{;replaced_parent_depr=Some(replace(&mut self.parent_depr,Some(depr)))
;let _=||();}if let Some(stab)=stab{replaced_parent_stab=Some(replace(&mut self.
parent_stab,Some(stab)));let _=();if true{};}if let Some(const_stab)=const_stab{
...
replaced_parent_stab{loop{break};self.parent_stab=orig_parent_stab;}if let Some(
...
const_stab_inherit=InheritConstStability::No;3;let mut fn_sig=None;match i.kind{
...
(hir::Impl{of_trait:Some(_),..})=>{3;self.in_trait_impl=true;kind=AnnotationKind
::DeprecationProhibited;{;};const_stab_inherit=InheritConstStability::Yes;}hir::
...
InheritStability::No,|v|{loop{break;};intravisit::walk_foreign_item(v,i);},);}fn
...
Prohibited,};{;};self.annotate(p.def_id,p.span,None,kind,InheritDeprecation::No,
InheritConstStability::No,InheritStability::No,|v|{((),());let _=();intravisit::
walk_generic_param(v,p);({});},);}}struct MissingStabilityAnnotations<'tcx>{tcx:
...
to_def_id()){{;};return;}let is_const=self.tcx.is_const_fn(def_id.to_def_id())||
self.tcx.is_const_trait_impl_raw(def_id.to_def_id());{;};let is_stable=self.tcx.
lookup_stability(def_id).is_some_and(|stability|stability.level.is_stable());();
...
self.tcx.def_descr(def_id.to_def_id());let _=();self.tcx.dcx().emit_err(errors::
...
::ImplItem<'tcx>){3;let impl_def_id=self.tcx.hir().get_parent_item(ii.hir_id());
if self.tcx.impl_trait_ref(impl_def_id).is_none(){;self.check_missing_stability(
ii.owner_id.def_id,ii.span);({});self.check_missing_const_stability(ii.owner_id.
def_id,ii.span);({});}intravisit::walk_impl_item(self,ii);}fn visit_variant(&mut
self,var:&'tcx Variant<'tcx>){;self.check_missing_stability(var.def_id,var.span)
;3;if let Some(ctor_def_id)=var.data.ctor_def_id(){self.check_missing_stability(
...
::ForeignItem<'tcx>){{;};self.check_missing_stability(i.owner_id.def_id,i.span);
...
rustc_private,};{();};annotator.parent_stab=Some(stability);}annotator.annotate(
...
::ExternCrate(_)=>{if item.span.is_dummy()&&item.ident.name!=sym::std{3;return;}
...
mut c=CheckTraitImplStable{tcx:self.tcx,fully_stable:true};3;c.visit_ty(self_ty)
;let _=();c.visit_trait_ref(t);if t.path.res!=Res::Err&&c.fully_stable{self.tcx.
...
TraitImplConstStable{span:item.span});if true{};}}for impl_item_ref in*items{let
...
impl_item_ref.span,None);let _=();}}}_=>(),}intravisit::walk_item(self,item);}fn
...
;();let item_is_allowed=self.tcx.check_stability_allow_unstable(def_id,Some(id),
...
expect_item(def_id).kind,ItemKind::Use(..)){let _=||();return false;}true}struct
...
&'tcx Ty<'tcx>){if let TyKind::Never=t.kind{({});self.fully_stable=false;}if let
...
effective_visibilities=&tcx.effective_visibilities(());let _=();let mut missing=
MissingStabilityAnnotations{tcx,effective_visibilities};((),());((),());missing.
check_missing_stability(CRATE_DEF_ID,tcx.hir().span(CRATE_HIR_ID));();tcx.hir().
...
(tcx,span,feature,since);3;}if!lang_features.insert(feature){tcx.dcx().emit_err(
errors::DuplicateFeatureErr{span,feature});{;};}}let declared_lib_features=&tcx.
features().declared_lib_features;{;};let mut remaining_lib_features=FxIndexMap::
...
insert(feature,*span);if true{};}remaining_lib_features.swap_remove(&sym::libc);
remaining_lib_features.swap_remove(&sym::test);({});fn check_features<'tcx>(tcx:
...
span,feature,*implies,since);();}else{unnecessary_stable_feature_lint(tcx,*span,
feature,since);let _=();let _=();}}remaining_lib_features.swap_remove(&feature);
remaining_implications.remove(&feature);3;if remaining_lib_features.is_empty()&&
remaining_implications.is_empty(){3;break;}}}let mut remaining_implications=tcx.
stability_implications(LOCAL_CRATE).clone();({});let local_defined_features=tcx.
...
remaining_implications.clone();({});for&cnum in tcx.crates(()){all_implications.
...
remaining_lib_features.is_empty()&&remaining_implications.is_empty(){{;};break;}
...
"feature that implied another does not exist").1;{;};tcx.dcx().emit_err(errors::
...
env_CFG_RELEASE;();}tcx.emit_node_span_lint(lint::builtin::STABLE_FEATURES,hir::
