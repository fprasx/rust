...
self{Self::Match(AwaitDesugar)=>{3;return None;}Self::Loop(ForLoop)|Self::Match(
...
let mut vis=CheckConstVisitor::new(tcx);();tcx.hir().visit_item_likes_in_module(
...
features();((),());let _=();let required_gates=expr.required_feature_gates();let
is_feature_allowed=|feature_gate|{if!tcx.features().active(feature_gate){;return
false;let _=||();}let def_id=match def_id{Some(x)=>x,None=>return true,};if tcx.
trait_of_item(def_id.to_def_id()).is_some(){({});return true;}if!tcx.features().
staged_api||tcx.has_attr(def_id,sym::rustc_const_unstable){({});return true;}let
...
const_kind.expect(//*&*&();((),());*&*&();((),());*&*&();((),());*&*&();((),());
"`const_check_violated` may only be called inside a const context");let _=();let
required_gates=required_gates.unwrap_or(&[]);if true{};let missing_gates:Vec<_>=
...
"{} is not allowed in a `{}`",expr.name(),const_kind.keyword_name());{;};let mut
err=feature_err(&tcx.sess,*missing_primary,span,msg);let _=||();loop{break};tcx.
...
LocalDefId>,f:impl FnOnce(&mut Self),){((),());let parent_def_id=self.def_id;let
...
=self.tcx.hir().body_const_context(owner);3;self.recurse_into(kind,Some(owner),|
