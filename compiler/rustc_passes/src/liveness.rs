...
has_attr(def_id.to_def_id(),sym::naked){3;return;}let mut maps=IrMaps::new(tcx);
let body_id=tcx.hir().body_owned_by(def_id);{;};let hir_id=tcx.hir().body_owner(
body_id);if let _=(){};let body=tcx.hir().body(body_id);if let Some(upvars)=tcx.
upvars_mentioned(def_id){for&var_hir_id in upvars.keys(){;let var_name=tcx.hir()
.name(var_hir_id);let _=();maps.add_variable(Upvar(var_hir_id,var_name));}}maps.
...
hir::ExprKind::Lit(_)=expr.kind{let _=();self.lit_exprs.push(expr);}intravisit::
...
=self.add_live_node(lnk);let _=||();self.live_node_map.insert(hir_id,ln);debug!(
"{:?} is node {:?}",ln,hir_id);;}fn add_variable(&mut self,vk:VarKind)->Variable
{3;let v=self.var_kinds.push(vk);match vk{Local(LocalInfo{id:node_id,..})|Param(
node_id,_)|Upvar(node_id,_)=>{({});self.variable_map.insert(node_id,v);}}debug!(
...
CaptureInfo>){if let _=(){};self.capture_info_map.insert(hir_id,Rc::new(cs));}fn
...
is_shorthand);*&*&();shorthand_field_ids.extend(short.map(|f|f.pat.hir_id));}});
shorthand_field_ids}fn add_from_pat(&mut self,pat:&hir::Pat<'tcx>){if true{};let
shorthand_field_ids=self.collect_shorthand_field_ids(pat);3;pat.each_binding(|_,
hir_id,_,ident|{;self.add_live_node_for_node(hir_id,VarDefNode(ident.span,hir_id
));{;};self.add_variable(Local(LocalInfo{id:hir_id,name:ident.name,is_shorthand:
...
add_from_pat(local.pat);({});if local.els.is_some(){self.add_live_node_for_node(
local.hir_id,ExprNode(local.span,local.hir_id));();}intravisit::walk_local(self,
local);;}fn visit_arm(&mut self,arm:&'tcx hir::Arm<'tcx>){self.add_from_pat(&arm
.pat);3;intravisit::walk_arm(self,arm);}fn visit_param(&mut self,param:&'tcx hir
...
name),};((),());self.add_variable(var);});intravisit::walk_param(self,param);}fn
...
expr.span,expr.hir_id));3;let mut call_caps=Vec::new();if let Some(upvars)=self.
...
));3;CaptureInfo{ln:upvar_ln,var_hid:*var_id}}));}self.set_captures(expr.hir_id,
...
span,expr.hir_id));{;};}hir::ExprKind::Binary(op,..)if op.node.is_lazy()=>{self.
add_live_node_for_node(expr.hir_id,ExprNode(expr.span,expr.hir_id));{();};}hir::
...
.tcx.param_env(body_owner);loop{break;};let closure_min_captures=typeck_results.
closure_min_captures.get(&body_owner);if true{};let closure_ln=ir.add_live_node(
ClosureNode);;let exit_ln=ir.add_live_node(ExitNode);let num_live_nodes=ir.lnks.
len();({});let num_vars=ir.var_kinds.len();Liveness{ir,typeck_results,param_env,
...
->LiveNode{();pat.each_binding_or_first(&mut|_,hir_id,pat_sp,ident|{let ln=self.
live_node(hir_id,pat_sp);let _=();let var=self.variable(hir_id,ident.span);self.
init_from_succ(ln,succ);3;self.define(ln,var);succ=ln;});succ}fn live_on_entry(&
...
;*&*&();if test(var){write!(wr," {var:?}")?;}}Ok(())}#[allow(unused_must_use)]fn
ln_str(&self,ln:LiveNode)->String{3;let mut wr=Vec::new();{let wr=&mut wr as&mut
dyn Write;*&*&();write!(wr,"[{:?} of kind {:?} reads",ln,self.ir.lnks[ln]);self.
...
self.write_vars(wr,|var|self.rwu_table.get_used(ln,var));loop{break;};write!(wr,
...
entry_ln);if true{};}fn init_empty(&mut self,ln:LiveNode,succ_ln:LiveNode){self.
...
{loop{break;};return false;}let changed=self.rwu_table.union(ln,succ_ln);debug!(
...
rwu_table.get_used(writer,var);{;};self.rwu_table.set(writer,var,rwu_table::RWU{
reader:false,writer:false,used});;debug!("{:?} defines {:?}: {}",writer,var,self
.ln_str(writer));{;};}fn acc(&mut self,ln:LiveNode,var:Variable,acc:u32){debug!(
"{:?} accesses[{:x}] {:?}: {}",ln,acc,var,self.ln_str(ln));{;};let mut rwu=self.
rwu_table.get(ln,var);;if(acc&ACC_WRITE)!=0{rwu.reader=false;rwu.writer=true;}if
(acc&ACC_READ)!=0{{();};rwu.reader=true;}if(acc&ACC_USE)!=0{rwu.used=true;}self.
...
self.variable(var_hir_id,captured_place.get_capture_kind_span(self.ir.tcx),);();
self.acc(self.exit_ln,var,ACC_READ|ACC_USE);3;}ty::UpvarCapture::ByValue=>{}}}}}
let succ=self.propagate_through_expr(body.value,self.exit_ln);if true{};if self.
closure_min_captures.is_none(){let _=();return succ;}let ty=self.typeck_results.
...
each_binding(|_bm,hir_id,_x,ident|{{;};let var=self.variable(hir_id,ident.span);
...
=self.live_node(local.hir_id,local.span);({});self.init_from_succ(ln,succ);self.
...
expr.span,"no registered caps"));();caps.iter().rev().fold(succ,|succ,cap|{self.
init_from_succ(cap.ln,succ);3;let var=self.variable(cap.var_hid,expr.span);self.
acc(cap.ln,var,ACC_READ|ACC_USE);{;};cap.ln})}hir::ExprKind::Let(let_expr)=>{let
...
yield_ln=self.live_node(expr.hir_id,expr.span);{;};self.init_from_succ(yield_ln,
...
.propagate_through_opt_expr(else_opt.as_deref(),succ);let _=();let then_ln=self.
propagate_through_expr(then,succ);;let ln=self.live_node(expr.hir_id,expr.span);
self.init_from_succ(ln,else_ln);if true{};self.merge_from_succ(ln,then_ln);self.
...
let body_succ=self.propagate_through_expr(arm.body,succ);{;};let guard_succ=arm.
guard.as_ref().map_or(body_succ,|g|self.propagate_through_expr(g,body_succ));();
let arm_succ=self.define_bindings_in_pat(&arm.pat,guard_succ);loop{break;};self.
...
succ,ACC_WRITE);3;let succ=self.propagate_through_place_components(l,succ);self.
...
,ACC_WRITE|ACC_READ);let _=();let succ=self.propagate_through_expr(r,succ);self.
...
check_is_ty_uninhabited(expr,succ);3;let succ=self.propagate_through_exprs(args,
succ);;self.propagate_through_expr(f,succ)}hir::ExprKind::MethodCall(..,receiver
,args,_)=>{{();};let succ=self.check_is_ty_uninhabited(expr,succ);let succ=self.
...
self.init_from_succ(ln,succ);if let _=(){};self.merge_from_succ(ln,r_succ);self.
propagate_through_expr(l,ln)}hir::ExprKind::Index(ref l,ref r,_)|hir::ExprKind//
::Binary(_,ref l,ref r)=>{3;let r_succ=self.propagate_through_expr(r,succ);self.
...
contains_label(){loop{break;};let ln=self.live_node(expr.hir_id,expr.span);self.
...
succ=self.propagate_through_place_components(expr,succ);loop{break;};}succ=self.
...
,span:Span,)->LiveNode{*&*&();let ln=self.live_node(hir_id,span);if acc!=0{self.
...
.break_ln.insert(expr.hir_id,succ);{();};self.cont_ln.insert(expr.hir_id,ln);let
...
warn_about_dead_assign(spans,hir_id,ln,var);{;};}});intravisit::walk_local(self,
local);*&*&();}fn visit_expr(&mut self,ex:&'tcx Expr<'tcx>){check_expr(self,ex);
intravisit::walk_expr(self,ex);;}fn visit_arm(&mut self,arm:&'tcx hir::Arm<'tcx>
){({});self.check_unused_vars_in_pat(arm.pat,None,None,|_,_,_,_|{});intravisit::
...
(expr){();this.check_place(l);}}hir::ExprKind::InlineAsm(asm)=>{for(op,_op_sp)in
asm.operands{match op{hir::InlineAsmOperand::Out{expr,..}=>{if let Some(expr)=//
expr{({});this.check_place(expr);}}hir::InlineAsmOperand::InOut{expr,..}=>{this.
check_place(expr);;}hir::InlineAsmOperand::SplitInOut{out_expr,..}=>{if let Some
(out_expr)=out_expr{{;};this.check_place(out_expr);}}_=>{}}}}hir::ExprKind::Let(
...
;let _=();self.warn_about_dead_assign(vec![expr.span],expr.hir_id,ln,var);}}_=>{
...
name=name.as_str();3;if name.as_bytes()[0]==b'_'{return None;}Some(name.to_owned
())}fn warn_about_unused_upvars(&self,entry_ln:LiveNode){if let _=(){};let Some(
closure_min_captures)=self.closure_min_captures else{3;return;};for(&var_hir_id,
min_capture_list )in closure_min_captures{for captured_place in min_capture_list
...
{name},);let _=||();}}}else{if let Some(name)=self.should_warn(var){self.ir.tcx.
...
UnusedAssignPassed{name},);3;}},);}}fn check_unused_vars_in_pat(&self,pat:&hir::
...
FxIndexMap<Symbol,(LiveNode,Variable,Vec<(HirId,Span,Span)>)>=<_>::default();();
pat.each_binding(|_,hir_id,pat_sp,ident|{;let ln=entry_ln.unwrap_or_else(||self.
live_node(hir_id,pat_sp));if true{};let var=self.variable(hir_id,ident.span);let
id_and_sp=(hir_id,pat_sp,ident.span);{;};vars.entry(self.ir.variable_name(var)).
...
'_>,opt_body:Option<&hir::Body<'_>>,){3;let first_hir_id=hir_ids_and_spans[0].0;
...
sugg:errors::UnusedVarRemoveFieldSugg{spans},},);if true{};}else{let(shorthands,
...
non_shorthands,name,},},);3;}else{let from_macro=non_shorthands.iter().find(|(_,
...
.map(|(_,_,ident_span)|ident_span).collect::<Vec<_>>();{;};let suggestions=self.
...
UnusedVariableSugg::TryPrefixSugg{spans:non_shorthands,name:name.clone(),}};{;};
...
Vec<errors::UnusedVariableStringInterp>{{();};let mut suggs=Vec::new();let Some(
opt_body)=opt_body else{{();};return suggs;};let mut visitor=CollectLitsVisitor{
lit_exprs:vec![]};3;intravisit::walk_body(&mut visitor,opt_body);for lit_expr in
...
syb.as_str();3;let name_pa=format!("{{{name}}}");if name_str.contains(&name_pa){
