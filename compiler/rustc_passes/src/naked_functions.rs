...
=tcx.has_attr(def_id,sym::naked);({});if!naked{continue;}let(fn_header,body_id)=
...
fn_header.abi);3;check_no_patterns(tcx,body.params);check_no_parameters_use(tcx,
body);;check_asm(tcx,def_id,body);check_inline(tcx,def_id);}}fn check_inline(tcx
:TyCtxt<'_>,def_id:LocalDefId){3;let attrs=tcx.get_attrs(def_id,sym::inline);for
attr in attrs{3;tcx.dcx().emit_err(CannotInlineNakedFunction{span:attr.span});}}
...
CheckInlineAssembly{tcx,items:Vec::new()};((),());this.visit_body(body);if let[(
...
if has_asm=>{({});must_show_error=true;multiple_asms.push(span);}ItemKind::Asm=>
has_asm=true,ItemKind::NonAsm=>{{();};must_show_error=true;non_asms.push(span);}
...
::Asm,span));3;self.check_inline_asm(asm,span);}ExprKind::DropTemps(..)|ExprKind
...
collect();let _=||();if!unsupported_operands.is_empty(){self.tcx.dcx().emit_err(
NakedFunctionsOperands{unsupported_operands});{;};}let unsupported_options:Vec<&
...
expr)=>{();self.check_expr(expr,stmt.span);}}}fn visit_expr(&mut self,expr:&'tcx
hir::Expr<'tcx>){if let _=(){};*&*&();((),());self.check_expr(expr,expr.span);}}
