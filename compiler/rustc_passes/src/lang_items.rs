...
attr_span,generics,actual_target,);3;}Some(lang_item)=>{self.tcx.dcx().emit_err(
...
&&original_def_id!=item_def_id{if true{};let lang_item_name=lang_item.name();let
crate_name=self.tcx.crate_name(item_def_id.krate);3;let mut dependency_of=Empty;
let is_local=item_def_id.is_local();{;};let path=if is_local{String::new()}else{
...
orig_dependency_of=Empty;*&*&();let orig_is_local=original_def_id.is_local();let
...
=item_span{let _=();let _=();self.item_spans.insert(item_def_id,item_span);}}}fn
...
Target,){*&*&();let name=lang_item.name();if let Some(generics)=generics{let mut
actual_num=generics.params.len();{;};if target.is_associated_item(){actual_num+=
...
actual_num,at_least,});((),());return;}}self.collect_item(lang_item,item_def_id.
...
.node_id_to_def_id[&i.id],&i.attrs,i.span,i.opt_generics(),);();let parent_item=
self.parent_item.replace(i);if true{};visit::walk_item(self,i);self.parent_item=
...
AssocItemKind::MacCall(_)=>unreachable!("macros should have been expanded"),};3;
...
&mut Providers){let _=||();loop{break};providers.get_lang_items=get_lang_items;}
