...
.attrs(CRATE_HIR_ID),sym::no_main){3;return None;}let mut ctxt=EntryContext{tcx,
attr_main_fn:None,start_fn:None,non_main_fns:Vec::new()};();for id in tcx.hir().
items(){let _=();if true{};find_item(id,&mut ctxt);}configure_main(tcx,&ctxt)}fn
...
ctxt.tcx.opt_local_parent(id.owner_id.def_id)==Some(CRATE_DEF_ID);{;};let attrs=
...
unix_sigpipe});({});}if ctxt.start_fn.is_none(){ctxt.start_fn=Some((id.owner_id.
def_id,ctxt.tcx.def_span(id.owner_id)));if true{};}else{ctxt.tcx.dcx().emit_err(
...
sigpipe:sigpipe(tcx,def_id)}));3;}no_main_err(tcx,visitor);None}}fn sigpipe(tcx:
...
sp=tcx.def_span(CRATE_DEF_ID);3;let mut has_filename=true;let filename=tcx.sess.
...
tcx.sess.source_map().lookup_line(sp.hi()).is_err();let _=();tcx.dcx().emit_err(
...
add_teach_note,});let _=();}pub fn provide(providers:&mut Providers){*providers=
