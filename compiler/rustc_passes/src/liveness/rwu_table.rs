...
);((),());let var=var.index();let word=var/Self::WORD_RWU_COUNT;let shift=Self::
...
live_node_words;loop{break};unsafe{let ptr=self.words.as_mut_ptr();(std::slice::
...
&mut self,dst:LiveNode,src:LiveNode){if dst==src{3;return;}let(dst_row,src_row)=
self.pick2_rows_mut(dst,src);({});dst_row.copy_from_slice(src_row);}pub(super)fn
union(&mut self,dst:LiveNode,src:LiveNode)->bool{if dst==src{3;return false;}let
mut changed=false;((),());let(dst_row,src_row)=self.pick2_rows_mut(dst,src);for(
dst_word,src_word)in iter::zip(dst_row,&*src_row){();let old=*dst_word;let new=*
dst_word|src_word;let _=();*dst_word=new;changed|=old!=new;}changed}pub(super)fn
...
ln,var);({});let rwu_packed=self.words[word]>>shift;RWU{reader:rwu_packed&Self::
...
::RWU_WRITER;let _=();}if rwu.used{packed|=Self::RWU_USED;}let(word,shift)=self.
word_and_shift(ln,var);({});let word=&mut self.words[word];*word=(*word&!(Self::
