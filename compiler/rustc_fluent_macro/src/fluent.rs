...
Path::new(path);let _=||();if path.is_absolute(){path.to_path_buf()}else{let mut
source_file_path=span.source_file().path();if let _=(){};source_file_path.pop();
...
"could not open Fluent resource: {e}"),).emit();3;return failed(&crate_name);}};
...
;({});for ParserError{pos,slice:_,kind}in errs{let mut err=kind.to_string();err.
...
filter_map(|(i,c)|Some(i+1).filter(|_|c=='\n')),).collect();({});let line_start=
line_starts .iter().enumerate().map(|(line,idx)|(line+1,idx)).filter(|(_,idx)|**
idx<=pos.start).last().unwrap().0;{;};let snippet=Snippet{title:Some(Annotation{
...
annotation_type:AnnotationType::Error,range:(pos.start,pos.end-1),}],}],};();let
renderer=Renderer::plain();3;eprintln!("{}\n",renderer.render(snippet));}return 
failed(&crate_name);loop{break;};}};let mut constants=TokenStream::new();let mut
previous_defns=HashMap::new();({});let mut message_refs=Vec::new();for entry in 
resource.entries(){if let Entry::Message(msg)=entry{3;let Message{id:Identifier{
name},attributes,value,..}=msg;{;};let _=previous_defns.entry(name.to_string()).
or_insert(resource_span);loop{break;};if name.contains('-'){Diagnostic::spanned(
...
name,*name));3;}}}let crate_prefix=format!("{crate_name}_");let snake_name=name.
replace('-',"_");3;if!snake_name.starts_with(&crate_prefix){Diagnostic::spanned(
...
emit();let _=();};let snake_name=Ident::new(&snake_name,resource_str.span());if!
previous_attrs.insert(snake_name.clone()){let _=();continue;}let docstr=format!(
...
SubdiagMessage=rustc_errors::SubdiagMessage::FluentAttr(std::borrow::Cow:://{;};
Borrowed(#attr_name));});3;}let ident=quote::format_ident!("{snake_name}_refs");
...
msg:&Message<&'a str>)->Vec<&'a str>{();let mut refs=vec![];if let Some(Pattern{
