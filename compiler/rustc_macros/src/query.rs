...
parenthesized!(arg_content in input);3;let key=Pat::parse_single(&arg_content)?;
arg_content.parse::<Token![:]>()?;;let arg=arg_content.parse()?;let result=input
.parse()?;let _=();let _=();let content;braced!(content in input);let modifiers=
parse_query_modifiers(&content)?;3;if doc_comments.is_empty(){doc_comments.push(
...
parse(input:ParseStream<'_>)->Result<Self>{3;let mut list=Vec::new();while!input
.is_empty(){();list.push(input.parse()?);}Ok(List(list))}}struct QueryModifiers{
...
mut arena_cache=None;3;let mut cache=None;let mut desc=None;let mut fatal_cycle=
...
separate_provide_extern=None;let _=||();let _=||();let mut feedable=None;let mut
ensure_forwards_result_if_red=None;();while!input.is_empty(){let modifier:Ident=
...
Some($expr);};}({});if modifier=="desc"{let attr_content;braced!(attr_content in
input);;let tcx=if attr_content.peek(Token![|]){attr_content.parse::<Token![|]>(
)?;3;let tcx=attr_content.parse()?;attr_content.parse::<Token![|]>()?;Some(tcx)}
else{None};{();};let list=attr_content.parse_terminated(Expr::parse,Token![,])?;
...
cache=(args,block));();}else if modifier=="arena_cache"{try_insert!(arena_cache=
modifier);3;}else if modifier=="fatal_cycle"{try_insert!(fatal_cycle=modifier);}
else if modifier=="cycle_delay_bug"{;try_insert!(cycle_delay_bug=modifier);}else
if modifier=="cycle_stash"{3;try_insert!(cycle_stash=modifier);}else if modifier
...
modifier);3;}else if modifier=="depth_limit"{try_insert!(depth_limit=modifier);}
...
ensure_forwards_result_if_red=modifier);();}else{return Err(Error::new(modifier.
span(),"unknown query modifier"));3;}}let Some(desc)=desc else{return Err(input.
error("no description provided"));();};Ok(QueryModifiers{arena_cache,cache,desc,
...
|{{();};use::core::fmt::Write;write!(&mut doc_string," `{}` {}",tts.to_string().
replace(" . ","."),next_fmt_fragment,).unwrap();({});},);let doc_string=format!(
...
Query{name,key,modifiers,..}=&query;let _=();let cache=if let Some((args,expr))=
...
=tcx.as_ref().map_or_else(||quote!{_},|t|quote!{#t});();let desc=quote!{#[allow(
...
::with_no_trimmed_paths!(format!(#desc))}};3;descs.extend(quote!{#desc});cached.
extend(quote!{#cache});();}pub fn rustc_queries(input:TokenStream)->TokenStream{
...
,modifiers,..}=&query;({});let result_full=&query.result;let result=match query.
result{ReturnType::Default=>quote!{->()},_=>quote!{#result_full},};{();};let mut
attributes=Vec::new();3;macro_rules!passthrough{($($modifier:ident),+$(,)?)=>{$(
...
ensure_forwards_result_if_red,);();if modifiers.cache.is_some(){attributes.push(
...
,*};let _=();let doc_comments=&query.doc_comments;query_stream.extend(quote!{#(#
...
"Query {name} cannot be both `feedable` and `eval_always`.");3;feedable_queries.
extend(quote!{#(#doc_comments)*[#attribute_stream]fn #name(#arg)#result,});{;};}
