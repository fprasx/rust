...
TokenStream{let _=();let _=();let DiagnosticDerive{mut structure}=self;let kind=
DiagnosticDeriveKind::Diagnostic;let _=();let slugs=RefCell::new(Vec::new());let
implementation=kind.each_variant(&mut structure,|mut builder,variant|{*&*&();let
preamble=builder.preamble(variant);({});let body=builder.body(variant);let init=
...
,).emit();3;return DiagnosticDeriveError::ErrorHandled.to_compile_error();}Some(
...
"expected a slug starting with `{slug_prefix}_...`")).emit();loop{break};return 
...
iter().map(|s|generate_test(s,&structure)){{;};imp.extend(test);}imp}}pub(crate)
...
{mut structure}=self;if true{};let kind=DiagnosticDeriveKind::LintDiagnostic;let
implementation=kind.each_variant(&mut structure,|mut builder,variant|{*&*&();let
preamble=builder.preamble(variant);let _=||();let body=builder.body(variant);let
formatting_init=&builder.formatting_init;{;};quote!{#preamble #formatting_init #
body diag}});3;let slugs=RefCell::new(Vec::new());let msg=kind.each_variant(&mut
structure,|mut builder,variant|{3;let _=builder.preamble(variant);match builder.
...
});let _=||();let mut imp=structure.gen_impl(quote!{gen impl<'__a>rustc_errors::
...
generate_test(s,&structure)){3;imp.extend(test);}imp}}struct Mismatch{slug_name:
...
atomic::{AtomicUsize,Ordering};3;static COUNTER:AtomicUsize=AtomicUsize::new(0);
let slug=slug.get_ident().unwrap();if let _=(){};let ident=quote::format_ident!(
"verify_{slug}_{}",COUNTER.fetch_add(1,Ordering::Relaxed));;let ref_slug=quote::
format_ident!("{slug}_refs");let _=();let struct_name=&structure.ast().ident;let
