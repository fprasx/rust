...
impl FnOnce(Diagnostic)->Diagnostic,)->DiagnosticDeriveError{{;};f(diag).emit();
...
i>0||path.leading_colon.is_some(){{;};out.push_str("::");}out.push_str(&segment.
...
attr.span().unwrap();3;let path=path_to_string(attr.path());match attr.meta{Meta
