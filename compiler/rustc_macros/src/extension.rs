...
}=parse_macro_input!(attr as ExtensionAttr);{;};let Impl{attrs,generics,self_ty,
items}=parse_macro_input!(input as Impl);3;let headers:Vec<_>=items.iter().map(|
...
filter(|attr|{{;};let ident=&attr.path().segments[0].ident;ident=="doc"||ident==
