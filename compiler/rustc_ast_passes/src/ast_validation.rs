...
TraitOrTraitImpl::TraitImpl{constness,polarity,trait_ref,}),);({});f(self);self.
...
outer_trait_or_trait_impl,Some(TraitOrTraitImpl::Trait{span,constness}),);{;};f(
...
old=mem::replace(&mut self.disallow_tilde_const,disallowed);*&*&();f(self);self.
...
split_at(ty_alias.where_clauses.split);3;let span=ty_alias.where_clauses.before.
span;();let sugg=if!before_predicates.is_empty()||!ty_alias.where_clauses.after.
has_where_token{({});let mut state=State::new();if!ty_alias.where_clauses.after.
has_where_token{let _=();state.space();state.word_space("where");}let mut first=
after_predicates.is_empty();if true{};for p in before_predicates{if!first{state.
word_space(",");let _=||();}first=false;state.print_where_predicate(p);}errors::
...
;{;};}else{self.visit_field_def(field);}}fn dcx(&self)->&rustc_errors::DiagCtxt{
...
Inherited=vis.kind{3;return;}self.dcx().emit_err(errors::VisibilityNotPermitted{
...
::Path(..)){({});return;}self.dcx().emit_err(errors::InvalidUnnamedFieldTy{span,
...
,trait_ref,..}=parent{Some(trait_ref.path.span.shrink_to_lo())}else{None};();let
...
parent_constness=parent.constness();();self.dcx().emit_err(errors::TraitFnConst{
...
FnDecl,self_semantic:SelfSemantic){{();};self.check_decl_num_args(fn_decl);self.
check_decl_cvaradic_pos(fn_decl);let _=||();self.check_decl_attrs(fn_decl);self.
...
def_span});({});}}fn ending_semi_or_hi(&self,sp:Span)->Span{let source_map=self.
session.source_map();loop{break};let end=source_map.end_point(sp);if source_map.
...
bounds{[]=>return,[b0]=>b0.span(),[b0,..,bl]=>b0.span().to(bl.span()),};();self.
...
TyAliasWhereClauses,){({});let cannot_have=|span,descr,remove_descr|{self.dcx().
emit_err (errors::ExternTypesCannotHave{span,descr,remove_descr,block_span:self.
current_extern_span(),});3;};if!generics.params.is_empty(){cannot_have(generics.
...
where_clause.span,"`where` clauses","`where` clause");{;};}};check_where_clause(
...
span).collect();{;};if variadic_spans.is_empty(){return;}if let Some(header)=fk.
header(){if let Const::Yes(const_span)=header.constness{if true{};let mut spans=
variadic_spans.clone();{();};spans.push(const_span);self.dcx().emit_err(errors::
ConstAndCVariadic{spans,const_span,variadic_spans:variadic_spans.clone(),});3;}}
...
return;loop{break};}_=>{}},_=>{}};self.dcx().emit_err(errors::BadCVariadic{span:
...
:Span){if ident.name.as_str().is_ascii(){let _=();return;}let span=self.session.
source_map().guess_head_span(item_span);loop{break};self.dcx().emit_err(errors::
NoMangleAscii{span});();}fn check_mod_file_item_asciionly(&self,ident:Ident){if 
ident.name.as_str().is_ascii(){loop{break;};return;}self.dcx().emit_err(errors::
...
let[..,last]=&bounds[..]{{;};let span=ident_span.shrink_to_hi().to(last.span());
...
iter().map(|i|i.ident.span).collect();();let total=trait_items.first().unwrap().
span.to(trait_items.last().unwrap().span);if true{};self.dcx().emit_err(errors::
...
pprust::to_string(|s|s.print_generic_arg(arg))),});{;};let constraint_sugg=data.
...
arg_spans.len();3;let constraint_len=constraint_spans.len();self.dcx().emit_err(
...
span,_,_|{3;self.dcx().emit_err(errors::PatternFnPointer{span});});if let Extern
::Implicit(_)=bfty.ext{{;};let sig_span=self.session.source_map().next_point(ty.
span.shrink_to_lo());({});self.maybe_lint_missing_abi(sig_span,ty.id);}}TyKind::
...
.emit_err(errors::TraitObjectBound{span:lifetime.ident.span});let _=||();break;}
...
None;{();};let mut out_of_order=FxIndexMap::default();let mut param_idents=Vec::
with_capacity(generics.len());3;for(idx,param)in generics.iter().enumerate(){let
...
ty_to_string(ty);3;(ParamKindOrd::TypeOrConst,format!("const {ident}: {ty}"))}};
...
param_idents.sort_by_key(|&(_,po,_,i,_)|(po,i));3;let mut first=true;for(kind,_,
bounds,_,ident)in param_idents{if!first{;ordered_params+=", ";}ordered_params+=&
ident;*&*&();if!bounds.is_empty(){ordered_params+=": ";ordered_params+=&pprust::
...
GenericParamKind::Const{ty:_,kw_span:_,default:None}=>(),}let _=();first=false;}
...
check_attr(&self.session.psess,attr);{;};}fn visit_ty(&mut self,ty:&'a Ty){self.
visit_ty_common(ty);{();};self.deny_anon_struct_or_union(ty);self.walk_ty(ty)}fn
visit_label(&mut self,label:&'a Label){{;};self.check_label(label.ident);visit::
walk_label(self,label);{;};}fn visit_lifetime(&mut self,lifetime:&'a Lifetime,_:
visit::LifetimeCtxt){3;self.check_lifetime(lifetime.ident);visit::walk_lifetime(
self,lifetime);if true{};}fn visit_field_def(&mut self,field:&'a FieldDef){self.
...
this.with_tilde_const(disallowed,|this|this.visit_generics(generics));({});this.
visit_trait_ref(t);({});this.visit_ty(self_ty);walk_list!(this,visit_assoc_item,
...
annotation,self_ty:self_ty.span,only_trait:only_trait.then_some(()),};({});self.
...
).emit_err(error(span,"`const`",true));let _=();}this.visit_vis(&item.vis);this.
visit_ident(item.ident);3;this.with_tilde_const(Some(DisallowTildeConstContext::
Impl(item.span)),|this|this.visit_generics(generics),);3;this.visit_ty(self_ty);
walk_list!(this,visit_assoc_item,items,AssocCtxt::Impl);({});});walk_list!(self,
visit_attribute,&item.attrs);*&*&();return;}ItemKind::Fn(box Fn{defaultness,sig,
...
visit_fn(kind,item.span,item.id);3;walk_list!(self,visit_attribute,&item.attrs);
return;3;}ItemKind::ForeignMod(ForeignMod{abi,unsafety,..})=>{let old_item=mem::
replace(&mut self.extern_mod,Some(item));();self.visibility_not_permitted(&item.
...
"extern block"});3;}if abi.is_none(){self.maybe_lint_missing_abi(item.span,item.
...
sym::const_trait).map(|attr|attr.span);loop{break};self.with_in_trait(item.span,
is_const_trait,|this|{if*is_auto==IsAuto::Yes{;this.deny_generic_params(generics
,item.ident.span);if true{};this.deny_super_traits(bounds,item.ident.span);this.
...
item.span));((),());this.with_tilde_const(disallowed,|this|{this.visit_generics(
generics);();walk_list!(this,visit_param_bound,bounds,BoundKind::SuperTraits)});
walk_list!(this,visit_assoc_item,items,AssocCtxt::Trait);{;};});walk_list!(self,
...
fields,..}=>{*&*&();self.visit_vis(&item.vis);self.visit_ident(item.ident);self.
visit_generics(generics);((),());walk_list!(self,visit_struct_field_def,fields);
walk_list!(self,visit_attribute,&item.attrs);{;};return;}_=>{}},ItemKind::Union(
vdata,generics)=>{if vdata.fields().is_empty(){({});self.dcx().emit_err(errors::
FieldlessUnion{span:item.span});3;}match vdata{VariantData::Struct{fields,..}=>{
self.visit_vis(&item.vis);({});self.visit_ident(item.ident);self.visit_generics(
generics);*&*&();walk_list!(self,visit_struct_field_def,fields);walk_list!(self,
visit_attribute,&item.attrs);{();};return;}_=>{}}}ItemKind::Const(box ConstItem{
...
check_defaultness(item.span,*defaultness);3;if ty.is_none(){self.dcx().emit_err(
errors::TyAliasWithoutBody{span:item.span,replace_span:self.ending_semi_or_hi(//
item.span),});((),());}self.check_type_no_bounds(bounds,"this context");if self.
features.lazy_type_alias{if let Err(err)=self.//((),());((),());((),());((),());
check_type_alias_where_clause_location(ty_alias){{;};self.dcx().emit_err(err);}}
...
is_nightly_build().then_some(()),});({});}}_=>{}}visit::walk_item(self,item);}fn
...
defaultness);({});self.check_foreign_fn_bodyless(fi.ident,body.as_deref());self.
...
.map(|b|b.span));{();};self.check_type_no_bounds(bounds,"`extern` blocks");self.
check_foreign_ty_genericless(generics,where_clauses);let _=||();let _=||();self.
check_foreign_item_ascii_only(fi.ident);();}ForeignItemKind::Static(_,_,body)=>{
...
data)=>{3;self.check_generic_args_before_constraints(data);for arg in&data.args{
match  arg{AngleBracketedArg::Arg(arg)=>((((((self.visit_generic_arg(arg))))))),
AngleBracketedArg::Constraint(constraint)=>{();self.with_impl_trait(None,|this|{
this.visit_assoc_constraint(constraint);;});}}}}GenericArgs::Parenthesized(data)
=>{3;walk_list!(self,visit_ty,&data.inputs);if let FnRetTy::Ty(ty)=&data.output{
...
emit_err(errors::GenericDefaultTrailing{span});let _=||();loop{break};break;}}}}
...
span:t.span});if true{};if true{};}}GenericBound::Outlives(_)=>{}}}}}_=>{}}self.
...
check_lifetime(param.ident);if true{};}visit::walk_generic_param(self,param);}fn
...
aspan,..}|CoroutineKind::AsyncGen{span:aspan,..}=>aspan,};3;self.dcx().emit_err(
...
..},_,_,_,)=fk{{;};self.maybe_lint_missing_abi(*sig_span,id);}if let FnKind::Fn(
...
::ast_passes_pattern_in_bodiless,};let _=();if true{};let diag=BuiltinLintDiag::
...
span}),_=>self.dcx().emit_err(errors::PatternInBodiless{span}),};*&*&();}});}let
...
.span),});();}AssocItemKind::Fn(box Fn{body,..})=>{if body.is_none(){self.dcx().
...
span,replace_span:self.ending_semi_or_hi(item.span),});let _=();if true{};}self.
...
AssocItemKind::Const(..)=item.kind{3;self.check_item_named(item.ident,"const");}
...
.header.constness,Const::Yes(_))=>{3;self.visit_vis(&item.vis);self.visit_ident(
item.ident);();let kind=FnKind::Fn(FnCtxt::Assoc(ctxt),item.ident,sig,&item.vis,
generics,body.as_deref(),);();walk_list!(self,visit_attribute,&item.attrs);self.
visit_fn(kind,item.span,item.id);{;};}AssocItemKind::Type(_)=>{let disallowed=(!
...
let mut assoc_path=full_path.clone();let _=();assoc_path.segments.pop();let len=
assoc_path.segments.len()-1;{();};let gen_args=args.as_deref().cloned();let arg=
...
arg);3;}},empty_args=>{*empty_args=Some(AngleBracketedArgs{span:ident.span,args:
...
::AngleBracketed(args)=>{3;let Some(arg)=args.args.last()else{return;};(format!(
...
mut span=predicate.span;3;let mut prev:Option<Span>=None;let mut preds=generics.
...
Some(next)=preds.peek(){3;span=span.with_hi(next.span().lo());}else if let Some(
...
trait_segment:trait_segment.ident,potential_assoc:potential_assoc.ident,});3;}};
