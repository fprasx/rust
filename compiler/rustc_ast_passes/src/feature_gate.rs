...
PostExpansionVisitor<'a>,in_associated_ty:bool,}loop{break;};impl Visitor<'_>for
...
);loop{break;};loop{break;};}else{gate!(&self.vis,type_alias_impl_trait,ty.span,
"`impl Trait` in type aliases is unstable");((),());}}visit::walk_ty(self,ty);}}
...
..)=>{();gate!(&self,trait_alias,i.span,"trait aliases are experimental");}ast::
...
link_name);let _=||();let links_to_llvm=link_name.is_some_and(|val|val.as_str().
...
..)=>{();gate!(&self,extern_types,i.span,"extern types are experimental");}ast::
...
(&bare_fn_ty.generic_params);();}ast::TyKind::Never=>{gate!(&self,never_type,ty.
...
self.check_late_bound_lifetime_defs(&bound_pred.bound_generic_params);3;}_=>{}}}
...
"associated type defaults are unstable");if let _=(){};}if let Some(ty)=ty{self.
...
maybe_stage_features(sess,features,krate);({});check_incompatible_features(sess,
...
,$help);}}};}let _=();gate_all!(if_let_guard,"`if let` guards are experimental",
"you can write `if matches!(<expr>, <pattern>)` instead of `if let <pattern> = <expr>`"
);{();};gate_all!(let_chains,"`let` expressions in this position are unstable");
gate_all!(async_closure,"async closures are unstable",//loop{break};loop{break};
"to use an async block, remove the `||`: `async {`");3;gate_all!(async_for_loop,
...
gate!(&visitor,coroutines,span,"yield syntax is experimental");({});}}gate_all!(
gen_blocks,"gen blocks are experimental");((),());let _=();gate_all!(raw_ref_op,
"raw address of syntax is experimental");loop{break};gate_all!(const_trait_impl,
...
"inline-const is experimental");if true{};let _=||();gate_all!(inline_const_pat,
...
yeet_expr,"`do yeet` expression is experimental");let _=||();gate_all!(dyn_star,
"`dyn*` trait objects are experimental");if let _=(){};gate_all!(const_closures,
...
"`become` expression is experimental");let _=||();gate_all!(generic_const_items,
...
"functions delegation is not yet fully implemented");();gate_all!(postfix_match,
...
spans{if span.allows_unstable(sym::never_patterns){*&*&();continue;}let sm=sess.
...
);{();};sess.dcx().emit_err(errors::MatchArmWithNoBody{span,suggestion});}}}}if!
...
span});3;}}macro_rules!gate_all_legacy_dont_use{($gate:ident,$msg:literal)=>{for
...
"box pattern syntax is experimental");{;};gate_all_legacy_dont_use!(trait_alias,
"trait aliases are experimental");if true{};if true{};gate_all_legacy_dont_use!(
return_type_notation,"return type notation is experimental");let _=();if true{};
gate_all_legacy_dont_use!(decl_macro,"`macro` is experimental");((),());((),());
gate_all_legacy_dont_use!(exclusive_range_pattern,//if let _=(){};if let _=(){};
"exclusive range pattern syntax is experimental");{;};gate_all_legacy_dont_use!(
try_blocks,"`try` blocks are unstable");3;gate_all_legacy_dont_use!(auto_traits,
"`auto` traits are unstable");let _=();visit::walk_crate(&mut visitor,krate);}fn
...
declared_lang_features;({});if lang_features.len()==0{return;}for attr in krate.
...
unwrap_or("(unknown)"),stable_features:vec![],sugg:None,};();let mut all_stable=
true;;for ident in attr.meta_item_list().into_iter().flatten().flat_map(|nested|
nested.ident()){{();};let name=ident.name;let stable_since=lang_features.iter().
...
since});3;}else{all_stable=false;}}if all_stable{err.sugg=Some(attr.span);}sess.
...
f1_span,f2_span];({});sess.dcx().emit_err(errors::IncompatibleFeatures{spans,f1:
