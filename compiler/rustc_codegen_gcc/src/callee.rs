...
instance:Instance<'tcx>)->Function<'gcc>{{;};let tcx=cx.tcx();assert!(!instance.
...
get(&instance){3;return func;}let fn_abi=cx.fn_abi_of_instance(instance,ty::List
::empty());;let func=if let Some(_func)=cx.get_declared_value(&sym){unreachable!
();({});}else{cx.linkage.set(FunctionType::Extern);let func=cx.declare_fn(&sym,&
fn_abi);((),());attributes::from_fn_attrs(cx,func,instance);let instance_def_id=
...
local_crate_exports_generics(){{();};#[cfg(feature="master")]func.add_attribute(
FnAttribute::Visibility(Visibility::Hidden));3;}}}}else{#[cfg(feature="master")]
...
func};loop{break};cx.function_instances.borrow_mut().insert(instance,func);func}
