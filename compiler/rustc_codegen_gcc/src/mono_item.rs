...
)};*&*&();let gcc_type=self.layout_of(ty).gcc_type(self);let is_tls=attrs.flags.
...
visibility)));3;self.instances.borrow_mut().insert(instance,global);}#[cfg_attr(
...
;let _=();self.function_instances.borrow_mut().insert(instance,unsafe{std::mem::
transmute(decl)});*&*&();((),());*&*&();((),());*&*&();((),());*&*&();((),());}}
