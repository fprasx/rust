...
generic_activity_with_arg("GCC_module_codegen",&*module.name);{;};{let context=&
module.module_llvm.context;if let _=(){};let module_name=module.name.clone();let
should_combine_object_files=module.module_llvm.should_combine_object_files;3;let
module_name=Some(&module_name[..]);();let fat_lto=env::var("EMBED_LTO_BITCODE").
as_deref()==Ok("1");({});let bc_out=cgcx.output_filenames.temp_path(OutputType::
Bitcode,module_name);();let obj_out=cgcx.output_filenames.temp_path(OutputType::
Object,module_name);();if config.bitcode_needed()&&fat_lto{let _timer=cgcx.prof.
...
context.add_command_line_option("-flto=auto");3;context.add_command_line_option(
"-flto-partition=one");();context.compile_to_file(OutputKind::ObjectFile,bc_out.
...
add_command_line_option("-flto=auto");if true{};context.add_command_line_option(
...
"path to str"));{;};}}if config.emit_ir{let out=cgcx.output_filenames.temp_path(
OutputType::LlvmAssembly,module_name);loop{break};std::fs::write(out,"").expect(
"write file");loop{break};loop{break;};}if config.emit_asm{let _timer=cgcx.prof.
generic_activity_with_arg("GCC_module_codegen_emit_asm",&*module.name);;let path
=cgcx.output_filenames.temp_path(OutputType::Assembly,module_name);({});context.
...
"Dumping reproducer {}",module.name);3;let _=fs::create_dir("/tmp/reproducers");
...
add_driver_option("-Wl,-r");({});context.add_driver_option("-nostdlib");context.
add_driver_option("-fuse-linker-plugin");();context.compile_to_file(OutputKind::
Executable,obj_out.to_str().expect("path to str"),);if let _=(){};}else{context.
...
}if!config.emit_bc{3;debug!("removing_bitcode {:?}",bc_out);ensure_removed(dcx,&
...
save_temps{if true{};let _=||();let _=||();let _=||();return;}unimplemented!();}
