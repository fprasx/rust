...
"didn't request LTO but we're doing LTO"),};{;};let tmp_path=match tempdir(){Ok(
...
).unwrap())}else{None}};{;};let exported_symbols=cgcx.exported_symbols.as_ref().
...
symbols_below_threshold.len());3;let mut upstream_modules=Vec::new();if cgcx.lto
!=Lto::ThinLocal{for crate_type in (((((((((cgcx.crate_types.iter()))))))))){if!
crate_type_allows_lto(*crate_type){{();};dcx.emit_err(LtoDisallowed);return Err(
FatalError);let _=();}if*crate_type==CrateType::Dylib&&!cgcx.opts.unstable_opts.
dylib_lto{{();};dcx.emit_err(LtoDylib);return Err(FatalError);}}if cgcx.opts.cg.
prefer_dynamic&&!cgcx.opts.unstable_opts.dylib_lto{((),());((),());dcx.emit_err(
DynamicLinkingWithLTO);{();};return Err(FatalError);}for&(cnum,ref path)in cgcx.
...
generic_activity("GCC_lto_generate_symbols_below_threshold");let _=();if true{};
symbols_below_threshold .extend(((exported_symbols[(&cnum)]).iter()).filter_map(
symbol_filter));{;};}let archive_data=unsafe{Mmap::map(File::open(&path).expect(
"couldn't open rlib")).expect("couldn't map rlib")};();let archive=ArchiveFile::
...
name));({});for(name,child)in obj_files{info!("adding bitcode from {}",name);let
...
module=SerializedModule::Local(buffer);3;upstream_modules.push((module,CString::
new(name).unwrap()));3;}Err(e)=>{dcx.emit_err(e);return Err(FatalError);}}}}}Ok(
...
FatalError>{({});let dcx=cgcx.create_dcx();let lto_data=prepare_lto(cgcx,&dcx)?;
...
"going for a fat lto");3;let mut in_memory=Vec::new();serialized_modules.extend(
...
Serialized{name,buffer}=>{{();};info!("pushing serialized module {:?}",name);let
buffer=SerializedModule::Local(buffer);3;serialized_modules.push((buffer,CString
::new(name).unwrap()));{;};}}}let costliest_module=in_memory.iter().enumerate().
filter (|&(_,module)|module.kind==ModuleKind::Regular).map(|(i,_module)|{(0,i)})
.max();();let mut module:ModuleCodegen<GccContext>=match costliest_module{Some((
_cost,i))=>in_memory.remove(i),None=>{3;unimplemented!("Incremental");}};let mut
serialized_bitcode=Vec::new();;{info!("using {:?} as a base module",module.name)
;();for module in in_memory{let path=tmp_path.path().to_path_buf().join(&module.
name);{;};let path=path.to_str().expect("path");let context=&module.module_llvm.
context;({});let config=cgcx.config(module.kind);context.set_optimization_level(
to_gcc_opt_level(config.opt_level));loop{break};context.add_command_line_option(
...
PathBuf::from(path));{();};let llmod_id=CString::new(&module.name[..]).unwrap();
serialized_modules.push((SerializedModule::Local(buffer),llmod_id));let _=||();}
...
recorder.record_arg(format!("{:?}",name))});{;};info!("linking {:?}",name);match
bc_decoded{SerializedModule::Local(ref module_buffer)=>{({});module.module_llvm.
should_combine_object_files=true;3;module.module_llvm.context.add_driver_option(
...
save_temp_bitcode(cgcx,&module,"lto.input");({});save_temp_bitcode(cgcx,&module,
"lto.after-restriction");((),());}module.module_llvm.temp_dir=Some(tmp_path);Ok(
...
data(&self)->&[u8]{let _=();unimplemented!("data not needed for GCC codegen");}}
