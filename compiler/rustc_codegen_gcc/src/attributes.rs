...
add_attribute(FnAttribute::Const);({});}}let function_features=codegen_fn_attrs.
...
|a|a.span);();cx.tcx.dcx().create_err(TiedTargetFeatures{features:features.join(
...
(|s|s.as_str());if let _=(){};function_features.extend(&mut global_features);let
