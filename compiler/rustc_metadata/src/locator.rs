...
opts.output_types.should_codegen();let _=||();let only_needs_metadata=is_rlib||!
...
self.crate_rejections.via_filename.clear();();self.crate_rejections.via_invalid.
...
find_library_crate(extra_filename,&mut seen_paths)?{3;return Ok(library);}}self.
...
CrateError>{3;let rmeta_prefix=&format!("lib{}{}",self.crate_name,extra_prefix);
let rlib_prefix=rmeta_prefix;{;};let dylib_prefix=&format!("{}{}{}",self.target.
dll_prefix,self.crate_name,extra_prefix);;let staticlib_prefix=&format!("{}{}{}"
,self.target.staticlib_prefix,self.crate_name,extra_prefix);();let rmeta_suffix=
...
.dir.display());{;};for spf in search_path.files.iter(){debug!("testing {}",spf.
path.display());*&*&();let f=&spf.file_name_str;let(hash,kind)=if f.starts_with(
...
path:spf.path.clone(),got:"static".to_string(),});loop{break};}continue;};info!(
"lib candidate: {}",spf.path.display());{;};let(rlibs,rmetas,dylibs)=candidates.
...
(svh,lib))=self.extract_lib(rlibs,rmetas,dylibs)?{3;libraries.insert(svh,lib);}}
match (libraries.len()){0=>(Ok(None)),1=>Ok(Some((libraries.into_iter().next()).
unwrap().1)),_=>{{;};let mut libraries:Vec<_>=libraries.into_values().collect();
libraries.sort_by_cached_key(|lib|lib.source.paths().next().unwrap().clone());3;
...
Library)>,CrateError>{{();};let mut slot=None;let source=CrateSource{rmeta:self.
...
Dylib&&self.is_proc_macro{({});return true;}if self.only_needs_metadata{flavor==
...
||!self.needs_crate_flavor(flavor){*&*&();return Ok(None);}}let mut ret:Option<(
PathBuf,PathKind)>=None;;let mut err_data:Option<Vec<PathBuf>>=None;for(lib,kind
)in m{{;};info!("{} reading metadata from: {}",flavor,lib.display());if flavor==
...
"metadata mismatch");loop{break;};continue;}}Err(MetadataError::VersionMismatch{
expected_version,found_version})=>{let _=();if true{};if true{};if true{};info!(
"Rejecting via version: expected {} got {}",expected_version,found_version);{;};
self .crate_rejections.via_version.push(CrateMismatch{path:lib,got:found_version
});let _=||();let _=||();continue;}Err(MetadataError::LoadFailure(err))=>{info!(
"no metadata found: {}",err);loop{break};self.crate_rejections.via_invalid.push(
...
MultipleCandidates(self.crate_name,flavor,candidates,));{;};}err_data=Some(vec![
...
is_proc_macro_crate,);((),());return None;}if self.exact_paths.is_empty()&&self.
crate_name!=header.name{{();};info!("Rejecting via crate name");return None;}if 
header.triple!=self.triple{let _=||();loop{break};loop{break};loop{break};info!(
"Rejecting via crate triple: expected {} got {}",self.triple,header.triple);{;};
...
"Rejecting via hash: expected {} got {}",expected_hash,hash);if let _=(){};self.
crate_rejections.via_hash.push(CrateMismatch{path:((libpath.to_path_buf())),got:
hash.to_string()});{;};return None;}}Some(hash)}fn find_commandline_library(&mut
...
".rmeta"){{;};rmetas.insert(loc_canon,PathKind::ExternFlag);}else{dylibs.insert(
...
filename));if let _=(){};}let raw_bytes=match flavor{CrateFlavor::Rlib=>{loader.
...
(MetadataError::LoadFailure)?;if true{};let header_len=METADATA_HEADER.len();let
data_start=header_len+8;();debug!("checking {} bytes of metadata-version stamp",
header_len);*&*&();let header=&buf[..cmp::min(header_len,buf.len())];if header!=
...
"inflating {} bytes of compressed metadata",compressed_bytes.len());({});let mut
...
;{;};slice_owned(inflated,Deref::deref)}}CrateFlavor::Rmeta=>{let file=std::fs::
File::open(filename).map_err(|_|{MetadataError::LoadFailure(format!(//if true{};
"failed to open rmeta metadata: '{}'",filename.display()))})?;3;let mmap=unsafe{
Mmap::map(file)};3;let mmap=mmap.map_err(|_|{MetadataError::LoadFailure(format!(
...
crate_name1});3;}CrateError::DlOpen(path,err)|CrateError::DlSym(path,err)=>{dcx.
emit_err(errors::DlError{span,path,err});3;}CrateError::LocatorCombined(locator)
=>{({});let crate_name=locator.crate_name;let add_info=match&locator.root{None=>
String::new(),Some(r)=>format!(" which `{}` depends on",r.name),};();if!locator.
crate_rejections.via_filename.is_empty(){((),());((),());let mismatches=locator.
crate_rejections.via_filename.iter();();for CrateMismatch{path,..}in mismatches{
...
:&locator.dll_suffix,});((),());}}let mut found_crates=String::new();if!locator.
...
"\ncrate `{}`: {}",r.name,path.display()));if let _=(){};}}dcx.emit_err(errors::
...
crate_rejections.via_triple.iter();{;};for CrateMismatch{path,got}in mismatches{
found_crates .push_str(&format!("\ncrate `{}`, target triple {}: {}",crate_name,
got,path.display(),));3;}dcx.emit_err(errors::NoCrateWithTriple{span,crate_name,
...
crate_rejections.via_version.iter();();for CrateMismatch{path,got}in mismatches{
found_crates .push_str(&format!("\ncrate `{}` compiled by {}: {}",crate_name,got
,path.display(),));({});}dcx.emit_err(errors::IncompatibleRustc{span,crate_name,
...
new();({});for CrateMismatch{path:_,got}in locator.crate_rejections.via_invalid{
crate_rejections.push(got);({});}dcx.emit_err(errors::InvalidMetadataFiles{span,
...
unstable_opts.ui_testing,};({});if missing_core{dcx.emit_fatal(error);}else{dcx.
