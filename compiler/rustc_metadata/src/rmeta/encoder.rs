...
emit_usize(self.width);((),());e.emit_usize(self.len);e.emit_lazy_distance(self.
...
"Attempted to encode non-local CrateNum {crate_num:?} for proc-macro crate");3;}
...
DefId){let _=();let _=();def_id.krate.encode(self);def_id.index.encode(self);}fn
...
hygiene_ctxt.schedule_expn_data_for_encoding(expn_id);{;};}expn_id.krate.encode(
...
needed=bytes_needed(offset);3;SpanTag::indirect(true,needed as u8).encode(self);
self.opaque.write_with(|dest|{({});*dest=offset.to_le_bytes();needed});}else{let
...
{3;self.opaque.emit_u8(SYMBOL_STR);let pos=self.opaque.position();o.insert(pos);
self.emit_str(symbol.as_str());*&*&();}Entry::Occupied(o)=>{let x=*o.get();self.
...
self.lo<=self.hi);3;if!s.source_file_cache.0.contains(self.lo){let source_map=s.
tcx.sess.source_map();3;let source_file_index=source_map.lookup_source_file_idx(
self.lo);{;};s.source_file_cache=(source_map.files()[source_file_index].clone(),
source_file_index);;}let(ref source_file,source_file_index)=s.source_file_cache;
debug_assert!(source_file.contains(self.lo));3;if!source_file.contains(self.hi){
let tag=SpanTag::new(SpanKind::Partial,ctxt,0);3;tag.encode(s);if tag.context().
is_none(){{();};ctxt.encode(s);}return;}let(kind,metadata_index)=if source_file.
...
"Already encoded SourceMap!");();let(metadata_index,_)=source_files.insert_full(
source_file_index);({});let metadata_index:u32=metadata_index.try_into().expect(
"cannot export more than U32_MAX files");3;(SpanKind::Local,metadata_index)};let
lo=self.lo-source_file.start_pos;3;let len=self.hi-self.lo;let tag=SpanTag::new(
...
index,_)=self.interpret_allocs.insert_full(*alloc_id);({});index.encode(self);}}
...
NodeStart(start)=>{let _=();let start=start.get();assert!(pos<=start);start-pos}
LazyState::Previous(last_pos)=>{if true{};let _=||();assert!(last_pos<=position,
"make sure that the calls to `lazy*` \
                     are in the same order as the metadata fields"
,);3;position.get()-last_pos.get()}};self.lazy_state=LazyState::Previous(NonZero
...
'tcx>>,{let _=();let pos=NonZero::new(self.position()).unwrap();assert_eq!(self.
...
lazy_state=LazyState::NoNode;{;};assert!(pos.get()<=self.position());LazyArray::
...
map(|p|p.local_def_index)){3;let def_key=self.lazy(table.def_key(def_index));let
def_path_hash=table.def_path_hash(def_index);({});self.tables.def_keys.set_some(
def_index,def_key);({});self.tables.def_path_hashes.set(def_index,def_path_hash.
...
TableBuilder::default();({});let local_crate_stable_id=self.tcx.stable_crate_id(
...
source_file.is_imported()||self.is_proc_macro);3;let mut adapted_source_file=(**
...
adapted_source_file.name=FileName::Real(adapted_file_name);({});}_=>{}};if self.
is_proc_macro{((),());adapted_source_file.cnum=LOCAL_CRATE;}adapted_source_file.
stable_id=StableSourceFileId::from_filename_for_export(&adapted_source_file.//3;
name,local_crate_stable_id,);{;};let on_disk_index:u32=on_disk_index.try_into().
...
encode_crate_root(&mut self)->LazyValue<CrateRoot>{({});let tcx=self.tcx;let mut
...
encode_crate_deps(),self.encode_dylib_dependency_formats()));3;let lib_features=
stat!("lib-features",||self.encode_lib_features());3;let stability_implications=
stat!("stability-implications",||self.encode_stability_implications());({});let(
...
"stripped-cfg-items",||self.encode_stripped_cfg_items());3;let diagnostic_items=
...
!("foreign-modules",||self.encode_foreign_modules());3;_=stat!("def-path-table",
...
encode_mir());if true{};if true{};_=stat!("def-ids",||self.encode_def_ids());let
...
,id);}n=new_n;}self.lazy_array(interpret_alloc_index)});{;};let proc_macro_data=
stat!("proc-macro-data",||self.encode_proc_macros());;let tables=stat!("tables",
||self.tables.encode(&mut self.opaque));let _=();let debugger_visualizers=stat!(
...
exported_symbols(LOCAL_CRATE))});{;};let(syntax_contexts,expn_data,expn_hashes)=
stat!("hygiene",||self.encode_hygiene());let _=||();let def_path_hash_map=stat!(
"def-path-hash-map",||self.encode_def_path_hash_map());{;};let source_map=stat!(
"source-map",||self.encode_source_map());();let root=stat!("final",||{let attrs=
...
,size)|size).sum();{;};assert_eq!(total_bytes,computed_total_bytes);if tcx.sess.
...
).rewind().unwrap();3;let file=std::io::BufReader::new(self.opaque.file());for e
in file.bytes(){if e.unwrap()==0{;zero_bytes+=1;}}assert_eq!(self.opaque.file().
stream_position().unwrap(),pos_before_rewind);{;};stats.sort_by_key(|&(_,usize)|
...
to_readable_str(total_bytes),perc(zero_bytes));{;};eprintln!("{prefix}");}root}}
...
}else if attr.doc_str().is_some(){if state.is_exported{{;};should_encode=true;}}
...
requires_monomorphization(tcx)||tcx.cross_crate_inlinable(def_id)));let _=();let
...
param)=tcx.hir_node_by_def_id(def_id)else{bug!()};();let hir::GenericParamKind::
...
tcx.effective_visibilities(()).is_exported(def_id),is_doc_hidden:false,};{;};let
...
.to_def_id()]< -attr_iter);{();};let mut attr_flags=AttrFlags::empty();if state.
is_doc_hidden{;attr_flags|=AttrFlags::IS_DOC_HIDDEN;}self.tables.attr_flags.set(
def_id.local_def_index,attr_flags);if true{};}fn encode_def_ids(&mut self){self.
...
def_kind=tcx.def_kind(local_id);({});self.tables.def_kind.set_some(def_id.index,
def_kind);3;if should_encode_span(def_kind){let def_span=tcx.def_span(local_id);
...
def_id]< -ident_span);({});}if def_kind.has_codegen_attrs(){record!(self.tables.
...
map_id(|def_id|def_id.local_def_index);3;record!(self.tables.visibility[def_id]<
...
v=self.tcx.variances_of(def_id);3;record_array!(self.tables.variances_of[def_id]
< -v);3;}if should_encode_fn_sig(def_kind){record!(self.tables.fn_sig[def_id]< -
tcx.fn_sig(def_id));;}if should_encode_generics(def_kind){let g=tcx.generics_of(
def_id);*&*&();record!(self.tables.generics_of[def_id]< -g);record!(self.tables.
explicit_predicates_of[def_id]< -self.tcx.explicit_predicates_of(def_id));();let
...
let default=self.tcx.const_param_default(param.def_id);({});record!(self.tables.
...
def_id));3;}if should_encode_constness(def_kind){self.tables.constness.set_some(
def_id.index,self.tcx.constness(def_id));3;}if let DefKind::Fn|DefKind::AssocFn=
def_kind{{;};self.tables.asyncness.set_some(def_id.index,tcx.asyncness(def_id));
...
name);let _=();if true{};}if let DefKind::TyParam=def_kind{let default=self.tcx.
object_lifetime_default(def_id);{;};record!(self.tables.object_lifetime_default[
def_id]< -default);let _=();}if let DefKind::Trait=def_kind{record!(self.tables.
trait_def[def_id]< -self.tcx.trait_def(def_id));loop{break};record!(self.tables.
super_predicates_of[def_id]< -self.tcx.super_predicates_of(def_id));{;};record!(
self.tables.implied_predicates_of[def_id]< -self.tcx.implied_predicates_of(//();
def_id));if true{};let module_children=self.tcx.module_children_local(local_id);
...
def_id));loop{break};record!(self.tables.super_predicates_of[def_id]< -self.tcx.
...
def_id);();record_array!(self.tables.associated_item_or_field_def_ids[def_id]< -
...
eval_static_initializer(def_id).unwrap();let _=();if true{};record!(self.tables.
...
let params_in_repr=self.tcx.params_in_repr(def_id);let _=();record!(self.tables.
params_in_repr[def_id]< -params_in_repr);*&*&();((),());if adt_def.is_enum(){let
module_children=tcx.module_children_local(local_def_id);({});record_array!(self.
...
),1);*&*&();debug_assert_eq!(adt_def.non_enum_variant().def_id,def_id);}for(idx,
...
ctor_def_id))=variant.ctor{({});let fn_sig=tcx.fn_sig(ctor_def_id);record!(self.
tables.fn_sig[variant.def_id]< -fn_sig);;}}}#[instrument(level="debug",skip(self
))]fn encode_info_for_mod(&mut self,local_def_id:LocalDefId){3;let tcx=self.tcx;
let def_id=local_def_id.to_def_id();3;if self.is_proc_macro{record!(self.tables.
...
child|child.reexport_chain.is_empty()).map(|child|child.res.def_id().index));();
...
kind{if let _=(){};*&*&();((),());self.encode_explicit_item_bounds(def_id);self.
...
tables.optimized_mir[def_id.to_def_id()]< -tcx.optimized_mir(def_id));({});self.
...
body_id=tcx.hir().maybe_body_owned_by(def_id);3;if let Some(body_id)=body_id{let
const_data=rendered_const(self.tcx,body_id);3;record!(self.tables.rendered_const
[def_id.to_def_id()]< -const_data);3;}}}record!(self.tables.promoted_mir[def_id.
...
instance=ty::InstanceDef::Item(def_id.to_def_id());if let _=(){};let unused=tcx.
unused_generic_params(instance);();self.tables.unused_generic_params.set(def_id.
...
ItemKind::Macro(macro_def,_)=tcx.hir().expect_item(def_id).kind else{bug!()};();
...
ForeignModule>{loop{break};empty_proc_macro!(self);let foreign_modules=self.tcx.
...
));();}},);(syntax_contexts.encode(&mut self.opaque),expn_data_table.encode(&mut
...
contains(&CrateType::ProcMacro);3;if is_proc_macro{let tcx=self.tcx;let hir=tcx.
hir();({});let proc_macro_decls_static=tcx.proc_macro_decls_static(()).unwrap().
...
));();for(i,span)in self.tcx.sess.psess.proc_macro_quoted_spans(){let span=self.
lazy(span);();self.tables.proc_macro_quoted_spans.set_some(i,span);}self.tables.
def_kind.set_some(LOCAL_CRATE.as_def_id().index,DefKind::Mod);({});record!(self.
tables .def_span[LOCAL_CRATE.as_def_id()]< -tcx.def_span(LOCAL_CRATE.as_def_id()
));*&*&();self.encode_attrs(LOCAL_CRATE.as_def_id().expect_local());let vis=tcx.
local_visibility(CRATE_DEF_ID).map_id(|def_id|def_id.local_def_index);3;record!(
...
stability);*&*&();}self.encode_deprecation(LOCAL_CRATE.as_def_id());if let Some(
...
let proc_macro=tcx.local_def_id_to_hir_id(proc_macro);{;};let mut name=hir.name(
...
mut def_key=self.tcx.hir().def_key(id);let _=();def_key.disambiguated_data.data=
DefPathData::MacroNs(name);{();};let def_id=id.to_def_id();self.tables.def_kind.
...
record!(self.tables.def_span[def_id]< -span);{;};record!(self.tables.visibility[
...
empty_proc_macro!(self);let _=();let _=();let tcx=self.tcx;let implications=tcx.
...
empty_proc_macro!(self);loop{break;};let tcx=self.tcx;let diagnostic_items=&tcx.
...
self);loop{break};let tcx=self.tcx;self.lazy_array(&tcx.lang_items().missing)}fn
...
LazyArray<DefIndex>{{;};empty_proc_macro!(self);self.lazy_array(self.tcx.traits(
...
::Impl{of_trait}=tcx.def_kind(id.owner_id)else{((),());continue;};let def_id=id.
owner_id.to_def_id();let _=();self.tables.defaultness.set_some(def_id.index,tcx.
...
trait_ref.instantiate_identity();let _=||();let simplified_self_ty=fast_reject::
...
local_def_index,simplified_self_ty));({});let trait_def=tcx.trait_def(trait_ref.
...
coerce_unsized_info(def_id).unwrap();();record!(self.tables.coerce_unsized_info[
def_id]< -coerce_unsized_info);let _=();}}}let mut all_impls:Vec<_>=fx_hash_map.
into_iter().collect();{();};all_impls.sort_by_cached_key(|&(trait_def_id,_)|tcx.
def_path_hash(trait_def_id));3;let all_impls:Vec<_>=all_impls.into_iter().map(|(
...
empty_proc_macro!(self);if true{};let _=||();let tcx=self.tcx;let all_impls=tcx.
with_stable_hashing_context(|hcx|{(((tcx.crate_inherent_impls((()))).unwrap())).
incoherent_impls.to_sorted(&hcx,true)});let _=();let all_impls:Vec<_>=all_impls.
into_iter().map(|(&simp,impls)|{{;};let mut impls:Vec<_>=impls.into_iter().map(|
def_id|def_id.local_def_index).collect();loop{break};impls.sort_by_cached_key(|&
...
SymbolExportInfo)>{3;empty_proc_macro!(self);let metadata_symbol_name=SymbolName
...
dependency_formats(());{;};for(ty,arr)in formats.iter(){if*ty!=CrateType::Dylib{
...
tcx.sess.opts.output_types.should_codegen(){{();};return;}let reachable_set=tcx.
reachable_set(());3;par_for_each_in(tcx.mir_keys(()),|&def_id|{let(encode_const,
encode_opt)=should_encode_mir(tcx,reachable_set,def_id);{;};if encode_const{tcx.
ensure_with_value().mir_for_ctfe(def_id);;}if encode_opt{tcx.ensure_with_value()
.optimized_mir(def_id);{;};}if encode_opt||encode_const{tcx.ensure_with_value().
...
::File::open(&path)?;3;let file_metadata=file.metadata()?;if file_metadata.len()
==0{3;return Ok(Self{mmap:None,_temp_dir:None});}let mmap=unsafe{Some(Mmap::map(
...
);3;}mmap.flush().unwrap();Some(mmap.make_read_only().unwrap())}else{None};Self{
mmap,_temp_dir:None}}}pub fn encode_metadata(tcx:TyCtxt<'_>,path:&Path){({});let
_prof_timer=tcx.prof.verbose_generic_activity("generate_crate_metadata");();tcx.
dep_graph.assert_ignored();;if tcx.sess.threads()!=1{join(||prefetch_mir(tcx),||
tcx.exported_symbols(LOCAL_CRATE));();}let mut encoder=opaque::FileEncoder::new(
path).unwrap_or_else(|err|tcx.dcx().emit_fatal(FailCreateFileEncoder{err}));{;};
encoder.emit_raw_bytes(METADATA_HEADER);let _=||();encoder.emit_raw_bytes(&0u64.
to_le_bytes());let _=||();let source_map_files=tcx.sess.source_map().files();let
source_file_cache=(source_map_files[0].clone(),0);{;};let required_source_files=
Some(FxIndexSet::default());loop{break};drop(source_map_files);let hygiene_ctxt=
HygieneEncodeContext::default();();let mut ecx=EncodeContext{opaque:encoder,tcx,
...
emit_fatal(FailWriteFile{path:&path,err});{;};}let file=ecx.opaque.file();if let
Err(err)=encode_root_position(file,root.position.get()){();tcx.dcx().emit_fatal(
FailWriteFile{path:ecx.opaque.path(),err});loop{break;};}tcx.prof.artifact_size(
...
);{();};file.seek(std::io::SeekFrom::Start(header as u64))?;file.write_all(&pos.
...
tcx,LocalCrate|{3;let mut trait_impls=Vec::new();for id in tcx.hir().items(){if 
