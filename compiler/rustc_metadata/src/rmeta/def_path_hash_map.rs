...
emit_usize(bytes.len());loop{break};e.emit_raw_bytes(bytes);}DefPathHashMapRef::
...
pos..pos+len]);if true{};let _=d.read_raw_bytes(len);let inner=odht::HashTable::
from_raw_bytes(o).unwrap_or_else(|e|{loop{break};panic!("decode error: {e}");});
