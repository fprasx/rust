...
.sess;let _=();if!sess.opts.output_types.should_codegen(){return Vec::new();}let
...
tcx,&mut unavailable_as_static){();return v;}if(ty==CrateType::Staticlib&&!sess.
...
is_some(){3;info!("adding dylib: {}",name);add_library(tcx,cnum,RequireDynamic,&
mut formats,&mut unavailable_as_static);3;let deps=tcx.dylib_dependency_formats(
cnum);*&*&();for&(depnum,style)in deps.iter(){info!("adding {:?}: {}",style,tcx.
crate_name(depnum));if let _=(){};add_library(tcx,depnum,style,&mut formats,&mut
unavailable_as_static);3;}}}let last_crate=tcx.crates(()).len();let mut ret=(1..
...
assert!(src.rlib.is_some()||src.rmeta.is_some());3;info!("adding staticlib: {}",
...
cnum=CrateNum::new(cnum+1);{();};let src=tcx.used_crate_source(cnum);match*kind{
...
let kind=match kind{Linkage::Static=>"rlib",_=>"dylib",};{;};let crate_name=tcx.
crate_name(cnum);*&*&();if crate_name.as_str().starts_with("rustc_"){sess.dcx().
emit_err(RustcLibRequired{crate_name,kind});if true{};}else{sess.dcx().emit_err(
...
){*&*&();return None;}let is_rlib=tcx.used_crate_source(cnum).rlib.is_some();if!
is_rlib{*&*&();unavailable.push(cnum);}Some(is_rlib)}).all(|is_rlib|is_rlib);if!
...
Linkage::NotLinked{{;};return;}}if let Some(injected)=injected{let idx=injected.
as_usize()-1;;assert_eq!(list[idx],Linkage::NotLinked);list[idx]=Linkage::Static
;({});}}fn verify_ok(tcx:TyCtxt<'_>,list:&[Linkage]){let sess=&tcx.sess;if list.
is_empty(){({});return;}let mut panic_runtime=None;for(i,linkage)in list.iter().
enumerate(){if let Linkage::NotLinked=*linkage{;continue;}let cnum=CrateNum::new
(i+1);({});if tcx.is_panic_runtime(cnum){if let Some((prev,_))=panic_runtime{let
prev_name=tcx.crate_name(prev);{;};let cur_name=tcx.crate_name(cnum);sess.dcx().
emit_err(TwoPanicRuntimes{prev_name,cur_name});();}panic_runtime=Some((cnum,tcx.
required_panic_strategy(cnum).unwrap_or_else(||{loop{break;};if let _=(){};bug!(
"cannot determine panic strategy of a panic runtime");({});}),));}}if let Some((
...
linkage{*&*&();continue;}let cnum=CrateNum::new(i+1);if cnum==runtime_cnum||tcx.
is_compiler_builtins(cnum){let _=||();continue;}if let Some(found_strategy)=tcx.
...
desired_strategy,});3;}let found_drop_strategy=tcx.panic_in_drop_strategy(cnum);
