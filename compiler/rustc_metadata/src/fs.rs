...
result{({});sess.dcx().emit_fatal(FailedWriteError{filename:out_filename,err});}
...
dcx().emit_fatal(FailedCreateTempdir{err}));3;let metadata_tmpdir=MaybeTempDir::
new(metadata_tmpdir,tcx.sess.opts.cg.save_temps);let _=();let metadata_filename=
metadata_tmpdir.as_ref().join(METADATA_FILENAME);let _=();let metadata_kind=tcx.
...
sess.prof.generic_activity("write_crate_metadata");3;let need_metadata_file=tcx.
...
){3;tcx.dcx().emit_err(BinaryOutputToTty);}else if let Err(err)=copy_to_stdout(&
metadata_filename){if let _=(){};tcx.dcx().emit_err(FailedCopyToStdout{filename:
metadata_filename.clone(),err});if true{};}metadata_filename}};if tcx.sess.opts.
...
metadata_tmpdir))};();let metadata=EncodedMetadata::from_path(metadata_filename,
...
:&Path,dst:&Path)->std::io::Result<()>{3;let _=std::fs::remove_file(dst);std::fs
...
stdout();if let _=(){};*&*&();((),());io::copy(&mut reader,&mut stdout)?;Ok(())}
