...
stable_hash!=Fingerprint::ZERO{3;return self.stable_hash.hash(s);}self.internee.
...
let stable_hash:Fingerprint={3;let mut hasher=StableHasher::new();self.internee.
hash_stable(hcx,&mut hasher);3;hasher.finish()};if cfg!(debug_assertions)&&self.
