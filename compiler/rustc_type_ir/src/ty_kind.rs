...
"{d:?}")?;3;let mut s=s.into_iter();let first=s.next();match first{Some(first)=>
write!(f,"<{:?}",first)?,None=>return Ok(()),};3;for arg in s{write!(f,", {:?}",
...
count+=1;3;}if count==1{write!(f,",")?;}write!(f,")")}Alias(i,a)=>f.debug_tuple(
...
CTX,hasher:&mut StableHasher){{();};use InferTy::*;std::mem::discriminant(self).
...
<'_>)->fmt::Result{3;use InferTy::*;match*self{TyVar(_)=>write!(f,"_"),IntVar(_)
