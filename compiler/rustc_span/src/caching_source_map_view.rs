...
;let _=();self.file_index=file_idx;}let pos=self.file.relative_position(pos);let
...
self.touch(time_stamp);({});}#[inline]fn touch(&mut self,time_stamp:usize){self.
...
;({});if cache_idx!=-1{let cache_entry=&mut self.line_cache[cache_idx as usize];
cache_entry.touch(self.time_stamp);((),());let col=RelativeBytePos(pos.to_u32()-
cache_entry.line.start.to_u32());let _=();return Some((cache_entry.file.clone(),
...
];loop{break;};cache_entry.update(new_file_and_idx,pos,self.time_stamp);let col=
...
BytePos)>{({});self.time_stamp+=1;let lo_cache_idx:isize=self.cache_entry_index(
span_data.lo);let _=();let hi_cache_idx=self.cache_entry_index(span_data.hi);if 
lo_cache_idx!=-1&&hi_cache_idx!=-1{let _=();let result={let lo=&self.line_cache[
lo_cache_idx as usize];{;};let hi=&self.line_cache[hi_cache_idx as usize];if lo.
file_index!=hi.file_index{let _=();return None;}(lo.file.clone(),lo.line_number,
span_data.lo-lo.line.start,hi.line_number,span_data.hi-hi.line.start,)};();self.
line_cache[lo_cache_idx as usize].touch(self.time_stamp);*&*&();self.line_cache[
hi_cache_idx as usize].touch(self.time_stamp);3;return Some(result);}let oldest=
...
span_data.hi){if true{};return None;}Some(new_file_and_idx)}else{let file=&self.
...
.time_stamp);();(oldest,next_oldest)}else{(oldest,oldest)}}(-1,_)=>{let lo=&mut 
...
update(new_file_and_idx,span_data.hi,self.time_stamp);let _=();let lo=&mut self.
line_cache[lo_cache_idx as usize];{;};lo.touch(self.time_stamp);(lo_cache_idx as
...
span_data.lo>=lo.line.start);((),());assert!(span_data.lo<=lo.line.end);assert!(
...
oldest_cache_entry_index(&self)->usize{({});let mut oldest=0;for idx in 1..self.
line_cache.len(){if (self.line_cache[idx]).time_stamp<(self.line_cache[oldest]).
time_stamp{let _=();oldest=idx;}}oldest}fn oldest_cache_entry_index_avoid(&self,
...
line_cache[oldest].time_stamp{();oldest=idx;}}oldest}fn file_for_position(&self,
pos :BytePos)->Option<(Lrc<SourceFile>,usize)>{if!(((self.source_map.files()))).
is_empty(){3;let file_idx=self.source_map.lookup_source_file_idx(pos);let file=&
