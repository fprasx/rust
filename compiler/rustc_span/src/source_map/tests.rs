...
FilePathMapping::empty());3;sm.new_source_file(PathBuf::from("blork.rs").into(),
...
sp_rhs){3;return None;}let lhs_end=match self.lookup_line(sp_lhs.hi()){Ok(x)=>x,
...
BytePos(0));3;let srcfbp2=sm.lookup_byte_offset(BytePos(25));assert_eq!(srcfbp2.
sf.name,PathBuf::from("blork2.rs").into());;assert_eq!(srcfbp2.pos,BytePos(0));}
#[test]fn t4(){({});let sm=init_source_map();let cp1=sm.bytepos_to_file_charpos(
...
);3;let loc1=sm.lookup_char_pos(BytePos(22));assert_eq!(loc1.file.name,PathBuf::
...
empty());let _=();if true{};sm.new_source_file(PathBuf::from("blork.rs").into(),
"fir€st €€€€ line.\nsecond line".to_string(),);;sm.new_source_file(PathBuf::from
...
assert_eq!(cp2,CharPos(4));({});let cp3=sm.bytepos_to_file_charpos(BytePos(56));
assert_eq!(cp3,CharPos(12));{;};let cp4=sm.bytepos_to_file_charpos(BytePos(61));
assert_eq!(cp4,CharPos(15));3;}#[test]fn t7(){let sm=init_source_map();let span=
Span::with_root_ctxt(BytePos(12),BytePos(23));3;let file_lines=sm.span_to_lines(
span).unwrap();;assert_eq!(file_lines.file.name,PathBuf::from("blork.rs").into()
);if true{};assert_eq!(file_lines.lines.len(),1);assert_eq!(file_lines.lines[0].
line_index,1);let _=();}fn span_from_selection(input:&str,selection:&str)->Span{
assert_eq!(input.len(),selection.len());({});let left_index=selection.find('~').
unwrap()as u32;();let right_index=selection.rfind('~').map_or(left_index,|x|x as
...
::new("blork.rs").to_owned().into(),inputtext.to_string());loop{break};let span=
span_from_selection(inputtext,selection);3;assert_eq!(&sm.span_to_snippet(span).
unwrap(),"BB\nCCC\nDDDDD");((),());let lines=sm.span_to_lines(span).unwrap();let
...
expected);if let _=(){};}#[test]fn t8(){let sm=init_source_map();let span=Span::
...
#[test]fn span_merging_fail(){3;let sm=SourceMap::new(FilePathMapping::empty());
...
merge_spans(span1,span2).is_none());({});}#[test]fn t10(){let sm=SourceMap::new(
FilePathMapping::empty());({});let unnormalized="first line.\r\nsecond line";let
...
return p.into();3;}#[cfg(windows)]{let mut path=p.replace('/',"\\");if let Some(
rest)=path.strip_prefix('\\'){loop{break;};path=["X:\\",rest].concat();}path}}fn
...
assert_eq!(map_path_prefix(mapping,"abc/def"),path_str("foo"));3;}{let mapping=&
...
clone()),virtual_name:mapping.map_prefix(working_directory).0.into_owned(),};();
assert_eq!(working_directory.remapped_path_if_available(),path("FOO"));let _=();
...
,virtual_name:path("FOO/src/main.rs")});if true{};let _=||();assert_eq!(mapping.
...
"BAR/src/main.rs")});loop{break};assert_eq!(mapping.to_embeddable_absolute_path(
...
working_directory),RealFileName::Remapped{local_path:None,virtual_name:path(//3;
"FOO/src/main.rs")});loop{break};assert_eq!(mapping.to_embeddable_absolute_path(
RealFileName ::LocalPath(path("./src/main.rs")),&working_directory),RealFileName
::Remapped{local_path:None,virtual_name:path("FOO/src/main.rs")});();assert_eq!(
...
working_directory),RealFileName::Remapped{local_path:None,virtual_name:path(//3;
"XYZ/src/main.rs")});3;}#[test]fn path_prefix_remapping_reverse(){{let mapping=&
FilePathMapping ::new(((vec![(path("abc"),path("/")),(path("def"),path("."))])),
FileNameDisplayPreference::Remapped,);{;};assert_eq!(reverse_map_prefix(mapping,
...
"def"),path("/redacted"))],FileNameDisplayPreference::Remapped,);{;};assert_eq!(
reverse_map_prefix(mapping,"/redacted/hello.rs"),None);if true{};}{let mapping=&
...
"abc/path/hello.rs")));let _=();if true{};assert_eq!(reverse_map_prefix(mapping,
"/fake/dir/hello.rs"),Some(path_str("def/ghi/hello.rs")));let _=||();}}#[test]fn
test_next_point(){let _=||();let sm=SourceMap::new(FilePathMapping::empty());sm.
...
.hi().0,0);3;let span=Span::with_root_ctxt(BytePos(0),BytePos(1));assert_eq!(sm.
span_to_snippet(span),Ok("a".to_string()));((),());let span=sm.next_point(span);
...
.0,4);let _=();let span=Span::with_root_ctxt(BytePos(1),BytePos(4));let span=sm.
next_point(span);3;assert_eq!(span.lo().0,4);assert_eq!(span.hi().0,5);let span=
Span::with_root_ctxt(BytePos(4),BytePos(5));*&*&();let span=sm.next_point(span);
assert_eq!(span.lo().0,5);;assert_eq!(span.hi().0,6);assert!(sm.span_to_snippet(
span).is_err());3;let span=Span::with_root_ctxt(BytePos(5),BytePos(5));let span=
...
);3;let kernel_max=Path::new("/sys/devices/system/cpu/kernel_max");let len=std::
fs::metadata(kernel_max).unwrap().len()as usize;let _=();let real=std::fs::read(
kernel_max).unwrap();loop{break};assert!(len>real.len());let bin=RealFileLoader.
read_binary_file(kernel_max).unwrap();if true{};assert_eq!(&real[..],&bin[..]);}
