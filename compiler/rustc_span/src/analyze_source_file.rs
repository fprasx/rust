...
NonNarrowChar>){((),());let mut lines=vec![RelativeBytePos::from_u32(0)];let mut
multi_byte_chars=vec![];loop{break};loop{break};let mut non_narrow_chars=vec![];
...
source_file_end=RelativeBytePos::from_usize(src.len());3;assert!(source_file_end
>=last_line_start);{;};if last_line_start==source_file_end{lines.pop();}}(lines,
...
len()>=scan_len);3;let mut i=0;let src_bytes=src.as_bytes();while i<scan_len{let
...
RelativeBytePos(1));3;}b'\t'=>{non_narrow_chars.push(NonNarrowChar::Tab(pos));}_
=>{();non_narrow_chars.push(NonNarrowChar::ZeroWidth(pos));}}}else if byte>=127{
let c=src[i..].chars().next().unwrap();let _=||();char_len=c.len_utf8();let pos=
RelativeBytePos::from_usize(i)+output_offset;({});if char_len>1{assert!((2..=4).
contains(&char_len));let _=||();let mbc=MultiByteChar{pos,bytes:char_len as u8};
...
}}loop{break;};if let _=(){};loop{break;};if let _=(){};i+=char_len;}i-scan_len}
