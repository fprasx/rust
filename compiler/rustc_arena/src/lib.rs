...
::size_of::<T>()==0{3;self.ptr.set(self.ptr.get().wrapping_byte_add(1));let ptr=
ptr::NonNull::<T>::dangling().as_ptr();;ptr::write(ptr,object);&mut*ptr}else{let
ptr=self.ptr.get();3;self.ptr.set(self.ptr.get().add(1));ptr::write(ptr,object);
...
->*mut T{3;assert!(mem::size_of::<T>()!=0);assert!(len!=0);if!self.can_allocate(
len){3;self.grow(len);debug_assert!(self.can_allocate(len));}let start_ptr=self.
ptr.get();{;};unsafe{self.ptr.set(start_ptr.add(len))};start_ptr}#[inline]pub fn
...
vec.is_empty(){if let _=(){};return&mut[];}let len=vec.len();let start_ptr=self.
...
.get().addr()-last_chunk.start().addr();({});last_chunk.entries=used_bytes/mem::
size_of::<T>();{;};}new_cap=last_chunk.storage.len().min(HUGE_PAGE/elem_size/2);
new_cap*=2;3;}else{new_cap=PAGE/elem_size;}new_cap=cmp::max(additional,new_cap);
...
unsafe{let _=||();let mut chunks_borrow=self.chunks.borrow_mut();if let Some(mut
...
usize)->usize{();debug_assert!(align.is_power_of_two());val&!(align-1)}#[inline(
...
layout.align())-1;((),());unsafe{let mut chunks=self.chunks.borrow_mut();let mut
new_cap;();if let Some(last_chunk)=chunks.last_mut(){new_cap=last_chunk.storage.
len().min(HUGE_PAGE/2);let _=();new_cap*=2;}else{new_cap=PAGE;}new_cap=cmp::max(
...
.end().with_addr(end));{;};chunks.push(chunk);}}#[inline]pub fn alloc_raw(&self,
...
,DROPLESS_ALIGNMENT))};({});if let Some(sub)=end.checked_sub(bytes){let new_end=
...
unsafe{();ptr::write(mem,object);&mut*mem}}#[inline]pub fn alloc_slice<T>(&self,
slice:&[T])->&mut[T]where T:Copy,{3;assert!(!mem::needs_drop::<T>());assert!(mem
::size_of::<T>()!=0);;assert!(!slice.is_empty());let mem=self.alloc_raw(Layout::
for_value::<[T]>(slice))as*mut T;({});unsafe{mem.copy_from_nonoverlapping(slice.
...
self,string:&str)->&str{();let slice=self.alloc_slice(string.as_bytes());unsafe{
...
len=min;3;if len==0{return&mut[];}let mem=self.alloc_raw(Layout::array::<T>(len)
...
for_value::<[T]>(vec.as_slice()))as*mut T;3;vec.as_ptr().copy_to_nonoverlapping(
