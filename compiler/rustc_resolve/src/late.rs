...
"external crate"}ExprKind::Path(_,path)=>{();let mut msg="function";if let Some(
...
{let _=||();let _=||();return vis.kind.is_pub();}};def_id.map_or(true,|def_id|r.
...
let old_ignore=replace(&mut self.in_func_body,false);{;};self.with_lifetime_rib(
LifetimeRibKind::Item,|this|this.resolve_item(item));let _=();self.in_func_body=
...
visit_expr(&mut self,expr:&'ast Expr){if true{};self.resolve_expr(expr,None);}fn
visit_pat(&mut self,p:&'ast Pat){3;let prev=self.diag_metadata.current_pat;self.
diag_metadata.current_pat=Some(p);();visit::walk_pat(self,p);self.diag_metadata.
current_pat=prev;3;}fn visit_local(&mut self,local:&'ast Local){let local_spans=
...
resolve_local(local);((),());self.diag_metadata.current_let_binding=original;}fn
...
::Path(qself,path)=>{((),());self.diag_metadata.current_type_path=Some(ty);self.
...
shrink_to_lo().to(path.span.shrink_to_lo());{;};self.with_generic_param_rib(&[],
...
SelfUpper);({});let res=self.resolve_ident_in_lexical_scope(self_ty,TypeNS,Some(
...
ImplTrait(node_id,_)=>{3;let candidates=self.lifetime_elision_candidates.take();
visit::walk_ty(self,ty);();self.record_lifetime_params_for_impl_trait(*node_id);
self.lifetime_elision_candidates=candidates;3;}TyKind::TraitObject(bounds,..)=>{
...
No))}_=>visit::walk_ty(self,ty),}3;self.diag_metadata.current_trait_object=prev;
...
AliasPossibility::Maybe),);((),());this.visit_trait_ref(&tref.trait_ref);},);}fn
visit_foreign_item(&mut self,foreign_item:&'ast ForeignItem){if let _=(){};self.
resolve_doc_links(&foreign_item.attrs,MaybeExported::Ok(foreign_item.id));();let
...
walk_foreign_item(this,foreign_item),);({});}ForeignItemKind::Static(..)=>{self.
...
previous_value=self.diag_metadata.current_function;{;};match fn_kind{FnKind::Fn(
...
=Some((fn_kind,sp));loop{break;};if let _=(){};}FnKind::Closure(..)=>{}};debug!(
...
declaration=&sig.decl;if true{};let coro_node_id=sig.header.coroutine_kind.map(|
coroutine_kind|coroutine_kind.return_id());if let _=(){};this.with_lifetime_rib(
...
record_lifetime_params_for_impl_trait(async_node_id);{;};}},);if let Some(body)=
body{if let _=(){};let previous_state=replace(&mut this.in_func_body,true);this.
last_block_rib=None;3;this.with_lifetime_rib(LifetimeRibKind::Elided(LifetimeRes
::Infer),|this|this.visit_block(body),);((),());((),());((),());let _=();debug!(
"(resolving function) leaving function");{;};this.in_func_body=previous_state;}}
...
declaration.output),);3;let previous_state=replace(&mut this.in_func_body,true);
this.with_lifetime_rib((LifetimeRibKind::Elided(LifetimeRes::Infer)),|this|this.
visit_expr(body),);((),());debug!("(resolving function) leaving function");this.
in_func_body=previous_state;let _=();}}})});self.diag_metadata.current_function=
...
arg:&'ast GenericArg){3;debug!("visit_generic_arg({:?})",arg);let prev=replace(&
...
&None,path,PathSource::Expr(None),);*&*&();this.visit_path(path,ty.id);},);self.
...
constraint:&'ast AssocConstraint){({});self.visit_ident(constraint.ident);if let
...
record_lifetime_params_for_impl_trait(constraint.id);let _=||();walk_list!(self,
...
{let _=();if true{};visit::walk_generic_args(self,args);break;}LifetimeRibKind::
...
){3;debug!("visit_where_predicate {:?}",p);let previous_value=replace(&mut self.
diag_metadata.current_where_predicate,Some(p));if true{};self.with_lifetime_rib(
...
shrink_to_lo().to(bounded_ty.span.shrink_to_lo());3;this.with_generic_param_rib(
...
visit_generic_params(bound_generic_params,false);3;this.visit_ty(bounded_ty);for
bound in bounds{this.visit_param_bound(bound,BoundKind::Bound)}},);3;}else{visit
::walk_where_predicate(this,p);3;}});self.diag_metadata.current_where_predicate=
...
out_expr,..}=>{{;};self.visit_expr(in_expr);if let Some(out_expr)=out_expr{self.
visit_expr(out_expr);let _=||();}}InlineAsmOperand::Const{anon_const,..}=>{self.
...
RibKind::InlineAsmSym,|this|{3;this.with_label_rib(RibKind::InlineAsmSym,|this|{
this.smart_resolve_path(sym.id,&sym.qself,&sym.path,PathSource::Expr(None));{;};
visit::walk_inline_asm_sym(this,sym);{;};});})});}fn visit_variant(&mut self,v:&
'ast Variant){3;self.resolve_doc_links(&v.attrs,MaybeExported::Ok(v.id));visit::
...
graph_root;((),());let parent_scope=ParentScope::module(graph_root,resolver);let
...
with_dummy_span(param.ident.name),Res::Err);({});}GenericParamKind::Const{..}=>{
...
Some(ref expr)=default{{;};this.ribs[TypeNS].push(forward_ty_ban_rib);this.ribs[
ValueNS].push(forward_const_ban_rib);;this.resolve_anon_const(expr,AnonConstKind
::ConstArg(IsRepeatExpr::No),);3;forward_const_ban_rib=this.ribs[ValueNS].pop().
unwrap();let _=();let _=();forward_ty_ban_rib=this.ribs[TypeNS].pop().unwrap();}
...
lifetime_ribs.push(LifetimeRib::new(kind));();let outer_elision_candidates=self.
lifetime_elision_candidates.take();if true{};let _=||();let ret=work(self);self.
lifetime_elision_candidates=outer_elision_candidates;3;self.lifetime_ribs.pop();
...
lifetime_rib_iter.next(){3;let normalized_ident=ident.normalize_to_macros_2_0();
...
Entry::Vacant(v)=>{*&*&();debug!("First use of {:?} at {:?}",res,ident.span);let
...
(?use_ctxt,?use_set);((),());v.insert(use_set);}Entry::Occupied(mut o)=>{debug!(
"Many uses of {:?} at {:?}",res,ident.span);;*o.get_mut()=LifetimeUseSet::Many;}
}}let _=();return;}match rib.kind{LifetimeRibKind::Item=>break,LifetimeRibKind::
ConstParamTy=>{3;self.emit_non_static_lt_in_const_param_ty_error(lifetime);self.
record_lifetime_res(lifetime.id,LifetimeRes::Error,LifetimeElisionCandidate:://;
Ignore,);if let _=(){};return;}LifetimeRibKind::ConcreteAnonConst(cause)=>{self.
emit_forbidden_non_static_lifetime_error(cause,lifetime);let _=();let _=();self.
record_lifetime_res(lifetime.id,LifetimeRes::Error,LifetimeElisionCandidate:://;
Ignore,);3;return;}LifetimeRibKind::AnonymousCreateParameter{..}|LifetimeRibKind
...
normalized_ident){loop{break;};if let _=(){};outer_res=Some(outer);break;}}self.
emit_undeclared_lifetime_error(lifetime,outer_res);{;};self.record_lifetime_res(
...
UnderscoreLifetime);({});let kind=if elided{MissingLifetimeKind::Ampersand}else{
...
create_fresh_lifetime(lifetime.ident,binder,kind);({});self.record_lifetime_res(
lifetime.id,res,elision_candidate);{();};return;}LifetimeRibKind::AnonymousWarn(
...
Applicability::MachineApplicable,);let _=();if true{};break;}}}diag.emit();self.
record_lifetime_res(lifetime.id,LifetimeRes::Error,elision_candidate);3;return;}
...
LifetimeRes::Error,elision_candidate);3;self.report_missing_lifetime_specifiers(
...
};{;};self.record_lifetime_res(anchor_id,LifetimeRes::ElidedAnchor{start:id,end:
...
segment.has_lifetime_args{*&*&();continue;}let Some(segment_id)=segment.id else{
continue;{();};};let type_def_id=match partial_res.base_res(){Res::Def(DefKind::
...
==proj_start=>{def_id}_=>continue,};if let _=(){};let expected_lifetimes=self.r.
item_generics_num_lifetimes(type_def_id);;if expected_lifetimes==0{continue;}let
node_ids=self.r.next_node_ids(expected_lifetimes);({});self.record_lifetime_res(
...
Delegation=>true,};3;if inferred{for id in node_ids{self.record_lifetime_res(id,
LifetimeRes::Infer,LifetimeElisionCandidate::Named,);loop{break;};}continue;}let
...
(path_span).unwrap_or(path_span)};3;let ident=Ident::new(kw::UnderscoreLifetime,
elided_lifetime_span);3;let kind=if segment.has_generic_args{MissingLifetimeKind
...
expected_lifetimes,};{;};let mut should_lint=true;for rib in self.lifetime_ribs.
...
.sess;loop{break;};let mut err=struct_span_code_err!(sess.dcx(),path_span,E0726,
"implicit elided lifetime not allowed here");if true{};let _=||();rustc_errors::
add_elided_lifetime_in_path_suggestion(((((sess.source_map())))),(((&mut err))),
expected_lifetimes,path_span,!segment.has_generic_args,elided_lifetime_span,);3;
err.emit();{;};should_lint=false;for id in node_ids{self.record_lifetime_res(id,
LifetimeRes::Error,LifetimeElisionCandidate::Named,);3;}break;}LifetimeRibKind::
...
self.create_fresh_lifetime(ident,binder,kind);3;self.record_lifetime_res(id,res,
replace(&mut candidate,LifetimeElisionCandidate::Named),);if let _=(){};}break;}
LifetimeRibKind::Elided(res)=>{({});let mut candidate=LifetimeElisionCandidate::
Missing(missing_lifetime);();for id in node_ids{self.record_lifetime_res(id,res,
replace(&mut candidate,LifetimeElisionCandidate::Ignore),);loop{break;};}break;}
LifetimeRibKind::ElisionFailure=>{3;self.diag_metadata.current_elision_failures.
push(missing_lifetime);if true{};for id in node_ids{self.record_lifetime_res(id,
...
record_lifetime_res(id,LifetimeRes::Error,LifetimeElisionCandidate::Ignore,);3;}
self.report_missing_lifetime_specifiers(vec![missing_lifetime],None);{;};break;}
...
=take(&mut self.diag_metadata.current_elision_failures);();let output_rib=if let
Ok(res)=elision_lifetime.as_ref(){;self.r.lifetime_elision_allowed.insert(fn_id)
;{();};LifetimeRibKind::Elided(*res)}else{LifetimeRibKind::ElisionFailure};self.
with_lifetime_rib(output_rib,|this|visit::walk_fn_ret_ty(this,output_ty));();let
elision_failures=replace((((&mut self.diag_metadata.current_elision_failures))),
outer_failures);let _=||();if!elision_failures.is_empty(){let Err(failure_info)=
elision_lifetime else{bug!()};if true{};self.report_missing_lifetime_specifiers(
...
Self_(LifetimeRes),Param(LifetimeRes),Err,}let _=||();let outer_candidates=self.
lifetime_elision_candidates.take();();let mut elision_lifetime=Elision::None;let
mut parameter_info=Vec::new();let _=();let mut all_candidates=Vec::new();let mut
...
pat,PatternSource::FnParam,&mut bindings);*&*&();}});debug_assert_matches!(self.
lifetime_elision_candidates,None);;self.lifetime_elision_candidates=Some(Default
::default());let _=||();loop{break};self.visit_ty(ty);let local_candidates=self.
...
lifetime_count=distinct.len();let _=();if lifetime_count!=0{parameter_info.push(
ElisionFnParameter {index,ident:if let Some(pat)=pat&&let PatKind::Ident(_,ident
,_)=pat.kind{Some(ident)}else{None},lifetime_count,span:ty.span,});loop{break;};
...
None}LifetimeElisionCandidate::Missing(missing)=>Some(missing),}}));{;};}let mut
...
lifetime);loop{break};loop{break};}else{elision_lifetime=Elision::None;}}debug!(
"(resolving function / closure) recorded parameter");{;};}debug_assert_matches!(
...
LifetimeRes>,}({});impl SelfVisitor<'_,'_,'_>{fn is_self_ty(&self,ty:&Ty)->bool{
...
::SelfTyAlias{..})=path_res{();return true;}self.impl_self.is_some()&&path_res==
self.impl_self}_=>false,}}}{();};impl<'a>Visitor<'a>for SelfVisitor<'_,'_,'_>{fn
visit_ty(&mut self,ty:&'a Ty){3;trace!("SelfVisitor considering ty={:?}",ty);if 
...
::ElidedAnchor{start,..}=res else{bug!()};loop{break;};start};let lt_res=self.r.
...
Expr){}}3;let impl_self=self.diag_metadata.current_self_type.as_ref().and_then(|
...
visitor=SelfVisitor{r:self.r,impl_self,lifetime:Set1::Empty};3;visitor.visit_ty(
ty);*&*&();trace!("SelfVisitor found={:?}",visitor.lifetime);visitor.lifetime}fn
...
.macro_def(label.span.ctxt()){*&*&();label.span.remove_mark();}}let ident=label.
...
->bool{{;};let ribs=&self.label_ribs[rib_index+1..];for rib in ribs{if rib.kind.
...
item.id);3;self.with_current_self_item(item,|this|{this.with_generic_param_rib(&
...
is_trait_impl:false,},|this|{let _=();visit::walk_item(this,item);},);},);});}fn
future_proof_import(&mut self,use_tree:&UseTree){;let segments=&use_tree.prefix.
segments;let _=||();if!segments.is_empty(){let ident=segments[0].ident;if ident.
is_path_segment_keyword()||ident.span.is_rust_2015(){{();};return;}let nss=match
...
let what=if ns==TypeNS{"type parameters"}else{"local variables"};3;this.r.dcx().
emit_err(ImportsCannotReferTo{span:ident.span,what});{;};}};for&ns in nss{match 
...
ident,ns,None,Some(binding)){({});report_error(self,ns);}}None=>{}}}}else if let
...
::Use(..)){3;self.resolve_doc_links(&item.attrs,MaybeExported::Ok(item.id));}let
name=item.ident.name;();debug!("(resolving item) resolving {} ({:?})",name,item.
kind);{;};let def_kind=self.r.local_def_kind(item.id);match item.kind{ItemKind::
...
generics)|ItemKind::Union(_,ref generics)=>{();self.resolve_adt(item,generics);}
ItemKind::Impl(box Impl{ref generics,ref of_trait,ref self_ty,items:ref//*&*&();
impl_items,..})=>{3;self.diag_metadata.current_impl_items=Some(impl_items);self.
resolve_implementation (&item.attrs,generics,of_trait,self_ty,item.id,impl_items
,);{;};self.diag_metadata.current_impl_items=None;}ItemKind::Trait(box Trait{ref
...
);let _=();this.with_self_rib(Res::SelfTyParam{trait_:local_def_id},|this|{this.
visit_generics(generics);();walk_list!(this,visit_param_bound,bounds,BoundKind::
...
local_def_id=this.r.local_def_id(item.id).to_def_id();3;this.with_self_rib(Res::
SelfTyParam{trait_:local_def_id},|this|{;this.visit_generics(generics);walk_list
!(this,visit_param_bound,bounds,BoundKind::Bound);3;});},);}ItemKind::Mod(..)=>{
...
macro_rules;3;visit::walk_item(this,item);if item.attrs.iter().all(|attr|{!attr.
has_name(sym::macro_use)&&!attr.has_name(sym::macro_escape)}){;this.parent_scope
.macro_rules=old_macro_rules;3;}});}ItemKind::Static(box ast::StaticItem{ref ty,
ref expr,..})=>{{;};self.with_static_rib(def_kind,|this|{this.with_lifetime_rib(
...
::Static)));({});}});}ItemKind::Const(box ast::ConstItem{ref generics,ref ty,ref
...
this|this.visit_ty(ty),);();if let Some(expr)=expr{this.resolve_const_body(expr,
Some((item.ident,ConstantItemKind::Const)),);3;}},);}ItemKind::Use(ref use_tree)
...
.macro_rules{let _=();let def_id=self.r.local_def_id(item.id);self.parent_scope.
macro_rules=self.r.macro_rules_scopes[&def_id];*&*&();}}ItemKind::ForeignMod(_)|
ItemKind::GlobalAsm(_)=>{3;visit::walk_item(self,item);}ItemKind::Delegation(ref
...
"with_generic_param_rib");loop{break};let LifetimeRibKind::Generics{binder,span:
generics_span,kind:generics_kind,..}=lifetime_kind else{panic!()};*&*&();let mut
function_type_rib=Rib::new(kind);3;let mut function_value_rib=Rib::new(kind);let
mut function_lifetime_rib=LifetimeRib::new(lifetime_kind);;let mut seen_bindings
=FxHashMap::default();((),());let mut seen_lifetimes=FxHashSet::default();if let
RibKind::AssocItem=kind{();let mut add_bindings_for_ns=|ns|{let parent_rib=self.
...
keys().map(|ident|(*ident,ident.span)));let _=();};add_bindings_for_ns(ValueNS);
...
LifetimeRibKind::Item=rib.kind{({});break;}}for param in params{let ident=param.
ident.normalize_to_macros_2_0();3;debug!("with_generic_param_rib: {}",param.id);
...
sess,original,param.ident);{;};self.record_lifetime_param(param.id,LifetimeRes::
Error);3;continue;}match seen_bindings.entry(ident){Entry::Occupied(entry)=>{let
...
Error);loop{break};continue;}GenericParamKind::Type{..}=>&mut function_type_rib,
GenericParamKind::Const{..}=>&mut function_value_rib,};let _=();let _=();self.r.
record_partial_res(param.id,PartialRes::new(Res::Err));({});rib.bindings.insert(
...
ident.span,"`'_` is a reserved lifetime name").emit();if true{};let _=||();self.
record_lifetime_param(param.id,LifetimeRes::Error);{;};continue;}if param.ident.
...
param.ident.span,"'static is a reserved lifetime name").emit();loop{break};self.
record_lifetime_param(param.id,LifetimeRes::Error);;continue;}let def_id=self.r.
local_def_id(param.id);{;};let(rib,def_kind)=match param.kind{GenericParamKind::
...
let res=LifetimeRes::Param{param:def_id,binder};({});self.record_lifetime_param(
param.id,res);{();};function_lifetime_rib.bindings.insert(ident,(param.id,res));
continue;3;}};let res=match kind{RibKind::Item(..)|RibKind::AssocItem=>Res::Def(
...
span_bug!(param.ident.span,"Unexpected rib kind {:?}",kind),};let _=||();self.r.
record_partial_res(param.id,PartialRes::new(res));;rib.bindings.insert(ident,res
);{();};}self.lifetime_ribs.push(function_lifetime_rib);self.ribs[ValueNS].push(
function_value_rib);;self.ribs[TypeNS].push(function_type_rib);f(self);self.ribs
[TypeNS].pop();let _=();self.ribs[ValueNS].pop();let function_lifetime_rib=self.
...
){({});candidates.retain(|(r,_)|r!=res);}}if let LifetimeBinderKind::BareFnType|
...
with_label_rib(RibKind::ConstantItem(may_use_generics,item),f);{;};},)})};if let
...
self.diag_metadata.current_trait_assoc_items,Some(trait_items));loop{break;};let
...
(&item.attrs,MaybeExported::Ok(item.id));3;match&item.kind{AssocItemKind::Const(
...
visit_generics(generics);let _=();this.visit_ty(ty);if let Some(expr)=expr{this.
...
Self,Option<DefId>)->T,)->T{({});let mut new_val=None;let mut new_id=None;if let
...
),self_type.clone()));({});let res=self.smart_resolve_path_fragment(&None,&path,
...
opt_def_id(){{;};new_id=Some(def_id);new_val=Some((self.r.expect_module(def_id),
...
Res,f:impl FnOnce(&mut Self)){3;let mut self_type_rib=Rib::new(RibKind::Normal);
self_type_rib.bindings.insert(Ident::with_dummy_span(kw::SelfUpper),self_res);3;
...
"resolve_implementation");3;self.with_generic_param_rib(&generics.params,RibKind
...
this.resolve_doc_links(attrs,MaybeExported::Impl(trait_id));{;};let item_def_id=
this.r.local_def_id(item_id);;if let Some(trait_id)=trait_id{this.r.trait_impls.
entry(trait_id).or_default().push(item_def_id);{;};}let item_def_id=item_def_id.
to_def_id();;let res=Res::SelfTyAlias{alias_to:item_def_id,forbid_generic:false,
is_trait_impl:trait_id.is_some()};{;};this.with_self_rib(res,|this|{if let Some(
trait_ref)=opt_trait_reference.as_ref(){;visit::walk_trait_ref(this,trait_ref);}
this.visit_ty(self_type);if true{};if true{};this.visit_generics(generics);this.
...
expr,..})=>{let _=();debug!("resolve_implementation AssocItemKind::Const");self.
...
,|this|{3;this.with_lifetime_rib(LifetimeRibKind::AnonymousWarn(item.id),|this|{
this.check_trait_item(item.id,item.ident,(((((&item.kind))))),ValueNS,item.span,
seen_trait_items,|i,s,c|ConstNotMemberOfTrait(i,s,c),);({});this.visit_generics(
...
this,item,AssocCtxt::Impl)});3;},);}AssocItemKind::Delegation(box delegation)=>{
...
ident.name,kind);3;let path=&self.current_trait_ref.as_ref().unwrap().1.path;let
path_names=path_names_to_string(path);let _=();self.report_error(span,err(ident,
path_names,candidate));3;feed_visibility(self,module.def_id());return;};let res=
binding.res();if let _=(){};let Res::Def(def_kind,id_in_trait)=res else{bug!()};
feed_visibility(self,id_in_trait);{;};match seen_trait_items.entry(id_in_trait){
...
PartialRes::new(res));3;return;}_=>{}}let path=&self.current_trait_ref.as_ref().
unwrap().1.path;({});let(code,kind)=match kind{AssocItemKind::Const(..)=>(E0323,
...
path_names_to_string(path);loop{break;};self.report_error(span,ResolutionError::
...
;();if let Some(body)=&delegation.body{let mut bindings=smallvec![(PatBoundCtx::
Product,Default::default())];;let span=delegation.path.segments.last().unwrap().
ident.span;({});self.fresh_binding(Ident::new(kw::SelfLower,span),delegation.id,
PatternSource::FnParam,&mut bindings,);if let _=(){};self.visit_block(body);}}fn
...
{3;debug!("resolving local ({:?})",local);walk_list!(self,visit_ty,&local.ty);if
let Some((init,els))=local.kind.init_else_opt(){{;};self.visit_expr(init);if let
Some(els)=els{{();};self.visit_block(els);}}self.resolve_pattern_top(&local.pat,
...
FxIndexMap::default();3;let mut is_never_pat=false;pat.walk(&mut|pat|{match pat.
...
true,_=>{}}true});loop{break};if is_never_pat{for(_,binding)in binding_map{self.
...
Vec<_>>();if true{};for(map_outer,pat_outer)in not_never_pats.iter(){let inners=
...
name.as_str().starts_with(char::is_uppercase),});();binding_error.origin.insert(
binding_inner.span);if true{};binding_error.target.insert(pat_outer.span);}Some(
...
not_never_pats{let _=||();let _=||();binding_map.extend(bm);}Ok(binding_map)}}fn
check_consistent_bindings(&mut self,pat:&'ast Pat){;let mut is_or_or_never=false
;loop{break;};pat.walk(&mut|pat|match pat.kind{PatKind::Or(..)|PatKind::Never=>{
is_or_or_never=true;if let _=(){};false}_=>true,});if is_or_or_never{let _=self.
compute_and_check_binding_map(pat);();}}fn resolve_arm(&mut self,arm:&'ast Arm){
...
SmallVec<[(PatBoundCtx,FxHashSet<Ident>);1]>,){3;visit::walk_pat(self,pat);self.
...
mut SmallVec<[(PatBoundCtx,FxHashSet<Ident>);1]>,){();pat.walk(&mut|pat|{debug!(
"resolve_pattern pat={:?} node={:?}",pat,pat.kind);({});match pat.kind{PatKind::
Ident(bmode,ident,ref sub)=>{loop{break};let has_sub=sub.is_some();let res=self.
...
PartialRes::new(res));((),());self.r.record_pat_span(pat.id,pat.span);}PatKind::
...
qself,ref path)=>{3;self.smart_resolve_path(pat.id,qself,path,PathSource::Pat);}
...
Default::default()));3;for p in ps{bindings.push((PatBoundCtx::Product,Default::
default()));*&*&();self.resolve_pattern_inner(p,pat_src,bindings);let collected=
...
collected);3;return false;}_=>{}}true});}fn fresh_binding(&mut self,ident:Ident,
...
bound_iter=bindings.iter().filter(|(_,set)|set.contains(&ident));loop{break};let
already_bound_and=bound_iter.clone().any(|(ctx,_)|*ctx==PatBoundCtx::Product);3;
let already_bound_or=bound_iter.any(|(ctx,_)|*ctx==PatBoundCtx::Or);if true{};if
already_bound_and{3;use ResolutionError::*;let error=match pat_src{PatternSource
::FnParam=>IdentifierBoundMoreThanOnceInParameterList,_=>//if true{};let _=||();
IdentifierBoundMoreThanOnceInSamePattern,};3;self.report_error(ident.span,error(
...
ident_valid{({});self.innermost_rib_bindings(ValueNS).insert(ident,res);}res}}fn
...
Res>{{();};let is_syntactic_ambiguity=!has_sub&&ann==BindingAnnotation::NONE;let
ls_binding=self.maybe_resolve_ident_in_lexical_scope(ident,ValueNS)?;();let(res,
...
span,},);();None}Res::Def(DefKind::ConstParam,def_id)=>{self.report_error(ident.
...
this.smart_resolve_report_errors(path,None,path_span,source,res);{;};let def_id=
this.parent_scope.module.nearest_parent_mod();({});let instead=res.is_some();let
suggestion =if let Some((start,end))=this.diag_metadata.in_range&&path[0].ident.
span.lo()==end.span.lo(){3;let mut sugg=".";let mut span=start.span.between(end.
...
suggest_adding_generic_parameter(path,source)}else{None};();let ue=UseError{err,
...
path.is_empty()=>(Some(last),path),_=>return Some(parent_err),};{;};let(mut err,
candidates)=this.smart_resolve_report_errors(prefix_path,following_seg,//*&*&();
path_span,PathSource::Type,None,);3;let mut parent_err=this.r.into_struct_error(
...
err.is_lint=parent_err.is_lint.clone();3;fn append_result<T,E>(res1:&mut Result<
Vec<T>,E>,res2:Result<Vec<T>,E>){;match res1{Ok(vec1)=>match res2{Ok(mut vec2)=>
vec1.append(&mut vec2),Err(e)=>*res1=Err(e),},Err(_)=>(),};3;}append_result(&mut
...
prefix_path[0].ident.span,rustc_errors::StashKey::CallAssocMethod,);3;}else{err.
...
else{({});err.cancel();}None};let partial_res=match self.resolve_qpath_anywhere(
...
traits);();}if PrimTy::from_name(path[0].ident.name).is_some(){let mut std_path=
...
.r.confused_type_with_std_module.insert(item_span,path_span);loop{break};self.r.
...
span,err.node);({});}PartialRes::new(Res::Err)}_=>report_errors(self,None),};if 
record_partial_res==RecordPartialRes::Yes{{;};self.r.record_partial_res(node_id,
partial_res);({});self.resolve_elided_lifetimes_in_path(partial_res,path,source,
path_span);{;};self.lint_unused_qualifications(path,ns,finalize);}partial_res}fn
...
Ident::new(kw::SelfLower,self_span);loop{break;};if let _=(){};let binding=self.
...
should_report_errs(){();self.r.report_error(span,resolution_error);}}#[inline]fn
...
;3;let trait_res=self.smart_resolve_path_fragment(&None,&path[..qself.position],
PathSource::Trait(AliasPossibility::No),Finalize::new(finalize.node_id,qself.//;
path_span),RecordPartialRes::No,);({});if trait_res.expect_full_res()==Res::Err{
return Ok(Some(trait_res));;}self.r.privacy_errors.truncate(num_privacy_errors);
let ns=if qself.position+1==path.len(){ns}else{TypeNS};{;};let partial_res=self.
...
qself.path_span),RecordPartialRes::No,);loop{break;};return Ok(Some(PartialRes::
with_unresolved_segments (partial_res.base_res(),partial_res.unresolved_segments
()+path.len()-qself.position-1,)));{;};}let result=match self.resolve_path(path,
...
gate_err_sym_msg{{();};let span=path[0].ident.span;if!span.allows_unstable(sym){
feature_err(tcx.sess,sym,span,msg).emit();let _=||();loop{break};}};PartialRes::
...
().unwrap().bindings.insert(ident,id);loop{break;};f(this);});}else{f(self);}}fn
resolve_labeled_block (&mut self,label:Option<Label>,id:NodeId,block:&'ast Block
){if true{};self.with_resolved_label(label,id,|this|this.visit_block(block));}fn
resolve_block(&mut self,block:&'ast Block){*&*&();((),());*&*&();((),());debug!(
"(resolving block) entering block");();let orig_module=self.parent_scope.module;
let anonymous_module=self.r.block_map.get(&block.id).cloned();let _=||();let mut
num_macro_definition_ribs=0;({});if let Some(anonymous_module)=anonymous_module{
debug!("(resolving block) found anonymous module, moving down");{();};self.ribs[
ValueNS].push(Rib::new(RibKind::Module(anonymous_module)));();self.ribs[TypeNS].
push(Rib::new(RibKind::Module(anonymous_module)));({});self.parent_scope.module=
anonymous_module;3;}else{self.ribs[ValueNS].push(Rib::new(RibKind::Normal));}let
...
item.kind{{;};num_macro_definition_ribs+=1;let res=self.r.local_def_id(item.id).
...
[ValueNS].pop();3;self.label_ribs.pop();}self.last_block_rib=self.ribs[ValueNS].
pop();loop{break};if anonymous_module.is_some(){self.ribs[TypeNS].pop();}debug!(
...
ConstArg(is_repeat_expr)=>is_repeat_expr,_=>IsRepeatExpr::No,};if let _=(){};let
...
,|this|{*&*&();resolve_expr(this);});});}fn resolve_expr_field(&mut self,f:&'ast
ExprField,e:&'ast Expr){3;self.resolve_expr(&f.expr,Some(e));self.visit_ident(f.
ident);{;};walk_list!(self,visit_attribute,f.attrs.iter());}fn resolve_expr(&mut
...
=>{{;};self.smart_resolve_path(expr.id,&se.qself,&se.path,PathSource::Struct);if
let Some(qself)=&se.qself{();self.visit_ty(&qself.ty);}self.visit_path(&se.path,
expr.id);({});walk_list!(self,resolve_expr_field,&se.fields,expr);match&se.rest{
...
node_id);{();};}Err(error)=>{self.report_error(label.ident.span,error);}}visit::
walk_expr(self,expr);3;}ExprKind::Break(None,Some(ref e))=>{self.resolve_expr(e,
...
cond,ref then,ref opt_else)=>{3;self.with_rib(ValueNS,RibKind::Normal,|this|{let
...
block);3;})});}ExprKind::ForLoop{ref pat,ref iter,ref body,label,kind:_}=>{self.
visit_expr(iter);loop{break;};self.with_rib(ValueNS,RibKind::Normal,|this|{this.
resolve_pattern_top(pat,PatternSource::For);();this.resolve_labeled_block(label,
...
{3;self.resolve_expr(arg,None);}self.visit_path_segment(seg);}ExprKind::Call(ref
callee,ref arguments)=>{{;};self.resolve_expr(callee,Some(expr));let const_args=
...
_ty)=>{3;visit::walk_expr(self,expr);}ExprKind::Closure(box ast::Closure{binder:
...
ExprKind::Repeat(ref elem,ref ct)=>{((),());let _=();self.visit_expr(elem);self.
resolve_anon_const(ct,AnonConstKind::ConstArg(IsRepeatExpr::Yes));();}ExprKind::
ConstBlock(ref ct)=>{();self.resolve_anon_const(ct,AnonConstKind::InlineConst);}
ExprKind::Index(ref elem,ref idx,_)=>{3;self.resolve_expr(elem,Some(expr));self.
...
in_range=Some((start,end));loop{break};self.resolve_expr(start,Some(expr));self.
resolve_expr(end,Some(expr));{();};self.diag_metadata.in_range=None;}_=>{visit::
...
"(recording candidate traits for expr) recording traits for {}",expr.id);{;};let
traits=self.traits_in_scope(call.seg.ident,ValueNS);{;};self.r.trait_map.insert(
...
extra_lifetime_params.extend(earlier_fresh);if true{};if true{};}}_=>{}}}self.r.
...
doc_link_resolutions);{;};let res=*doc_link_resolutions.entry(self.parent_scope.
...
=>{let _=();return;}ResolveDocLinks::ExportedMetadata|ResolveDocLinks::Exported|
ResolveDocLinks::All=>{}}if!attrs.iter().any(|attr|attr.may_have_doc_links()){3;
return;loop{break};}let mut need_traits_in_scope=false;for path_str in rustdoc::
attrs_to_preprocessed_links(attrs){let _=||();let mut any_resolved=false;let mut
...
need_assoc||!any_resolved{3;let mut path=&path_str[..];while let Some(idx)=path.
rfind("::"){*&*&();path=&path[..idx];need_traits_in_scope=true;for ns in[TypeNS,
ValueNS,MacroNS]{loop{break};self.resolve_and_cache_rustdoc_path(path,ns);}}}}if
need_traits_in_scope{;let mut doc_link_traits_in_scope=std::mem::take(&mut self.
r.doc_link_traits_in_scope);();doc_link_traits_in_scope.entry(self.parent_scope.
...
ResolveDocLinks::ExportedMetadata){3;return None;}Some(tr.def_id)}).collect()});
...
Finalize){if let Some(seg)=path.first()&&seg.ident.name==kw::PathRoot{3;return;}
if (finalize.path_span.from_expansion())||(path.iter()).any(|seg|seg.ident.span.
from_expansion()){loop{break};return;}let end_pos=path.iter().position(|seg|seg.
...
LateResolutionVisitor::new(self);{;};late_resolution_visitor.resolve_doc_links(&
krate.attrs,MaybeExported::Ok(CRATE_NODE_ID));loop{break};visit::walk_crate(&mut
