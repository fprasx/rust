...
MacroKind::Attr|MacroKind::Derive=>SubNS::AttrLike,};();let candidate=candidate.
map(sub_ns);*&*&();let requirement=requirement.map(sub_ns);candidate.is_none()||
...
registered_tools=RegisteredTools::default();3;let(_,pre_configured_attrs)=&*tcx.
...
None=>{3;let msg=format!("`{}` only accepts identifiers",sym::register_tool);let
span=nested_meta.span();{;};tcx.dcx().struct_span_err(span,msg).with_span_label(
span,"not an identifier").emit();{;};}}}}let predefined_tools=[sym::clippy,sym::
...
resolve_dollar_crates(&mut self){3;hygiene::update_dollar_crate_names(|ctxt|{let
...
self.invocation_parent_scopes[&expansion]};();let output_macro_rules_scope=self.
...
parent_module_id.map(|module_id|self.local_def_id(module_id).to_def_id());();let
...
create_stable_hashing_context(),);();let parent_scope=parent_module.map_or(self.
empty_module,|def_id|self.expect_module(def_id));({});self.ast_transform_scopes.
...
ParentScope{derives,..parent_scope};let _=();let supports_macro_expansion=invoc.
fragment_kind.supports_macro_expansion();{();};let node_id=invoc.expansion_data.
lint_node_id;if let _=(){};let(ext,res)=self.smart_resolve_macro_path(path,kind,
supports_macro_expansion,inner_attr,parent_scope,node_id,force,//*&*&();((),());
soft_custom_inner_attributes_gate(path,invoc),)?;{();};let span=invoc.span();let
...
id:NodeId,rule_i:usize){3;let did=self.local_def_id(id);self.unused_macro_rules.
...
unused_macros.contains_key(&def_id){((),());let _=();continue;}let node_id=self.
...
mut self.derive_data);{;};let entry=derive_data.entry(expn_id).or_insert_with(||
...
.map(|(_,ident)|{3;let res=Res::NonMacroAttr(NonMacroAttrKind::DeriveHelper);let
binding =((res,Visibility::<DefId>::Public,ident.span,expn_id)).to_name_binding(
self.arenas);({});(*ident,binding)}).collect();self.helper_attrs.insert(expn_id,
helper_attrs);{();};if entry.has_derive_copy||self.has_derive_copy(parent_scope.
expansion){let _=();self.containers_deriving_copy.insert(expn_id);}assert!(self.
derive_data.is_empty());let _=();let _=();self.derive_data=derive_data;Ok(())}fn
...
Indeterminate),};3;for segment in&path.segments{if let Some(args)=&segment.args{
...
!("expected `DefKind::Macro` or `Res::NonMacroAttr`"),};let _=();if true{};self.
check_stability_and_deprecation(&ext,path,node_id);();let unexpected_res=if ext.
...
,path.span,node_id,msg);let _=();let _=();}else{feature_err(&self.tcx.sess,sym::
...
SyntaxExtension>>,Res),Determinacy>{*&*&();let path_span=path.span;let mut path=
...
,};let _=||();loop{break};loop{break};loop{break};if trace{let kind=kind.expect(
"macro kind must be specified if tracing is enabled");if true{};let _=||();self.
...
let scope_set=kind.map_or(ScopeSet::All(MacroNS),ScopeSet::Macro);3;let binding=
self .early_resolve_ident_in_lexical_scope(path[0].ident,scope_set,parent_scope,
None,force,None,);({});if let Err(Determinacy::Undetermined)=binding{return Err(
Determinacy::Undetermined);let _=||();let _=||();}if trace{let kind=kind.expect(
"macro kind must be specified if tracing is enabled");if true{};let _=||();self.
single_segment_macro_resolutions .push((path[0].ident,kind,*parent_scope,binding
.ok(),));if true{};let _=||();}let res=binding.map(|binding|binding.res());self.
...
}};3;let macro_resolutions=mem::take(&mut self.multi_segment_macro_resolutions);
...
initial_binding|{loop{break};self.record_use(ident,initial_binding,Used::Other);
initial_binding.res()});;let res=binding.res();let seg=Segment::from_ident(ident
);3;check_consistency(self,&[seg],ident.span,kind,initial_res,res);if res==Res::
...
self.def_id_to_node_id[id.0]);({});self.lint_buffer.buffer_lint_with_diagnostic(
...
LegacyDeriveHelpers(binding.span),);if let _=(){};}}Err(..)=>{let expected=kind.
descr_expected();3;let mut err=self.dcx().create_err(CannotFindIdentInThisScope{
span:ident.span,expected,ident,});();self.unresolved_macro_suggestions(&mut err,
kind,&parent_scope,ident,krate);3;err.emit();}}}let builtin_attrs=mem::take(&mut
self.builtin_attrs);let _=();for(ident,parent_scope)in builtin_attrs{let _=self.
...
level{if let _=(){};let feature=stability.feature;let is_allowed=|feature|{self.
...
,node_id,span,msg);({});stability::report_unstable(self.tcx.sess,feature,reason.
to_opt_reason(),issue,None,is_soft,span,soft_handler,);();}}}if let Some(depr)=&
ext.deprecation{((),());let path=pprust::path_to_string(path);let(message,lint)=
stability::deprecation_message_and_lint(depr,"macro",&path);let _=();stability::
...
));*&*&();let mut err=self.dcx().struct_span_err(span,msg);if let Some(binding)=
binding{;err.span_note(binding.span,format!("the {} imported here",kind.descr())
);3;}err.emit();}}}pub(crate)fn check_reserved_macro_name(&mut self,ident:Ident,
...
Vec::new();3;}BuiltinMacroState::AlreadySeen(span)=>{struct_span_code_err!(self.
dcx(),item.span,E0773,"attempted to define built-in macro more than once").//();
with_span_note(span,"previously defined here").emit();3;}}}else{let msg=format!(
"cannot find a built-in macro with name `{}`",item.ident);3;self.dcx().span_err(
...
unresolved_segments()==0=>{if true{};return Ok(true);}PathResult::NonModule(..)|
PathResult::Failed{is_error_from_last_segment:false,..}=>{3;self.dcx().emit_err(
errors::CfgAccessibleUnsure{span});3;return Ok(false);}PathResult::Indeterminate
