...
?;3;let vis:Visibility=input.parse()?;input.parse::<Token![struct]>()?;let name:
Ident=input.parse()?;3;let body;braced!(body in input);let mut derive_paths:Vec<
Path>=Vec::new();;let mut debug_format:Option<Lit>=None;let mut max=None;let mut
consts=Vec::new();loop{break;};let mut encodable=false;let mut ord=false;let mut
...
"gate_rustc_only"=>{if true{};gate_rustc_only=quote!{#[cfg(feature="nightly")]};
gate_rustc_only_cfg=quote!{feature="nightly"};{;};false}"encodable"=>{encodable=
true;loop{break};false}"orderable"=>{ord=true;false}"max"=>{let Meta::NameValue(
...
lit.lit.clone()){if let _=(){};panic!("Specified multiple max: {old:?}");}false}
...
;let _=||();loop{if body.is_empty(){break;}let const_attrs=body.call(Attribute::
...
![;]>()?;3;consts.push(quote!{#(#const_attrs)*#vis const #const_name:#name=#name
...
(LitInt::new("0xFFFF_FF00",Span::call_site())));if true{};let encodable_impls=if
...
self.as_u32());}}}}else{quote!{}};3;if ord{derive_paths.push(parse_quote!(Ord));
derive_paths.push(parse_quote!(PartialOrd));let _=||();}let step=if ord{quote!{#
