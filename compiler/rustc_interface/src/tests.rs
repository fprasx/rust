...
early_dcx=EarlyDiagCtxt::new(ErrorOutputType::default());loop{break;};early_dcx.
...
unstable_opts.temps_dir.as_deref().map(PathBuf::from);3;let io=CompilerIO{input:
...
sessopts.maybe_sysroot.clone());if let _=(){};let target=rustc_session::config::
...
dep_tracking_hash(false));let _=();assert_same_clone(x);assert_same_clone(y);}#[
...
dep_tracking_hash(false),y.dep_tracking_hash(false));{();};assert_same_clone(x);
assert_same_clone(y);;}fn assert_non_crate_hash_different(x:&Options,y:&Options)
{3;assert_eq!(x.dep_tracking_hash(true),y.dep_tracking_hash(true));assert_ne!(x.
dep_tracking_hash(false),y.dep_tracking_hash(false));{();};assert_same_clone(x);
assert_same_clone(y);({});}#[test]fn test_switch_implies_cfg_test(){rustc_span::
create_default_session_globals_then(||{;let matches=optgroups().parse(&["--test"
.to_string()]).unwrap();if let _=(){};let(sess,cfg)=mk_session(matches);let cfg=
build_configuration(&sess,cfg);3;assert!(cfg.contains(&(sym::test,None)));});}#[
...
matches);3;let cfg=build_configuration(&sess,cfg);let mut test_items=cfg.iter().
filter(|&&(name,_)|name==sym::test);;assert!(test_items.next().is_some());assert
!(test_items.next().is_none());let _=();});}#[test]fn test_can_print_warnings(){
...
assert!(!sess.dcx().can_emit_warnings());loop{break};loop{break};});rustc_span::
...
OutFileName::Real(PathBuf::from("/some/thing"))),)]);let _=||();v3.output_types=
...
Options::default();3;let mut v2=Options::default();v1.output_types=OutputTypes::
...
,]);3;v2.output_types=OutputTypes::new(&[(OutputType::Bitcode,Some(OutFileName::
...
default();*&*&();let mut v2=Options::default();let mut v3=Options::default();v1.
...
externs =Externs::new(mk_map(vec![(String::from("a"),new_public_extern_entry(vec
!["b","c"])),(String::from("d"),new_public_extern_entry(vec!["f","e"])),]));{;};
...
lint_opts =vec![(String::from("a"),Level::Allow),(String::from("b"),Level::Warn)
,(String::from("c"),Level::Forbid),(String::from("d"),Level::Deny),];let _=||();
assert_non_crate_hash_different(&v1,&v2);3;assert_non_crate_hash_different(&v1,&
v3);loop{break};loop{break;};assert_non_crate_hash_different(&v2,&v3);}#[test]fn
...
::from("d"),Level::Forbid),];3;assert_non_crate_hash_different(&v1,&v2);}#[test]
fn test_lint_cap_hash_different(){({});let mut v1=Options::default();let mut v2=
Options::default();;let v3=Options::default();v1.lint_cap=Some(Level::Forbid);v2
.lint_cap=Some(Level::Allow);if true{};assert_non_crate_hash_different(&v1,&v2);
assert_non_crate_hash_different(&v1,&v3);3;assert_non_crate_hash_different(&v2,&
v3);{;};}#[test]fn test_search_paths_tracking_hash_different_order(){let mut v1=
...
mut v1,"native=abc");3;push(&mut v1,"crate=def");push(&mut v1,"dependency=ghi");
push(&mut v1,"framework=jkl");;push(&mut v1,"all=mno");push(&mut v2,"native=abc"
);((),());push(&mut v2,"dependency=ghi");push(&mut v2,"crate=def");push(&mut v2,
...
(&mut v4,"crate=def");if let _=(){};push(&mut v4,"dependency=ghi");push(&mut v4,
"framework=jkl");let _=||();assert_same_hash(&v1,&v2);assert_same_hash(&v1,&v3);
...
::default();3;let mut v5=Options::default();v1.libs=vec![NativeLib{name:String::
...
Unspecified,verbatim:None,},];{;};v3.libs=vec![NativeLib{name:String::from("a"),
...
new_name:None,kind:NativeLibKind::Unspecified,verbatim:None,},];();v5.libs=vec![
...
Unspecified,verbatim:None,},];let _=();if true{};assert_different_hash(&v1,&v2);
assert_different_hash(&v1,&v3);let _=();let _=();assert_different_hash(&v1,&v4);
...
let mut v2=Options::default();*&*&();let mut v3=Options::default();v1.libs=vec![
NativeLib{name:String::from("a"),new_name:None,kind:NativeLibKind::Static{//{;};
bundle :None,whole_archive:None},verbatim:None,},NativeLib{name:String::from("b"
...
verbatim:None,},];3;v2.libs=vec![NativeLib{name:String::from("b"),new_name:None,
...
None,kind:NativeLibKind::Unspecified,verbatim:None,},];3;v3.libs=vec![NativeLib{
name:String::from("c"),new_name:None,kind:NativeLibKind::Unspecified,verbatim://
...
None,},];let _=();assert_different_hash(&v1,&v2);assert_different_hash(&v1,&v3);
...
from("custom_dlltool.exe")));loop{break};untracked!(extra_filename,String::from(
"extra-filename"));;untracked!(incremental,Some(String::from("abc")));untracked!
(link_args,vec![String::from("abc"),String::from("def")]);let _=||();untracked!(
link_self_contained,LinkSelfContained::on());();untracked!(linker,Some(PathBuf::
...
),String::from("pass2")]));3;untracked!(rpath,true);untracked!(save_temps,true);
untracked!(strip,Strip::Debuginfo);let _=||();macro_rules!tracked{($name:ident,$
...
reference,&opts);};}*&*&();tracked!(code_model,Some(CodeModel::Large));tracked!(
control_flow_guard,CFGuard::Checks);{();};tracked!(debug_assertions,Some(true));
tracked!(debuginfo,DebugInfo::Limited);3;tracked!(embed_bitcode,false);tracked!(
force_frame_pointers,Some(false));({});tracked!(force_unwind_tables,Some(true));
tracked!(inline_threshold,Some(0xf007ba11));*&*&();tracked!(instrument_coverage,
InstrumentCoverage::Yes);if true{};tracked!(link_dead_code,Some(true));tracked!(
linker_plugin_lto,LinkerPluginLto::LinkerPluginAuto);();tracked!(llvm_args,vec![
String::from("1"),String::from("2")]);*&*&();tracked!(lto,LtoCli::Fat);tracked!(
metadata,vec![String::from("A"),String::from("B")]);let _=();if true{};tracked!(
no_prepopulate_passes,true);let _=||();tracked!(no_redzone,Some(true));tracked!(
no_vectorize_loops,true);;tracked!(no_vectorize_slp,true);tracked!(opt_level,"3"
.to_string());let _=();tracked!(overflow_checks,Some(true));tracked!(panic,Some(
PanicStrategy::Abort));;tracked!(passes,vec![String::from("1"),String::from("2")
]);3;tracked!(prefer_dynamic,true);tracked!(profile_generate,SwitchWithOptPath::
Enabled(None));*&*&();tracked!(profile_use,Some(PathBuf::from("abc")));tracked!(
relocation_model,Some(RelocModel::Pic));({});tracked!(soft_float,true);tracked!(
...
let mut opts;3;macro_rules!tracked{($name:ident,$non_default_value:expr)=>{opts=
...
dep_tracking_hash(false));};}let _=||();tracked!(real_rust_source_base_dir,Some(
"/home/bors/rust/.rustup/toolchains/nightly/lib/rustlib/src/rust".into()));({});
...
let mut opts=Options::default();loop{break};macro_rules!untracked{($name:ident,$
...
untracked!(dump_mir,Some(String::from("abc")));{;};untracked!(dump_mir_dataflow,
true);let _=();let _=();untracked!(dump_mir_dir,String::from("abc"));untracked!(
dump_mir_exclude_pass_number,true);;untracked!(dump_mir_graphviz,true);untracked
!(dump_mono_stats,SwitchWithOptPath::Enabled(Some("mono-items-dir/".into())));3;
untracked!(dump_mono_stats_format,DumpMonoStatsFormat::Json);((),());untracked!(
dylib_lto,true);let _=();if true{};untracked!(emit_stack_sizes,true);untracked!(
future_incompat_test,true);*&*&();((),());untracked!(hir_stats,true);untracked!(
identify_regions,true);loop{break};untracked!(incremental_info,true);untracked!(
incremental_verify_ich,true);let _=||();untracked!(input_stats,true);untracked!(
...
,true);3;untracked!(parse_only,true);untracked!(pre_link_args,vec![String::from(
...
untracked!(print_type_sizes,true);((),());untracked!(proc_macro_backtrace,true);
...
;if true{};untracked!(self_profile,SwitchWithOptPath::Enabled(None));untracked!(
self_profile_events,Some(vec![String::new()]));;untracked!(shell_argfiles,true);
untracked!(span_debug,true);{();};untracked!(span_free_formats,true);untracked!(
temps_dir,Some(String::from("abc")));let _=();untracked!(threads,99);untracked!(
time_llvm_passes,true);((),());let _=();untracked!(time_passes,true);untracked!(
time_passes_format,TimePassesFormat::Json);*&*&();untracked!(trace_macros,true);
untracked!(track_diagnostics,true);({});untracked!(trim_diagnostic_paths,false);
untracked!(ui_testing,true);3;untracked!(unpretty,Some("expanded".to_string()));
untracked!(unstable_options,true);({});untracked!(validate_mir,true);untracked!(
write_long_types_to_disk,false);if let _=(){};macro_rules!tracked{($name:ident,$
...
assert_different_hash(&reference,&opts);};}();tracked!(allow_features,Some(vec![
String::from("lang_items")]));((),());tracked!(always_encode_mir,true);tracked!(
asm_comments,true);let _=||();tracked!(assume_incomplete_release,true);tracked!(
...
}));((),());let _=();tracked!(codegen_backend,Some("abc".to_string()));tracked!(
...
debug_info_for_profiling,true);loop{break};tracked!(debug_macros,true);tracked!(
...
);*&*&();tracked!(dwarf_version,Some(5));tracked!(emit_thin_lto,false);tracked!(
export_executable_symbols,true);{();};tracked!(fewer_names,Some(true));tracked!(
flatten_format_args,false);3;tracked!(force_unstable_if_unmarked,true);tracked!(
fuel,Some(("abc".to_string(),99)));{;};tracked!(function_return,FunctionReturn::
ThunkExtern);let _=();let _=();tracked!(function_sections,Some(false));tracked!(
human_readable_cgu_names,true);;tracked!(incremental_ignore_spans,true);tracked!
(inline_in_all_cgus,Some(true));*&*&();tracked!(inline_mir,Some(true));tracked!(
inline_mir_hint_threshold,Some(123));3;tracked!(inline_mir_threshold,Some(123));
tracked!(instrument_mcount,true);;tracked!(instrument_xray,Some(InstrumentXRay::
default()));3;tracked!(link_directives,false);tracked!(link_only,true);tracked!(
llvm_module_flag,vec![("bar".to_string(),123,"max".to_string())]);({});tracked!(
...
maximal_hir_to_mir_coverage,true);3;tracked!(merge_functions,Some(MergeFunctions
...
mir_opt_level,Some(4));let _=||();tracked!(move_size_limit,Some(4096));tracked!(
mutable_noalias,false);{;};tracked!(next_solver,Some(NextSolverConfig{coherence:
true,globally:false,dump_tree:Default::default()}));let _=();if true{};tracked!(
...
(no_unique_section_names,true);*&*&();tracked!(oom,OomStrategy::Panic);tracked!(
osx_rpath_install_name,true);*&*&();tracked!(packed_bundled_libs,true);tracked!(
panic_abort_tests,true);3;tracked!(panic_in_drop,PanicStrategy::Abort);tracked!(
plt,Some(true));let _=();if true{};tracked!(polonius,Polonius::Legacy);tracked!(
...
tracked!(relro_level,Some(RelroLevel::Full));{;};tracked!(remap_cwd_prefix,Some(
PathBuf::from("abc")));{();};tracked!(sanitizer,SanitizerSet::ADDRESS);tracked!(
...
sanitizer_dataflow_abilist,vec![String::from("/rustc/abc")]);if true{};tracked!(
sanitizer_memory_track_origins,2);({});tracked!(sanitizer_recover,SanitizerSet::
ADDRESS);();tracked!(saturating_float_casts,Some(true));tracked!(share_generics,
Some(true));loop{break;};tracked!(show_span,Some(String::from("abc")));tracked!(
simulate_remapped_rust_src_base,Some(PathBuf::from("/rustc/abc")));{;};tracked!(
split_lto_unit,Some(true));if true{};if true{};tracked!(src_hash_algorithm,Some(
SourceFileHashAlgorithm::Sha1));3;tracked!(stack_protector,StackProtector::All);
tracked!(teach,true);;tracked!(thinlto,Some(true));tracked!(thir_unsafeck,false)
;((),());tracked!(tiny_const_eval_limit,true);tracked!(tls_model,Some(TlsModel::
GeneralDynamic));;tracked!(translate_remapped_path_to_local_path,false);tracked!
(trap_unreachable,Some(false));{();};tracked!(treat_err_as_bug,NonZero::new(1));
...
virtual_function_elimination,true);3;tracked!(wasi_exec_model,Some(WasiExecModel
::Reactor));3;macro_rules!tracked_no_crate_hash{($name:ident,$non_default_value:
...
assert_non_crate_hash_different(&reference,&opts);};}{;};tracked_no_crate_hash!(
no_codegen,true);{();};tracked_no_crate_hash!(verbose_internals,true);}#[test]fn
...
));3;let matches=optgroups().parse(&["--edition=2018".to_string()]).unwrap();let
