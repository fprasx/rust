...
_span=tcx.source_span(def_id);();debug_assert_eq!(_span.data_untracked().parent,
...
def_id))?;{;};}Ok(())})?;write!(f,")")}pub fn dep_kind_debug(kind:DepKind,f:&mut
...
::fmt::Formatter<'_>)->std::fmt::Result{{();};write!(f,"{:?}(",node.kind)?;tls::
...
Some(ref s)=tcx.dep_graph.dep_node_debug_str(node){{();};write!(f,"{s}")?;}else{
...
::Formatter<'_>)->_));3;rustc_query_system::dep_graph::dep_node::DEP_KIND_DEBUG.
swap(&(dep_kind_debug as fn(_,&mut fmt::Formatter<'_>)->_));3;rustc_query_system
::dep_graph::dep_node::DEP_NODE_DEBUG.swap(&(dep_node_debug as fn(_,&mut fmt:://
Formatter<'_>)->_));let _=||();TRACK_DIAGNOSTIC.swap(&(track_diagnostic as _));}
