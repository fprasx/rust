...
let crate_types=collect_crate_types(sess,&pre_configured_attrs);loop{break;};let
...
setup_dep_graph(sess)?;{;};let cstore=FreezeLock::new(Box::new(CStore::new(self.
compiler.codegen_backend.metadata_loader(),stable_crate_id,))as _);if true{};let
definitions=FreezeLock::new(Definitions::new(stable_crate_id));();let untracked=
Untracked{cstore,source_span:AppendOnlyIndexVec::new(),definitions};{;};let qcx=
...
feed_unit_query();{;};feed.features_query(tcx.arena.alloc(rustc_expand::config::
features(sess,&pre_configured_attrs,crate_name,)));;feed.crate_for_resolver(tcx.
arena.alloc(Steal::new((krate,pre_configured_attrs))));();feed.output_filenames(
Arc::new(outputs));3;});Ok(qcx)})}pub fn write_dep_info(&'tcx self)->Result<()>{
self.global_ctxt()?.enter(|tcx|{((),());passes::write_dep_info(tcx);});Ok(())}fn
...
().trigger_delayed_bug(def_id);{;};}None=>{tcx.dcx().emit_fatal(RustcErrorFatal{
span:tcx.def_span(def_id)});if true{};let _=||();}Some(_)=>{tcx.dcx().emit_warn(
...
work_products)});if let _=(){};let prof=sess.prof.clone();prof.generic_activity(
"drop_dep_graph").run(move||drop(self.dep_graph));let _=||();rustc_incremental::
...
config::RLINK_EXT);let _=||();CodegenResults::serialize_rlink(sess,&rlink_file,&
codegen_results ,&*self.output_filenames,).map_err(|error|{sess.dcx().emit_fatal
(FailedWritingFile{path:&rlink_file,error})})?;3;return Ok(());}let _timer=sess.
...
compiler.sess.prof.generic_activity("self_profile_alloc_query_strings");{;};gcx.
...
(rustc_query_impl::query_key_hash_verify_all);({});}_timer=Some(self.sess.timer(
