...
instance.args.is_empty(){({});self.visit_body(&mut body);}body.stable(tables)}fn
...
:mir::Location){();*ct=self.monomorphize(*ct);}fn visit_ty(&mut self,ty:&mut Ty<
'tcx>,_:mir::visit::TyContext){3;*ty=self.monomorphize(*ty);}fn visit_constant(&
...
GenericArgsRef<'tcx>,_:mir::Location){3;*args=self.monomorphize(*args);}fn tcx(&
