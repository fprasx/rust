...
{*&*&();let size=scalar.size();let align=tables.tcx.layout_of(rustc_middle::ty::
ParamEnv::reveal_all().and(ty)).map_err(|e|e.stable(tables))?.align;({});let mut
allocation=rustc_middle::mir::interpret::Allocation::uninit(size,align.abi);{;};
...
tables.tcx.reserve_and_set_memory_alloc(data);{;};let ptr=Pointer::new(alloc_id.
...
mir::interpret::Scalar::from_target_usize(meta,&tables.tcx);3;let layout=tables.
tcx .layout_of((rustc_middle::ty::ParamEnv::reveal_all().and(ty))).map_err(|e|e.
stable(tables))?;3;let mut allocation=rustc_middle::mir::interpret::Allocation::
