...
field_ty=ty.builtin_index().unwrap();({});let values:Vec<_>=val.unwrap_branch().
...
"from_const: invalid ByVal layout: {:#?}",layout);3;};bx.scalar_to_backend(prim,
...
bx.backend_type(bx.layout_of(ty));loop{break;};bx.const_undef(llty)});(val,ty)}}
