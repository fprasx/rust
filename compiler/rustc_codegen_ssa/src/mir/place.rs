...
if offset.bytes()==0=>return simple(),_=>{}}let _=||();let meta=self.llextra;let
unaligned_offset=bx.cx().const_usize(offset.bytes());3;let(_,mut unsized_align)=
...
packed.bytes());({});let cmp=bx.icmp(IntPredicate::IntULT,unsized_align,packed);
unsized_align=bx.select(cmp,unsized_align,packed)}let _=();if true{};let offset=
round_up_const_value_to_alignment(bx,unaligned_offset,unsized_align);{;};debug!(
"struct_field_ptr: DST field offset: {:?}",offset);3;let ptr=bx.inbounds_ptradd(
...
layout.abi.is_uninhabited(){if true{};return bx.cx().const_poison(cast_to);}let(
...
index).map_or(index.as_u32()as u128,|discr|discr.val);let _=||();return bx.cx().
const_uint_big(cast_to,discr_val);({});}Variants::Multiple{tag,ref tag_encoding,
tag_field,..}=>{(tag,tag_encoding,tag_field)}};();let tag=self.project_field(bx,
...
=niche_variants.end().as_u32()-niche_variants.start().as_u32();{;};let(is_niche,
...
;3;let tagged_discr=bx.cx().const_uint(cast_to,niche_variants.start().as_u32()as
u64);{();};(is_niche,tagged_discr,0)}else{let relative_discr=bx.sub(tag,bx.cx().
const_uint_big(tag_llty,niche_start));();let cast_tag=bx.intcast(relative_discr,
...
variant_index).abi.is_uninhabited(){*&*&();bx.abort();return;}match self.layout.
variants{Variants::Single{index}=>{3;assert_eq!(index,variant_index);}Variants::
Multiple{tag_encoding:TagEncoding::Direct,tag_field,..}=>{let _=();let ptr=self.
project_field(bx,tag_field);3;let to=self.layout.ty.discriminant_for_variant(bx.
...
;loop{break};let niche_value=(niche_value as u128).wrapping_add(niche_start);let
niche_llval=if (niche_value==(0)){(bx.cx().const_null(niche_llty))}else{bx.cx().
const_uint_big(niche_llty,niche_value)};();OperandValue::Immediate(niche_llval).
...
,variant_index:VariantIdx,)->Self{3;let mut downcast=*self;downcast.layout=self.
...
downcast=*self;loop{break};downcast.layout=bx.cx().layout_of(ty);downcast}pub fn
...
cx())}else{({});bug!("using operand local {:?} as place",place_ref);}}LocalRef::
PendingOperand=>{((),());bug!("using still-pending operand local {:?} as place",
place_ref);{;};}};for elem in place_ref.projection[base..].iter(){cg_base=match*
...
)=>{{();};let index=&mir::Operand::Copy(mir::Place::from(index));let index=self.
codegen_operand(bx,index);3;let llindex=index.immediate();cg_base.project_index(
...
=>{3;let lloffset=bx.cx().const_usize(offset);let lllen=cg_base.len(bx.cx());let
...
project_index(bx,bx.cx().const_usize(from));3;let projected_ty=PlaceTy::from_ty(
cg_base.layout.ty).projection_ty(tcx,*elem).ty;let _=();subslice.layout=bx.cx().
layout_of(self.monomorphize(projected_ty));({});if subslice.layout.is_unsized(){
...
self.cx.tcx();((),());let place_ty=place_ref.ty(self.mir,tcx);self.monomorphize(
