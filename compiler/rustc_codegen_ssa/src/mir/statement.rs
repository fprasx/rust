...
operand));3;self.debug_introduce_local(bx,index);}LocalRef::Operand(op)=>{if!op.
...
)=self.locals[local]{3;cg_indirect_place.storage_live(bx);}}mir::StatementKind::
...
)=self.locals[local]{3;cg_indirect_place.storage_dead(bx);}}mir::StatementKind::
...
self.codegen_operand(bx,op);3;bx.assume(op_val.immediate());}}mir::StatementKind
...
pointee_info_at(bx,rustc_target::abi::Size::ZERO).expect("Expected pointer");();
let bytes=bx.mul(count,bx.const_usize(pointee_layout.size.bytes()));3;let align=
