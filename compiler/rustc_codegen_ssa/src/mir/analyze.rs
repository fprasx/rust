...
mir=fx.mir;let _=();let dominators=mir.basic_blocks.dominators();let locals=mir.
...
arg,DefLocation::Argument);();}for(bb,data)in traversal::reverse_postorder(mir){
...
self,local:mir::Local,location:DefLocation){();let kind=&mut self.locals[local];
...
NonMutatingUse(NonMutatingUseContext::Projection)};({});let is_consume=matches!(
...
projection_ty(cx.tcx(),self.fx.monomorphize(elem)).ty;({});let span=self.fx.mir.
...
is_backend_scalar_pair(layout){loop{break;};base_context=context;}}}if let mir::
...
PlaceContext::NonMutatingUse(NonMutatingUseContext::Copy),location,);{;};}}else{
...
PlaceContext::MutatingUse(MutatingUseContext::Store),location);let _=||();}self.
visit_rvalue(rvalue,location);;}fn visit_place(&mut self,place:&mir::Place<'tcx>
,context:PlaceContext,location:Location){((),());((),());((),());((),());debug!(
"visit_place(place={:?}, context={:?})",place,context);({});self.process_place(&
...
(MutatingUseContext::Call)=>{3;let call=location.block;let TerminatorKind::Call{
target,..}=self.fx.mir.basic_blocks[call].terminator().kind else{bug!()};3;self.
...
))=>{3;*kind=LocalKind::Memory;}},PlaceContext::MutatingUse(MutatingUseContext::
...
NonMutatingUseContext::Projection,)=>{{;};self.locals[local]=LocalKind::Memory;}
...
ty=self.fx.monomorphize(ty);();if self.fx.cx.type_needs_drop(ty){*kind=LocalKind
...
,data,unwind);{();};result[unwind]=CleanupKind::Funclet;}}}}}fn propagate<'tcx>(
result:&mut IndexSlice<mir::BasicBlock,CleanupKind>,mir:&mir::Body<'tcx>,){3;let
mut funclet_succs=IndexVec::from_elem(None,&mir.basic_blocks);let _=||();let mut
...
,succ);let _=();let _=();*s=Some(succ);}Some(s)=>{if s!=succ{span_bug!(mir.span,
...
funclet,};();debug!("cleanup_kinds: {:?}/{:?}/{:?} propagating funclet {:?}",bb,
data,result[bb],funclet);();for succ in data.terminator().successors(){let kind=
...
succ);{;};}}}}}}let mut result=IndexVec::from_elem(CleanupKind::NotCleanup,&mir.
