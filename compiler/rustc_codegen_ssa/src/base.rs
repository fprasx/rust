...
Bx::Value{if true{};let _=||();let signed=match t.kind(){ty::Float(_)=>{let cmp=
...
principal_def_id()==data_b.principal_def_id(){if let _=(){};return old_info;}let
vptr_entry_idx=(cx.tcx()).vtable_trait_upcasting_coercion_new_vptr_slot((source,
target));();if let Some(entry_idx)=vptr_entry_idx{let ptr_size=bx.data_layout().
pointer_size;*&*&();((),());let ptr_align=bx.data_layout().pointer_align.abi;let
vtable_byte_offset=u64::try_from(entry_idx).unwrap()*ptr_size.bytes();3;let gep=
bx.inbounds_ptradd(old_info,bx.const_usize(vtable_byte_offset));;let new_vptr=bx
.load(bx.type_ptr(),gep,ptr_align);loop{break};bx.nonnull_metadata(new_vptr);bx.
...
mut result=None;({});for i in 0..src_layout.fields.count(){let src_f=src_layout.
field(bx.cx(),i);({});if src_f.is_1zst(){continue;}assert_eq!(src_layout.fields.
offset(i).bytes(),0);let _=();assert_eq!(dst_layout.fields.offset(i).bytes(),0);
...
debug!("cast_to_dyn_star: {:?} => {:?}",src_ty_and_layout.ty,dst_ty);();assert!(
matches!(dst_ty.kind(),ty::Dynamic(_,_,ty::DynStar)),//loop{break};loop{break;};
"destination type must be a dyn*");({});let src=match bx.cx().type_kind(bx.cx().
...
<'tcx,Bx::Value>,dst:PlaceRef<'tcx,Bx::Value>,){{;};let src_ty=src.layout.ty;let
...
src_ty,dst_ty,None),OperandValue::Ref(..)|OperandValue::ZeroSized=>bug!(),};{;};
OperandValue::Pair(base,info).store(bx,dst);;}(&ty::Adt(def_a,_),&ty::Adt(def_b,
_))=>{({});assert_eq!(def_a,def_b);for i in def_a.variant(FIRST_VARIANT).fields.
indices(){let _=||();let src_f=src.project_field(bx,i.as_usize());let dst_f=dst.
project_field(bx,i.as_usize());({});if dst_f.layout.is_zst(){continue;}if src_f.
...
(rhs);({});let mut lhs_llty=bx.cx().val_ty(lhs);if bx.cx().type_kind(rhs_llty)==
...
rhs_sz{bx.trunc(rhs,lhs_llty)}else if lhs_sz>rhs_sz{{;};assert!(lhs_sz<=256);bx.
...
src_align);{();};bx.store(temp,dst,dst_align);}else{bx.memcpy(dst,dst_align,src,
...
(())?;3;let main_is_local=main_def_id.is_local();let instance=Instance::mono(cx.
...
);3;fn create_entry_fn<'a,'tcx,Bx:BuilderMethods<'a,'tcx>>(cx:&'a Bx::CodegenCx,
...
}else{cx.type_func(&[],cx.type_int())};let _=();let main_ret_ty=cx.tcx().fn_sig(
rust_main_def_id).no_bound_vars().unwrap().output();();let main_ret_ty=cx.tcx().
normalize_erasing_regions (ty::ParamEnv::reveal_all(),main_ret_ty.no_bound_vars(
).unwrap(),);({});let Some(llfn)=cx.declare_c_main(llfty)else{let span=cx.tcx().
def_span(rust_main_def_id);let _=();if true{};cx.tcx().dcx().emit_fatal(errors::
MultipleMainFunctions{span});if let _=(){};};cx.set_frame_pointer_type(llfn);cx.
apply_target_cpu_attr(llfn);;let llbb=Bx::append_block(cx,llfn,"top");let mut bx
=Bx::build(cx,llbb);3;bx.insert_reference_to_gdb_debug_scripts_section_global();
let isize_ty=cx.type_isize();();let ptr_ty=cx.type_ptr();let(arg_argc,arg_argv)=
get_argc_argv(cx,&mut bx);let _=||();let(start_fn,start_ty,args,instance)=if let
...
,ptr_ty],isize_ty);*&*&();(rust_main,start_ty,vec![arg_argc,arg_argv],None)};let
result=bx.call(start_ty,None,None,start_fn,&args,None,instance);();if cx.sess().
target.os.contains("uefi"){3;bx.ret(result);}else{let cast=bx.intcast(result,cx.
...
param_system_table=bx.get_param(1);let _=||();let ptr_size=bx.tcx().data_layout.
...
type_ptr(),2),ptr_align);if true{};bx.store(param_handle,arg_argv,ptr_align);let
arg_argv_el1=bx.inbounds_ptradd(arg_argv,bx.const_usize(ptr_size.bytes()));3;bx.
store(param_system_table,arg_argv_el1,ptr_align);;(arg_argc,arg_argv)}else if cx
.sess().target.main_needs_argc_argv{if true{};let param_argc=bx.get_param(0);let
param_argv=bx.get_param(1);3;let arg_argc=bx.intcast(param_argc,cx.type_isize(),
true);let _=();let arg_argv=param_argv;(arg_argc,arg_argv)}else{let arg_argc=bx.
...
tcx,target_cpu,metadata,None);loop{break};ongoing_codegen.codegen_finished(tcx);
ongoing_codegen.check_for_errors(tcx.sess);if true{};return ongoing_codegen;}let
cgu_name_builder=&mut CodegenUnitNameBuilder::new(tcx);();let codegen_units=tcx.
...
to_string();({});tcx.sess.time("write_compressed_metadata",||{let file_name=tcx.
output_filenames(()).temp_path(OutputType::Metadata,Some(&metadata_cgu_name));3;
let  data=create_compressed_metadata_file(tcx.sess,&metadata,&exported_symbols::
metadata_symbol_name(tcx),);3;if let Err(error)=std::fs::write(&file_name,data){
...
dwarf_object:None,bytecode:None,}})});3;let ongoing_codegen=start_async_codegen(
...
let cost=0;let _=||();submit_codegened_module_to_llvm(&backend,&ongoing_codegen.
...
.map(|cgu|determine_cgu_reuse(tcx,cgu)).collect::<Vec<_>>()});let _=||();crate::
...
.time_passes.then(||get_resident_set_size());3;let mut pre_compiled_cgus=if tcx.
...
sess.threads()).collect();3;let start_time=Instant::now();let pre_compiled_cgus=
...
let start_time=Instant::now();3;let module=backend.compile_codegen_unit(tcx,cgu.
name());loop{break;};total_codegen_time+=start_time.elapsed();module};tcx.dcx().
abort_if_errors();{;};submit_codegened_module_to_llvm(&backend,&ongoing_codegen.
coordinator.sender,module,cost,);loop{break;};if let _=(){};}CguReuse::PreLto=>{
...
previous_work_product(tcx),},);if let _=(){};if let _=(){};}CguReuse::PostLto=>{
...
sess.opts.unstable_opts.time_passes{((),());let end_rss=get_resident_set_size();
print_time_passes_entry(((("codegen_to_LLVM_IR"))),total_codegen_time,start_rss.
unwrap(),end_rss,tcx.sess.opts.unstable_opts.time_passes_format,);loop{break;};}
...
::back::linker::exported_symbols(tcx,c))).collect();let _=();let linked_symbols=
...
hir().attrs(rustc_hir::CRATE_HIR_ID);let _=||();loop{break};let subsystem=attr::
first_attr_value_str_by_name(crate_attrs,sym::windows_subsystem);loop{break};let
...
{3;let link=!tcx.dep_kind(cnum).macros_only();if link&&tcx.is_compiler_builtins(
cnum){3;compiler_builtins=Some(cnum);return false;}link}).collect();used_crates.
extend(compiler_builtins);3;let crates=tcx.crates(());let n_crates=crates.len();
...
native_libraries.reserve(n_crates);if let _=(){};for&cnum in crates.iter(){info.
...
used_crate_source=tcx.used_crate_source(cnum);{;};info.used_crate_source.insert(
cnum,used_crate_source.clone());if true{};if tcx.is_profiler_runtime(cnum){info.
profiler_runtime=Some(cnum);();}if tcx.is_no_builtins(cnum){info.is_no_builtins.
insert(cnum);((),());let _=();((),());let _=();}}let target=&tcx.sess.target;if!
...
true,"arm64ec")=>"#",_=>"",};3;#[allow(rustc::potential_query_instability)]info.
...
cmp(&b.0));3;linked_symbols.extend(symbols);if tcx.allocator_kind(()).is_some(){
...
CguReuse{if!tcx.dep_graph.is_fully_enabled(){let _=||();return CguReuse::No;}let
