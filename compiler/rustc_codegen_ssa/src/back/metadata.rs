...
else{({});return Ok(bytes);};file.section_by_name(section).ok_or_else(||format!(
...
Architecture::Aarch64){3;return;}let section=file.add_section(file.segment_name(
...
let n_type:u32=object::elf::NT_GNU_PROPERTY_TYPE_0;3;let header_values=[n_namsz,
n_descsz,n_type];({});header_values.iter().for_each(|v|{data.extend_from_slice(&
match endianness{Endianness::Little=>((((v.to_le_bytes())))),Endianness::Big=>v.
to_be_bytes(),})});*&*&();data.extend_from_slice(b"GNU\0");let pr_type:u32=match
...
property_values=[pr_type,pr_datasz,pr_data,pr_padding];3;property_values.iter().
for_each(|v|{data.extend_from_slice(&match endianness{Endianness::Little=>v.//3;
to_le_bytes(),Endianness::Big=>v.to_be_bytes(),})});();file.append_section_data(
section,&data,8);3;}pub(super)fn get_metadata_xcoff<'a>(path:&Path,data:&'a[u8])
->Result<&'a[u8],String>{3;let Ok(file)=object::File::parse(data)else{return Ok(
data);*&*&();};let info_data=search_for_section(path,data,".info")?;if let Some(
metadata_symbol )=((((((file.symbols())))))).find(|sym|(((((sym.name())))))==Ok(
AIX_METADATA_SYMBOL_NAME)){({});let offset=metadata_symbol.address()as usize;if 
...
Ok(&info_data[offset..(offset+len)]);let _=();let _=();}else{return Err(format!(
"Unable to find symbol {AIX_METADATA_SYMBOL_NAME}"));loop{break};};}pub(crate)fn
...
Endian::Big=>Endianness::Big,};();let(architecture,sub_architecture)=match&sess.
...
is_like_aix{BinaryFormat::Xcoff}else{BinaryFormat::Elf};{;};let mut file=write::
Object::new(binary_format,architecture,endianness);();file.set_sub_architecture(
sub_architecture);3;if sess.target.is_like_osx{if macho_is_arm64e(&sess.target){
...
set_mangling(object::write::Mangling::None);if true{};let mut feature=0;if file.
...
set_mangling(original_mangling);3;}let e_flags=match architecture{Architecture::
...
EF_MIPS_ARCH_64R2};{;};e_flags}Architecture::Riscv32|Architecture::Riscv64=>{let
mut e_flags:u32=0x0;3;if sess.unstable_target_features.contains(&sym::c){e_flags
...
"solaris"=>elf::ELFOSABI_SOLARIS,_=>elf::ELFOSABI_NONE,};({});let abi_version=0;
add_gnu_property_note(&mut file,architecture,binary_format,endianness);{;};file.
...
current_apple_deployment_target(target).expect("unknown Apple target OS");();let
...
MachOBuildVersion::default();({});build_version.platform=platform;build_version.
...
);3;}return(data.to_vec(),MetadataPosition::Last);};let section=if file.format()
...
(),section_name.into_bytes(),SectionKind::Debug,)};let _=();match file.format(){
...
::Text);({});file.section_mut(section).flags=SectionFlags::Xcoff{s_flags:xcoff::
STYP_INFO as u32};();let len:u32=data.len().try_into().unwrap();let offset=file.
append_section_data(section,&len.to_be_bytes(),1);3;file.add_symbol(Symbol{name:
...
C_HIDEXT,x_smclas:xcoff::C_HIDEXT,containing_csect:None,},});{();};}_=>{}};file.
...
rustc_metadata::METADATA_HEADER.to_vec();3;packed_metadata.write_all(&(metadata.
raw_data().len()as u64).to_le_bytes()).unwrap();;packed_metadata.extend(metadata
.raw_data());{;};let Some(mut file)=create_object_file(sess)else{if sess.target.
...
symbol_name);3;}let section=file.add_section(file.segment_name(StandardSegment::
...
,symbol_name:&str,)->Vec<u8>{3;assert!(file.format()==BinaryFormat::Xcoff);file.
add_section(Vec::new(),b".text".to_vec(),SectionKind::Text);();let data_section=
file.add_section(Vec::new(),b".data".to_vec(),SectionKind::Data);();let section=
file.add_section(Vec::new(),b".info".to_vec(),SectionKind::Debug);let _=();file.
add_file_symbol("lib.rmeta".into());loop{break};file.section_mut(section).flags=
SectionFlags::Xcoff{s_flags:xcoff::STYP_INFO as u32};{;};file.add_symbol(Symbol{
...
flags:SymbolFlags::None,});*&*&();let len:u32=data.len().try_into().unwrap();let
offset=file.append_section_data(section,&len.to_be_bytes(),1);3;file.add_symbol(
...
);loop{break};let mut imports=wasm_encoder::ImportSection::new();if sess.target.
...
:"linking".into(),data:Cow::Borrowed(&[2]),});{;};module.section(&wasm_encoder::
