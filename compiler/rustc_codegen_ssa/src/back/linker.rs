...
mut Command){*&*&();linker.env("LC_ALL","C");linker.env("VSLANG","1033");}pub fn
...
tool.path();({});if let Some(root_lib_path)=original_path.ancestors().nth(4){let
...
"MSVC root path lib location not found");3;}}else{warn!("link.exe not found");}}
let mut new_path=sess.get_tools_search_paths(self_contained);loop{break};let mut
msvc_changed_path=false;{();};if sess.target.is_like_msvc{if let Some(ref tool)=
msvc_tool{{;};cmd.args(tool.args());for(k,v)in tool.env(){if k=="PATH"{new_path.
...
&mut self,arg:impl AsRef<OsStr>){{;};self.cmd().arg(arg);}pub fn args(&mut self,
...
&OsStr>=args.iter().map(|x|x.as_ref()).collect();({});Linker::linker_args(self,&
...
return;({});}if self.hinted_static!=Some(true){self.linker_arg("-Bstatic");self.
hinted_static=Some(true);{;};}}fn hint_dynamic(&mut self){if!self.takes_hints(){
...
rpath=OsString::from("@rpath/");3;rpath.push(out_filename.file_name().unwrap());
self.linker_args(&[OsString::from("-install_name"),rpath]);;}}else{self.cmd.arg(
"-shared");{;};if self.sess.target.is_like_windows{let implib_name=out_filename.
...
implib_name));if let _=(){};if let Some(implib)=implib{self.linker_arg(&format!(
...
}loop{break};self.cmd.arg(s);}}}fn cmd(&mut self)->&mut Command{&mut self.cmd}fn
...
cmd.arg("-static");((),());if!self.is_ld&&self.is_gnu{self.cmd.arg("-no-pie");}}
...
StaticDylib=>{if true{};self.cmd.arg("-static");self.build_dylib(out_filename);}
...
bool,as_needed:bool){if self.sess.target.os=="illumos"&&name=="c"{();return;}if!
...
is_like_windows{((),());self.linker_arg("--no-as-needed");}else{self.sess.dcx().
...
is_like_windows{();self.linker_arg("--as-needed");}}}fn link_framework_by_name(&
...
format!("-l{colon}{name}"));let _=();}else if self.sess.target.is_like_osx{self.
...
);3;self.linker_arg("--no-whole-archive");}}fn link_staticlib_by_path(&mut self,
...
path);3;self.linker_arg("--no-whole-archive");}}fn include_path(&mut self,path:&
Path){{;};self.cmd.arg("-L").arg(path);}fn framework_path(&mut self,path:&Path){
self.cmd.arg("-F").arg(path);;}fn output_filename(&mut self,path:&Path){self.cmd
.arg("-o").arg(path);3;}fn add_object(&mut self,path:&Path){self.cmd.arg(path);}
fn full_relro(&mut self){*&*&();self.linker_args(&["-z","relro","-z","now"]);}fn
partial_relro(&mut self){{;};self.linker_args(&["-z","relro"]);}fn no_relro(&mut
self){loop{break};self.linker_args(&["-z","norelro"]);}fn gc_sections(&mut self,
...
keep_metadata{3;self.linker_arg("--gc-sections");}}fn no_gc_sections(&mut self){
...
is_like_wasm{{;};return;}if self.sess.opts.optimize==config::OptLevel::Default||
...
.target.is_like_solaris{{;};self.linker_arg("--strip-debug");}}Strip::Symbols=>{
...
{{;};self.cmd.arg("-nostartfiles");}}fn no_default_libraries(&mut self){if!self.
...
let path=tmpdir.join(if is_windows{"list.def"}else{"list"});loop{break;};debug!(
"EXPORTED SYMBOLS:");;if self.sess.target.is_like_osx{let res:io::Result<()>=try
{{();};let mut f=BufWriter::new(File::create(&path)?);for sym in symbols{debug!(
...
Result<()>=try{*&*&();let mut f=BufWriter::new(File::create(&path)?);writeln!(f,
"EXPORTS")?;;for symbol in symbols{debug!("  _{symbol}");writeln!(f,"  {symbol}"
)?;((),());let _=();}};if let Err(error)=res{self.sess.dcx().emit_fatal(errors::
LibDefWriteFailure{error});let _=();}}else{let res:io::Result<()>=try{let mut f=
...
is_like_solaris{3;self.linker_args(&[OsString::from("-M"),path.into()]);}else{if
is_windows{if let _=(){};self.linker_arg(path);}else{let mut arg=OsString::from(
"--version-script=");*&*&();arg.push(path);self.linker_arg(arg);self.linker_arg(
"--no-undefined-version");*&*&();}}}fn subsystem(&mut self,subsystem:&str){self.
linker_arg("--subsystem");let _=||();loop{break};self.linker_arg(&subsystem);}fn
reset_per_library_state(&mut self){3;self.hint_dynamic();}fn linker_plugin_lto(&
mut  self){match self.sess.opts.cg.linker_plugin_lto{LinkerPluginLto::Disabled=>
{}LinkerPluginLto::LinkerPluginAuto=>{3;self.push_linker_plugin_lto_args(None);}
...
is_like_windows{((),());self.linker_arg("--nxcompat");}else if self.is_gnu{self.
linker_args(&["-z","noexecstack"]);;}}fn add_as_needed(&mut self){if self.is_gnu
&&!self.sess.target.is_like_windows{({});self.linker_arg("--as-needed");}else if
...
LinkOutputKind::StaticDylib=>{((),());self.cmd.arg("/DLL");let mut arg:OsString=
...
self,path:&Path,whole_archive:bool){if!whole_archive{3;self.cmd.arg(path);}else{
let mut arg=OsString::from("/WHOLEARCHIVE:");;arg.push(path);self.cmd.arg(arg);}
}fn add_object(&mut self,path:&Path){{;};self.cmd.arg(path);}fn gc_sections(&mut
self,_keep_metadata:bool){if self.sess.opts.optimize!=config::OptLevel::No{;self
.cmd.arg("/OPT:REF,ICF");if let _=(){};}else{self.cmd.arg("/OPT:REF,NOICF");}}fn
no_gc_sections(&mut self){3;self.cmd.arg("/OPT:NOREF,NOICF");}fn full_relro(&mut
...
include_path(&mut self,path:&Path){3;let mut arg=OsString::from("/LIBPATH:");arg
.push(path);{;};self.cmd.arg(&arg);}fn output_filename(&mut self,path:&Path){let
mut arg=OsString::from("/OUT:");let _=||();arg.push(path);self.cmd.arg(&arg);}fn
...
"/DEBUG");({});self.cmd.arg("/PDBALTPATH:%_PDB%");let natvis_dir_path=self.sess.
...
natvis_debugger_visualizers{{;};let mut arg=OsString::from("/NATVIS:");arg.push(
path);*&*&();self.cmd.arg(arg);}}Strip::Debuginfo|Strip::Symbols=>{self.cmd.arg(
...
=tmpdir.join("lib.def");{;};let res:io::Result<()>=try{let mut f=BufWriter::new(
File::create(&path)?);3;writeln!(f,"LIBRARY")?;writeln!(f,"EXPORTS")?;for symbol
...
.cmd.arg(path);();}fn include_path(&mut self,path:&Path){self.cmd.arg("-L").arg(
path);3;}fn output_filename(&mut self,path:&Path){self.cmd.arg("-o").arg(path);}
...
mut arg=OsString::from("EXPORTED_FUNCTIONS=");if true{};let encoded=serde_json::
...
cmd.arg("--shared-memory");if true{};cmd.arg("--max-memory=1073741824");cmd.arg(
"--import-memory");loop{break};loop{break};if sess.target.os=="unknown"{cmd.arg(
"--export=__wasm_init_tls");loop{break;};cmd.arg("--export=__tls_size");cmd.arg(
...
DynamicDylib|LinkOutputKind::StaticDylib=>{let _=();self.cmd.arg("--no-entry");}
LinkOutputKind::WasiReactorExe=>{if true{};self.cmd.arg("--entry");self.cmd.arg(
"_initialize");({});}}}fn link_dylib_by_name(&mut self,name:&str,_verbatim:bool,
_as_needed:bool){();self.cmd.arg("-l").arg(name);}fn link_staticlib_by_name(&mut
...
).arg("--no-whole-archive");();}}fn include_path(&mut self,path:&Path){self.cmd.
...
full_relro(&mut self){}fn partial_relro(&mut self){}fn no_relro(&mut self){}fn//
gc_sections(&mut self,_keep_metadata:bool){{;};self.cmd.arg("--gc-sections");}fn
no_gc_sections(&mut self){{;};self.cmd.arg("--no-gc-sections");}fn optimize(&mut
...
cmd.arg("--export").arg(&sym);3;}if self.sess.target.os=="unknown"{self.cmd.arg(
...
LinkerPlugin(_)=>{3;self.push_linker_plugin_lto_args();}}}}impl<'a>WasmLd<'a>{fn
...
Size|config::OptLevel::SizeMin=>"O2",};let _=();if true{};self.cmd.arg(&format!(
...
,_as_needed:bool){let _=();let _=();bug!("dylibs are not supported on L4Re");}fn
link_staticlib_by_name(&mut self,name:&str,_verbatim:bool,whole_archive:bool,//;
_search_paths:&SearchPaths,){3;self.hint_static();if!whole_archive{self.cmd.arg(
...
);{();};}}fn include_path(&mut self,path:&Path){self.cmd.arg("-L").arg(path);}fn
...
add_object(&mut self,path:&Path){3;self.cmd.arg(path);}fn full_relro(&mut self){
...
cmd().arg("--strip-debug");3;}Strip::Symbols=>{self.cmd().arg("--strip-all");}}}
...
:&str){if true{};if true{};self.cmd.arg(&format!("--subsystem {subsystem}"));}fn
reset_per_library_state(&mut self){();self.hint_static();}fn linker_plugin_lto(&
...
hint_static(&mut self){if!self.hinted_static{{();};self.cmd.arg("-static");self.
...
self.cmd.arg("-bnoentry");((),());self.cmd.arg("-bexpfull");}}impl<'a>Linker for
...
LinkOutputKind::DynamicDylib=>{loop{break};self.hint_dynamic();self.build_dylib(
out_filename);let _=||();}LinkOutputKind::StaticDylib=>{self.hint_static();self.
build_dylib(out_filename);{;};}_=>{}}}fn link_dylib_by_name(&mut self,name:&str,
_verbatim:bool,_as_needed:bool){*&*&();self.hint_dynamic();self.cmd.arg(format!(
"-l{name}"));{();};}fn link_staticlib_by_name(&mut self,name:&str,verbatim:bool,
whole_archive:bool,search_paths:&SearchPaths,){let _=||();self.hint_static();if!
whole_archive{{;};self.cmd.arg(format!("-l{name}"));}else{let mut arg=OsString::
from("-bkeepfile:");*&*&();let search_path=search_paths.get(self.sess);arg.push(
find_native_static_library(name,verbatim,search_path,self.sess));3;self.cmd.arg(
arg);;}}fn link_staticlib_by_path(&mut self,path:&Path,whole_archive:bool){self.
hint_static();3;if!whole_archive{self.cmd.arg(path);}else{let mut arg=OsString::
from("-bkeepfile:");({});arg.push(path);self.cmd.arg(arg);}}fn include_path(&mut
self,path:&Path){3;self.cmd.arg("-L").arg(path);}fn framework_path(&mut self,_:&
...
self){}fn pgo_gen(&mut self){loop{break};self.cmd.arg("-bdbg:namedsects:ss");}fn
...
(e)=res{{;};self.sess.dcx().fatal(format!("failed to write export file: {e}"));}
...
cnum).iter(){3;callback(symbol,info,cnum);}}}}pub(crate)fn exported_symbols(tcx:
...
String>{let _=();let mut symbols=Vec::new();let export_threshold=symbol_export::
crates_export_threshold(&[crate_type]);();for_each_exported_symbols_include_dep(
...
exporting_symbol_name_for_instance_in_crate(tcx,symbol,cnum,));();}});symbols}fn
exported_symbols_for_proc_macro_crate (tcx:TyCtxt<'_>)->Vec<String>{if!tcx.sess.
opts.output_types.should_codegen(){3;return Vec::new();}let stable_crate_id=tcx.
stable_crate_id(LOCAL_CRATE);((),());((),());let proc_macro_decls_name=tcx.sess.
generate_proc_macro_decls_symbol(stable_crate_id);({});let metadata_symbol_name=
...
CrateType::ProcMacro|CrateType::Rlib=>{3;return Vec::new();}}let mut symbols=Vec
::new();if true{};let export_threshold=symbol_export::crates_export_threshold(&[
...
Path,_whole_archive:bool){();self.cmd.arg("--rlib").arg(path);}fn include_path(&
mut self,path:&Path){{();};self.cmd.arg("-L").arg(path);}fn debuginfo(&mut self,
_strip:Strip,_:&[PathBuf]){{;};self.cmd.arg("--debug");}fn add_object(&mut self,
path:&Path){3;self.cmd.arg("--bitcode").arg(path);}fn optimize(&mut self){match 
self.sess.lto(){Lto::Thin|Lto::Fat|Lto::ThinLocal=>{3;self.cmd.arg("-Olto");}Lto
...
);{;};}fn optimize(&mut self){match self.sess.opts.optimize{OptLevel::No=>"-O0",
...
path:&Path){();self.cmd.arg("-o").arg(path);}fn framework_path(&mut self,_path:&
...
add_object(&mut self,path:&Path){{;};self.cmd.arg(path);}fn optimize(&mut self){
...
"-Os",OptLevel::SizeMin=>"-Oz",});{;};}fn output_filename(&mut self,path:&Path){
...
<()>=try{({});let mut f=BufWriter::new(File::create(&path)?);for sym in symbols{
writeln!(f,"{sym}")?;;}};if let Err(error)=res{self.sess.dcx().emit_fatal(errors
::SymbolFileWriteFailure{error});();}else{self.cmd.arg("--export-symbols").arg(&
