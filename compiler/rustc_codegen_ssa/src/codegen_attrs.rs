...
LocalDefId,name:&str)->Linkage{{;};use rustc_middle::mir::mono::Linkage::*;match
...
(did);let _=();let _=();let _=();if true{};assert!(def_kind.has_codegen_attrs(),
"unexpected `def_kind` in `codegen_fn_attrs`: {def_kind:?}",);();}let attrs=tcx.
hir().attrs(tcx.local_def_id_to_hir_id(did));if true{};let mut codegen_fn_attrs=
...
rustc_hir::CRATE_HIR_ID);3;let no_builtins=attr::contains_name(crate_attrs,sym::
...
"this attribute can only be applied to functions");3;None}};let Some(Ident{name,
...
sym::coverage=>{();let inner=attr.meta_item_list();match inner.as_deref(){Some([
...
RUSTC_STD_INTERNAL_SYMBOL}sym::used=>{{;};let inner=attr.meta_item_list();match 
...
"`#[used(linker)]` is currently unstable",).emit();{;};}codegen_fn_attrs.flags|=
...
span,"`#[used(compiler)]` is currently unstable",).emit();{;};}codegen_fn_attrs.
flags|=CodegenFnAttrFlags::USED;let _=();let _=();}Some(_)=>{tcx.dcx().emit_err(
ExpectedUsedSymbol{span:attr.span});3;}None=>{let is_like_elf=!(tcx.sess.target.
is_like_osx||tcx.sess.target.is_like_windows||tcx.sess.target.is_like_wasm);{;};
...
track_caller=>{if true{};let is_closure=tcx.is_closure_like(did.to_def_id());if!
...
emit();{;};}codegen_fn_attrs.export_name=Some(s);}}sym::target_feature=>{if!tcx.
...
with_span_label(tcx.def_span(did),"not an `unsafe` function").emit();({});}else{
...
tcx,did,val.as_str()));loop{break};if tcx.is_foreign_item(did){codegen_fn_attrs.
import_linkage=linkage;if true{};}else{codegen_fn_attrs.linkage=linkage;}}}sym::
...
no_sanitize=>{if true{};no_sanitize_span=Some(attr.span);if let Some(list)=attr.
...
"`#[instruction_set]` requires an argument").emit();let _=();if true{};None}_=>{
struct_span_code_err!(tcx.dcx(),attr.span,E0779,//*&*&();((),());*&*&();((),());
"cannot specify more than one instruction set").emit();({});None}})}sym::repr=>{
...
::None}}Some(MetaItemKind::NameValue(_))=>ia,None=>ia,}});({});codegen_fn_attrs.
optimize=(attrs.iter()).fold(OptimizeAttr::None,|ia,attr|{if!attr.has_name(sym::
optimize){{;};return ia;}let err=|sp,s|struct_span_code_err!(tcx.dcx(),sp,E0722,
"{}",s).emit();;match attr.meta_kind(){Some(MetaItemKind::Word)=>{err(attr.span,
"expected one argument");3;ia}Some(MetaItemKind::List(ref items))=>{inline_span=
Some(attr.span);*&*&();if items.len()!=1{err(attr.span,"expected one argument");
...
no_sanitize_span,inline_span){();let hir_id=tcx.local_def_id_to_hir_id(did);tcx.
...
link_name);loop{break};loop{break;};codegen_fn_attrs.link_name=Some(link_name);}
...
MetaItemLit};*&*&();let meta_item_list=attr.meta_item_list();let meta_item_list=
meta_item_list.as_deref();3;let sole_meta_list=match meta_item_list{Some([item])
...
.emit();{;};None}}else{tcx.dcx().emit_err(errors::InvalidLinkOrdinalFormat{span:
...
.link_name.is_none()||codegen_fn_attrs.link_ordinal.is_none(){3;return;}let msg=
"cannot use `#[link_name]` with `#[link_ordinal]`";let _=||();if let Some(span)=
inline_span{{();};tcx.dcx().span_err(span,msg);}else{tcx.dcx().err(msg);}}pub fn
