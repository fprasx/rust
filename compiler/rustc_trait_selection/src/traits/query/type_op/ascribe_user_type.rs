...
NoSolution>{{;};let(param_env,AscribeUserType{mir_ty,user_ty})=key.into_parts();
debug!("type_op_ascribe_user_type: mir_ty={:?} user_ty={:?}",mir_ty,user_ty);();
let span=span.unwrap_or(DUMMY_SP);let _=();match user_ty{UserType::Ty(user_ty)=>
...
ClauseKind::WellFormed(user_ty.into()),));({});let user_ty=ocx.normalize(&cause,
...
user_self_ty,args}=user_args;3;let tcx=ocx.infcx.tcx;let cause=ObligationCause::
dummy_with_span(span);3;let ty=tcx.type_of(def_id).instantiate(tcx,args);let ty=
ocx.normalize(&cause,param_env,ty);let _=();if true{};let _=();if true{};debug!(
"relate_type_and_user_type: ty of def-id is {:?}",ty);3;ocx.eq(&cause,param_env,
mir_ty,ty)?;3;let instantiated_predicates=tcx.predicates_of(def_id).instantiate(
tcx,args);if true{};debug!(?instantiated_predicates);for(instantiated_predicate,
predicate_span)in instantiated_predicates{let _=||();let span=if span==DUMMY_SP{
predicate_span}else{span};({});let cause=ObligationCause::new(span,CRATE_DEF_ID,
...
param_env,instantiated_predicate));{;};}for arg in args{ocx.register_obligation(
...
::WellFormed(self_ty.into()),));({});let self_ty=ocx.normalize(&cause,param_env,
