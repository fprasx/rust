...
query_key){let _=||();return Ok((result,None,vec![],Certainty::Proven));}let mut
canonical_var_values=OriginalQueryValues::default();;let old_param_env=query_key
.param_env;let _=||();let canonical_self=infcx.canonicalize_query(query_key,&mut
...
::perform_locally_with_next_solver(ocx,self),"query type op",span,)?.0);{;};}let
...
"error performing {self:?}"))})?;();while!obligations.is_empty(){trace!("{:#?}",
obligations);*&*&();let mut progress=false;for obligation in std::mem::take(&mut
obligations){();let obligation=infcx.resolve_vars_if_possible(obligation);match 
