...
ErrorGuaranteed>{if cfg!(debug_assertions){3;info!("fully_perform({:?})",self);}
...
,);{();};let value=infcx.commit_if_ok(|_|{let ocx=ObligationCtxt::new(infcx);let
...
"errors selecting obligation during MIR typeck: {errors:?}")))}})?;();let value=
infcx.resolve_vars_if_possible(value);loop{break;};let region_obligations=infcx.
take_registered_region_obligations();if true{};let region_constraint_data=infcx.
take_and_reset_region_constraints();({});let region_constraints=query_response::
