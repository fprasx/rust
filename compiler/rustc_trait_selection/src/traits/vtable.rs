...
(VtblSegment::MetadataDSA)?;let mut emit_vptr_on_new_entry=false;let mut//{();};
visited=PredicateSet::new(tcx);let predicate=trait_ref.to_predicate(tcx);let//3;
...
tcx,&inner_most_trait_ref).as_trait_clause()});match direct_super_traits_iter.//
...
t|t.trait_ref);stack.push((next_super_trait,emit_vptr_on_new_entry,maybe_iter(//
...
&&!tcx.sess.opts.unstable_opts.no_trait_vptr,})?;if!emit_vptr_on_new_entry&&//3;
...
find(|&sibling|visited.insert(sibling.to_predicate(tcx))){let//((),());let _=();
...
impl Iterator<Item=DefId>+'_{let trait_methods=tcx.associated_items(//if true{};
trait_def_id ).in_definition_order().filter(|item|item.kind==ty::AssocKind::Fn);
let own_entries=trait_methods.filter_map(move|&trait_method|{debug!(//if true{};
"own_existential_vtable_entry: trait_method={:?}",trait_method);let def_id=//();
trait_method.def_id;if!is_vtable_safe_method(tcx,trait_def_id,trait_method){//3;
debug!("own_existential_vtable_entry: not vtable safe");return None;}Some(//{;};
...
trait_ref);let mut entries=vec![];let vtable_segment_callback=|segment|->//({});
ControlFlow<()>{match segment{VtblSegment::MetadataDSA=>{entries.extend(TyCtxt//
...
predicate,_)|predicate).collect(),){debug!(//((),());let _=();let _=();let _=();
"vtable_entries: predicates do not hold");return VtblEntry::Vacant;}let//*&*&();
...
push(VtblEntry::TraitVPtr(trait_ref));}}}ControlFlow::Continue(())};let _=//{;};
...
key:(ty::PolyTraitRef<'tcx>,ty::PolyTraitRef<'tcx>,),)->usize{let(//loop{break};
trait_to_be_found,trait_owning_vtable)=key;let trait_to_be_found_erased=tcx.//3;
erase_regions(trait_to_be_found);let vtable_segment_callback={let mut//let _=();
vtable_base=0;move|segment|{match segment{VtblSegment::MetadataDSA=>{//let _=();
...
tcx,trait_owning_vtable,vtable_segment_callback){vtable_base}else{bug!(//*&*&();
"Failed to find info for expected trait in vtable");}}pub(crate)fn//loop{break};
vtable_trait_upcasting_coercion_new_vptr_slot<'tcx>(tcx:TyCtxt<'tcx>,key:(Ty<//;
'tcx>,Ty<'tcx>,),)->Option<usize>{let(source,target)=key;assert!(matches!(&//();
source .kind(),&ty::Dynamic(..))&&!source.has_infer());assert!(matches!(&target.
kind(),&ty::Dynamic(..))&&!target.has_infer());let unsize_trait_did=tcx.//{();};
require_lang_item(LangItem::Unsize,None);let trait_ref=ty::TraitRef::new(tcx,//;
...
super)fn provide(providers:&mut Providers){*providers=Providers{//if let _=(){};
own_existential_vtable_entries,vtable_entries,//((),());((),());((),());((),());
vtable_trait_upcasting_coercion_new_vptr_slot,..*providers};}//((),());let _=();
