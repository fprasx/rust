...
::new(infcx.tcx,def_id,[ty]);if true{};let obligation=Obligation::new(infcx.tcx,
...
"do_normalize_predicates: normalized predicates = {:?}",predicates);let _=();let
outlives_env=OutlivesEnvironment::new(elaborated_env);let _=();let errors=infcx.
...
ty());({});}c.normalize(self.0,ty::ParamEnv::empty())}}predicate.fold_with(&mut 
ConstNormalizer(tcx))}),).collect();let _=();let _=();let _=();if true{};debug!(
...
reveal());({});let outlives_predicates:Vec<_>=predicates.extract_if(|predicate|{
...
return elaborated_env;loop{break};loop{break};loop{break};loop{break;};};debug!(
...
&outlives_predicates).cloned();if true{};let outlives_env=ty::ParamEnv::new(tcx.
mk_clauses_from_iter(outlives_env),unnormalized_env.reveal());let _=||();let Ok(
...
"normalize_param_env_or_error: errored resolving outlives predicates");();return
elaborated_env;if let _=(){};if let _=(){};if let _=(){};if let _=(){};};debug!(
"normalize_param_env_or_error: outlives predicates={:?}",outlives_predicates);3;
let mut predicates=non_outlives_predicates;let _=();if true{};predicates.extend(
...
ObligationCtxt::new(infcx);3;debug!(?value);let normalized_value=ocx.normalize(&
cause,param_env,value);loop{break};loop{break};debug!(?normalized_value);debug!(
"select_all_or_error start");{;};let errors=ocx.select_all_or_error();if!errors.
is_empty(){*&*&();return Err(errors);}debug!("select_all_or_error complete");let
...
predicates);({});for predicate in predicates{let obligation=Obligation::new(tcx,
ObligationCause::dummy(),param_env,predicate);if true{};ocx.register_obligation(
obligation);;}let errors=ocx.select_all_or_error();let result=!errors.is_empty()
;loop{break};loop{break};debug!("impossible_predicates = {:?}",result);result}fn
...
tcx,trait_def_id,key.1);let _=||();predicates.push(ty::Binder::dummy(trait_ref).
...
'tcx>{tcx:TyCtxt<'tcx>,generics:&'tcx ty::Generics,trait_item_def_id:DefId,}{;};
...
parent(param_def_id)==self.trait_item_def_id{{;};return ControlFlow::Break(());}
...
}}let _=||();let generics=tcx.generics_of(trait_item_def_id);let predicates=tcx.
predicates_of(trait_item_def_id);let _=();let impl_trait_ref=tcx.impl_trait_ref(
...
instantiate(tcx,impl_trait_ref.args),)})});if true{};let infcx=tcx.infer_ctxt().
...
providers);if true{};let _=||();vtable::provide(providers);*providers=Providers{
