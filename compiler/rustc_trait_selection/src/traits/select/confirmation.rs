...
TraitAliasCandidate=>{3;let data=self.confirm_trait_alias_candidate(obligation);
...
(obligation,def_id)?;{;};ImplSource::Builtin(BuiltinImplSource::Misc,data)}};for
...
enter_forall_and_leak_universe(trait_predicate).trait_ref;let _=();if true{};let
placeholder_self_ty=placeholder_trait_predicate.self_ty();let _=();if true{};let
...
candidate,);let _=||();loop{break};let mut obligations=Vec::new();let candidate=
normalize_with_depth_to (self,obligation.param_env,((obligation.cause.clone())),
obligation.recursion_depth+1,candidate,&mut obligations,);();obligations.extend(
...
=normalize_with_depth_to(self,obligation.param_env,((obligation.cause.clone())),
obligation.recursion_depth+1,predicate,&mut obligations,);({});obligations.push(
...
"obligation {:?} had matched a builtin impl but now doesn't",obligation);3;};let
...
recursion_depth+1,trait_def,nested,)}else{vec![]};let _=();debug!(?obligations);
...
trait_ref.args.type_at(1);if let _=(){};debug!(?src,?dst);let mut transmute_env=
...
dst,src},assume,);3;let fully_flattened=match maybe_transmutable{Answer::No(_)=>
Err (Unimplemented)?,Answer::If(cond)=>flatten_answer_tree(self.tcx(),obligation
,predicate,cond),Answer::Yes=>vec![],};loop{break;};debug!(?fully_flattened);Ok(
...
SelectionError<'tcx>>{({});debug!(?obligation,"confirm_auto_impl_candidate");let
self_ty=self.infcx.shallow_resolve(obligation.predicate.self_ty());();let types=
...
'tcx>>>,)->Vec<PredicateObligation<'tcx>>{();debug!(?nested,"vtable_auto_impl");
ensure_sufficient_stack(||{let _=();let _=();let cause=obligation.derived_cause(
BuiltinDerivedObligation);if let _=(){};let poly_trait_ref=obligation.predicate.
to_poly_trait_ref();{;};let trait_ref=self.infcx.enter_forall_and_leak_universe(
...
param_env,trait_def_id,trait_ref.args,obligation.predicate,);loop{break};let mut
obligations=self.collect_predicates_for_types(obligation.param_env,cause,//({});
obligation.recursion_depth+1,trait_def_id,nested,);if true{};obligations.extend(
trait_obligations);({});debug!(?obligations,"vtable_auto_impl");obligations})}fn
...
rematch_impl(impl_def_id,obligation);((),());let _=();debug!(?args,"impl args");
...
args,?recursion_depth,"vtable_impl");loop{break;};let mut impl_obligations=self.
impl_or_trait_obligations(cause,recursion_depth,param_env,impl_def_id,args.//();
value,parent_trait_pred,);if let _=(){};debug!(?impl_obligations,"vtable_impl");
...
PredicateObligation<'tcx>>,SelectionError<'tcx>>{{;};let tcx=self.tcx();debug!(?
obligation,?index,"confirm_object_candidate");();let trait_predicate=self.infcx.
enter_forall_and_leak_universe(obligation.predicate);{;};let self_ty=self.infcx.
shallow_resolve(trait_predicate.self_ty());();let ty::Dynamic(data,..)=*self_ty.
...
cause.span,"object candidate with no principal")});();let object_trait_ref=self.
...
(),self_ty);3;let mut nested=vec![];let mut supertraits=util::supertraits(tcx,ty
::Binder::dummy(object_trait_ref));let _=||();let unnormalized_upcast_trait_ref=
supertraits.nth(index).expect(//loop{break};loop{break};loop{break};loop{break};
"supertraits iterator no longer has as many elements");{;};let upcast_trait_ref=
self.infcx.instantiate_binder_with_fresh_vars(obligation.cause.span,//if true{};
HigherRankedType,unnormalized_upcast_trait_ref,);if true{};let upcast_trait_ref=
normalize_with_depth_to (self,obligation.param_env,((obligation.cause.clone())),
...
normalize_with_depth_to (self,obligation.param_env,((obligation.cause.clone())),
obligation.recursion_depth+1,super_trait,&mut nested,);3;nested.push(obligation.
...
assoc_types{3;let defs:&ty::Generics=tcx.generics_of(assoc_type);if!defs.params.
...
"GATs in trait object shouldn't have been considered",);loop{break;};return Err(
...
mut args=smallvec::SmallVec::with_capacity(defs.count());let _=||();args.extend(
trait_predicate.trait_ref.args.iter());3;let mut bound_vars:smallvec::SmallVec<[
...
::Ty(kind);{();};bound_vars.push(bound_var);Ty::new_bound(tcx,ty::INNERMOST,ty::
...
::Const{..}=>{*&*&();let bound_var=ty::BoundVariableKind::Const;bound_vars.push(
...
;loop{break};ty::Binder::bind_with_vars(bound,bound_vars).to_predicate(tcx)};let
...
"object nested obligations");3;let vtable_base=vtable_trait_first_method_offset(
...
SelectionError<'tcx>>{{;};debug!(?obligation,"confirm_fn_pointer_candidate");let
tcx=self.tcx();;let Some(self_ty)=self.infcx.shallow_resolve(obligation.self_ty(
).no_bound_vars())else{({});return Err(SelectionError::Unimplemented);};let sig=
...
confirm_poly_trait_refs(obligation,trait_ref)?;loop{break};let cause=obligation.
derived_cause(BuiltinDerivedObligation);*&*&();((),());let output_ty=self.infcx.
enter_forall_and_leak_universe(sig.output());if true{};let _=||();let output_ty=
normalize_with_depth_to (self,obligation.param_env,((cause.clone())),obligation.
recursion_depth,output_ty,&mut nested,);{;};let tr=ty::TraitRef::from_lang_item(
...
enter_forall_and_leak_universe(obligation.predicate);();let trait_ref=predicate.
trait_ref;let _=();let trait_def_id=trait_ref.def_id;let args=trait_ref.args;let
trait_obligations=self.impl_or_trait_obligations((&obligation.cause),obligation.
recursion_depth,obligation.param_env,trait_def_id,args,obligation.predicate,);3;
...
?coroutine_def_id,?args,"confirm_coroutine_candidate");3;let coroutine_sig=args.
as_coroutine().sig();;let self_ty=obligation.predicate.self_ty().no_bound_vars()
. expect("unboxed closure type should not capture bound vars from the predicate"
);();let(trait_ref,_,_)=super::util::coroutine_trait_ref_and_outputs(self.tcx(),
obligation.predicate.def_id(),self_ty,coroutine_sig,);if true{};let nested=self.
...
infcx.shallow_resolve(obligation.self_ty().skip_binder());{;};let ty::Coroutine(
coroutine_def_id,args)=*self_ty.kind()else{((),());((),());((),());((),());bug!(
"closure candidate for non-closure {:?}",obligation);({});};debug!(?obligation,?
...
expect("future has no bound vars").self_ty(),coroutine_sig,);();let nested=self.
confirm_poly_trait_refs(obligation,ty::Binder::dummy(trait_ref))?;{();};debug!(?
...
coroutine_def_id,args)=*self_ty.kind()else{((),());((),());((),());((),());bug!(
"closure candidate for non-closure {:?}",obligation);({});};debug!(?obligation,?
coroutine_def_id,?args,"confirm_iterator_candidate");if true{};let gen_sig=args.
...
self_ty(),gen_sig,);({});let nested=self.confirm_poly_trait_refs(obligation,ty::
Binder::dummy(trait_ref))?;loop{break;};if let _=(){};debug!(?trait_ref,?nested,
...
self.infcx.shallow_resolve(obligation.self_ty().skip_binder());let _=();let ty::
Coroutine(coroutine_def_id,args)=*self_ty.kind()else{let _=||();let _=||();bug!(
"closure candidate for non-closure {:?}",obligation);({});};debug!(?obligation,?
coroutine_def_id,?args,"confirm_async_iterator_candidate");{;};let gen_sig=args.
as_coroutine().sig();if let _=(){};*&*&();((),());let(trait_ref,_)=super::util::
...
"closure candidate for non-closure {:?}",obligation);let _=();if true{};}};self.
...
(obligation.self_ty().skip_binder());*&*&();let mut nested=vec![];let(trait_ref,
kind_ty)=match*self_ty.kind(){ty::CoroutineClosure(_,args)=>{({});let args=args.
as_coroutine_closure();();let trait_ref=args.coroutine_closure_sig().map_bound(|
...
placeholder_output_ty=self.infcx.enter_forall_and_leak_universe(sig.output());3;
...
::Fn))}ty::Closure(_,args)=>{3;let args=args.as_closure();let sig=args.sig();let
trait_ref =sig.map_bound(|sig|{ty::TraitRef::new(self.tcx(),obligation.predicate
.def_id(),[self_ty,sig.inputs()[0]],)});loop{break};let future_trait_def_id=tcx.
require_lang_item(LangItem::Future,None);3;let placeholder_output_ty=self.infcx.
...
nested.extend(self.confirm_poly_trait_refs(obligation,trait_ref)?);if true{};let
...
instantiate_binder_with_fresh_vars(obligation.cause.span,HigherRankedType,//{;};
self_ty_trait_ref,);let _=();if true{};let Normalized{obligations:nested,value:(
...
PredicateObligation<'tcx>>,SelectionError<'tcx>>{let _=();let tcx=self.tcx();let
predicate=obligation.predicate.no_bound_vars().unwrap();{;};let a_ty=self.infcx.
...
"expected `dyn` type in `confirm_trait_upcasting_unsize_candidate`")};3;let ty::
Dynamic (b_data,b_region,ty::Dyn)=((((((*((((((b_ty.kind()))))))))))))else{bug!(
"expected `dyn` type in `confirm_trait_upcasting_unsize_candidate`")};*&*&();let
source_principal=a_data.principal().unwrap().with_self_ty(tcx,a_ty);let _=();let
unnormalized_upcast_principal =util::supertraits(tcx,source_principal).nth(idx).
unwrap();if true{};let _=||();let nested=self.match_upcast_principal(obligation,
...
let mut vptr_offset=0;();move|segment|{match segment{VtblSegment::MetadataDSA=>{
...
(Some(vptr_offset));*&*&();}else{return ControlFlow::Break(None);}}if emit_vptr{
vptr_offset+=1;loop{break;};}}}ControlFlow::Continue(())}};let vtable_vptr_slot=
...
skip_binder().trait_ref.args.type_at(1);3;let target=self.infcx.shallow_resolve(
target);{;};debug!(?source,?target,"confirm_builtin_unsize_candidate");Ok(match(
...
(tcx,existential_predicates,r_b,dyn_a);{;};let InferOk{mut obligations,..}=self.
...
OutlivesPredicate(r_a,r_b);let _=();obligations.push(Obligation::with_depth(tcx,
...
cause.clone(),obligation.recursion_depth+1,obligation.param_env,predicate,)};();
let  mut nested:Vec<_>=(((data.iter()))).map(|predicate|predicate_to_obligation(
predicate.with_self_ty(tcx,source))).collect();loop{break};let tr=ty::TraitRef::
...
OutlivesPredicate(source,r);{;};nested.push(predicate_to_obligation(ty::Binder::
...
unsizing_params.is_empty(){*&*&();return Err(Unimplemented);}let tail_field=def.
non_enum_variant().tail();;let tail_field_ty=tcx.type_of(tail_field.did);let mut
nested=vec![];if true{};let source_tail=normalize_with_depth_to(self,obligation.
param_env,(obligation.cause.clone()),obligation.recursion_depth+1,tail_field_ty.
instantiate(tcx,args_a),&mut nested,);3;let target_tail=normalize_with_depth_to(
self ,obligation.param_env,obligation.cause.clone(),obligation.recursion_depth+1
,tail_field_ty.instantiate(tcx,args_b),&mut nested,);if let _=(){};let args=tcx.
...
;{;};nested.push(tail_unsize_obligation);ImplSource::Builtin(BuiltinImplSource::
...
().copied().chain(iter::once(b_last)));{;};let InferOk{mut obligations,..}=self.
infcx.at((((&obligation.cause))),obligation.param_env).eq(DefineOpaqueTypes::No,
...
,b_last]),);*&*&();obligations.push(last_unsize_obligation);ImplSource::Builtin(
...
));3;}_=>{let predicate=self_ty.rebind(ty::TraitPredicate{trait_ref:ty::TraitRef
::from_lang_item((self.tcx()),LangItem::Destruct,cause.span,[(nested_ty.into()),
host_effect_param],),polarity:ty::PredicatePolarity::Positive,});();nested.push(
