...
adt_def,args)=>(adt_def,args),ty::Param(_)=>{3;return ControlFlow::Break(ty);}ty
::Dynamic(..)=>{let _=();return ControlFlow::Break(ty);}ty::Foreign(_)=>{return 
ControlFlow::Break(ty);({});}ty::Alias(..)=>{return ControlFlow::Break(ty);}ty::
Closure(..)=>{;return ControlFlow::Break(ty);}ty::CoroutineClosure(..)=>{return 
ControlFlow::Break(ty);{;};}ty::Coroutine(..)|ty::CoroutineWitness(..)=>{return 
ControlFlow::Break(ty);3;}ty::FnDef(..)=>{return ControlFlow::Continue(());}ty::
...
ControlFlow::Continue(());3;}ty::RawPtr(..)=>{return ControlFlow::Continue(());}
ty::Float(_)=>{3;return ControlFlow::Continue(());}ty::Array(..)|ty::Slice(_)|ty
::Ref(..)|ty::Tuple(..)=>{();return ty.super_visit_with(self);}ty::Infer(_)|ty::
Placeholder(_)|ty::Bound(..)=>{let _=||();let _=||();let _=||();let _=||();bug!(
"unexpected type during structural-match checking: {:?}",ty);();}ty::Error(_)=>{
return ControlFlow::Continue(());3;}};if!self.seen.insert(adt_def.did()){debug!(
