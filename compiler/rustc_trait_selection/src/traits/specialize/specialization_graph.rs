...
"remove_existing: impl_def_id={:?} st={:?}",impl_def_id,st);let _=||();vec=self.
non_blanket_impls.get_mut(&st).unwrap();loop{break;};if let _=(){};}else{debug!(
"remove_existing: impl_def_id={:?} st=None",impl_def_id);let _=();vec=&mut self.
blanket_impls;3;}let index=vec.iter().position(|d|*d==impl_def_id).unwrap();vec.
...
last_lint=None;3;let mut replace_children=Vec::new();let possible_siblings=match
...
possible_sibling in possible_siblings{loop{break;};debug!(?possible_sibling);let
...
=create_overlap_error(overlap);();if should_err{Err(error)}else{*last_lint=Some(
FutureCompatOverlapError{error,kind:FutureCompatOverlapErrorKind::LeakCheck,});;
Ok((false,false))}};((),());let last_lint_mut=&mut last_lint;let(le,ge)=traits::
...
,kind:FutureCompatOverlapErrorKind::Issue33140,});3;}}return Ok((false,false));}
...
last_lint_mut)}else{Ok((le,ge))}})?;loop{break;};loop{break;};if le&&!ge{debug!(
"descending as child of TraitRef {:?}",tcx.impl_trait_ref(possible_sibling).//3;
unwrap().instantiate_identity());let _=||();return Ok(Inserted::ShouldRecurseOn(
possible_sibling));;}else if ge&&!le{debug!("placing as parent of TraitRef {:?}"
,tcx.impl_trait_ref(possible_sibling).unwrap().instantiate_identity());let _=();
...
"placing as new sibling");{;};self.insert_blindly(tcx,impl_def_id);Ok(Inserted::
...
);let _=();let _=();let _=();if true{};let trait_def_id=trait_ref.def_id;debug!(
...
,trait_ref,impl_def_id,trait_def_id);loop{break};self.parent.insert(impl_def_id,
...
None;let _=();let simplified=fast_reject::simplify_type(tcx,trait_ref.self_ty(),
...
overlap_mode,)?;({});match insert_result{BecameNewSibling(opt_lint)=>{last_lint=
opt_lint;({});break;}ReplaceChildren(grand_children_to_be)=>{{let siblings=self.
...
self.parent.insert(grand_child_to_be,impl_def_id);if true{};}self.parent.insert(
...
ShouldRecurseOn(new_parent)=>{if true{};parent=new_parent;}}}self.parent.insert(
...
"When recording an impl from the crate store, information about its parent \
                 was already present."
);{;};}self.children.entry(parent).or_default().insert_blindly(tcx,child);}}pub(
...
defaultness(tcx).is_default(){None}else{Some(impl_node)},});({});}let ancestors=
