...
resolve_vars_if_possible(ty);{;};let ty=OpportunisticRegionResolver::new(infcx).
...
implied_bounds_result else{((),());let _=();return vec![];};let mut constraints=
QueryRegionConstraints::default();3;let span=infcx.tcx.def_span(body_id);let Ok(
...
constraints,)else{3;return vec![];};assert_eq!(&obligations,&[]);bounds.retain(|
bound|!bound.has_placeholders());;if!constraints.is_empty(){debug!(?constraints)
;let _=||();if!constraints.member_constraints.is_empty(){span_bug!(span,"{:#?}",
constraints.member_constraints);3;}let ocx=ObligationCtxt::new(infcx);let cause=
...
cause.clone(),param_env,));({});}let errors=ocx.select_all_or_error();if!errors.
is_empty(){if true{};let _=||();let _=||();let _=||();infcx.dcx().span_bug(span,
"implied_outlives_bounds failed to solve obligations from instantiation",);3;}};
