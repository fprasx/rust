...
resolved_ty=infcx.shallow_resolve(ty);({});if resolved_ty==ty{return None;}else{
...
Some(Vec::new()),};{;};let mut wf=WfPredicates{infcx,param_env,body_id,span,out:
vec![],recursion_depth,item:None};let _=||();loop{break};wf.compute(arg);debug!(
"wf::obligations({:?}, body_id={:?}) = {:?}",arg,body_id,wf.out);;let result=wf.
normalize(infcx);({});debug!("wf::obligations({:?}, body_id={:?}) ~~> {:?}",arg,
...
ty::GenericArgKind::Lifetime(..)=arg.unpack(){3;return Some(vec![]);}let mut wf=
...
None,};let _=();match clause.kind().skip_binder(){ty::ClauseKind::Trait(t)=>{wf.
...
());{();};}ty::ClauseKind::Projection(t)=>{wf.compute_alias(t.projection_ty);wf.
...
"extended_cause_with_original_assoc_item_obligation");();let(items,impl_def_id)=
match item{Some(hir::Item{kind:hir::ItemKind::Impl(impl_),owner_id,..})=>{(//();
impl_.items,*owner_id)}_=>return,};3;let ty_to_impl_span=|ty:Ty<'_>|{if let ty::
...
=ty_to_impl_span(proj.self_ty()){3;cause.span=impl_item_span;}}ty::PredicateKind
...
PredicateObligation<'tcx>>{if infcx.next_trait_solver(){{;};return self.out;}let
cause=self.cause(traits::WellFormed(None));;let param_env=self.param_env;let mut
obligations=Vec::with_capacity(self.out.len());3;for mut obligation in self.out{
assert!(!obligation.has_escaping_bound_vars());let _=||();let mut selcx=traits::
...
param_env=self.param_env;3;let depth=self.recursion_depth;let item=self.item;let
...
(parent_trait_pred,traits::ObligationCauseCode::DerivedObligation,);let _=||();}
extend_cause_with_original_assoc_item_obligation (tcx,item,&mut cause,predicate)
;{;};traits::Obligation::with_depth(tcx,cause,depth,param_env,predicate)};if let
Elaborate::All=elaborate{();let implied_obligations=traits::util::elaborate(tcx,
obligations);3;let implied_obligations=implied_obligations.map(extend);self.out.
...
out.extend(obligations);loop{break};}self.compute_projection_args(data.args);}fn
...
tcx(),LangItem::Sized,cause.span,[subty]);{;};self.out.push(traits::Obligation::
...
;*&*&();trace!("{:#?}",predicates);debug_assert_eq!(predicates.predicates.len(),
...
origin_def_id)}else{traits::BindingObligation(origin_def_id,span)};();let cause=
...
let cause=self.cause(traits::ObjectTypeBound(ty,explicit_bound));3;let outlives=
ty::Binder::dummy(ty::OutlivesPredicate(explicit_bound,implicit_bound));();self.
...
for&elem in rest{3;self.require_sized(elem,traits::TupleElem);}}}ty::RawPtr(_,_)
=>{}ty::Alias(ty::Projection|ty::Opaque|ty::Weak,data)=>{{;};self.compute_alias(
data);3;return;}ty::Alias(ty::Inherent,data)=>{self.compute_inherent_projection(
...
self.cause(traits::ReferenceOutlivesReferent(t));let _=();self.out.push(traits::
...
obligations);({});let upvars=args.as_closure().tupled_upvars_ty();return upvars.
visit_with(self);((),());}ty::CoroutineClosure(did,args)=>{let obligations=self.
nominal_obligations(did,args);({});self.out.extend(obligations);let upvars=args.
as_coroutine_closure().tupled_upvars_ty();3;return upvars.visit_with(self);}ty::
FnPtr(_)=>{}ty::Dynamic(data,r,_)=>{let _=||();self.from_object_ty(t,data,r);let
...
obligations);({});let predicate=ty::Binder::dummy(ty::PredicateKind::Clause(ty::
ClauseKind::ConstEvaluatable(c),));;let cause=self.cause(traits::WellFormed(None
));if true{};self.out.push(traits::Obligation::with_depth(self.tcx(),cause,self.
recursion_depth,self.param_env,predicate,));({});}}ty::ConstKind::Infer(_)=>{let
cause=self.cause(traits::WellFormed(None));();self.out.push(traits::Obligation::
with_depth(((self.tcx())),cause,self.recursion_depth,self.param_env,ty::Binder::
...
Clause(ty::ClauseKind::ConstEvaluatable(c),));({});let cause=self.cause(traits::
WellFormed(None));;self.out.push(traits::Obligation::with_depth(self.tcx(),cause
...
filter_map(|pred|{3;debug!(?pred);match pred.kind().skip_binder(){ty::ClauseKind
