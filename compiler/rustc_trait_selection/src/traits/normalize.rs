...
let mut selcx=SelectionContext::new(self.infcx);let _=||();let Normalized{value,
...
fulfill_cx.select_where_possible(self.infcx);if let _=(){};let value=self.infcx.
...
>>,{({});let mut obligations=Vec::new();let value=normalize_with_depth_to(selcx,
...
TyCtxt<'tcx>>,{{;};debug!(obligations.len=obligations.len());let mut normalizer=
AssocTypeNormalizer::new(selcx,param_env,cause,depth,obligations);();let result=
ensure_sufficient_stack(||normalizer.fold(value));();debug!(?result,obligations.
len=normalizer.obligations.len());3;debug!(?normalizer.obligations,);result}pub(
...
type_of(data.def_id);{;};let concrete_ty=generic_ty.instantiate(self.interner(),
...
,self.param_env,data,self.cause.clone(),self.depth,self.obligations,);3;debug!(?
...
data);((),());let _=();let data=data.fold_with(self);let normalized_ty=project::
...
ty::Weak=>{loop{break};let recursion_limit=self.interner().recursion_limit();if!
...
trait_selection_ty_alias_overflow);((),());},);}let infcx=self.selcx.infcx;self.
...
predicate,);loop{break};}let mut cause=self.cause.clone();cause.map_code(|code|{
ObligationCauseCode::TypeAlias(code,span,data.def_id)});3;Obligation::new(infcx.
tcx,cause,self.param_env,predicate)},),);*&*&();self.depth+=1;let res=infcx.tcx.
type_of(data.def_id).instantiate(infcx.tcx,data.args).fold_with(self);({});self.
depth-=1;{;};res}ty::Inherent if!data.has_escaping_bound_vars()=>{let data=data.
...
BoundVarReplacer::replace_bound_vars(infcx,&mut self.universes,data);3;let data=
data.fold_with(self);3;let ty=project::normalize_inherent_projection(self.selcx,
...
super_fold_with(self);loop{break};loop{break};debug!(?constant,?self.param_env);
