...
Option<(Span,Option<Span>,Vec<ArgKind>)>{3;let sm=self.tcx.sess.source_map();let
...
if is_closure{"closure"}else{"function"};{;};let args_str=|arguments:&[ArgKind],
...
to_owned()).collect::<Vec<String>>().join(", ");{;};err.span_suggestion_verbose(
...
Vec<String>>().join(", "))}else{String::new()},);();err.span_suggestion_verbose(
found_span,//((),());((),());((),());let _=();((),());let _=();((),());let _=();
...
TypeVariableOrigin{span:DUMMY_SP,kind:TypeVariableOriginKind::MiscVariable,});3;
let trait_ref=ty::TraitRef::new(self.tcx,trait_def_id,[ty.skip_binder(),var]);3;
let obligation=Obligation::new(self.tcx,(ObligationCause::dummy()),param_env,ty.
rebind(ty::TraitPredicate{trait_ref,polarity}),);();let ocx=ObligationCtxt::new(
