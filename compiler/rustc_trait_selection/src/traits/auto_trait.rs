...
::TraitRef::new(tcx,trait_did,[ty]);3;let infcx=tcx.infer_ctxt().build();let mut
selcx=SelectionContext::new(&infcx);({});for polarity in[true,false]{let result=
...
,trait_ref);3;return AutoTraitResult::ExplicitImpl;}}let infcx=tcx.infer_ctxt().
build();3;let mut fresh_preds=FxHashSet::default();let Some((new_env,user_env))=
self .evaluate_predicates(&infcx,trait_did,ty,orig_env,orig_env,&mut fresh_preds
)else{3;return AutoTraitResult::NegativeImpl;};let(full_env,full_user_env)=self.
...
"Failed to fully process: {ty:?} {trait_did:?} {orig_env:?}")});let _=();debug!(
"find_auto_trait_generics({:?}): fulfilling \
             with {:?}" ,trait_ref
,full_env);((),());infcx.clear_caches();let ocx=ObligationCtxt::new(&infcx);ocx.
register_bound(ObligationCause::dummy(),full_env,ty,trait_did);3;let errors=ocx.
select_all_or_error();*&*&();((),());*&*&();((),());if!errors.is_empty(){panic!(
"Unable to fulfill trait {trait_did:?} for '{ty:?}': {errors:?}");if true{};}let
outlives_env=OutlivesEnvironment::new(full_env);if true{};if true{};let _=infcx.
process_registered_region_obligations(&outlives_env,|ty,_|Ok(ty));let _=||();let
region_data =(((((((infcx.inner.borrow_mut()))).unwrap_region_constraints())))).
region_constraint_data().clone();({});let vid_to_region=self.map_vid_to_region(&
...
()));{();};}let mut select=SelectionContext::new(infcx);let mut already_visited=
...
=param_env.caller_bounds().iter().map(|c|c.as_predicate());if let _=(){};let mut
user_computed_preds :FxIndexSet<_>=((user_env.caller_bounds()).iter()).map(|c|c.
as_predicate()).collect();loop{break};let mut new_env=param_env;let dummy_cause=
ObligationCause::dummy();({});while let Some(pred)=predicates.pop_front(){infcx.
clear_caches();3;if!already_visited.insert(pred){continue;}let obligation=infcx.
resolve_vars_if_possible (Obligation::new(tcx,dummy_cause.clone(),new_env,pred,)
);let _=||();let result=select.poly_select(&obligation);match result{Ok(Some(ref
...
"evaluate_nested_obligations: found explicit negative impl\
                                        {:?}, bailing out"
,impl_def_id);loop{break};loop{break};return None;}}let obligations=impl_source.
...
predicates.push_back(pred);if true{};if true{};if true{};if true{};}else{debug!(
"evaluate_nested_obligations: `Unimplemented` found, bailing: \
                             {:?} {:?} {:?}"
,ty,pred,pred.skip_binder().trait_ref.args);loop{break};return None;}}_=>panic!(
"Unexpected error for '{ty:?}': {result:?}"),};3;let normalized_preds=elaborate(
...
as_clause())),param_env.reveal(),);();}let final_user_env=ty::ParamEnv::new(tcx.
...
ReBound(_,_),_)|(_,ty::ReVar(_))=>{({});return false;}(_,ty::ReBound(_,_))|(ty::
ReVar(_),_)=>{should_add_new=false}_=>{}}}}}true});let _=||();if should_add_new{
...
vid_map:FxHashMap<RegionTarget<'cx>,RegionDeps<'cx>>=FxHashMap::default();();let
...
RegionTarget::RegionVid(r1)).or_default();{;};deps1.larger.insert(RegionTarget::
...
();3;deps1.larger.insert(RegionTarget::RegionVid(vid));}let deps2=vid_map.entry(
RegionTarget::RegionVid(vid)).or_default();3;deps2.smaller.insert(RegionTarget::
Region(region));3;}&Constraint::VarSubReg(vid,region)=>{finished_map.insert(vid,
region);3;}&Constraint::RegSubReg(r1,r2)=>{{let deps1=vid_map.entry(RegionTarget
...
(RegionTarget::Region(r1));if true{};}}}while!vid_map.is_empty(){#[allow(rustc::
...
=v.into_mut();if true{};smaller_deps.larger.insert(*larger);smaller_deps.larger.
swap_remove(&target);{();};}if let Entry::Occupied(v)=vid_map.entry(*larger){let
larger_deps=v.into_mut();{();};larger_deps.smaller.insert(*smaller);larger_deps.
smaller.swap_remove(&target);{;};}}(&RegionTarget::RegionVid(v1),&RegionTarget::
Region(r1))=>{let _=||();finished_map.insert(v1,r1);}(&RegionTarget::Region(_),&
...
smaller_deps=v.into_mut();({});smaller_deps.larger.insert(*larger);smaller_deps.
larger.swap_remove(&target);3;}if let Entry::Occupied(v)=vid_map.entry(*larger){
let larger_deps=v.into_mut();3;larger_deps.smaller.insert(*smaller);larger_deps.
...
SelectionContext<'_,'tcx>,)->bool{3;let dummy_cause=ObligationCause::dummy();for
...
(obligation.predicate);if let _=(){};let bound_predicate=predicate.kind();match 
...
(ty::ClauseKind::Projection(p))=>{*&*&();let p=bound_predicate.rebind(p);debug!(
...
"evaluate_nested_obligations: adding projection predicate \
                            to computed_preds: {:?}"
,predicate);let _=();if true{};if self.is_self_referential_projection(p){debug!(
"evaluate_nested_obligations: encountered a projection
                                 predicate equating a type with itself! Skipping"
);let _=();let _=();}else{self.add_user_pred(computed_preds,predicate);}}debug!(
...
"evaluate_nested_obligations: Unable to unify predicate \
                                 '{:?}' '{:?}', bailing out"
,ty,e);((),());let _=();return false;}ProjectAndUnifyResult::Recursive=>{debug!(
...
,predicates,selcx,){3;return false;}}}ProjectAndUnifyResult::FailedNormalization
...
bound_predicate.rebind(binder);loop{break;};match(binder.no_bound_vars(),binder.
...
dummy_cause,);if true{};}(Some(ty::OutlivesPredicate(t_a,r_b)),_)=>{selcx.infcx.
...
{*&*&();((),());*&*&();((),());let tcx=self.tcx;let reported=tcx.dcx().emit_err(
