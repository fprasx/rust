...
AliasRelationDirection)>,)->QueryResult<'tcx>{{();};let tcx=self.tcx();let Goal{
param_env,predicate:(lhs,rhs,direction)}=goal;*&*&();((),());let Some(lhs)=self.
try_normalize_term(param_env,lhs)?else{if let _=(){};*&*&();((),());return self.
evaluate_added_goals_and_make_canonical_response(Certainty::overflow(true));3;};
...
self.next_term_infer_of_kind(term);3;self.add_normalizes_to_goal(Goal::new(self.
...
|this|{loop{break};let tcx=this.tcx();let normalized_ty=this.next_ty_infer();let
...
normalizes_to))}}let _=||();let _=||();this.try_evaluate_added_goals()?;Ok(this.
