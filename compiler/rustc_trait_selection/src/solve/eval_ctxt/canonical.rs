...
);({});let mut orig_values=Default::default();let canonical_goal=Canonicalizer::
...
try_evaluate_added_goals()?;if true{};let _=||();assert_eq!(self.tainted,Ok(()),
...
if cfg!(debug_assertions){({});assert!(normalizes_to_goals.is_empty());if goals.
...
goals_certainty);loop{break;};(certainty,NestedNormalizationGoals::empty())};let
external_constraints=self.compute_external_query_constraints(//((),());let _=();
normalization_nested_goals)?;{;};let(var_values,mut external_constraints)=(self.
...
.0.as_region().map_or(true,|re|re!=outlives.1));();let canonical=Canonicalizer::
...
NoSolution>{{;};self.infcx.leak_check(self.max_input_universe,None).map_err(|e|{
...
outlives.retain(|outlives|seen.insert(*outlives));{;};let mut opaque_types=self.
...
;let _=();if true{};let ExternalConstraintsData{region_constraints,opaque_types,
...
CanonicalVarValues<'tcx>{((),());let _=();let prev_universe=infcx.universe();let
universes_created_in_query=response.max_universe.index();let _=||();for _ in 0..
universes_created_in_query{((),());infcx.create_next_universe();}let var_values=
...
eq_structurally_relating_aliases(orig,response).unwrap();();assert!(obligations.
...
;3;let state=state.fold_with(&mut EagerResolver::new(ecx.infcx));Canonicalizer::
...
instantiate(infcx.tcx,&instantiation);();EvalCtxt::unify_query_var_values(infcx,
