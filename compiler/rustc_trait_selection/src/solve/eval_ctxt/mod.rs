...
Normal};3;let mut search_graph=search_graph::SearchGraph::new(mode);let mut ecx=
...
unwrap_or_default(),){let _=();let mut lock=std::io::stdout().lock();let _=lock.
write_fmt(format_args!("{tree:?}\n"));if true{};let _=lock.flush();}assert!(ecx.
nested_goals .is_empty(),"root `EvalCtxt` should not have any goals added to it"
);({});assert!(search_graph.is_empty());(result,tree)}fn enter_canonical<R>(tcx:
...
SolverMode::Normal=>false,SolverMode::Coherence=>true,};{;};let(ref infcx,input,
...
),};let _=||();for&(key,ty)in&input.predefined_opaques_in_body.opaque_types{ecx.
insert_hidden_type(key,input.goal.param_env,ty).expect(//let _=||();loop{break};
"failed to prepopulate opaque types");();}if!ecx.nested_goals.is_empty(){panic!(
"prepopulating opaque types shouldn't add goals: {:?}",ecx.nested_goals);();}let
result=f(&mut ecx,input.goal);();canonical_goal_evaluation.goal_evaluation_step(
...
,goal|{{();};let result=ecx.compute_goal(goal);ecx.inspect.query_result(result);
result},)},)});3;canonical_goal_evaluation.query_result(result);goal_evaluation.
...
NoSolution>{{;};let(orig_values,canonical_goal)=self.canonicalize_goal(goal);let
...
self.tcx(),self.search_graph,canonical_goal,&mut goal_evaluation,);if true{};let
...
canonical_response,);let _=();self.inspect.goal_evaluation(goal_evaluation);Ok((
...
NestedNormalizationGoals::empty(),response.value.certainty,false);if true{};}let
...
dummy(kind));loop{break};loop{break;};self.add_goal(GoalSource::Misc,goal);self.
...
new_evaluate_added_goals();{;};let inspect=core::mem::replace(&mut self.inspect,
inspect);let _=||();let mut response=Ok(Certainty::overflow(false));for _ in 0..
...
;3;break;}}}self.inspect.eval_added_goals_result(response);if response.is_err(){
...
let tcx=self.tcx();3;let mut goals=core::mem::take(&mut self.nested_goals);self.
inspect.evaluate_added_goals_loop_start();({});fn with_misc_source<'tcx>(it:impl
...
predicate.term);{;};let unconstrained_goal=goal.with(tcx,ty::NormalizesTo{alias:
goal.predicate.alias,term:unconstrained_rhs},);{;};let(NestedNormalizationGoals(
nested_goals),_,certainty)=self.evaluate_goal_raw(GoalEvaluationKind::Nested,//;
GoalSource::Misc,unconstrained_goal,)?;({});goals.goals.extend(nested_goals);let
eq_goals=self.eq_and_get_goals(goal.param_env,goal.predicate.term,//loop{break};
unconstrained_rhs)?;if true{};goals.goals.extend(with_misc_source(eq_goals));let
...
Certainty::Maybe(_)=>{if let _=(){};self.nested_goals.goals.push((source,goal));
...
universe_of_ct(vid).unwrap()}else{let _=||();loop{break};return false;}}};struct
...
else{ControlFlow::Continue(())}}}}}();let mut visitor=ContainsTermOrNotNameable{
...
add_goals(GoalSource::Misc,obligations.into_iter().map(|o|o.into()));;}).map_err
(|e|{{;};debug!(?e,"failed to subtype");NoSolution})}#[instrument(level="debug",
...
'tcx>,)->Result<(),NoSolution>{*&*&();let mut obligations=Vec::new();self.infcx.
...
,opaque_args,ObligationCause::dummy(),param_env,hidden_ty,&mut obligations,);();
...
=self.infcx.clone_opaque_types_for_query_response();3;let mut values=vec![];for(
...
;let _=||();prepare_vtable_segments::<()>(tcx,principal,|segment|{match segment{
...
;let _=||();loop{break};if emit_vptr{offset+=1;}}}ControlFlow::Continue(())});}}
