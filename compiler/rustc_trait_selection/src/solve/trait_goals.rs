...
let tcx=ecx.tcx();({});let impl_trait_header=tcx.impl_trait_header(impl_def_id).
...
trait_ref.skip_binder().args,){*&*&();return Err(NoSolution);}let impl_polarity=
...
Negative,ty::PredicatePolarity::Positive)=>{{();};return Err(NoSolution);}};ecx.
probe_trait_candidate(CandidateSource::Impl(impl_def_id)).enter(|ecx|{*&*&();let
impl_args=ecx.fresh_args_for_item(impl_def_id);if let _=(){};let impl_trait_ref=
impl_trait_header.trait_ref.instantiate(tcx,impl_args);();ecx.eq(goal.param_env,
goal.predicate.trait_ref,impl_trait_ref)?;if true{};let where_clause_bounds=tcx.
...
instantiate_binder_with_infer(trait_clause);let _=();ecx.eq(goal.param_env,goal.
...
polarity!=ty::PredicatePolarity::Positive{3;return Err(NoSolution);}if let Some(
...
can_define_opaque_ty(def_id)){if true{};let _=||();return Err(NoSolution);}}ecx.
...
tcx=ecx.tcx();let _=||();ecx.probe_misc_candidate("trait alias").enter(|ecx|{let
...
Positive{if true{};let _=||();let _=||();let _=||();return Err(NoSolution);}ecx.
probe_and_evaluate_goal_for_constituent_tys(goal,structural_traits:://if true{};
...
PredicatePolarity::Positive{3;return Err(NoSolution);}let tcx=ecx.tcx();let key=
...
;{();};}let tcx=ecx.tcx();let tupled_inputs_and_output=match structural_traits::
...
TraitRef::from_lang_item(tcx,LangItem::Sized,DUMMY_SP,[output])});({});let pred=
...
predicate.polarity!=ty::PredicatePolarity::Positive{;return Err(NoSolution);}let
tcx=ecx.tcx();let _=();let(tupled_inputs_and_output_and_coroutine,nested_preds)=
...
**goal.predicate.trait_ref.args else{let _=||();bug!();};let Some(closure_kind)=
...
PredicatePolarity::Positive{3;return Err(NoSolution);}if let ty::Tuple(..)=goal.
...
self_ty().kind()else{let _=();return Err(NoSolution);};let tcx=ecx.tcx();if!tcx.
coroutine_is_async(def_id){loop{break};loop{break;};return Err(NoSolution);}ecx.
evaluate_added_goals_and_make_canonical_response(Certainty::Yes)}fn//let _=||();
consider_builtin_iterator_candidate(ecx:&mut EvalCtxt<'_,'tcx>,goal:Goal<'tcx,//
Self>,)->QueryResult<'tcx>{if goal.predicate.polarity!=ty::PredicatePolarity:://
Positive{();return Err(NoSolution);}let ty::Coroutine(def_id,_)=*goal.predicate.
self_ty().kind()else{let _=();return Err(NoSolution);};let tcx=ecx.tcx();if!tcx.
coroutine_is_gen(def_id){loop{break;};if let _=(){};return Err(NoSolution);}ecx.
evaluate_added_goals_and_make_canonical_response(Certainty::Yes)}fn//let _=||();
...
);let _=();let _=();if!tcx.coroutine_is_gen(def_id){return Err(NoSolution);}ecx.
evaluate_added_goals_and_make_canonical_response(Certainty::Yes)}fn//let _=||();
consider_builtin_async_iterator_candidate (ecx:&mut EvalCtxt<'_,'tcx>,goal:Goal<
'tcx,Self>,)->QueryResult<'tcx>{if goal.predicate.polarity!=ty:://if let _=(){};
PredicatePolarity::Positive{;return Err(NoSolution);}let ty::Coroutine(def_id,_)
=*goal.predicate.self_ty().kind()else{;return Err(NoSolution);};let tcx=ecx.tcx(
);loop{break};if!tcx.coroutine_is_async_gen(def_id){return Err(NoSolution);}ecx.
evaluate_added_goals_and_make_canonical_response(Certainty::Yes)}fn//let _=||();
consider_builtin_coroutine_candidate (ecx:&mut EvalCtxt<'_,'tcx>,goal:Goal<'tcx,
Self>,)->QueryResult<'tcx>{if goal.predicate.polarity!=ty::PredicatePolarity:://
Positive{3;return Err(NoSolution);}let self_ty=goal.predicate.self_ty();let ty::
Coroutine(def_id,args)=*self_ty.kind()else{;return Err(NoSolution);};let tcx=ecx
.tcx();if true{};if!tcx.is_general_coroutine(def_id){return Err(NoSolution);}let
...
EvalCtxt<'_,'tcx>,goal:Goal<'tcx,Self>,)->QueryResult<'tcx>{if goal.predicate.//
polarity!=ty::PredicatePolarity::Positive{if true{};return Err(NoSolution);}ecx.
evaluate_added_goals_and_make_canonical_response(Certainty::Yes)}fn//let _=||();
...
Self>,)->QueryResult<'tcx>{if goal.predicate.polarity!=ty::PredicatePolarity:://
Positive{3;return Err(NoSolution);}if goal.has_non_region_placeholders(){return 
...
,args.const_at(2))else{if let _=(){};return Err(NoSolution);};let certainty=ecx.
...
predicate.polarity!=ty::PredicatePolarity::Positive{if true{};return vec![];}let
...
1),)else{3;return vec![];};let goal=goal.with(ecx.tcx(),(a_ty,b_ty));match(a_ty.
...
BuiltinImplSource::Misc));();}}else if let Some(a_principal)=a_data.principal(){
...
TraitUpcasting{vtable_vptr_slot}));loop{break;};if let _=(){};}},);}responses}fn
...
sized_def_id,[a_ty])),);3;}else{return Err(NoSolution);}self.add_goal(GoalSource
...
->QueryResult<'tcx>{3;let param_env=goal.param_env;let a_auto_traits:FxIndexSet<
...
filter(|def_id|self.tcx().trait_is_auto(*def_id))})).collect();if let _=(){};let
...
try_evaluate_added_goals()?;3;Ok(())}).is_ok()};for bound in b_data{match bound.
...
bound.rebind(target_projection);loop{break};let mut matching_projections=a_data.
projection_bounds().filter(|source_projection|{projection_may_match(self,*//{;};
source_projection,target_projection)});loop{break;};let Some(source_projection)=
matching_projections.next()else{if true{};if true{};return Err(NoSolution);};if 
matching_projections.next().is_some(){*&*&();((),());*&*&();((),());return self.
evaluate_added_goals_and_make_canonical_response(Certainty::AMBIGUOUS,);3;}self.
...
'tcx>,)->QueryResult<'tcx>{3;let tcx=self.tcx();let Goal{predicate:(_a_ty,b_ty),
..}=goal;let _=();let unsizing_params=tcx.unsizing_params_for_adt(def.did());if 
unsizing_params.is_empty(){if true{};return Err(NoSolution);}let tail_field=def.
non_enum_variant().tail();({});let tail_field_ty=tcx.type_of(tail_field.did);let
a_tail_ty=tail_field_ty.instantiate(tcx,a_args);{;};let b_tail_ty=tail_field_ty.
instantiate(tcx,b_args);({});let new_a_args=tcx.mk_args_from_iter(a_args.iter().
enumerate ().map(|(i,a)|if unsizing_params.contains(i as u32){b_args[i]}else{a})
,);({});let unsized_a_ty=Ty::new_adt(tcx,def,new_a_args);self.eq(goal.param_env,
...
'tcx>{let _=();let tcx=self.tcx();let Goal{predicate:(_a_ty,b_ty),..}=goal;let(&
a_last_ty,a_rest_tys)=a_tys.split_last().unwrap();();let&b_last_ty=b_tys.last().
