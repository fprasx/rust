...
tcx();{;};let ty::Dynamic(bounds,_,_)=*goal.predicate.self_ty().kind()else{bug!(
...
vec![];*&*&();((),());*&*&();((),());};if normalized_self_ty.is_ty_var(){debug!(
"self type has been normalized to infer");let _=();return self.forced_ambiguity(
MaybeCause::Ambiguity);let _=||();}let goal=goal.with(self.tcx(),goal.predicate.
with_self_ty(self.tcx(),normalized_self_ty));if true{};let _=||();let goal=self.
resolve_vars_if_possible(goal);let _=();let _=();let mut candidates=vec![];self.
...
assemble_alias_bound_candidates(goal,&mut candidates);if true{};let _=||();self.
...
Misc);if true{};if true{};let certainty=Certainty::Maybe(cause);let result=self.
evaluate_added_goals_and_make_canonical_response(certainty).unwrap();{;};let mut
dummy_probe=self.inspect.new_probe();let _=();dummy_probe.probe_kind(ProbeKind::
...
predicate.self_ty();if true{};let trait_impls=tcx.trait_impls_of(goal.predicate.
trait_def_id(tcx));({});let mut consider_impls_for_simplified_type=|simp|{if let
Some (impls_for_type)=trait_impls.non_blanket_impls().get(&simp){for&impl_def_id
in impls_for_type{if tcx.defaultness(impl_def_id).is_default(){3;return;}match G
...
I128|Isize):ty::IntTy;loop{break;};let(U8|U16|U32|U64|U128|Usize):ty::UintTy;let
...
SimplifiedType::Uint(Usize),];if true{};if true{};for simp in possible_integers{
consider_impls_for_simplified_type(simp);3;}}ty::Infer(ty::FloatVar(_))=>{let(ty
::FloatTy::F16|ty::FloatTy::F32|ty::FloatTy::F64|ty::FloatTy::F128);let _=();let
...
Goal<'tcx,G>,candidates:&mut Vec<Candidate<'tcx>>,){{();};let tcx=self.tcx();let
lang_items=tcx.lang_items();3;let trait_def_id=goal.predicate.trait_def_id(tcx);
...
>>,){3;let()=self.probe(|_|ProbeKind::NormalizedSelfTyAssembly).enter(|ecx|{ecx.
...
;if let _=(){};}Err(NoSolution)=>{}}}if kind!=ty::Projection{return;}match self.
...
.predicate.trait_def_id(tcx)).implement_via_object{{;};return;}let self_ty=goal.
...
){if true{};if true{};return;}for bound in bounds{match bound.skip_binder(){ty::
...
;3;let lazily_normalize_ty=|ty|ecx.structurally_normalize_ty(goal.param_env,ty);
...
.with(tcx,goal.predicate.trait_ref(tcx));3;let mut trait_candidates_from_env=Vec
...
trait_candidates_from_env);if true{};if!trait_candidates_from_env.is_empty(){let
...
CandidateSource::AliasBound=>true,});3;}Certainty::Maybe(cause)=>{debug!(?cause,
...
try_merge_responses(&responses){let _=();return Ok(result);}else{self.flounder(&
responses)}}}//((),());((),());((),());((),());((),());((),());((),());let _=();
