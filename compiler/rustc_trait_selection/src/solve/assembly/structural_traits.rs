...
NoSolution>{3;let tcx=ecx.tcx();match*ty.kind(){ty::Uint(_)|ty::Int(_)|ty::Bool|
...
))])}ty::Coroutine(_,args)=>{3;let coroutine_args=args.as_coroutine();Ok(vec![ty
...
NoSolution)}}ty::Closure(_,args)=>{{;};let closure_args=args.as_closure();match 
closure_args.kind_ty().to_opt_closure_kind(){Some(closure_kind)=>{if!//let _=();
closure_kind.extends(goal_kind){3;return Err(NoSolution);}}None=>{if goal_kind!=
...
goal_kind){3;return Err(NoSolution);}coroutine_closure_to_certain_coroutine(tcx,
...
let bound_sig=self_ty.fn_sig(tcx);let _=||();let sig=bound_sig.skip_binder();let
...
to_predicate(tcx),];if let _=(){};let future_output_def_id=tcx.associated_items(
future_trait_def_id).filter_by_name_unhygienic(sym::Output).next().unwrap().//3;
def_id;();let future_output_ty=Ty::new_projection(tcx,future_output_def_id,[sig.
...
output()])).to_predicate(tcx),];let _=();let kind_ty=args.kind_ty();if let Some(
...
require_lang_item(LangItem::AsyncFnKindHelper,None);3;nested.push(ty::TraitRef::
...
tcx=ecx.tcx();if let _=(){};let mut requirements=vec![];requirements.extend(tcx.
...
generics_require_sized_self(item.def_id){({});continue;}requirements.extend(tcx.
...
unwrap());let _=();}}let mut folder=ReplaceProjectionWith{ecx,param_env,mapping:
replace_projection_with,nested:vec![]};{;};let folded_requirements=requirements.
