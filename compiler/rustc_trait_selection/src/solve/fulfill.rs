...
self,obligation:PredicateObligation<'tcx>){{;};self.pending.push(obligation);}fn
...
=mem::take(&mut self.pending);let _=();obligations.append(&mut self.overflowed);
...
overflowed.extend(self.pending.extract_if(|o|{({});let goal=o.clone().into();let
...
num_open_snapshots());((),());let _=();self.obligations.register(obligation);}fn
...
).map(|obligation|fulfillment_error_for_stalled(infcx,obligation)).collect();();
...
usable_in_snapshot,infcx.num_open_snapshots());3;let mut errors=Vec::new();for i
...
fulfillment_error_for_no_solution(infcx,obligation));3;continue;}};has_changed|=
changed;;match certainty{Certainty::Yes=>{}Certainty::Maybe(_)=>self.obligations
.register(obligation),}}if!has_changed{3;break;}}errors}fn pending_obligations(&
