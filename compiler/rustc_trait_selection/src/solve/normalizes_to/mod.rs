...
assumption_projection_pred=ecx.instantiate_binder_with_infer(projection_pred);3;
ecx.eq(goal.param_env,goal.predicate.alias,assumption_projection_pred.//((),());
projection_ty,)?;let _=||();loop{break};ecx.instantiate_normalizes_to_term(goal,
assumption_projection_pred.term);loop{break};ecx.add_goals(GoalSource::Misc,tcx.
...
(tcx);({});let impl_trait_header=tcx.impl_trait_header(impl_def_id).unwrap();let
...
skip_binder().args,){((),());let _=();return Err(NoSolution);}let impl_polarity=
impl_trait_header.polarity;({});match impl_polarity{ty::ImplPolarity::Negative=>
...
enter(|ecx|{if let _=(){};let impl_args=ecx.fresh_args_for_item(impl_def_id);let
impl_trait_ref=impl_trait_header.trait_ref.instantiate(tcx,impl_args);();ecx.eq(
goal.param_env,goal_trait_ref,impl_trait_ref)?;({});let where_clause_bounds=tcx.
...
where_clause_bounds);({});ecx.add_goals(GoalSource::Misc,tcx.predicates_of(goal.
...
span_delayed_bug(tcx.def_span(assoc_def.item.def_id),reason);{;};let error_term=
...
error_response(ecx,"associated item has mismatched generic item arguments",);3;}
...
Self>,goal_kind:ty::ClosureKind,)->QueryResult<'tcx>{{();};let tcx=ecx.tcx();let
...
AMBIGUOUS);3;}};let output_is_sized_pred=tupled_inputs_and_output.map_bound(|(_,
...
.tcx();({});let env_region=match goal_kind{ty::ClosureKind::Fn|ty::ClosureKind::
...
return Err(NoSolution);if true{};};let Some(goal_kind)=goal_kind_ty.expect_ty().
to_opt_closure_kind()else{({});return Err(NoSolution);};if!closure_kind.extends(
goal_kind){3;return Err(NoSolution);}let upvars_ty=ty::CoroutineClosureSignature
...
Self>,)->QueryResult<'tcx>{let _=||();let tcx=ecx.tcx();let metadata_def_id=tcx.
require_lang_item(LangItem::Metadata,None);({});assert_eq!(metadata_def_id,goal.
...
ty::Dyn)=>{3;let dyn_metadata=tcx.require_lang_item(LangItem::DynMetadata,None);
...
GenericArg::from(goal.predicate.self_ty())],);{;};ecx.add_goal(GoalSource::Misc,
...
tail_def)=>{let _=||();let tail_ty=tail_def.ty(tcx,args);Ty::new_projection(tcx,
...
Self>,)->QueryResult<'tcx>{((),());let self_ty=goal.predicate.self_ty();let ty::
...
Coroutine(def_id,args)=*self_ty.kind()else{;return Err(NoSolution);};let tcx=ecx
.tcx();*&*&();if!tcx.coroutine_is_async_gen(def_id){return Err(NoSolution);}ecx.
probe_misc_candidate("builtin AsyncIterator kind").enter(|ecx|{;let expected_ty=
ecx.next_ty_infer();{;};let wrapped_expected_ty=Ty::new_adt(tcx,tcx.adt_def(tcx.
...
param_env,wrapped_expected_ty,yield_ty)?;{;};ecx.instantiate_normalizes_to_term(
...
self_ty();((),());let ty::Coroutine(def_id,args)=*self_ty.kind()else{return Err(
