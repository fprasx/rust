...
NormalizesTo<'tcx>>,)->QueryResult<'tcx>{3;let tcx=self.tcx();let inherent=goal.
...
type_of(impl_def_id).instantiate(tcx,impl_args),)?;3;let inherent_args=inherent.
rebase_inherent_args_onto_impl(impl_args,tcx);3;self.add_goals(GoalSource::Misc,
