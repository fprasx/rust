...
opaque_ty_def_id)=opaque_ty.def_id.as_local()else{3;return Err(NoSolution);};if!
...
=>{{;};return Err(NoSolution);}Ok(())=>{}}let opaque_type_key=ty::OpaqueTypeKey{
...
response);loop{break;};}else{return self.flounder(&matches);}}let expected=self.
structurally_normalize_ty(goal.param_env,expected)?;{;};if expected.is_ty_var(){
return self.evaluate_added_goals_and_make_canonical_response(Certainty:://{();};
AMBIGUOUS);3;}self.insert_hidden_type(opaque_type_key,goal.param_env,expected)?;
...
instantiate(tcx,opaque_ty.args);3;self.eq(goal.param_env,expected,actual)?;self.
