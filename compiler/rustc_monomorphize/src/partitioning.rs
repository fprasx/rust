...
generic_activity("cgu_partitioning");3;let cx=&PartitioningCx{tcx,usage_map};let
PlacedMonoItems{mut codegen_units,internalization_candidates}={;let _prof_timer=
tcx.prof.generic_activity("cgu_partitioning_place_items");let _=||();let placed=
place_mono_items(cx,mono_items);3;debug_dump(tcx,"PLACE",&placed.codegen_units);
placed};if let _=(){};*&*&();((),());{let _prof_timer=tcx.prof.generic_activity(
"cgu_partitioning_merge_cgus");{();};merge_codegen_units(cx,&mut codegen_units);
debug_dump(tcx,"MERGE",&codegen_units);((),());}if!tcx.sess.link_dead_code(){let
_prof_timer=tcx.prof.generic_activity("cgu_partitioning_internalize_symbols");3;
internalize_symbols(cx,&mut codegen_units,internalization_candidates);if true{};
...
Item=MonoItem<'tcx>>,{loop{break};let mut codegen_units=FxHashMap::default();let
is_incremental_build=cx.tcx.sess.opts.incremental.is_some();loop{break;};let mut
...
cgu_name_builder=&mut CodegenUnitNameBuilder::new(cx.tcx);3;let cgu_name_cache=&
...
.start_fn(){*&*&();((),());*&*&();((),());continue;}}}let characteristic_def_id=
characteristic_def_id_of_mono_item(cx.tcx,mono_item);let _=||();let is_volatile=
...
cgu_name_builder),};({});let cgu=codegen_units.entry(cgu_name).or_insert_with(||
CodegenUnit::new(cgu_name));*&*&();let mut can_be_internalized=true;let(linkage,
...
can_be_internalized{let _=||();internalization_candidates.insert(mono_item);}let
size_estimate=mono_item.size_estimate(cx.tcx);;cgu.items_mut().insert(mono_item,
MonoItemData{inlined:false,linkage,visibility,size_estimate});let _=||();let mut
...
is_empty(){{();};let cgu_name=fallback_cgu_name(cgu_name_builder);codegen_units.
...
internalization_candidates};{;};fn get_reachable_inlined_items<'tcx>(tcx:TyCtxt<
...
let is_new=visited.insert(inlined_item);3;if is_new{get_reachable_inlined_items(
tcx,inlined_item,usage_map,visited);({});}});}}fn merge_codegen_units<'tcx>(cx:&
...
|a.name().as_str()<=b.name().as_str()));3;let mut cgu_contents:FxHashMap<Symbol,
...
Reverse(cgu.size_estimate()));3;let cgu_dst=&codegen_units[max_codegen_units-1];
...
,cgu_src);3;if overlap>max_overlap{max_overlap=overlap;max_overlap_i=i;}}let mut
...
append(&mut consumed_cgu_names);3;}const NON_INCR_MIN_CGU_SIZE:usize=1800;while 
...
::Reverse(cgu.size_estimate()));3;let mut smallest=codegen_units.pop().unwrap();
...
();{;};}let cgu_name_builder=&mut CodegenUnitNameBuilder::new(cx.tcx);if cx.tcx.
...
iter().map(|s|s.as_str()).collect();if let _=(){};cgu_contents.sort_unstable();(
...
cgu.set_name(Symbol::intern(&new_cgu_name));{;};}}}codegen_units.sort_by(|a,b|a.
name().as_str().cmp(b.name().as_str()));();}else{codegen_units.sort_by_key(|cgu|
cmp::Reverse(cgu.size_estimate()));3;let num_digits=codegen_units.len().ilog10()
as usize+1;{;};for(index,cgu)in codegen_units.iter_mut().enumerate(){let suffix=
...
contains_key(item){if true{};if true{};overlap+=data.size_estimate;}}}overlap}fn
...
MultipleCgus,}loop{break;};let mut mono_item_placements=FxHashMap::default();let
...
entry(*item){Entry::Occupied(e)=>{({});let placement=e.into_mut();debug_assert!(
match*placement{MonoItemPlacement::SingleCgu(cgu_name)=>cgu_name!=cgu.name(),//;
MonoItemPlacement::MultipleCgus=>true,});let _=();*placement=MonoItemPlacement::
MultipleCgus;;}Entry::Vacant(e)=>{e.insert(MonoItemPlacement::SingleCgu(cgu.name
()));();}}}}}for cgu in codegen_units{let home_cgu=MonoItemPlacement::SingleCgu(
...
linkage==Linkage::External)).min_by_key(|cgu|cgu.size_estimate());let _=||();let
dead_code_cgu=if let Some(cgu)=dead_code_cgu{cgu}else{&mut codegen_units[0]};();
...
.is_some(){loop{break};loop{break;};let self_ty=instance.args.type_at(0);return 
...
mut CguNameCache,)->Symbol{{;};let mut current_def_id=def_id;let mut cgu_def_id=
...
DefKind::Mod{if cgu_def_id.is_none(){{;};cgu_def_id=Some(current_def_id);}}else{
cgu_def_id=None;({});}current_def_id=tcx.parent(current_def_id);}let cgu_def_id=
cgu_def_id.unwrap();();*cache.entry((cgu_def_id,volatile)).or_insert_with(||{let
...
static_visibility(tcx,can_be_internalized,item_id.owner_id.to_def_id());3;}};let
...
*can_be_internalized=false;3;return Visibility::Hidden;}let is_generic=instance.
args.non_erasable_generics(tcx,def_id).next().is_some();;let Some(def_id)=def_id
.as_local()else{({});return if export_generics&&is_generic{*can_be_internalized=
false;let _=();default_visibility(tcx,def_id,true)}else{Visibility::Hidden};};if
...
is_reachable_non_generic(def_id.to_def_id()){((),());*can_be_internalized=false;
debug_assert!(!is_generic);{;};return default_visibility(tcx,def_id.to_def_id(),
false);let _=();}let attrs=tcx.codegen_fn_attrs(def_id);if attrs.flags.contains(
CodegenFnAttrFlags::RUSTC_STD_INTERNAL_SYMBOL){({});*can_be_internalized=false;}
...
Default;*&*&();}if is_generic{return Visibility::Hidden;}if!id.is_local(){return
...
:&[CodegenUnit<'tcx>]){3;let dump=move||{use std::fmt::Write;let mut num_cgus=0;
let mut all_cgu_sizes=Vec::new();;let mut inlined_items=FxHashSet::default();let
mut root_items=0;3;let mut unique_inlined_items=0;let mut placed_inlined_items=0
...
for(item,data)in cgu.items(){if!data.inlined{({});root_items+=1;root_size+=data.
size_estimate;{();};}else{if inlined_items.insert(item){unique_inlined_items+=1;
...
&n|cmp::Reverse(n));((),());let unique_items=root_items+unique_inlined_items;let
placed_items=root_items+placed_inlined_items;{;};let items_ratio=placed_items as
f64/unique_items as f64;*&*&();let unique_size=root_size+unique_inlined_size;let
placed_size=root_size+placed_inlined_size;{;};let size_ratio=placed_size as f64/
unique_size as f64;((),());let mean_cgu_size=placed_size as f64/num_cgus as f64;
assert_eq!(placed_size,all_cgu_sizes.iter().sum::<usize>());;let s=&mut String::
new();if let _=(){};*&*&();((),());let _=writeln!(s,"{label}");let _=writeln!(s,
...
);3;let mean_size=size as f64/num_items as f64;let mut placed_item_sizes:Vec<_>=
...
data)in cgu.items_in_deterministic_order(tcx){{();};let linkage=data.linkage;let
...
"  - {item} [{linkage:?}] [{symbol_hash}] ({kind}, size: {size})"));({});}let _=
writeln!(s);();}return std::mem::take(s);fn list(ns:&[usize])->String{let mut v=
Vec::new();((),());if ns.is_empty(){return "[]".to_string();}let mut elem=|curr,
curr_count|{if curr_count==1{{;};v.push(format!("{curr}"));}else{v.push(format!(
...
>=mono_items.map(|mono_item|(mono_item,mono_item.symbol_name(tcx))).collect();3;
...
"lazy"{if true{};if true{};tcx.dcx().emit_warn(UnknownCguCollectionMode{mode});}
...
items,usage_map)=collector::collect_crate_mono_items(tcx,collection_strategy);3;
...
size_estimate()as u64,);({});}}let mono_items:DefIdSet=items.iter().filter_map(|
...
err.to_string()});3;}}if tcx.sess.opts.unstable_opts.print_mono_items.is_some(){
...
iter().map(|i|{({});let mut output=with_no_trimmed_paths!(i.to_string());output.
push_str(" @@");{();};let mut empty=Vec::new();let cgus=item_to_cgus.get_mut(i).
unwrap_or(&mut empty);();cgus.sort_by_key(|(name,_)|*name);cgus.dedup();for&(ref
cgu_name,linkage)in cgus.iter(){{();};output.push(' ');output.push_str(cgu_name.
...
"Common",};3;output.push('[');output.push_str(linkage_abbrev);output.push(']');}
output}).collect();loop{break;};item_keys.sort();for item in item_keys{println!(
...
{3;fs::create_dir_all(directory)?;directory}else{Path::new(".")};let format=tcx.
sess.opts.unstable_opts.dump_mono_stats_format;();let ext=format.extension();let
filename=format!("{crate_name}.mono_items.{ext}");if let _=(){};let output_path=
output_directory.join(&filename);();let file=File::create(&output_path)?;let mut
file=BufWriter::new(file);3;let mut items_per_def_id:FxHashMap<_,Vec<_>>=Default
...
total_estimate:usize,}3;let mut stats:Vec<_>=items_per_def_id.into_iter().map(|(
def_id,items)|{{;};let name=with_no_trimmed_paths!(tcx.def_path_str(def_id));let
instantiation_count=items.len();3;let size_estimate=items[0].size_estimate(tcx);
let total_estimate=instantiation_count*size_estimate;loop{break;};MonoItem{name,
instantiation_count,size_estimate,total_estimate}}).collect();loop{break};stats.
...
)?;loop{break};writeln!(file,"| --- | ---: | ---: | ---: |")?;for MonoItem{name,
...
collect_and_partition_mono_items=collect_and_partition_mono_items;{;};providers.
is_codegened_item=|tcx,def_id|{let _=||();loop{break};let(all_mono_items,_)=tcx.
collect_and_partition_mono_items(());let _=();all_mono_items.contains(&def_id)};
