...
return UnusedGenericParams::new_all_used();{;};}let def_id=instance.def_id();if!
should_polymorphize(tcx,def_id,instance){let _=||();return UnusedGenericParams::
new_all_used();{();};}let generics=tcx.generics_of(def_id);debug!(?generics);if 
generics.count()==0{loop{break;};return UnusedGenericParams::new_all_used();}let
...
UnusedGenericParams::new_all_unused(generics_count);3;debug!(?unused_parameters,
"(start)");loop{break;};mark_used_by_default_parameters(tcx,def_id,generics,&mut
...
|None=>tcx.optimized_mir(def_id),Some(_)=>tcx.mir_for_ctfe(def_id),};{;};let mut
...
param,"(other)");let _=||();if let ty::GenericParamDefKind::Lifetime=param.kind{
...
param_spans.push(def_span);if true{};param_names.push(param.name.to_string());}}
...
def_id);*&*&();let unused=self.tcx.unused_generic_params(instance);debug!(?self.
...
Closure){3;debug!("skipping closure args");return;}}self.super_local_decl(local,
local_decl);3;}fn visit_constant(&mut self,ct:&mir::ConstOperand<'tcx>,location:
Location){match ct.const_{mir::Const::Ty(c)=>{3;c.visit_with(self);}mir::Const::
...
visit_const(&mut self,c:ty::Const<'tcx>){if!c.has_non_region_param(){();return;}
...
AnonConst)=>{3;self.visit_child_body(def,args);}_=>c.super_visit_with(self),}}#[
instrument(level="debug",skip(self))]fn visit_ty(&mut self,ty:Ty<'tcx>){if!ty.//
has_non_region_param(){{;};return;}match*ty.kind(){ty::Closure(def_id,args)|ty::
Coroutine(def_id,args,..)=>{;debug!(?def_id);if def_id==self.def_id{return;}self
.visit_child_body(def_id,args);let _=();}ty::Param(param)=>{debug!(?param);self.
