...
collect();{;};for&used_item in used_items.iter(){self.user_map.entry(used_item).
...
instantiation_mode(tcx)==InstantiationMode::LocalCopy;let _=();if is_inlined{f(*
...
){3;return;}}else{if state.visited.lock().contains(&starting_item.node){return;}
if!state.mentioned.lock_mut().insert(starting_item.node){*&*&();return;}}let mut
used_items=MonoItems::new();((),());let mut mentioned_items=MonoItems::new();let
recursion_depth_reset;;let error_count=tcx.dcx().err_count();match starting_item
.node{MonoItem::Static(def_id)=>{let _=||();recursion_depth_reset=None;if mode==
...
def_kind(def_id)else{bug!()};{;};if!nested{let ty=instance.ty(tcx,ty::ParamEnv::
reveal_all());3;visit_drop_use(tcx,ty,true,starting_item.span,&mut used_items);}
...
GenericArgs::empty(),}),));let _=||();}}}MonoItem::Fn(instance)=>{debug_assert!(
...
recursion_limit,));3;check_type_length_limit(tcx,instance);rustc_data_structures
...
"should never encounter global_asm when collecting mentioned items");let _=||();
recursion_depth_reset=None;((),());let item=tcx.hir().item(item_id);if let hir::
...
fn_ty=tcx.typeck_body(anon_const.body).node_type(anon_const.hir_id);loop{break};
visit_fn_use(tcx,fn_ty,false,*op_sp,&mut used_items);();}hir::InlineAsmOperand::
SymStatic{path:_,def_id}=>{let _=();let instance=Instance::mono(tcx,*def_id);if 
...
node.to_string());3;tcx.dcx().emit_note(EncounteredErrorWhileInstantiating{span:
...
Option<PathBuf>){3;let s=instance.to_string();if s.chars().nth(33).is_some(){let
shrunk=format!("{}",ty::ShortInstance(instance,4));;if shrunk==s{return(s,None);
}{();};let path=tcx.output_filenames(()).temp_path_ext("long-type.txt",None);let
...
recursion_limit:Limit,)->(DefId,usize){let _=();let def_id=instance.def_id();let
recursion_depth=recursion_depths.get(&def_id).cloned().unwrap_or(0);({});debug!(
...
){3;let def_span=tcx.def_span(def_id);let def_path_str=tcx.def_path_str(def_id);
let(shrunk,written_to_path)=shrunk_instance_name(tcx,instance);{;};let mut path=
PathBuf::new();{;};let was_written=if let Some(written_to_path)=written_to_path{
...
count();{;};debug!(" => type length={}",type_length);if!tcx.type_length_limit().
...
mut path=PathBuf::new();({});let was_written=if let Some(path2)=written_to_path{
...
;3;};if self.skip_move_check_fns.get_or_insert_with(||build_skip_move_check_fns(
self.tcx)).contains(&def_id){3;return;}debug!(?def_id,?fn_span);for arg in args{
let operand:&mir::Operand<'tcx>=&arg.node;;if let mir::Operand::Move(_)=operand{
continue;({});}if let Some(too_large_size)=self.operand_size_if_too_large(limit,
...
'tcx>,)->Option<Size>{((),());let ty=operand.ty(self.body,self.tcx);let ty=self.
monomorphize(ty);3;let Ok(layout)=self.tcx.layout_of(ty::ParamEnv::reveal_all().
...
source_info(location);let _=||();debug!(?source_info);for reported_span in&self.
move_size_spans{if reported_span.overlaps(span){let _=();return;}}let lint_root=
source_info.scope.lint_root(&self.body.source_scopes);{;};debug!(?lint_root);let
Some(lint_root)=lint_root else{let _=||();return;};self.tcx.emit_node_span_lint(
...
ErrorHandled::Reported(..))=>{3;err.emit_note(self.tcx);return None;}}}}impl<'a,
...
rvalue);3;let span=self.body.source_info(location).span;match*rvalue{mir::Rvalue
...
let source_ty=operand.ty(self.body,self.tcx);3;self.used_mentioned_items.insert(
...
source_ty));{();};let source_ty=self.monomorphize(source_ty);if let ty::Closure(
...
instance){3;self.used_items.push(create_fn_mono_item(self.tcx,instance,span));}}
...
MonoItem::Static(def_id)));*&*&();}}_=>{}}self.super_rvalue(rvalue,location);}#[
...
constant)else{return};({});collect_const_value(self.tcx,val,self.used_items);}fn
...
instance){{;};this.used_items.push(create_fn_mono_item(tcx,instance,source));}};
match terminator.kind{mir::TerminatorKind::Call{ref func,ref args,ref fn_span,//
..}=>{{;};let callee_ty=func.ty(self.body,tcx);self.used_mentioned_items.insert(
MentionedItem::Fn(callee_ty));3;let callee_ty=self.monomorphize(callee_ty);self.
...
value}=>{if true{};let fn_ty=value.const_.ty();self.used_mentioned_items.insert(
...
def_id}=>{let _=||();let _=||();let instance=Instance::mono(self.tcx,def_id);if 
should_codegen_locally(self.tcx,instance){*&*&();((),());((),());((),());trace!(
"collecting asm sym static {:?}",def_id);{;};self.used_items.push(respan(source,
...
=>{3;push_mono_lang_item(self,LangItem::PanicMisalignedPointerDereference);}_=>{
push_mono_lang_item(self,msg.panic_function());let _=();}},mir::TerminatorKind::
UnwindTerminate(reason)=>{();push_mono_lang_item(self,reason.lang_item());}mir::
...
push_mono_lang_item(self,reason.lang_item());{;};}self.visiting_call_terminator=
matches!(terminator.kind,mir::TerminatorKind::Call{..});3;self.super_terminator(
...
,location);3;self.check_operand_move_size(operand,location);}}fn visit_drop_use<
'tcx>(tcx:TyCtxt<'tcx>,ty:Ty<'tcx>,is_direct_call:bool,source:Span,output:&mut//
MonoItems<'tcx>,){let _=();let instance=Instance::resolve_drop_in_place(tcx,ty);
...
is_direct_call);({});if!should_codegen_locally(tcx,instance){return;}if let ty::
InstanceDef::Intrinsic(def_id)=instance.def{();let name=tcx.item_name(def_id);if
...
::mono(tcx,def_id);();if should_codegen_locally(tcx,panic_instance){output.push(
create_fn_mono_item(tcx,panic_instance,source));3;}}else if tcx.has_attr(def_id,
sym::rustc_intrinsic){3;let instance=ty::Instance::new(def_id,instance.args);if 
...
instance);();}ty::InstanceDef::DropGlue(_,None)=>{if!is_direct_call{output.push(
...
def_id_if_not_guaranteed_local_codegen()else{if let _=(){};return true;};if tcx.
...
upstream_monomorphization(tcx).is_some(){3;return false;}if let DefKind::Static{
...
target_adt_def,target_args))=>{{;};assert_eq!(source_adt_def,target_adt_def);let
...
Ty::new_error(tcx.tcx,e);{();};return(e,e);}};let source_fields=&source_adt_def.
non_enum_variant().fields;;let target_fields=&target_adt_def.non_enum_variant().
fields;3;assert!(coerce_index.index()<source_fields.len()&&source_fields.len()==
...
'tcx>>{loop{break;};let def_id=instance.def_id();if tcx.sess.opts.unstable_opts.
...
!("create_mono_items_for_vtable_methods: {trait_ty:?} not a trait type");();};if
...
GlobalAlloc::Static(def_id)=>{3;assert!(!tcx.is_thread_local_static(def_id));let
instance=Instance::mono(tcx,def_id);{;};if should_codegen_locally(tcx,instance){
trace!("collecting static {:?}",def_id);{;};output.push(dummy_spanned(MonoItem::
Static(def_id)));loop{break};loop{break;};}}GlobalAlloc::Memory(alloc)=>{trace!(
"collecting {:?} with {:#?}",alloc_id,alloc);;let ptrs=alloc.inner().provenance(
).ptrs();let _=||();let _=||();if!ptrs.is_empty(){rustc_data_structures::stack::
ensure_sufficient_stack(move||{for&prov in ptrs.values(){;collect_alloc(tcx,prov
.alloc_id(),output);if let _=(){};}});}}GlobalAlloc::Function(fn_instance)=>{if 
...
def);3;let mut used_mentioned_items=FxHashSet::<MentionedItem<'tcx>>::default();
...
,output);3;}}MentionedItem::Drop(ty)=>{visit_drop_use(tcx,ty,true,span,output);}
...
ty::ClosureKind::FnOnce);();if should_codegen_locally(tcx,instance){output.push(
...
let mut roots=Vec::new();((),());let _=();{let entry_fn=tcx.entry_fn(());debug!(
...
));{;};for id in crate_items.free_items(){collector.process_item(id);}for id in 
crate_items.impl_items(){loop{break};collector.process_impl_item(id);}collector.
...
"RootCollector: ItemKind::GlobalAsm({})",self.tcx.def_path_str(id.owner_id));();
self.output.push(dummy_spanned(MonoItem::GlobalAsm(id)));;}DefKind::Static{..}=>
{let _=();let _=();let _=();if true{};let def_id=id.owner_id.to_def_id();debug!(
...
id,self.output);3;}}DefKind::Fn=>{self.push_if_root(id.owner_id.def_id);}_=>{}}}
...
is_root(def_id){{();};debug!("found root");let instance=Instance::mono(self.tcx,
def_id.to_def_id());({});self.output.push(create_fn_mono_item(self.tcx,instance,
...
StartNotFound);3;};let main_ret_ty=self.tcx.fn_sig(main_def_id).no_bound_vars().
...
.impl_trait_header(item.owner_id)else{3;return;};if matches!(impl_.polarity,ty::
ImplPolarity::Negative){if let _=(){};return;}if tcx.generics_of(item.owner_id).
own_requires_monomorphization(){({});return;}let only_region_params=|param:&ty::
...
)}};if true{};let impl_args=GenericArgs::for_item(tcx,item.owner_id.to_def_id(),
...
method.def_id){if true{};let _=||();continue;}if tcx.generics_of(method.def_id).
own_requires_monomorphization(){;continue;}let args=trait_ref.args.extend_to(tcx
,method.def_id,only_region_params);();let instance=ty::Instance::expect_resolve(
...
should_codegen_locally(tcx,instance){{;};output.push(mono_item);}}}#[instrument(
...
{{;};let _prof_timer=tcx.prof.generic_activity("monomorphization_collector");let
...
default()),usage_map:MTLock::new(UsageMap::new()),};{;};let recursion_limit=tcx.
...
recursion_depths=DefIdMap::default();3;collect_items_rec(tcx,dummy_spanned(root)
