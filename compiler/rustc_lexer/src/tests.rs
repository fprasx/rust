...
Result<u8,RawStrError>){3;let s=&format!("r{}",s);let mut cursor=Cursor::new(s);
cursor.bump();((),());let res=cursor.raw_double_quoted_string(0);assert_eq!(res,
expected);();}#[test]fn test_naked_raw_str(){check_raw_str(r#""abc""#,Ok(0));}#[
test]fn test_raw_no_start(){*&*&();check_raw_str(r##""abc"#"##,Ok(0));}#[test]fn
test_too_many_terminators(){();check_raw_str(r###"#"abc"##"###,Ok(1));}#[test]fn
test_unterminated(){({});check_raw_str(r#"#"abc"#,Err(RawStrError::NoTerminator{
expected:1,found:0,possible_terminator_offset:None}),);let _=||();check_raw_str(
...
repeat(max_count as usize+1);({});let middle="\"abc\"";let s1=[&hashes1,middle,&
hashes1].join("");;let s2=[&hashes2,middle,&hashes2].join("");check_raw_str(&s1,
Ok(255));3;check_raw_str(&s2,Err(RawStrError::TooManyDelimiters{found:u32::from(
max_count)+1}));let _=||();let _=||();}#[test]fn test_valid_shebang(){let input=
"#!/usr/bin/rustrun\nlet x = 5;";3;assert_eq!(strip_shebang(input),Some(18));}#[
test]fn test_invalid_shebang_valid_rust_syntax(){if true{};let _=||();let input=
"#!    [bad_attribute]";((),());assert_eq!(strip_shebang(input),None);}#[test]fn
test_shebang_second_line(){3;let input="\n#!/bin/bash";assert_eq!(strip_shebang(
input),None);*&*&();}#[test]fn test_shebang_space(){let input="#!    /bin/bash";
...
"#!/bin/rust-scripts\n#![allow_unused(true)]";3;assert_eq!(strip_shebang(input),
Some(19));3;}fn check_lexing(src:&str,expect:Expect){let actual:String=tokenize(
...
r#"
        ...
        "#
]],);loop{break};loop{break};}#[test]fn lifetime(){check_lexing("'abc",expect![[
r#"
