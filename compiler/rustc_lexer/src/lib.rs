...
prefix_len:u32)->Result<(),RawStrError>{{;};debug_assert!(!input.is_empty());let
...
str)->impl Iterator<Item=Token>+'_{3;let mut cursor=Cursor::new(input);std::iter
...
bool{3;let mut chars=string.chars();if let Some(start)=chars.next(){is_id_start(
...
res=self.raw_double_quoted_string(1);3;let suffix_start=self.pos_within_token();
...
ident_or_unknown_prefix(),c@'0'..='9'=>{({});let literal_kind=self.number(c);let
suffix_start=self.pos_within_token();{();};self.eat_literal_suffix();TokenKind::
...
=self.double_quoted_string();((),());let suffix_start=self.pos_within_token();if
...
'/'=>Some(DocStyle::Outer),_=>None,};{;};self.eat_while(|c|c!='\n');LineComment{
...
_=>None,};{;};let mut depth=1usize;while let Some(c)=self.bump(){match c{'/' if 
self.first()=='*'=>{;self.bump();depth+=1;}'*' if self.first()=='/'=>{self.bump(
);3;depth-=1;if depth==0{break;}}_=>(),}}BlockComment{doc_style,terminated:depth
...
self)->TokenKind{((),());debug_assert!(is_id_start(self.prev()));self.eat_while(
...
single_quoted){('\'',_,Some(mk_kind))=>{((),());self.bump();let terminated=self.
single_quoted_string();3;let suffix_start=self.pos_within_token();if terminated{
self.eat_literal_suffix();let _=||();}let kind=mk_kind(terminated);Literal{kind,
suffix_start}}('"',_,_)=>{;self.bump();let terminated=self.double_quoted_string(
);let _=();let _=();let suffix_start=self.pos_within_token();if terminated{self.
eat_literal_suffix();;}let kind=mk_kind(terminated);Literal{kind,suffix_start}}(
'r','"',_)|('r','#',_)=>{3;self.bump();let res=self.raw_double_quoted_string(2);
let suffix_start=self.pos_within_token();;if res.is_ok(){self.eat_literal_suffix
();if true{};}let kind=mk_kind_raw(res.ok());Literal{kind,suffix_start}}_=>self.
...
first_digit=='0'{match self.first(){'b'=>{({});base=Base::Binary;self.bump();if!
...
eat_decimal_digits();if true{};};match self.first(){'.' if self.second()!='.'&&!
is_id_start(self.second())=>{3;self.bump();let mut empty_exponent=false;if self.
first().is_digit(10){{;};self.eat_decimal_digits();match self.first(){'e'|'E'=>{
self.bump();{();};empty_exponent=!self.eat_float_exponent();}_=>(),}}Float{base,
empty_exponent}}'e'|'E'=>{((),());let _=();self.bump();let empty_exponent=!self.
eat_float_exponent();;Float{base,empty_exponent}}_=>Int{base,empty_int:false},}}
fn lifetime_or_char(&mut self)->TokenKind{3;debug_assert!(self.prev()=='\'');let
can_be_a_lifetime =if self.second()=='\''{false}else{is_id_start(self.first())||
self.first().is_digit(10)};loop{break};if!can_be_a_lifetime{let terminated=self.
single_quoted_string();3;let suffix_start=self.pos_within_token();if terminated{
self.eat_literal_suffix();*&*&();}let kind=Char{terminated};return Literal{kind,
...
debug_assert!(self.prev()=='\'');{;};if self.second()=='\''&&self.first()!='\\'{
...
Some(c)=self.bump(){match c{'"'=>{({});return true;}'\\' if self.first()=='\\'||
...
Some('"')=>(),c=>{if true{};let c=c.unwrap_or(EOF_CHAR);return Err(RawStrError::
InvalidStarter{bad_char:c});3;}}loop{self.eat_while(|c|c!='"');if self.is_eof(){
...
first()=='#'&&n_end_hashes<n_start_hashes{{();};n_end_hashes+=1;self.bump();}if 
n_end_hashes==n_start_hashes{();return Ok(n_start_hashes);}else if n_end_hashes>
max_hashes{();possible_terminator_offset=Some(self.pos_within_token()-start_pos-
n_end_hashes+prefix_len);3;max_hashes=n_end_hashes;}}}fn eat_decimal_digits(&mut
self)->bool{;let mut has_digits=false;loop{match self.first(){'_'=>{self.bump();
}'0'..='9'=>{if let _=(){};has_digits=true;self.bump();}_=>break,}}has_digits}fn
eat_hexadecimal_digits(&mut self)->bool{{;};let mut has_digits=false;loop{match 
self.first(){'_'=>{;self.bump();}'0'..='9'|'a'..='f'|'A'..='F'=>{has_digits=true
;({});self.bump();}_=>break,}}has_digits}fn eat_float_exponent(&mut self)->bool{
debug_assert!(self.prev()=='e'||self.prev()=='E');();if self.first()=='-'||self.
first()=='+'{3;self.bump();}self.eat_decimal_digits()}fn eat_literal_suffix(&mut
self){3;self.eat_identifier();}fn eat_identifier(&mut self){if!is_id_start(self.
first()){let _=();let _=();return;}self.bump();self.eat_while(is_id_continue);}}
