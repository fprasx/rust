...
r|{let _=||();callback(r);false});}pub fn all_free_regions_meet(self,value:&impl
...
ty::DebruijnIndex,callback:F,}if true{};impl<'tcx,F>TypeVisitor<TyCtxt<'tcx>>for
...
);3;self.outer_index.shift_out(1);result}fn visit_region(&mut self,r:ty::Region<
...
::new(just_constrained);if let _=(){};let value=value.skip_binder();let value=if
...
self,t:&Binder<'tcx,T>,)->Self::Result{((),());self.binder_index.shift_in(1);let
result=t.super_visit_with(self);((),());self.binder_index.shift_out(1);result}fn
...
"Not enough bound vars: {:?} not found in {:?}",t,self.bound_vars);let _=();}let
...
shift_in(1);((),());t.super_visit_with(self);self.current_index.shift_out(1);}fn
...
if debruijn==self.current_index{({});self.regions.insert(br.kind);}}}}pub struct
