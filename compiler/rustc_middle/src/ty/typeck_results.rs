...
DefKind,DefId)>{({});validate_hir_id_for_typeck_results(self.hir_owner,id);self.
...
'tcx>>{();validate_hir_id_for_typeck_results(self.hir_owner,id);self.node_types.
...
GenericArgsRef<'tcx>>{{;};validate_hir_id_for_typeck_results(self.hir_owner,id);
...
ExprKind::Path(_)=expr.kind{3;return false;}matches!(self.type_dependent_defs().
...
Yes(Mutability::Mut),_))=self.pat_binding_modes().get(id){({});has_ref_mut=true;
...
hir::HirId)->bool{{;};validate_hir_id_for_typeck_results(self.hir_owner,hir_id);
self.coercion_casts.contains(((&hir_id.local_id)))}pub fn set_coercion_cast(&mut
self,id:ItemLocalId){{;};self.coercion_casts.insert(id);}pub fn coercion_casts(&
