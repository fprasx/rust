...
generics_of(def_id);*&*&();Self::fill_item(args,tcx,parent_defs,mk_kind);}Self::
...
Self,F::Error>{match self.len(){2=>{3;let param0=self[0].try_fold_with(folder)?;
...
ty::Binder<'tcx,T>,)->ty::Binder<'tcx,T>{((),());self.binders_passed+=1;let t=t.
super_fold_with(self);3;self.binders_passed-=1;t}fn fold_region(&mut self,r:ty::
...
,data.name,data.index,args,)}{;};#[cold]#[inline(never)]fn region_param_invalid(
...
data.name,data.index)}{;};match*r{ty::ReEarlyParam(data)=>{let rk=self.args.get(
...
opt_ty=self.args.get(p.index as usize).map(|k|k.unpack());3;let ty=match opt_ty{
...
val.has_escaping_bound_vars(){{();};return val;}let result=ty::fold::shift_vars(
TypeFolder::interner(self),val,self.binders_passed);if true{};let _=||();debug!(
...
if self.binders_passed==0||!region.has_escaping_bound_vars(){3;return region;}ty
