...
"from_anon_const can only process anonymous constants"),};3;let expr=&tcx.hir().
body(body_id).value;{();};debug!(?expr);let ty=tcx.type_of(def).no_bound_vars().
...
ResolvedArg::EarlyBound(_))=>{3;let item_def_id=tcx.parent(def_id);let generics=
...
());3;};Ok(c)}ConstKind::Value(val)=>Ok(val),ConstKind::Error(g)=>Err(g.into()),
