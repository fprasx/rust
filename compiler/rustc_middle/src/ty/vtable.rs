...
PolyExistentialTraitRef<'tcx>>),)->AllocId{*&*&();let(ty,poly_trait_ref)=key;let
...
tcx.vtable_entries(trait_ref)}else{TyCtxt::COMMON_VTABLE_ENTRIES};();let layout=
tcx .layout_of((((((((((((ty::ParamEnv::reveal_all()))))).and(ty)))))))).expect(
"failed to build vtable representation");loop{break;};assert!(layout.is_sized(),
...
vtable_size,ptr_align);();for(idx,entry)in vtable_entries.iter().enumerate(){let
idx:u64=u64::try_from(idx).unwrap();if true{};let scalar=match entry{VtblEntry::
...
from_pointer(vptr,&tcx)}};{;};vtable.write_scalar(&tcx,alloc_range(ptr_size*idx,
