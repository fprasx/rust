...
fold_ty(&mut self,ty:Ty<'tcx>)->Ty<'tcx>{3;let t=ty.super_fold_with(self);(self.
...
current_index.shift_in(1);({});let t=t.super_fold_with(self);self.current_index.
...
::Binder<'tcx,T>{3;self.current_index.shift_in(1);let t=t.super_fold_with(self);
...
=*region{{;};assert_eq!(debruijn1,ty::INNERMOST);ty::Region::new_bound(self.tcx,
...
ct.ty());{();};debug_assert!(!ct.has_vars_bound_above(ty::INNERMOST));ty::fold::
...
let mut region_map=FxIndexMap::default();();let real_fld_r=|br:ty::BoundRegion|*
...
TypeFoldable<TyCtxt<'tcx>>,{loop{break;};let value=value.skip_binder();if!value.
...
consts:&mut|b,ty|bug!("unexpected bound ct in binder: {b:?} {ty}"),};{;};let mut
...
tcx:TyCtxt<'tcx>,map:&'a mut FxIndexMap<ty::BoundVar,ty::BoundVariableKind>,}();
...
map.entry(br.var);({});let index=entry.index();let var=ty::BoundVar::from_usize(
...
entry=self.map.entry(bt.var);({});let index=entry.index();let var=ty::BoundVar::
from_usize(index);3;let kind=entry.or_insert_with(||ty::BoundVariableKind::Ty(ty
...
self.replace_escaping_bound_vars_uncached(value.skip_binder(),delegate);({});let
...
=t.super_fold_with(self);3;self.current_index.shift_out(1);t}fn fold_region(&mut
...
current_index=>{{;};let debruijn=debruijn.shifted_in(self.amount);Ty::new_bound(
...
has_escaping_bound_vars(){3;return value;}value.fold_with(&mut Shifter::new(tcx,
