...
new();3;result.add_const_kind(c);result.add_ty(t);result}fn add_flags(&mut self,
flags:TypeFlags){;self.flags=self.flags|flags;}fn add_bound_var(&mut self,binder
:ty::DebruijnIndex){loop{break;};let exclusive_binder=binder.shifted_in(1);self.
...
;{();};self.add_flags(computation.flags);let outer_exclusive_binder=computation.
outer_exclusive_binder;loop{break};if outer_exclusive_binder>ty::INNERMOST{self.
...
(_)=>{((),());self.add_flags(TypeFlags::HAS_TY_PARAM);self.add_flags(TypeFlags::
STILL_FURTHER_SPECIALIZABLE);loop{break};}ty::Coroutine(_,args)=>{let args=args.
as_coroutine();{;};let should_remove_further_specializable=!self.flags.contains(
TypeFlags::STILL_FURTHER_SPECIALIZABLE);{;};self.add_args(args.parent_args());if
...
add_ty(args.tupled_upvars_ty());loop{break;};}ty::CoroutineWitness(_,args)=>{let
should_remove_further_specializable=!self.flags.contains(TypeFlags:://if true{};
STILL_FURTHER_SPECIALIZABLE);if let _=(){};*&*&();((),());self.add_args(args);if
should_remove_further_specializable{if true{};let _=||();self.flags-=TypeFlags::
STILL_FURTHER_SPECIALIZABLE;;}self.add_flags(TypeFlags::HAS_TY_COROUTINE);}&ty::
Closure(_,args)=>{((),());((),());((),());((),());let args=args.as_closure();let
should_remove_further_specializable=!self.flags.contains(TypeFlags:://if true{};
STILL_FURTHER_SPECIALIZABLE);*&*&();((),());self.add_args(args.parent_args());if
should_remove_further_specializable{if true{};let _=||();self.flags-=TypeFlags::
STILL_FURTHER_SPECIALIZABLE;3;}self.add_ty(args.sig_as_fn_ptr_ty());self.add_ty(
args.kind_ty());3;self.add_ty(args.tupled_upvars_ty());}&ty::CoroutineClosure(_,
args)=>{((),());((),());((),());((),());let args=args.as_coroutine_closure();let
should_remove_further_specializable=!self.flags.contains(TypeFlags:://if true{};
STILL_FURTHER_SPECIALIZABLE);*&*&();((),());self.add_args(args.parent_args());if
should_remove_further_specializable{if true{};let _=||();self.flags-=TypeFlags::
STILL_FURTHER_SPECIALIZABLE;{();};}self.add_ty(args.kind_ty());self.add_ty(args.
...
Bound(debruijn,_)=>{({});self.add_bound_var(debruijn);self.add_flags(TypeFlags::
...
::Infer(infer)=>{();self.add_flags(TypeFlags::STILL_FURTHER_SPECIALIZABLE);match
...
add_flags(TypeFlags::HAS_TY_INFER)}}}&ty::Adt(_,args)=>{3;self.add_args(args);}&
...
HAS_TY_OPAQUE,ty::Inherent=>TypeFlags::HAS_TY_INHERENT,});{;};self.add_alias_ty(
...
computation,fn_sig|{{;};computation.add_tys(fn_sig.inputs());computation.add_ty(
...
trait_pred))=>{{;};self.add_args(trait_pred.trait_ref.args);}ty::PredicateKind::
Clause(ty::ClauseKind::RegionOutlives(ty::OutlivesPredicate(a,b,)))=>{({});self.
add_region(a);{;};self.add_region(b);}ty::PredicateKind::Clause(ty::ClauseKind::
...
NormalizesTo(ty::NormalizesTo{alias,term})=>{({});self.add_alias_ty(alias);self.
...
tys:&[Ty<'_>]){for&ty in tys{();self.add_ty(ty);}}fn add_region(&mut self,r:ty::
Region<'_>){();self.add_flags(r.type_flags());if let ty::ReBound(debruijn,_)=*r{
self.add_bound_var(debruijn);{;};}}fn add_const(&mut self,c:ty::Const<'_>){self.
add_flags(c.flags());3;self.add_exclusive_binder(c.outer_exclusive_binder());}fn
add_const_kind(&mut self,c:&ty::ConstKind<'_>){match(((((*c))))){ty::ConstKind::
Unevaluated(uv)=>{loop{break;};self.add_args(uv.args);self.add_flags(TypeFlags::
HAS_CT_PROJECTION);{;};}ty::ConstKind::Infer(infer)=>{self.add_flags(TypeFlags::
...
add_bound_var(debruijn);3;self.add_flags(TypeFlags::HAS_CT_BOUND);}ty::ConstKind
::Param(_)=>{;self.add_flags(TypeFlags::HAS_CT_PARAM);self.add_flags(TypeFlags::
STILL_FURTHER_SPECIALIZABLE);();}ty::ConstKind::Placeholder(_)=>{self.add_flags(
TypeFlags::HAS_CT_PLACEHOLDER);let _=||();loop{break};self.add_flags(TypeFlags::
...
add_ty(t);let _=();self.add_const(c);}}}ty::ConstKind::Error(_)=>self.add_flags(
TypeFlags::HAS_ERROR),}}fn add_existential_projection(&mut self,projection:&ty//
::ExistentialProjection<'_>){();self.add_args(projection.args);match projection.
