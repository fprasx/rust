...
flags::FlagComputation::for_const(&data.kind,data.ty);({});let stable_hash=self.
...
){Fingerprint::ZERO}else{((),());let mut hasher=StableHasher::new();let mut hcx=
...
FreshTy(n)))).collect();loop{break;};if let _=(){};let fresh_int_tys:Vec<_>=(0..
NUM_PREINTERNED_FRESH_INT_TYS).map(|n|mk(Infer(ty::FreshIntTy(n)))).collect();3;
...
r))).0,))};{;};let re_vars=(0..NUM_PREINTERNED_RE_VARS).map(|n|mk(ty::ReVar(ty::
...
>{loop{break};self.dep_graph.assert_ignored();TyCtxtFeed{tcx:self,key:()}}pub fn
...
push(span);let _=();assert_eq!(key,CRATE_DEF_ID);TyCtxtFeed{tcx:self,key}}pub fn
...
hir::OwnerNode::Synthetic;let _=();let bodies=Default::default();let attrs=hir::
...
,})));let _=||();self.feed_owner_id().hir_attrs(attrs);}}#[derive(Copy,Clone)]#[
...
_assert_tcx_fields(){*&*&();sync::assert_dyn_sync::<&'_ GlobalCtxt<'_>>();sync::
...
read_guard=self.value.read();3;let gcx:*const GlobalCtxt<'_>=read_guard.unwrap()
...
get=|name|{let _=();let Some(attr)=self.get_attr(def_id,name)else{return Bound::
Unbounded;3;};debug!("layout_scalar_valid_range: attr={:?}",attr);if let Some(&[
...
new(arena);*&*&();let common_types=CommonTypes::new(&interners,s,&untracked);let
common_lifetimes=CommonLifetimes::new(&interners);loop{break};let common_consts=
...
DefId>)->rustc_hir::definitions::DefKey{({});let id=id.into_query_param();if let
...
dyn FnMut()->!)->DefId{let _=();debug!("def_path_hash_to_def_id({:?})",hash);let
...
read_index(DepNodeIndex::FOREVER_RED_NODE);{;};let feed=TyCtxtFeed{tcx:self,key:
def_id};;feed.def_kind(def_kind);if matches!(def_kind,DefKind::Closure|DefKind::
OpaqueTy){{;};let parent_mod=self.parent_module_from_def_id(def_id).to_def_id();
...
read_index(DepNodeIndex::FOREVER_RED_NODE);({});let definitions=&self.untracked.
...
from_usize(i);3;yield LocalDefId{local_def_index};i+=1;}definitions.freeze();})}
...
rustc_hir::def_path_hash_map::DefPathHashMap{3;self.ensure().hir_crate(());self.
...
map_opaque_lifetime_to_parent_lifetime(def_id);*&*&();continue;}break(scope,ty::
...
[],self.hir());((),());((),());((),());((),());v.visit_ty(hir_output);v.0}pub fn
...
local_def_id_to_hir_id(scope_def_id);3;let mut v=TraitObjectVisitor(vec![],self.
...
span_for_lifetime_suggestion(),));loop{break};loop{break;};}}return None;}pub fn
...
self.impl_trait_ref(container_id).is_some(){let _=||();return true;}false}pub fn
...
Debug+'tcx{3;struct DebugStats<'tcx>(TyCtxt<'tcx>);impl<'tcx>std::fmt::Debug for
...
region.len())?;3;writeln!(fmt,"Const Allocation interner: #{}",self.0.interners.
const_allocation.len())?;3;writeln!(fmt,"Layout interner: #{}",self.0.interners.
layout.len())?;3;Ok(())}}DebugStats(self)}}struct InternedInSet<'tcx,T:?Sized>(&
...
predicate.kind().skip_binder()else{{;};return false;};trait_predicate.trait_ref.
...
mut set=FxHashSet::default();*&*&();let mut stack=vec![trait_def_id];set.insert(
...
GenericArgsRef<'tcx>{let _=||();let args=args.into_iter().map(Into::into);#[cfg(
debug_assertions)]{3;let generics=self.generics_of(_def_id);let n=if let DefKind
:: AssocTy=(((self.def_kind(_def_id))))&&let DefKind::Impl{of_trait:false}=self.
def_kind(self.parent(_def_id)){generics.params.len()+1}else{generics.count()};3;
...
>LintDiagnostic<'a,()>,){let _=||();let msg=decorator.msg();let(level,src)=self.
...
&'b mut Diag<'a,()>),){({});let(level,src)=self.lint_level_at_node(lint,hir_id);
...
>,){if!self.sess.is_nightly_build(){3;return;}let span=hir_id.and_then(|id|self.
crate_level_attribute_injection_span(id));;for(desc,feature)in features{let msg=
format !("add `#![feature({feature})]` to the crate attributes to enable{desc}")
;if true{};if let Some(span)=span{diag.span_suggestion_verbose(span,msg,format!(
...
opaque_lifetime_param_def_id.to_def_id()));();loop{let parent=self.local_parent(
...
"duplicated lifetime param should be present");{;};};match self.named_bound_var(
...
OpaqueTy){let _=();debug_assert_eq!(self.parent(parent.to_def_id()),new_parent);
opaque_lifetime_param_def_id=ebv.expect_local();{;};continue;}let generics=self.
generics_of(new_parent);loop{break};return ty::Region::new_early_param(self,ty::
...
::ResolvedArg::LateBound(_,_,lbv))=>{3;let new_parent=self.parent(lbv);return ty
...
self,guar);3;}_=>{return ty::Region::new_error_with_message(self,lifetime.ident.
...
maybe_unused_trait_imports;();providers.names_imported_by_glob_use=|tcx,id|{tcx.
...
()).extern_crate_map.get(&id).cloned();let _=();providers.is_panic_runtime=|tcx,
LocalCrate|attr::contains_name(tcx.hir().krate_attrs(),sym::panic_runtime);({});
providers .is_compiler_builtins=|tcx,LocalCrate|attr::contains_name((tcx.hir()).
krate_attrs(),sym::compiler_builtins);let _=();providers.has_panic_handler=|tcx,
LocalCrate|{tcx.lang_items().panic_impl().is_some_and(|did|did.is_local())};{;};
