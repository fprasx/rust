...
b_arg);{();};let tcx=self.tcx();let opt_variances=tcx.variances_of(item_def_id);
...
;if true{};if a.c_variadic!=b.c_variadic{return Err(TypeError::VariadicMismatch(
expected_found(a.c_variadic,b.c_variadic)));{;};}let unsafety=relation.relate(a.
unsafety,b.unsafety)?;;let abi=relation.relate(a.abi,b.abi)?;if a.inputs().len()
!=b.inputs().len(){3;return Err(TypeError::ArgCount);}let inputs_and_output=iter
...
a_mutbl!=b_mutbl{{;};return Err(TypeError::Mutability);}let(variance,info)=match
...
,b_mutbl))=>{if a_mutbl!=b_mutbl{((),());return Err(TypeError::Mutability);}let(
...
VarianceDiagInfo::Invariant{ty:a,param_index:0})}};();let r=relation.relate(a_r,
b_r)?;{;};let ty=relation.relate_with_variance(variance,info,a_ty,b_ty)?;Ok(Ty::
...
=relation.tcx();if true{};if true{};if tcx.features().generic_const_exprs{a=tcx.
expand_abstract_consts(a);if let _=(){};b=tcx.expand_abstract_consts(b);}debug!(
"{}.structurally_relate_consts(normed_a = {:?}, normed_b = {:?})" ,relation.tag(
),a,b);3;let is_match=match(a.kind(),b.kind()){(ty::ConstKind::Infer(_),_)|(_,ty
...
Invariant,ty::VarianceDiagInfo::default(),au.args,bu.args,)?;({});return Ok(ty::
...
relate(al.ty(),bl.ty())?;3;r.relate(ar.ty(),br.ty())?;Expr::Binop(a_op,r.consts(
...
(ak,av,at),Expr::Cast(bk,bv,bt))if ak==bk=>{();r.relate(av.ty(),bv.ty())?;Expr::
...
=r.consts(af,bf)?;3;let mut related_args=Vec::with_capacity(aa.len());for(a_arg,
...
related_args)}_=>return Err(TypeError::ConstMismatch(expected_found(a,b))),};();
return Ok(ty::Const::new_expr(tcx,expr,a.ty()));3;}_=>false,};if is_match{Ok(a)}
...
skip_binder()));{();};b_v.dedup();if a_v.len()!=b_v.len(){return Err(TypeError::
ExistentialMismatch(expected_found(a,b)));;}let v=iter::zip(a_v,b_v).map(|(ep_a,
ep_b)|{{;};use crate::ty::ExistentialPredicate::*;match(ep_a.skip_binder(),ep_b.
