...
'tcx>>>{();let clause=self.kind();if let ty::ClauseKind::TypeOutlives(o)=clause.
skip_binder(){(((((((Some(((((((clause.rebind(o)))))))))))))))}else{None}}pub fn
as_region_outlives_clause(self,)->Option<ty::Binder<'tcx,//if true{};let _=||();
RegionOutlivesPredicate<'tcx>>>{3;let clause=self.kind();if let ty::ClauseKind::
...
->ty::ExistentialTraitRef<'tcx>{if true{};let def_id=tcx.parent(self.def_id);let
...
pred_bound_vars=bound_pred.bound_vars();let _=();let trait_bound_vars=trait_ref.
...
Predicate<'tcx>{3;let pred:PolyTraitPredicate<'tcx>=self.to_predicate(tcx);pred.
...
<'tcx>{let _=||();let pred:PolyTraitPredicate<'tcx>=self.to_predicate(tcx);pred.
