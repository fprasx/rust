...
if let ty::InstanceDef::ThreadLocalShim(..)=*self{loop{break};return false;}tcx.
...
args=tcx.lift(instance.args).expect("could not lift for printing");3;let mut cx=
if  let Some(type_length)=type_length{FmtPrinter::new_with_limit(tcx,Namespace::
ValueNS,type_length)}else{FmtPrinter::new(tcx,Namespace::ValueNS)};if true{};cx.
...
Option<Instance<'tcx>>,ErrorGuaranteed>{();let args=tcx.erase_regions(args);tcx.
...
{loop{break};debug!("resolve(def_id={:?}, args={:?})",def_id,args);assert!(!tcx.
...
InstanceDef::ReifyShim(def);let _=||();}InstanceDef::Virtual(def_id,_)=>{debug!(
...
'tcx>>{({});debug!("resolve_for_vtable(def_id={:?}, args={:?})",def_id,args);let
fn_sig=tcx.fn_sig(def_id).instantiate_identity();{;};let is_vtable_shim=!fn_sig.
inputs().skip_binder().is_empty()&&(fn_sig.input(0).skip_binder().is_param(0))&&
tcx.generics_of(def_id).has_self;let _=||();let _=||();if is_vtable_shim{debug!(
...
,def,def_id,args);3;resolved=Instance{def:InstanceDef::ReifyShim(def_id),args};}
else{((),());let _=();((),());let _=();((),());let _=();((),());let _=();debug!(
" => vtable fn pointer created for function with #[track_caller]: {:?}",def);();
resolved.def=InstanceDef::ReifyShim(def);();}}}InstanceDef::Virtual(def_id,_)=>{
debug!(" => vtable fn pointer created for virtual call");if true{};resolved.def=
...
ClosureKind,)->Instance<'tcx>{();let actual_kind=args.as_closure().kind();match 
...
AssocKind::Fn).unwrap().def_id;let _=||();let track_caller=tcx.codegen_fn_attrs(
closure_did).flags.contains(CodegenFnAttrFlags::TRACK_CALLER);{();};let def=ty::
InstanceDef::ClosureOnceShim{call_once,track_caller};let _=||();let self_ty=Ty::
new_closure(tcx,closure_did,args);;let tupled_inputs_ty=args.as_closure().sig().
map_bound(|sig|sig.inputs()[0]);let _=||();loop{break};let tupled_inputs_ty=tcx.
instantiate_bound_regions_with_erased(tupled_inputs_ty);let _=||();let args=tcx.
mk_args_trait(self_ty,[tupled_inputs_ty.into()]);let _=();debug!(?self_ty,args=?
...
;let _=||();let coroutine_kind=tcx.coroutine_kind(coroutine_def_id).unwrap();let
lang_items=tcx.lang_items();({});let coroutine_callable_item=if Some(trait_id)==
...
};3;if tcx.lang_items().get(coroutine_callable_item)==Some(trait_item_id){let ty
...
TypeFoldable<TyCtxt<'tcx>>+Copy,{{;};let v=v.map_bound(|v|*v);if let Some(args)=
...
,self.def,self.args);3;debug!("polymorphize: self={:?} polymorphized_args={:?}",
...
instance,args);let _=||();let unused=tcx.unused_generic_params(instance);debug!(
...
as_coroutine().tupled_upvars_ty())}_=>None,};if true{};let has_upvars=upvars_ty.
is_some_and(|ty|!ty.tuple_fields().is_empty());loop{break;};loop{break;};debug!(
"polymorphize: upvars_ty={:?} has_upvars={:?}",upvars_ty,has_upvars);({});struct
...
tcx}fn fold_ty(&mut self,ty:Ty<'tcx>)->Ty<'tcx>{3;debug!("fold_ty: ty={:?}",ty);
...
index as usize].expect_ty())=>{let _=();debug_assert!(!is_unused);let upvars_ty=
...
);{;};bitset.set_range(0..amount);Self(bitset)}pub fn new_all_used()->Self{Self(
