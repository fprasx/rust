...
{3;assert!(key.parent.is_none());self.path_crate(def_id.krate)}DefPathData::Impl
=>{3;let generics=self.tcx().generics_of(def_id);let self_ty=self.tcx().type_of(
def_id);*&*&();let impl_trait_ref=self.tcx().impl_trait_ref(def_id);let(self_ty,
...
{index:key.parent.unwrap(),..def_id};let _=||();let mut parent_args=args;let mut
trait_qualify_parent=false;if true{};if!args.is_empty(){let generics=self.tcx().
...
(self.tcx(),args);();return self.path_generic_args(|cx|cx.print_def_path(def_id,
parent_args),args,);3;}}}trait_qualify_parent=generics.has_self&&generics.parent
...
parent_def_id=DefId{index:key.parent.unwrap(),..impl_def_id};();let in_self_mod=
...
(ty,visited);({});}return None;}),ty::FnDef(def_id,_)|ty::Closure(def_id,_)|ty::
...
TyCtxt<'_>)->String{();let def_id=def_id.into();if def_id.is_top_level_module(){
