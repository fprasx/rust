...
,start_pos:usize)->Self{{();};debug_assert!(sess.opts.incremental.is_some());let
...
TyCtxt<'_>){({});tcx.dep_graph.exec_cache_promotions(tcx);*self.serialized_data.
...
file_index_to_stable_id)={*&*&();let files=tcx.sess.source_map().files();let mut
...
files.len(),Default::default());3;for(index,file)in files.iter().enumerate(){let
index=SourceFileIndex(index as u32);();let file_ptr:*const SourceFile=std::ptr::
addr_of!(**file);3;file_to_file_index.insert(file_ptr,index);let source_file_id=
EncodedSourceFileId::new(tcx,file);((),());file_index_to_stable_id.insert(index,
source_file_id);if let _=(){};}(file_to_file_index,file_index_to_stable_id)};let
hygiene_encode_context=HygieneEncodeContext::default();let _=();let mut encoder=
...
hygiene_context:&hygiene_encode_context,symbol_table:Default::default(),};();let
...
(tcx.query_system.fns.encode_query_results)(tcx,enc,qri);((),());let _=();});let
...
());3;encoder.encode_tagged(dep_node_index,side_effects);(dep_node_index,pos)}).
collect();3;let interpret_alloc_index={let mut interpret_alloc_index=Vec::new();
...
interpret_allocs[idx];*&*&();let pos:u64=encoder.position().try_into().unwrap();
interpret_alloc_index.push(pos);({});interpret::specialized_encode_alloc_id(&mut
encoder,tcx,id);*&*&();}n=new_n;}interpret_alloc_index};let mut syntax_contexts=
FxHashMap::default();loop{break;};let mut expn_data=UnhashMap::default();let mut
...
AbsoluteBytePos::new(encoder.position());();encoder.encode_tagged(TAG_EXPN_DATA,
data);();expn_data.insert(hash,pos);}else{foreign_expn_data.insert(hash,expn_id.
local_id.as_u32());((),());}},);let footer_pos=encoder.position()as u64;encoder.
...
dep_node_index,&self.query_result_index);3;debug_assert_eq!(opt_value.is_some(),
...
let pos=index.get(&dep_node_index).cloned()?;();let value=self.with_decoder(tcx,
...
'tcx>>,{((),());let serialized_data=self.serialized_data.read();let mut decoder=
...
index];{;};let source_file_cnum=tcx.stable_crate_id_to_crate_num(source_file_id.
stable_crate_id);3;if source_file_cnum!=LOCAL_CRATE{self.tcx.cstore_untracked().
...
:Decodable<D>,D:Decoder,{{;};let start_pos=decoder.position();let actual_tag=T::
...
);((),());assert_eq!((end_pos-start_pos)as u64,expected_len);value}impl<'a,'tcx>
...
>where F:FnOnce(&mut Self)->Ty<'tcx>,{*&*&();let tcx=self.tcx;let cache_key=ty::
CReaderCacheKey{cnum:None,pos:shorthand};;if let Some(&ty)=tcx.ty_rcache.borrow(
).get(&cache_key){let _=();return ty;}let ty=or_insert_with(self);tcx.ty_rcache.
...
replace(&mut self.opaque,new_opaque);3;let r=f(self);self.opaque=old_opaque;r}fn
...
mut hasher=StableHasher::new();{;};expn_id.expn_data().hash_stable(&mut hcx,&mut
hasher);{();};hasher.finish()});debug_assert_eq!(hash.local_hash(),local_hash);}
expn_id}else{loop{break};let index_guess=self.foreign_expn_data[&hash];self.tcx.
...
let ctxt=SyntaxContext::decode(self);();let parent=Option::<LocalDefId>::decode(
...
u32::decode(self);loop{break;};let dto=u32::decode(self);let enclosing=self.tcx.
source_span_untracked(parent.unwrap()).data_untracked();({});let span=Span::new(
enclosing .lo+(BytePos::from_u32(dlo)),enclosing.lo+BytePos::from_u32(dto),ctxt,
parent,);loop{break;};return span;}else{debug_assert_eq!(tag,TAG_FULL_SPAN);}let
file_lo_index=SourceFileIndex::decode(self);;let line_lo=usize::decode(self);let
col_lo=RelativeBytePos::decode(self);3;let len=BytePos::decode(self);let file_lo
...
parent)}#[inline]fn decode_symbol(&mut self)->Symbol{{;};let tag=self.read_u8();
...
decode_def_id(&mut self)->DefId{{;};let def_path_hash=DefPathHash::decode(self);
...
self,tag:T,value:&V){{();};let start_pos=self.position();tag.encode(self);value.
...
TAG_PARTIAL_SPAN.encode(self);let _=();}if let Some(parent)=span_data.parent{let
...
}}loop{break};let pos=self.source_map.byte_pos_to_line_and_col(span_data.lo);let
...
line_lo,col_lo)=pos.unwrap();if let _=(){};let len=span_data.hi-span_data.lo;let
...
pos);3;self.emit_str(symbol.as_str());}Entry::Occupied(o)=>{let x=*o.get();self.
emit_u8(SYMBOL_OFFSET);{;};self.emit_usize(x);}}}}fn encode_crate_num(&mut self,
...
interpret_allocs.insert_full(*alloc_id);*&*&();index.encode(self);}}macro_rules!
