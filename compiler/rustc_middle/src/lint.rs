...
Warn{let _=||();level=configured_warning_level;*src=warnings_src;}}}level=if let
...
sess.driver_lint_caps.get(&lint){{;};level=cmp::min(*driver_level,level);}level}
...
owner;3;let mut specs=&self.specs;for parent in tcx.hir().parent_id_iter(start){
if parent.owner!=owner{{;};owner=parent.owner;specs=&tcx.shallow_lint_levels_on(
...
src)=self.probe_for_lint_level(tcx,lint,cur);({});let level=reveal_actual_level(
...
"requested on the command line with `{flag} {hyphen_case_lint_name}`"));3;}else{
...
rationale)=reason{({});err.note(rationale.to_string());}err.span_note_once(span,
"the lint level is defined here");if true{};if lint_attr_name.as_str()!=name{let
...
mut Diag<'a,()>)>,){((),());let future_incompatible=lint.future_incompatible;let
...
Level::Allow}else{({});return;}}Level::Expect(expect_id)=>{rustc_errors::Level::
...
).iter().any(|s|in_external_macro(sess,*s)){*&*&();err.disable_suggestions();let
incompatible=future_incompatible.is_some_and(|f|f.reason.edition().is_none());3;
if!incompatible&&!lint.report_in_external_macro{{();};err.cancel();return;}}err.
primary_message(msg);3;err.is_lint(lint.name_lower(),has_future_breakage);if let
Level::Expect(_)=level{*&*&();decorate(&mut err);err.emit();return;}if let Some(
...
reason)=>reason.to_owned(),};{;};if future_incompatible.explain_reason{err.warn(
...
);if let _=(){};}}let skip=err_level==rustc_errors::Level::Warning&&!sess.dcx().
...
:Span)->bool{3;let expn_data=span.ctxt().outer_expn_data();match expn_data.kind{
