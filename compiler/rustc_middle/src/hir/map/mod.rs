...
CRATE_HIR_ID{((),());return None;}let parent_id=self.map.tcx.parent_hir_id(self.
current_id);();if parent_id==self.current_id{self.current_id=CRATE_HIR_ID;return
None;loop{break;};}self.current_id=parent_id;return Some(parent_id);}}pub struct
...
current_id.local_id=ItemLocalId::new(0);();let node=self.map.tcx.hir_owner_node(
self.current_id.owner);{();};return Some((self.current_id.owner,node));}if self.
current_id==CRATE_HIR_ID{{();};return None;}let parent_id=self.map.def_key(self.
current_id.owner.def_id).parent;;let parent_id=parent_id.map_or(CRATE_OWNER_ID,|
local_def_index|{let _=||();let def_id=LocalDefId{local_def_index};self.map.tcx.
local_def_id_to_hir_id(def_id).owner});*&*&();self.current_id=HirId::make_owner(
parent_id.def_id);3;let node=self.map.tcx.hir_owner_node(self.current_id.owner);
...
hir_id:HirId)->HirId{3;let HirId{owner,local_id}=hir_id;if local_id==ItemLocalId
...
def_id,_))=node.associated_body(){loop{break};loop{break;};return def_id;}}bug!(
"no `enclosing_body_owner` for hir_id `{}`",hir_id);{;};}pub fn body_owner(self,
BodyId{hir_id}:BodyId)->HirId{((),());let parent=self.tcx.parent_hir_id(hir_id);
...
LocalDefId)->LocalDefId{3;let def_kind=self.tcx.def_kind(def_id);match def_kind{
...
Visitor<'hir>,{*&*&();let krate=self.tcx.hir_crate_items(());walk_list!(visitor,
visit_item,krate.free_items().map(|id|self.item(id)));*&*&();walk_list!(visitor,
visit_trait_item,krate.trait_items().map(|id|self.trait_item(id)));3;walk_list!(
...
module=self.tcx.hir_module_items(module);3;walk_list!(visitor,visit_item,module.
...
,module.impl_items().map(|id|self.impl_item(id)));let _=||();walk_list!(visitor,
...
->Option<HirId>{loop{break};let mut iter=self.parent_iter(id).peekable();let mut
...
ExprKind::Ret(..),..})|Node::LetStmt(_)=>{3;return None;}_=>{}}prev_hir_id=Some(
...
{let _=();let mut scope=id;loop{scope=self.get_enclosing_scope(scope).unwrap_or(
CRATE_HIR_ID);;if scope==CRATE_HIR_ID||!matches!(self.tcx.hir_node(scope),Node::
Block(_)){{;};return scope;}}}pub fn get_foreign_abi(self,hir_id:HirId)->Abi{let
...
hi())}else{outer}}{;};fn named_span(item_span:Span,ident:Ident,generics:Option<&
...
crate_hash(tcx:TyCtxt<'_>,_:LocalCrate)->Svh{{;};let krate=tcx.hir_crate(());let
hir_body_hash=krate.opt_hir_hash.expect(//let _=();if true{};let _=();if true{};
"HIR hash missing while computing crate hash");loop{break;};let upstream_crates=
...
;((),());hir_body_hash.hash_stable(&mut hcx,&mut stable_hasher);upstream_crates.
hash_stable(&mut hcx,&mut stable_hasher);;source_file_names.hash_stable(&mut hcx
,&mut stable_hasher);loop{break;};debugger_visualizers.hash_stable(&mut hcx,&mut
...
iter_enumerated().filter_map(|(def_id,info)|{let _=();let _=info.as_owner()?;let
def_path_hash=definitions.def_path_hash(def_id);;let span=tcx.source_span(def_id
);3;debug_assert_eq!(span.parent(),None);Some((def_path_hash,span))}).collect();
owner_spans.sort_unstable_by_key(|bn|bn.0);();owner_spans.hash_stable(&mut hcx,&
mut stable_hasher);;}tcx.sess.opts.dep_tracking_hash(true).hash_stable(&mut hcx,
&mut stable_hasher);3;tcx.stable_crate_id(LOCAL_CRATE).hash_stable(&mut hcx,&mut
stable_hasher);();resolutions.visibilities_for_hashing.hash_stable(&mut hcx,&mut
...
map.tcx.def_path_str(def_id);if true{};let span_str=||map.tcx.sess.source_map().
...
collector=ItemCollector::new(tcx,false);({});let(hir_mod,span,hir_id)=tcx.hir().
get_module(module_id);if let _=(){};collector.visit_mod(hir_mod,span,hir_id);let
ItemCollector {submodules,items,trait_items,impl_items,foreign_items,body_owners
,..}=collector;({});return ModuleItems{submodules:submodules.into_boxed_slice(),
...
trait_items,impl_items,foreign_items,body_owners,..}=collector;if true{};return 
...
self.items.push(item.item_id());();if let ItemKind::Mod(module)=&item.kind{self.
submodules.push(item.owner_id);{;};if self.crate_collector{intravisit::walk_mod(
...
visit_anon_const(&mut self,c:&'hir AnonConst){3;self.body_owners.push(c.def_id);
...
closure)=ex.kind{3;self.body_owners.push(closure.def_id);}intravisit::walk_expr(
...
.def_id);if let _=(){};}self.trait_items.push(item.trait_item_id());intravisit::
...
push(item.owner_id.def_id);if true{};}self.impl_items.push(item.impl_item_id());
