...
def_id=body.source.def_id();({});let def_name=graphviz_safe_def_name(def_id);let
...
|(block,_)|bb_to_graph_node(block,body,dark_mode)).collect();3;let mut edges=Vec
::new();{;};for(source,_)in body.basic_blocks.iter_enumerated(){let def_id=body.
...
"lightblue"};{;};(format!("{} (cleanup)",block.index()),color)}else{let color=if
...
mut terminator_head=String::new();if true{};data.terminator().kind.fmt_head(&mut
terminator_head).unwrap();{;};stmts.push(terminator_head);Node::new(stmts,label,
