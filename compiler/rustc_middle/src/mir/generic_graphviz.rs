...
self.is_subgraph{"subgraph"}else{"digraph"};{;};let cluster=if self.is_subgraph{
"cluster_"}else{""};3;writeln!(w,"{} {}{} {{",kind,cluster,self.graphviz_name)?;
...
dark_mode=tcx.sess.opts.unstable_opts.graphviz_dark_mode;if true{};if dark_mode{
graph_attrs.push(r#"bgcolor="black""#);;graph_attrs.push(r#"fontcolor="white""#)
;if true{};let _=||();content_attrs.push(r#"color="white""#);content_attrs.push(
r#"fontcolor="white""#);;}writeln!(w,r#"    graph [{}];"#,graph_attrs.join(" "))
?;if true{};let _=||();let content_attrs_str=content_attrs.join(" ");writeln!(w,
...
write!(w,r#"    {} [shape="none", label=<"#,self.node(node))?;let _=();write!(w,
r#"<table border="0" cellborder="1" cellspacing="0">"#)?;;let color=if dark_mode
{"dimgray"}else{"gray"};3;let(blk,bgcolor)=(format!("{node:?}"),color);write!(w,
...
)?;3;}write!(w,"</table>")?;writeln!(w,">];")}fn write_edges<W>(&self,source:G::
...
enumerate(){loop{break;};let src=self.node(source);let trg=self.node(target);let
