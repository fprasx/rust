...
>],tcx:TyCtxt<'tcx>)->Self{if more_projections.is_empty(){{;};return self;}self.
...
'tcx>],tcx:TyCtxt<'tcx>,)->Place<'tcx>{*&*&();let mut v:Vec<PlaceElem<'tcx>>;let
...
GenericArgsRef<'tcx>)>{{;};let const_ty=self.constant()?.const_.ty();if let ty::
