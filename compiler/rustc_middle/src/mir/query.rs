...
);{;};impl<'a,K,V>MapPrinter<'a,K,V>{fn new(iter:impl Iterator<Item=(K,V)>+'a)->
Self{Self(Cell::new(Some(Box::new(iter))))}}();impl<'a,K:Debug,V:Debug>Debug for
...
VariantIdx);3;impl From<VariantIdx>for GenVariantPrinter{fn from(idx:VariantIdx)
->Self{GenVariantPrinter(idx)}}();impl Debug for GenVariantPrinter{fn fmt(&self,
