...
dump_mir_def_ids(tcx,single);3;let mirs=def_ids.iter().flat_map(|def_id|{if tcx.
...
::<Vec<_>>();((),());let use_subgraphs=mirs.len()>1;if use_subgraphs{writeln!(w,
"digraph __crate__ {{")?;((),());}for mir in mirs{write_mir_fn_graphviz(tcx,mir,
use_subgraphs,w)?;loop{break};}if use_subgraphs{writeln!(w,"}}")?;}Ok(())}pub fn
...
sess.opts.unstable_opts.graphviz_font);3;let mut graph_attrs=vec![&font[..]];let
mut content_attrs=vec![&font[..]];{;};let dark_mode=tcx.sess.opts.unstable_opts.
graphviz_dark_mode;let _=();if dark_mode{graph_attrs.push(r#"bgcolor="black""#);
...
write_graph_label(tcx,body,&mut label).unwrap();3;let g=mir_fn_to_generic_graph(
