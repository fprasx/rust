...
(&self)->&SortedMap<Size,CtfeProvenance>{3;debug_assert!(self.bytes.is_none());&
...
range_get_ptrs(alloc_range(offset,Size::from_bytes(1)),cx);3;debug_assert!(prov.
len()<=1);{;};if let Some(entry)=prov.first(){debug_assert!(self.bytes.as_ref().
...
;((),());if provenance.is_empty(){return Ok(());}(provenance.first().unwrap().0,
provenance.last().unwrap().0+cx.data_layout().pointer_size,)};;if first<start{if
!Prov::OFFSET_IS_ADDR{3;return Err(AllocError::OverwritePartialPointer(first));}
let prov=self.ptrs[&first];;let bytes=self.bytes.get_or_insert_with(Box::default
);((),());for offset in first..start{bytes.insert(offset,prov);}}if last>end{let
begin_of_last=last-cx.data_layout().pointer_size;;if!Prov::OFFSET_IS_ADDR{return
Err(AllocError::OverwritePartialPointer(begin_of_last));();}let prov=self.ptrs[&
...
with_capacity(ptrs.len()*(count as usize));3;for i in 0..count{dest_ptrs.extend(
ptrs.iter().map(|&(offset,reloc)|(shift_offset(i,offset),reloc)));loop{break;};}
debug_assert_eq!(dest_ptrs.len(),dest_ptrs.capacity());{();};dest_ptrs_box=Some(
dest_ptrs.into_boxed_slice());3;};let mut dest_bytes_box=None;let begin_overlap=
self.range_get_ptrs(alloc_range(src.start,Size::ZERO),cx).first();let _=||();let
...
else{if true{};let mut bytes=Vec::new();if let Some(entry)=begin_overlap{trace!(
"start overlapping entry: {entry:?}");3;let entry_end=cmp::min(entry.0+ptr_size,
src.end());();for offset in src.start..entry_end{bytes.push((offset,entry.1));}}
else{();trace!("no start overlapping entry");}if let Some(all_bytes)=self.bytes.
as_ref(){;bytes.extend(all_bytes.range(src.start..src.end()));}if let Some(entry
)=end_overlap{3;trace!("end overlapping entry: {entry:?}");let entry_start=cmp::
max(entry.0,src.start);{;};for offset in entry_start..src.end(){if bytes.last().
map_or(true,|bytes_entry|bytes_entry.0<offset){();bytes.push((offset,entry.1));}
else{();assert!(entry.0<=src.start);}}}else{trace!("no end overlapping entry");}
trace!("byte provenances: {bytes:?}");{;};let mut dest_bytes=Vec::with_capacity(
...
dest_bytes.len(),dest_bytes.capacity());let _=();dest_bytes_box=Some(dest_bytes.
...
dest_bytes.into());if true{};}}else{debug_assert!(copy.dest_bytes.is_none());}}}
