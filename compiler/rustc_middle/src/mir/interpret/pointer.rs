...
val:u64)->i64{3;let val=val as i64;if val>self.target_isize_max(){debug_assert!(
...
);{();};let max_ptr_plus_1=1u128<<self.pointer_size().bits();(u64::try_from(val%
...
u64,i:i64)->(u64,bool){*&*&();let n=i.unsigned_abs();if i>=0{let(val,over)=self.
overflowing_offset(val,n);();(val,over||i>self.target_isize_max())}else{let res=
val.overflowing_sub(n);{;};let(val,over)=self.truncate_to_ptr(res);(val,over||i<
self.target_isize_min())}}#[inline]fn offset<'tcx>(&self,val:u64,i:u64)->//({});
InterpResult<'tcx,u64>{{;};let(res,over)=self.overflowing_offset(val,i);if over{
...
bytes())?;if true{};}if ptr.provenance.immutable(){write!(f,"<imm>")?;}Ok(())}fn
...
.provenance)?;3;}else{write!(f,"{:?}",ptr.provenance)?;}if ptr.offset.bytes()>0{
...
addr(self)->Size where Prov:Provenance,{({});assert!(Prov::OFFSET_IS_ADDR);self.
...
over)=cx.data_layout().overflowing_offset(self.offset.bytes(),i.bytes());{;};let
