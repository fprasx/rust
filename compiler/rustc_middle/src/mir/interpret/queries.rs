...
def_id);3;let instance=ty::Instance::new(def_id,args);let cid=GlobalId{instance,
...
.args.has_non_region_infer(){3;bug!("did not expect inference variables here");}
match (ty::Instance::resolve(self,param_env,ct.def,ct.args,)){Ok(Some(instance))
=>{();let cid=GlobalId{instance,promoted:ct.promoted};self.const_eval_global_id(
...
mir_body=self.mir_for_ctfe(instance.def_id());{;};if mir_body.is_polymorphic{let
...
param_env(def_id).with_reveal_all_normalized(self.tcx);({});let inputs=self.tcx.
