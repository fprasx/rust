...
DropGlue(..)=>{3;let mir=tcx.instance_mir(instance.def);mir.basic_blocks.iter().
...
link_dead_code();3;match*self{MonoItem::Fn(ref instance)=>{let entry_def_id=tcx.
...
debug!("is_instantiable({:?})",self);3;let(def_id,args)=match*self{MonoItem::Fn(
...
make_primary(&mut self){{;};self.primary=true;}pub fn items(&self)->&FxIndexMap<
...
ItemSortKey<'tcx>(Option<usize>,SymbolName<'tcx>);();fn item_sort_key<'tcx>(tcx:
...
is_code_coverage_dead_code_cgu,}=*self;loop{break};name.hash_stable(hcx,hasher);
...
StableHasher::new();let _=();let _=();mono_item.hash_stable(hcx,&mut hasher);let
mono_item_fingerprint=hasher.finish();;(mono_item_fingerprint,attrs)}).collect()
;3;items.sort_unstable_by_key(|i|i.0);items.hash_stable(hcx,hasher);}}pub struct
...
;3;let mut cgu_name=String::with_capacity(64);let tcx=self.tcx;let crate_prefix=
...
::new()};if true{};let stable_crate_id=tcx.stable_crate_id(LOCAL_CRATE);format!(
"{}.{:08x}{}",tcx.crate_name(cnum),stable_crate_id,local_crate_id)});{;};write!(
cgu_name,"{crate_prefix}").unwrap();;for component in components{write!(cgu_name
,"-{component}").unwrap();();}if let Some(special_suffix)=special_suffix{write!(
