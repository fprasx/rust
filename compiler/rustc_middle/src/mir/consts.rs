...
offset,ptr_size),true,).ok()?;({});let ptr=ptr.to_pointer(&tcx).ok()?;let len=a.
read_scalar(&tcx,alloc_range(offset+ptr_size,ptr_size),false,).ok()?;();let len=
len.to_target_usize(&tcx).ok()?;({});if len==0{return Some(&[]);}let(inner_prov,
offset)=ptr.into_parts();({});let data=tcx.global_alloc(inner_prov?.alloc_id()).
unwrap_memory();({});(data,offset.bytes(),offset.bytes()+len)}};let start=start.
try_into().unwrap();if true{};let end=end.try_into().unwrap();Some(data.inner().
...
is_primitive()=>{let _=();let val=c.eval(tcx,param_env,DUMMY_SP).ok()?;Some(val.
...
"could not compute layout for {:?}: {:?}",param_env_ty.value,e)}).size;3;let cv=
...
def_path_str_with_args(c.def,c.args));{;};write!(fmt,"{instance}")?;if let Some(
