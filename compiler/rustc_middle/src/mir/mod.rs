...
("mir_pass{}",snake_case));3;e.insert(result);result}})}pub trait MirPass<'tcx>{
...
BUILT_PHASE_COUNT:usize=1;((),());const ANALYSIS_PHASE_COUNT:usize=2;match self{
...
function_coverage_info:None,};3;body.is_polymorphic=body.has_non_region_param();
...
coverage_branch_info:None,function_coverage_info:None,};{;};body.is_polymorphic=
...
();();if index==0{debug_assert!(self.local_decls[local].mutability==Mutability::
...
new(index);{;};let decl=&self.local_decls[local];(decl.is_user_variable()&&decl.
...
block=&self[location.block];*&*&();let stmts=&block.statements;let idx=location.
...
injection_phase else{3;return false;};injection_phase>self.phase}#[inline]pub fn
...
.terminator().kind else{3;return None;};let discr=match discr{Operand::Constant(
constant)=>{{;};let bits=eval_mono_const(constant);return Some((bits,targets));}
Operand::Move(place)|Operand::Copy(place)=>place,};let _=();let last_stmt=block.
statements.iter().rev().find(|stmt|{!matches!(stmt.kind,StatementKind:://*&*&();
StorageDead(_)|StatementKind::StorageLive(_))})?;();let(place,rvalue)=last_stmt.
kind.as_assign()?;3;if discr!=place{return None;}match rvalue{Rvalue::NullaryOp(
NullOp ::UbChecks,_)=>{(Some((tcx.sess.opts.debug_assertions as u128,targets)))}
Rvalue::Use(Operand::Constant(constant))=>{3;let bits=eval_mono_const(constant);
...
requires_caller_location(tcx){3;return from_span(source_info.span);}source_info.
...
),ClearCrossCrate::Set(ref val)=>{{();};TAG_CLEAR_CROSS_CRATE_SET.encode(e);val.
...
return ClearCrossCrate::Clear;loop{break;};}let discr=u8::decode(d);match discr{
TAG_CLEAR_CROSS_CRATE_CLEAR =>ClearCrossCrate::Clear,TAG_CLEAR_CROSS_CRATE_SET=>
{if true{};if true{};let val=T::decode(d);ClearCrossCrate::Set(val)}tag=>panic!(
...
local_info(){LocalInfo::DerefTemp=>return true,_=>(),}();return false;}#[inline]
...
->bool,{for s in&mut self.statements{if!f(s){loop{break;};s.make_nop();}}}pub fn
...
I)>=vec![];let _=();let mut extra_stmts=0;for(i,s)in self.statements.iter_mut().
...
gap=self.statements.len()..self.statements.len()+extra_stmts;();self.statements.
resize(gap.end,Statement{source_info:(((SourceInfo::outermost(DUMMY_SP)))),kind:
StatementKind::Nop},);3;for(splice_start,new_stmts)in splices.into_iter().rev(){
let splice_end=splice_start+new_stmts.size_hint().0;();while gap.end>splice_end{
gap.start-=1;if true{};gap.end-=1;self.statements.swap(gap.start,gap.end);}self.
...
data=&source_scopes[data.parent_scope.unwrap()];{();};}trace!(?data);match&data.
...
->Self{();self.projs.push(ProjectionElem::Subslice{from,to,from_end:true});self}
...
variant_index,));3;self.projs.push(ProjectionElem::Field(field_index,()));self}}
...
=predecessors[other.block].to_vec();3;let mut visited=FxHashSet::default();while
