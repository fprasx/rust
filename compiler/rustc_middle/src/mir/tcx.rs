...
assert!(adt_def.is_enum());({});adt_def.variant(variant_index)}};let field_def=&
...
BinaryOp(op,box(ref lhs,ref rhs))=>{{();};let lhs_ty=lhs.ty(local_decls,tcx);let
rhs_ty=rhs.ty(local_decls,tcx);;op.ty(tcx,lhs_ty,rhs_ty)}Rvalue::CheckedBinaryOp
(op,box(ref lhs,ref rhs))=>{3;let lhs_ty=lhs.ty(local_decls,tcx);let rhs_ty=rhs.
...
BitOr=>{({});assert_eq!(lhs_ty,rhs_ty);lhs_ty}&BinOp::Shl|&BinOp::ShlUnchecked|&
