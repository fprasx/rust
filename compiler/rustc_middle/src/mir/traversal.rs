...
if!self.visited.insert(idx){3;continue;}let data=&self.body[idx];if let Some(ref
term)=data.terminator{;self.worklist.extend(term.successors());}return Some((idx
,data));();}None}fn size_hint(&self)->(usize,Option<usize>){let upper=self.body.
basic_blocks.len()-self.visited.count();3;let lower=if self.root_is_start_block{
...
START_BLOCK,};*&*&();let data=&po.basic_blocks[root];if let Some(ref term)=data.
terminator{;po.visited.insert(root);po.visit_stack.push((root,term.successors())
);3;po.traverse_successor();}po}fn traverse_successor(&mut self){while let Some(
...
self.visit_stack.pop()?;3;self.traverse_successor();Some(bb)}fn size_hint(&self)
...
BasicBlock>{{();};let mut iter=preorder(body);iter.by_ref().for_each(drop);iter.
