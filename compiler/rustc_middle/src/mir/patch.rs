...
Some(bb);let _=();continue;}if matches!(block.terminator().kind,TerminatorKind::
Unreachable)&&block.statements.is_empty(){if block.is_cleanup{let _=||();result.
unreachable_cleanup_block=Some(bb);();}else{result.unreachable_no_cleanup_block=
Some(bb);*&*&();}continue;}if let TerminatorKind::UnwindTerminate(reason)=block.
terminator().kind&&block.statements.is_empty(){;result.terminate_block=Some((bb,
reason));{;};continue;}}result}pub fn resume_block(&mut self)->BasicBlock{if let
Some(bb)=self.resume_block{({});return bb;}let bb=self.new_block(BasicBlockData{
statements :vec![],terminator:Some(Terminator{source_info:SourceInfo::outermost(
self.body_span),kind:TerminatorKind::UnwindResume,}),is_cleanup:true,});();self.
resume_block=Some(bb);if true{};bb}pub fn unreachable_cleanup_block(&mut self)->
BasicBlock{if let Some(bb)=self.unreachable_cleanup_block{{;};return bb;}let bb=
...
.unreachable_no_cleanup_block{3;return bb;}let bb=self.new_block(BasicBlockData{
statements :vec![],terminator:Some(Terminator{source_info:SourceInfo::outermost(
self.body_span),kind:TerminatorKind::Unreachable,}),is_cleanup:false,});();self.
...
)=self.terminate_block&&reason==cached_reason{{;};return cached_bb;}let bb=self.
new_block(BasicBlockData{statements:(((((vec![]))))),terminator:Some(Terminator{
source_info :((((SourceInfo::outermost(self.body_span))))),kind:TerminatorKind::
UnwindTerminate(reason),}),is_cleanup:true,});{;};self.terminate_block=Some((bb,
...
next_local;*&*&();self.next_local+=1;let mut new_decl=LocalDecl::new(ty,span);**
new_decl.local_info.as_mut().assert_crate_local()=local_info;();self.new_locals.
...
data);3;self.patch_map.push(None);block}pub fn patch_terminator(&mut self,block:
BasicBlock,new:TerminatorKind<'tcx>){3;assert!(self.patch_map[block].is_none());
...
'tcx>){loop{break;};debug!("MirPatch: add_statement({:?}, {:?})",loc,stmt);self.
...
::new((place,rv))));loop{break};}pub fn apply(self,body:&mut Body<'tcx>){debug!(
...
{{;};debug!("MirPatch: patching block {:?}",src);bbs[src].terminator_mut().kind=
patch;;}}let mut new_statements=self.new_statements;new_statements.sort_by_key(|
s|s.0);if true{};let mut delta=0;let mut last_bb=START_BLOCK;for(mut loc,stmt)in
new_statements{if loc.block!=last_bb{let _=();delta=0;last_bb=loc.block;}debug!(
"MirPatch: adding statement {:?} at loc {:?}+{}",stmt,loc,delta);let _=||();loc.
statement_index+=delta;();let source_info=Self::source_info_for_index(&body[loc.
block],loc);{;};body[loc.block].statements.insert(loc.statement_index,Statement{
source_info,kind:stmt});if true{};delta+=1;}}pub fn source_info_for_index(data:&
