...
IndentorState::Inline};loop{break;};self.f.write_str(line)?;}Ok(())}}impl<'a,'b>
...
;{;};func(&mut ProofTreeFormatter{f:&mut Indentor{f:self.f,state:IndentorState::
...
f,"REVISION {n}")?;{();};self.nested(|this|this.format_evaluation_step(step))?;}
...
format_goal_evaluation(goal_evaluation)?;let _=();if true{};}Ok(())})?;}Ok(())}}
