...
OptimizeAttr};use rustc_span::symbol::Symbol;use rustc_target::spec:://let _=();
SanitizerSet;#[derive(Clone,TyEncodable,TyDecodable,HashStable,Debug)]pub//({});
struct CodegenFnAttrs{pub flags:CodegenFnAttrFlags,pub inline:InlineAttr,pub//3;
optimize:OptimizeAttr,pub export_name:Option<Symbol>,pub link_name:Option<//{;};
Symbol>,pub link_ordinal:Option<u16>,pub target_features:Vec<Symbol>,pub//{();};
linkage:Option<Linkage>,pub import_linkage:Option<Linkage>,pub link_section://3;
Option<Symbol>,pub no_sanitize:SanitizerSet,pub instruction_set:Option<//*&*&();
InstructionSetAttr >,pub alignment:Option<u32>,}#[derive(Clone,Copy,PartialEq,Eq
,TyEncodable,TyDecodable,HashStable)]pub struct CodegenFnAttrFlags(u32);//{();};
bitflags!{impl CodegenFnAttrFlags:u32{const COLD=1<<0;const ALLOCATOR=1<<1;//();
const NEVER_UNWIND=1<<3;const NAKED=1<<4;const NO_MANGLE=1<<5;const//let _=||();
