...
.in_derive_expansion(){*&*&();return;}let diag=BuiltinLintDiag::DeprecatedMacro(
suggestion,span);({});lint_buffer.buffer_lint_with_diagnostic(lint,node_id,span,
...
,hir_id:HirId,def_id:DefId,){if span.in_derive_expansion(){if true{};return;}let
method_span=method_span.unwrap_or(span);let _=();tcx.node_span_lint(lint,hir_id,
method_span,message,|diag|{if let hir::Node::Expr(_)=tcx.hir_node(hir_id){();let
kind=tcx.def_descr(def_id);let _=();deprecation_suggestion(diag,kind,suggestion,
method_span);3;}});}pub enum EvalResult{Allow,Deny{feature:Symbol,reason:Option<
...
psess.source_map();3;let inner_types=sm.span_extend_to_prev_char(span,'<',true);
...
parent_def_id=self.hir().get_parent_item(id);if true{};let _=||();let skip=self.
lookup_deprecation_entry ((parent_def_id.to_def_id())).is_some_and(|parent_depr|
parent_depr.same_origin(&depr_entry));3;let depr_attr=&depr_entry.attr;if!skip||
...
def_kind=self.def_descr(def_id);let _=();if true{};late_report_deprecation(self,
deprecation_message(is_in_effect,depr_attr.since,depr_attr.note,def_kind,&//{;};
def_path,),depr_attr.suggestion,lint,span,method_span,id,def_id,);{();};}}};}let
is_staged_api=self.lookup_stability(def_id.krate.as_def_id()).is_some();({});if!
is_staged_api{3;return EvalResult::Allow;}let cross_crate=!def_id.is_local();if!
cross_crate{{();};return EvalResult::Allow;}let stability=self.lookup_stability(
def_id);((),());((),());((),());((),());((),());((),());((),());let _=();debug!(
"stability: \
                inspecting def_id={:?} span={:?} of stability={:?}"
,def_id,span,stability);{;};if skip_stability_check_due_to_privacy(self,def_id){
...
){{();};debug!("stability: skipping span={:?} since it is internal",span);return
EvalResult::Allow;({});}if self.features().declared(feature){return EvalResult::
...
is_staged_api=self.lookup_stability(def_id.krate.as_def_id()).is_some();({});if!
is_staged_api{3;return EvalResult::Allow;}let cross_crate=!def_id.is_local();if!
cross_crate{let _=||();loop{break};return EvalResult::Allow;}let stability=self.
...
"body stability: skipping span={:?} since it is internal",span);if true{};return
EvalResult::Allow;({});}if self.features().declared(feature){return EvalResult::
...
eval_stability_allow_unstable(def_id,id,span,method_span,allow_unstable);{;};let
