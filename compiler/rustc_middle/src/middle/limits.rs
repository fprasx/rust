...
Option<usize>{for attr in krate_attrs{if!attr.has_name(name){();continue;}if let
...
unwrap_or(attr.span);();let error_str=match e.kind(){IntErrorKind::PosOverflow=>
...
"unimplemented IntErrorKind variant: {:?}",kind),};let _=();sess.dcx().emit_err(
