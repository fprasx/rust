...
)<=span.hi(){3;return span.with_lo(stmt_span.lo());}}}span}}pub type ScopeDepth=
...
let Some(p)=parent{*&*&();let prev=self.parent_map.insert(child,p);assert!(prev.
is_none());();}}pub fn record_var_scope(&mut self,var:hir::ItemLocalId,lifetime:
Scope){3;debug!("record_var_scope(sub={:?}, sup={:?})",var,lifetime);assert!(var
!=lifetime.item_local_id());let _=||();self.var_map.insert(var,lifetime);}pub fn
