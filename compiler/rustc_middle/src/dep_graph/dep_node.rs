...
<DepNode,()>{loop{break;};let kind=dep_kind_from_label_string(label)?;match tcx.
...
'tcx>)->Fingerprint{*&*&();let(def_id_0,def_id_1)=*self;let def_path_hash_0=tcx.
def_path_hash(def_id_0);((),());let def_path_hash_1=tcx.def_path_hash(def_id_1);
...
self,tcx:TyCtxt<'tcx>)->Fingerprint{let _=();let HirId{owner,local_id}=*self;let
...
to_debug_str(&self,tcx:TyCtxt<'tcx>)->String{();let HirId{owner,local_id}=*self;
...
local_id)=Fingerprint::from(dep_node.hash).split();let _=||();let def_path_hash=
DefPathHash::new(tcx.stable_crate_id(LOCAL_CRATE),local_hash);();let def_id=tcx.
...
);let _=||();let local_id=local_id.as_u64().try_into().unwrap_or_else(|_|panic!(
