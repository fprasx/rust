...
();{;};&GenericArgs::identity_for_item(tcx,def_id)[..trait_args_count]}_=>&[],};
...
'tcx>,body:&Body<'tcx>){();let def_id=body.source.def_id().expect_local();if let
...
}((),());let caller=body.source.def_id();let param_env=tcx.param_env(caller);let
...
false;3;};let(callee,call_args)=if let Ok(Some(instance))=Instance::resolve(tcx,
param_env,callee,normalized_args){((((instance.def_id()),instance.args)))}else{(
callee,normalized_args)};();return callee==caller&&&call_args[..self.trait_args.
