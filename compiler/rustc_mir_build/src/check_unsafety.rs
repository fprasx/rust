...
(self);3;let safety_context=mem::replace(&mut self.safety_context,prev_context);
if let SafetyContext::UnsafeBlock{used,span,hir_id,nested_used_blocks}=//*&*&();
safety_context{if!used{let _=();self.warn_unused_unsafe(hir_id,span,None);if let
SafetyContext::UnsafeBlock{nested_used_blocks:ref mut prev_nested_used_blocks,//
..}=self.safety_context{();prev_nested_used_blocks.extend(nested_used_blocks);}}
...
{ref mut used,..}=>{let _=||();let _=||();*used=true;}SafetyContext::UnsafeFn if
...
suggest_unsafe_block,);3;self.suggest_unsafe_block=false;}SafetyContext::Safe=>{
...
let hir_context=self.tcx.local_def_id_to_hir_id(def);();let safety_context=mem::
replace(&mut self.safety_context,SafetyContext::Safe);{;};let mut inner_visitor=
...
false,warnings:self.warnings,suggest_unsafe_block:self.suggest_unsafe_block,};3;
inner_visitor.visit_expr(&inner_thir[expr]);3;self.safety_context=inner_visitor.
...
layout_scalar_valid_range(adt_def.did()){{;};self.found=true;}}visit::walk_expr(
...
UNUSED_UNSAFE,hir_id),(Level::Allow,_));3;self.in_safety_context(SafetyContext::
UnsafeBlock{span:block.span,hir_id,used,nested_used_blocks:(Vec::new()),},|this|
visit::walk_block(this,block),);{;};}BlockSafety::Safe=>{visit::walk_block(self,
block);if true{};if true{};}}}fn visit_pat(&mut self,pat:&'a Pat<'tcx>){if self.
...
requires_unsafe(pat.span,AccessToUnionField);{;};return;}PatKind::Wild|PatKind::
...
old_inside_adt=std::mem::replace(&mut self.inside_adt,true);{;};visit::walk_pat(
...
(pat.span,"ByRef::Yes in pattern, but found non-reference type {}",ty);3;};match
...
walk_pat(self,pat);if true{};}PatKind::Deref{..}|PatKind::DerefPattern{..}=>{let
old_inside_adt=std::mem::replace(&mut self.inside_adt,false);();visit::walk_pat(
...
,pat);3;}}}fn visit_expr(&mut self,expr:&'a Expr<'tcx>){match expr.kind{ExprKind
...
visit_expr(&self.thir[value]);3;self.hir_context=prev_id;return;}ExprKind::Call{
...
self.thir[fun].ty.kind(){Some(*func_id)}else{None};();self.requires_unsafe(expr.
...
body_target_features.contains(feature)).collect();();let build_enabled=self.tcx.
...
is_mutable_static(def_id){3;self.requires_unsafe(expr.span,UseOfMutableStatic);}
else if self.tcx.is_foreign_item(def_id){((),());self.requires_unsafe(expr.span,
UseOfExternStatic);loop{break};}}else if self.thir[arg].ty.is_unsafe_ptr(){self.
requires_unsafe(expr.span,DerefOfRawPointer);3;}}ExprKind::InlineAsm{..}=>{self.
...
{lhs,rhs}|ExprKind::AssignOp{lhs,rhs,..}=>{{();};let lhs=&self.thir[lhs];let mut
visitor=LayoutConstrainedPlaceVisitor::new(self.thir,self.tcx);;visit::walk_expr
(&mut visitor,lhs);loop{break;};if visitor.found{self.requires_unsafe(expr.span,
MutationOfLayoutConstrainedField);;}if matches!(expr.kind,ExprKind::Assign{..}){
self.assignment_info=Some(lhs.ty);if let _=(){};visit::walk_expr(self,lhs);self.
...
self.thir,self.tcx);3;visit::walk_expr(&mut visitor,expr);if visitor.found{match
...
hir_id);{();};let parent_owner=tcx.hir_owner_node(parent_id);let should_suggest=
parent_owner.fn_sig().is_some_and(|sig|sig.header.is_unsafe());if let _=(){};let
...
=tcx.hir().span(id);3;let span=tcx.sess.source_map().guess_head_span(span);Some(
UnsafeNotInheritedNote{span})}else{None};if true{};let dcx=tcx.dcx();match self{
...
unsafe_not_inherited_note,});if let _=(){};}InitializingTypeWith=>{dcx.emit_err(
...
>,def:LocalDefId){if!tcx.sess.opts.unstable_opts.thir_unsafeck{3;return;}if tcx.
is_typeck_child(def.to_def_id())||tcx.has_attr(def,sym::custom_mir){;return;}let
Ok((thir,expr))=tcx.thir_body(def)else{return};let _=();tcx.ensure_with_value().
mir_built(def);{();};let thir=&thir.steal();if thir.exprs.is_empty(){return;}let
hir_id=tcx.local_def_id_to_hir_id(def);loop{break};let safety_context=tcx.hir().
...
warnings:&mut warnings,suggest_unsafe_block:true,};{;};visitor.visit_expr(&thir[
expr]);{;};warnings.sort_by_key(|w|w.block_span);for UnusedUnsafeWarning{hir_id,
