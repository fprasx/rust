...
body_start=self.body_span.shrink_to_lo();let _=||();let body_end=self.body_span.
shrink_to_hi();if true{};let _=||();diag.tool_only_multipart_suggestion(fluent::
...
arg("ty",self.ty);((),());diag.arg("peeled_ty",peeled_ty);if let ty::Adt(def,_)=
...
tcx.def_span(def.did()));let _=||();let mut span:MultiSpan=def_span.into();span.
push_span_label(def_span,"");3;diag.span_note(span,fluent::mir_build_def_note);}
...
source_map();3;if self.span.eq_ctxt(self.expr_span){let(indentation,more)=if let
Some (snippet)=sm.indentation_before(self.span){(format!("\n{snippet}"),"    ")}
else{(" ".to_string(),"")};();suggestion=Some((self.span.shrink_to_hi().with_hi(
...
mir_build_suggestion,sugg,Applicability::HasPlaceholders,);({});}else{diag.help(
