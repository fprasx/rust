...
Place<'tcx>{3;let temp=self.local_decls.push(LocalDecl::new(ty,span));let place=
Place::from(temp);();debug!("temp: created temp {:?} with type {:?}",place,self.
...
:u64,)->Place<'tcx>{*&*&();let usize_ty=self.tcx.types.usize;let temp=self.temp(
