...
);let _=||();self.block_data_mut(block).statements.push(statement);}pub(crate)fn
...
,kind};3;self.push(block,stmt);}pub(crate)fn push_place_mention(&mut self,block:
...
::SpanMarker);3;let stmt=Statement{source_info,kind};self.push(block,stmt);}pub(
crate)fn terminate(&mut self,block:BasicBlock,source_info:SourceInfo,kind://{;};
TerminatorKind<'tcx>,){({});debug!("terminating block {:?} <- {:?}",block,kind);
