...
assert_matches!(thir[unary_not].kind,ExprKind::Unary{op:UnOp::Not,..});{;};self.
...
is_flipped,..not_info};3;self.visit_with_not_info(thir,arg,not_info);}ExprKind::
...
{((),());std::mem::swap(&mut then_block,&mut else_block);}}let source_info=self.
source_info(self.thir[expr_id].span);;let branch_info=self.coverage_branch_info.
as_mut().expect("upgrading & to &mut");({});let mut inject_branch_marker=|block:
BasicBlock|{3;let id=branch_info.next_block_marker_id();let marker_statement=mir
...
inject_branch_marker(then_block);let _=();let false_marker=inject_branch_marker(
else_block);({});branch_info.branch_spans.push(BranchSpan{span:source_info.span,
