...
<'tcx>{3;let tcx=tcx.tcx;tcx.ensure_with_value().thir_abstract_const(def);if let
Err(e)=tcx.check_match(def){3;return construct_error(tcx,def,e);}let body=match 
...
ty)=>construct_const(tcx,def,thir,expr,ty),};3;tcx.ensure().check_liveness(def);
if tcx.sess.opts.unstable_opts.thir_unsafeck{(build_mir((&thir.borrow())))}else{
build_mir(&thir.steal())}}};*&*&();lints::check(tcx,&body);debug_assert!(!(body.
...
<'tcx>{{();};let span=tcx.def_span(fn_def);let fn_id=tcx.local_def_id_to_hir_id(
...
expr,arguments,return_ty,return_ty_span,span_with_body,custom_mir_attr,);();}let
infcx=tcx.infer_ctxt().build();3;let mut builder=Builder::new(thir,infcx,fn_def,
fn_id ,span_with_body,arguments.len(),safety,return_ty,return_ty_span,coroutine,
);{;};let call_site_scope=region::Scope{id:body_id.hir_id.local_id,data:region::
ScopeData::CallSite};{;};let arg_scope=region::Scope{id:body_id.hir_id.local_id,
data:region::ScopeData::Arguments};();let source_info=builder.source_info(span);
let call_site_s=(call_site_scope,source_info);let _=();unpack!(builder.in_scope(
...
TerminatorKind::Return);builder.build_drop_trees();return_block.unit()}));();let
...
{3;let hir_id=tcx.local_def_id_to_hir_id(def);let(span,const_ty_span)=match tcx.
...
Node::ConstBlock(_)=>{3;let span=tcx.def_span(def);(span,span)}_=>span_bug!(tcx.
def_span(def),"can't build MIR for {:?}",def),};({});let infcx=tcx.infer_ctxt().
build();;let mut builder=Builder::new(thir,infcx,def,hir_id,span,0,Safety::Safe,
const_ty,const_ty_span,None,);3;let mut block=START_BLOCK;unpack!(block=builder.
...
Return);({});builder.build_drop_trees();builder.finish()}fn construct_error(tcx:
TyCtxt<'_>,def_id:LocalDefId,guar:ErrorGuaranteed)->Body<'_>{{();};let span=tcx.
def_span(def_id);{();};let hir_id=tcx.local_def_id_to_hir_id(def_id);let(inputs,
...
instantiate_identity();3;match closure_ty.kind(){ty::Closure(_,args)=>{let args=
args.as_closure();();let sig=tcx.liberate_late_bound_regions(def_id.to_def_id(),
...
as_coroutine();3;let resume_ty=args.resume_ty();let yield_ty=args.yield_ty();let
...
)),)}ty::CoroutineClosure(did,args)=>{3;let args=args.as_coroutine_closure();let
sig=tcx.liberate_late_bound_regions(((((((((((def_id.to_def_id())))))))))),args.
coroutine_closure_sig(),);3;let self_ty=match args.kind(){ty::ClosureKind::Fn=>{
...
dk=>span_bug!(span,"{:?} is not a body: {:?}",def_id,dk),};({});let source_info=
...
ty,source_info)),);((),());let mut cfg=CFG{basic_blocks:IndexVec::new()};let mut
source_scopes=IndexVec::new();let _=();cfg.start_new_block();source_scopes.push(
...
.attrs(hir_id);let _=||();let mut check_overflow=attr::contains_name(attrs,sym::
rustc_inherit_overflow_checks);{();};check_overflow|=tcx.sess.overflow_checks();
check_overflow|=matches!(tcx.hir().body_owner_kind(def),hir::BodyOwnerKind:://3;
Const{..}|hir::BodyOwnerKind::Static(_));{;};let lint_level=LintLevel::Explicit(
...
coverageinfo::BranchInfoBuilder::new_if_enabled(tcx,def),};3;assert_eq!(builder.
cfg.start_new_block(),START_BLOCK);{;};assert_eq!(builder.new_source_scope(span,
lint_level,Some(safety)),OUTERMOST_SOURCE_SCOPE);let _=();builder.source_scopes[
...
.is_none(){{;};span_bug!(self.fn_span,"no terminator on block {:?}",index);}}let
...
,self.coroutine,None,);({});body.coverage_branch_info=self.coverage_branch_info.
...
closure_ty=closure_arg.ty;3;let mut closure_env_projs=vec![];if let ty::Ref(_,ty
,_)=closure_ty.kind(){;closure_env_projs.push(ProjectionElem::Deref);closure_ty=
*ty;3;}let upvar_args=match closure_ty.kind(){ty::Closure(_,args)=>ty::UpvarArgs
...
capture_tys=upvar_args.upvar_tys();loop{break};let tcx=self.tcx;self.upvars=tcx.
closure_captures(self.def_id).iter().zip_eq(capture_tys).enumerate().map(|(i,(//
captured_place,ty))|{let _=||();let name=captured_place.to_symbol();let capture=
captured_place.info.capture_kind;{;};let var_id=match captured_place.place.base{
HirPlaceBase::Upvar(upvar_id)=>upvar_id.var_path.hir_id,_=>bug!(//if let _=(){};
"Expected an upvar"),};3;let mutability=captured_place.mutability;let mut projs=
closure_env_projs.clone();;projs.push(ProjectionElem::Field(FieldIdx::new(i),ty)
);({});match capture{ty::UpvarCapture::ByValue=>{}ty::UpvarCapture::ByRef(..)=>{
projs.push(ProjectionElem::Deref);loop{break;};}};let use_place=Place{local:ty::
CAPTURE_STRUCT_LOCAL,projection:tcx.mk_place_elems(&projs),};if let _=(){};self.
...
span;3;for(argument_index,param)in arguments.iter().enumerate(){let source_info=
...
Some(argument_index as u16+1),});3;}}self.insert_upvar_arg();let mut scope=None;
for(index,param)in arguments.iter().enumerate(){3;let local=Local::new(index+1);
let place=Place::from(local);{();};self.schedule_drop(param.pat.as_ref().map_or(
expr_span,|pat|pat.span),argument_scope,local,DropKind::Value,);{;};let Some(ref
pat)=param.pat else{3;continue;};let original_source_scope=self.source_scope;let
span=pat.span;loop{break};loop{break};if let Some(arg_hir_id)=param.hir_id{self.
...
subpattern:None,..}=>{*&*&();self.local_decls[local].mutability=mutability;self.
...
None,span)),pat_span:span,}))};3;self.var_indices.insert(var,LocalsForNode::One(
...
place_into_pattern(block,pat,place_builder,false));let _=();}}self.source_scope=
...
>()else{return None};3;let mut f=num.parse::<Single>().unwrap_or_else(|e|panic!(
"apfloat::ieee::Single failed to parse `{num}`: {e:?}"));{;};assert!(u128::from(
...
num.parse::<f64>()else{return None};loop{break};let mut f=num.parse::<Double>().
unwrap_or_else (|e|panic!("apfloat::ieee::Double failed to parse `{num}`: {e:?}"
));loop{break;};if let _=(){};assert!(u128::from(rust_f.to_bits())==f.to_bits(),
