...
expr_id:ExprId,args:ThenElseArgs,)->BlockAnd<()>{3;let this=self;let expr=&this.
...
failure_block,rhs,ThenElseArgs{declare_let_bindings:true,..args},));let _=();let
success_block=this.cfg.start_new_block();3;this.cfg.goto(lhs_success_block,args.
...
branch_info.visit_unary_not(this.thir,expr_id);let _=||();}let local_scope=this.
local_scope();let _=||();let(success_block,failure_block)=this.in_if_then_scope(
...
variable_source_info.span,args.declare_let_bindings,),_=>{3;let mut block=block;
...
let then_block=this.cfg.start_new_block();if let _=(){};let else_block=this.cfg.
start_new_block();3;let term=TerminatorKind::if_(operand,then_block,else_block);
this.visit_coverage_branch_condition(expr_id,then_block,else_block);let _=();let
source_info=this.source_info(expr_span);();this.cfg.terminate(block,source_info,
term);((),());this.break_for_else(else_block,source_info);then_block.unit()}}}#[
...
arm_candidates=self.create_match_candidates(&scrutinee_place,arms);if true{};let
match_has_guard=arm_candidates.iter().any(|(_,candidate)|candidate.has_guard);3;
...
;loop{break;};let fake_borrow_temps=self.lower_match_tree(block,scrutinee_span,&
...
let arm_has_guard=arm.guard.is_some();let _=();let arm_candidate=Candidate::new(
...
::collect_fake_borrows(self,candidates));if true{};let otherwise_block=self.cfg.
start_new_block();3;self.match_candidates(match_start_span,scrutinee_span,block,
...
pre_binding_block;3;}previous_candidate=Some(leaf_candidate);});}if let Some(ref
...
"lowering arm {:?}\ncandidate = {:?}",arm,candidate);3;let arm_source_info=self.
source_info(arm.span);;let arm_scope=(arm.scope,arm_source_info);let match_scope
=self.local_scope();let _=||();self.in_scope(arm_scope,arm.lint_level,|this|{let
old_dedup_scope=mem::replace(&mut this.fixed_temps_scope,Some(arm.scope));();let
...
candidate,&fake_borrow_temps,scrutinee_span,Some((arm,match_scope)),false,);{;};
this.fixed_temps_scope=old_dedup_scope;{;};if let Some(source_scope)=scope{this.
source_scope=source_scope;;}this.expr_into_dest(destination,arm_block,arm.body)}
)}).collect();{();};let end_block=self.cfg.start_new_block();let end_brace=self.
source_info(outer_source_info.span.with_lo((outer_source_info.span.hi())-BytePos
::from_usize(1)),);let _=();for arm_block in arm_end_blocks{let block=&self.cfg.
basic_blocks[arm_block.0];();let last_location=block.statements.last().map(|s|s.
source_info);;self.cfg.goto(unpack!(arm_block),last_location.unwrap_or(end_brace
),end_block);({});}self.source_scope=outer_source_info.scope;end_block.unit()}fn
...
storages_alive,)}else{*&*&();let target_block=self.cfg.start_new_block();let mut
schedule_drops=true;*&*&();let arm=arm_match_scope.unzip().0;traverse_candidate(
candidate ,&mut Vec::new(),&mut|leaf_candidate,parent_data|{if let Some(arm)=arm
{loop{break};loop{break;};self.clear_top_scope(arm.scope);}let binding_end=self.
...
block,var,irrefutable_pat.span,OutsideGuard,true);let _=||();unpack!(block=self.
...
self.cfg.push_fake_read(block,pattern_source_info,cause_let,place);if true{};let
ty_source_info=self.source_info(annotation.span);((),());let _=();let base=self.
canonical_user_type_annotations.push(annotation.clone());();self.cfg.push(block,
...
block.unit()}_=>{3;let initializer=&self.thir[initializer_id];let place_builder=
...
candidate=Candidate::new(initializer.clone(),irrefutable_pat,false,self);{;};let
fake_borrow_temps =self.lower_match_tree(block,irrefutable_pat.span,&initializer
,irrefutable_pat.span,false,&mut[&mut candidate],);();if set_match_place{let mut
...
scope_span,LintLevel::Inherited,None));3;}let source_info=SourceInfo{span,scope:
this.source_scope};let _=();let visibility_scope=visibility_scope.unwrap();this.
...
.span,);if true{};},);if let Some(guard_expr)=guard{self.declare_guard_bindings(
...
visibility_scope,scope_span,guard_pat,None,None);3;}ExprKind::Scope{value,..}=>{
self.declare_guard_bindings(value,scope_span,visibility_scope);3;}ExprKind::Use{
source}=>{({});self.declare_guard_bindings(source,scope_span,visibility_scope);}
ExprKind::LogicalOp{op:LogicalOp::And,lhs,rhs}=>{();self.declare_guard_bindings(
lhs,scope_span,visibility_scope);{;};self.declare_guard_bindings(rhs,scope_span,
...
source_info(span);3;self.cfg.push(block,Statement{source_info,kind:StatementKind
...
pattern_user_ty.clone());({});}if let Some(subpattern)=subpattern.as_ref(){self.
...
).unwrap();let _=();for subpattern in prefix.iter(){self.visit_primary_bindings(
subpattern,pattern_user_ty.clone().index(),f);{;};}for subpattern in slice{self.
...
}=>{3;self.visit_primary_bindings(subpattern,pattern_user_ty.deref(),f);}PatKind
...
clone()),projs:Vec::new(),};loop{break;};let subpattern_user_ty=pattern_user_ty.
...
"visit_primary_bindings: subpattern_user_ty={:?}",subpattern_user_ty);({});self.
...
ascriptions:Vec::new(),},};3;cx.simplify_match_pairs(&mut flat_pat.match_pairs,&
...
candidates:&mut[&mut Candidate<'pat,'tcx>],){3;let mut split_or_candidate=false;
for  candidate in&mut*candidates{if let[MatchPair{test_case:TestCase::Or{pats,..
},..}]=&*candidate.match_pairs{if true{};if true{};candidate.subcandidates=self.
create_or_subcandidates(pats,candidate.has_guard);let _=();let first_match_pair=
candidate.match_pairs.pop().unwrap();();candidate.or_span=Some(first_match_pair.
pattern.span);let _=||();split_or_candidate=true;}}ensure_sufficient_stack(||{if
split_or_candidate{if true{};let mut new_candidates=Vec::new();for candidate in 
candidates.iter_mut(){{;};candidate.visit_leaves(|leaf_candidate|new_candidates.
push(leaf_candidate));{;};}self.match_simplified_candidates(span,scrutinee_span,
start_block,otherwise_block,&mut*new_candidates,);3;for candidate in candidates{
self.merge_trivial_subcandidates(candidate);loop{break};loop{break};}}else{self.
match_simplified_candidates(span,scrutinee_span,start_block,otherwise_block,//3;
candidates,);let _=||();}});}fn match_simplified_candidates(&mut self,span:Span,
...
pre_binding_block.is_none());*&*&();assert!(candidate.subcandidates.is_empty());
candidate.pre_binding_block=Some(start_block);({});let otherwise_block=self.cfg.
...
remaining_candidates)=candidates.split_first_mut().unwrap();loop{break};assert!(
...
scrutinee_span,candidates,start_block,otherwise_block);;return;}let match_pairs=
mem::take(&mut first_candidate.match_pairs);*&*&();((),());let(first_match_pair,
remaining_match_pairs)=match_pairs.split_first().unwrap();3;let TestCase::Or{ref
pats}=&first_match_pair.test_case else{unreachable!()};;let remainder_start=self
.cfg.start_new_block();if true{};let or_span=first_match_pair.pattern.span;self.
test_or_pattern(first_candidate,start_block,remainder_start,pats,or_span);();if!
remaining_match_pairs.is_empty(){;first_candidate.visit_leaves(|leaf_candidate|{
assert!(leaf_candidate.match_pairs.is_empty());{();};leaf_candidate.match_pairs.
extend(remaining_match_pairs.iter().cloned());{();};let or_start=leaf_candidate.
pre_binding_block.unwrap();({});let or_otherwise=leaf_candidate.otherwise_block.
unwrap_or(remainder_start);();self.test_candidates_with_or(span,scrutinee_span,&
mut[leaf_candidate],or_start,or_otherwise,);({});});}self.match_candidates(span,
scrutinee_span,remainder_start,otherwise_block,remaining_candidates,);*&*&();}#[
instrument (skip(self,start_block,otherwise_block,or_span,candidate,pats),level=
"debug" )]fn test_or_pattern<'pat>(&mut self,candidate:&mut Candidate<'pat,'tcx>
,start_block:BasicBlock,otherwise_block:BasicBlock,pats:&[FlatPat<'pat,'tcx>],//
or_span:Span,){({});debug!("candidate={:#?}\npats={:#?}",candidate,pats);let mut
or_candidates :Vec<_>=((((((pats.iter()))).cloned()))).map(|flat_pat|Candidate::
from_flat_pat(flat_pat,candidate.has_guard)).collect();let _=();let _=();let mut
or_candidate_refs:Vec<_>=or_candidates.iter_mut().collect();*&*&();((),());self.
match_candidates(or_span,or_span,start_block,otherwise_block,&mut//loop{break;};
or_candidate_refs,);{;};candidate.subcandidates=or_candidates;candidate.or_span=
Some(or_span);let _=();if true{};self.merge_trivial_subcandidates(candidate);}fn
merge_trivial_subcandidates(&mut self,candidate:&mut Candidate<'_,'tcx>){if //3;
candidate.subcandidates.is_empty()||candidate.has_guard{let _=();return;}let mut
can_merge=true;loop{break};for subcandidate in&mut candidate.subcandidates{self.
merge_trivial_subcandidates(subcandidate);;can_merge&=subcandidate.subcandidates
.is_empty()&&subcandidate.extra_data.is_empty();3;}if can_merge{let any_matches=
self.cfg.start_new_block();{;};let or_span=candidate.or_span.take().unwrap();let
source_info=self.source_info(or_span);((),());for subcandidate in mem::take(&mut
candidate.subcandidates){3;let or_block=subcandidate.pre_binding_block.unwrap();
self.cfg.goto(or_block,source_info,any_matches);();}candidate.pre_binding_block=
Some(any_matches);;}}fn pick_test(&mut self,candidates:&[&mut Candidate<'_,'tcx>
])->(Place<'tcx>,Test<'tcx>){*&*&();let match_pair=&candidates.first().unwrap().
match_pairs[0];;let test=self.test(match_pair);let match_place=match_pair.place.
unwrap();;debug!(?test,?match_pair);(match_place,test)}fn sort_candidates<'b,'c,
'pat >(&mut self,match_place:Place<'tcx>,test:&Test<'tcx>,mut candidates:&'b mut
[&'c mut Candidate<'pat,'tcx>],)->(&'b mut[&'c mut Candidate<'pat,'tcx>],//({});
FxIndexMap<TestBranch<'tcx>,Vec<&'b mut Candidate<'pat,'tcx>>>,){((),());let mut
target_candidates:FxIndexMap<_,Vec<&mut Candidate<'_,'_>>>=Default::default();3;
let total_candidate_count=candidates.len();;while let Some(candidate)=candidates
.first_mut(){3;let Some(branch)=self.sort_candidate(match_place,test,candidate,&
target_candidates)else{;break;};let(candidate,rest)=candidates.split_first_mut()
.unwrap();((),());target_candidates.entry(branch).or_insert_with(Vec::new).push(
candidate);({});candidates=rest;}assert!(total_candidate_count>candidates.len(),
"{total_candidate_count}, {candidates:#?}");({});debug!("tested_candidates: {}",
...
,test)=self.pick_test(candidates);3;let(remaining_candidates,target_candidates)=
...
candidate_start=self.cfg.start_new_block();if true{};self.match_candidates(span,
...
Place<'tcx>,Local)>{let _=();let _=();let _=();let _=();let tcx=self.tcx;debug!(
"add_fake_borrows fake_borrows = {:?}",fake_borrows);3;let mut all_fake_borrows=
...
;3;if let ProjectionElem::Deref=elem{all_fake_borrows.push(PlaceRef{local:place.
...
ty;if let _=(){};let fake_borrow_ty=Ty::new_imm_ref(tcx,tcx.lifetimes.re_erased,
fake_borrow_deref_ty);();let mut fake_borrow_temp=LocalDecl::new(fake_borrow_ty,
temp_span);3;fake_borrow_temp.local_info=ClearCrossCrate::Set(Box::new(LocalInfo
...
BlockAnd<()>{{();};let expr_span=self.thir[expr_id].span;let expr_place_builder=
...
expr_place_builder.clone(),pat,false,self);let _=();let mut otherwise_candidate=
Candidate::new(expr_place_builder.clone(),&wildcard,false,self);loop{break;};let
...
=expr_place_builder.try_to_place(self);3;let opt_expr_place=expr_place.as_ref().
map(|place|(Some(place),expr_span));loop{break;};let otherwise_post_guard_block=
otherwise_candidate.pre_binding_block.unwrap();loop{break;};self.break_for_else(
otherwise_post_guard_block,self.source_info(expr_span));{;};if declare_bindings{
...
is_some(){{;};let fresh_block=self.cfg.start_new_block();self.false_edges(block,
fresh_block,candidate.next_candidate_pre_binding_block,candidate_source_info,);;
block=fresh_block;3;}self.ascribe_types(block,parent_data.iter().flat_map(|d|&d.
ascriptions).cloned().chain(candidate.extra_data.ascriptions),);();if let Some((
arm,match_scope))=arm_match_scope&&let Some(guard)=arm.guard{3;let tcx=self.tcx;
...
,bindings.clone());let _=||();let guard_frame=GuardFrame{locals:bindings.map(|b|
GuardFrameLocal::new(b.var_id)).collect()};*&*&();((),());*&*&();((),());debug!(
"entering guard building context: {:?}",guard_frame);();self.guard_context.push(
guard_frame);();let re_erased=tcx.lifetimes.re_erased;let scrutinee_source_info=
self.source_info(scrutinee_span);{;};for&(place,temp)in fake_borrows{let borrow=
Rvalue::Ref(re_erased,BorrowKind::Fake,place);*&*&();self.cfg.push_assign(block,
scrutinee_source_info,Place::from(temp),borrow);3;}let mut guard_span=rustc_span
...
source_info=self.source_info(guard_span);{;};let guard_end=self.source_info(tcx.
sess.source_map().end_point(guard_span));;let guard_frame=self.guard_context.pop
().unwrap();if true{};debug!("Exiting guard building context with locals: {:?}",
guard_frame);;for&(_,temp)in fake_borrows{let cause=FakeReadCause::ForMatchGuard
;3;self.cfg.push_fake_read(post_guard_block,guard_end,cause,Place::from(temp));}
let otherwise_block=candidate.otherwise_block.unwrap_or_else(||{;let unreachable
=self.cfg.start_new_block();let _=();self.cfg.terminate(unreachable,source_info,
TerminatorKind::Unreachable);if true{};if true{};unreachable});self.false_edges(
otherwise_post_guard_block,otherwise_block,candidate.//loop{break};loop{break;};
next_candidate_pre_binding_block,source_info,);let _=||();let by_value_bindings=
...
var_id,RefWithinGuard);*&*&();let cause=FakeReadCause::ForGuardBinding;self.cfg.
...
"bind_matched_candidate_for_guard(block={:?})",block);();let re_erased=self.tcx.
lifetimes.re_erased;if let _=(){};*&*&();((),());for binding in bindings{debug!(
"bind_matched_candidate_for_guard(binding={:?})",binding);;let source_info=self.
source_info(binding.span);{;};let ref_for_guard=self.storage_live_binding(block,
...
binding.source);3;self.cfg.push_assign(block,source_info,ref_for_guard,rvalue);}
...
.lifetimes.re_erased;3;for binding in bindings{let source_info=self.source_info(
binding.span);({});let local=if storages_alive{self.var_local_id(binding.var_id,
OutsideGuard).into()}else{self.storage_live_binding(block,binding.var_id,//({});
binding.span,OutsideGuard,schedule_drops,)};loop{break;};if schedule_drops{self.
...
re_erased,util::ref_pat_borrow_kind(mutbl),binding.source)}};if true{};self.cfg.
...
Option<Place<'tcx>>,Span)>,pat_span:Span,){((),());let _=();let tcx=self.tcx;let
debug_source_info=SourceInfo{span:source_info.span,scope:visibility_scope};3;let
...
composite:None,argument_index:None,});loop{break;};let locals=if has_guard.0{let
...
BindingForm::RefForGuard,))),});({});self.var_debug_info.push(VarDebugInfo{name,
...
ref_for_guard,for_arm_body}}else{LocalsForNode::One(for_arm_body)};({});debug!(?
locals);3;self.var_indices.insert(var_id,locals);}pub(crate)fn ast_let_else(&mut
...
>{{;};let else_block_span=self.thir[else_block].span;let(matching,failure)=self.
in_if_then_scope(*let_else_scope,else_block_span,|this|{3;let scrutinee=unpack!(
block=this.lower_scrutinee(block,init_id,initializer_span));({});let pat=Pat{ty:
pattern.ty,span:else_block_span,kind:PatKind::Wild};3;let mut wildcard=Candidate
...
candidate,&mut wildcard],);({});let matching=this.bind_pattern(this.source_info(
pattern .span),candidate,fake_borrow_temps.as_slice(),initializer_span,None,true
,);{;};let failure=this.bind_pattern(this.source_info(else_block_span),wildcard,
fake_borrow_temps.as_slice(),initializer_span,None,true,);3;this.break_for_else(
failure,this.source_info(initializer_span));({});matching.unit()});matching.and(
