...
>,expr_id:ExprId,mutability:Mutability,)->BlockAnd<Local>{({});let this=self;let
expr=&this.thir[expr_id];if true{};let expr_span=expr.span;let source_info=this.
...
temp_lifetime;let _=();let temp=if deduplicate_temps&&let Some(temp_index)=this.
...
local_decls.push(local_decl)};({});if deduplicate_temps{this.fixed_temps.insert(
...
::StorageLive(temp)});loop{break};if let Some(temp_lifetime)=temp_lifetime{this.
