...
let this=self;((),());let _=();let mut let_scope_stack=Vec::with_capacity(8);let
outer_source_scope=this.source_scope;loop{break};let outer_in_scope_unsafe=this.
in_scope_unsafe;((),());let _=();let mut last_remainder_scope=region_scope;this.
update_source_scope_for_safety_mode(span,safety_mode);({});let source_info=this.
...
{ignores_expr_result:true});({});let si=(*scope,source_info);unpack!(block=this.
...
ignores_expr_result=matches!(pattern.kind,PatKind::Wild);{;};this.block_context.
...
;{;};unpack!(failure_block=this.ast_block(dummy_place,failure_entry,*else_block,
this.source_info(else_block_span),));({});this.cfg.terminate(failure_block,this.
source_info(else_block_span),TerminatorKind::Unreachable,);3;let remainder_span=
remainder_scope.span(this.tcx,this.region_scope_tree);((),());this.push_scope((*
remainder_scope,source_info));let _=();let_scope_stack.push(remainder_scope);let
visibility_scope =Some(this.new_source_scope(remainder_span,LintLevel::Inherited
,None));if true{};let initializer_span=this.thir[*initializer].span;let scope=(*
init_scope,source_info);let _=();let failure=unpack!(block=this.in_scope(scope,*
...
visibility_scope{let _=();this.source_scope=source_scope;}last_remainder_scope=*
...
PatKind::Wild);let _=();if true{};this.block_context.push(BlockFrame::Statement{
ignores_expr_result});if true{};this.push_scope((*remainder_scope,source_info));
let_scope_stack.push(remainder_scope);3;let remainder_span=remainder_scope.span(
...
span,_,_|{{;};this.storage_live_binding(block,node,span,OutsideGuard,true);this.
...
remainder_scope;let _=||();}}let popped=this.block_context.pop();assert!(popped.
is_some_and(|bf|bf.is_statement()));*&*&();}let tcx=this.tcx;let destination_ty=
destination.ty(&this.local_decls,tcx).ty;();if let Some(expr_id)=expr{let expr=&
this.thir[expr_id];();let tail_result_is_ignored=destination_ty.is_unit()||this.
block_context.currently_ignores_tail_results();let _=();this.block_context.push(
...
;{;};assert!(popped.is_some_and(|bf|bf.is_tail_expr()));}else{if destination_ty.
...
"update_source_scope_for({:?}, {:?})",span,safety_mode);3;let new_unsafety=match
...
::ExplicitUnsafe(hir_id);{;};Safety::ExplicitUnsafe(hir_id)}};self.source_scope=
