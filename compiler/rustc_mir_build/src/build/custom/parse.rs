...
.unwrap())},);3;self.parse_debuginfo(debuginfo.iter().copied())?;let block_defs=
...
is_cleanup=self.body.basic_blocks_mut()[BasicBlock::from_usize(i)].is_cleanup;3;
let  block=self.parse_block_def(self.statement_as_expr(*block_def)?,is_cleanup)?
;*&*&();self.body.basic_blocks_mut()[BasicBlock::from_usize(i)]=block;}Ok(())}fn
parse_block_decls(&mut self,stmts:impl Iterator<Item=StmtId>)->PResult<()>{for//
stmt in stmts{if true{};let _=||();self.parse_basic_block_decl(stmt)?;}Ok(())}fn
...
mir_basic_block,Normal)=>false,@variant(mir_basic_block,Cleanup)=>true,);{;};let
...
.insert(ret_var,Local::from_u32(0));{;};for stmt in stmts{let(var,ty,span)=self.
parse_let_statement(stmt)?;;let decl=LocalDecl::new(ty,span);let local=self.body
.local_decls.push(decl);loop{break;};self.local_map.insert(var,local);}Ok(())}fn
...
expected:"debuginfo".to_string(),});3;}StmtKind::Expr{expr,..}=>expr,};let span=
self.thir[expr].span;;let(name,operand)=parse_by_kind!(self,expr,_,"debuginfo",@
call(mir_debuginfo,args)=>{(args[0],args[1])},);();let name=parse_by_kind!(self,
...
expr,"let statement"));{;};}};self.parse_var(pattern)}fn parse_var(&mut self,mut
...
ExprKind::Block{block}=>&self.thir[*block],);3;let mut data=BasicBlockData::new(
None);({});data.is_cleanup=is_cleanup;for stmt_id in&*block.stmts{let stmt=self.
statement_as_expr(*stmt_id)?;3;let span=self.thir[stmt].span;let statement=self.
parse_statement(stmt)?;();data.statements.push(Statement{source_info:SourceInfo{
span,scope:self.source_scope},kind:statement,});3;}let Some(trailing)=block.expr
else{return Err(self.expr_error(expr_id,"terminator"))};({});let span=self.thir[
