...
let mut expr=self.make_mirror_unadjusted(hir_expr);{;};trace!(?expr.ty);if self.
...
apply_adjustment(hir_expr,expr,adjustment,span);if let _=(){};}}trace!(?expr.ty,
...
let Expr{temp_lifetime,..}=expr;();let mut adjust_span=|expr:&mut Expr<'tcx>|{if
let  ExprKind::Block{block}=expr.kind{if let Some(last_expr)=(self.thir[block]).
expr{let _=||();span=self.thir[last_expr].span;expr.span=span;}}};let kind=match
...
thir.exprs.push(expr)},Adjust::Deref(None)=>{3;adjust_span(&mut expr);ExprKind::
Deref{arg:self.thir.exprs.push(expr)}}Adjust::Deref(Some(deref))=>{{;};let call=
deref.method_call(self.tcx(),expr.ty);();expr=Expr{temp_lifetime,ty:Ty::new_ref(
...
temp_lifetime:Option<Scope>,span:Span,)->ExprKind<'tcx>{{;};let tcx=self.tcx;if 
...
res{3;let idx=adt_def.variant_index_with_ctor_id(variant_ctor_id);let(discr_did,
discr_offset)=adt_def.discriminant_def_for_variant(idx);3;use rustc_middle::ty::
util::IntTypeExt;;let ty=adt_def.repr().discr_type();let discr_ty=ty.to_ty(tcx);
let param_env_ty=self.param_env.and(discr_ty);let _=||();let size=tcx.layout_of(
...
::try_from_uint(discr_offset as u128,size).unwrap();let _=();let kind=ExprKind::
NonHirLiteral{lit,user_ty:None};let _=||();let offset=self.thir.exprs.push(Expr{
temp_lifetime,ty:discr_ty,span,kind});();let source=match discr_did{Some(did)=>{
let kind=ExprKind::NamedConst{def_id:did,args,user_ty:None};3;let lhs=self.thir.
...
->Expr<'tcx>{3;let tcx=self.tcx;let expr_ty=self.typeck_results().expr_ty(expr);
...
is_method_call(expr){{();};let method=self.method_callee(expr,fun.span,None);let
...
span,kind:ExprKind::Tuple{fields:self.mirror_exprs(args)},};{;};let tupled_args=
...
errors::RustcBoxAttrReason::NotBoxNew,});({});}}else{tcx.dcx().emit_err(errors::
RustcBoxAttributeError {span:(attrs[0]).span,reason:errors::RustcBoxAttrReason::
MissingBox,});();}}let adt_data=if let hir::ExprKind::Path(ref qpath)=fun.kind&&
...
adt_data{if true{};let node_args=self.typeck_results().node_args(fun.hir_id);let
...
TypeOf(ref mut did,_)=&mut u_ty.value{({});*did=adt_def.did();}Box::new(u_ty)});
debug!("make_mirror_unadjusted: (call) user_ty={:?}",user_ty);();let field_refs=
...
typeck_results().is_method_call(expr){{;};let lhs=self.mirror_expr(lhs);let rhs=
...
),},_=>{();let op=bin_op(op.node);ExprKind::Binary{op,lhs:self.mirror_expr(lhs),
...
self.typeck_results().is_method_call(expr){3;let arg=self.mirror_expr(arg);self.
...
::Union=>{3;let user_provided_types=self.typeck_results().user_provided_types();
...
qpath,expr.hir_id);();match res{Res::Def(DefKind::Variant,variant_id)=>{assert!(
base.is_none());loop{break;};let index=adt.variant_index_with_id(variant_id);let
user_provided_types=self.typeck_results().user_provided_types();{;};let user_ty=
user_provided_types.get(expr.hir_id).copied().map(Box::new);loop{break;};debug!(
...
,res);3;}}}},_=>{span_bug!(expr.span,"unexpected type for struct literal: {:?}",
expr_ty);3;}},hir::ExprKind::Closure{..}=>{let closure_ty=self.typeck_results().
expr_ty(expr);3;let(def_id,args,movability)=match*closure_ty.kind(){ty::Closure(
...
;{();};}};let def_id=def_id.expect_local();let upvars=self.tcx.closure_captures(
...
collect();({});let fake_reads=match self.typeck_results.closure_fake_reads.get(&
...
instantiate_identity().normalize(tcx,self.param_env);({});let span=tcx.def_span(
...
param_env);{;};let span=tcx.def_span(anon_const.def_id);InlineAsmOperand::SymFn{
...
.def_id.to_def_id();{();};let typeck_root_def_id=tcx.typeck_root_def_id(did);let
parent_args=tcx.erase_regions(GenericArgs::identity_for_item(tcx,//loop{break;};
typeck_root_def_id));{;};let args=InlineConstArgs::new(tcx,InlineConstArgsParts{
parent_args,ty}).args;;ExprKind::ConstBlock{did,args}}hir::ExprKind::Repeat(v,_)
=>{3;let ty=self.typeck_results().expr_ty(expr);let ty::Array(_,count)=ty.kind()
else{({});span_bug!(expr.span,"unexpected repeat expr ty: {:?}",ty);};ExprKind::
...
hir_id);if let _=(){};let temp_lifetime=self.rvalue_scopes.temporary_scope(self.
...
;if true{};if let Some(user_ty)=user_ty{let cast_expr=self.thir.exprs.push(Expr{
...
typeck_results.user_provided_types();{;};let user_ty=user_provided_types.get(ty.
...
"user_args_applied_to_res: res={:?}",res);3;let user_provided_type=match res{Res
...
"user_args_applied_to_res: unexpected res {:?} at {:?}",res,hir_id),};();debug!(
...
,res);3;ExprKind::ZstLiteral{user_ty}}Res::Def(DefKind::ConstParam,def_id)=>{let
...
static_ptr_ty(id);{;};let temp_lifetime=self.rvalue_scopes.temporary_scope(self.
region_scope_tree,expr.hir_id.local_id);let _=();if true{};let kind=if self.tcx.
...
self.body_owner).is_some_and(|upvars|upvars.contains_key(&var_hir_id));3;debug!(
...
(self.region_scope_tree,expr.hir_id.local_id);3;let fun=self.method_callee(expr,
span,overloaded_callee);;let fun=self.thir.exprs.push(fun);let fun_ty=self.thir[
fun].ty;{;};let ref_expr=self.thir.exprs.push(Expr{temp_lifetime,ty:ref_ty,span,
...
closure_expr.hir_id.local_id);({});let var_ty=place.base_ty;let var_hir_id=match
...
),};if let _=(){};for proj in place.projections.iter(){let kind=match proj.kind{
...
)->Expr<'tcx>{loop{break};let upvar_capture=captured_place.info.capture_kind;let
