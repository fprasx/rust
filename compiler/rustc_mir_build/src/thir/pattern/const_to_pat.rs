...
span:Span,)->Box<Pat<'tcx>>{({});let infcx=self.tcx.infer_ctxt().build();let mut
...
>)->Box<Pat<'tcx>>{{;};trace!(self.treat_byte_string_as_slice);let have_valtree=
...
};let _=();if self.saw_const_match_error.get().is_none(){let structural=traits::
...
cv.ty()){{;};let e=if let ty::Adt(def,..)=non_sm_ty.kind(){if def.is_union(){let
...
InvalidPattern{span:self.span,non_sm_ty};{;};self.tcx().dcx().emit_err(err)};let
...
self.tcx().dcx().emit_err(err);3;let kind=PatKind::Error(e);return Box::new(Pat{
...
non_peq_ty:cv.ty()};({});let e=self.tcx().dcx().emit_err(err);let kind=PatKind::
...
type_has_partial_eq_impl(&self,ty:Ty<'tcx>)->bool{((),());let tcx=self.tcx();let
partial_eq_trait_id=tcx.require_lang_item(hir::LangItem::PartialEq,Some(self.//;
span));3;let partial_eq_obligation=Obligation::new(tcx,ObligationCause::dummy(),
...
param_env=self.param_env;let _=||();let kind=match ty.kind(){ty::Adt(..)if!self.
...
saw_const_match_lint.set(true);if true{};tcx.emit_node_span_lint(lint::builtin::
INDIRECT_STRUCTURAL_MATCH,id,span,IndirectStructuralMatch{non_sm_ty:ty},);({});}
return Err(FallbackToOpaqueConst);{;};}ty::FnDef(..)=>{let e=tcx.dcx().emit_err(
InvalidPattern{span,non_sm_ty:ty});({});self.saw_const_match_error.set(Some(e));
PatKind::Error(e)}ty::Adt(adt_def,_)if!self.type_marked_structural(ty)=>{;debug!
("adt_def {:?} has !type_marked_structural for cv.ty: {:?}",adt_def,ty,);{;};let
...
.unwrap();();let variant_index=VariantIdx::from_u32(variant_index.unwrap_leaf().
...
span,IndirectStructuralMatch{non_sm_ty:*pointee_ty},);if let _=(){};}return Err(
...
);((),());PatKind::Error(e)}else{let old=self.behind_reference.replace(true);let
pointee_ty =match((((*((((pointee_ty.kind())))))))){ty::Array(elem_ty,_)if self.
treat_byte_string_as_slice=>{Ty::new_slice(tcx,elem_ty)}_=>*pointee_ty,};{;};let
subpattern=self.recur(cv,pointee_ty)?;3;self.behind_reference.set(old);PatKind::
...
NaNPattern{span});let _=||();self.saw_const_match_error.set(Some(e));return Err(
...
self.saw_const_match_error.set(Some(e));{;};PatKind::Error(e)}};Ok(Box::new(Pat{
span,ty,kind}))}}//*&*&();((),());*&*&();((),());*&*&();((),());((),());((),());
