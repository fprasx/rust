...
),ErrorGuaranteed>{{;};let typeck_results=tcx.typeck(def_id);let(thir,expr)=tcx.
...
return;3;}ExprKind::Match{scrutinee,scrutinee_hir_id,box ref arms}=>{let source=
...
AwaitDesugar,_=>hir::MatchSource::Normal,};({});self.check_match(scrutinee,arms,
...
chain_refutabilities,ex.span);3;}return;}_=>{}};self.with_let_source(LetSource::
...
else_block.is_some(){LetSource::LetElse}else{LetSource::PlainLet};let _=();this.
with_let_source(let_source,|this|{this.check_let(pattern,initializer,span)});();
...
let old_let_source=self.let_source;let _=();let _=();self.let_source=let_source;
ensure_sufficient_stack(||f(self));let _=||();self.let_source=old_let_source;}fn
...
self.lint_level;let _=();self.lint_level=hir_id;let ret=f(self);self.lint_level=
...
LetSource::None,|this|{3;this.visit_expr(ex);});Ok(None)}}}fn lower_pattern(&mut
...
.and(check_never_pattern(cx,pat));*&*&();});err?;Ok(self.pattern_arena.alloc(cx.
lower_pat(pat)))}}fn is_known_valid_scrutinee(&self,scrutinee:&Expr<'tcx>)->//3;
bool{3;use ExprKind::*;match&scrutinee.kind{Deref{..}=>false,Field{lhs,..}=>{let
...
scrutinee:Option<ExprId>,span:Span){3;assert!(self.let_source!=LetSource::None);
...
);3;if got_error{return;}}let Ok(report)=self.analyze_patterns(&cx,&tarms,scrut.
...
debug_assert_eq!(pat.span.desugaring_kind(),Some(DesugaringKind::ForLoop));3;let
PatKind::Variant{ref subpatterns,..}=pat.kind else{bug!()};({});let[pat_field]=&
subpatterns[..]else{bug!()};*&*&();self.check_binding_is_irrefutable(&pat_field.
pattern,"`for` loop binding",None,None,);let _=();let _=();}else{self.error=Err(
...
whole_chain_span,);{();};return;}if let Some(until)=chain_refutabilities.iter().
...
().0;if true{};let span=span_start.to(span_end);let count=suffix.len();self.tcx.
...
(refutability,None,scrut,pat.span);({});let pat=self.lower_pattern(&cx,pat)?;let
arms=[MatchArm{pat,arm_data:self.lint_level,has_guard:false}];3;let report=self.
...
Refutable,scrut)?;loop{break};report_arm_reachability(&cx,&report);Ok(if report.
...
pattern_ty=pat.ty;({});let Ok((cx,report))=self.analyze_binding(pat,Irrefutable,
scrut)else{return};((),());let witnesses=report.non_exhaustiveness_witnesses;if 
witnesses.is_empty(){3;return;}let inform=sp.is_some().then_some(Inform);let mut
let_suggestion=None;3;let mut misc_suggestion=None;let mut interpreted_as_const=
...
];let _=();pat.each_binding(|name,_,_,_|bindings.push(name));let semi_span=span.
shrink_to_hi();*&*&();let start_span=span.shrink_to_lo();let end_span=semi_span.
shrink_to_lo();*&*&();let count=witnesses.len();let_suggestion=Some(if bindings.
is_empty(){((SuggestLet::If{start_span,semi_span,count}))}else{SuggestLet::Else{
end_span,count}});{();};};let adt_defined_here=report_adt_defined_here(self.tcx,
...
variant=adt.variant(*variant_index);3;let inhabited=variant.inhabited_predicate(
self.tcx,*adt).instantiate(self.tcx,args);3;assert!(inhabited.apply(self.tcx,cx.
param_env,cx.module));{;};!inhabited.apply_ignore_module(self.tcx,cx.param_env)}
else{false};3;self.error=Err(self.tcx.dcx().emit_err(PatternNotCovered{span:pat.
...
kind else{if true{};let _=||();return;};let is_binding_by_move=|ty:Ty<'tcx>|!ty.
...
new();let _=();sub.each_binding(|_,mode,_,span|{if matches!(mode,ByRef::Yes(_)){
conflicts_ref.push(span)}});{;};if!conflicts_ref.is_empty(){sess.dcx().emit_err(
BorrowOfMovedValue{binding_span:pat.span,conflicts_ref,name,ty,//*&*&();((),());
suggest_borrowing:Some(pat.span.shrink_to_lo()),});3;}return;}ByRef::No=>return,
...
);({});occurrences.extend(conflicts_move);if report_mut_mut{sess.dcx().emit_err(
MultipleMutBorrows{span:pat.span,occurrences});{;};}else if report_mut_ref{match
...
pat.span,occurrences});{;};}};}else if report_move_conflict{sess.dcx().emit_err(
...
.variants().len();3;let ty_path=with_no_trimmed_paths!(cx.tcx.def_path_str(edef.
...
&&pat_is_catchall(arm.pat){loop{break;};catchall=Some(arm.pat.data().span);}}}fn
...
expr_span:Span,)->ErrorGuaranteed{((),());let is_empty_match=arms.is_empty();let
non_empty_enum =match ((scrut_ty.kind())){ty::Adt(def,_)=>(def.is_enum())&&!def.
variants().is_empty(),_=>false,};3;if is_empty_match&&!non_empty_enum{return cx.
...
"non-exhaustive patterns: {joined_patterns} not covered"),);3;err.span_label(sp,
...
err.span_note(multi_span,format!("`{ty}` defined here"));({});}err.note(format!(
...
"`{ty}` does not have a fixed maximum value, so half-open ranges are necessary to match \
                             exhaustively"
,));loop{break;};if let _=(){};}else if ty==cx.tcx.types.isize{err.note(format!(
"`{ty}` does not have fixed minimum and maximum values, so half-open ranges are necessary to match \
                             exhaustively"
,));((),());let _=();let _=();let _=();}}else if ty==cx.tcx.types.str_{err.note(
...
witnesses.len()<4;*&*&();let suggested_arm=if suggest_the_witnesses{let pattern=
...
let sm=cx.tcx.sess.source_map();{;};match arms{[]if sp.eq_ctxt(expr_span)=>{let(
indentation,more)=if let Some(snippet)=(((sm.indentation_before(sp)))){(format!(
"\n{snippet}"),"    ")}else{(" ".to_string(),"")};if true{};suggestion=Some((sp.
...
"\n{snippet}"),true)}else{(" ".to_string(),false)};{;};let only_body=&thir[only.
body];({});let comma=if matches!(only_body.kind,ExprKind::Block{..})&&only.span.
eq_ctxt(only_body.span)&&is_multiline{""}else{","};3;suggestion=Some((only.span.
shrink_to_hi(),format!("{comma}{pre_indentation}{suggested_arm}"),));;}[..,prev,
last]=>{3;let prev=&thir[*prev];let last=&thir[*last];if prev.span.eq_ctxt(last.
span){{();};let last_body=&thir[last.body];let comma=if matches!(last_body.kind,
...
NonExhaustiveMatchAllArmsGuarded);({});}if let Some((span,sugg))=suggestion{err.
...
'tcx>,witnesses:&[WitnessPat<'p,'tcx>],)->String{((),());const LIMIT:usize=3;let
...
tcx,*def,witnesses.iter().take(5)){if true{};variants.push(Variant{span});}Some(
...
WitnessPat<'p,'tcx>>,)->Vec<Span>{let _=();let mut covered=vec![];for pattern in
...
}*&*&();covered.push(sp);}covered.extend(maybe_point_at_variant(tcx,def,pattern.
