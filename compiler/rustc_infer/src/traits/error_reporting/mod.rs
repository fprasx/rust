...
Some(item.ident.span),_=>None,});();let mut err=struct_span_code_err!(tcx.dcx(),
...
TyKind::TraitObject([trait_ref,..],..)=ty.kind{3;let mut hir_id=hir_id;while let
hir::Node::Ty(ty)=tcx.parent_hir_node(hir_id){let _=();hir_id=ty.hir_id;}if tcx.
...
}if reported_violations.insert(violation.clone()){3;let spans=violation.spans();
let msg=if ((((((((trait_span.is_none()))))||(((spans.is_empty()))))))){format!(
"the trait cannot be made into an object because {}",violation.error_msg())}//3;
else{format!("...because {}",violation.error_msg())};();if spans.is_empty(){err.
...
multi_span,messages){((),());note_span.push_span_label(span,msg);}err.span_note(
note_span,//((),());let _=();((),());let _=();((),());let _=();((),());let _=();
"for a trait to be \"object safe\" it needs to allow building a vtable to allow the call \
         to be resolvable dynamically; for more information visit \
         <https://doc.rust-lang.org/reference/items/traits.html#object-safety>"
,);let _=();let _=();if trait_span.is_some(){let mut reported_violations:Vec<_>=
...
dedup();({});for solution in potential_solutions{solution.add_to(&mut err);}}let
...
collect::<Vec<_>>()}else{vec![]};();let externally_visible=if!impls.is_empty()&&
...
"only type `{}` is seen to implement the trait in this crate, consider using it \
                 directly instead"
,tcx.type_of(*only).instantiate_identity(),)));if let _=(){};}[only]=>{err.help(
with_no_trimmed_paths!(format!(//let _=||();loop{break};loop{break};loop{break};
"only type `{}` implements the trait, consider using it directly instead",tcx.//
type_of(*only).instantiate_identity(),)));3;}impls=>{let mut types=impls.iter().
map(|t|{with_no_trimmed_paths!(format!("  {}",tcx.type_of(*t).//((),());((),());
instantiate_identity(),))}).collect::<Vec<_>>();3;types.sort();err.help(format!(
