...
self.insert(pred);({});}}fn extend_one(&mut self,pred:ty::Predicate<'tcx>){self.
...
Filter::All};({});elaborator.extend_deduped(obligations);elaborator}impl<'tcx,O:
...
self.visited.insert(o.predicate())));3;}pub fn filter_only_self(mut self)->Self{
self.mode=Filter::OnlySelf;3;self}pub fn filter_only_self_that_defines(mut self,
assoc_ty:Ident)->Self{();self.mode=Filter::OnlySelfThatDefines(assoc_ty);self}fn
...
polarity!=ty::PredicatePolarity::Positive{{;};return;}let predicates=match self.
...
.super_predicates_that_define_assoc_item((data.def_id(),ident))}};let _=||();let
...
bound_clause.rebind(data.trait_ref)),span,bound_clause.rebind(data),index,)});3;
