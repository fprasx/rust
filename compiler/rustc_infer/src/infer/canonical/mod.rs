...
create_next_universe())).collect();let _=||();let canonical_inference_vars=self.
...
PlaceholderType{universe,bound})=>{3;let universe_mapped=universe_map(universe);
...
;3;let placeholder_mapped=ty::PlaceholderRegion{universe:universe_mapped,bound};
