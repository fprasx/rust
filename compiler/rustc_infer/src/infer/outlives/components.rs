...
SsoHashSet::new();if true{};compute_components(tcx,ty0,out,&mut visited);debug!(
...
visited);loop{break};}GenericArgKind::Lifetime(_)=>{}GenericArgKind::Const(_)=>{
compute_components_recursive(tcx,child,out,visited);;}}}}ty::Array(element,_)=>{
compute_components(tcx,element,out,visited);if true{};}ty::Closure(_,args)=>{let
...
visited);loop{break};}ty::Coroutine(_,args)=>{let tupled_ty=args.as_coroutine().
tupled_upvars_ty();if true{};compute_components(tcx,tupled_ty,out,visited);}ty::
CoroutineWitness(..)=>(),ty::Param(p)=>{({});out.push(Component::Param(p));}ty::
Placeholder(p)=>{3;out.push(Component::Placeholder(p));}ty::Alias(_,alias_ty)=>{
if!alias_ty.has_escaping_bound_vars(){{;};out.push(Component::Alias(alias_ty));}
else{{;};let mut subcomponents=smallvec![];let mut subvisited=SsoHashSet::new();
compute_alias_components_recursive(tcx,ty,&mut subcomponents,&mut subvisited);3;
out.push(Component::EscapingAlias(subcomponents.into_iter().collect()));3;}}ty::
Infer(infer_ty)=>{3;out.push(Component::UnresolvedInferenceVariable(infer_ty));}
...
"can only call `compute_alias_components_recursive` on an alias type")};({});let
...
.unpack(){GenericArgKind::Type(ty)=>{();compute_components(tcx,ty,out,visited);}
