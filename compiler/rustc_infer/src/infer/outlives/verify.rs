...
declared_generic_bounds_from_env(ty);3;debug!(?declared_bounds_from_env);let mut
param_bounds=vec![];if true{};for declared_bound in declared_bounds_from_env{let
...
VerifyBound::AllBounds(vec![]);({});}}if let Some(r)=self.implicit_region_bound{
...
'tcx>>>>{3;let erased_alias_ty=self.tcx.erase_regions(alias_ty.to_ty(self.tcx));
...
>{if let _=(){};let alias_ty_as_ty=alias_ty.to_ty(self.tcx);let env_bounds=self.
...
verify_if_eq_b)}});3;let definition_bounds=self.declared_bounds_from_definition(
alias_ty).map(|r|VerifyBound::OutlivedBy(r));*&*&();let recursive_bound={let mut
components=smallvec![];loop{break;};compute_alias_components_recursive(self.tcx,
...
.tcx;let _=||();let param_bounds=self.caller_bounds.iter().copied().filter(move|
...
.to_ty(tcx);let _=();let erased_p_ty=self.tcx.erase_regions(p_ty);(erased_p_ty==
...
item_super_predicates(alias_ty.def_id);{;};trace!("{:#?}",bounds.skip_binder());
