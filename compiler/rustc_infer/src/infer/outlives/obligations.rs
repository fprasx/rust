...
undo_log.push(UndoLog::PushRegionObligation);({});inner.region_obligations.push(
...
sub_region,?cause);3;let origin=SubregionOrigin::from_obligation_cause(cause,||{
...
);();let normalized_caller_bounds:Vec<_>=outlives_env.param_env.caller_bounds().
...
0..{({});let my_region_obligations=self.take_registered_region_obligations();if 
my_region_obligations.is_empty(){if true{};break;}if!self.tcx.recursion_limit().
...
outlives=ty::Binder::dummy(ty::OutlivesPredicate(sup_type,sub_region));();let ty
...
("started with no bound vars, should end with no bound vars");({});let(sup_type,
...
normalized_caller_bounds,);((),());let category=origin.to_constraint_category();
...
category:ConstraintCategory<'tcx>,){3;assert!(!ty.has_escaping_bound_vars());let
mut components=smallvec![];;push_outlives_components(self.tcx,ty,&mut components
);if true{};self.components_must_outlive(origin,&components,region,category);}fn
...
.clone();loop{break};match component{Component::Region(region1)=>{self.delegate.
push_sub_region_constraint(origin,region,*region1,category);3;}Component::Param(
param_ty)=>{{;};self.param_ty_must_outlive(origin,region,*param_ty);}Component::
...
tcx));({});self.delegate.push_verify(origin,GenericKind::Param(param_ty),region,
...
placeholder_ty));({});self.delegate.push_verify(origin,GenericKind::Placeholder(
...
declared_bounds_from_definition(*alias_ty).all(|r|r!=bound.1)});3;let is_opaque=
...
alias_ty.def_id));let _=||();self.args_must_outlive(alias_ty.args,origin,region,
...
trait_bounds[0];((),());let _=();let _=();let _=();debug!(?unique_bound);debug!(
"unique declared bound appears in trait ref");if let _=(){};let category=origin.
...
alias_ty,&mut Default::default());{;};debug!("alias_must_outlive: pushing {:?}",
verify_bound);{;};self.delegate.push_verify(origin,GenericKind::Alias(alias_ty),
