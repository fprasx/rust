...
let mut inner=self.inner.borrow_mut();((),());let inner=&mut*inner;assert!(self.
...
mut inner.undo_log).into_infos_and_data()};();let region_rels=&RegionRelations::
new(self.tcx,outlives_env.free_region_map());{;};let(lexical_region_resolutions,
errors)=lexical_region_resolve::resolve(region_rels,var_infos,data);let _=();let
old_value=self.lexical_region_resolutions.replace(Some(//let _=||();loop{break};
lexical_region_resolutions));let _=();assert!(old_value.is_none());errors}pub fn
...
->R,)->R{loop{break};loop{break};let mut inner=self.inner.borrow_mut();op(inner.
