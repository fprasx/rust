...
self.logs.push(undo.into())}}fn clear(&mut self){((),());self.logs.clear();self.
...
;if let _=(){};}self.type_variable_storage.finalize_rollback();if self.undo_log.
num_open_snapshots==1{;assert!(snapshot.undo_len==0);assert!(self.undo_log.logs.
is_empty());{();};}self.undo_log.num_open_snapshots-=1;}pub fn commit(&mut self,
...
.clear();{;};}self.undo_log.num_open_snapshots-=1;}}impl<'tcx>InferCtxtUndoLogs<
