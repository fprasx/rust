...
"start_snapshot()");({});let mut inner=self.inner.borrow_mut();CombinedSnapshot{
...
region_constraints_snapshot,universe}=snapshot;3;self.universe.set(universe);let
mut inner=self.inner.borrow_mut();*&*&();inner.rollback_to(undo_snapshot);inner.
...
CombinedSnapshot<'tcx>){if true{};let _=||();let CombinedSnapshot{undo_snapshot,
region_constraints_snapshot:_,universe:_}=snapshot;({});self.inner.borrow_mut().
...
commit_from(snapshot);{;};}Err(_)=>{self.rollback_to(snapshot);}}r}#[instrument(
skip(self,f),level="debug")]pub fn probe<R,F>(&self,f:F)->R where F:FnOnce(&//3;
CombinedSnapshot<'tcx>)->R,{((),());let snapshot=self.start_snapshot();let r=f(&
snapshot);((),());let _=();let _=();let _=();self.rollback_to(snapshot);r}pub fn
