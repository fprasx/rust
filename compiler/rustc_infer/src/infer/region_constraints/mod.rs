...
undo_entry{AddVar(vid)=>{let _=();self.var_infos.pop().unwrap();assert_eq!(self.
...
self.data.verifys.pop();loop{break;};assert_eq!(self.data.verifys.len(),index);}
AddCombination(Glb,ref regions)=>{;self.glbs.remove(regions);}AddCombination(Lub
,ref regions)=>{loop{break};loop{break};self.lubs.remove(regions);}}}}impl<'tcx>
...
assert!(!UndoLogs::<UndoLog<'_>>::in_snapshot(&self.undo_log));if let _=(){};let
...
data);((),());if*any_unifications{*any_unifications=false;ut::UnificationTable::
...
"RegionConstraintCollector: add_constraint({:?})",constraint);();let index=self.
storage.data.constraints.len();3;self.storage.data.constraints.push((constraint,
origin));({});self.undo_log.push(AddConstraint(index));}fn add_verify(&mut self,
...
return;3;}let index=self.data.verifys.len();self.data.verifys.push(verify);self.
...
).is_ok(){loop{break};self.any_unifications=true;};}(_,ty::ReVar(vid))=>{debug!(
...
any_unifications=true;3;};}(_,_)=>{}}}}pub(super)fn member_constraint(&mut self,
...
choice_regions.iter().any(|&r|r==member_region){if let _=(){};return;}self.data.
...
"origin = {:#?}",origin);{;};match(*sub,*sup){(ReBound(..),_)|(_,ReBound(..))=>{
...
Constraint::RegSubVar(sub,sup_id),origin);loop{break};}(ReVar(sub_id),_)=>{self.
add_constraint(Constraint::VarSubReg(sub_id,sup),origin);loop{break;};}_=>{self.
...
RegionVid,)->ty::Region<'tcx>{*&*&();let mut ut=self.unification_table_mut();let
...
,b};3;if let Some(&c)=self.combine_map(t).get(&vars){return ty::Region::new_var(
