...
>where T:Relate<'tcx>,{let _=();let Trace{at,trace}=self;debug_assert!(at.infcx.
...
where T:Relate<'tcx>,{let _=();let Trace{at,trace}=self;let mut fields=at.infcx.
...
ImplSubject::Trait(_))=>{();bug!("can not trace TraitRef and Ty");}}}}impl<'tcx>
