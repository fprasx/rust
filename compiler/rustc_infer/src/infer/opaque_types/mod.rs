...
value,obligations:vec![]};{;};}if!value.has_opaque_types(){return InferOk{value,
...
is_some())};3;let value=value.fold_with(&mut BottomUpFolder{tcx:self.tcx,lt_op:|
...
TypeVariableOrigin{kind:TypeVariableOriginKind::MiscVariable,span,});let _=||();
obligations .extend(self.handle_opaque_type(ty,ty_var,&cause,param_env).unwrap()
.obligations,);if let _=(){};ty_var}_=>ty,},});InferOk{value,obligations}}pub fn
...
self.opaque_type_origin(def_id).is_none(){3;return None;}}DefiningAnchor::Bubble
...
resolve_vars_if_possible(concrete_ty);3;debug!(?concrete_ty);let variances=self.
...
upvar_tys(){*&*&();upvar.visit_with(self);}args.as_closure().sig_as_fn_ptr_ty().
...
.visit_with(self);3;args.as_coroutine().resume_ty().visit_with(self);}ty::Alias(
...
Bivariant{{();};s.visit_with(self);}}}_=>{ty.super_visit_with(self);}}}}pub enum
...
,hidden_ty,&mut obligations)?;loop{break;};self.add_item_bounds_for_hidden_type(
...
,);3;}};Ok(())}pub fn add_item_bounds_for_hidden_type(&self,def_id:DefId,args:ty
...
self.tcx;3;obligations.push(traits::Obligation::new(tcx,cause.clone(),param_env,
ty::ClauseKind::WellFormed(hidden_ty.into()),));loop{break};let item_bounds=tcx.
...
;({});obligations.push(traits::Obligation::new(self.tcx,cause.clone(),param_env,
predicate,));((),());((),());((),());((),());((),());((),());((),());((),());}}}
