...
return inner;*&*&();}let next_universe=self.create_next_universe();let delegate=
...
bound:bound_var},ty,)},};let _=||();loop{break};debug!(?next_universe);self.tcx.
...
enter_forall_and_leak_universe(forall);let _=||();debug!(?value);f(value)}pub fn
...
unstable_opts.no_leak_check||self.skip_leak_check{{;};return Ok(());}self.inner.
