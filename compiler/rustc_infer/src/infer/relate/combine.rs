...
'tcx>,{({});debug!("{}.consts({:?}, {:?})",relation.tag(),a,b);debug_assert!(!a.
...
probe(|_|{if a.ty()==b.ty(){{();};return;}let canonical=self.canonicalize_query(
...
"cannot relate consts of different types (a={a:?}, b={b:?})",));3;});});match(a.
...
relation,true,vid,b)?;();Ok(b)}(_,ty::ConstKind::Infer(InferConst::Var(vid)))=>{
