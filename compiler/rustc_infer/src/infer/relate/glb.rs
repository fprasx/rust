...
::Region<'tcx>>{({});debug!("{}.regions({:?}, {:?})",self.tag(),a,b);let origin=
...
;if true{};let _=||();}debug!("binders(a={:?}, b={:?})",a,b);if a.skip_binder().
...
self.fields.sub();let _=();if true{};sub.relate(v,a)?;sub.relate(v,b)?;Ok(())}fn
...
:PredicateObligations<'tcx>){3;self.fields.register_obligations(obligations);}fn
