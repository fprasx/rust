...
this.infcx();let _=();if true{};let a=infcx.inner.borrow_mut().type_variables().
...
LatticeVariable,span:this.cause().span,});3;this.relate_bound(v,b,a)?;Ok(v)}(_,&
ty::Infer(TyVar(..)))=>{((),());let v=infcx.next_ty_var(TypeVariableOrigin{kind:
TypeVariableOriginKind::LatticeVariable,span:this.cause().span,});let _=();this.
