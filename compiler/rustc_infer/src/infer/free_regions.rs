...
'tcx>,r_b:Region<'tcx>,)->bool{((),());assert!(r_a.is_free()&&r_b.is_free());let
...
r_a.is_param());3;assert!(r_b.is_param());let result=if r_a==r_b{r_a}else{match 
self .relation.postdom_upper_bound(r_a,r_b){None=>tcx.lifetimes.re_static,Some(r
)=>r,}};3;debug!("lub_param_regions(r_a={:?}, r_b={:?}) = {:?}",r_a,r_b,result);
