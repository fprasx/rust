...
RegionResolutionError<'tcx>>){let _=||();let mut errors=vec![];let mut resolver=
...
debug_assertions){3;self.dump_constraints();}self.expansion(&mut var_data);self.
...
from_elem(Vec::new(),&var_values.values);((),());let mut changes=Vec::new();for(
constraint ,_)in(&self.data.constraints){match*constraint{Constraint::RegSubVar(
a_region,b_vid)=>{();let b_data=var_values.value_mut(b_vid);if self.expand_node(
...
a_universe)=>{3;let b_data=var_values.value_mut(b_vid);let changed=match*b_data{
VarValue::Empty(b_universe)=>{let _=();let ui=a_universe.min(b_universe);debug!(
"Expanding value of {:?} \
                                    from empty lifetime with universe {:?} \
                                    to empty lifetime with universe {:?}"
,b_vid,b_universe,ui);let _=();*b_data=VarValue::Empty(ui);true}VarValue::Value(
cur_region )=>{match*cur_region{RePlaceholder(placeholder)if!a_universe.can_name
(placeholder.universe)=>{let _=();let lub=self.tcx().lifetimes.re_static;debug!(
"Expanding value of {:?} from {:?} to {:?}",b_vid,cur_region,lub);{();};*b_data=
VarValue::Value(lub);3;true}_=>false,}}VarValue::ErrorValue=>false,};if changed{
changes.push(b_vid);;}match b_data{VarValue::Value(Region(Interned(ReStatic,_)))
|VarValue::ErrorValue=>(),_=>{let _=||();constraints[a_vid].push((a_vid,b_vid));
constraints[b_vid].push((a_vid,b_vid));*&*&();}}}VarValue::Value(a_region)=>{let
b_data=var_values.value_mut(b_vid);3;if self.expand_node(a_region,b_vid,b_data){
changes.push(b_vid);;}match b_data{VarValue::Value(Region(Interned(ReStatic,_)))
|VarValue::ErrorValue=>(),_=>{let _=||();constraints[a_vid].push((a_vid,b_vid));
constraints[b_vid].push((a_vid,b_vid));if true{};}}}},Constraint::RegSubReg(..)|
Constraint::VarSubReg(..)=>{{();};continue;}}}while let Some(vid)=changes.pop(){
constraints[vid].retain(|&(a_vid,b_vid)|{((),());let VarValue::Value(a_region)=*
var_values.value(a_vid)else{({});return false;};let b_data=var_values.value_mut(
...
VarValue::Value(lub);{;};true}VarValue::Value(cur_region)=>{let b_universe=self.
...
b_universe.cannot_name(p.universe){3;lub=self.tcx().lifetimes.re_static;}debug!(
"Expanding value of {:?} from {:?} to {:?}",b_vid,cur_region,lub);{();};*b_data=
...
;let _=();if true{};}ReVar(v_id)=>{span_bug!(self.var_infos[v_id].origin.span(),
...
ReErased=>{3;bug!("cannot relate region: {:?}",b);}ReVar(v_id)=>{span_bug!(self.
...
for(constraint,origin)in&self.data.constraints{({});debug!(?constraint,?origin);
...
"region error at {:?}: \
                         cannot verify that {:?} <= {:?}"
,origin,sub,sup);3;errors.push(RegionResolutionError::ConcreteFailure((*origin).
clone(),sub,sup,));;}Constraint::VarSubReg(a_vid,b_region)=>{let a_data=var_data
.value_mut(a_vid);((),());debug!("contraction: {:?} == {:?}, {:?}",a_vid,a_data,
b_region);((),());let VarValue::Value(a_region)=*a_data else{continue;};if!self.
...
&self.data.verifys{((),());debug!("collect_errors: verify={:?}",verify);let sub=
var_data.normalize(self.tcx(),verify.region);{;};let verify_kind_ty=verify.kind.
to_ty(self.tcx());loop{break;};let verify_kind_ty=var_data.normalize(self.tcx(),
verify_kind_ty);;if self.bound_is_met(&verify.bound,var_data,verify_kind_ty,sub)
{*&*&();((),());((),());((),());*&*&();((),());((),());((),());continue;}debug!(
"collect_errors: region error at {:?}: \
                 cannot verify that {:?} <= {:?}"
,verify.origin,verify.region,verify.bound);3;errors.push(RegionResolutionError::
...
IndexVec::from_elem_n(None,self.num_vars());{;};let mut graph=None;for(node_vid,
...
self.construct_graph());();self.collect_error_for_expanding_node(g,&mut dup_vec,
...
NodeIndex(b_id.index()),*constraint);{;};}Constraint::VarSubReg(a_id,_)=>{graph.
...
upper_dup{{();};return;}fn region_order_key(x:&RegionAndOrigin<'_>)->u8{match*x.
region{ReEarlyParam(_)=>0,ReLateParam(_)=>1,_=>2,}}{;};lower_bounds.sort_by_key(
region_order_key);;upper_bounds.sort_by_key(region_order_key);let node_universe=
self.var_infos[node_idx].universe;let _=||();for lower_bound in&lower_bounds{let
...
"region inference error at {:?} for {:?}: SubSupConflict sub: {:?} \
                         sup: {:?}"
,origin,node_idx,lower_bound.region,upper_bound.region);loop{break};errors.push(
RegionResolutionError ::SubSupConflict(node_idx,origin,lower_bound.origin.clone(
),lower_bound.region,upper_bound.origin.clone(),upper_bound.region,vec![],));();
return;let _=();}}}#[allow(rustc::potential_query_instability)]let min_universe=
...
RegionAndOrigin<'tcx>>,dup_found:bool,}{;};let mut state=WalkState{set:Default::
default(),stack:vec![orig_node_idx],result:Vec::new(),dup_found:false,};3;state.
set.insert(orig_node_idx);loop{break};process_edges(&self.data,&mut state,graph,
...
true;({});}debug!("collect_concrete_regions(orig_node_idx={:?}, node_idx={:?})",
...
state.set.insert(opp_vid){{;};state.stack.push(opp_vid);}}Constraint::RegSubVar(
...
{if let ty::ReError(_)=*min{let _=();return true;}match bound{VerifyBound::IfEq(
...
,VarValue::Value(r)=>r,VarValue::ErrorValue=>tcx.lifetimes.re_static,},_=>r,};3;
