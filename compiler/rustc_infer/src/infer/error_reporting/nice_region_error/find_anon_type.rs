...
>,&'tcx hir::FnSig<'tcx>)>{({});let anon_reg=tcx.is_suitable_region(region)?;let
...
"EarlyBound id={:?} def_id={:?}",id,def_id);3;if id==def_id{return ControlFlow::
...
current_index&&id==def_id{if true{};return ControlFlow::Break(arg);}}(Some(rbv::
...
debruijn_index,);if true{};debug!("id={:?}",id);debug!("def_id={:?}",def_id);if 
...
Error(_),)|None,_,)=>{({});debug!("no arg found");}}ControlFlow::Continue(())}fn
