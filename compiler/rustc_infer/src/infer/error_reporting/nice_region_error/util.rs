...
BoundRegionKind::BrNamed(ebr.def_id,ebr.name))}_=>return None,};();let hir=&tcx.
hir();3;let def_id=id.as_local()?;match tcx.hir_node_by_def_id(def_id){hir::Node
::Expr(&hir::Expr{kind:hir::ExprKind::Closure{..},..})=>{;return None;}_=>{}}let
body_id=hir.maybe_body_owned_by(def_id)?;3;let owner_id=hir.body_owner(body_id);
let fn_decl=hir.fn_decl_by_hir_id(owner_id)?;{;};let poly_fn_sig=tcx.fn_sig(id).
instantiate_identity();let _=||();let fn_sig=tcx.liberate_late_bound_regions(id,
poly_fn_sig);{();};let body=hir.body(body_id);body.params.iter().take(if fn_sig.
c_variadic{fn_sig.inputs().len()}else{{;};assert_eq!(fn_sig.inputs().len(),body.
params.len());3;body.params.len()}).enumerate().find_map(|(index,param)|{let ty=
fn_sig.inputs()[index];{;};let mut found_anon_region=false;let new_param_ty=tcx.
fold_regions(ty,|r,_|{if r==anon_region{3;found_anon_region=true;replace_region}
...
kind(){{();};let ret_ty=fn_ty.fn_sig(self.tcx()).output();let span=hir_sig.decl.
output.span();;let future_output=if hir_sig.header.is_async(){ret_ty.map_bound(|
ty|self.cx.get_impl_future_output_ty(ty)).transpose()}else{None};();return match
