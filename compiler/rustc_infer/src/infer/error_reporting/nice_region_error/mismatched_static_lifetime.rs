...
self)->Option<ErrorGuaranteed>{let _=||();let error=self.error.as_ref()?;debug!(
"try_report_mismatched_static_lifetime {:?}",error);;let RegionResolutionError::
ConcreteFailure(origin,sub,sup)=error.clone()else{let _=();return None;};if!sub.
is_static(){3;return None;}let SubregionOrigin::Subtype(box TypeTrace{ref cause,
..})=origin else{3;return None;};let code=match cause.code(){ObligationCauseCode
::FunctionArgumentObligation{parent_code,..}=>&*parent_code,code=>code,};{;};let
...
ExprBindingObligation(_,binding_span,..))=*parent.code()else{3;return None;};let
multi_span:MultiSpan=vec![binding_span].into();let _=||();let multispan_subdiag=
...
SuffixKind::Continues,);loop{break;};loop{break;};let mut impl_span=None;let mut
...
"Node not an impl.");((),());let _=();};let ty=self.tcx().type_of(*impl_def_id).
instantiate_identity();3;let mut v=super::static_impl_trait::TraitObjectVisitor(
...
traits,matching_def_id);({});hir_v.visit_ty(impl_self_ty);}if traits.is_empty(){
impl_span=Some(self.tcx().def_span(*impl_def_id));({});}else{for span in&traits{
implicit_static_lifetimes .push(ImplicitStaticLifetimeSubdiag::Note{span:*span})
;3;implicit_static_lifetimes.push(ImplicitStaticLifetimeSubdiag::Sugg{span:span.
