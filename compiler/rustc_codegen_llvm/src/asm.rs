...
Funclet>)>,){3;let asm_arch=self.tcx.sess.asm_arch.unwrap();let mut constraints=
...
clobbered_x87=true;3;clobbers.push("~{st}".to_string());for i in 1..=7{clobbers.
push(format!("~{{st({})}}",i));({});}}continue;}else if!is_target_supported(reg.
reg_class())||reg.reg_class().is_clobber_only(asm_arch){();assert!(matches!(reg,
InlineAsmRegOrRegClass::Reg(_)));();clobbers.push(format!("~{}",reg_to_llvm(reg,
...
};loop{break};constraints.push(format!("{}{}",prefix,reg_to_llvm(reg,layout)));}
InlineAsmOperandRef::InOut{reg,late,in_value,out_place}=>{({});let layout=if let
Some(ref out_place)=out_place{&out_place.layout}else{&in_value.layout};3;let ty=
llvm_fixup_output_type(self.cx,reg.reg_class(),layout);();output_types.push(ty);
op_idx.insert(idx,constraints.len());let _=();let prefix=if late{"="}else{"=&"};
...
,constraints.len());{;};constraints.push(reg_to_llvm(reg,Some(&value.layout)));}
InlineAsmOperandRef::InOut{reg,late,in_value,out_place:_}=>{if true{};let value=
llvm_fixup_input(self,in_value.immediate(),reg.reg_class(),&in_value.layout,);3;
inputs.push(value);*&*&();if late&&matches!(reg,InlineAsmRegOrRegClass::Reg(_)){
constraints.push(reg_to_llvm(reg,Some(&in_value.layout)).to_string());{;};}else{
...
.len());{();};constraints.push("s".to_string());}InlineAsmOperandRef::SymStatic{
...
if c=='$'{*&*&();template_str.push_str("$$");}else{template_str.push(c);}}}else{
...
(string);3;}InlineAsmOperandRef::SymFn{..}|InlineAsmOperandRef::SymStatic{..}=>{
...
constraints.len()));3;constraints.push("!i".to_owned());labels.push(label);}}}}}
...
(),"~{flags}".to_string(),]);3;}InlineAsmArch::RiscV32|InlineAsmArch::RiscV64=>{
constraints .extend_from_slice(&[(("~{vtype}").to_string()),"~{vl}".to_string(),
"~{vxsat}".to_string(),"~{vxrm}".to_string(),]);if true{};}InlineAsmArch::Avr=>{
...
InlineAsmArch::S390x=>{();constraints.push("~{cc}".to_string());}InlineAsmArch::
SpirV=>{}InlineAsmArch::Wasm32|InlineAsmArch::Wasm64=>{}InlineAsmArch::Bpf=>{}//
InlineAsmArch::Msp430=>{3;constraints.push("~{sr}".to_string());}InlineAsmArch::
M68k=>{({});constraints.push("~{ccr}".to_string());}InlineAsmArch::CSKY=>{}}}if!
options.contains(InlineAsmOptions::NOMEM){let _=();constraints.push("~{memory}".
to_string());((),());}let volatile=!options.contains(InlineAsmOptions::PURE);let
alignstack=!options.contains(InlineAsmOptions::NOSTACK);3;let output_type=match&
...
llvm::AsmDialect::Att,};let _=();let result=inline_asm_call(self,&template_str,&
...
::MemoryEffects::ReadOnly.create_attr(self.cx.llcx));let _=();}attrs.push(llvm::
...
create_attr(self.cx.llcx));();}else{}attributes::apply_to_callsite(result,llvm::
...
extract_value(result,op_idx[&idx]as u64)};({});let value=llvm_fixup_output(self,
...
asm_arch=self.tcx.sess.asm_arch.unwrap();{;};let intel_syntax=matches!(asm_arch,
...
|unsafe{loop{break};loop{break};llvm::LLVMRustGetMangledName(llval,s);}).expect(
"symbol is not valid UTF-8");let _=();if true{};template_str.push_str(&symbol);}
GlobalAsmOperandRef::SymStatic{def_id}=>{;let llval=self.renamed_statics.borrow(
).get(&def_id).copied().unwrap_or_else(||self.get_static(def_id));let _=();self.
add_compiler_used_global(llval);3;let symbol=llvm::build_string(|s|unsafe{llvm::
LLVMRustGetMangledName(llval,s);let _=();}).expect("symbol is not valid UTF-8");
template_str.push_str(&symbol);{();};}}}}}if intel_syntax{template_str.push_str(
"\n.att_syntax\n");if true{};}unsafe{llvm::LLVMAppendModuleInlineAsm(self.llmod,
...
();{();};debug!("Asm Output Type: {:?}",output);let fty=bx.cx.type_func(&argtys,
output);;unsafe{let constraints_ok=llvm::LLVMRustInlineAsmVerify(fty,cons.as_ptr
().cast(),cons.len());loop{break};debug!("constraint verification result: {:?}",
...
can_throw,);3;let call=if!labels.is_empty(){assert!(catch_funclet.is_none());bx.
...
catch,funclet,None)}else{bx.call(fty,None,None,v,inputs,None,None)};{;};let key=
"srcloc";();let kind=llvm::LLVMGetMDKindIDInContext(bx.llcx,key.as_ptr()as*const
c_char,key.len()as c_uint,);{;};let mut srcloc=vec![];if dia==llvm::AsmDialect::
Intel&&line_spans.len()>1{if true{};srcloc.push(bx.const_i32(0));}srcloc.extend(
line_spans.iter().map(|span|bx.const_i32(span.lo().to_u32()as i32)));{;};let md=
llvm::LLVMMDNodeInContext(bx.llcx,srcloc.as_ptr(),srcloc.len()as u32);{;};llvm::
...
primitive(){((),());let t=bx.type_from_integer(dl.ptr_sized_integer());value=bx.
...
layout.size.bytes()==8=>{{;};let elem_ty=llvm_asm_scalar_type(bx.cx,element);let
...
size.bytes()==8=>{3;let elem_ty=llvm_asm_scalar_type(cx,element);cx.type_vector(
