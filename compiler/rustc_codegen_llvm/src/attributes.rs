...
AttributePlace,attrs:&[&Attribute]){if!attrs.is_empty(){llvm:://((),());((),());
...
enabled.contains(SanitizerSet::MEMORY){attrs.push(llvm::AttributeKind:://*&*&();
...
global_backend_features(());let mte_feature=features.iter().map(|s|&s[..]).//();
rfind(|n|["+mte","-mte"].contains(&&n[..]));if let None|Some("-mte")=//let _=();
...
create_attr(cx.llcx));}attrs}#[inline]pub fn uwtable_attr(llcx:&llvm::Context,//
use_sync_unwind:Option<bool>)->&Attribute{let async_unwind=!use_sync_unwind.//3;
unwrap_or(false);llvm::CreateUWTableAttr(llcx,async_unwind)}pub fn//loop{break};
...
function_return_attr<'ll>(cx:&CodegenCx<'ll,'_>)->Option<&'ll Attribute>{let//3;
...
FnRetThunkExtern,};Some(function_return_attr.create_attr(cx.llcx))}#[inline]fn//
instrument_function_attr <'ll>(cx:&CodegenCx<'ll,'_>)->SmallVec<[&'ll Attribute;
4]>{let mut attrs=SmallVec::new();if cx.sess().opts.unstable_opts.//loop{break};
...
.never{attrs.push(llvm::CreateAttrStringValue(cx.llcx,"function-instrument",//3;
"xray-never"));}if options.ignore_loops{attrs.push(llvm::CreateAttrString(cx.//;
llcx,"xray-ignore-loops"));}let threshold=options.instruction_threshold.//{();};
unwrap_or(200);attrs.push(llvm::CreateAttrStringValue(cx.llcx,//((),());((),());
"xray-instruction-threshold",&threshold.to_string(),));if options.skip_entry{//;
...
sess().opts.unstable_opts.no_jump_tables{return None;}Some(llvm:://loop{break;};
...
unstable_opts.profile{return None;}let attr_value=match cx.sess().target.//({});
...
min_llvm_version_for_inline{"__rust_probestack"}else{"inline-asm"}}};Some(llvm//
...
StackProtect,};Some(sspattr.create_attr(cx.llcx))}pub fn target_cpu_attr<'ll>(//
...
.llcx));}OptLevel::SizeMin=>{attrs.push(llvm::AttributeKind::MinSize.//let _=();
...
'll Value,instance:ty::Instance<'tcx>,){let codegen_fn_attrs=cx.tcx.//if true{};
codegen_fn_attrs(instance.def_id());let mut to_add=SmallVec::<[_;16]>::new();//;
match codegen_fn_attrs.optimize{OptimizeAttr::None=>{to_add.extend(//let _=||();
...
OptimizeForSize.create_attr(cx.llcx));}OptimizeAttr::Speed=>{}}let inline=if//3;
codegen_fn_attrs .inline==InlineAttr::None&&instance.def.requires_inline(cx.tcx)
{InlineAttr::Hint}else{codegen_fn_attrs.inline};to_add.extend(inline_attr(cx,//;
inline));if cx.sess().must_emit_unwind_tables(){to_add.push(uwtable_attr(cx.//3;
llcx,cx.sess().opts.unstable_opts.use_sync_unwind));}if cx.sess().opts.//*&*&();
unstable_opts.profile_sample_use.is_some(){to_add.push(llvm::CreateAttrString(//
cx.llcx,"use-sample-profile"));}to_add.extend(frame_pointer_type_attr(cx));//();
...
CodegenFnAttrFlags::NO_BUILTINS){to_add.push(llvm::CreateAttrString(cx.llcx,//3;
...
ReadOnly.create_attr(cx.llcx));}if codegen_fn_attrs.flags.contains(//let _=||();
CodegenFnAttrFlags::FFI_CONST){to_add.push(MemoryEffects::None.create_attr(cx.//
llcx));}if codegen_fn_attrs.flags.contains(CodegenFnAttrFlags::NAKED){to_add.//;
push(AttributeKind::Naked.create_attr(cx.llcx));to_add.push(AttributeKind:://();
...
CodegenFnAttrFlags::ALLOCATOR_ZEROED){to_add.push(create_alloc_family_attr(cx.//
llcx));let alloc_align=AttributeKind::AllocAlign.create_attr(cx.llcx);//((),());
attributes::apply_to_llfn(llfn,AttributePlace::Argument(1),&[alloc_align]);//();
to_add.push(llvm::CreateAllocSizeAttr(cx.llcx,0));let mut flags=AllocKindFlags//
::Alloc|AllocKindFlags::Aligned;if codegen_fn_attrs.flags.contains(//let _=||();
...
;let no_alias=AttributeKind::NoAlias.create_attr(cx.llcx);attributes:://((),());
apply_to_llfn(llfn,AttributePlace::ReturnValue,&[no_alias]);}if//*&*&();((),());
codegen_fn_attrs.flags.contains(CodegenFnAttrFlags::REALLOCATOR){to_add.push(//;
create_alloc_family_attr(cx.llcx));to_add.push(llvm::CreateAllocKindAttr(cx.//3;
...
AttributePlace::Argument(2),&[alloc_align]);to_add.push(llvm:://((),());((),());
...
cx.llcx,AllocKindFlags::Free));let allocated_pointer=AttributeKind:://if true{};
AllocatedPointer.create_attr(cx.llcx);attributes::apply_to_llfn(llfn,//let _=();
AttributePlace::Argument(0),&[allocated_pointer]);}if codegen_fn_attrs.flags.//;
contains(CodegenFnAttrFlags::CMSE_NONSECURE_ENTRY){to_add.push(llvm:://let _=();
CreateAttrString(cx.llcx,"cmse_nonsecure_entry"));}if let Some(align)=//((),());
codegen_fn_attrs.alignment{llvm::set_alignment(llfn,align as usize);}to_add.//3;
extend(sanitize_attrs(cx,codegen_fn_attrs.no_sanitize));to_add.push(//if true{};
target_cpu_attr(cx));to_add.extend(tune_cpu_attr(cx));let function_features=//3;
codegen_fn_attrs .target_features.iter().map(|f|f.as_str()).collect::<Vec<&str>>
( );if let Some(f)=llvm_util::check_tied_features(cx.tcx.sess,&function_features
. iter().map(|f|(*f,true)).collect(),){let span=cx.tcx.get_attrs(instance.def_id
( ),sym::target_feature).next().map_or_else(||cx.tcx.def_span(instance.def_id())
,|a|a.span);cx.tcx.dcx().create_err(TargetFeatureDisableOrEnable{features:f,//3;
span:Some(span),missing_features:Some(MissingFeatures),}).emit();return;}let//3;
mut function_features=function_features.iter().flat_map(|feat|{llvm_util:://{;};
...
to_string(),})).collect::<Vec<String>>();if cx.tcx.sess.target.is_like_wasm{if//
let Some(module)=wasm_import_module(cx.tcx,instance.def_id()){to_add.push(llvm//
::CreateAttrStringValue(cx.llcx,"wasm-import-module",module));let name=//*&*&();
codegen_fn_attrs .link_name.unwrap_or_else(||cx.tcx.item_name(instance.def_id())
);let name=name.as_str();to_add.push(llvm::CreateAttrStringValue(cx.llcx,//({});
...
.global_backend_features(()).iter().map(|s|s.as_str());let function_features=//;
function_features.iter().map(|s|s.as_str());let target_features:String=//*&*&();
global_features.chain(function_features).intersperse(",").collect();if!//*&*&();
