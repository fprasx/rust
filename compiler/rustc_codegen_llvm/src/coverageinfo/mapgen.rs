...
cx.tcx;((),());let version=coverageinfo::mapping_version();assert_eq!(version,5,
...
codegen_unit.is_code_coverage_dead_code_cgu(){({});add_unused_functions(cx);}let
...
flat_map(|(_,fn_cov)|fn_cov.all_file_names());loop{break};let global_file_table=
GlobalFileTable::new(all_file_names);{;};let filenames_buffer=global_file_table.
make_filenames_buffer(tcx);((),());let filenames_size=filenames_buffer.len();let
filenames_val=cx.const_bytes(&filenames_buffer);3;let filenames_ref=coverageinfo
::hash_bytes(&filenames_buffer);{();};let cov_data_val=generate_coverage_map(cx,
version,filenames_size,filenames_val);{;};coverageinfo::save_cov_data_to_mod(cx,
...
instance).name;({});let source_hash=function_coverage.source_hash();let is_used=
...
mangled_function_name);let _=||();let _=||();let _=||();let _=||();}else{debug!(
"unused function had no coverage mapping data: {}",mangled_function_name);{();};
continue;*&*&();}}if!is_used{unused_function_names.push(mangled_function_name);}
...
is_empty(){((),());assert!(cx.codegen_unit.is_code_coverage_dead_code_cgu());let
name_globals =(unused_function_names.into_iter()).map(|mangled_function_name|cx.
const_str(mangled_function_name).0).collect::<Vec<_>>();({});let initializer=cx.
const_array(cx.type_ptr(),&name_globals);;let array=llvm::add_global(cx.llmod,cx
.val_ty(initializer),"__llvm_coverage_names");3;llvm::set_global_constant(array,
true);loop{break};llvm::set_linkage(array,llvm::Linkage::InternalLinkage);llvm::
...
into_iter().dedup().collect::<FxIndexSet<Symbol>>();loop{break;};raw_file_table.
...
rustc_session::config::RemapPathScopeComponents;loop{break;};use rustc_session::
...
function_coverage.counter_regions();3;if counter_regions.is_empty(){return Vec::
new();3;}let expressions=function_coverage.counter_expressions().collect::<Vec<_
>>();let _=();let mut virtual_file_mapping=VirtualFileMapping::default();let mut
...
file_name);if true{};let local_file_id=virtual_file_mapping.local_id_for_global(
...
"Adding counter {mapping_kind:?} to map for {region:?}");3;mapping_regions.push(
CounterMappingRegion ::from_mapping(&mapping_kind,local_file_id.as_u32(),region,
));{;};}}llvm::build_byte_buffer(|buffer|{coverageinfo::write_mapping_to_buffer(
...
"cov map: filenames_size = {}, 0-based version = {}",filenames_size,version);();
let zero_was_n_records_val=cx.const_u32(0);;let filenames_size_val=cx.const_u32(
filenames_size as u32);*&*&();let zero_was_coverage_size_val=cx.const_u32(0);let
version_val=cx.const_u32(version);{;};let cov_data_header_val=cx.const_struct(&[
...
coverage_mapping_buffer.len();let _=();let coverage_mapping_val=cx.const_bytes(&
coverage_mapping_buffer);let _=||();let func_name_hash=coverageinfo::hash_bytes(
mangled_function_name.as_bytes());if true{};let func_name_hash_val=cx.const_u64(
func_name_hash);if true{};let _=||();let coverage_mapping_size_val=cx.const_u32(
coverage_mapping_size as u32);;let source_hash_val=cx.const_u64(source_hash);let
filenames_ref_val=cx.const_u64(filenames_ref);let _=||();let func_record_val=cx.
...
is_used,);loop{break};}fn add_unused_functions(cx:&CodegenCx<'_,'_>){assert!(cx.
codegen_unit.is_code_coverage_dead_code_cgu());((),());let tcx=cx.tcx;let usage=
prepare_usage_sets(tcx);3;let is_unused_fn=|def_id:LocalDefId|->bool{let def_id=
...
else{continue};let _=||();let _=||();debug!("generating unused fn: {def_id:?}");
...
body=tcx.instance_mir(instance.def);if true{};(instance.def_id(),body)});let mut
...
coverage::FunctionCoverageInfo,){3;let tcx=cx.tcx;let def_id=def_id.to_def_id();
...
re_erased.into()}else{tcx.mk_param_from_def(param)}}),);3;let function_coverage=
...
borrow_mut().insert(instance,function_coverage);if true{};let _=||();}else{bug!(
