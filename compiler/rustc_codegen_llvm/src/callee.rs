...
;3;debug!("get_fn(instance={:?})",instance);assert!(!instance.args.has_infer());
...
).name;({});debug!("get_fn({:?}: {:?}) => {}",instance,instance.ty(cx.tcx(),ty::
ParamEnv::reveal_all()),sym);;let fn_abi=cx.fn_abi_of_instance(instance,ty::List
::empty());3;let llfn=if let Some(llfn)=cx.get_declared_value(sym){llfn}else{let
instance_def_id=instance.def_id();3;let llfn=if tcx.sess.target.arch=="x86"&&let
...
);3;attributes::from_fn_attrs(cx,llfn,instance);unsafe{llvm::LLVMRustSetLinkage(
llfn,llvm::Linkage::ExternalLinkage);if let _=(){};let is_generic=instance.args.
...
}llfn};if true{};if true{};cx.instances.borrow_mut().insert(instance,llfn);llfn}
