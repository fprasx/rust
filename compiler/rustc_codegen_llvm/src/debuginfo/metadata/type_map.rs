...
normalize_erasing_regions(ParamEnv::reveal_all(),self_type));3;debug_assert_eq!(
...
let mut hasher=StableHasher::new();();tcx.with_stable_hashing_context(|mut hcx|{
...
create_DIArray(DIB(cx),&[]);if let _=(){};let unique_type_id_str=unique_type_id.
generate_unique_id_string(cx.tcx);();let metadata=match kind{Stub::Struct|Stub::
...
None);({});debug_context(cx).type_map.insert(stub_info.unique_type_id,stub_info.
metadata);();let members:SmallVec<_>=members(cx,stub_info.metadata).into_iter().
map(|node|Some(node)).collect();({});let generics:SmallVec<Option<&'ll DIType>>=
...
[..]);let _=||();let generics_array=create_DIArray(DIB(cx),&generics[..]);llvm::
