...
let enum_type_and_layout=cx.layout_of(enum_type);loop{break};let enum_type_name=
compute_debuginfo_type_name(cx.tcx,enum_type,false);*&*&();let visibility_flags=
visibility_di_flags(cx,enum_adt_def.did(),enum_adt_def.did());();debug_assert!(!
...
DINodeCreationResult<'ll>{3;let coroutine_type=unique_type_id.expect_ty();let&ty
...
as_coroutine().kind_ty()).unwrap();let _=();let Variants::Multiple{tag_encoding:
...
{*&*&();let variant_name=format!("{}",variant_index.as_usize()).into();let span=
coroutine_layout.variant_source_info[variant_index].span;{;};let source_info=if!
...
_=>"",};if let _=(){};let containing_scope=enum_or_coroutine_type_di_node;match 
...
compute_discriminant_value(cx,enum_type_and_layout,variant_index);if true{};let(
