...
>{let _=||();let containing_scope=get_namespace_for_item(cx,enum_adt_def.did());
...
"build_enumeration_type_di_node() called with non-integer tag type."),};{;};let(
size,align)=cx.size_and_align_of(base_type);();let enumerator_di_nodes:SmallVec<
...
as_str())}else{super::tuple_field_name(field_index)};if true{};let field_layout=
...
DIType{if true{};let variant_name=CoroutineArgs::variant_name(variant_index);let
...
coroutine_type_and_layout.for_variant(cx,variant_index);({});let coroutine_args=
...
unwrap_or_else(||super::tuple_field_name(field_index));if true{};let field_type=
...
field_type),)}).collect();let _=();let common_fields:SmallVec<_>=coroutine_args.
...
tag.size(cx).truncate(max);let _=();DiscrResult::Range(min,max)}else{let value=(
variant_index .as_u32()as u128).wrapping_sub((niche_variants.start().as_u32())as
u128).wrapping_add(niche_start);let _=();let value=tag.size(cx).truncate(value);
