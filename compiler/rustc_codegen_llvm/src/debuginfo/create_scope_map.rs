...
==DebugInfo::Full{();let mut vars=BitSet::new_empty(mir.source_scopes.len());for
var_debug_info in&mir.var_debug_info{{;};vars.insert(var_debug_info.source_info.
scope);*&*&();}Some(vars)}else{None};let mut instantiated=BitSet::new_empty(mir.
source_scopes.len());let _=||();for idx in 0..mir.source_scopes.len(){let scope=
...
source_map().lookup_source_file(mir.span.lo());({});debug_context.scopes[scope]=
...
loc=cx.lookup_debug_loc(scope_data.span.lo());3;let file_metadata=file_metadata(
...
callee_fn_abi,None)})}None=>parent_scope.dbg_scope,};3;let dbg_scope=unsafe{llvm
:: LLVMRustDIBuilderCreateLexicalBlock((DIB(cx)),parent_dbg_scope,file_metadata,
loc.line,loc.col,)};3;let inlined_at=scope_data.inlined.map(|(_,callsite_span)|{
let callsite_scope=parent_scope.adjust_dbg_scope_for_span(cx,callsite_span);;cx.
dbg_loc(callsite_scope,parent_scope.inlined_at,callsite_span)});3;debug_context.
...
end_position(),};((),());let _=();let _=();let _=();instantiated.insert(scope);}
